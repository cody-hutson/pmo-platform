...
- [work-item-type-consumer-map.md](work-item-type-consumer-map.md) — every surface that reads, creates, updates or deletes a work-item kind: the read paths, the create/update/delete paths and their execution loci, the flows leaving the repository, the first- and second-order impact of relaxing the archetype binding, the named breakages, and the instrument blind spots that bound the enumeration.
