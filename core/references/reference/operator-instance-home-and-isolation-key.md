...
**Direction, unchanged and ratified: the writer is what changes.** Configuration belongs in the XDG config root under §2.1. The deviating surface is the *writer*, not the reader, so pointing the resolver at the operator-instance home is rejected — it would move configuration out of XDG config and contradict both the running behavior and the distribution ADR, which is the direction fork 0a exists to settle.

**Mechanism, corrected against the measured read-path.** This section originally selected *"the manifest gains a config tier"* and priced the two directions in a cost table. Both the pricing and the mechanism rested on a read-path this record had not measured, and the divergence card's Stage-5 analysis measured it. Two corrections follow, and the second is a safety finding rather than a cost revision:

1. **Rung 1 already reads the in-repo template.** The resolver locates `core/config/platform-config.toml.template` from the clone and reads it at the global rung. The substantive Layer-1 body therefore already reaches every reader, and no installed copy is needed to deliver it. What the composition manifest was actually writing was a further full copy of that template into the operator-instance home that **no code path read** — an orphan write target shipping an empty OPERATOR ADDITIONS fence that invited edits taking no effect. The divergence is closed by **removing** that write, not by relocating it: the writer still changes, but the correct change is deregistration, not a fifth destination tier.
