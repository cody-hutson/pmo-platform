...
| **Relationship to existing fields** | Composes with `folder` (§ Category 6 — the current folder) and `promoted_from` (§ Domain C — promotion provenance). Consistency rule: `promotion_state: promoted ⇒ folder ≠ 08-generated` (the rule artifact-lint Check 4 already enforces, now expressed on a dedicated field instead of inferred from `artifact_state: PROMOTED`). |
