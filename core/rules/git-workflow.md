...
| Issue-reference validity | Every changed markdown file, minus a single path exemption: everything under the release-tracking surface `release/releases/*` — the release log, index, digest, notes and plans, where issue and pull-request references are native provenance rather than prose citations, mirroring the reference-durability gate's posture on the same surface. That surface is exempt by path because its basis is provenance rather than a marker, and only a minority of its files carry one. This rules file and the PR template are skipped too, but they earn it the ordinary way — each carries the whole-file override marker in the next column, which the gate re-checks on every run. The gate names neither of them in its source, deliberately: an enumerated exemption outlives the justification its own comment claims (strip the marker and the file stays silently exempt forever), while a discovered one cannot. | An issue reference (`#N`, or the deprecated `IMP-NNN`) that does not resolve to a real issue IN THIS REPO (checked by direct API resolution — a 404, a redirect to another repo, or a pull-request number all fail), OR a resolving reference placed outside a designated reference block (`Issue References` / `References` / `Related` / `Provenance` / `Source` / `Sources` / `Source(s)`, at any heading level 1–6). | `<!-- repo-integrity: allow-issue-ref — limb N: rationale -->` (declared shape — see § Declaring an issue-ref override below) |
...
**Declaring an issue-ref override.** The `repo-integrity: allow-issue-ref` marker is warranted only when BOTH limbs hold: (1) the file DISPLAYS an issue-reference construct as its subject matter — self-documentation, a template, a worked example, a test fixture — or its numbers are synthetic and cannot resolve by construction; AND (2) neither remedy is available, because the reference can be neither relocated into a designated reference block nor replaced by an inline summary without destroying what the file is for. A marker declared under that criterion **carries a trailing rationale on the same line, inside the comment, naming which limb applies** — the gate accepts `<!-- repo-integrity: allow-issue-ref — limb 1: synthetic fixture ids, not repo issues -->` as readily as the bare form. The bare marker still suppresses, but it is a silenced warning rather than a declaration: a reviewer cannot tell an examined exemption from an unexamined one without the rationale. The obligation is stated once, for every marker class, in [`core/standards/adr-authoring-guide.md`](/core/standards/adr-authoring-guide.md) § Declaration obligation; this row is its projection onto the issue-reference gate. The trailing rationale may not contain a literal `>`, which would close the comment early — reword rather than escape it.
