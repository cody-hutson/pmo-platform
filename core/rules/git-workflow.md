...
**ADR `deciders:` carve-out.** The operator's literal name is permitted on a frontmatter `deciders:` line inside an ADR (`core/ADRs/**`, `release/ADRs/**`) — architect-of-record attribution on an owned repo. The exemption is field-scoped: it fires only when the changed file is an ADR **and** the added line matches `^deciders:` (both ANDed). It does not loosen the operator email, home/workspace paths, the handle on any non-`deciders` line or non-ADR file, ADR body lines, non-ADR files, or the collaborator dimension — all of those stay blocked. Exempted lines are reported in the run summary (via the same suppression channel as the line-scoped override), so the exemption is auditable, not silent. See [`core/standards/depersonalization-spec.md`](../standards/depersonalization-spec.md) §1.2.

**Non-blocking integrity jobs.** The same workflow carries additional jobs that report without blocking, so a new invariant can be calibrated against real traffic before it gates a merge. Two apply to ADR files. The **ADR-number integrity** job asserts the one global, gap-free `ADR-NNN` sequence across both ADR directories and blocks on a duplicate or a gap — it is whole-tree, because numbering is a global invariant. The **ADR durability** job is its lifecycle sibling: it lints ADR files changed in the PR, on added lines only, for three rules — a `status:` value whose leading token falls outside the four-token Nygard enum; a hardcoded commit SHA or a live corpus-population count baked into durable prose; and the operator's literal GitHub handle. The handle rule is deliberately stricter than the `deciders:` carve-out above: that carve-out sanctions the literal *name* on a `deciders:` line, never the *handle*, and this lint closes the gap a line-scoped skip would otherwise leave open. Fenced code blocks, the `source_observations:` frontmatter block, explicitly history-anchored lines, and frozen `Superseded`/`Deprecated` records are exempt from the stale-anchor rule by construction; a file that genuinely needs a pinned anchor declares `<!-- adr-durability: allow-anchor -->`, and `<!-- repo-integrity: allow-adr-durability -->` skips the file entirely. Both the operator handle and the repository it is derived from are resolved at run time, never written into the checker.
