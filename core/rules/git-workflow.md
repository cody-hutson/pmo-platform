...
| Issue-reference validity | Every changed markdown file, minus three exemptions: this file and the PR template, which quote worked examples; and everything under the release-tracking surface `release/releases/*` — the release log, index, digest, notes and plans, where issue and pull-request references are native provenance rather than prose citations, mirroring the reference-durability gate's posture on the same surface. | An issue reference (`#N`, or the deprecated `IMP-NNN`) that does not resolve to a real issue IN THIS REPO (checked by direct API resolution — a 404, a redirect to another repo, or a pull-request number all fail), OR a resolving reference placed outside a designated reference block (`Issue References` / `References` / `Related` / `Provenance` / `Source` / `Sources` / `Source(s)`, at any heading level 1–6). | `<!-- repo-integrity: allow-issue-ref — limb N: rationale -->` (declared shape — see § Declaring an issue-ref override below) |
