...
|---|---|---|---|---|
| **DTA-1** | About to report a **zero / clean / absent / N-of-M** result produced by a probe you ran | [`review-discipline-principles.md`](/core/disciplines/review-discipline-principles.md) § 1 Rule 15 + § 8 (`PV-0`..`PV-6`) | Run the **sensitivity arm** in the same invocation shape and record its observed non-zero result; state the denominator and how it was obtained. A zero whose control arm also returned zero is a broken probe, not a clean result. | transient/ambiguous-state reads |
| **DTA-2** | About to rely on a **delegated agent's, subagent's, or tool's self-reported summary** as evidence | [`review-discipline-principles.md`](/core/disciplines/review-discipline-principles.md) § 8 `PV-3` (extraction) + § 7(d) (no self-referential validation) | Read the **underlying artifact** the summary describes, at its canonical home. A summary is a pointer to evidence, never the evidence. | close-out · planning |
...
| **DTA-4** | About to **recommend** work derived from a point-in-time artifact — an analysis, an audit, a snapshot, a prior-session note, a summary, or recalled framing | [`decision-discipline.md`](/core/disciplines/decision-discipline.md) § 2.1.1 (Audit-Snapshot Reconciliation) + [`CLAUDE.md.template`](/core/CLAUDE.md.template) § Universal Preferences — *Verify before recommend* | Run § 2.1.1's verification primitives 1 + 2 (and 3 when the recommendation names a specific symbol or section). | planning |
| **DTA-5** | About to report a write or any externally-observable state mutation as **done** | [`CLAUDE.md.template`](/core/CLAUDE.md.template) § Quality Standards — *Write-first-speak-second* | **Read back the mutated state.** Report the observed post-state, not the intended one. | close-out |
| **DTA-6** | About to **create a new entry — or add a new durable claim to an existing entry — in the operator auto-memory store**, when the operator did not ask for it in this turn | [`memory-architecture.md`](/core/disciplines/memory-architecture.md) § 5 write rule 4 (*No unrequested save.*) | **Search the corpus for an existing governed home for the claim**, and state what the search returned. A governed home exists → do not save; the corpus is SSOT and a memory copy is a shadow SSOT. A home is in flight → save only as an eviction-pointer to it. No home **and** the claim is durable (still true and useful months from now) **and** non-obvious (hard to re-derive) → save, stating that justification. Anything else → **do not save**. | close-out |
...
| **DTA-10** | About to **conclude that a comment is warranted on an external seam** — a human-facing system the platform integrates with, its work-item records, its knowledge pages, their comment streams — **or that a field there is safe to write**, where the ground of that conclusion is your own draft, your own derivation, or a summary of the record rather than a present read of the destination itself | [`external-seam-conduct.md`](/core/disciplines/external-seam-conduct.md) | **Read the destination surface itself** — the record's own body and fields, and its comment stream — and establish three things from that read: (a) which of the two the system treats as **state**, and which as conversation; (b) for a comment, **which of § 1's two limbs it satisfies** (it addresses a person; it carries an ask or an answer) — a comment satisfying neither is a register entry and belongs in the record; (c) for a field write, the **authorship of the text being replaced** (§ 4) and the field's class against § 5's never-write categories as resolved from operator config. Authorship the read cannot establish is human-authored. | close-out · planning |

The **Locus** column records where a checkpoint's failures cluster, and it governs **admission to this index — not whether a checkpoint fires.** Firing is determined by § 1 and the row's own trigger signature alone: a claim matching a trigger carries its checkpoint wherever it arises, including outside the loci listed. What the locus bounds is index *growth* — § 6 makes it one of the four fields a discipline must declare to become a checkpoint at all, and a discipline with no recurring locus is not one. A proposed row therefore cannot enter by resemblance to an existing row's subject matter; it enters by declaring its own four fields. Every row's **Governing rule** points outward to that rule's own home, with one stated exception: `DTA-8`'s governing rule is § 8 of this file, because that discipline has no prior home in the corpus. When it acquires one, the cell repoints and no content moves.

**The ratified locus set.** Five loci are in force, ratified in full by the workspace owner at this release's Stage-9 review: **close-out**, **discovery**, **operator-pushback turn**, **planning**, and **transient/ambiguous-state reads**. Two of them — `operator-pushback turn` and `discovery` — arrived with checkpoints added during this release rather than with the original three, so the set is recorded here explicitly rather than left to be inferred by reading the column. A sixth locus is admitted only as part of a checkpoint that satisfies the § 6 declaration contract; it is never added on its own.
...
A checkpoint that fired and emitted nothing is indistinguishable from a checkpoint that never fired. The token is what makes adherence reviewable — the same property [`review-discipline-principles.md`](/core/disciplines/review-discipline-principles.md) § 8 `PV-6` requires of a mechanized check.
...
- **The mechanism exemptions in [`decision-discipline.md`](/core/disciplines/decision-discipline.md) § 3.** Those remain exempt from the M1/M2/M3 Decision-Briefing mechanisms and this rule does not re-open them. This rule binds the **claim**; § 3 governs the **briefing**. They are different objects.
...
- Read contradicts the claim, or the read cannot be performed → the signal is `DIVERGENT` or `UNGROUNDED` → enter [`decision-confidence-protocol.md`](/core/specs/decision-confidence-protocol.md) § 2's threshold matrix and, where it routes there, its § 3 bounded pause-to-learn loop with exits `E1`–`E4`.
...
|---|---|
| [`decision-confidence-protocol.md`](/core/specs/decision-confidence-protocol.md) | Owns the mechanism (signal / threshold / loop / guard). This rule is its § 1.0 trigger layer, deployed. |
| [`decision-discipline.md`](/core/disciplines/decision-discipline.md) § 3 | Governs the Decision **Briefing**. This rule governs the **claim**. Its exemptions stand unchanged. |
| [`review-discipline-principles.md`](/core/disciplines/review-discipline-principles.md) § 1 Rule 15 + § 8 | The governing rule for `DTA-1` / `DTA-2`. Cited, never restated — `PV-0`..`PV-6` live there. |
| [`reconcile-dont-annotate.md`](/core/disciplines/reconcile-dont-annotate.md) | The edit-time twin of `DTA-4`: verify before you recommend; reconcile when you are already editing. |
| [`autonomous-execution-model.md`](/core/disciplines/autonomous-execution-model.md) | Where a `DIVERGENT` read escalates (§ 5). |
