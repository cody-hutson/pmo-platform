...
| BLOCK-DESTRUCTIVE-022 | Bash subprocess script execution not in `.claude/script-execution-allowlist.txt` — three arms: `bash`/`sh`/`zsh` (interpreter), `source`/`.` (sourced file), and a slash-bearing command word (direct execution, rollout phase `warn`) |
