...
>
> **The exec arm's discriminator is the shell's own, and this is why it is a slash and not a suffix or an exec bit.** POSIX Shell Command Language § 2.9.1.1: a command name containing at least one slash is used directly as a pathname; otherwise the shell performs a `PATH` search. So a slash-bearing command word *is* the shell's own execute-this-file test, and every `PATH`-resolved utility (`ls`, `git`, `grep`) is excluded by construction rather than by enumeration. The five canonical system-bin prefixes are then excluded explicitly, reusing `ANCHOR_PREFIX_BASH`'s shipped prefix set so the file keeps one definition of "system bin".
>
