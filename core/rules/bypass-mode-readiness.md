...
| BLOCK-DESTRUCTIVE-022 | Bash subprocess script execution not in `.claude/script-execution-allowlist.txt` — three arms: `bash`/`sh`/`zsh` (interpreter), `source`/`.` (sourced file), and a slash-bearing command word (direct execution, rollout phase `warn`). **Parse-only invocations are exempt:** a bare `-n` under `bash`/`sh`/`zsh` with no `-c` in the flag span executes nothing and is not adjudicated. The exemption is keyed on the interpreter (every other interpreter hits an explicit documented null), and clusters (`-nx`) plus `-o noexec` are declared residuals that still block — see the parse-only paragraphs above |
