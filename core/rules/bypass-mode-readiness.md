...
> **What BLOCK-DESTRUCTIVE-022 resolves, and what it cannot.** Given all four conditions above, the rule adjudicates **the file that executes**, through **three arms of one matcher**: the first non-flag operand of each command whose command word is `bash`, `sh` or `zsh` (the *interpreter* arm) or `source` / `.` (the *source* arm), and the **command word itself** when it contains a slash (the *exec* arm). All three run through one matcher, one command-position walk, one token normalization and one adjudicator, so the guarantees below hold identically across them. Sourcing executes the file's contents in the current shell, and direct execution executes the file outright; both are the same execution capability an interpreter invocation is, not lesser ones.
>
