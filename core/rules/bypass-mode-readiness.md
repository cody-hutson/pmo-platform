...
> 2. **Prefix forms whose head token is itself a command word** — `env`, `command`, `exec`, `nohup`, `timeout`, `xargs`, `eval` — are not skipped when resolving command position, because under the shell grammar they are commands rather than prefixes. Closing that class requires a denylist of evaluating verbs, and a denylist inside a fail-closed control is itself a fail-open surface: miss one entry and the evasion is silent. It is deliberately left open rather than half-closed.
