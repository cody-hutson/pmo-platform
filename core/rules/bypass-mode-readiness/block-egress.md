...
**Reading `unparseable` records.** Their `evidence` is the constant `path=unknown cause=unparseable`, so the warn-log and block-log records of this one class also carry a `features` object and a top-level `hook_build`, and they are classified by `features.shell_parse`, scoped by `features.oracle` — the parser that judged the syntax: `error` → true-positive (the command could not run either), `ok` → benign-shape (a false refusal of a well-formed command), `skipped` or `unavailable` → unclassified. `features.heredoc` splits the false refusals by heredoc delimiter quoting, and `hook_build` names the hook build that wrote the record, so a record carrying `hook_build` without `features` is a current-build record whose features could not be written, never one from before the field set existed. No field carries the command's text, an offset or a length. The decision and its rejected alternatives are recorded in {{ADR:a-refusal-record-carries-structure-never-the-command}}.
