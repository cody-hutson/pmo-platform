...
| Rule ID | Description |
|---|---|
| BLOCK-AUTONOMY-001 | Governance-file modification (Write/Edit) — resolved path matching `CLAUDE.md`, `OPERATIONS.md`, `RELEASE_PROTOCOL.md`, any `SKILL.md`, `.claude/settings.json`, or any file under `.claude/hooks/` or `.claude/rules/`. **Every entry is matched by LOCATION, never by basename alone:** the charter at the workspace root, the platform checkout's own governance documents at any depth beneath `<workspace>/pmo-platform/` (so a worktree nested under the checkout stays covered), and the `.claude/` security surface at the workspace root. A same-named file anywhere else — an unrelated repository's root doc, a backup copy — is NOT in this set and falls through to the normal mode- and level-dependent path. Irreducible Tier-0 (operator-only per "No ungoverned changes"); always-blocks regardless of `automation_level` and without the worktree exemption `BLOCK-DESTRUCTIVE-019` carves out (governance edits are operator-irreducible regardless of cwd) |
