...
>
> The discriminator is **who opened the quote**. Quote state is tracked across the whole command under shell quoting rules (inside `'…'` only the closing quote is special; inside `"…"` a backslash escapes the next character; outside quotes a backslash escapes the next character, so `\'` opens nothing). A segment is skipped only when the state at its start is *inside a quote* **and** the command that opened that quote is one of a small allowlisted set of verbs that cannot evaluate their arguments (`gh`, `printf`, `echo`, `jq`). `git` is deliberately **not** in the set: `git -c alias.x='!<cmd>' x` evaluates its own quoted argument, so it fails the set's membership criterion.
>
> **The invariant, and the thing to check any edit against:** a segment whose start state is *outside quotes* begins at **command position** and is **always** adjudicated. Suppression can only ever reach text sitting inside a quote some earlier command opened, and only when that command cannot evaluate it. This is what keeps `bash -c '…'` safe — the quote is opened by `bash`, so no prefix in front of it (`echo x; bash -c '…'`) can reattribute the program string to `echo`. A token at command position of an actually-executing context is adjudicated whatever quotes surround it.
>
