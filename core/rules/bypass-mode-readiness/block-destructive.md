...
>
> **Quoted-fragment suppression (why a described execution is not a blocked one).** The matcher is lexical: it splits raw argv on separators, so a separator and an interpreter appearing inside a **quoted argument** are shredded into fragments that look like commands. To stop the rule firing on text that *describes* an execution rather than performing one, a segment after the first is skipped when it has **odd quote parity** — the signature of a fragment of a quoted string — **and only when** the command word of the first segment is one of a small allowlisted set of verbs that cannot evaluate their arguments (`gh`, `git`, `printf`, `echo`, `jq`). The gating direction is deliberate and is the property to preserve if this set is ever edited: a verb **missing** from the set means a false positive persists, and can never mean an evasion is admitted. The inverse construction — a denylist of *evaluating* verbs — would make one missed entry a silent fail-open, which is not an acceptable trade inside a fail-closed control. A genuinely executing command has balanced quotes within its own segment, and the first segment is never skipped.
>
