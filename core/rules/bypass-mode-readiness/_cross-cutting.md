...
|---|---|---|
| **workflow** | `block-draft-files`, `block-external-seam-shape`, `block-fragile-refs`, `block-fs-boundary`, `block-mcp-writes`, `block-skill-direct-edit`, and the mode-gated ceiling of `block-autonomy-ceiling` | hook goes inert (`exit 0`) |
...
`.claude/hooks/.mode` is currently read by **seven** hooks — four inside this registry (`block-egress.sh`, `block-mcp-writes.sh`, `block-shell-injection.sh`, `block-fs-boundary.sh`) and three owned by other discipline docs (`block-skill-direct-edit.sh`, `block-fragile-refs.sh`, `block-draft-files.sh`). Five further hooks carry their **own** mode file and are unaffected by a `.mode` change: `block-autonomy-ceiling.sh` (`.autonomy-mode`), `block-scope-segregation.sh` (`.scope-segregation-mode`), `verify-session-config.sh` (`.verify-session-config-mode`), `block-gh-path-leak.sh` (`.gh-path-leak-mode`), `block-external-seam-shape.sh` (`.seam-shape-mode`). Giving a hook its own mode file removes it from the shared cohort and adds a row to that list; re-run the command above rather than trusting this paragraph's count after any such change.
...
- [ ] Operator confirms readiness via commit to `.mode` change (note: this flip affects **every** hook whose `MODE_FILE` names the shared `.mode`, simultaneously — currently seven: `block-egress`, `block-mcp-writes`, `block-shell-injection`, `block-fs-boundary`, **plus the three registry-external readers** `block-skill-direct-edit`, `block-fragile-refs`, `block-draft-files`. It does **not** affect the five hooks carrying their own mode file, each of which graduates on its own evidence. Re-derive the cohort before flipping — see § Warn-Mode vs. Enforce-Mode for the command)

### HSR-1 — Hook-layer specificity review (owner, cadence, record)

The checklist above fires on a flip. Nothing schedules that flip, so a drain can accumulate indefinitely without ever being read — which is what happened: `fs-boundary-warn-log.jsonl` and `shell-injection-warn-log.jsonl` have been written continuously since 2026-08-24 and neither has been reviewed. `HSR-1` is the recurring obligation that makes the reading happen independently of whether a flip is contemplated.

**Trigger (the stated condition):** each release's **Stage 13 Close**.
**Owner:** the workspace owner.
**Act:** for every hook rule that wrote drain rows since the previous review, record a **specificity reading** per `core/standards/gate-efficacy-standard.md` § *Over-firing* — the sample, its denominator, and each sampled firing classified `true-positive` | `benign-shape` | `mandated-tool-blocked`. Where the reading changes a gate's disposition, update that gate's row in the flip-decision register.
**Declared observable:** the classification line committed to the gate's flip-decision register row, in the form `specificity reading YYYY-MM-DD: <n> sampled of <denominator> — TP <n> / benign <n> / mandated <n>`. The compliant path emits it; the bypassing path leaves the row's previous reading in place, so a skipped review is visible as a stale date rather than as nothing at all.
**Class:** a prose-declared normative predicate, class **3-O** (a procedure with a stated trigger and no verdict). It is **advisory**: no `--check` gate or CI job executes it, and `required` is unavailable under Requirement (b′) while that remains true. Registered in the gate-coverage register.

**Reading the drains — the substrate is structured, so the classification is not free-form.** Each drain carries a field that already partitions its firings: `reason` for `block-fs-boundary` and `block-shell-injection`, `cause` for `block-destructive`. Classify by that field's value class and report the partition; do not re-read command strings, which the drains do not carry — they store `input_digest`, never the command. A rule whose drain carries **no** such field yields no classification substrate, and that absence is itself the finding.

### HSR-2 — Report a control firing as a layer-level signal

Every block is individually reworkable in about thirty seconds, so an agent rewords and continues. The cost is real, distributed, and never aggregated — which is why a rule can fire hundreds of times on benign shapes without any session ever surfacing it. This is the standing instruction that converts a local inconvenience into a layer-level signal.

**Threshold:** the **same rule id fires 25 or more times in one session.** The number is not new — `core/hooks/block-destructive.sh` already declares `DESTRUCTIVE_022_EXEC_REVIEW_ROWS=25` as the evidence-arm bar on hook-rollout drain rows, so the reporting threshold and the graduation-evidence threshold agree rather than being two numbers for one population. It is also discriminating against the live firing distribution: measured per `(rule × day)` cell across the block log and the drains, the median is 3 and the 75th percentile is 10, while 25 selects roughly a sixth of cells.
**Channel:** the session's own output — a spoke reports it on its `Control firings:` line; an ad-hoc session reports it to the operator — naming the **rule id**, the **count**, and the **shape** that provoked it. A session that merely rewords past the threshold and reports nothing has absorbed a layer-level defect as a local cost, which is the failure this instruction exists to prevent.
**Not an escalation to a gate.** The report is an input to `HSR-1`'s next reading; it authorizes no change to any hook, allowlist, or mode file.
