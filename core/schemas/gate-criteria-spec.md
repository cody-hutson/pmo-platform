...
- Added **G3-15** (per-milestone risk-weighted size bound) to Gate 3. 1 new `validation` / `metrics` / `recommend` criterion + 1 self-repair row. Asserts `effective_pts = round_half_up(Σ(member.pts) × class_weight) ≤ upper-bound([bundling].release_size_target_pts)` per milestone, reading the `release_class_capacity_weights` config and taking the round-half-up mode by reference from the `bundle-composition-doctrine.md § 3 Step 5 Risk-Weighting` definitional home; on breach routes to the bundle-composition-doctrine § 4 tight-merge / § 3 Step 5 split-merge-reframe disposition (not a dead-end fail). Degraded path (config absent): surfaces a loud WARN and requires operator confirmation rather than silently passing with `class_weight = 1.0`. First `metrics`-Check row in Gate 3 (the `metrics` Check value is already defined in § Schema; no enum change). Complements the per-issue G2-11 / G3-12 family at the per-milestone-sum altitude. Paired with the risk-weighted capacity model (target ↔ enforcement pair) and coordinated as a non-colliding gate-ID set (G3-15 here; G3-14 and G3-16 reserved for the sibling parse-rate and cross-milestone gates; G3-13 is a RETIRED tombstone, not reused).
- Schema bump v1.11 → v1.12 (non-breaking minor; additive only). Schema consumers (automated gate-validation tooling, stage-gate evaluator, CER Claim agents) treat the new criterion as additive. Existing G1-01..G3-14 + G-BR1..G-BR4 + G-PR1..G-PR9 + G-EX1..G-EX8 + G-CL1..G-CL8 IDs unchanged.
- **Cutover discipline (v1.12 additions):** G3-15 applies to milestones entering Stage 3 strictly AFTER its introducing-release merge SHA recorded in the release log; pre-cutover milestones are grandfathered; the introducing release itself is exempt (reflexive-pipeline-loop discipline — it cannot fire its own new gate).
