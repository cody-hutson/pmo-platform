...
- **Generated-vs-source separation:** the `domain` (`source`/`managed`/`generated`, Category 6) and `folder` (`08-generated` vs `01-07`, Category 6) fields, together with the source-vs-generated field split, are the schema's canonical generated-vs-source boundary — a generated artifact is `domain: generated` + `folder: 08-generated`; a source artifact is `domain: source` + an `01-07` folder.
...
| Field | Type | Required | Valid Values | Description |
|-------|------|----------|-------------|-------------|
| `id` | String | No | Stable slug: `<type-slug>-<project-slug>-<YYYY-MM-DD>-<nn>` (e.g., `decision-package-acme-2026-04-09-01`) | A **stable, filename-independent** artifact identifier, assigned at generation and never changed on rename/move. Decouples addressability from `original_filename` so a back-link survives a rename and a **missing back-link is a detectable dangling edge** (a referrer cites an `id` that resolves to no artifact). Distinct from the SQLite `file_id` (a disposable cache key, rebuilt per index) and from the tracker-row entity `id` in [`entity-field-schemas.md`](entity-field-schemas.md) (which identifies a RAID/decision *row*, not a generated *file*). **On a file that *backs* an entity record** — e.g. `PROJECT.md` backs the Project entity (`project-entity-model.md` §7 row 1) — `id` carries the **entity** id (V-CORE-01) and the artifact-id form above does not apply to that file. The two senses never co-occur on one file, so which one is meant resolves from whether the file is an entity backing file, never from the value's shape. The structured `source_ref` tracker field + `relationships[]` edge population that consume this id are defined by the **edge-population / emit contract** (see § Relationship Edge Population below); this row defines the `id` field, that section defines how writing agents emit the edges that resolve against it. |
...
| `source_system` | String | No | `teams`, `jira`, `email`, `confluence`, `manual`, `agent-generated`, `smartsheet`, `sharepoint`, `gdrive`, `github` | Originating system. **This row is the canonical value domain for the system-of-origin concept platform-wide** — the entity-level external-identity field of the same name ([`entity-field-schemas.md`](entity-field-schemas.md) §3.0c) references this enum rather than restating it, and the external-sync path's snapshot names the same value domain in its own persistence dialect. The four members beyond the original six were added with the systems the platform mirrors against but could not previously name. |
...
**Decision:** keep `generated_by` (do not collapse into `created_by`-with-version), scoped to the creator≠generator divergence case — an artifact ingested by one actor then regenerated by a skill needs the versioned-generator trace a regression relies on; rationale per the dual-semantics note above (§ this file).
...
| `folder` | String | Yes | **Legacy 8** (DEPRECATED): `01-governance`, `02-design`, `03-testing`, `04-operations`, `05-transcripts`, `06-emails`, `07-reference`, `08-generated` · **New closed 5-bin set** (ADR-080): `1-Governance`, `2-Delivery`, `3-Operations`, `4-Evidence`, `5-Reference` + transient `_inbox`, `_generated` + the NON-BIN sentinel `_project-root` | Originating folder in the project structure. **The legacy 8-folder taxonomy is being reshaped to the closed 5-bin set (ADR-080)** via the additive-union deprecation-window pattern (mirrors the `domain` A/B/C→source/managed/generated migration above). During the migration window, enum-validating consumers accept the **union** `{01-governance, 02-design, 03-testing, 04-operations, 05-transcripts, 06-emails, 07-reference, 08-generated, 1-Governance, 2-Delivery, 3-Operations, 4-Evidence, 5-Reference, _inbox, _generated, _project-root}`; writers scaffolding new projects SHOULD emit the new closed-set value. **`_project-root` is a NON-BIN SENTINEL, not a sixth bin (ADR-139):** it names a project's own governance files (`PROJECT.md`, `README.md`, `CORRECTIONS.md`), which sit at the project ROOT and in no bin at all. ADR-080's closed-set invariant is therefore unchanged — the value describes a location *outside* the taxonomy rather than adding a member *to* it, which is why it carries the `_` prefix the schema already uses to mark a non-bin area. Agents route into the five bins and never into `_project-root`. Migration tail (`sqlite-index-schema.md` folder queries, `entity-field-schemas.md` V-ART folder rule, `agent-processing-contracts.md` emit sites, `artifact-workflow-protocol.md` promotion rule) is sequenced in the build; on convergence, exactly **one live taxonomy** (the 5-bin set) carries the concept (duplicate-source-discipline §1). Existing-project migration is deferred. 8→5 mapping: `01-governance`→`1-Governance`; `02-design`+`03-testing`→`2-Delivery`; `04-operations`→`3-Operations`; `05-transcripts`+`06-emails`→`4-Evidence`; `07-reference`→`5-Reference`; `08-generated`→`_generated`. |
...
| `delivery/{domain}` | Content files by area | Values: `governance`, `design`, `testing`, `operations`, `transcripts`, `comms`, `synthesis`. **Which `domain` this is:** concept 3 (content-area tag) — see `core/specs/domain-token-registry.md`. Here `{domain}` is a placeholder inside a tag pattern, never a field name; the value `governance` is shared with the deliverable-class `domain` on a different axis. |
...
### Domain A — Source Artifacts (source bins: legacy `01-07` · new `1-Governance` / `2-Delivery` / `4-Evidence` / `5-Reference` per ADR-080)
...
### Domain B — Managed Knowledge (trackers: legacy `04-operations` · new `3-Operations` per ADR-080)
...
### Domain C — Synthesized Intelligence (generated staging: legacy `08-generated` · new `_generated` per ADR-080)
...
- `promotion_state: promoted` requires `folder ∉ {08-generated, _generated}` (a promoted file has physically left the staging area — both the legacy `08-generated` and the new ADR-080 `_generated` staging bin; this is the rule artifact-lint Check 4 enforces, now schema-declared on the dedicated `promotion_state` field rather than inferred from the deprecated `artifact_state: PROMOTED`).
...
- `domain` records **origin** and is **preserved across the `staged → promoted` move** — a `promotion_state: promoted` artifact keeps `domain: generated` (only `folder` + `promotion_state` change on promotion), so it stays queryable as generated after it leaves the staging bin. See [`artifact-workflow-protocol.md`](../artifact-workflow-protocol.md) §4.1.1 (move-with-preserve).
...
**Live Domain-C provenance stamp set (emitted by `artifact-generator` on every generated artifact).** Four provenance/origin fields form the queryable record of a synthesized artifact, stamped at emit (artifact-generator Step 5) and **preserved across promotion**: `domain: generated` (Category 6 — origin classification; emit the live value, not the deprecated alias `C`), `generated_by` (Category 3 — the versioned generating skill), `source_inputs` (Category 3 — the upstream human evidence drawn on), and `trigger_source` (Domain C — what triggered the run). Together they let a regression trace to the generating skill version and the evidence/trigger behind any generated artifact; `domain: generated` in particular **survives promotion** (see the consistency rule above), so a promoted artifact remains queryable as generated. Full emit + promotion-preserve protocol: [`artifact-workflow-protocol.md`](../artifact-workflow-protocol.md) §4.
