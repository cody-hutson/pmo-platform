...
### Domain A — Source Artifacts (source bins: legacy `01-07` · new `1-Governance` / `2-Delivery` / `4-Evidence` / `5-Reference` per ADR-079)
...
### Domain B — Managed Knowledge (trackers: legacy `04-operations` · new `3-Operations` per ADR-079)
...
### Domain C — Synthesized Intelligence (generated staging: legacy `08-generated` · new `_generated` per ADR-079)
...
- `promotion_state: promoted` requires `folder ∉ {08-generated, _generated}` (a promoted file has physically left the staging area — both the legacy `08-generated` and the new ADR-079 `_generated` staging bin; this is the rule artifact-lint Check 4 enforces, now schema-declared on the dedicated `promotion_state` field rather than inferred from the deprecated `artifact_state: PROMOTED`).
