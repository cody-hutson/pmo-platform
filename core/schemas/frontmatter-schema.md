...
| `delivery/{domain}` | Content files by area | Values: `governance`, `design`, `testing`, `operations`, `transcripts`, `comms`, `synthesis`. **Which `domain` is this?** Concept 3, content-area tag, per `core/specs/domain-token-registry.md` — a tag path segment, never a `domain:` field. It shares the value `governance` with the deliverable-class sense; the registry names the discriminator. |
