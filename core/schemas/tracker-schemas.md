...
| **Hybrid** | Trackers partition by phase: predictive-phase uses milestone trackers (Waterfall-style); iterative-phase uses sprint or flow trackers. Requires SPM-Bridge reporting that consolidates both framings per stakeholder audience. | Phase-specific | [INFERRED] PMBOK 7 hybrid + SPM Bridge convention in `operational-runbook.md`. |
