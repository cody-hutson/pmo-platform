...
**This subsection is descriptive**, in the same sense as *What `source` guarantees, and what it does not* above. It records a posture and states the boundary between two rule sets so that a rule author can tell which set a new rule belongs to. It declares no new obligation on a pack author, and it names no runner: the intended runner for the content surface is the pack/kit content-completeness lint already named in the `**Runner:**` label above, which does not exist yet.
...
| **Content-completeness** — a sibling lint over the same pack reader, in its own `PACKC-*` namespace | **WARN** | **Provisional.** It ships warn and graduates to enforce by the criterion recorded as its flip-decision row in the gate-coverage register (`core/standards/gate-efficacy-standard.md`). |
...
**The boundary predicate.** A rule belongs to **grammar-conformance** if its verdict is computable from the pack's structure alone — the presence, type, shape and closed-domain membership of a key. It belongs to **content-completeness** if its verdict requires reading what a declaration *says*, or requires observing that a declaration is *absent where the author owed a position*. Read the discriminator as: *does the rule need to open a criteria block's interior?* The `PACK-*` set does not — it tests the `criteria` **object's** presence by truthiness over the kind-level required-field tuple and reads nothing nested inside it. Every `PACKC-*` predicate reads only inside that interior. The two sets are disjoint by construction, not by convention, which is what keeps a content rule from disturbing the grammar check's own discrimination control.
