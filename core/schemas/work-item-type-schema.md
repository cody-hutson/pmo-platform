...
| A projection **onto the general hierarchy** via the work-organization mapping framework's Layer-2 hierarchy-by-methodology map. | A projection onto any release-pipeline ticket model. `core/` governance must not couple to release-pipeline dev tooling (the ADR-017 kernel discipline). |
...
| `base` | const `Work Item` | ✅ | Every kind IS the canonical `Work Item` entity (§18). No other base permitted — this is what keeps a kind a *projection*, not a new entity (D2; the ADR-017 rejected alternative (B) "don't add an entity per kind"). |
...
- **Conditional.** Do NOT treat `work_item_type` as a closed enum in any consumer, because the discriminator's value domain is an EXTERNAL, OPEN registry (ADR-017 cross-D upstream-compat); a closed enum re-traps the open set in the frozen-roster anti-pattern this layer exists to eliminate.
