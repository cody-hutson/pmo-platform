...
**The partition is by specification-presence, never by observed enforcement.** A property is M2 because a rule in this grammar states it — whether or not any runner executes that rule. It is M3 only where **no rule states it at all**. Those two questions coincide for M1 and diverge across most of this surface, since the posture column above reads `GAP — no runner` on the majority of it. Partitioning by "does something read it?" instead would classify schema-fixed keys as absent, and the tier table would then instruct a consumer to skip a key this grammar requires.

| Tier | What a consumer may assume | Basis | Enforced by |
|---|---|---|---|
| **M1 — GUARANTEED** | A resolved kind has a `kind_id`: lowercase slug, unique within its declaring pack, and the value that lands in the `work_item_type` discriminator. The union is **archetype-blind** — it absorbs any kind-bearing pack unchanged, so no consumer branches on the pack to obtain the kind set. Resolution is **fail-loud in both directions**: a partial parse, an empty vocabulary, and an unresolvable kit selection each exit non-zero, the last naming the pack and its actual role. | `PACK-K01`, `PACK-K02`; the resolver's licensed-kind load, kind emission and archetype-resolution paths in `check-work-hierarchy.py` | live rules, exercised by discrimination arms |
| **M2 — SPECIFIED, UNENFORCED** | Everything F2 names as the join surface — the three `criteria` sub-objects; the check shape `{id, statement, level, automatable, source}`; the `level` domain; `fields.core` and `fields.kind_specific[]`; `methodology_projection`; `projects_kind`. Plus `criteria_version`'s presence and semver shape (C3), F3's `required_edges[]` limb, and F4's `lifecycle_behavior` value and `condition` legality. | §1.5.3 rows F2 / F3 / F4; §1.5.2 rows C1 / C2 / C3 / C8 / C11 | **nothing executes these** — they are the named gaps §1.5.7 routes to the register |
...
**What is readable today, stated affirmatively because a consumer needs the positive form and not only the prohibition.** Measured by importing the shipped pack reader and parsing both shipped archetype manifests: `criteria_version` parses correctly on all 8 criteria blocks; a block-level `source` parses correctly and in full on all 4 `criteria.gate` blocks; and a genuinely empty `checks = []` parses correctly as an empty list on all 4. **Only a *populated* inline-table array entry mis-parses** — the reader's value scanner splits an array on commas with no nesting model, so a 6-entry `checks[]` yields 37 comma-split fragments while the parse reports no degradation. That is exactly an M2 violation, and the disposition rule above already governs it: the consumer emits `NOT-EVALUATED` for the entry-level keys and does not iterate the fragments. It is also why a count over a populated block is the dangerous read — `len(checks)` returns a plausible non-zero on shredded input and a correct zero on a genuinely empty one, so a consumer written as *skip when empty* takes the wrong branch on exactly the input that matters.

**The single M3 member, stated as an invariant rather than as a statistic.** `PACK-L02` walks the pack's `[[labels]]` rows and requires each `type:*` row's `projects_kind` to resolve into the **declaring pack's own** `kinds[]`. The loop's population is the label rows, so a kind can never be its subject, and there is no central label source a kit could inherit from. **Nothing therefore requires a declared kind to have a label carrier**, and the consequence is conditional rather than realized: *a kit that declares kinds and no `type:` rows resolves cleanly, validates cleanly, and produces work items that cannot be typed on the label surface every create/update/delete path joins through.* **No such pack exists in this corpus.** Measured across all 17 tracked `pack.toml` manifests and 21 declared kinds: **0 of 4** shipped kinds unprojected; **0 of 4** fixture kinds in packs that declare a `[[labels]]` facet unprojected; **13 of 13** fixture kinds in packs declaring **no `[[labels]]` facet at all** unprojected. The correlation is perfect and one-directional — every pack that declares the facet projects every one of its kinds — so the 13 measure minimal fixtures being minimal, not drift. The failure shape the conditional describes, a pack that projects *some* kinds and omits others, has **zero instances**. The gap is in the rule, not in those fixtures, and it is recorded as the second one-directional limb of F3 above.

**What this subsection is, mechanically.** A prose-declared normative predicate of the same class as the named gaps it sits beside: a rule a consumer follows, not one a runner checks. It therefore **mints no `PACK-*` id, declares no resolution pointer, and adds no register row** — the three things §1.5.7 gives the reason for, and which together keep Check 62's denominator unmoved. Stating that plainly is the honest shape of an interim contract; claiming coverage it does not have would reproduce the gate-that-cannot-fail class this whole boundary exists to make countable.

**Measurement stamp.** Every count in this subsection was measured on 2026-09-11 against the tracked pack corpus and the shipped pack reader, each with a sensitivity arm required to fire and a specificity arm required to return zero. They are measurements at that date, not invariants: re-derive them before relying on a number, and read the tier table — which is expressed over *properties* and gains no row when a pack, kind or archetype is added — as the durable part.
