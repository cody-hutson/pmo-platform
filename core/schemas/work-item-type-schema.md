...
| `criteria` | object | ✅ | Three keyed sub-objects: `readiness` (DoR), `done` (DoD), `gate` (phase/stage gate). Each = `{ criteria_version (semver), checks[] }`, where each check = `{ id, statement, level (L1 structural / L2 referential / L3 judgment), automatable (bool), source (string — the body of practice the check derives from; §1.2.1) }` plus the **two OPTIONAL keys** `guards_transition` + `condition` (the relationship-conditioned / set-aggregate gate construct — §1.2.1). **The criteria carry their own version**, independent of `pack_version` — the grandfather hook (§6.3). An empty `checks[]` is valid and carries its `source` on the **enclosing `[kinds.criteria.<axis>]` block** instead, exactly as an empty `kind_specific[]` does on `[kinds.fields]` (§1.2.1 *Content provenance*) — **that block-level `source` is the marker distinguishing a reasoned empty bar from an unauthored one**, and it is the key a consumer reads to tell them apart rather than inferring from emptiness alone. |
...
**Runner: the pack content-completeness lint — `deploy.sh --check` Check 80, warn-mode initial.** This was a registered named gap and is one no longer. The runner is `core/deploy/tools/check-work-hierarchy.py --validate-pack-content`, and it grades this rule at all three of its altitudes: an entry's `source` on a `criteria.*.checks[]` entry and on a `fields.kind_specific[]` FieldDecl, a block-level `source` on a present-and-empty table, and — per the adjacent-scope-boundary rule below — a `source` on every `[[controls]]` declaration. The no-inheritance clause is carried literally: the block rule never consults an entry and the entry rules never consult the block. The obligation's row in the gate-coverage register (`core/standards/gate-efficacy-standard.md`) carries the runner pointer and the declared observable.

**How the runner is scoped, and the earlier reasoning it corrects.** The intended runner was described here as necessarily **entry-scoped** (*every entry carries a `source`*) rather than **array-scoped** (*`checks` is absent-or-empty*), on the ground that an array-scoped rule fires on most criteria tables across the **tracked pack corpus** — every `pack.toml` under `core/packs/` and `core/deploy/tests/fixtures/packs/`, fixtures included, both of the pack-grammar check's discrimination fixtures among them — and that check's discrimination branch increments the issue counter outside the warn-mode gate. **The second half of that reasoning is exactly right and still binds; the first half was too strong.** What it correctly rules out is an absence rule *inside `--validate-packs`*, and that constraint is unchanged. What it wrongly ruled out is an absence rule *at all*: an entry-scoped rule cannot see a hollow kind, because a hollow kind has no entries to range over, so provenance alone leaves the larger gap open. The shipped runner is therefore **two-scoped** — an entry-scoped provenance family, exactly as mandated, and a **kind-scoped** completeness family that fires on the absent array — and it is sited as a **separate check with its own fixture pair**, which is what makes the array-scoped hazard inapplicable rather than merely tolerated.
...
**This subsection is descriptive**, in the same sense as *What `source` guarantees, and what it does not* above. It records a posture and states the boundary between two rule sets so that a rule author can tell which set a new rule belongs to. It declares no new obligation on a pack author. The runner for the content surface is the one named in the `**Runner:**` label above — `deploy.sh --check` Check 80 — and it now exists; this subsection was written while it did not, and the posture it records is what that check implements.

| Surface | Verdict on a kind with no fields and no criteria | Permanence |
|---|---|---|
| **Grammar-conformance** — the `PACK-*` rule set the pack validator runs over the live pack root | **PASS** | **Permanent.** Not a warn held pending a flip, and never gradeable to fail. |
| **Content-completeness** — a sibling lint over the same pack reader, in its own `PACKC-*` namespace (`deploy.sh --check` Check 80) | **WARN** | **Provisional.** It ships warn and graduates to enforce by the criterion recorded as its flip-decision row in the gate-coverage register (`core/standards/gate-efficacy-standard.md`). |

**The reconciliation, stated in this schema's own terms.** The thin-generic-floor rule above — *an empty kind-specific field set is valid* — is a **grammar** verdict about a **declaration**. The completeness rule renders no verdict on that declaration's validity; it renders a verdict on a different object: whether the pack author has **stated a position**. So a content finding leaves the declaration grammatically valid and the grammar check green, and a green grammar check says nothing about whether the kind has content. The two never meet, and neither is weakened by the other. This is the pattern the materialization layer already uses one layer down, where the minimum-negative-test floor never declares a thin kind invalid and is satisfiable by the thinnest kind the grammar admits — a floor sited where the thin-generic case can meet it.

**The boundary predicate.** A rule belongs to **grammar-conformance** if its verdict is computable from the pack's structure alone — the presence, type, shape and closed-domain membership of a key. It belongs to **content-completeness** if its verdict requires reading what a declaration *says*, or requires observing that a declaration is *absent where the author owed a position*. Read the discriminator as: *does the rule need to open a declaration's interior?* The `PACK-*` set does not — it tests the `criteria` **object's** presence by truthiness over the kind-level required-field tuple and reads nothing nested inside it, and it reads no key inside a `[[controls]]` entry either. Every `PACKC-*` predicate does the opposite: seven of the eight read inside a criteria block or a `fields` table, and the eighth (`PACKC-P04`) reads inside a `[[controls]]` declaration — a pack-level construct rather than a kind surface, which is why the discriminator is stated over a declaration's interior generally and not over the criteria block alone. The two sets are disjoint by construction, not by convention, which is what keeps a content rule from disturbing the grammar check's own discrimination control.

**The findable condition is silence, never emptiness — and the distinction is the whole posture.** A surface whose array is **present and empty** while carrying a block-level `source` is a *reasoned empty set*: the author took a position, said the position is *nothing*, and attributed it. That is **complete content** and it is green. A surface whose array is **absent** is a surface on which the kind has said nothing at all, and that is the finding. A rule keyed on emptiness would fire on the artifact this grammar created to make emptiness auditable, and — because most criteria tables across the **tracked pack corpus** omit the `checks` key entirely — fixture packs included, both of the pack-conformance check's discrimination fixtures among them — would turn that check red on both its control arms. A rule keyed on silence does neither.

**The remedy is one line, and it is an artifact this grammar already defines.** A kind that genuinely owes nothing on a surface writes `= []` plus a block-level `source` naming the practice basis for the emptiness — the two-altitude construct stated above, unchanged. Nothing is ever forced to invent content, and no kind is ever obliged to declare a field it does not have.

**The named residual is not new and is not closed here.** Whether a named body of practice *actually* defines content for a given surface is a question about that body of practice, and no lint can answer it. A completeness rule converts an undecidable question into a decidable one: it does not check whether the author is *right*, it checks that the author *answered*. That residual is the one *What `source` guarantees, and what it does not* already owns — a green completeness lint is never a correctness verdict on the practice cited — and it is closed the same way, by a person reading the value against the named work.

**The sequencing consequence, stated because the posture decides it.** Under **warn**, packs that scaffold ahead of their content proceed **in parallel** with the kit-content work rather than strictly after it, and — unlike under an entry-scoped-only rule, which a hollow pack is invisible to — they are *instrumented* while they do: each unpositioned surface emits a finding into the shared warn-log sink without incrementing the issue counter, so a scaffolded-but-unauthored pack cannot fail a deploy check for any operator while its shortfall stays visible rather than silent. Under **enforce** the same packs would be strictly sequenced after the content standard, because a pack scaffolded before that standard exists would turn the deploy check red on every run until its content landed. The second-order consequence is the one worth carrying: the archetype-breadth work's delivery and this gate's graduation are the same event — the packs it lands under warn are what supply the firing population the flip criterion requires.
...
| **T-III — Content** | **nothing** | everything | `PACKC-C01 P01 P02 P03 P04 S01 S02 S03` — Check 80, **warn**. The platform still fixes **nothing** about content and the pack still chooses **everything**: what the runner grades is that a position was taken on each surface and that each position names its practice basis, never which position is right |
...
|---|---|---|---|---|---|
| **C1** | `criteria.{readiness,done,gate}.checks[]` — membership, cardinality, wording, level mix, ordering | The whole list, **including the empty list with a stated reason** | Each entry conforms to the check shape (F2) | Check 80 (`pack-content-completeness`), **warn** — `PACKC-C01` on an absent array, `PACKC-P01`/`P03` on provenance, `PACKC-S01`/`S02` on the entry shape | **COVERED, warn-mode.** *Membership, cardinality, wording, level mix and ordering remain the methodology's to choose* — that is what makes this row configurable. What is now enforced is that a position was **taken** and that each entry conforms |
| **C2** | `fields.kind_specific[]` | The field set, including empty | `FieldDecl` per `entity-field-schemas.md` §3.0 | Check 80 (`pack-content-completeness`), **warn** — `PACKC-C01` on an absent array, `PACKC-P02`/`P03` on provenance | **PARTIAL, warn-mode.** Presence of a position and per-FieldDecl provenance are enforced; the **`FieldDecl` shape itself** is not, because that shape is owned by `entity-field-schemas.md` §3.0 and this lint reads the pack grammar, not that schema. The residual is narrower than the gap it replaces and is stated rather than smoothed |
| **C3** | `criteria_version` **value** | Any semver, per block | Key required + semver-shaped | Check 80 (`pack-content-completeness`), **warn** — `PACKC-S03` | **COVERED, warn-mode.** Both limbs the gap named are now graded: the key's **presence** and the value's **semver shape**, on every criteria table that is present. It remains true that `criteria_version` is **not** a member of `PACK-K01`'s required-field tuple, so the grammar check still validates a manifest with the key removed from every block — the coverage comes from the sibling check, not from a widened grammar rule |
...
| **F2 — Read-by-name keys (the join surface)** | The three `criteria` sub-objects; the check shape `{id, statement, level, automatable}` plus `source`; `level` ∈ `{L1,L2,L3}`; `fields.core` / `kind_specific[]`; `methodology_projection`; `projects_kind` | A cross-pack consumer must resolve without branching on the pack. §3.1 step 5 **switches on `level` and `automatable`** to pick the materialization class | `PACK-K01` covers the `criteria` **object's presence only** — a truthiness test over its required-field tuple, so it covers neither the members nor the check shape. Check 80 (`pack-content-completeness`) covers both, in its own `PACKC-*` namespace and outside `--validate-packs` | Check 80, **warn** — `PACKC-S01` (the entry tuple), `PACKC-P01` (its `source` member), `PACKC-S02` (`level` ∈ `{L1,L2,L3}`), `PACKC-C01` (a `criteria` sub-object renamed away reads as an absent array), `PACKC-S03` (`criteria_version`) | **the falsification arms below were run against the grammar check alone and still hold of it** — a check entry with no `level` → **clean** · `level = "L9"`, `automatable = "yes-please"` → **clean** · `criteria.done` renamed away → **clean** · a malformed `kind_specific` entry → **clean** · **all three** sub-objects removed → **`PACK-K01`**, exit 1, which is what proves the enforced unit is the parent object. **Re-run against Check 80 over the same corpus, three of the four now produce a finding** — `PACKC-S01` · `PACKC-S02` · `PACKC-C01` respectively — and so does `criteria_version` removed from every block (`PACKC-S03`). **The fourth is covered only on its provenance limb:** a `kind_specific` entry stripped of its `source` → `PACKC-P02`, but one malformed in a non-`source` way (a missing `type`, a missing `name`) stays **clean**, because the `FieldDecl` shape is owned by `entity-field-schemas.md` §3.0 and this lint reads the pack grammar rather than that schema. That is C2's stated residual, measured here rather than assumed away |
| **F3 — Aggregation contracts** | The 7 MVP relationship types; `work_item_type` as the rollup discriminator; the label **group** set; one `control_id` and one value domain spanning kinds | Two methodologies must be **summable**. The §4 anti-EAV rollup keys on a shared discriminator — and a kind with no carrier on the label surface cannot be summed there, which is the second limb below | `PACK-K04` (**`allowed_types[]` only**), `PACK-L01`; `PACK-L02` binds **`label → kind` only** — it walks the pack's `[[labels]]` rows, so a kind is never the loop's subject and no rule reads the reverse direction | **PARTIAL — GAPs on `required_edges[]` and the kind→label projection.** Both limbs are one-directional in the same shape: the enforced direction is covered and its converse has no runner | an 8th type in `allowed_types[]` → **`PACK-K04`**, exit 1 · **the same 8th type in `required_edges[]` → clean, exit 0** · an unknown label `group` → **`PACK-L01`**, exit 1 · **removing a `type:` row while its kind stays declared → clean, exit 0** (the pack projects two of three kinds and validates) · paired control, the same row's `projects_kind` pointed at an undeclared kind → **`PACK-L02`**, exit 1 |
...
**Two named gaps, stated rather than smoothed: F3 and F4.** Both are class **3-O** prose-declared normative predicates per `gate-efficacy-standard.md` § Scope boundary, and each carries a row in that file's gate-coverage register with its declared observable. **F2 was the third and is now closed** — `deploy.sh --check` Check 80 (`pack-content-completeness`) runs it in warn-mode, and that row's register entry carries the runner pointer.

**How F2 closed, and which half of the reasoning that held it open still binds.** It was recorded here as mechanically unclosable: the runner had to be **entry-scoped** (*"every `checks[]` entry carries X"*), which fired on zero entries in the corpus of the day, and must not be **array-scoped** (*"`checks` is absent-or-empty"*), which fires on most criteria tables across the **tracked pack corpus** — fixture packs included, both of Check 75's discrimination fixtures among them — a branch that increments the issue counter **outside** the warn-mode gate, so an array-scoped rule hard-fails `--check` on both control arms. **The ratio is deliberately not restated here**, for the reason §1.5.7 gives about the gap count: it moved inside this very release when the content lint's two fixture packs landed, and a second copy would have to be re-measured in the same edit or go stale in place. The gate-coverage register row in [`gate-efficacy-standard.md`](../standards/gate-efficacy-standard.md) carries the measured figure and names its population. **That second constraint is unchanged and still governs `--validate-packs`:** no absence rule may be added to the grammar check's own rule set, for exactly the reason stated, and this is the written basis for siting the content lint elsewhere. **The first was too strong.** An entry-scoped rule cannot see a hollow kind at all, so entry-scoping alone leaves the larger gap open; and the corpus that fired on zero entries has since been populated, so that premise has expired on its own terms too. Check 80 carries both scopes — an entry-scoped provenance family and a kind-scoped completeness family — as a **separate check with its own discrimination fixture pair**, which makes the array-scoped hazard inapplicable rather than tolerated. Two self-test arms assert the contrast directly: the grammar check's own conforming template is grammar-clean and content-silent on all four surfaces.
...
| `criteria_version` — key present, semver-shaped | **FIXED** | F2/F4 — the grandfather stamp (§6.3). **Enforced on both limbs by Check 80 (`PACKC-S03`), warn-mode** (C3) — presence and semver shape alike, on every criteria table that is present |
...
The named gaps live as rows in the gate-coverage register in [`gate-efficacy-standard.md`](../standards/gate-efficacy-standard.md) § Gate-coverage register, each carrying its declared observable and the falsification arm that ran. **This section points at them; it does not copy them** — a second copy would be the duplicate-source pattern, and the register is where every coverage reader and audit already looks. **The count is deliberately not restated here.** It was *three* while F2, F3 and F4 were all open; F2 closed when Check 80 shipped, and a second copy of the number would have had to be found and changed in the same edit or would have gone stale in place. The register is the authority on how many rows are open, as it is on what each one says.

**No resolution-pointer token appears anywhere in this section, and the reason is editorial rather than mechanical — stated precisely, because the mechanical version of this claim is false.** `deploy.sh --check` Check 62 does extract resolution pointers **file-wide** rather than table-scoped, so inside the register's own file a pointer written in prose enters the denominator exactly as one written in a row. But *this* file is not that file: the check's extraction population is a single path, `core/standards/gate-efficacy-standard.md`, and the only other files it opens are the resolution **targets** of pointers it already extracted, plus the check bank it closes their namespace against. It never reads this schema. A pointer written here is therefore invisible to Check 62 — it can neither inflate the count nor flip the register's verdict to `UNRESOLVED`.

**The convention stands on its own ground, which is the stronger one anyway.** The register is the single home for resolution pointers, and a pointer written here would be a second copy of a coverage claim in a file that §1.5.7 has just finished saying is not the authority on coverage — a shadow SSOT that no check would catch precisely *because* Check 62 cannot see it. A named-gap row correctly carries none until its runner ships; this section carries none because the claim is not its to make.

#### 1.5.8 What a consumer may assume — the reading rule

§1.5.1–§1.5.7 state this boundary from the **producer's** side: what a pack may declare, and what the platform fixes regardless. This subsection states the same boundary from the **consumer's** side — what a program reading a *resolved* kit may rely on, and what it must do when a reliance fails. The two halves live in one section deliberately: a consumer rule kept in a second file would be one boundary maintained in two places that must agree, which is the duplicate-source pattern §1.5.7 declines by name.

**The partition is by specification-presence, never by observed enforcement, and it draws both boundaries.** A property sits in M1 or M2 because a rule states it — this grammar's, or the contract of the resolver that reads it — whether or not any runner executes that rule, and in M3 only where **no rule states it at all**. Partitioning by "does something read it?" instead would classify schema-fixed keys as absent, and the tier table would then instruct a consumer to skip a key this grammar requires. **What separates M1 from M2 is what the specification is about, not how firmly it is enforced:** M1 is specified of the **kind set a resolution delivers** — which kinds there are and what each is called — and M2 of **the declaration behind each resolved kind**, the join surface a consumer reads by name. The resolver keys its union on `kind_id` and consults no M2 key, so M1 reaches a consumer already resolved while every M2 key reaches it unconsulted; that, and not a guarantee, is why the disposition rule below reads the two tiers differently. Enforcement is reported per property in the `Enforced by` column and is never the basis of the split, so a runner shipping or flipping re-derives that column and moves no property between tiers.

| Tier | What a consumer may assume | Basis | Enforced by |
|---|---|---|---|
| **M1 — SPECIFIED: the resolved kind set** | A resolved kind has a `kind_id`: lowercase slug, unique within its declaring pack, and the value that lands in the `work_item_type` discriminator. The union is **archetype-blind** — it absorbs any kind-bearing pack unchanged, so no consumer branches on the pack to obtain the kind set. Resolution is **fail-loud in both directions**: a partial parse, an empty pack root, and an unresolvable kit selection each exit non-zero, the last naming the pack and its actual role. | §1.2's `kind_id` row; `PACK-K01`, `PACK-K02`; the resolver's licensed-kind load, kind emission and archetype-resolution paths in `check-work-hierarchy.py` | **The slug form and within-pack uniqueness: `PACK-K02` alone — Check 75, warn-mode only.** Neither resolution path enforces uniqueness: a duplicate `kind_id` collapses silently on `--resolve` and on `--emit-kinds` alike, at exit 0. The slug form fails loud on `--emit-kinds` alone (exit 3, read as a partial parse); `--resolve` emits a non-slug id verbatim at exit 0. **The fail-loud limb is the resolver's own exit status, not a lint** — a partial parse, an empty pack root and an unresolvable `--kit` each exit 3 on the path that reads them. An empty *vocabulary* exits 3 on `--emit-kinds` only; `--resolve` reports an eligible set that contributes no kinds as a measured `COUNT 0` at exit 0, by that mode's own contract |
| **M2 — SPECIFIED: each kind's join surface** | Everything F2 names as the join surface — the three `criteria` sub-objects; the check shape `{id, statement, level, automatable, source}`; the `level` domain; `fields.core` and `fields.kind_specific[]`; `methodology_projection`; `projects_kind`. Plus `criteria_version`'s presence and semver shape (C3), F3's `required_edges[]` limb, and F4's `lifecycle_behavior` value and `condition` legality. | §1.5.3 rows F2 / F3 / F4; §1.5.2 rows C1 / C2 / C3 / C8 / C11 | **Warn-mode at most, and never at resolution.** Check 75 grades the presence of the `criteria`, `fields`, `methodology_projection` and `lifecycle_behavior` objects (`PACK-K01`), `methodology_projection`'s members (`PACK-K05 K06 K06a K07 K08`), and `projects_kind` in the label → kind direction only (`PACK-L02`). Check 80 (`pack-content-completeness`) grades the check shape, the `level` domain, a `criteria` sub-object renamed away, `criteria_version`'s presence and semver shape, and `kind_specific[]`'s position and provenance, under the `PACKC-*` ids §1.5.2 C1–C3, §1.5.3 F2 and §1.5.4 attribute. **No runner reads** F3's `required_edges[]`, F4's `lifecycle_behavior` value or its `condition` legality, C8's `value_domain`, C11's `guards_transition` and `condition`, `fields.core` below its parent object, or the `FieldDecl` shape — C2's stated residual. The register is the authority on which of these stand open as named gaps (§1.5.7) |
| **M3 — ABSENT** | Exactly one member: **no consumer may assume that a declared kind has a `type:<kind_id>` label row.** Nothing in this grammar requires the projection in that direction (see below). | measured, stamped below | n/a — no rule states it, so there is nothing for a runner to execute |

**The disposition rule — the one thing §1.5.1–§1.5.7 do not carry.** A consumer reads M1 unconditionally and M2 **defensively**. On any M2 violation — an absent sub-object, an out-of-domain `level`, a malformed or unparseable entry — it emits **`NOT-EVALUATED`, naming the kind, the key, and the denominator**, and renders no verdict for that kind. Never a silent default, never a substituted kind, never an unreadable input reading as a clean one. **M3 is not read**: a consumer neither relies on an M3 property nor emits a caveat for its absence, because there is no specification for it to have violated.

**The denominator is required, not advisory, and the reason is that the rule can otherwise hide its own failure.** A consumer emitting `NOT-EVALUATED` for every kind it meets is indistinguishable from one that is working correctly unless the emission carries *kinds read* against *kinds evaluated*. Without it, a reading rule written to prevent a vacuous clean verdict produces a vacuous caveat instead — the same defect in the opposite sign. The three-element form is the contract; the two-element form is not a shorthand for it.

**Not a new discipline — the general statement of two narrow ones this corpus already carries.** §1.2.2 applies it to the status axis when no platform adapter is configured (*a gap is surfaced, never silently defaulted*); [`gate-criteria-spec.md`](gate-criteria-spec.md)'s Gate-1 Step 0 applies it to the kind vocabulary when that vocabulary cannot be read (*resolution unavailable is a fifth outcome, and it is not a verdict … the criterion reads `NOT-EVALUATED`, never `FAILED`*). Both are scoped to one case. This subsection states the rule for **any** read of a resolved kit, and adopts the existing token verbatim rather than minting a synonym, which would fork a live vocabulary.

**Composition with §3.1 step 5, so the two uses of the token are not confused.** Step 5 carries `NOT-EVALUATED` as a **per-check evaluability disposition** — a measured property of a judgment-level check, established against a falsification/specificity arm pair and recorded in its own architectural record. That is a different object at a different altitude: step 5 governs whether a measured check may be **admitted to an aggregate**, while this subsection governs whether a **key may be read at all**. The two agree on the discipline they encode — an unmeasured or unreadable input is branched on before any count is consumed, never rounded into one — and a consumer must not infer either from the other. In particular, **`level` and `automatable` are M2 keys and say nothing about a check's evaluability**, which §3.1 step 5 fixes as a per-check measured property never inferred from the level.

**What is readable today, stated affirmatively because a consumer needs the positive form and not only the prohibition.** Measured by importing the shipped pack reader and parsing both shipped archetype manifests, whose four kinds carry 12 criteria blocks: `criteria_version` parses correctly on all 12; a block-level `source` parses correctly and in full on every `criteria.gate` block that carries one, 3 of the 4; and a genuinely empty `checks = []` parses correctly as an empty list on the same 3 — the fourth, Kanban's, is populated. **Entries are readable today, through one of the reader's two array paths.** Its default value scanner splits an array on commas with no nesting model, so a 6-entry `checks[]` yields 37 comma-split fragments while the parse reports no degradation. Its content path — `parse_pack(…, capture_content=True)`, the nesting- and string-aware scanner Check 80 reads entries through — reads every populated array in both manifests back at its declared entry count. Read through the default path instead, a populated array is exactly an M2 violation — an unparseable entry — and the disposition rule above already governs it: the consumer emits `NOT-EVALUATED` for the entry-level keys and does not iterate the fragments. It is also why a count over a populated block is the dangerous read on that path — `len(checks)` returns a plausible non-zero on shredded input and a correct zero on a genuinely empty one, so a consumer written as *skip when empty* takes the wrong branch on exactly the input that matters.

**The single M3 member, stated as an invariant rather than as a statistic.** `PACK-L02` walks the pack's `[[labels]]` rows and requires each `type:*` row's `projects_kind` to resolve into the **declaring pack's own** `kinds[]`. The loop's population is the label rows, so a kind can never be its subject, and there is no central label source a kit could inherit from. **Nothing therefore requires a declared kind to have a label carrier**, and the consequence is conditional rather than realized: *a kit that declares kinds and no `type:` rows resolves cleanly, validates cleanly, and produces work items that cannot be typed on the label surface every create/update/delete path joins through.* **No such pack exists in this corpus.** Measured across all 19 tracked `pack.toml` manifests and 23 declared kinds: **0 of 4** shipped kinds unprojected; **0 of 6** fixture kinds in packs that declare a `[[labels]]` facet unprojected; **13 of 13** fixture kinds in packs declaring **no `[[labels]]` facet at all** unprojected. The correlation is perfect and one-directional — every pack that declares the facet projects every one of its kinds — so the 13 measure minimal fixtures being minimal, not drift. The failure shape the conditional describes, a pack that projects *some* kinds and omits others, has **zero instances**. The gap is in the rule, not in those fixtures, and it is recorded as the second one-directional limb of F3 above.

**What this subsection is, mechanically.** A prose-declared normative predicate of the same class as the named gaps it sits beside: a rule a consumer follows, not one a runner checks. It therefore **mints no rule id in either namespace, declares no resolution pointer, and adds no register row**. The restraint is editorial, on the ground §1.5.7 gives for its own section — the register, not this file, is the authority on coverage — and not mechanical: Check 62 never reads this file, so nothing written here could move its denominator in either direction. Stating that plainly is the honest shape of an interim contract; claiming coverage it does not have would reproduce the gate-that-cannot-fail class this whole boundary exists to make countable.

**Measurement stamp.** Every count in this subsection was measured on 2026-09-17 against the tracked pack corpus and the shipped pack reader, each with a sensitivity arm required to fire and a specificity arm required to return zero. They are measurements at that date, not invariants: re-derive them before relying on a number, and read the tier table — which is expressed over *properties* and gains no row when a pack, kind or archetype is added — as the durable part.
...
5. **Each `criteria.checks[]` projects by level:** `automatable: true` ∧ `level: L1` → a schema-expressible constraint (enum / pattern / required); `level: L2` → an `x-pmo-referential` entry; `level: L3` → an `x-pmo-criteria-judgment` annotation (recorded so a reviewer/skill can surface it, not machine-enforced). **The annotation additionally carries the check's `evaluability` disposition** — `gate-capable`, `gate-capable-under-conditions`, `advisory-only`, or `NOT-EVALUATED` where the check has not been measured — together with the per-arm run counts the disposition was measured at. **Evaluability is a per-check MEASURED property, never inferred from the level**: it is established against a falsification/specificity arm pair required unanimous across context-isolated runs, and a check's level says nothing about it. The disposition's source of record is {{ADR:l3-judgment-criteria-evaluability}}; this clause states the rule, and that record carries the measurement, its validity threats and its routing. **A check whose disposition is `advisory-only` or `NOT-EVALUATED` MUST NOT be admitted to an aggregated criterion set whose rule rounds an unevaluable member to FAIL** — an `advisory-only` check because admission makes that aggregate either structurally incapable of its clean verdict (where a run returns no renderable judgment) or nondeterministic and therefore re-rollable (where runs render definite verdicts inconsistently), and a `NOT-EVALUATED` check because an unmeasured member's effect on the aggregate is unknown rather than benign. **`NOT-EVALUATED` is a measurement state and is never spelled as a zero disposition**: a consumer branches on it before reading any count, because reading the count alone consumes *nothing was examined* as *nothing was found*. **When the L2 check carries a `condition` (§1.2.1), the annotation is selected by `condition.kind`:** `related-item-status` → an `x-pmo-referential` entry that resolves the target's *state* (not its existence), with the gate-layer `on-unresolved: BLOCK-TRANSITION`; `set-aggregate` → an `x-pmo-aggregate` annotation (the NEW class, §3.1a). A check with no `condition` keeps the existing behavior unchanged. **Independently of level, a `checks[]` entry carrying `source` (§1.2.1) contributes one `x-pmo-content-source[]` entry at the schema root** — the three by-level arms above are byte-identical whether or not `source` is present, because provenance and enforcement class are orthogonal axes.
...
- **The `kinds` requiredness relaxation is grounded separately — NOT under the ADR-039 optional-ADD precedent.** A relaxation is *not* symmetric the way an optional add is: an old validator meeting an unknown optional key simply ignores it, but an old validator that assumes `kinds` present would *reject* a now-valid `role = "base"` pack. That forward-incompat was **theoretical when this section was written**: no meta-schema validator existed yet, so the relaxation landed before any validator could be broken by it, and the first validator (built against this widened grammar) is role-aware from the start (it applies the completeness rule per `role`). The relaxation was safe because the widen shipped the only validator — not because it matches ADR-039's optional-ADD symmetry. **That window is now closed**: the first meta-schema validator ships in the work-item-kit release as `check-work-hierarchy.py --validate-packs`, evaluating 20 pack rules where both that release's merge-base and the mainline carry 0, so a further relaxation of this grammar meets a live reader and must be analysed against it — §6.2c Axis 2 records the same closure from the other side.
...
- **Specifying the Arm-3 body converts a fail-loud reserved value into a defined one** — no existing valid pack changes meaning, because a pack using `kind: control-field` before this specification FAILED pack-validation by design ("reserved, not yet implemented"); reserve-then-specify is exactly the lifecycle the slot was registered for. Validator-forward: the widen **shipped ahead of** any meta-schema validator (the §6.2a posture held at the time — the first validator was to be controls-aware from the start). **That posture is now spent rather than standing**: the first meta-schema validator ships in the work-item-kit release, so this bullet records a window that has closed, not one still open.
...
### 6.2c The work-item-kit extension is additive in effect — meta-schema version stays v1 (ADR-180)

The work-item-kit layer (§1 kind-bearing-by-role, §1.1 `role = "kit"` / `kit_class` / the widened `extends` / the two-level `kinds` rule, §1.3 the neutral `archetype` sentinel and the `level_role` traversal carrier) extends the meta-schema **without** a version bump — the third such extension, after §6.2a and §6.2b. The architectural decision this records is [ADR-180](../ADRs/ADR-180-work-item-kit-first-class-unit.md), a grammar-altitude sibling extension of [ADR-018](../ADRs/ADR-018-work-item-type-layer.md) in the same lineage as ADR-039, ADR-070 and ADR-077.
...
**Axis 3 — the RESTRICTION, and it meets a population of ZERO.** Measured at the commit that introduces the rule, by parsing every `pack.toml` the corpus held **at that commit** — shipped and fixture alike, 17 manifests — the count of `[[controls]]` entries **immediately before this change is 0**, against live controls on the same reader over the same files (`[[kinds]]` 21, `[[labels]]` 48) and a zero specificity arm on a fabricated array name. **Every figure in this axis is anchored to that commit and is not a live corpus count.** The corpus moved later in this same release, when the content lint's two fixture packs landed: at the release head the same readers report **19 manifests, 23 kinds, 50 labels**. The pre-change zero is unaffected — it is a historical fact about the moment the rule took effect, which is the only moment at which "the population it meets" is defined.

| Limb | Site class | State | Count | Requirement fires? |
|---|---|---|---|---|
| Entry | `[[controls]]` declarations | pre-change, whole corpus **at the introducing commit** | **0** | — none existed anywhere |
| Block | — | **structurally inapplicable** | — | there is no enclosing required table (§1.2.1) |
| Entry | `[[controls]]` declarations | post-change, whole corpus **at the introducing commit** | **1** | **yes — and it adopts the key in the same commit** |

**So this is the extension §6.2d could not be.** §6.2d recorded, as its own finding, that it was *"the first of the four extensions whose restriction meets a NON-EMPTY population"* — 16 affected declarations, none of them remediable by a runner that does not exist. This one inverts that: **no declaration is invalidated, because none existed to invalidate**, and **every** declaration the rule governs is authored under the rule rather than retrofitted to it — a property that holds however many arrive later, which is why it is stated without a count. §6.2c's *"every shipped pack is byte-identical after this change"* sentence, unavailable to §6.2d, is available here for every pack except the one making the declaration at that commit.

**The cheap window is real, it was taken deliberately, and it does not recur.** A population of zero empties exactly once. Had this requirement waited until a second or third pack declared a control, the analysis above would have had to be written against a non-empty population and the retrofit §6.2d describes would have been reproduced one facet over. That reasoning, and the reversibility band the first declaration crosses, are recorded in `ADR-196`.

**The residual, named rather than dismissed — and closed in the same release that named it.** It was stated as the residual §6.2d owns: **no runner reads `source`**, on a control or anywhere else, because the pack validator does not open a criteria block's interior and does not read the controls facet's content. That was true when this section was written and it is true of the pack validator still. It is no longer true of the corpus: `deploy.sh --check` Check 80 (`pack-content-completeness`) grades `source` at every altitude the rule binds, and `PACKC-P04` is the limb that grades it on a `[[controls]]` declaration. At this release's head the tracked pack corpus holds **two** `[[controls]]` declarations — one in a shipped pack (`core/packs/kanban`, the WIP-limit control) and one in the content lint's own `cc-complete` fixture — and **both carry `source`**, so the governed population conforms in full. The second arrived after this section was first written, which is why the count is stated against a named population rather than as *exactly one*: a requirement and its runner shipped in the same release, and the population the runner grades has been growing ever since.
