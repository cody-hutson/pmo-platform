...
| Field | Type | Req | Constraint / Semantics |
|---|---|---|---|
| `group` | enum | ✅ | The label-grammar-owned group this row lands in — one of `category` / `status` / `work-status` / `cluster` / `initiative` / `triage-flag` / `disposition` (the groups `label-taxonomy.md` defines, in that document's own section order). The grammar owns the group set; the pack fills it. |
