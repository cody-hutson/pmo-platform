...
- [ ] Every field in [`platform-config.toml.template`](../config/platform-config.toml.template) `[meta]` / `[bundling]` / `[release_class]` / `[relationship_mapping]` / `[calibration]` / `[failure_mode_detectors]` / `[progressive_rollout]` / `[spoke_runtime]` / `[security_hooks]` has a row above with type / allowed-values / default / consuming-surface (the `[spoke_runtime]` effort posture is a pointer, not a field — noted above the row, no schema row by design).
