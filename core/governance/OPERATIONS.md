...
- **Project-scoped output:** All generated artifacts must route to the active project's closed 5-bin structure (`1-Governance`…`5-Reference`; legacy `01-08` during the ADR-078 migration window) or its `_generated/` staging area. Active project = the project whose PROJECT.md is loaded. If ambiguous, ask the user. Governance files at projects/ root are exempt.
...
```
[Project Name]/                             ← Uniform closed 5-bin set (ADR-078) — identical for every project
...
Folder purposes for the closed 5-bin set (ADR-078). Legacy `01-08` bins map per the tree note above and remain valid during the migration window.
...
Classify incoming files by pattern and route to correct folder (the ADR-078 closed 5-bin set). If pattern doesn't match, route to `_inbox/_unsorted/` as fallback. Transcript cadence sub-folders flatten to `4-Evidence/Transcripts/` under the closed set; the file-router wave finalizes the intake-classification mechanics.
