...
4. **Recurrence — a periodic chore, not a pipeline step.** Where a sweep recurs, it runs as its own chore PR on a threshold predicate, not as a step inside the release close-out. The release-corpus sweep is the worked case: its predicate is *"the four ledgers' combined size exceeds the byte ceiling"*, evaluated by `release/tools/sweep-release-corpus.py` (whose `--plan` mode reports the verdict and writes nothing). **Why not a close-out step:** a close-out step would run at every release to do nothing at almost all of them, and it would add a failure surface to the critical path that publishes a release — cost on every release, benefit on a few. A chore PR also keeps a byte-moving change reviewable on its own diff instead of buried inside a close-out. The hysteresis between the trigger ceiling and the sweep floor is what makes this safe: each sweep leaves roughly three releases of headroom, so the budget holds continuously between chore runs rather than only at the moment one finishes.
