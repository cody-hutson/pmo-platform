...
deciders: "Stage 5 Solutioning spoke (Principal Engineer — Architecture Assessment) + operator at the v3.93 / 56-runtime-config-and-posture Collective Review scope-lock"
...
### Issue References
Originating deliverable: #340 (milestone `56-runtime-config-and-posture`, #183) — establish the canonical spoke model/effort edit surface while preserving the detection composite. Downstream consumers of the carrier shape locked here: #339 (the SessionStart model/effort hook reads `[spoke_runtime].default_spoke_model` + the effort pointer; sequence #340 → #339) and #310 (adds a sibling `[security_hooks]` section to the same `platform-config.toml`). Sibling v3.93 config-add: #1212 (`[failure_mode_detectors]` + `[progressive_rollout]`). The platform-config surface + `resolve_platform_config` rung-reader this surface reuses were introduced by #22 (adapter-config-foundation). The prior model-REQUIRED-EXPLICIT + dual-anchor decision this ADR supersedes-in-part was carried by a merged pull-request that does not resolve as an issue in the current numbering (pre-renumber); it is named in prose above, not by number, per the reference-durability discipline.
