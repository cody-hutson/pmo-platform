...
- **A live implementation risk replaces the eliminated one.** The template-sync source resolver matches on basename, and this basename exists at two paths; the resolver's disambiguation must be verified at implementation rather than assumed. **Discharged at implementation, with a result the authoring did not anticipate:** the resolver does not choose between the two real paths at all. Its basename cases cover an explicit shared-standards set and a `template-*` prefix, and this basename matches neither — so it falls to the default arm and resolves to a **third path that does not exist**. Registering the entry without also giving the basename its own resolver arm would therefore have failed the always-enforce template-injection check ("canonical missing from registry") and returned non-zero from the packager on every build of that skill. The arm is part of this decision's implementation, not an incidental edit; it lives in the single resolver definition both staging paths source, so the two remain aligned by construction.
...
Confidence is MEDIUM rather than HIGH for one named reason: the resolver-disambiguation residual in decision 5 is verified at implementation, not at authoring. **That named reason is now discharged** — the residual was probed at implementation, resolved by an explicit resolver arm, and the always-enforce package-freshness check reports fresh with a zero exit after the rebuild, which is the empirical confirmation that both staging paths produce identical bytes. The confidence level is therefore re-assessable at the ratification gate; it is left as authored here rather than raised by the implementing change, because the rendered value is the operator's to set.
