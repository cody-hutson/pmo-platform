...
---
title: "ADR-150 — BLOCK-DESTRUCTIVE-022 governs execution capability, and its widened arm graduates on a repo-derivable deadline"
status: Accepted
date: 2026-08-24
...
# ADR-150 — BLOCK-DESTRUCTIVE-022 governs execution capability
...
**The accepted scope reaches NON-SHELL interpreters, and the pattern above was reused rather than re-decided (recorded 2026-09-03, release `hooks-block-only-their-scope`).** The Decision below states that this allowlist governs *execution capability, not interpreter invocations only*. Direct execution was the half of that with a defect report; the other half is that the verb classifier admitted `bash|sh|zsh` and nothing else, so a script invoked through `python3`, `perl`, `ruby` or `node` was never classified as an execution at all and no arm ever asked the allowlist about it. **That is the same root cause the exec arm closed** — an execution model expressed as a fixed vocabulary of shell tokens and shell suffixes — and admitting those interpreters is an *application* of the decision recorded here rather than a new one, which is why it amends this record instead of opening a second one for the same rationale. The interpreter set is a **closed exact-literal enumeration**, chosen on capability-to-launder rather than corpus usage; a trailing glob was rejected on the file's own `ORDER IS LOAD-BEARING` constraint, since the verb `case` precedes the exec discriminator and a glob-matched false verb would remove its own token from exec-arm adjudication — a widening whose implementation narrows.

**What was reused, and the two places the pattern needed to grow.** The rollout took a *sibling* constant family, `DESTRUCTIVE_022_INTERP_*`, at the same entry rung `warn` with the same 60/90-day thresholds — deliberately not a reuse of `DESTRUCTIVE_022_EXEC_*`, because two widenings with two populations must be able to retreat independently, and one constant cannot record two decisions. That is the generalizable pattern working as named above. Two extensions were required and are the novel content of this amendment: **(1)** the drain gained an `arm` field, because two phase-gated arms writing one undifferentiated population makes both graduations unreadable — a defect the exec arm's own rows already had, since 1,067 accumulated rows carrying only `would-fire` could size the rule and attribute nothing; a row with no `arm` field predates it and belongs to `exec`, which was then the only writer. **(2)** Check 71 was **parameterized over a family list** rather than copied per arm, on the register-or-remove rule: a graduation *reader* duplicated per family is exactly the drift surface that rule exists to prevent, and a third literal copy of it was the alternative. Adding a fourth phased arm is now one word in that list plus five constants in the hook.

**A recorded re-dating, not an incidental edit.** `DESTRUCTIVE_022_EXEC_ARMED` moved from 2026-08-24 to 2026-09-03 because the same change widened the exec arm's operand domain to the new suffixes, so rows accumulated from that commit forward are not the population the original arming was measuring. Check 71 names re-dating as the sanctioned one-time extension; this is that one. A second would have to be a decision to graduate or retreat instead.

**A2 and A4 below were re-tested against the widening and both held.** The four per-interpreter operand domains were added as *sibling* `case` arms; the shipped `interp) *.sh` body is byte-preserved, and every basename the new selector does not name — the shell trio included — defaults back to `interp`, so the shell arms are unchanged by construction rather than by argument. A10 held too: the shell trio does **not** route through the new verdict router, so its always-enforce posture — the basis on which the mode-capable cohort was permitted to degrade — is preserved at the call site rather than re-argued.

**Three further residuals accompany the interpreter widening, each declared rather than half-closed:** versioned interpreter spellings (`python3.11`), unadmitted interpreters (`pwsh`, `awk`, `osascript`, `php`, `deno`, `bun`, `Rscript`), and the option-taking-flag shapes (`python3 -X faulthandler x.py`) whose real cause is the operand-locating walk not consuming a flag's argument — filed separately and **pinned by test arm** so that fixing the walk flips a pinned arm rather than drifting. `awk`'s exclusion is load-bearing rather than arbitrary: its operand arrives through `-f`, so an `awk` label could not reach its operand today and would ship a dead branch on a security surface.
