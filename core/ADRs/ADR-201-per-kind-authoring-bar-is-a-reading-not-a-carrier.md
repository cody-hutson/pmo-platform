...
**The run was reproduced by a second executor.** The adversarial reviewer executed the same sequent from the section text rather than from the first run's arm-by-arm report, and reached the same verdict — but with the first run's routing known, which the reviewer's own write-up quotes. That makes it a **replication, not a blinded re-run**, and the record claims no more than that: a placement result re-derived from the section text by a second executor is the evidence carried forward, and it is stronger than the first run's report alone while falling short of an independent blinded confirmation.

**Stated limitation — the independence wording elsewhere in this record describes the *review*, not the placement run.** The clause immediately above is this record's operative statement of the run's evidentiary grade, and it governs wherever another site reads otherwise. Sites that still word it as independence are deliberately left byte-unchanged, because each is either frozen frontmatter or sits in a section this record does not reopen: the `source_observations:` entry's *"executed independently a second time"*, the Status section's *"independent Phase-A6.5 adversarial review"*, and the Alternatives Considered entry's *"executed and independently reproduced"*. Their accurate referent is the **adversarial review**, which was genuinely conducted by a second party at Phase A6.5 — not the **placement run**, whose second execution had the first run's routing in hand and is therefore a replication. (The Provenance section's *"independently adversarially reviewed at Phase A6.5"* needs no such reading; it is about the review on its face.) **No site in this record should be read as claiming a blinded confirmation the evidence does not support.** This is the disposition ADR-200 applies to the contradictions in its own instrument — state the limitation in the open, leave the frozen text alone, and let the record claim only what its evidence carries — and it is taken here for the same reason: correcting frozen provenance text would rewrite what was recorded at the time, while leaving the limitation unstated would let the stronger reading stand unchallenged.
