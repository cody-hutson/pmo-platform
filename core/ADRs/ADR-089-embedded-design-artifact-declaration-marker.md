...
The marker bounds the **declared region** — from the marker line to the next heading of the same or higher level — which *is* the artifact; everything else in the parent doc is ordinary prose. Discovery is `grep -rn '<!-- design-artifact:' core operations release`. The mechanism reuses the corpus's dominant directive-marker idiom (495 files already carry `<!-- key: value -->` markers), so the parsing tooling pattern — and eventual hook/CI wiring — already exists.
