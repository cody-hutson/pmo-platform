...
**Renumber log.** Core ADR-024 (`operations-consume-core-safety-controls-via-public-api`) → **ADR-028** on 2026-06-18, resolving a platform-wide collision with the earlier `release/ADRs/ADR-024-cross-release-impact-model.md` (which had correctly claimed 024).
Release ADR-100 (`mode-r-disposition-set-fit-test`) → **ADR-099** on 2026-07-28, closing a gap rather than resolving a collision: 099 read as claimed by two sibling release branches when the record was authored, so the author stepped past it to 100 — but an unmerged claim does not bind the sequence. `main` topped out at ADR-098, making 099 the true next-free slot; merging at 100 would have landed the gap on `main`, where the gate would then fail every subsequent PR. First-to-merge takes the number and the other claimants renumber.
Release ADR-099 (`event-log-payload-pipe-grammar`) → **ADR-100** on 2026-07-28 is that rule applying in the other direction: the `mode-r-disposition-set-fit-test` record above merged first and took 099, so this claimant — which had itself already moved 098 → 099 for the same reason — moved again to the new next-free slot.
Core ADR-098 (`decision-audit-host-qa-auditor-mode-j`) → **ADR-101** on 2026-07-28 is the fourth claimant of that same contended slot resolving the same way: its branch parked awaiting a dependency while `portability-seventh-first-class-value` merged first and kept 098, after which the two records above claimed 099 and 100 — so on rebase `main` topped out at ADR-100 and 101 was the true next-free slot.
...
ADR-111 (`adr-number-claim-binds-at-merge`) → **ADR-115** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 111; the record's Status section carries the provenance note.
ADR-110 (`adr-section-set-and-durability-hygiene-carve-out`) → **ADR-114** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 110; the record's Status section carries the provenance note.
ADR-112 (`adr-deciders-carve-out-is-name-only`) → **ADR-116** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 112; the record's Status section carries the provenance note.
ADR-113 (`adr-index-derived-surface-and-scoped-conformance-claim`) → **ADR-117** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 113; the record's Status section carries the provenance note.
ADR-114 (`adr-section-set-and-durability-hygiene-carve-out`) → **ADR-118** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 114. This is the **second** move of the record logged above as ADR-110 → ADR-114, so its lineage is `110 → 114 → 118` and its Status section carries one provenance note per hop. The second note was **not** written when this hop ran, and this sentence originally asserted that it was: the tool's idempotence guard keyed on the note's *shape* — any `NNN → NNN` pair — so it found the first hop's note, reported the write already done, and left the record naming a number a different mainline ADR holds; the verify step asked the same shape question and passed over the gap. The guard and the verify now key on **this move's** `old → new` pair, a double-move regression arm pins it, and the record was reconciled at this release's Stage-9 gate. The lesson generalizes past numbering: a predicate that matches the *form* of an audit record is not a check that the record is *there*.
ADR-120 (`claude-md-composition-surface-recategorization`) → **ADR-122** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 120; the record's Status section carries the provenance note.
ADR-120 (`domain-is-a-parameter-of-the-architect-role`) → **ADR-121** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 120; the record's Status section carries the provenance note.
ADR-121 (`domain-is-a-parameter-of-the-architect-role`) → **ADR-123** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 121; the record's Status section carries the provenance note.
ADR-121 (`status-fallback-k1-binding-k4`) → **ADR-125** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 121; the record's Status section carries the provenance note.
ADR-122 (`sub-task-status-mirror-not-resynced`) → **ADR-126** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 122; the record's Status section carries the provenance note.
ADR-123 (`domain-is-a-parameter-of-the-architect-role`) → **ADR-127** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 123; the record's Status section carries the provenance note.
ADR-120 (`version-tags-are-retained-not-reaped`) → **ADR-121** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 120; the record's Status section carries the provenance note.
ADR-121 (`version-tags-are-retained-not-reaped`) → **ADR-123** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 121. This is the **second** move of that record, so its lineage is `120 → 121 → 123` and its Status section carries one provenance note per hop. The first hop's log sentence had to be restored by hand: the tool's § Renumber log step rewrote the existing entry for this record in place rather than appending a second one, so the log briefly read as though the record had originally claimed 121. That hand-restore left a duplicated, truncated fragment of the sentence in place, which this merge removed — the record's own Status section was unaffected throughout, appending correctly one note per hop, so the file-set-derived surfaces stayed accurate while the prose log did not.
ADR-123 (`version-tags-are-retained-not-reaped`) → **ADR-127** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 123; the record's Status section carries the provenance note.
ADR-127 (`version-tags-are-retained-not-reaped`) → **ADR-128** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 127; the record's Status section carries the provenance note.
ADR-127 (`close-class-is-a-declared-deliverable-value-conditioning-one-gate-spec`) → **ADR-128** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 127; the record's Status section carries the provenance note.
ADR-128 (`close-class-is-a-declared-deliverable-value-conditioning-one-gate-spec`) → **ADR-129** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 128; the record's Status section carries the provenance note.
ADR-129 (`lib-missing-guard-is-mode-coupled`) → **ADR-130** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 129; the record's Status section carries the provenance note.
ADR-133 (`degraded-state-emit-contract`) → **ADR-134** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 133; the record's Status section carries the provenance note.
ADR-133 (`a-gate-ships-armed-by-a-committed-default`) → **ADR-134** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 133; the record's Status section carries the provenance note.
ADR-134 (`a-gate-ships-armed-by-a-committed-default`) → **ADR-135** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 134; the record's Status section carries the provenance note.
ADR-134 (`hook-dependency-integrity-invariant`) → **ADR-135** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 134; the record's Status section carries the provenance note.
ADR-135 (`hook-dependency-integrity-invariant`) → **ADR-136** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 135. This is the **third** move of that record, so its lineage is `133 → 134 → 135 → 136` and its Status section carries one provenance note per hop. Each hop was forced by a different sibling release merging ahead of it during a long-running run — the allocation rule behaving correctly under sustained contention, not drift.
ADR-135 (`hook-dependency-integrity-invariant`) → **ADR-136** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 135; the record's Status section carries the provenance note.
ADR-137 (`project-root-is-a-non-bin-sentinel`) → **ADR-139** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 137; the record's Status section carries the provenance note.
ADR-142 (`surface-1-emit-provenance-not-existence`) → **ADR-148** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 142; the record's Status section carries the provenance note.
ADR-142 (`block-destructive-022-governs-execution-capability`) → **ADR-144** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 142; the record's Status section carries the provenance note.
ADR-143 (`cross-domain-bridge-writes-are-not-symmetric`) → **ADR-149** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 143; the record's Status section carries the provenance note.
ADR-144 (`block-destructive-022-governs-execution-capability`) → **ADR-150** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 144; the record's Status section carries the provenance note.
ADR-142 (`word-boundary-matching-is-engine-parity-not-syntax`) → **ADR-146** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 142; the record's Status section carries the provenance note.
ADR-143 (`measurement-state-is-an-optional-merged-object-not-a-version-bump`) → **ADR-149** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 143; the record's Status section carries the provenance note.
ADR-144 (`token-registry-conformance-is-family-aware`) → **ADR-150** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 144; the record's Status section carries the provenance note.
ADR-145 (`anchor-resolution-is-a-surfacing-instrument-not-a-gate`) → **ADR-151** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 145; the record's Status section carries the provenance note.
ADR-146 (`word-boundary-matching-is-engine-parity-not-syntax`) → **ADR-152** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 146; the record's Status section carries the provenance note.
ADR-149 (`measurement-state-is-an-optional-merged-object-not-a-version-bump`) → **ADR-153** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 149; the record's Status section carries the provenance note.
ADR-150 (`token-registry-conformance-is-family-aware`) → **ADR-154** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 150; the record's Status section carries the provenance note.
ADR-143 (`provenance-is-the-eighth-label-group`) → **ADR-149** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 143; the record's Status section carries the provenance note.
ADR-144 (`checkpoint-b-second-axis-is-measured-not-declared`) → **ADR-150** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 144; the record's Status section carries the provenance note.
ADR-145 (`wave-width-is-a-second-checkpoint-b-output-not-a-verdict`) → **ADR-151** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 145; the record's Status section carries the provenance note.
ADR-146 (`dry-run-predicts-apply-asserts-mode-branch-placement`) → **ADR-152** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 146; the record's Status section carries the provenance note.
ADR-147 (`one-frontmatter-strip-bound-to-a-conformance-fixture`) → **ADR-153** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 147; the record's Status section carries the provenance note.
ADR-148 (`arm-e-population-is-the-directory-never-the-manifest`) → **ADR-154** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 148; the record's Status section carries the provenance note.
ADR-149 (`provenance-is-the-eighth-label-group`) → **ADR-155** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 149; the record's Status section carries the provenance note.
ADR-150 (`checkpoint-b-second-axis-is-measured-not-declared`) → **ADR-156** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 150; the record's Status section carries the provenance note.
ADR-151 (`wave-width-is-a-second-checkpoint-b-output-not-a-verdict`) → **ADR-157** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 151; the record's Status section carries the provenance note.
ADR-152 (`dry-run-predicts-apply-asserts-mode-branch-placement`) → **ADR-158** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 152; the record's Status section carries the provenance note.
ADR-153 (`one-frontmatter-strip-bound-to-a-conformance-fixture`) → **ADR-159** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 153; the record's Status section carries the provenance note.
ADR-154 (`arm-e-population-is-the-directory-never-the-manifest`) → **ADR-160** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 154; the record's Status section carries the provenance note.
ADR-142 (`release-plan-status-lifecycle`) → **ADR-151** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 142; the record's Status section carries the provenance note.
ADR-151 (`release-plan-status-lifecycle`) → **ADR-155** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 151; the record's Status section carries the provenance note.
ADR-155 (`release-plan-status-lifecycle`) → **ADR-161** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 155; the record's Status section carries the provenance note.
ADR-162 (`system-of-record-per-mirrored-element`) → **ADR-164** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 162; the record's Status section carries the provenance note.
ADR-163 (`split-predicate-gate-graduation`) → **ADR-166** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 163; the record's Status section carries the provenance note.
ADR-164 (`written-is-not-repo-derivable`) → **ADR-167** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 164; the record's Status section carries the provenance note.
ADR-165 (`bounded-by-relocation-not-by-discard`) → **ADR-169** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 165; the record's Status section carries the provenance note.
ADR-170 (`partial-supersession-is-a-reciprocal-frontmatter-edge`) → **ADR-172** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 170; the record's Status section carries the provenance note.
ADR-170 (`governance-states-standing-policy-not-past-tense-state`) → **ADR-174** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 170; the record's Status section carries the provenance note.
ADR-171 (`kind-projection-of-a-declared-category-row-is-one-classification`) → **ADR-175** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 171; the record's Status section carries the provenance note.
ADR-172 (`arm-posture-is-carried-by-the-emitter-not-the-mode-dial`) → **ADR-176** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 172; the record's Status section carries the provenance note.
ADR-173 (`lifecycle-audit-trail-carrier-or-retirement`) → **ADR-177** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 173; the record's Status section carries the provenance note.
ADR-174 (`instance-conformance-validator-surface`) → **ADR-178** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 174; the record's Status section carries the provenance note.
ADR-175 (`cross-boundary-key-form-for-entity-and-tracker-identifiers`) → **ADR-179** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 175; the record's Status section carries the provenance note.
ADR-170 (`work-item-kit-first-class-unit`) → **ADR-171** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 170; the record's Status section carries the provenance note.
ADR-171 (`work-item-kit-first-class-unit`) → **ADR-177** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 171; the record's Status section carries the provenance note.
ADR-177 (`work-item-kit-first-class-unit`) → **ADR-180** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 177; the record's Status section carries the provenance note.
ADR-177 (`output-scoring-runner-consumes-the-shipped-eval-harness-schema`) → **ADR-180** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 177; the record's Status section carries the provenance note.
ADR-178 (`behavioral-regression-floor-and-major-release-binding-boundary`) → **ADR-181** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 178; the record's Status section carries the provenance note.
ADR-180 (`output-scoring-runner-consumes-the-shipped-eval-harness-schema`) → **ADR-182** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 180; the record's Status section carries the provenance note.
ADR-177 (`adr-citations-bind-at-the-claim-not-at-authorship`) → **ADR-181** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 177; the record's Status section carries the provenance note.
ADR-181 (`behavioral-regression-floor-and-major-release-binding-boundary`) → **ADR-183** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 181; the record's Status section carries the provenance note.
ADR-181 (`automation-registry-is-the-gate-what-how-who-decoupled`) → **ADR-182** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 181; the record's Status section carries the provenance note.
ADR-182 (`automation-registry-is-the-gate-what-how-who-decoupled`) → **ADR-184** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 182; the record's Status section carries the provenance note.
ADR-177 (`command-position-is-a-canonicalizer-not-a-parser`) → **ADR-182** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 177; the record's Status section carries the provenance note.
ADR-182 (`command-position-is-a-canonicalizer-not-a-parser`) → **ADR-184** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 182; the record's Status section carries the provenance note.
ADR-184 (`command-position-is-a-canonicalizer-not-a-parser`) → **ADR-187** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 184; the record's Status section carries the provenance note.
ADR-185 (`pack-configurable-vs-platform-fixed-boundary`) → **ADR-188** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 185; the record's Status section carries the provenance note.
ADR-186 (`kit-content-provenance-key`) → **ADR-189** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 186; the record's Status section carries the provenance note.
ADR-187 (`pack-default-is-the-declared-kind-set`) → **ADR-190** by `release/tools/renumber-adr.py` at merge time, because the mainline already claimed 187; the record's Status section carries the provenance note.
...
**File:** [ADR-197-l3-judgment-criteria-evaluability.md](ADR-197-l3-judgment-criteria-evaluability.md)
...
**Status:** Accepted (ratified at the `authoring-bar-and-consumers` Collective Review scope-lock; the record itself was authored after Stage 7 under operator authorization and is owed a targeted re-review).
**Decision:** What a program reading a *resolved* type-pack kit may rely on is stated as a **three-tier contract partitioned by specification-presence, never by observed enforcement** — a property is guaranteed or specified-but-unenforced because a rule states it, and absent only where no rule states it at all, so a runner shipping or flipping posture re-derives the per-property enforcement column and moves nothing between tiers. What separates the first tier from the second is **what the specification is about**: the first is specified of the kind set a resolution delivers, the second of the declaration behind each resolved kind, and the resolver consults no second-tier key — so the first arrives resolved and the second arrives unconsulted, which is why they are read differently. On any second-tier violation a consumer emits **`NOT-EVALUATED` naming the kind, the key, and the denominator**, and renders no verdict for that kind; the denominator is required rather than advisory, because a caveat emitted for every kind is indistinguishable from a working consumer without it. The absent tier is not read at all. Two placement consequences are recorded rather than absorbed: the rule **extends the producer-side grammar section** rather than opening a consumer-side file, because one boundary in two files that must agree is the duplicate-source pattern that grammar declines by name; and the asymmetric kind-to-label limb is registered as a **second limb of an existing gate-coverage row**, stated as a **zero-instance invariant** rather than as a ratio, after the adversarial review showed the ratio's denominator was the population the probe could reach rather than the population the claim was about.
**Reversibility:** CHEAP (prose only — a boundary section, one register row extended in place, and this record; nothing mints an identifier, declares a resolution pointer, adds a register row, or changes a validator, pack or fixture, so no runner's behaviour and no gate's denominator depends on it).
**File:** [ADR-199-consumer-contract-is-tiered-and-unenforced.md](ADR-199-consumer-contract-is-tiered-and-unenforced.md)
...
**File:** [ADR-184-automation-registry-is-the-gate-what-how-who-decoupled.md](ADR-184-automation-registry-is-the-gate-what-how-who-decoupled.md)
