---
title: ADR-200 — Criterion evaluability is a per-check measured property, and it gates admission
status: Accepted
date: 2026-09-11
...
**Numbering provenance — `197 → 200`.** Held **ADR-197** branch-local; renumbered to **ADR-200** at merge time by `release/tools/renumber-adr.py`, because the mainline already claimed 197. In-release citations that read "ADR-197" denote this record.
...
|---|---|---|
| **Unrenderable** — a run cannot render the judgment: it declines, errors or emits no resolvable verdict, or the check quantifies over an entity its evidence set does not define (the rendering rule's first two questions, § Methodology) | The judgment codes to the no-evidence state, which rounds to failure, which blocks the transition | Admission makes the aggregate **structurally incapable of its clean verdict** for any work item whose evidence is absent |
...
**What is required to complete the measurement, stated so it is actionable rather than open-ended:** three context-isolated judgment runs over the 42 pair-members **recorded in Appendix A**, each run seeing every pair-member exactly once and none of A.4's arm labels, rendering per the ordered procedure in § Methodology, and emitting the run-report contract's header and table. Isolation is required *between runs of the same pair-member*, which three fresh contexts satisfy — it does not require a fresh context per judgment. The frame is enumerated, the instrument is recorded, and the reporting shape is fixed, **but the runs are not all that is outstanding.** A second round has since run against the recorded instrument and did not complete the measurement: the instrument as recorded cannot withhold the arm labels from a run, and several of its bindings declare conforming facts their evidence sets do not carry — § Limitations of the instrument as run. Completing the measurement therefore also needs a presentation that withholds the arm labels and a disposition of those bindings. How they are supplied is not decided by this record.

## Limitations of the instrument as run

Appendix A is recorded exactly as the second round of context-isolated runs read it — the round executed against the pinned instrument after the first round, recorded in § Results, had been defeated by its free-variable item. **It is not repaired in place.** A result can be read only against the instrument that produced it, and rewriting A.1 to A.4 now would leave that round's reports pointing at text this record no longer carries. The limitations below are therefore stated as properties of the instrument as it was run, and a further round run against it as recorded would meet every one of them. They are recorded after that round's results, which is why they sit here rather than in § Validity Threats: that table governs the results it was declared before.

**The second round ran unblinded, and the instrument's layout cannot blind a run.** A.1 specifies that a run is not given A.4's arm labels, because a rater shown which member conforms can return the specificity arm by construction. A.1 also places each member's own binding in A.4 inside its evidence set, and A.4 is the only place a violating member's literal text is recorded — beside the arm labels, the `substantive` / `contested` annotations and the `Conforming:` declarations. A run cannot construct a violating member without reading them, so the blinding A.1 specifies cannot be realized from this layout. A.5 declares an interleaved presentation order but records none, so nothing in the instrument keeps a conforming member from being judged beside its violating twin either. **The workspace owner weighed this before the second round ran** and chose a shared, pinned instrument with the limitation stated over a blinded presentation assembled by one orchestrating session, because that session would have been a single point of failure for the instrument's integrity. Every run of that round saw the arm labels and said so under A.1's own provision. Its results are therefore unblinded on both arms: the specificity arm on A.1's own account, and the violating arm because A.4 presents each delta as the violating member — a flip judged against that presentation shows that the delta is legible, not that the check discriminates. That round's verdict sets are not carried into this record, and `G` stays `NOT-EVALUATED`.

**The done-standard bindings declare facts their evidence sets do not carry.** A.3 records that `S2`'s second, fourth and fifth acceptance-criteria entries are not recorded as satisfied on any surface the evidence set can carry. The fifth — the convention is documented where a kit author will find it — is `CTX-DOD` element (a), yet the `dod-story-definition-of-done-met` binding declares that `CTX-DOD`'s three elements hold against `S2`'s record. Element (c), a recorded reviewer acceptance, is carried by no A.2 or A.3 element for `S2`, nor for `T2`, whose `dod-task-parent-done-standard` binding declares that `T2`'s delivered output meets `CTX-DOD`. Both conforming arms rest on declarations rather than evidence, so the completeness property § Methodology requires of an evidence set does not hold for them.

**Both control bindings declare facts no evidence element carries.** `kanban-dor-pull-policy-explicit` declares that `C1` satisfies both clauses of `CTX-SERVICE`'s pull policy, but `T1`'s block, which `C1` reads, records a size and no named owner, and the commitment point that clause (i) turns on is recorded only in the delta of the sibling `kanban-dor-commitment-point-reached` binding, outside this member's evidence set. `kanban-dod-exit-policy-satisfied` declares that `C2` satisfies both clauses of the exit policy, but `S2`'s block, which `C2` reads, records neither a customer acceptance nor the state of any defect. Read with A.1's evidence-set definition, which admits a member's own binding, these conforming arms hold by the author's declaration and the specificity arm measures nothing; read with member content alone, they rest on facts no element carries. The second round's runs resolved these arms by different readings, and each recorded the reading it took. Either way, the control pair — the only pair whose verdicts can resolve whether the operative axis is `level` or `automatable` — resolved nothing in that round.
...
**Instrument version token:** `ADR-200-INSTRUMENT-v1`. **Members:** the frame's checks × {`conforming`, `violating`} — as of the baseline, 42.
