...
---
title: "ADR-164 — A warn-mode declaration carries two axes; only the repo-derivable one can end a shakedown"
...
tags: [gate-efficacy, warn-mode, progressive-rollout, deploy-check, flip-decision-register, drain-evidence, ADR-163, ADR-106]
source_observations:
...
# ADR-164 — Written is not repo-derivable
...
**Numbering provenance.** Allocated at this Engineering commit as the next number above the union of the mainline anchor and this branch's own in-flight claim. `renumber-adr.py --detect` reported `ANCHOR 162 origin/main`, `NEXT-FREE 163`, and `CLAIMED-SET-BRANCH-ONLY 163 (detection only — never binds)` — the oracle is anchored on mainline and cannot see a sibling's unmerged claim, so `--next-free` alone would have collided with [ADR-163](ADR-163-split-predicate-gate-graduation.md) on this same branch. 164 was taken against the union and re-verified to report `BINDS`. A gap blocks the repo; a duplicate is tooled at merge, so the union is the safe side to err on.
...
- [`core/ADRs/ADR-163-split-predicate-gate-graduation.md`](ADR-163-split-predicate-gate-graduation.md) — the split-by-locus-of-input graduation path and the *blocker-not-schedule* disposition rule this record reconciles W2 against
