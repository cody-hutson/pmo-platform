...
# ADR-180 — Work-item kit as a first-class unit
...
Selection is **per kit class**, so a deployment selects at most one kit per class. Whether that is expressed as a class-keyed map or as one configuration key per class is a **configuration-surface decision deferred to the selection work at Stage 6** and deliberately not fixed here: the two candidate shapes were argued from opposite premises about cost, and the cost argument that favoured the map was falsified — a new key costs a schema entry under either shape. Under either shape a second class costs one configuration entry plus its schema row, and the resolver is unchanged. **That deferral is closed, and it closed inside this same release rather than a later one:** the selection slice rendered the one-key-per-class shape as the flat `[methodology].default_work_item_kit` — a single key naming a `role = "kit"` pack's `pack_id`, empty for no kit — not the class-keyed map. The field's contract lives in `core/schemas/platform-config-schema.md` (which cites this record for the selection rule) and its operator-facing surfaces are `core/config/operator.toml.template` and `docs/platform-config-reference.md`. The pointer is recorded here because a deferral whose resolution ships in the same release, and which names a Stage the release has since passed, otherwise reads on this surface as still outstanding.
...
- **The relaxation is asymmetric on the forward axis.** Widening `extends` to the kit role means a pre-existing validator asserting that only an archetype pack may extend would reject a now-valid kit pack. Theoretical **at the moment the relaxation was taken**, and measured rather than asserted: **before this change, no tracked executable validated against this grammar at all**. The relaxation therefore landed before any validator could be broken by it, and the **first** such validator ships in this same change, kit-aware from the start rather than retrofitted — which is the obligation the composition-grammar record transferred forward and this record discharges. The window in which this relaxation was free is now closed: a future relaxation of this grammar meets a live reader and must be analysed against it.
