...
tags: [records-management, retention, disposition, warn-mode, deploy-check, drain-evidence, byte-budget, least-destructive-disposition, reversibility-moderate, ADR-106, ADR-054, ADR-167]
source_observations:
...
- **Adopting the orphaned pre-relocation log is one-way, and it must claim the next free index — never `00000`.** It is a move of ~70 MB of operator-instance state that cannot land in a pull-request diff and cannot be rehearsed by reading one, which is why the release's dry-run stage stays uncompressed. **Rotation has since fired on the operator instance, so `00000` is a live segment holding 97,434,070 B of records; a `mv` onto that name overwrites them silently and irrecoverably — IRREVERSIBLE, not merely one-way.** The adoption therefore resolves its target index at the moment of the move, as the one above the family's current maximum read from `warn_log_segment_set()`. Because indices only ever increase and the orphan predates every row in the family, the adopted segment is an explicit **pre-anchor predecessor** — highest index, oldest records — and the family's ordering contract is index order **chronological from the anchor forward**, with that one declared exception. A consumer needing true chronology reads the rows' own timestamps and never infers age from the index. The orphan is **two** files, not one, and the merged family carries a **12-day instrumentation gap** that a drain must declare rather than read as twelve quiet days.
...
- [`core/ADRs/ADR-167-written-is-not-repo-derivable.md`](ADR-167-written-is-not-repo-derivable.md) — the sibling record in this same release. Its W1 axis (a drain-history criterion may be declared only where the sink is written) is the reason the read surface here is a shared symbol rather than a path, and its `INSTRUMENTATION-SUSPECT` vocabulary is what the merged family's 12-day instrumentation gap must be declared under
