...
An epic altitude row would therefore hardcode one methodology's kind into the neutral toolkit — **directly contradicting the card's own criterion forbidding hardcoded kinds.** The two criteria were mutually unsatisfiable as written. The corpus-canonical model is **one kind-agnostic traversal plus two projections** — a kind filter and a label filter. Only the *kind* projection is specified in the meta-schema; the **label filter is a sound derivation from it, not a specified surface** (a probe for it returns zero on both cited surfaces). Stated precisely here because an earlier draft read as though both projections were already specified, which would have made this record cite a surface that does not exist. The criteria are graded against that, satisfied by intent.
