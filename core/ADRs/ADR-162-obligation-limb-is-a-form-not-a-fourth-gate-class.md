...
status: Accepted — ratified by the operator at this release's Stage 9 Plan Review gate (milestone `declarations-have-a-firing-surface`, 2026-08-29, `gate-outcome/plan-review-go` verdict GO), and the flip is backfilled here. The flip is recorded in this file's own `status:` field, which is where it must be verified — a green close-out is not evidence the flip landed.
date: 2026-08-29
...
**Accepted.** Ratified at this release's Stage 9 Plan Review gate — the operator rendered GO for milestone `declarations-have-a-firing-surface` on 2026-08-29, recorded as a `gate-outcome/plan-review-go` row. The flip is **backfilled**: it was owed at that release's Stage 13 Close and did not land there. Per the established precedent the flip is verified against this file's own `status:` field and never assumed from a green close-out.
