...
**Positive.** One source of truth for one behaviour class, with a stated reason a third should not be authored. The trigger fires where the failure actually occurs and is falsifiable by a reviewer from the response text. Previously-unguarded declaring surfaces **within the rule's delivered reach** are covered without a per-surface edit each. The ad-hoc conversational turn is reached for the first time **in platform-engineering sessions**. And because the rule lives on the one corpus surface carrying a governed mirror contract, it can reach a running agent at all.
...
**A fourth — the reach is bounded to platform-engineering sessions, and the remainder is named.** The rule ships into `core/rules/`, and that directory is loaded on one routing branch only. [`CLAUDE.md.template`](../CLAUDE.md.template) § Routing places `.claude/rules/` under **Claude Code (platform engineering)**; the **Cowork (PMO operations)** branch routes to `OPERATIONS.md`, the active `PROJECT.md`, and a work area's `CONTEXT.md`, and never reads the rules directory. [`operating-model.md`](../disciplines/operating-model.md) § 2.3 states the same boundary from the skill side: operations-scoped skills **MUST NOT** read `core/rules/*` and do not load it. The exclusion is therefore at **session altitude**, not a gap in the rule's own text — no wording change to `analysis-mandate.md` can reach a session that never loads it.

The bounded remainder is consequently **every operations-scoped surface and every Cowork-routed session**. Measured at Stage-9 review, roughly half the declaring surfaces are out of reach — about ten of the nineteen are in it. The remainder is not hypothetical: the originating card's own Evidence field names one of its two motivating instances as a project-operations analysis request, which is precisely a session class this rule does not reach. Closing it requires a second carrier on the operations routing branch — an `OPERATIONS.md` clause, or a mirror contract for a surface that branch actually loads — and that is a separate decision, deliberately not taken here. What this record supersedes is the authoring of further **per-surface point fixes**; it does not claim coverage it cannot deliver.
