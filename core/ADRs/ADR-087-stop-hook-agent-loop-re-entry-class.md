...
- **Obligation 2, sharpened by the defect it predicted — the guard must be written on the FIRE path ONLY, and it must carry exactly one meaning.** The instance defect recorded above was resolved, and resolving it showed the original wording ("a sentinel keyed to the boundary, written before the decision") to be necessary but not sufficient. Three refinements now bind the class:

  1. **A guard written on the SKIP path freezes the predicate at the first turn.** Turn 1 is structurally the least substantive turn of any session, so a guard that records "evaluated" rather than "fired" decides the question at the worst possible moment and the session can never become eligible as it grows. The skip path must write **nothing**. Write-before-decide still holds — it is simply narrowed to the fire path, which is the only path the obligation ever concerned.
  2. **One record, one job.** The defect's root cause was not the sentinel; it was the *conflation* of two records — an anti-loop guard (which must be terminal) and a sampling memo (which must not be) — in one file. A re-entrant hook may keep per-turn progress state, but it must not be the same record as the fire guard. A progress tally is derived and re-derivable; a decision memo is not.
  3. **The harness already supplies the native re-entry flag** (`stop_hook_active`, true precisely when the agent is continuing *because* a `Stop` hook blocked). The platform hand-rolled a replacement for it, and the hand-rolled version is what acquired the second job. Prefer the native flag; if a hook keeps its own guard as well, that guard stays single-purpose.

- **A re-entrant `Stop` hook cannot observe session-terminality, and its documentation MUST name the crossing it actually fires at.** Every `Stop` is structurally identical; the last one in a session is identifiable only in hindsight. A hook that claims to fire "at the session boundary" is therefore claiming an unobservable — and that claim is what let code and documentation agree with each other while agreeing with nothing that could happen. The class obligation is honesty about the fire point: state the observable *threshold crossing* the hook fires at, and state the tradeoff that firing later risks never firing at all. This is the fifth standing obligation in substance; it is recorded here rather than renumbering the four above, which have shipped and are cited elsewhere.

- **The regression-test shape this ADR asked for is now a worked instance.** The multi-`Stop` sequence with the real payload shape is no longer only a stated requirement: the motivating capability ships one, driven from a payload fixture pinned against the live harness contract, with a negative control asserting the fixture does **not** carry fields the payload never sends. A future re-entrant hook has a pattern to copy rather than a paragraph to interpret.
