...
**Carry the deployed obligation as a new rule** — `core/rules/decision-time-adherence.md` — holding a bounded **Adherence Checkpoint Index** keyed on observable claim signatures. Each row binds a governing rule, a required ground-truth read stated as a method rather than a host command, and a locus. The rule lands in the rules directory because that is the only tracked corpus surface with a governed mirror contract to the running agent; the mechanism stays in the spec. One thing is carried on both surfaces on purpose — the trigger predicate itself, restated in the rule's `§ 1` so the deployed surface reads standalone. That duplication is registered as a per-domain prose exemption under the duplicate-source discipline, and its record — the necessity, why the byte-identity and complementary-pair registries cannot hold the pair, and the co-edit obligation — lives in the spec's `§ 1.0`, which owns the predicate.
