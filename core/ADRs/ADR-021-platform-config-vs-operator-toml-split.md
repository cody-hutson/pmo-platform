...
- **(B) Two files, relocating adapters out of operator.toml** — the Stage 5 spec's proposal: put the #703 seam at a new `platform-config.toml` and stop using operator.toml `[platform]` adapter selectors. Clean separation, but it RELOCATES adapter selection out of operator.toml — a deviation from ADR-017 §S2 (which names adapters as an operator.toml concern) and a breaking change for `deploy.sh`/hooks that read `[platform].work_board`.
...
3. **The legacy `[platform].work_board` / `comms_platform` fields are reconciled by ALIAS/deprecation, NOT removal.** `[platform].work_board` is superseded by `[adapters].ticketing` and carries a deprecation comment; both are kept because `deploy.sh` and `core/hooks/*.sh` read `[platform].work_board` today. When both are set and differ, `[adapters].ticketing` is authoritative. A future release MAY fold the alias once all readers migrate (a Track A governed change).
...
- **Breaking-change posture: additive-only.** `deploy.sh`/hooks keep reading the current `operator.toml` keys (including `[platform].work_board`); `delivery_approach` stays in PROJECT.md; the 5-rung resolver's fail-safe default-fallback means non-adopting consumers behave exactly as before; the #703 onboarding seam is stable.
