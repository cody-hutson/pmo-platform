...
Add a **Gate 0 — gate-eligibility precondition** at the head of the Information Sufficiency gate list (`hub-spoke-bridge.md` § Operating Principle: Decision Briefing), as a **literal "Gate 0"** that runs **before** the existing gates 1–5 — **without renumbering** them (the gates retain their numbers; all existing "gate 3/4/5" citations stay valid). Gate 0 is a MUST-pass classification: before issuing any operator gate, the hub classifies the candidate action against (1) the Stage-to-Autonomy-Tier mapping and (2) the Standing-GO Authorization list. An action whose effective **Autonomy Tier** is Tier 2 (Bounded Auto) or Tier 3 (Autonomous, within an approved-plan / Standing-GO scope), or a Standing-GO Tier-1 mechanical action after Stage 9 GO, is **NON-gate-eligible** — the hub executes it and reports the observable state change, it does not render a gate. Only the reserved genuine-judgment touchpoints — or a genuinely novel/ambiguous situation the framework does not resolve — are gate-eligible and proceed to gates 1–5.

**Where that reserved set is enumerated (amended).** This decision originally carried the list inline. It is now enumerated once, in the **Hub Gate Register** ([`orchestration-playbook.md`](/release/skills/release-hub/references/orchestration-playbook.md)), as the rows whose `Disposition` is `STOP` or `STOP-IF`; this record cites that register and states no count. The reserved set and the set of places the hub stops were never two facts — they are one table read on two columns, and maintaining them as separate prose lists across several surfaces is what let them drift apart.

**Reserved-set amendment (the substantive change, recorded rather than silently contradicted).** Two touchpoints that belong to the reserved set were absent from the list this record originally carried:

- **Early merge (Procedure 6).** An early merge puts pre-Stage-9 work on `main`. It is structurally the same class as Stage 12 Execute — an operator authorizing code onto the mainline — and its blast radius reaches every concurrently in-flight release. Its omission made this record's list incomplete; the surface that already treated early merge as a stop was right.
- **Action-item resolution (Procedure 7a).** Resolving an action item is judgment, and the attestation limb is irreducibly human: the hub cannot attest to the state of its own ledger without self-referential validation. The register states the disposition only and cites the action-item standard for the status predicate, so a change to that predicate requires no register edit.

This record is **superseded only on reserved-set membership**. Gate 0 itself, the no-renumber choice, the Tier disambiguation, the reversibility carve-out and the warn→enforce posture all stand unchanged — and the relocation of Gate 0's text into the deployed engagement contract is not a new decision but the restoration of **Alternative A's own intent** (a single pre-gate surface): the split that developed between the gate and its classification inputs was Alternative B's rejected fragmentation, arrived at accidentally by which files happen to be deployed.
