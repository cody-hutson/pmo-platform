...
status: Accepted — ratified at Stage 13 Close (Phase A13, the ratification beat that owns the `Proposed → Accepted` transition; gate row G-CL9 verifies) on 2026-09-03, under operator authorization. This field is the authority: the automated ratification-flip check is advisory-only and structurally cannot fail, so a green close-out is never evidence a flip landed — it must be read here, never inferred from milestone closure or from a review comment.
date: 2026-09-02
release: pda-decisions-and-conformance-baseline
deciders: "Stage 5 Solutioning spoke (options analysis, evidence-grounding, blast radius) + adversarial design review (predicate-soundness finding) + operator at Collective Review scope-lock (binding predicate-repair entry condition; ratification at Stage 13 Close) + Stage 6 Engineering spoke (authorship, repair-path selection)"
...
**Accepted** — ratified at Stage 13 Close (Phase A13, which owns the `Proposed → Accepted` performing beat; gate row G-CL9 verifies the flip) on 2026-09-03, under operator authorization. Recorded in this file's frontmatter `status:` field, which is where it must be verified.
