...
# ==========================================================================
echo "===== UNDERSCORE — a server id containing _ must not escape the surface gate ====="
# ==========================================================================
# `__` is the delimiter in mcp__<server>__<tool>, so a server id carrying `_` makes
# the split ambiguous. The prior `[^_]+` server class resolved that by not matching
# at all, and the tool escaped SILENTLY while carrying an in-scope verb and a comment
# noun. These arms are the sensitivity half: each FAILED before the class widened to
# `.+`, which is what makes them capable of failing rather than decorative.
#
# Server ids here are SYNTHETIC, per the CONTROL-3c rule below — the arms grade the
# SHAPE (one underscore, two underscores, an embedded double underscore), so any
# well-formed value exercises them identically and a real connector id would add only
# disclosure.
run "UND-1 one underscore in the server id → still reaches -001" \
  "$(p_mcp 'mcp__srv_two__add_comment' '{"comment":"**2026-09-05 — Stage 6 update**\n\nWork proceeded."}')" \
  enforce 2 "BLOCK-SEAM-SHAPE-001"
run "UND-2 two underscores in the server id → still reaches -001" \
  "$(p_mcp 'mcp__srv_two_three__post_comment' '{"comment":"## Sep 5, 2026 status\n\nDetail follows."}')" \
  enforce 2 "BLOCK-SEAM-SHAPE-001"
run "UND-3 embedded DOUBLE underscore in the server id → greedy split finds the verb" \
  "$(p_mcp 'mcp__Srv__Vendor___createComment' '{"body":"**2026-09-05 — Stage 6 update**\n\nWork proceeded."}')" \
  enforce 2 "BLOCK-SEAM-SHAPE-001"
run "UND-4 underscore server + CamelCase verb tail → -002" \
  "$(p_mcp 'mcp__srv_two__addCommentToRecord' '{"body":"Hi Dana — one question on the vendor date. Generated by the release agent, run 2 of 3."}')" \
  enforce 2 "BLOCK-SEAM-SHAPE-002"
# Specificity half. Widening the SERVER segment must not widen SCOPE: scope is
# decided by the verb set and the comment-noun gate, so every read-family and
# bodyless-write name stays out even when its server id carries underscores.
for undread in \
  'mcp__srv_two__list_document_thread_comments' \
  'mcp__srv_two_three__getConfluencePageFooterComments' \
  'mcp__Srv__Vendor___list_discussions' \
  'mcp__srv_two__get_discussion' ; do
  run "UND-GREEN read-family with an underscore server stays out of scope: ${undread##*__}" \
    "$(p_mcp "$undread" '{"pageId":"1"}')" enforce 0 "" "CLAUDE-HOOK"
done
run "UND-GREEN delete_comment with an underscore server stays out of scope" \
  "$(p_mcp 'mcp__srv_two__delete_comment' '{"commentId":"9"}')" enforce 0 "" "CLAUDE-HOOK"
run "UND-GREEN in-scope verb but NO comment noun stays out of scope" \
  "$(p_mcp 'mcp__srv_two__add_rows' '{"rows":[1]}')" enforce 0 "" "CLAUDE-HOOK"
