...
for req in "$SRC_FRAG" "$SRC_GHPL" "$SRC_DEPLIB" "$SRC_AWK" "$SRC_PRIM"; do
  if [ ! -f "$req" ]; then
...
# build_layout <dir> <mode> <awk:0|1> <primitive:0|1> <deplib: ok|stale|trunc>
build_layout() {
  local d="$1" mode="$2" awk="$3" prim="$4" deplib="$5"
