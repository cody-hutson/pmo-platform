...
# --- Repo-identity gate (#6115, #6200) — layer 4, AFTER the workspace-scope gate and BEFORE the
# `git check-ignore` / classification path below, so a foreign repo costs one rev-parse and not
# also a check-ignore. Precedence:
#   CLAUDE_HOOK_BYPASS -> master-enable -> SCOPE (workspace) -> REPO-IDENTITY (here) -> .mode -> rule
#
# WHY THIS IS HERE AND NOT IN lib/scope-guard.sh. That lib's header states the boundary as design
# intent: "Scope is decided by the session's working directory, not by repository identity."
# Identity is deliberately outside its contract, and every other consumer of the scope lib correctly
# wants workspace scope: block-egress.sh and block-gh-path-leak.sh MUST keep firing in a nested
# repository, which may be public. Only THIS hook's rule is specific to this repository's layout,
# so only this hook asks the identity question here. The scope contract is untouched.
#
# MEMBERSHIP, NOT PATH SHAPE — through the SHARED helper. The identity question is repository
# membership, answered by lib/platform-membership.sh, the one implementation that
# block-autonomy-ceiling.sh's governance and cross-domain floors also call. A linked worktree of the
# platform inside the workspace root is recognized. A foreign repo nested inside a platform
# worktree is not (nearest tree wins). The walk is content- and branch-independent, so a checkout
# before its first commit is still recognized, and it needs no git. It takes a PHYSICAL directory,
# so the working directory is resolved first with a builtin `cd -P`.
#
# FAIL DIRECTIONS — two axes, and only one is inverted.
#   1. IDENTITY axis (INVERTED): a working directory the helper does not prove a member of the
#      platform (not a member, or undeterminable) -> exit 0, inert. That is the fix. Its failure
#      mode is under-enforcement in a repo the rule never described.
#   2. ANCHOR axis (NOT inverted, identical to lib/scope-guard.sh clause 2): if no platform
#      repository resolves at the anchor, or the helper is missing, this gate ABSTAINS and the hook
#      keeps its existing .mode/rule path. Inverting this axis would make a single `rm -rf` of the
#      platform checkout a silent, total kill switch for BLOCK-DRAFT-001, which is the exact shape
#      that clause forbids.
#
# OUT OF ROOT: a platform worktree OUTSIDE the governed workspace root never reaches this gate —
# the scope gate above exits first — so BLOCK-DRAFT-001 does not apply there. That is the
# documented verdict (lib/scope-guard.sh "COVERAGE CONSEQUENCE"), not a recognition failure.
#
# ACCEPTED RESIDUAL: a platform clone at a different basename, or a checkout placed outside the
# workspace root, is not the anchor, and BLOCK-DRAFT-001 stops applying there. That is
# under-enforcement on a warn-posture workflow rule: recoverable, with no public surface. ---
readonly PLATFORM_MEMBERSHIP_LIB="${HOOK_DIR}/lib/platform-membership.sh"
if [ -r "$PLATFORM_MEMBERSHIP_LIB" ]; then . "$PLATFORM_MEMBERSHIP_LIB" 2>/dev/null || true; fi
if command -v platform_membership_anchor >/dev/null 2>&1 \
   && command -v platform_membership_of >/dev/null 2>&1; then
  _bdf_anchor="$(platform_membership_anchor)" || _bdf_anchor=""
  # Anchor axis: unresolvable -> abstain (fall through to the rule), never a kill switch.
  if [ -n "$_bdf_anchor" ]; then
    _bdf_cwd="$( ( cd -P -- "$CWD" 2>/dev/null && pwd -P ) 2>/dev/null )" || _bdf_cwd=""
    _bdf_rc=0; platform_membership_of "$_bdf_cwd" "$_bdf_anchor" || _bdf_rc=$?
    # Identity axis (inverted): anything but a proven member -> inert.
    if [ "$_bdf_rc" != 0 ]; then exit 0; fi
  fi
fi
