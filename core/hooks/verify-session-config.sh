...
# (the canonical [spoke_runtime] model/effort surface — ADR-095 — this hook is its
...
#            canonical `platform-config.toml [spoke_runtime]` surface (ADR-095) the
...
#            ADR-095 §Decision point 2) — the expected constant IS the directive.
#
...
# no-shadow-SSOT per ADR-095). The expected-effort constant IS the directive.
...
# --- canonical [spoke_runtime] reader (ADR-095 / #340). Reads the SAME field
...
[ -n "$expected_default" ] || expected_default="opus"   # resolver Rule-2 consumer-fallback (ADR-095 / Check 27)
