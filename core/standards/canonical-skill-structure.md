...
- **Soft ~500-line authoring guideline (`pmo-skill-editor`) — ADVISORY.** The "keep SKILL.md under 500 lines; refactor detail to reference docs if exceeded" heuristic in `pmo-skill-editor` is an authoring aid, not a gate. It expresses a default preference for compact bodies; it caps nothing, and the soft guideline has no regression-check surface of its own — the mechanical assertion is `deploy.sh --check` Check 6, which enforces the `references/` requirement rather than a line ceiling.
