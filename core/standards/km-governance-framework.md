...
| **Tier 2 — Frontmatter cache** | K1 reference doc YAML frontmatter `owner:` field | NO (mirrors registry; for in-context discoverability) | A subset of K1 reference docs carries `owner:` frontmatter; the rest are deferred, with backfill out of scope for this release (separate Issue + plan for a future release). No count is restated here — it drifts as docs are amended. Derive the live set with `grep -rl "^owner:" core/ --include="*.md"` |
