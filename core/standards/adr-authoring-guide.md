...
| Reconciling the `## Status` body restatement to the frontmatter `status:` field | [`adr-schema.md` §3](../schemas/adr-schema.md) defines the body section as **restating** `status:`, so the frontmatter is the value-bearing surface and the body is a projection of it. Repairing a projection that no longer matches its source records what the decision already contained — the record's status is the same before the edit and after it, so nothing about what was decided moves. **Direction is load-bearing:** only body → frontmatter is hygiene. Editing the frontmatter `status:` *value* is forbidden-list item 4, and editing it toward `Proposed` would silently de-ratify a decision the operator already ratified. A now-false forward-looking clause ("flips to Accepted at …") is removed as a rotted anchor under the first row above; no ratification date, authority, or gate absent from the record is ever introduced. |
