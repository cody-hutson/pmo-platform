...
- **Output encoding.** Untrusted data is encoded for the **sink context** it lands in — HTML body vs. inline `<script>` vs. attribute — using the DOM-safe primitive (`textContent` / `createElement` over `innerHTML`); serializing data with `json.dumps` straight into an inline `<script>` block is a defect because the serializer does not escape the `</script>` sequence. **Design-consumption note:** a design that emits untrusted data into a markup/transport sink is checked for context-appropriate encoding at the sink. The deeper context-aware output-encoding standard and its semgrep lint are **in flight** (S1, v3.73 build-security-hardening) and become the named enforcer for this concept and for the charter's Security × Skills cell. **Source:** `[PLATFORM: eval-viewer XSS remediation · GHSA-rw36]`.
