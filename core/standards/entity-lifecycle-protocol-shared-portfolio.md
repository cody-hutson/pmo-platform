...
**Establishing issue:** #1155 (shared+portfolio entity-lifecycle transition protocol)
**Architectural basis:** the Two-Axis Entity Lifecycle ADR (RATIFIED). This protocol transcribes (does **NOT** redesign) the FROZEN Axis-1 machines in `core/disciplines/project-entity-model.md §4` (#10–#17). State sets are **verbatim**; any change requires reopening the establishing issue via a Tier-2 SCOPE CHANGE.
**Co-lock:** the project-scoped tier (entities #1–#9 + #18) is the format-identical companion `core/standards/entity-lifecycle-protocol.md` (#156). Both docs use the identical 5-column transition-table format defined in §2 — this doc is **co-located in `core/standards/`** with the project-scoped protocol per the operator directive.

---

## §1 Purpose & Boundary

This protocol is the tier-(b) companion to the project-scoped protocol: it states, **per shared + portfolio entity, who triggers which Axis-1 transition on what evidence with what side-effect, and what is forbidden.** It wires the **8 already-FROZEN Axis-1 machines** (`project-entity-model.md §4`, roster #10–#17) into a per-entity transition protocol.

**Scope — 8 shared + portfolio entities**, partitioned by `storage_tier` so no entity straddles the #156 (project-scoped, 10 entities) / #1155 (shared+portfolio, 8 entities) boundary:

- **Cross-project-shared** (`storage_tier: cross-project-shared → _pmo/`): Person (#10), System (#11), Vendor (#12).
- **Portfolio-level** (`storage_tier: portfolio-level → projects/_config/`): Portfolio (#13), Program (#14), Cross-Project Dependency (#15), Cross-Project Resource Conflict (#16), Strategic Initiative (#17).

**This is transcription, not design.** Every state set is copied verbatim from `project-entity-model.md §4` Axis-1 lines (#10–#17); every triggering agent from §6 the owning-agent matrix (the *Maintains* column fires lifecycle transitions; the *Creates* agent fires only the create-entry transition); every side-effect cites only frozen §5.1 chains.

**Cross-references:** `project-entity-model.md` §4 (Axis-1 machines #10–#17) / §5.1 (directed chains) / §6 (owning-agent matrix); `lifecycle-states-canonical.md` §2 (the `<Object>-<State>` convention) / §3 (the registration target).
...
**Format-identity contract:** this header row is **byte-pattern-identical** to the header in `core/standards/entity-lifecycle-protocol.md`. Both docs share this exact 5-column schema (the #156 ∥ #1155 co-lock); skill integration (#1156) consumes the two table sets uniformly.
...
States are **VERBATIM** from `project-entity-model.md §4` (#10–#17). Triggering agent = the §6 *Maintains* column (intermediate transitions) / *Creates* column (the create-entry row). Side-effects cite only frozen §5.1 chains.
...
|---|---|---|---|---|
| (none) → `active` | project-initiator / file-router (create) | Person record created in `_pmo/` (new global identity); `full_name` / `person_id` / `primary_role` supplied (§4 #10) | — (identity entity; no outbound frozen §5.1 chain) | valid |
...
| From → To | Triggering agent | Qualifying evidence | Side-effects | Marker |
|---|---|---|---|---|
| (none) → `active` | ppm-agent (create) | System record created in `_pmo/` (ERP / data platform / integration bus etc.); `system_name` / `system_id` supplied (§4 #11) | — (no outbound frozen §5.1 chain; read by `pmo-technical-analyst`) | valid |
...
> **DOWNSTREAM CROSS-LINK (Stage 6 / #202):** entity **#11 System** (`active → deprecated → retired`) is the lifecycle machine the per-project-CMDB / skills-as-CIs work (#202, restored to full Artifact-Register + skill-CMDB scope, single-registry) reuses to model skill Configuration Items. The #202 Stage 5 / ADR-036 (reconciling ADR-035 / ADR-019 / ADR-007 under the single-skill-registry constraint) binds to THIS frozen machine rather than minting a parallel CI-state set. Flagged, not designed here (out of #1155 scope).
...
| From → To | Triggering agent | Qualifying evidence | Side-effects | Marker |
|---|---|---|---|---|
| (none) → `active` | ppm-agent (create) | Vendor record created in `_pmo/` (external supplier identity); `vendor_name` / `vendor_id` supplied (§4 #12) | — (no outbound frozen §5.1 chain; read by `change-management`) | valid |
...
### §3.6 Cross-Project Dependency (#15)

Axis-1 (verbatim §4): `open → satisfied | broken | waived` · creates `ppm-agent` · maintains `ppm-agent` · Axis-2: Living (B)

| From → To | Triggering agent | Qualifying evidence | Side-effects | Marker |
|---|---|---|---|---|
| (none) → `open` | ppm-agent (create) | A directed cross-project dependency detected; `dependency_id` / `from_entity_ref` / `to_entity_ref` typed refs set (§4 #15) | outbound `DEPENDS_ON` (§5.1 chain 13 Cross-Project Dependency→Milestone `many:many`) — the dependency points at the Milestone(s) it gates | valid |
...
### §3.7 Cross-Project Resource Conflict (#16)

Axis-1 (verbatim §4): `detected → acknowledged → resolved` · creates `delivery-engine` · maintains `delivery-engine` · Axis-2: Living (B)

| From → To | Triggering agent | Qualifying evidence | Side-effects | Marker |
|---|---|---|---|---|
| (none) → `detected` | delivery-engine (create) | ≥2 projects over-claim one Person; `conflict_id` / `person_id` / `competing_project_ids` (≥2) set; `over_allocation_pct` computed (§4 #16) | outbound `RELATES_TO` (§5.1 chain 14 Cross-Project Resource Conflict→Person `many:1`) — the conflict points at the contended Person | valid |
...
| From → To | Triggering agent | Qualifying evidence | Side-effects | Marker |
|---|---|---|---|---|
| (none) → `proposed` | ppm-agent (create) | A portfolio-level strategic thrust drafted; `initiative_name` / `initiative_id` / `sponsor` set (§4 #17) | — (proposed initiatives have not yet generated programs) | valid |
| `proposed` → `active` | weekly-status-rollup | **Human ratification** (Axis-2 Hybrid C — agent-drafted, human-ratified): operator approves the initiative to proceed | outbound `GENERATES` (§5.1 chain 16 Strategic Initiative→Program `1:many`) — an active initiative MAY generate `Program-active` records (the create-entry of #14) | valid |
| `active` → `completed` | weekly-status-rollup | Evidence the strategic outcome is achieved | terminal; generated Programs (chain 16) follow their own #14 machine independently | valid |
| `active` → `cancelled` | weekly-status-rollup | Operator decision to cancel the initiative | terminal; generated Programs (chain 16) are NOT auto-cancelled — each follows its own #14 lifecycle | valid |
...
Side-effects reference **ONLY** frozen `project-entity-model.md §5.1` chains. Of the 8 entities, only #13–#16 are sources or intra-tier targets of frozen chains; Person / System / Vendor (#10–#12) have **no outbound §5.1 chain** (identity / reference entities — targets of inbound chains 10/11/12/14 only).
...
**Non-cascade discipline (to pre-empt phantom cascades in #1156):**

- **Identity entities (#10/#11/#12)** emit a **referential-integrity FLAG** on inbound dependents (chains 10/11/12/14), never an auto-cascade. Deactivating a Person does not auto-mutate its ASSIGNED_TO / RELATES_TO dependents.
- **`StrategicInitiative-cancelled` does NOT auto-cancel generated Programs** — each generated Program (chain 16) follows its own #14 machine independently.
...
- For each of the 8 entities (#10–#17): its verbatim §4 Axis-1 state set appears, and a transition table with ≥2 valid rows (incl. the create-entry) is present.
...
- Side-effects cite only §5.1 chains 13/14/15/16; identity entities (#10/#11/#12) carry referential-integrity flags, not outbound cascades.
