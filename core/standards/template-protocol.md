...
| `domain` | YES | enum | `project` \| `software` \| `platform-internal` | Three-domain classification per [`template-taxonomy.md`](template-taxonomy.md) §2; informs P5 path (c) eligibility. **Which `domain` is this?** Concept 5, template-provenance, per `core/specs/domain-token-registry.md`. It classifies the **template structure**, not any instance made from it — the artifact-provenance `domain` in `frontmatter-schema.md` § Category 6 is a different concept that is *also* called a "three-domain classification" and *also* appears under `operations/templates/`. The two are separated by the presence of the sibling key `artifact_type: template` in the same provenance header, not by value and not by path. |
...
| `canon_compat` | YES | enum | `plugin-aligned` \| `PMO-extension` \| `none` | P5 gate evaluation result. Path `none` is permitted for a `domain: project` template with no Anthropic plugin counterpart (path c-i), and for a `domain: software` template whose `canon:` names an external canon and which has no plugin counterpart (path c-ii) — per §6 P5 path (c). |
...
**AC4 verification clauses (per template format).** Verify by locating the provenance block, then reading its `review_status:` — do NOT grep for a literal `review_status: APPROVED`. No template in the registry currently holds `APPROVED` (P3 is unmet registry-wide, per §7.1), so an `APPROVED`-literal grep returns empty and cannot distinguish "no header" from "header present at an earlier lifecycle state".

- For markdown templates: `grep -l '^artifact_type: template' operations/templates/*.md` locates every template carrying the header; `grep -H '^review_status:' operations/templates/*.md` reads the state each one holds.
- For CSV templates: the sibling provenance file per §4.4 — locate it beside the `.csv`, then read its `review_status:` the same way. (The sibling-file naming in this section describes the specified convention; the live sibling files use a different suffix. Reconciling that drift is out of this section's scope and is tracked separately.)
...
**P5. Upstream-compatibility resolution.** Either:
  ...
  - **(c) none**: EITHER **(c-i)** the template's `domain:` is `project` AND no Anthropic plugin counterpart exists for the artifact family (e.g., RAID Log, Status Report, KT-Onboarding), OR **(c-ii)** the template's `domain:` is `software` AND its required `canon:` field names an external best-practice canon (e.g., IETF RFC 7322 for the RFC family) AND no Anthropic plugin counterpart exists for the family. In both cases the Anthropic-plugin upstream-compat dimension does not apply. Under (c-ii) the promotion evidence is the populated `canon:` anchor — the external canon the template conforms to — which is the software domain's equivalent of the plugin-convention alignment that path (a) supplies; a software-domain family is therefore never stranded without a satisfiable path merely because no plugin implements its canon.

The `canon_compat:` field in the template provenance header (per §4.1) records which path (a/b/c) applies. P5 verification = `grep canon_compat: operations/templates/<file>` returns one of three permitted values; paths (a) and (b) require the cited evidence to be machine-locatable. Path (c) is permitted when EITHER `domain: project` AND no Anthropic plugin counterpart exists (c-i), OR `domain: software` AND the `canon:` field names an external canon AND no Anthropic plugin counterpart exists (c-ii) — plugin availability per L1 [`template-taxonomy.md`](template-taxonomy.md) §6, re-verified at promotion time. Together with (a) and (b), every §6 artifact family has a satisfiable path: a plugin-backed family resolves (a); a project-domain family with no plugin resolves (c-i); a software-domain family whose canon is an external document with no plugin counterpart resolves (c-ii); and (b) remains the operator-approved extension path for a template that deliberately diverges from an available convention.
