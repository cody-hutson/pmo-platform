...
| Registry parity | **Out of Check 44's scope, and asserted elsewhere rather than left unasserted.** Check 44 reads §4 as its registry; it does not ask whether §4 agrees with the two config registries. Leg **C70d** of `core/deploy/tools/check-operator-toml-schema.sh` asserts that three-way agreement (§4 table ↔ `operator.toml.template` ↔ `operator-toml-schema.json`), in every direction, at deploy time and at pull-request time. |
...
§4's one gateable obligation — that a **newly authored** angle token carries a table row in the same change — is a property of a diff, not of a working tree. It is **still not enforced at deploy time, and this spec does not claim otherwise**; a stored list of tolerated tokens was considered for that job and rejected, because such a list is editable by the very change it would gate.

**It is now enforced where a diff exists.** Leg **C70e** of `core/deploy/tools/check-operator-toml-schema.sh`, invoked with `--diff-base <ref>` from the `operator-toml-schema` job in `.github/workflows/repo-integrity.yml`, reads the pull request's added lines and fails when an angle token that is **absent from the corpus at the base commit** appears in them without a §4 row added in the same diff. Three properties bound it, and each is a deliberate narrowing rather than an omission:

- **It fires only on genuinely new tokens.** A diff that adds another reference to a token already in the corpus is exactly the sanctioned incremental use §4's Convention scope describes, and firing on it would fail correct work — the failure mode a uniform closed-set rule was rejected for. The base-commit absence test is what separates the two.
- **It is ENFORCING, not BLOCKING.** A finding exits non-zero and turns the job red. Whether a red job blocks a merge is decided by branch-protection required-context registration, which is repository configuration and not readable from this tree. That registration is a separate operator action; until it is made, the arm reports without stopping a merge.
- **It sees only what a pull request presents.** Administrator enforcement being off, a direct push to the default branch opens no pull request, so no pull-request-time gate runs at all — a property this arm shares with every other job on that workflow. It is recorded here rather than papered over.
...
**The `X` in the two lines flanking this one is a schema metavariable, not a token.** Both lines carry the `depersonalization-token: allow` marker for that reason: the conformance gate's registry read is table-scoped, so the metavariable never registers, and without the marker its literal prose occurrences would surface in the gate's un-codified inventory as though a token by that name existed.

**Canonical defaults:** Most operator-instance paths default to subpaths under `${CLAUDE_WORKSPACE_ROOT}/pmo-instance/`, which is `.gitignored` per the public-repo discipline. The default suffix follows the token stem — `<OPERATOR_INSTANCE_X_PATH>` defaults to `${CLAUDE_WORKSPACE_ROOT}/pmo-instance/<x>/`. **Exceptions to the stem rule are the table's business, not a prose aside**: every row whose canonical default does not follow the stem — the `.claude`-nested family, the workspace-root siblings, the two-segment stems, and the two in-repo homes — carries its exception in its own **Codified by** cell, marked `stem exception`. Reading the table is therefore the whole answer; this paragraph states the default rule and points at the table for every departure from it. <!-- depersonalization-token: allow — the X is a schema metavariable standing for any token stem, not a token -->
...
| `<OPERATOR_INSTANCE_CLAUDE_MD>` | `${CLAUDE_WORKSPACE_ROOT}/CLAUDE.md` | `[paths].operator_instance_claude_md` | [`../hooks/block-destructive.sh`](../hooks/block-destructive.sh) **BLOCK-DESTRUCTIVE-019**, whose Layer-1 anchored arm spells this path as `"${PRIMARY_ROOT}/CLAUDE.md"` against `PRIMARY_ROOT="${CLAUDE_WORKSPACE_ROOT:-$HOME/Claude}"` — the workspace charter itself, referenced corpus-wide as the Tier-1 Director context file (**stem exception**: the leaf is a file, not a directory) |
| `<OPERATOR_INSTANCE_CLAUDE_DIR>` | `${CLAUDE_WORKSPACE_ROOT}/.claude` | `[paths].operator_instance_claude_dir` | the agent-config root the deployed rules mirror, hooks and mode files live under — consumed by [`doc-link-maintenance.md`](../rules/doc-link-maintenance.md) § Allowlist (`<OPERATOR_INSTANCE_CLAUDE_DIR>/skip-doc-link-check.txt`) and by the § Canonical-defaults prose above, which names this token as the worked **stem exception** |
| `<OPERATOR_INSTANCE_CLAUDE_SETTINGS>` | `${CLAUDE_WORKSPACE_ROOT}/.claude/settings.json` | `[paths].operator_instance_claude_settings` | §3 item 7 of this spec (the managed settings file regenerated from `core/settings.json.template` + operator.toml) and [`../disciplines/operating-model.md`](../disciplines/operating-model.md), which names it in **BLOCK-DESTRUCTIVE-019**'s Layer-1 primary-write set (**stem exception**: a file leaf under the `.claude` directory) |
| `<OPERATOR_INSTANCE_WORKTREES_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/.claude/worktrees` | `[paths].operator_instance_worktrees_path` | [`../rules/git-workflow.md`](../rules/git-workflow.md) § Primary Checkout Discipline, which spells the worktree home as `${HOME}/Claude/.claude/worktrees/[name]/` — the sole surface that fixes this leaf (**stem exception**: nested under `.claude`, not under `pmo-instance/`) |
| `<OPERATOR_INSTANCE_SKILLS_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/.claude/skills` | `[paths].operator_instance_skills_path` | the workspace charter's Layer-2 classification row (`.claude/skills/` — the skill **deployment target**, git-ignored) and [`../rules/skill-deployment.md`](../rules/skill-deployment.md) (**stem exception**: nested under `.claude`) |
| `<OPERATOR_INSTANCE_PROJECTS_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/projects` | `[paths].operator_instance_projects_path` | [`../deploy/lib-instance-path.sh`](../deploy/lib-instance-path.sh) `pmo_operations_path_for()` → `${_base}/projects`, the single resolution site for the operations-workspace root (**stem exception**: a workspace-root sibling, per the § Why-the-home-sits-at-the-workspace-root clause above) |
| `<OPERATOR_INSTANCE_PROJECTS_CONFIG_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/projects/_config` | `[paths].operator_instance_projects_config_path` | the same `pmo_operations_path_for()` resolver plus the charter's Governance File Map `projects/_config/` row (the program-scoped operational-config home — PORTFOLIO.md / SESSION_STATE.md / CORRECTIONS.md / SWAP_HANDOFF.md) |
| `<OPERATOR_INSTANCE_EVALS_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/pmo-instance/evals` | `[paths].operator_instance_evals_path` | [`review-composition-framework.md`](review-composition-framework.md) § Calibration Ledger, whose sites address `<OPERATOR_INSTANCE_EVALS_PATH>/judge-calibration/` — a **sibling** of `evals/results`, so the codified `<OPERATOR_INSTANCE_EVALS_RESULTS_PATH>` row above cannot express it and this is a distinct token rather than a variant of it |

**Convention scope:** Other angle-bracket tokens currently in use across the corpus (`<OPERATOR_INSTANCE_ENGINEERING_RULES_PATH>`, `<OPERATOR_INSTANCE_RELEASES_PLANS_PATH>`, etc. — the current set is live-derived by Check 44's arm (d) inventory, which reads this table and the git index rather than any stored list) inherit the same resolution-rule convention even when not yet codified in the table above. Codification is incremental — when a token's consumer (skill, hook, script) lands or changes, the token's row is added to the vocabulary table. Authoring NEW angle-bracket tokens MUST add a row to this table in the same PR — an obligation now enforced at the pull-request surface by leg **C70e** (see §1.5). <!-- depersonalization-token: allow — illustrative naming of two currently-un-codified tokens, prose not rows; the clause exists to describe the un-codified class and must be able to name a member of it without becoming a member of its own inventory -->

**Registry parity.** The table above is one of **three** surfaces that must agree about this vocabulary: this table (the registry Check 44 reads), `core/config/operator.toml.template` (the operator-facing prose), and `core/config/operator-toml-schema.json` (the machine declaration). Leg **C70d** of `core/deploy/tools/check-operator-toml-schema.sh` asserts all three carry the same key set, in every direction, so a row added here without its `[paths]` counterpart — or a counterpart added without a row — fails rather than drifting. This is a **registry-to-registry** assertion, where closure genuinely holds; it is not a claim that corpus *usage* is closed, which the Convention-scope clause above expressly denies.
