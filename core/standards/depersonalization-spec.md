...
| `<OPERATOR_INSTANCE_EVALS_RESULTS_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/personal/pmo-instance/evals/results` | `[paths].operator_instance_evals_results_path` | [`pipeline-event-log-schema.md`](../../release/references/standards/pipeline-event-log-schema.md) (pipeline event-log writer/reader + release-analytics tools — `append-pipeline-event.sh` / `query-pipeline-event.sh` / `compute-cycle-time.sh` / `synthesize-release-learnings.sh`, plus `deploy.sh` Check 19, which resolves through `lib-instance-path.sh`'s `pmo_evals_results_path()`; two-segment `evals/results` stem documented here per the stem-exception clause above) |
| `<OPERATOR_INSTANCE_FINOPS_STORE_PATH>` | `${CLAUDE_WORKSPACE_ROOT}/personal/pmo-instance/finops` | `[paths].operator_instance_finops_store_path` | [`finops-usage-store-schema.md`](../schemas/finops-usage-store-schema.md) §Store Location (the Agent-FinOps runtime token-spend store — per-session/per-subagent usage records extracted by the `finops-usage-extractor` skill; git-ignored operator-instance data carrying `gitBranch`/`cwd`/work-item identifiers, the derived-cache variant of the tracked-schema / git-ignored-instance split — no `.template` seed) |
