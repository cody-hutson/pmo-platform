...
**Mechanism:** Check 14 logs findings via `flag_warn_or_issue` to the shared deploy-check warn log (`deploy-check-warn-log.jsonl`, under the pmo-instance path) and exits 0. The check does NOT block deploy during the shakedown window.
...
| Threshold | Action |
|---|---|
| **2-3 releases post-merge** | Operator-driven review of the shared `deploy-check-warn-log.jsonl`; broken-ref backlog drainage (F-4) progress assessed |
