...
| **Customizable** | Mixed (template + tokens) | No (one-shot resolution) | Token-substitute → write to runtime location | Regenerate from current template + current operator.toml |
...
The **two new behaviors** introduced by this spec are the Customizable update behavior and the entire Composition-surface category. These are the gaps the durability work this spec introduces closes. Pre-spec, the package shipped install-time mechanisms but no update-time mechanism, forcing operators to manually merge on `git pull` or re-install (losing customizations).
...
- The package manifest (for Composition-surface and Customizable files — declared in `setup-workspace.sh` `COMPOSITION_SURFACE_FILES` / `CUSTOMIZABLE_FILES` arrays).
...
### §2.2 Markdown files (CLAUDE.md, README extensions)
...
- The **entire file is regenerated** on update from template + operator.toml. Operator customization for JSON-format runtime files flows through:
  - The user-scoped `~/.claude/settings.json` (Claude Code's existing surface for user-global config).
  - New fields added to operator.toml (which the template then resolves into the regenerated settings.json).
