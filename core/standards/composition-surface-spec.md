...
The authoritative manifest is the `COMPOSITION_SURFACE_FILES` array in [`core/deploy/composition-surface-manifest.sh`](../deploy/composition-surface-manifest.sh) (sourced by `setup-workspace.sh` and `update.sh`). The categories of file currently in this surface are the per-domain allowlists (`core/config/allowlists/*.txt`), the hub-state schema templates (`release/releases/hub-state/*.template`), the workspace-root charter template `core/CLAUDE.md.template` as of ADR-122, and the operations-workspace context anchor `operations/CLAUDE.md.template` — a pointer-only file naming the platform governance an operations-rooted session loads, restating none of it. Like every composition-surface source file, these templates carry NO fences in source — install-time composition (§3.1) wraps their content in the MANAGED SECTION fence and appends the empty OPERATOR ADDITIONS section the operator then extends.

**A registered row must name a target some reader resolves.** This is the invariant a row exists to satisfy. Registering a file installs a copy, and an installed copy no code path reads is not a configuration surface but a decoy: it ships an empty OPERATOR ADDITIONS fence inviting edits that then take no effect, and the operator has no way to tell the difference from the file alone. Name the resolving reader when adding a row. **Enforcing** this invariant mechanically is deliberately not built here — it is the durable fix for a defect class spanning several rows rather than a property of any one of them, and it is tracked as its own work item.

**Category determination — `core/config/platform-config.toml.template` is Universal, not Composition-surface.** It was registered as an `instance`-tier row and is no longer. Measured: every reader resolves the template **in place from the clone** — repo-relative in `deploy.sh`'s `resolve_platform_config` and in `release/tools/lib/platform-toggle.sh`, `${HOOK_DIR}`-relative in `core/hooks/verify-session-config.sh` — and **no** reader resolved the installed instance-tier copy. That is the Universal row's contract verbatim (*read in-place from clone; `git pull` updates source; no separate install or update step*). It is not the Composition-surface contract, which requires the operator to extend a **runtime** copy through a preserved fence — a behavior with no reader, hence no effect. Registration was writing a further full copy of a ~23 KB template that answered to no rung of the resolution cascade. ADR-022's subject — the security/access split between `platform-config.toml` and `operator.toml` — is untouched; only the distribution mechanism of one of the two files changes.

**Where a per-tier value lives instead.** The template ships Layer-1 defaults only; per-tier *value* overrides live in the Layer-2 surfaces (the XDG individual config `${PMO_PLATFORM_CONFIG_ROOT:-$HOME/.config/pmo-platform}/platform-config.toml`, `PORTFOLIO.md`, `program-config.toml`, `PROJECT.md`) per `core/schemas/platform-config-schema.md`. The XDG file is **Operator-instance** — never created and never touched by the package — which is exactly why it cannot itself be registered here. Registering it would place it under `update.sh` regeneration, and §3.3 discards operator content outside a fence; the live file carries no fences, so its contents would be discarded on the first update.

**Manifest entry format.** Each row is `<source-relative-path>|<runtime-tier>|<tokens-flag>[|<marker-dialect>]`. The 4th field is optional and defaults to `plain` when absent, so every row predating ADR-122 is unchanged. Tiers are `hook`, `instance`, `hub-state`, `workspace-root`, and `operations-root`. Two of them **strip a trailing `.template`** from the source basename to form the target name, because their target is the operator-facing file rather than a template copy: `workspace-root`, which resolves to the workspace root itself, and `operations-root`, which resolves to the operations-workspace root one sibling over. `operations-root` deliberately does not consume the operator-instance base, which is what keeps it independent of a relocation of that family.
...
Operator content outside any fence (above MANAGED SECTION or below OPERATOR ADDITIONS) is **discarded on update** with a warning. This enforces the two-fence structure as the canonical operator-extension surface. For a **`workspace-root`-tier or `operations-root`-tier** target the warning is upgraded to a named-path notice that also states the pre-write backup location — proportionate to a top-level governance-adjacent file, where a bare "content discarded" line is not enough to recover from. The update path tests this as a tier **class**, not as a single tier value, so a tier added to the class here is a tier the code actually treats that way.
...
The `COMPOSITION_SURFACE_FILES` manifest in [`core/deploy/composition-surface-manifest.sh`](../deploy/composition-surface-manifest.sh) (sourced by `setup-workspace.sh` and `update.sh`) is the authoritative list of files in this category — it currently includes the per-domain allowlists, the hub-state schema templates, `core/CLAUDE.md.template` (workspace-root tier, markdown dialect, ADR-122), and `operations/CLAUDE.md.template` (operations-root tier, markdown dialect). Adding a new composition-surface file is a single appended entry in that manifest — plus the manifest's own self-declared `<N> composition-surface` count, which an enforcing check asserts against the actual row count. No other code change is needed.

**Adding a new TIER costs more than adding a row.** A tier needs three things beyond the row: a `case` arm in the composition library's target resolver, a matching arm in the install validator's composition-surface completeness check — whose tier `case` falls through a catch-all `continue`, so a row on an unhandled tier is verified by nothing — and a widening of any tier-literal branch downstream of resolution, of which the update path's out-of-fence discard notice is one.

**Enumerate readers of a manifest-DERIVED value, not readers of the manifest.** The consumers of this surface are not exhausted by "who sources or iterates the array". Three consumer shapes are invisible to that question and each has produced a real miss: a consumer that **regex-parses the manifest file** rather than sourcing it (the QA check module's manifest-count regression guard, which reduces manifest membership to a single PASS/FAIL verdict); a consumer that **branches on a tier name downstream of resolution** (the update path's discard notice); and a located consumer that **filters the array before aggregating**, so its denominator is not the array's cardinality (the install validator's completeness check). Before changing a row, a tier name, or the row count, sweep for readers of that derived value by content — a sourcing-graph sweep will not find them.
