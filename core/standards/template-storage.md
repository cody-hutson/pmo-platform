...
- **Family-home rule (applies to every L1 artifact family — no per-family list is maintained here).** Every artifact family declared in [`template-taxonomy.md` §3–§5](template-taxonomy.md) homes its canonical template in `operations/templates/` under the §2.2 naming convention, **regardless of which surface renders the artifact** — a template rendered by a platform pipeline stage is homed by the same rule as one authored by a project role. A new family therefore inherits its home from this rule and adds nothing to this section. The worked instance is the software-domain **`pipeline-output`** family (L1 §4.8 — *Acceptance report / Stage verdict report*), whose producer is a pipeline stage rather than a project role and whose home was for that reason previously treated as an open question: its canonical is `operations/templates/qa-acceptance-report-template.md`, homed here on this rule and on no exception. **Propagation contract for the family:** canonical-only — no `TEMPLATE_SYNC_MAP` mirror is registered, because no consumer skill holds a runtime read-path to it; mirror registration follows the first consumer that does, per §6. Registering no mirror is not a placement constraint (see §3.1).
...
**The resolver is registry-gated, not placement-gated.** Every call site of `resolve_template_sync_source()` sits inside an iteration over `TEMPLATE_SYNC_MAP`, so the resolver is only ever applied to a filename that is **registered in the map**. A canonical template with no consumer-skill mirror is never resolved at all, and is therefore not constrained by the resolver to a resolver-mapped directory — its placement is governed by §2.1's family-home rule, and a canonical homed outside a resolver-mapped directory is a sync failure only if it is *also* registered. Read the constraint in that order: registration first, placement second. A resolver arm is owed by the §6 registration protocol (step 3), never by placement alone.
