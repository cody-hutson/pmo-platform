...
- **Pipeline-output templates** (L1 [`template-taxonomy.md`](template-taxonomy.md) §4.8 — the `Acceptance report / Stage verdict report` family, category slug `pipeline-output`): governed home is **`operations/templates/`**, the same canonical registry as every other family, under the §2.2 naming convention `<artifact-family>-template.md`. A pipeline-produced verdict report is a canonical template like any other; being *emitted by* a release-pipeline stage does not give it a release-side home. Placement is not a function of the producing stage, and — per §3.1 — it is not a function of the sync-map resolver either.
...
**The resolver is registry-gated, not placement-gated.** `resolve_template_sync_source()` is applied *only* to filenames registered in `TEMPLATE_SYNC_MAP` — all four of its call sites (three in [`deploy.sh`](../deploy/deploy.sh), one in `build-skill-packages.sh`) iterate `for entry in "${TEMPLATE_SYNC_MAP[@]}"`. A canonical template with no consumer-skill mirror is therefore never passed to the resolver at all, and is **not** constrained to a resolver-mapped directory by the resolver's existence. Adding a resolver arm is step 3 of the §6 registration protocol — a consequence of registering a mirror, never a precondition of placing a file. *(This supersedes the v3.64-era claim that a release-side `*-template.md` mechanically fails `deploy.sh --check`; that failure mode is real only for a **registered** entry, where Check 13 and `build_one()` both iterate the same map. The frozen v3.64 release records are historical and are not edited — the reconciliation lives here, in the live standard.)*
