...
> _Note (#159, v3.34):_ `plan_subtype` was superseded by the OPEN `plan_type` discriminator — the plan kind is now `plan_type: hypercare` (registry: `entity-field-schemas.md` §3.4a; per-subtype templates under `operations/templates/plan-templates/`).
