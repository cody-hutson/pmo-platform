...
| **Drift** | "Are cross-references intact and platform consistent?" (cross-domain reference integrity + governance contradiction detection) | `pmo-platform/engineering/tools/health-check.sh` + [`.claude/rules/doc-link-maintenance.md`](../rules/doc-link-maintenance.md) Check 14 / Check 15 | Cross-domain link resolution + stale-reference scan + version/count-claim validation | NO reciprocal edit (closed issue) |
