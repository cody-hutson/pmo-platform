...
**Gate 2 coverage boundary — FOUR conditions.** A control enforced by a PreToolUse hook is in force only when **all four** of **loading**, **bypass**, **master-activation class**, and **hook mode** hold together, and it is **not** in force when any one of them fails. The canonical statement is `core/standards/subagent-security-posture.md` § 3.1, *Coverage boundary of a PreToolUse-enforced control*; this site names the four and cites there for the expansion rather than restating it, so the wording cannot drift between sites. The bullets above cover two of the four — **bypass** at the `CLAUDE_HOOK_BYPASS` line, **hook mode** at the `.mode` line. The other two conditions decide whether Gate 2 is a control at all, and both bear on this hook:

- **Loading.** A session loads the hook only if it resolves a settings surface that declares the hook wiring, and only if its working directory is under the governed workspace root. A session resolving no such surface **loads no hooks at all**, so Gate 2 never runs there whatever the sentinel state. A git worktree placed outside the governed root is outside coverage even though it is a checkout of a governed repository — scope follows the working directory, not repository identity.
- **Master-activation class.** `block-skill-direct-edit.sh` is declared **`workflow`** class, and master activation ships **OFF**. A `workflow`-class hook is inert while master activation is off, so **on a default instance Gate 2 blocks nothing and records nothing**. Because the class gate runs *before* the `.mode` read, `warn` and `enforce` are indistinguishable there — a "the mode is set to enforce" claim says nothing about coverage on such an instance.

The two gates compose **when Gate 2's four conditions hold**: edit-time enforcement via the PreToolUse hook (fast feedback during the edit attempt, before any commit is created) + deploy-time guarantee via `deploy.sh --check` (last line of defense before any release ships). When any of the four fails, Gate 2 is a **convention, not an interlock**, and `deploy.sh --check` is the only gate actually enforcing.
