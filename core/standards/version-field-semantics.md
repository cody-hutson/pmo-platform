...
**Gate 2 coverage boundary — FOUR conditions.** A control enforced by a PreToolUse hook is in force only when **all four** of **loading**, **bypass**, **master-activation class**, and **hook mode** hold together, and it is **not** in force when any one of them fails. The canonical statement is `core/standards/subagent-security-posture.md` § 3.1, *Coverage boundary of a PreToolUse-enforced control*; this site names the four and cites there for the expansion rather than restating it, so the wording cannot drift between sites. The bullets above cover two of the four — **bypass** at the `CLAUDE_HOOK_BYPASS` line, **hook mode** at the `.mode` line. The other two decide whether Gate 2 is a control at all, and both bear on this hook:
