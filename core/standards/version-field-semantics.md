...
- Matcher scope: `Write` and `Edit` tool calls targeting `pmo-platform/skills/*/SKILL.md`, `pmo-platform/skills/*/reference/*.md`, or `pmo-platform/skills/*/references/*.md`.
...
- Reject `BLOCK-SKILL-EDIT-001` (SKILL.md) / `BLOCK-SKILL-EDIT-002` (`reference/*.md` or `references/*.md`) when: (a) no `pmo-skill-editor` session sentinel at `pmo-platform/skills/<skill>/.editor-session`, OR (b) sentinel `target_skill` does not match the edit target, OR (c) sentinel `started_at` is older than the 30-minute TTL or in the future, OR (d) sentinel JSON is malformed.
