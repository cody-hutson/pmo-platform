...
| **R3 — version tag + RELEASE_LOG entry** | Provenance is the load-bearing dimension; the target's authoritative content lives in the release log | `shipped at vX.Y; see pmo-platform/governance/RELEASE_LOG.md` (preferred over a bare `#N` issue link when "when did this ship" matters more than "what issue established it") |
...
| **Forward-binding provenance** | TODO / deferral block citing a future issue where the doc is operable standalone | The `#N` is forward-binding pointer (issue not yet shipped); content is fully described in-prose; consumer can act on the content without resolving the `#N` | `pmo-platform/skills/implementation-planner/SKILL.md:376-386,542` deferral TODO block; `pmo-platform/skills/pmo-skill-refiner/SKILL.md:241-244` session-canonicity ownership boundary. The `#N` asserts ownership/timing pointer; the failure mode / behavior is operable as written. |
...
#### §10.5.2 Audit findings table (provenance: audit dated 2026-05-15; see `pmo-platform/governance/RELEASE_LOG.md` monolith-cleanup entry)
