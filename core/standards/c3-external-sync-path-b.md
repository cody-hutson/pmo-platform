...
type: standard
status: ACTIVE
automation_id: [external-sync-path-b]
consumers: "core/automations/registry.md (the `external-sync-path-b` row that declares this routine; the operator's [adapters].scheduler resolves how it fires); OPERATIONS.md Daily Processing Cycle step-9 External Sync (the step this sweep extends with a scheduler + snapshot + Evidence-Gate close path); the configured ticketing/KB/Smartsheet MCP adapters (read-polled)"
