...
|---|---|---|---|---|---|
| `status` | enum {`green`, `yellow`, `red`} (RAG) | Required | weekly (published) · daily (refreshed) | Project (entity 1) — `status`; ppm-agent maintains | worst-component dominance — the existing Section-1 health rule + the §7.1 watermelon gate; a subsystem `red` cannot roll up to portfolio `green` |
...
| **S2** | per-project `### Health Indicators` (Schedule / Scope / Quality / Stakeholders / Integration; each `[STALE]` if aged) | EXISTING | `status` (per-dimension) | Project (1) |
