...
2. **Shipped.** The enforcement point: the `PreToolUse` object is re-homed to a settings surface every session resolves regardless of project root, and bounded to the governed workspace root by a scope guard (`core/hooks/lib/scope-guard.sh`) as a named fourth precedence layer. Inheritance is the property the fix RELIES on — a spawned spoke enforces by inheriting the hooks its now-hook-loaded session carries — so Mechanism 2 is load-bearing here, not superseded.
