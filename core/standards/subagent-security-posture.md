...
<!-- ====================================================================== -->
<!-- §1 RECONCILE — v2.23 (#189). The empirical-verification suite SHIPPED:  -->
<!-- a two-layer probe (deterministic CI logic-regression + manual operator  -->
<!-- live-delivery procedure) per ADR-041. Two branches below; the operator  -->
<!-- DELETES the non-selected branch at Stage 7/8 once the Layer-B probe is   -->
<!-- run and its verdict recorded. Until then BOTH are present.              -->
<!-- ====================================================================== -->

**§1 empirical-suite reconcile — BRANCH V (Layer-B probe confirms delivery in BOTH workspace-root AND worktree):**
- The former "One-time empirical verification suite (Stage 7 DT regression cases)" deferral is **REMOVED — SHIPPED in v2.23 (#189).** Hook-inheritance and frontmatter-enforcement are now covered by a two-layer probe: a deterministic CI logic-regression (`core/hooks/tests/subagent-hook-inheritance.test.sh`, the 6 hook patterns, gated by the `install-tests` hook-tests job) PLUS a manual operator-run live-delivery procedure (`core/hooks/tests/subagent-hook-inheritance-probe.md`). The Layer-B verdict EMPIRICALLY VERIFIED Mechanism 2 hook-delivery in both workspace-root and worktree contexts. See § 3 Mechanism 2 (EMPIRICALLY-VERIFIED) + ADR-041.

**§1 empirical-suite reconcile — BRANCH G (Layer-B probe shows worktree delivery FAILS — the #1472-predicted outcome):**
- The former "One-time empirical verification suite (Stage 7 DT regression cases)" deferral is **partially discharged in v2.23 (#189): the two-layer probe SHIPPED, hook LOGIC is empirically green, but live DELIVERY in a worktree context is a DOCUMENTED RESIDUAL GAP.** Layer A (`core/hooks/tests/subagent-hook-inheritance.test.sh`) proves the 6 hook patterns fire on subagent-shaped payloads (CI-gated). Layer B (`subagent-hook-inheritance-probe.md`) records that hooks fire at workspace-root but do NOT fire in a worktree session — **#1472 (PreToolUse hooks wired at workspace root only) is the blocker on real Mechanism-2 enforcement of a spoke** (a spoke runs in a worktree). Mechanism 2 stays LOGICAL-INFERRED for worktree-executed spokes until #1472 closes. See § 3 Mechanism 2 (DOCUMENTED RESIDUAL GAP) + ADR-041.
...
<!-- ====================================================================== -->
<!-- §3 MECHANISM 2 RECONCILE — v2.23 (#189). The empirical suite SHIPPED as -->
<!-- the two-layer probe (ADR-041). Two branches; the operator DELETES the   -->
<!-- non-selected branch at Stage 7/8 after recording the Layer-B verdict.   -->
<!-- The reconcile REPLACES the "LOGICAL-INFERRED-UNTIL-EMPIRICALLY-TESTED"   -->
<!-- qualifier above with the selected branch's basis.                       -->
<!-- ====================================================================== -->

**Mechanism 2 evidence-basis reconcile — BRANCH V (Layer-B probe confirms harness delivery in BOTH workspace-root AND worktree):** Mechanism 2 is **EMPIRICALLY-VERIFIED (v2.23, #189).** The "LOGICAL-INFERRED-UNTIL-EMPIRICALLY-TESTED" qualifier above is superseded. Evidence: the Layer-A CI logic-regression (`core/hooks/tests/subagent-hook-inheritance.test.sh`) proves all 6 hook patterns fire on subagent-shaped payloads; the Layer-B operator probe (`core/hooks/tests/subagent-hook-inheritance-probe.md`) recorded the harness DELIVERING subagent tool calls to the hooks in both workspace-root and worktree sessions. The "by construction" inference is now an empirically-confirmed fact. (ADR-041 records the two-layer-probe decision.)

**Mechanism 2 evidence-basis reconcile — BRANCH G (Layer-B probe shows worktree delivery FAILS — the #1472-predicted outcome):** Hook LOGIC is **empirically correct (Layer A green)** but live DELIVERY in a worktree context is an **unverified/failing DOCUMENTED RESIDUAL GAP**: the harness does not fire PreToolUse hooks in worktree sessions because hooks are wired at workspace root only. **#1472 is the named blocker on real Mechanism-2 enforcement of a spoke** — a spoke executes in a worktree, exactly the context where the hook may not fire. Mechanism 2 therefore holds **by construction for workspace-root sessions (Layer-A-confirmed logic + Layer-B-confirmed workspace-root delivery)** but stays **LOGICAL-INFERRED for worktree-executed spokes until #1472 closes**. The Layer-A regression guards the logic regardless; the Layer-B probe (`core/hooks/tests/subagent-hook-inheritance-probe.md`) is the standing re-test for when #1472 lands. (ADR-041 records the two-layer-probe decision + the #1472-blocker posture.)
...
| v2.23 | 2026-06-25 | Phase-2 empirical-verification suite SHIPPED (#189): the deferred one-time suite is replaced by a TWO-LAYER probe — a deterministic CI logic-regression (`core/hooks/tests/subagent-hook-inheritance.test.sh`, 6 hook patterns) + a manual operator-run live-delivery procedure (`core/hooks/tests/subagent-hook-inheritance-probe.md`), per ADR-041. § 1 deferral bullet + § 3 Mechanism 2 evidence-basis carry BOTH a Branch V (delivery empirically-verified) and a Branch G (worktree delivery a documented residual gap, #1472 the blocker); the operator deletes the non-selected branch at Stage 7/8 from the recorded Layer-B verdict. ADR-041 pairs with ADR-031. |
