...
**§1 empirical-suite outcome — hook-inheritance CONFIRMED at workspace root; worktree-loading verification → #1472 (v2.25, #189):**
- The former "One-time empirical verification suite (Stage 7 DT regression cases)" deferral is **discharged with a partial result in v2.25 (#189).** Hook-inheritance and frontmatter-enforcement coverage shipped as a two-layer probe: a deterministic CI logic-regression (`core/hooks/tests/subagent-hook-inheritance.test.sh`, the 6 hook patterns, gated by the `install-tests` hook-tests job) PLUS a manual operator-run live-delivery procedure (`core/hooks/tests/subagent-hook-inheritance-probe.md`). The Layer-B probe ran one pattern (`git push --force` → BLOCK-DESTRUCTIVE-001) at workspace root and confirmed delivery: a subagent's tool call was intercepted before execution. **Inheritance verification is therefore done** — a subagent inherits whatever PreToolUse hooks its session has loaded. The remaining open question was worktree-session hook-LOADING (whether a session launched from a repo/worktree cwd loads hooks at all), which the workspace-root probe cannot settle. It was tracked as **#1472**, **closed `not_planned` on 2026-07-01**, and is superseded — not reopened — by the `hub-spoke-execution-safety` release, which ships the enforcement point that removes the structural cause: the wiring is re-homed out of workspace-project scope so every session resolves it, bounded by a workspace-scope guard. **What that release does NOT claim is that the mechanism has been observed.** Non-interception from a worktree session is observed; that hook-*loading* is its cause remains **inferred**, and probe **P-1** (`core/hooks/tests/subagent-hook-inheritance-probe.md` § P-1) is the instrument that discriminates it from harness non-delivery for this session class. P-1 has not run. See § 3 Mechanism 2 + § 3.1 Coverage boundary + ADR-042.
...
**Mechanism 2 (hook-inheritance) — CONFIRMED at workspace root (v2.25, #189).** The "LOGICAL-INFERRED-UNTIL-EMPIRICALLY-TESTED" qualifier above is superseded for the workspace-root case. Evidence: (a) the Layer-A CI regression suite (6 cases, `core/hooks/tests/subagent-hook-inheritance.test.sh`) confirms each hook's logic fires on subagent-shaped payloads; (b) a Layer-B probe confirmed delivery — a subagent spawned from a workspace-root session attempting `git push --force` was intercepted by `block-destructive` (BLOCK-DESTRUCTIVE-001) before execution. A subagent's tool calls are subject to whatever PreToolUse hooks its session has loaded. (ADR-042 records the two-layer-probe decision.)

**Residual — worktree-session hook-LOADING: enforcement point SHIPPED, mechanism still INFERRED.** The Layer-B probe covered a single pattern at workspace root, where hooks are loaded; it did not establish whether hooks LOAD for a session launched from a repo/worktree cwd (a hook-loaded session cannot observe its own absence; that test needs a session started from the repo). This was tracked as **#1472**, **closed `not_planned` 2026-07-01**; the `hub-spoke-execution-safety` release supersedes it rather than reopening it. Three things must be held apart, because collapsing them is how a control gets claimed that does not exist:

1. **Observed.** No settings surface on a repo- or worktree-rooted session's resolution path declares any hook wiring — `setup-workspace.sh` wrote the resolved template to the workspace project directory and nowhere else. A real `Bash` tool call issued from such a session executed with no interception, reproduced independently. The same payload driven directly into the hook blocks with exit 2, and an allowlisted comparator passes.
2. **Shipped — the mechanism, wired and ready. NOT the state change that activates it.** The enforcement point: the `PreToolUse` object is re-homed to a settings surface every session resolves regardless of project root, and bounded to the governed workspace root by a scope guard (`core/hooks/lib/scope-guard.sh`) as a named fourth precedence layer. **The re-home does not happen at merge.** It is an **operator-executed** step — `docs/scripts/setup-workspace.sh --rehome-hook-wiring` — writing to the user-scope settings file **outside the repository**, a surface no merge can reach and no release can gate. It is deliberately not folded into any other flow: it writes outside the workspace root and it turns enforcement on for sessions that previously had none, so it is ordered **after** script-allowlist reconciliation. **Until an operator runs it on a given instance, this enforcement point is not active there** — condition 1 (loading) of § 3.1 still fails, exactly as it did before this release. Read the shipped machinery as *available*, never as *in force*: the operator-facing procedure is `docs/UPDATE.md` § 3.6. Inheritance is the property the fix RELIES on — a spawned spoke enforces by inheriting the hooks its now-hook-loaded session carries — so Mechanism 2 is load-bearing here, not superseded.
3. **Still inferred, and NOT claimed.** That the absence of interception is *caused* by the settings not loading; that this harness class delivers `PreToolUse` to a real tool call at all (the only recorded delivery positive is a Desktop-app-class session — see `core/hooks/tests/subagent-hook-inheritance-probe.md` § 5, the *alternatives* table, row **(c)** "harness non-delivery", which records that alternative as NOT RULED OUT and states that there is no recorded CLI/SDK-class positive control at any working directory); and therefore that a re-homed wiring will be loaded. **Probe P-1** (`core/hooks/tests/subagent-hook-inheritance-probe.md` § P-1) converts each of these to an observation. It is operator-executed and has **not run**. If P-1's workspace-root positive control does not block, no settings placement is an enforcement point and the design is void rather than mis-placed.

**Evidence rule for anyone grading this.** A `block-log.jsonl` row alone is **not** evidence that a hook fired in a real session: the row schema carries no `session_id` and its `cwd` is read from the payload, so an identical row is producible from a direct stdin pipe that never touched the harness. The grading contract is **W ∧ L ∧ R** — witness file ∧ log row ∧ tool result — against a pre-fix baseline. See the probe document's § Negative control.
...
## § 3.1 Coverage boundary of a PreToolUse-enforced control — FOUR conditions

**This is the canonical statement. Every doc that cites a PreToolUse hook — or an allowlist it reads — as a *control* must state this boundary and may cite here for the expansion rather than restating it in full.**

A control enforced by a PreToolUse hook is in force only when **all four** of the following hold together, and it is **not** in force when any one of them fails:

1. **Loading** — the session resolves a settings surface that declares the hook wiring. The wiring applies to any session, main or spawned, whose working directory is under the governed workspace root; a session outside that root is excluded by the scope guard (`core/hooks/lib/scope-guard.sh`), and a session that resolves no such surface **loads no hooks at all**. A git worktree placed outside the governed root is outside coverage even though it is a checkout of a governed repository: scope follows the working directory, not repository identity.
2. **Bypass** — `CLAUDE_HOOK_BYPASS` is not set to `1` in the environment the session was launched with. This is the **first** precedence layer, it is evaluated before the dependency gate in every hook, and it exits **both** hook classes. It is set before launch and is inherited by every spawned subagent. `BLOCK-DESTRUCTIVE-023` denies *setting* it (and the scope-guard root override) mid-session; it does not narrow the pre-launch case. **At this layer the security/workflow asymmetry does not exist** — a citation that carries the asymmetry up to layer 1 overstates what a `security` class buys.
3. **Master-activation class** — the hook's declared class admits it. A `security`-class hook enforces irrespective of the master-activation state, going inert only on the operator's explicit, logged security-class opt-out. A `workflow`-class hook is inert while master activation is off.
4. **Hook mode** — the hook's own mode admits blocking. Most block hooks read a shared mode file and, in warn mode, record a would-block and **allow** the call. A minority do not read it at all and are mode-independent.

Precedence inside each hook is `CLAUDE_HOOK_BYPASS → master-activation → scope → .mode → rule`, with loading upstream of all four.

**A citation of such a control that names fewer than these four conditions overstates its coverage.** The specific failure this boundary exists to prevent: an allowlist described as a control, relied on as a control in a threat model, and never in force on the path the work actually ran on.

**Currency is an orthogonal axis — and deliberately not a fifth condition.** The four conditions decide whether the control **runs**. None of them decides **which version** of it runs: a hook that is loaded, unbypassed, `security`-class and mode-independent can still be enforcing bytes that a merged fix superseded weeks ago, with every condition reading as satisfied throughout. The count above is correct as it stands, and this paragraph does not increment it.

The mechanism is the publisher, not the hook. The deployed bundle under `.claude/hooks/` is advanced by the update path, while the recorded checksum baseline that path consults is written only by a full installer run — so on any maintained instance the two diverge, and a refresh that classifies from the baseline alone reads a stale *platform* copy as an *operator edit* and declines to touch it. Before the publisher gained a second oracle, that decline was an inline warning inside a run that exited 0: the merged fix never landed and nothing said so.

"Is the control in force?" is answered by the four conditions. "Is the control the rule I am reading?" is a separate question with a separate answer, and neither this standard nor a registry entry that cites it is evidence for it. The publisher decides platform-copy versus operator-edit on a bounded git-history walk rather than on the baseline alone, preserves on every unclassifiable outcome, and exits non-zero naming any hook it declined — so a superseded copy is either updated or reported, never silently kept. Verify currency by refreshing the bundle and reading the result, not by re-reading the conditions: `docs/scripts/setup-workspace.sh --refresh-hooks`, with `--reconcile-hooks` as the force path for a copy the walk classifies as a genuine divergence.
...
|---|---|---|
| **Tier 0 (Manual / Irreducible Human Tasks)** | BLOCK regardless of `automation_level` AND regardless of mode (always-enforce class, mirrors `block-rm-prefer-trash` permanence). Checked FIRST. | **LIVE** — but only for the **payload-detectable** subset: governance-file writes, plus cross-domain bridge writes **in the `projects` → `pmo-platform` direction only** (resolved from the Write/Edit `file_path` + `cwd`). The converse direction is item 7a, a mode-gated layer-discipline signal (`BLOCK-AUTONOMY-004`) below the master gate, and is **not** part of this always-enforce class. Financial / account-creation / security-permission and the Stage 9 / Stage 12 gates are NOT mechanically detectable from a tool payload — they stay **operator-irreducible by convention**, documented but not hook-enforced. Item 8 (destructive outside the workspace) is already owned by `block-rm-prefer-trash` BLOCK-TRASH-001/003 — C5 does NOT duplicate it. |
