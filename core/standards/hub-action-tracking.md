...
Runtime ledgers seeded before this revision carry values this enum does not admit, because the template that seeds them restated all three enums (§ 2.1 `category`, § 2.2 `trigger_type`, § 2.3 `status`) and diverged from every one. Those ledgers are operator-instance files the platform does not rewrite — most are closed-release audit records, and rewriting a closed release's ledger to a later vocabulary would falsify the record the Stage-13 gate verdict was rendered against. They are made READABLE against this enum instead, by the alias table below. No ledger is migrated. Every alias in the table below is verdict-neutral at the § 4 routing-point-5 gate, and that neutrality is a property of **these rows** — each is terminal-to-terminal or non-status — not of how the gate reads a value in general.

**The gate's recognised set, stated once.** The § 4 routing-point-5 gate classifies `status` by **membership**, not by comparison against the two non-terminal values. Its recognised set is the § 2.3 enum (`open`, `in-flight`, `done`, `cancelled`, `superseded`) **union** the two `status` aliases in the table below (`resolved`, `withdrawn`), compared **case-folded**, so `OPEN` and `open` can never render opposite verdicts. Any other value — a typo, a vocabulary from another register, an empty value, or a value read out of a row whose column count is not the schema's 13 — is **unreadable, never resolved**. A value the gate cannot classify is a gate error: it counts toward neither the resolved population nor the unresolved one, and it blocks the close until it is normalised.

**Authors write the enum; the gate reads the enum plus the aliases.** These are two different vocabularies and the difference is deliberate. The authoring enum stays at the five § 2.3 values — a template, a schema check, or an author writing a new row conforms to those and to nothing else. The reading vocabulary is wider by exactly the two aliases above, because a reader must apply a read rule and this section is one. Widening the reading vocabulary is not a licence to widen the authoring enum, and admitting a further alias is a change to this table, made deliberately, never a side effect of making some ledger pass.
...
**What such a row now does at the gate.** A `status` value in neither vocabulary no longer reads as resolved. It is counted as unclassifiable and, absent any open row in the same ledger, it renders `UNCLASSIFIABLE` and **BLOCKS** § 4 routing point 5 — a state no attestation clears, because attestation licenses an *absent* ledger and never an *unreadable* one. The remedy is to normalise the value, or to restore the row to the schema's 13 columns where the value is being read out of the wrong column. This is not the section absorbing the substrate-repurposing class; it is the gate refusing to render a verdict over rows it cannot classify, which is what surfaces that class instead of hiding it.
...
| 5 | **Procedure 7 — Close** (via thin Procedure 7a binding — HARD GATE) | Stage 13 Milestone close | THREE counts over the whole AI-row population: the row total, the `status:open` + `status:in-flight` subset, and the subset whose `status` falls outside the recognised set of § 2.1a | **HARD GATE, 5-valued over 3 verdicts.** `UNRESOLVED` (≥1 open/in-flight) **BLOCKS** — operator MUST resolve each remaining row (transition to `done`, `cancelled`, or `superseded`) before Milestone close. `UNCLASSIFIABLE` (no open row, but ≥1 `status` the gate cannot classify) **BLOCKS** — the gate refuses to render a verdict over rows it cannot read, and no attestation clears it; the operator normalises the value, or restores the row to 13 columns where the value sits in the wrong one. Where a ledger carries both classes `UNRESOLVED` wins, because it names the dominant remedy, and its detail carries the unclassifiable rows too so one pass covers both. `RESOLVED` (≥1 row, every status classifiable, none unresolved) is the only silent pass. `NOT-RECORDED` (no ledger file) and `EMPTY-LEDGER` (ledger present, zero rows) **SURFACE for operator attestation** — a release may legitimately record no commitments, but so does a release whose emit step was skipped, and the gate must not conflate them. Carry-forward to next release is via `superseded` with explicit successor AI-NNN in the next release's `action-items.md`. Predicate + decision table: [`hub-spoke-bridge.md` § Procedure 7a](../../release/references/how-to/hub-spoke-bridge.md). |
