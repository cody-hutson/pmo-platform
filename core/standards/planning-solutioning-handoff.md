...
| **T3** | **Structural design decisions** | Issue requires a design choice that constrains downstream artifacts — schema, contract, naming convention, identifier format, directory layout. A structural design choice carries an **abstraction-altitude obligation** (see the §3.1 rider below): the design must name the platform seam the capability extends, or justify solving point-wise. | Read issue body; flag any "Design decisions to make" / "Open questions" / "Mechanism unresolved" framing. Body containing the literal phrase "deferred to Stage 5 Solutioning" auto-flags. ALSO: when T3 fires, the Stage 4 spoke records the §3.1 abstraction-altitude obligation against the issue (the obligation travels into the Stage 5 spec; it does not change the T3 ✓/✗ activation verdict). | Frontmatter schema definition; identifier-format rule; cross-doc reference protocol | [`pipeline/stage-05-solutioning.md`](../../release/references/pipeline/stage-05-solutioning.md) § 5 Phase 0 (alias `structural changes` appears in [`release-process.md`](../../release/governance/release-process.md) § Stage 5 § Activation — historical wording variant; this matrix supersedes) |
...
### § 3.1 Abstraction-Altitude Obligation (rider on T3) {#abstraction-altitude-obligation}

When T3 (structural design decisions) fires on an issue, that issue's design carries an **abstraction-altitude obligation** that travels with it from Stage 4 Planning into the Stage 5 Solutioning spec. The obligation is a *rider on T3*, not a seventh activation trigger: it does not change the release-level ACTIVATE/SKIP verdict (a structural design that extends an existing seam still activates Solutioning), it adds a question the design must answer.

**The obligation.** A structural design choice MUST do one of:
1. **Name the platform seam the capability extends** — an existing `[adapters]` selector (per [`repo-host-adapter-versioning.md`](repo-host-adapter-versioning.md) and the `operator.toml [adapters]` table), a module boundary, or a config surface — and state that the design extends that seam rather than introducing a parallel point-wise mechanism; OR
2. **Justify solving point-wise** — state explicitly that no existing seam fits and the design is a deliberate point solution, **citing the seam-search** (the surfaces searched for a reusable seam: `[adapters]`, module boundaries, config surfaces). A bare "no seam exists" without the cited search does NOT discharge the obligation.

**Why this fires at Stage 4 (upstream of frame-hardening).** This matrix is a Stage 4 Planning surface; the obligation is recorded here, before Stage 5 ratifies the design frame as un-relitigable and before the Stage 5 adversarial reviewer is estopped from challenging a pre-decided premise. Recording the seam question at activation keeps abstraction altitude a *design variable while the frame is still open*, rather than a settled premise no downstream surface may reopen. This is the earliest, highest-leverage hook for the abstraction-altitude forcing function; it is the first layer of a defense-in-depth sequence completed by the design-exploration `altitude` distinctness axis ([`design-exploration.md`](../../release/references/standards/design-exploration.md)) and the blocking seam-composition gate ([`design-review-checklist.md`](../../release/references/templates/design-review-checklist.md) Section 4).
...
**Reconciliation input (the pre-build step this obligation consumes).** The reviewed→retained/changed determination is informed by the ticket-vs-live-architecture reconciliation performed as the pre-build step (fired at Stage 4 / Stage 5 entry) that reconciles the ticket's structural premise against the live architecture (ADRs, disciplines, registries/ledgers/charters, roadmap) BEFORE design, using ticket-age-vs-architecture-date as the staleness signal. This obligation is the activation-time record; the reconciliation discipline is the method it consumes. *(Reserved reference slot: the reconciliation-discipline doc link is filled in by the ticket-architecture reconciliation discipline when it lands.)*
