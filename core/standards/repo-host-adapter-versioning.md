...
type: standard
status: ACTIVE
consumers: "the deterministic-version-claiming mechanism (calls the four adapter operations anchor / claimed_set / atomic_claim / lineage); operator.toml [adapters].repo_host (the config-selector that binds an adapter implementation); the GitHub/git v1 reference adapter (implements this interface)"
...
|---|---|
| `anchor()` | the **highest claimed version in the mainline lineage**, implemented as the maximum of `claimed_set()` (the union below, filtered to the mainline tag frontier). Because a signed version tag is pushed *before* its GitHub Release publishes, the anchor is taken over the **tag frontier**, not the `releases/latest` pointer — so it cannot sit below a freshly-pushed-but-unpublished mainline tag. (`releases/latest` is the greenfield fallback only, when no tag/Release exists yet.) The "how" of `anchor()` is an adapter-internal detail (§2.1); this reference mapping refines an earlier `releases/latest`-only mapping that lagged the signed-tag frontier across the held-but-unclaimed gap. |
| `claimed_set()` | the **union** of git tags ∪ published Releases ∪ the deployed/verified rows of the release log, with every member filtered to the **mainline tag frontier** (the major-line of the published frontier; not gated on Release publication). The union is required because a claim is recorded on more than one surface and an in-flight claim may appear on one before the others — in particular, a signed tag pushed before its Release publishes must already count as claimed. |
...
| `lineage(version)` | **mainline-frontier membership**, keyed on the **signed-tag frontier rather than Release publication** — `MAINLINE` iff the version is on the published frontier's major-line, or is the canonical first release of the next major, or is itself an explicitly-published release; otherwise (an off-mainline/parallel tag — e.g. a stray higher-major version lineage) `ORPHAN`. Keying on the tag frontier means a freshly-pushed mainline tag whose Release has not yet published is correctly `MAINLINE`, while a genuine stray stays `ORPHAN`. |
