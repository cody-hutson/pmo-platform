...
**Cross-reference to ecosystem-design Three-Domain Architecture.** The PMO platform also defines an artifact-provenance model in [`document-ecosystem-design.md` §3](../disciplines/document-ecosystem-design.md) (Source Artifacts / Managed Knowledge / Synthesized Intelligence), whose live values are `source` / `managed` / `generated` — the labels `Domain A` / `B` / `C` are **deprecated aliases** of those three, retained only for the duration of an open migration window. That model classifies **instances** by trust + lifecycle. This taxonomy classifies **template structures** by canon family. Both are valid orthogonal axes; a single template instance carries both classifications (e.g., a status report instance is `managed` for provenance + `Project` for canon family). No conflict. **Two further cautions for a reader of this file, both indexed in `core/specs/domain-token-registry.md`** — the corpus-wide index of every concept the bare token `domain` names. First, the two axes above are separated in practice by the sibling key `artifact_type: template`, not by their values, because both can carry `software` and both appear under `operations/templates/`. Second, §3 below uses the word in a **third** sense — the PMBOK 7 **Performance Domain**, an eight-value axis imported from an external standard — and the §2 table above names both senses in a single row. That adjacency is the sharpest live source of `domain` confusion in the corpus; the registry records the Performance-Domain sense as a deliberately excluded adjacent sense rather than a fourth classification of templates.
...
### §4.8 Quality / Acceptance reporting (pipeline-output)

The `pipeline-output` category covers the verdict artifacts a governed process emits when it grades completed work — the acceptance report a QA stage renders, and the stage verdict report a pipeline stage posts at a gate. **The discriminator from §4.7 is execution order: §4.7 is the pre-execution artifact (the test plan, authored before the work is graded); §4.8 is the post-execution artifact (the verdict report, authored after).** The two families share the PMBOK 7 §Quality canon and are distinguished by that ordering, not by audience.
...
| 12 | Acceptance report / Stage verdict report | software | PMBOK 7 §Quality + ISO/IEC/IEEE 29119-3 §Test Completion Report | (no Anthropic plugin equivalent) | `qa-acceptance-report-template.md` | Dual-anchor family — PMBOK 7 §Quality (quality-management framing) + ISO/IEC/IEEE 29119-3 §Test Completion Report (the post-execution verdict artifact, successor to IEEE 829-2008 §Test Summary Report). **Post-execution counterpart to row 9's pre-execution test plan** — the two share the PMBOK 7 §Quality canon and are separated by execution order. No Anthropic plugin counterpart, so the family stands on the external canon alone (`canon_compat: none`, P5 path c-ii — `domain: software` + external canon + no plugin counterpart). Category slug `pipeline-output` (see §4.8); the 29119-3 anchor is re-verified at the L4 P5 promotion gate per §7 (added 2026-08-01) |
...
**Reconciliation applied per row:** keep the upstream canon as PRIMARY; ADD an Anthropic plugin cross-reference — a convention-anchor naming the plugin's authoring convention, never an install claim — for the 6 rows where a plugin is named (rows 1–3, 5, 6 at the 2026-05-10 D-Gate; row 9 at its 2026-07-03 addition). No row CHANGES the underlying canon. **Which rows carry a plugin cross-reference is derived from §6's Plugin Cross-Ref column, never from this sentence** — re-derive the split against the live table before adding a row, rather than extending the enumeration by hand. As of the row-12 addition (2026-08-01) the derived split is: rows 1–3, 5, 6, and 9 carry plugin cross-references; rows 4 (RFC), 7 (Status report — consumed by a PMO skill, no Anthropic plugin), 8 (Stakeholder Register / RACI), 10 (Change Impact Matrix), 11 (Training Plan), and 12 (Acceptance report / Stage verdict report) have no Anthropic plugin equivalent and stand on the upstream canon alone.
