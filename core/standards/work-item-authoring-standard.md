...
**What this standard does not own.** The cross-kind authoring rules — over-definition, scope-altitude consistency, acceptance-criterion shape, title informativeness — are owned by [`intake-style-guide.md`](../../release/references/how-to/intake-style-guide.md) and are cited, never restated, in § 7. The grammar those manifests conform to is owned by [`work-item-type-schema.md`](../schemas/work-item-type-schema.md). Whether a check's judgment reproduces is owned by [`ADR-200`](../ADRs/ADR-200-l3-judgment-criteria-evaluability.md).
...
- **Presence precedes adequacy.** Where a machine-evaluable declaration tests a field's *presence*, a judgment-level declaration over that same field tests its *adequacy* — and the declared field is the **subject** of that judgment, never its resolution. That is not this standard's observation; it is a recorded finding of [`ADR-200`](../ADRs/ADR-200-l3-judgment-criteria-evaluability.md), which reached it by partitioning the shipped judgment-level set and finding the field-decidable class empty. Ordering presence ahead of adequacy is what keeps the content and depth dimensions from both claiming the same declaration.
...
**Both halves of that test are per-kind, and only the per-kind halves are this standard's.** They read a resolved kind's own declarations, and they are unavailable when no kit resolves, because the test's input is then empty. The **cross-kind** over-definition rule — whether a proposed change commits a HOW at intake at all, whatever kind it is — is a different rule with a different owner: [`intake-style-guide.md`](../../release/references/how-to/intake-style-guide.md) § 4, cited in § 7 and restated nowhere here. A consumer that needs both applies each to its own question rather than reading this test as covering the pair.
...
Every `criteria.*.checks[]` entry and every `fields.kind_specific[]` declaration in a type pack carries a non-empty `source` naming the body of practice it derives from, and a criteria or fields table whose declaration array is present and empty carries one at block level, stating the practice basis for the emptiness. **Those are the declaration sites this standard reads, and not the whole obligation.** It is **declared** at [`work-item-type-schema.md`](../schemas/work-item-type-schema.md) § 1.2.1, which owns the full set of sites it binds — wider than the three above — and it is **registered** in the gate-coverage register in [`gate-efficacy-standard.md`](gate-efficacy-standard.md), whose row owns its runner and its enforcement posture. This standard states neither: it cites that row, adds no row to the register, and restates the obligation nowhere.
...
1. Does the kind carry a criteria or fields table whose declaration array is **present and empty**, with a block-level `source` stating the practice basis for declaring nothing there? Then the zero is **reasoned** — the practice this kind draws on prescribes nothing at that altitude, and the manifest says so at the site where nothing was declared.
...
| § 4.1 — every declaration names its practice basis | **class 2, cited** | The obligation limb holds — the trigger is a source-bearing declaration being authored, the act is declaring its `source`, and no verdict is stated for the negation. The prose-only limb does **not**: `deploy.sh --check` Check 80 implements the assertion. A first limb without the prose-only limb is class 2 — the implementing check carries the declaration and prose merely cites it — so this rule is not admitted to the prose-declared class at all. The register row owns the runner and the posture; this standard cites it and adds none. |
...
**Zero rows and zero runner pointers are added to the gate-coverage register by this standard.** That is a consequence of the table above, not an aspiration: no rule this file states is admitted to the prose-declared class. The rules that are not admissible have no site at which a runner could fire, so they have no row to add; the rules that reach a real obligation cite a row the register already carries, rather than declaring one here.
...
- [`ADR-200`](../ADRs/ADR-200-l3-judgment-criteria-evaluability.md) — owns the judgment-level evaluability verdict, its measurement state, and the routing rule § 5 takes its branch from; also the presence-versus-adequacy finding § 2.2 grounds its second precedence rule on.
