...
The PreToolUse hook reads a different copy. It is a hook-tier composition surface, so the installer writes it to the workspace agent-config root — `<workspace>/.claude/reference-durability-allowlist.txt` — and the hook resolves it there at runtime, beside the other hook allowlists. Two consequences follow. First, an edit to the repo file does not reach the hook until the composition surfaces are reinstalled; `./update.sh` does that, and the skill/harness deploy path deliberately does not. The two installers divide differently than their names suggest: `./update.sh` **regenerates surfaces that already exist** and explicitly declines to create an absent one, routing that case to `docs/scripts/setup-workspace.sh`. So a workspace that has never had this surface installed needs the setup script first and `./update.sh` second — reaching for the update path alone on a fresh instance gets a refusal, not an install. Second, the deployed copy is regenerated from the repo file, so an operator who edits the deployed copy in place loses that edit on the next install — author in the repo, then reinstall. Seed entries cover historical-by-design corpus (the release archive) and files that are link-resolution maps by design. Allowlist additions document why the path is exempt — the rationale records the structural reason the path carries flagged constructs, not a desire to silence the gate.
...
**Anchor resolution is a third orthogonal discipline, and all three run.** It asks whether a cited **section** exists inside its target — a question neither of the other two reaches. Link resolution stops at the file: a link to a document that exists passes, whatever section the surrounding prose claims to be pointing at. Durability asks whether the construct belongs in durable corpus at all: an inline summary satisfies it while saying nothing about whether the section number quoted beside that summary is real. So a reference can be alive, durable, and still send its reader to a section that does not exist — most often because the target was restructured after the citation was written, which changes nothing a link checker can see. Anchor resolution is disjoint from both and is not a substitute for either: it is advisory and report-only wherever it is wired, it models a declared subset of citation forms, and a clean run means nothing was found in the modelled classes rather than that every anchor in the corpus resolves.
