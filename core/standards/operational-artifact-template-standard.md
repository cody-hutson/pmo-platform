...
|---|---|---|---|---|
| **A** | Provenance header | YES | The L4 15-field YAML block per `template-protocol.md` §4.1 (markdown) or sibling `.provenance.yml` per §4.4 (CSV). Verbatim; no field added/removed. | Consolidated (L4) |
...
The template file carries the L4 15-field provenance block **verbatim** per `template-protocol.md` §4.1 (markdown frontmatter) or the sibling `.provenance.yml` per §4.4 (CSV companion). This standard adds **no** field to the L4 schema — provenance is L4's, not PDA's (compose-not-modify, §2).
