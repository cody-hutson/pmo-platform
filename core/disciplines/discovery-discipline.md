...
**Second consumer of the same count — sequencing-mode selection.** The internal-edge count above is read twice, never computed twice. Its first read is **cohesion** (rule 14's own question: do these N items belong together?). Its second read is **which sequencing mode governs the order they are worked in**:

| Density read (same threshold) | Mode | What selects the next item |
|---|---|---|
| `E ≥ N-1` — **dense** | **topology-first** | the graph — order by what unblocks the most downstream work; value breaks ties |
| `E < N-1` — **flat** | **value/WIP-first** | value — order by the backlog's own priority signal; work-in-progress capacity caps the batch |

`E` is the count of internal edges the consumer's own edge model admits as **precedence-bearing** — for release planning, the edges that enter the dependency graph, whose admitted and excluded classes are enumerated at that consumer surface (`release/skills/release-planner/references/dependency-analysis.md` § Step 4) and not here. The threshold is `N-1`, exactly as `review-discipline-principles.md` § 1 rule 14 defines it. Rule 14's cohesion read admits relational edges as well; **the two reads share the threshold and may differ on the count.** `E ≥ N-1` is a connectivity bound, not a proof that the order is fully determined — on a dense graph leverage leads, on a flat graph value leads, and precedence binds in both. **Residual:** the count is therefore a connectivity *proxy* for order-determination, not a measure of it; the proxy is deliberate, because a longest-path measure would introduce a second threshold and this read holds to one. The selector is methodology-neutral — the dense regime is the critical-path idiom, the flat regime the WIP-limited pull idiom; neither is the default.

**Precedence is always binding; the mode selects only what leads.** A mode never authorizes violating a dependency edge. It selects the ordering key applied to the items *already free to start*. Consumers bind each mode to a concrete key at their own methodology surface and cite it rather than restating it — for release planning, `release/skills/release-planner/references/dependency-analysis.md` § Step 4.

**Guards.** Three failure modes attach to this read; all three are conditional rules, not preferences.

- **Over-sequencing a flat graph (false serialization).** Do NOT order a bundle by graph topology when `E < N-1`. A sequence rendered over a near-edgeless graph is indistinguishable in notation from a real chain, so downstream readers treat it as precedence — inventing a critical path and suppressing throughput to its width. **Surface scope:** this guard governs the surfaces where parallel execution is genuinely available — wave/batch plans, roadmap Now/Next/Later ordering, and multi-branch execution topologies. It does not govern a commit order under a serial execution posture, where the total order is a property of the topology and no parallel option exists to suppress. **Observable signal — one comparison, read off whichever surface is in hand:** the *rendered sequential depth* exceeds the graph's *longest precedence chain*. On a **wave/batch plan**, depth is the wave count — equivalently, two items in consecutive waves with no edge between them. On a **roadmap Now/Next/Later**, it is an item held in a later horizon than any unmet edge requires, where the horizon split is attributed to dependency rather than to value or WIP capacity (a value- or capacity-driven split is the flat regime behaving correctly, not this guard firing). On a **multi-branch topology**, it is the depth of the branch chain — equivalently, one branch carrying items with no edge between them under a stated dependency-driven order. An **implementation-sequence arrow list** is a reading of this signal only where it renders one of those three surfaces; the same arrow list rendering a commit order under a serial posture is the excluded case above, and counting it there manufactures a false positive.
- **Under-sequencing a dense graph (value-picking into a dependency wall).** Do NOT order the ready set by value alone when `E ≥ N-1`. On a dense graph the highest-value item is frequently not ready; picking it consumes the batch without advancing the chain, and the work stalls with capacity spent and nothing landable.
- **Serialization that is not density.** Do NOT infer density from the presence of a total order, and do NOT flag a total order as over-sequencing, when every non-dependency adjacency carries a named driver. **Recognised drivers:** contention on a shared file · a shared build artifact or package rebuild · a rebase window against a live sibling branch · reflexive risk (an item editing the tooling that runs its own release). These constrain **adjacency**, not **precedence** — they add nothing to the count and never select topology-first, and each must be named on the adjacency it justifies. Where a consumer separates a logical from a technical dependency layer (for release planning, § Two Dependency Layers), these drivers live in the technical layer and the count reads only the logical one.
