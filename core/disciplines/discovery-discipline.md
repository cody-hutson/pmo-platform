...
- **Over-sequencing a flat graph (false serialization).** Do NOT order a bundle by graph topology when `E < N-1`. A sequence rendered over a near-edgeless graph is indistinguishable in notation from a real chain, so downstream readers treat it as precedence — inventing a critical path and suppressing throughput to its width. Observable signal: an implementation-sequence arrow list longer than the count of precedence-bearing edges. **Surface scope:** this guard governs the surfaces where parallel execution is genuinely available — wave/batch plans, roadmap Now/Next/Later ordering, and multi-branch execution topologies. It does not govern a commit order under a serial execution posture, where the total order is a property of the topology and no parallel option exists to suppress.
