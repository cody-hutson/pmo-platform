...
> **Depersonalization note:** GitHub Project IDs (`PVT_*`, `PVTF_*`, `PVTSSF_*`) and single-select option IDs (`single-select-option-id <hex>` 8-char strings in example commands) are operator-instance per [universal-vs-localized-context.md](../standards/universal-vs-localized-context.md) DC1–DC4. The five top-level identifiers above are substituted to bracketed tokens; replace with your project's values via `gh project field-list --owner <YOUR_HANDLE> --number <YOUR_PROJECT_NUMBER>` before executing the example commands. The single-select option IDs embedded in example commands below are preserved literally for traceability — your project will have different option IDs; obtain them via `gh project field-list ... --format json | jq '.fields[] | select(.name=="Status") | .options'`.
