...
| [build-philosophy.md](build-philosophy.md) | The platform's first-class engineering values (Scalability, Best-Practice-per-Domain, Maintainability, Simplicity, Stability, Security, Portability + the read-before-edit / track-all-edits disciplines) and the philosophy × surface coverage matrix mapping each value to the artifact enforcing it across skills / agents / hub-spokes / hooks / slash-commands. Names and routes; cites, never restates. |
...
| [orchestration-mechanisms.md](orchestration-mechanisms.md) | Comparative **research survey** of named agent-orchestration mechanisms (centralized / hierarchical / market / blackboard / federated / decentralized / sequential / conversational) across a 12-dimension contract interface and a 7-class coordination problem taxonomy, with the incumbent hub-spoke's measured current-state value per dimension. Research-only: selects no mechanism, declares no config field, changes no skill. The evidence base the actor-model target's open decisions D3 and D5 need to become renderable. |
