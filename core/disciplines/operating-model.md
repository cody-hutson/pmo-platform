...
| Claude Code rules | [`core/rules/*`](../rules/) (8 files: bypass-mode-readiness, decision-time-adherence, doc-link-maintenance, git-workflow, governance-files, harness-deployment, operations-bridge, skill-deployment). The **deployed** `.claude/rules/` mirror set is **9** — these 8 plus `release-process`, which is sourced from [`release/governance/release-process.md`](../../release/governance/release-process.md) and is not itself a `core/rules/` file. | Claude Code (git-tracked) | Platform-engineering area only — operations-scoped skills do not load |
