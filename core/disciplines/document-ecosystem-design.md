...
2. SQLite index → Build index builder tool (rebuild from frontmatter) — **MATERIALIZED** as [`core/deploy/tools/build-doc-index.py`](../deploy/tools/build-doc-index.py) (#1769): all 7 tables + the 7 named queries, deterministic byte-identical rebuild + an incremental-update capability (the auto-invoking event source is open epic #1153)
