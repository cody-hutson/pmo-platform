...
- **Layer 1:** Immediately after returning from an operations-mode session and **before any git operation**, run `git status` (and `git diff` if status is dirty). An unexpected modified/untracked Layer 1 file is the concurrent-write signal. This is the operational form of the operations-bridge.md Rule 4 trigger.
