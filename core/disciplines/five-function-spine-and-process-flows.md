...
- **Operational-chain analog (World B):** [`cross-chain-architecture-map.md`](cross-chain-architecture-map.md) maps the operational delivery chains (escalation · project · program/portfolio + 6 operational sub-chains) to their governing data model / flow / holding gate the way this spine maps the release pipeline. This file is the release-flow (World A) half of that cross-chain index; the map is the operational half, and it references this spine as its Release row rather than restating it.
