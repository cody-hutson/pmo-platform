...
- **Security** — [`domain-best-practices/software.md §Security`](../standards/domain-best-practices/software.md), [`dep-resolve.sh`](../hooks/lib/dep-resolve.sh), [`check-hook-dep-hardening.sh`](../hooks/tests/check-hook-dep-hardening.sh) (static grep guard) / [`hook-fail-closed.test.sh`](../hooks/tests/hook-fail-closed.test.sh) (behavioral, glob-derived), [ADR-078](../ADRs/ADR-078-security-hook-dependency-resolution-posture.md) / [ADR-081](../ADRs/ADR-081-security-sixth-first-class-value.md). The context-aware output-encoding enforcer (standard + semgrep lint) is in flight (S1, v3.73 build-security-hardening) and populates the Security × Skills cell on landing; Agents + Hub/Spokes stay GAP (no agent-level or secure-SDLC-checklist security enforcer yet).
