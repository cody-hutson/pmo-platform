...
| **D1 — read-before-edit** | `review-discipline-principles` (anti-laziness 1/3/6/10) · `principal-standard-checklist` §C3 | **GAP** | review discipline | `block-skill-direct-edit` (narrow **and conditional** — `workflow`-class, so inert on a default instance; four-condition boundary below) | **GAP** |
...
- **D1 — read-before-edit** — [`review-discipline-principles.md`](review-discipline-principles.md), [`principal-standard-checklist.md`](../standards/principal-standard-checklist.md), [`block-skill-direct-edit.sh`](../hooks/block-skill-direct-edit.sh) (skill-edit-path only; general enforcement is a GAP, in flight). **The Hooks cell is narrow AND conditional, and the two are different limits.** Narrow: the matcher reaches only source-module skill paths. Conditional: the hook is in force only while the four-condition coverage boundary holds (loading ∧ no pre-launch `CLAUDE_HOOK_BYPASS` ∧ master-activation class ∧ hook mode, canonical at [`subagent-security-posture.md` § 3.1](../standards/subagent-security-posture.md)), and it is `workflow`-class against a master activation that ships OFF — so on a default instance the cell is a **declaration, not an interlock**. Read it as filled only for an instance that has enabled it; the hook-specific expansion is at [`canonical-skill-structure.md` § Coverage boundary of the edit-time gate](../standards/canonical-skill-structure.md).
