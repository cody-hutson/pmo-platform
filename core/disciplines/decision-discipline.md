...
**Rule of thumb:** If the consumer is **recommending to operator** AND operator is **expected to act on it**, all 3 mechanisms apply (or are explicitly exempt per this table, not per consumer whim). If the consumer is **relaying information** or **executing a determined procedure**, mechanisms are exempt. Exemption here is from the M1/M2/M3 **briefing** mechanisms and nothing else: a status read that is *promoted into a load-bearing claim* still carries its adherence checkpoint per [`core/rules/decision-time-adherence.md`](../rules/decision-time-adherence.md) § 2 — that rule binds the claim, this table governs the briefing.
...
|---|---|---|
| **New-signal-injected** | The pause cites a *specific* external fetch or tool call that was **not** in the inputs before the pause began — a named file read, a query result. A re-derivation counts **only if it incorporates newly-fetched external signal**; a re-derivation over the *same* inputs already in hand is **not** new signal. The injected signal is the thing that grounds (or revises) the decision. | A pause that re-states the same inputs already in hand and "concludes" without fetching anything — including a re-derivation over unchanged inputs — deliberation re-labeled as learning. |
...
- **Case C — re-derivation over unchanged inputs (MUST be REJECTED as a stall).** A pause that reports: *"Estimate confidence LOW. Cycle 1 of budget — re-derived the estimate from the same inputs already in hand (no new fetch); the re-derivation agrees, so I'm now confident. Exit E1 (resolved) → proceeding."* — reports a reportable cycle count (**bounded** ✓) and a named exit (**has-exit** ✓), but the cited re-derivation fetched **nothing external** and ran over the *same* inputs (fails **new-signal-injected**). G8 verdict: **REJECT** — a re-derivation over unchanged inputs is deliberation re-labeled as learning, the exact theater case the guard rejects; satisfying two of three signals does not pass it. The reviewer confirms the guard fires even when a re-derivation is dressed as the injected signal.

The pair is co-located here, adjacent to the guard it falsifies, because the anti-theater guard is a **governance/documentation mechanism**, not an automated-assertion gate — it has no `deploy.sh --check` Check and no CI workflow. Under the gate-efficacy standard's scope boundary that makes it a **prose-declared normative predicate**: it states a REJECT verdict and nothing in the tree implements it, which is that class's **verdict-limb** admission form (`L1 ∧ L2`) — one of the two forms that admit class 3, the other being the obligation limb `L1′ ∧ L2`. Its runner is the named review step this section already states — a reviewer, or a Dev-Testing spoke, runs the pair — so the guard is not runnerless. A gate-coverage register row is owed when a release next adds or modifies this guard; the register's obligation is diff-scoped by design, not a standing corpus sweep. The test-case pair is the falsification artifact for a documented guard, kept beside the mechanism so a reviewer reads guard-and-test together.
