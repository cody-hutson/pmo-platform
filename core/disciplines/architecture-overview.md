...
How Claude Code operates when working on the platform — 9 files:
...
`release-process.md` (the 13-stage pipeline rules) is **not** a `core/rules/` file — it is sourced from `release/governance/release-process.md` and mirrored into the deployed `.claude/rules/` set, which is therefore 10 files rather than 9.
...
- **As-built architecture-conformance audit** — the *retrospective* counterpart to the forward per-ticket architecture-fit gate: `pmo-qa-auditor` **Mode I** reads the release record and audits *delivered* work against this platform architecture (ADR corpus primary; the cross-chain index + this overview secondary), detecting conformance-drift and candidate cross-release fragmentation. Machinery: [`architecture-conformance-mode-spec.md`](../skills/pmo-qa-auditor/references/architecture-conformance-mode-spec.md) + [`architecture-conformance-dimension-rubric.md`](../skills/pmo-qa-auditor/references/architecture-conformance-dimension-rubric.md); when-to-run: [`architecture-conformance-cadence.md`](../../release/references/protocols/architecture-conformance-cadence.md) (the 4th audit-cadence axis). Observational, dated-audit-folder output; auto-files nothing.
