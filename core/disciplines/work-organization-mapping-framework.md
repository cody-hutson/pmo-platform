...
| [`architecture-evaluative-lens.md`](architecture-evaluative-lens.md) | **Consumes** this framework's altitude ladder ([§1.2 per-level-purpose](#per-level-purpose)) + [Layer 2](#layer-2-map) methodology map as two of the three Venn circles (methodology, altitude) of the design-time triple-Venn lens. The lens cites; this framework defines. |
