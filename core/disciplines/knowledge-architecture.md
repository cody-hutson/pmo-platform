...
| **External-target facts** (a repository or system the toolkit operates upon) | The universality axis routes by *whose context*; it has no scope value for a target that is neither this install's platform nor one of its projects. [§7.1](#external-target-scope) adds that scope and its SSOT rule, bounded to operator-controlled targets with uniquely-resolvable referents. | Cross-reference; the scope adds no tier and redefines nothing. |

---

## §7 Memory↔corpus boundary {#memory-corpus-boundary}

This section names **which surface is the source of truth (SSOT) when a fact could appear in more than one place**, and **how knowledge moves from memory into the corpus reliably**. §3 owns *where each tier lives*; §7 owns *which surface is authoritative when a fact could live in two, and how it migrates*. The body of this section resolves between the two surfaces that are both this install's own — the auto-memory store and the codified corpus — and over that pair it is a pure consumer of [§1](#five-tier-classification) and [§3](#placement-model), adding no taxonomy: the SSOT assignment below is a projection of the §3 "Authoritative home" column onto the auto-memory store specifically, and the routing test is the existing §1 Q1 universality classifier. [§7.1](#external-target-scope) then adds the one case that pair does not cover — a fact about an **external target**, a repository or system the toolkit operates upon rather than is — which is a scope gate ahead of the classifier rather than a third pole inside the assignment below.
...
The assignment above resolves between two surfaces that are both **this install's own** — the codified corpus and the auto-memory store. A third case exists and neither pole covers it: a fact about an **external target**, a repository or system the toolkit *operates upon* rather than *is*. The [§1 classifier](#tier-classifier)'s Q2 contextual-scope enumeration is closed over this install's own contexts and carries no scope value for a target, so a target fact routes by nearest fit — usually to K5 tacit, and therefore to the memory store — where nothing ever refreshes it. This subsection closes that gap. It is a scope addition on a different axis from [§4.1](#host-binding-leakage-class)'s host-binding class, and it grants no surface any new permission to hold anything.

**Scope — bounded to the evidence that validated it.** The rule below is stated for an **operator-controlled target** (one the operator owns or administers, so its referents are observable) carrying a **uniquely-resolvable referent** (one the operator-side practice can name with a predicate identifying exactly one thing in the target). Two adjacent cases are **deliberately outside this subsection** and are not governed by it: a target the operator does **not** control, whose conventions may be neither observable in the target nor the operator's own; and a referent whose practice cannot resolve it uniquely. A fact outside that bound is **surfaced to the operator for an explicit homing decision** — it is neither absorbed by the rule below nor homed by nearest fit. Stating the bound is what keeps this rule true rather than merely broad; the wider case is separate work with its own evidence.
...
| **Operator-side practice** | the operator's own decision about working with that target, stated *without* the referent and carrying the predicate that resolves it | continue to [Q1](#tier-classifier) and home normally |

A fact that has already answered Q0 YES and cannot be cleanly split is treated as target-side and read live. A fact whose practice half cannot carry a resolving predicate is outside the bound above and goes to the operator.

**The target-SSOT rule.** **The target is the sole source of truth for its own facts.** No surface of this install may hold a resolved target-side value — not the corpus, not the auto-memory store, not operator config. This is a rule about **homing**: it governs where a fact is placed when it is classified, and it does not by itself mandate a sweep of what already exists.

**The one permitted stored item, and its guard.** Bootstrapping a read requires an address, so the target's **address** is the single permitted stored item. Where the destination is already declared, that address is `operator.toml [trackers.<id>].identifier`. Two properties of that field must be stated rather than assumed:

- **It is not guaranteed to exist.** The `[trackers.*]` subtables ship **commented**; an install that has not opted into multi-destination routing has no live `[trackers.*]` section at all. Declaring one is not free: once **any** `[trackers.*]` is declared, filing resolution becomes **fail-closed** for a private-scope project whose tracker is unset, and each destination carries a `scope` that arms the scope-segregation guardrail. Declaring an address to satisfy this rule is therefore a deliberate configuration change with its own consequences, not a no-op.
- **It can go stale silently, so it needs an identity guard.** A stale address does *not* reliably fail hard: a host may redirect a renamed address and return a plausible result indistinguishable from a correct one, and a released address may later be re-taken by a different subject. The stored address is therefore **an assertion to verify, not a fact to trust** — resolve it, then confirm the resolved identity matches the configured address before any referent read is trusted, and halt on mismatch. Without that check the one item this rule permits to be stored is also the one item that goes stale invisibly.

Note also that the field's own domain is wider than a repository address — it may hold a project key or a board identifier, depending on the destination's platform — so "the address" means whatever concretely identifies the target on its own surface, not a repository path in every case.

**Reading a target-side referent.** A target-side referent is resolved by a **live read of the target through the platform's configured host-adapter seam**; the host mechanism lives only inside the adapter and is never named as the mechanism in this text. That is [§4.1](#host-binding-leakage-class)'s canonical rule applied at capability altitude. §4.1's preferred form is for the rule to name the adapter **operation** it calls — and this capability has **no such operation today**: the shipped adapter interface covers version-claiming, and its operation set is closed by its own contract. The read operation this capability needs is therefore **owed and not yet specified**, and specifying it is a governed interface change with its own conformance cost — separate work, deliberately not back-doored here. Recording the gap is the point: a future implementer must add the operation rather than infer that a binding already exists.

**Behaviour on divergence.** Because no value is stored, a target that has changed cannot serve a stale answer. It produces a **resolution failure**: the live read returns zero matches, or more than one, where the practice's predicate expects exactly one. That is a loud, halting signal. The agent surfaces it and asks the operator to restate the practice; it does not guess and does not fall back to a remembered value. **A session that cannot reach the target cannot resolve a target-side referent and halts — there is no offline cache by design, because a fallback cache would reintroduce the very defect this rule removes.**

**Where a target self-describes, that is the preferred source.** When the target documents its own conventions in-target, reading that document *is* a target-side derivation and is preferred over any operator-side restatement — the convention and its subject then version together. This rule does not require a target to self-describe and creates no obligation on a system the operator may not control.

**Bound — this scope does not weaken the corpus↔memory cut.** This subsection applies **only** to a fact whose source of truth is a repository or system other than this install's platform. Content whose SSOT is the corpus — `core/` and `release/` — is **definitionally outside this scope** and remains governed by [the two-tier assignment](#two-tier-ssot) and [the no-shadow-SSOT invariant](#no-shadow-ssot) unchanged. External-target facts are never universal-for-this-platform, so they never become K1 and never enter the [encode-and-evict lifecycle](#encode-and-evict); that lifecycle, its VERIFY-CORPUS gate, its ARCHIVE-first step, and its ordering guarantee are untouched by this subsection. Nothing here authorises a memory-resident copy of codified knowledge.

**Reversibility — split by limb.** The rule itself is **CHEAP / Confidence HIGH** (additive corpus text; reverting the commit restores prior state). Any operator-side retirement of an existing entry under this rule is a separate action on a Layer-2 surface that an engineering commit cannot make and a revert cannot undo — **IRREVERSIBLE by git**, mitigated only by the archive-before-deletion posture the [encode-and-evict lifecycle](#encode-and-evict) already establishes. Grading the corpus limb alone would misstate the profile of the package an operator acts on.

**Drift class — `external-target-referent-stored`.** Any surface of this install holding a resolved target-side referent (a target's label name, milestone title, item state, or file content recorded as a value rather than read live). **Detector deferred** — this class ships doc-only, matching the deferred-enforcement posture recorded in [`memory-architecture.md` §6](memory-architecture.md#composition-boundaries). It is therefore **not** one of [the five drift classes](#drift-classes) the standing drift audit emits, and that count is unchanged; when a detector ships, the class joins that table and the count moves five → six.
