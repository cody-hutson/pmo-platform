...
|---|---|---|
| [`operations/skills/file-router/SKILL.md`](../../operations/skills/file-router/SKILL.md) | `Context-Captured` → `Context-Structured` transition (mechanism 1) for inbound (direction 1) **and cross-project-out (direction 4 — mechanism 1 in the target project's tree)**; plus a **Domain-C promotion-gate front-door** — file-router resolves the target + enforces the approval gate for generated-staging (2) and promotion (3), citing the `promotion_state` field (`frontmatter-schema.md § Domain C` / `artifact-workflow-protocol.md §4`) rather than owning the Domain-C move | `## Movement Directions` + `## Framework Reference` subsection (demonstrating consumer) |
