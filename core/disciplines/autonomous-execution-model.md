...
**This is a general-agent class predicate, not any one surface's touchpoint roster — and it was being read as the latter.** The classes above answer *what kind of thing is worth an operator's attention* for any agent operating under this model. They are deliberately not a list of named touchpoints, and they are not a superset of any such list: against the release hub's enumeration, this sentence omits three touchpoints and carries one class (irreversible actions) that the hub holds on a separate axis as a reversibility carve-out rather than as a member. Three surfaces nonetheless cited this section as *"the single canonical enumeration"* of the hub's reserved touchpoints, which it has never been — it is at the wrong altitude to be one, and importing pipeline stage numbers and procedure names into a platform-wide agent-conduct discipline to make the claim true would be the defect, not the fix.

**The hub's instantiation lives in the Hub Gate Register** ([`orchestration-playbook.md`](/release/skills/release-hub/references/orchestration-playbook.md)), which enumerates the release hub's touchpoints with an acting party, a stop-or-execute disposition, an Autonomy Tier and a gate-eligibility verdict for each. Read this section for the class predicate; read the register for the hub's roster. Another surface instantiating these classes for its own domain does the same — it states its own roster and cites this section for the predicate, rather than marking this section canonical for a list it does not carry.
