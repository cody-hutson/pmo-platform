...
> **Amended per [SCOPE CHANGE — RESOLVED via ADR-162 + ADR-163] (one-system-of-record-per-element, Tier-2):** two **field-list** amendments in one reopening, **no roster change**. **(1)** Project ([#1](#1-project)) gains `health_rag` + `health_derived_date` per [ADR-163](../ADRs/ADR-163-project-health-home.md) — the project-level composed health RAG and its derivation stamp, mastering the value `portfolio-writeback-contract.md` §2 previously mis-sourced to the Axis-1 lifecycle field. `health_rag` is a **materialized projection, never self-reported**: bands and the transparent roll-up rule stay owned by `channel-formats.md` § RAG Threshold Standards (ADR-065, retained owner), indexed by `metric-registry.md` § Project Metrics, composed in `weekly-status-rollup` Section 1, produced by `ppm-agent`; `health_derived_date` is its required companion stamp (**V-PRJ-11**, biconditional). **(2)** RAID Item ([#6](#6-raid-item)) and Work Item ([#18](#18-work-item)) adopt the optional **external-identity group** — `source_system` · `external_id` · `mirrored_date` (`entity-field-schemas.md` §3.0c, **V-EXT-01..03**) — recording which system authors a mirrored element and under what key, so the per-element system-of-record rule in [ADR-162](../ADRs/ADR-162-system-of-record-per-mirrored-element.md) is applicable per record. `source_system`'s value domain is `frontmatter-schema.md` § Category 3; `c3-external-sync-path-b.md`'s `source_adapter` is that field's persistence-dialect name, not a second field. Every field in both groups is **optional**, so no live record becomes malformed and no migration is owed. The Entity Core ([§3](#3-entity-core-schema) / Frozen Artifact 2) is untouched and **the roster is unchanged at 19** — both are field-list amendments, precedented by ADR-040. Roster + field lists **RE-FROZEN as amended**, authorized by this milestone's Stage 9 Plan Review GO.
...
- **Fields:** `project_name`✅ str·1 · `project_owner`⚪ ref·1 → `Person.person_id` · `project_owner_external`⚪ str·1 *(external fallback — exactly-one-of {`project_owner`, `project_owner_external`}; Tier-2 SCOPE CHANGE — ADR-040)* · `status`✅ enum·1 · `delivery_approach`✅ enum8·1 · `portfolio_id`⚪ ref·1 · `program_id`⚪ ref·1 · `health_rag`⚪ enum·1 · `health_derived_date`⚪ date·1 *(the project-level composed health RAG and its derivation stamp — `entity-field-schemas.md` §3.1, V-PRJ-09/10/11; a materialized projection, never self-reported)*
...
- **Fields:** `raid_type`✅ enum{Risk,Assumption,Issue,Dependency}·1 · `summary`✅ str·1 · `project_id`✅ ref·1 · `owner_person_id`✅ ref·1 · `severity`⚪ enum·1 · `target_date`⚪ date·1 · **`impact`✅ str·1** *(Option-A amendment — business impact if realized/unresolved; legacy RAID Log Tracker 5 = Required; irreducible primary DATA)* · `source_system`⚪ enum·1 · `external_id`⚪ str·1 · `mirrored_date`⚪ date·1 *(the optional external-identity group — `entity-field-schemas.md` §3.0c, V-EXT-01..03; absent unless the record mirrors an element authored elsewhere)* · **`action_plan`⚪ str·1** *(Option-A amendment — what is being done about it; legacy conditionally-required)*
...
**Rationale & first-class justification.** A project deliverable file (report, tracker, FDD, design doc, etc.). First-class because it is the explicit **reconciliation seam** to `frontmatter-schema.md`: its `domain` + `content_lifecycle_pattern` bind the entity to a backing file's Domain A/B/C lifecycle. Its Axis-1 *delegates to Axis-2* — the Artifact's operational lifecycle **is** the Domain A/B/C content lifecycle of its backing file. G3/G4 must honor this seam when physicalizing. When an Artifact is **dual-format** (an agent-native source plus a stakeholder-facing rendering), its source/target pair is governed by the [Dual-Format Document Model](../standards/dual-format-document-model.md), whose `source.domain` + `content_lifecycle_pattern` ARE these reconciliation-seam fields — the model reuses this seam, adds no new field, and introduces no parallel concept.
...
- **Fields:** `artifact_title`✅ str·1 · `artifact_type`✅ enum·1 *(values = `frontmatter-schema.md` Type Taxonomy — referenced, not redefined)* · `project_id`✅ ref·1 · `domain`⚪ enum{A,B,C}·1 · `version`⚪ str·1
  - **On `domain`:** this is concept 2 (artifact-provenance) of the six concepts the bare token names — corpus index at `core/specs/domain-token-registry.md`. `A`/`B`/`C` are **deprecated aliases** of the live values `source`/`managed`/`generated` per `frontmatter-schema.md` § Category 6; the enum above is stated in the deprecated vocabulary and is not migrated here.
...
- **Fields:** `work_item_type`✅ str·1 *(discriminator; value domain = the declarative type registry — `[ASSUMPTION–CONFIRM @ C2 type layer]`)* · `parent_ref`✅ typed-ref·1 *(polymorphic `BELONGS_TO` → Milestone.id OR Workstream.id)* · `source_system`⚪ enum·1 · `external_id`⚪ str·1 · `mirrored_date`⚪ date·1 *(the optional external-identity group — `entity-field-schemas.md` §3.0c, V-EXT-01..03; absent unless the record mirrors an element authored elsewhere)*
...
plan_type: cutover                       # OPEN discriminator — registered value (entity-field-schemas.md §3.4a; resolved at v3.37)
project_id: acme-implementation
version: "2.0"
