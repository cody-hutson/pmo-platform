...
> **Amended per [SCOPE CHANGE — RESOLVED via ADR-164 + ADR-163] (one-system-of-record-per-element, Tier-2):** two **field-list** amendments in one reopening, **no roster change**. **(1)** Project ([#1](#1-project)) gains `health_rag` + `health_derived_date` per [ADR-163](../ADRs/ADR-163-project-health-home.md) — the project-level composed health RAG and its derivation stamp, mastering the value `portfolio-writeback-contract.md` §2 previously mis-sourced to the Axis-1 lifecycle field. `health_rag` is a **materialized projection, never self-reported**: bands and the transparent roll-up rule stay owned by `channel-formats.md` § RAG Threshold Standards (ADR-065, retained owner), indexed by `metric-registry.md` § Project Metrics, composed in `weekly-status-rollup` Section 1, produced by `ppm-agent`; `health_derived_date` is its required companion stamp (**V-PRJ-11**, biconditional). **(2)** RAID Item ([#6](#6-raid-item)) and Work Item ([#18](#18-work-item)) adopt the optional **external-identity group** — `source_system` · `external_id` · `mirrored_date` (`entity-field-schemas.md` §3.0c, **V-EXT-01..03**) — recording which system authors a mirrored element and under what key, so the per-element system-of-record rule in [ADR-164](../ADRs/ADR-164-system-of-record-per-mirrored-element.md) is applicable per record. `source_system`'s value domain is `frontmatter-schema.md` § Category 3; `c3-external-sync-path-b.md`'s `source_adapter` is that field's persistence-dialect name, not a second field. Every field in both groups is **optional**, so no live record becomes malformed and no migration is owed. The Entity Core ([§3](#3-entity-core-schema) / Frozen Artifact 2) is untouched and **the roster is unchanged at 19** — both are field-list amendments, precedented by ADR-040. Roster + field lists **RE-FROZEN as amended**, authorized by this milestone's Stage 9 Plan Review GO.
