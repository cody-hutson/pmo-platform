...
- [`knowledge-architecture.md §7`](knowledge-architecture.md#memory-corpus-boundary) is the **Knowledge cut** — the corpus↔memory boundary, the encode-and-evict lifecycle, and the five drift classes; its [§7.1](knowledge-architecture.md#external-target-scope) adds the external-target scope.
...
| **`projects/_config/CORRECTIONS.md`** (program tier — platform-wide redirects, read after CLAUDE.md) **+ `[Project]/CORRECTIONS.md`** (project tier — project-scoped redirects, read after PROJECT.md; overrides the program tier on the same claim) | Learning | both (Cowork + Claude Code) | Claude Code (operations) | **Tier 1** (operator gives the correction) | on every correction (Tier 1.7) | **operator-write-only** | operator issues a behavioral correction — project-scoped (a project's systems / people / cadence / artifacts) routes to the project tier; a redirect that holds regardless of the active project routes to the program tier |
...
The invariant reaches external-target facts by the same logic: a resolved target-side referent held on any surface of this install is a shadow of a source of truth that lives in another repository or system, and is prohibited on exactly the same grounds — see [`knowledge-architecture.md §7.1`](knowledge-architecture.md#external-target-scope).

---

## §4 Read protocol {#read-protocol}

Reads are governed by the [CLAUDE.md](<OPERATOR_INSTANCE_CLAUDE_MD>) **Context File Hierarchy** and **Session Management** sections — this contract cites that read order and does not restate it. The session-start read order (most-to-least specific) is: SESSION_STATE.md → SWAP_HANDOFF.md (when newer than last session activity) → CLAUDE.md → `projects/_config/CORRECTIONS.md` (program tier — platform-wide redirects) → OPERATIONS.md → the relevant PROJECT.md → `[Project]/CORRECTIONS.md` (project tier — project-scoped redirects; read best-effort, an absent file is normal rather than a drift finding). More-specific surfaces override less-specific ones on the same claim — so the project-tier corrections file overrides the program-tier one where both speak to the same claim.
...
- **External-target facts are out-of-table.** A repository or system the toolkit *operates upon* is not a memory surface of this install — it is an external source of truth, and its facts are read live rather than held. The rule (the decomposition gate, the target-SSOT assignment, the address guard, and the divergence behaviour) is [`knowledge-architecture.md §7.1`](knowledge-architecture.md#external-target-scope), bounded there to operator-controlled targets with uniquely-resolvable referents. The only item this install stores for a target is its address — already covered by the `operator.toml` row above, and subject to that rule's identity guard rather than trusted as given. Noted here to forestall a "missing row" reading.
...
- **External-target scope** — [`knowledge-architecture.md §7.1`](knowledge-architecture.md#external-target-scope): the decomposition gate (Q0), the target-SSOT rule, and the divergence behaviour for a fact about a repository or system the toolkit operates upon.
