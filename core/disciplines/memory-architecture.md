...
Four contract-level write rules:
...
4. **No unrequested save.** A write that **creates** a new entry in the operator auto-memory store (`~/.claude/memory/`), or adds a **new durable claim** to an existing entry, is permitted only when one of two conditions holds: **(a)** the operator asked, in the current turn, for it to be remembered; or **(b)** the agent has read the corpus for an existing governed home for the claim, found none, and states the justification — that the claim is **durable** (still true and useful months from now) *and* **non-obvious** (hard to re-derive from the corpus or the workspace). With neither condition met, **the default is not to save**; being unsure resolves to not-saving, never to saving. Three shapes recur and each fails both conditions: banking a mid-task learning unprompted; restating content the corpus already governs (a shadow SSOT under §3); and saving a conversational one-off (not durable). The deployed obligation — the observable trigger, the required read, and the emitted token — is [`decision-time-adherence.md`](../rules/decision-time-adherence.md) § 2 `DTA-6`.
