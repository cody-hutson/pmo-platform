...
4. **Co-manifestation (rung 4 — generalizes dual-framing co-manifestation).** If the OOM legitimately runs *both* contexts (e.g., the orthogonal co-management dual-framing flag → an Agile framing AND a Waterfall framing, or a Hybrid `[A, B]` two-archetype project → both constituent framings), the resolution is **produce both framings**, not pick one. This **names and generalizes** the dual-framing co-management capability ([CLAUDE.md § Dual-Framing Bridge](<OPERATOR_INSTANCE_CLAUDE_MD>) is its operator-instance instance — the intake's `[SOURCE]` evidence: "ad-hoc, not generalized to other practice conflicts" — now generalized to any dual-legitimate-context conflict). Composes with [`methodology-parameterization-v1.md`](../../release/references/specs/methodology-parameterization-v1.md) when the dual context is methodological — including a Hybrid two-archetype combination, where co-manifestation is the union of the two native framings independent of whether the co-management flag is set.
...
Every rung routes through or reinforces an existing mechanism (M1, evidence tiers, methodology axis, CLAUDE.md Dual-Framing Bridge). This is the structural guarantee of the no-contradiction AC: the framework is reference *content* the existing decision procedure consults, never a parallel runtime.
