...
| [`memory-architecture.md`](../disciplines/memory-architecture.md) | **Cross-surface memory read/write contract.** The SSOT for the read/write *class* of every operational surface (who reads/writes under what Autonomy Tier). [§8](#8-operational-state-lifecycle) is the *lifecycle* view of those same surfaces and points back at this contract's per-surface row for the read/write authority — reference-by-row, no duplication. |

---

## 8. Operational-State Lifecycle

**This section is ADDITIVE — §1–§7 (the FROZEN §2 schema, §3 decision tree, §4 derivation rules, §5 all 48 rows, §6 derived-signals, §7 cross-references) and the AC Build-Checklist are unchanged by it.** It adds the **over-time lifecycle dimension** — how each operational surface *grows, is cleaned, archives, and retires* — which is a different concern from the entity-crosswalk axis (`source_entity` / `schema_status` / `reconciliation_flag`) the §2 schema owns. The two axes compose without collision (the same orthogonality the four-axis reconciliation establishes): §5.3 = *which entity's record a surface persists*; the cross-surface memory contract ([`memory-architecture.md`](../disciplines/memory-architecture.md)) = *who reads/writes it under what authority*; this §8 = *how it grows/cleans/retires over time*. The read/write *class* of every surface below is **governed-by** that cross-surface contract (single-home); this section is **governed-by** it for read/write authority and owns only the lifecycle axis. The §5.3 `[ASSUMPTION – CONFIRM]` `source_entity` cells are **not touched** — §8 adds no `source_entity` claim; the lifecycle axis is decidable independently of the unresolved entity axis (downstream-triage-owned).

The table is keyed by **surface-class** (the §3 taxonomy values, with the `structural-file` class split into its named state files because they have *divergent* growth-patterns), **not** by the 48 §5 artifact rows: growth / cleanup / archive / retention are properties of the surface *class*, not of each artifact — every core-tracker grows append-event; every generated-artifact is inherit-per-file. A class-keyed table is therefore the non-denormalized home, the same anti-gold-plating logic the §2 "why 8 and not 9 columns" note applies. **To resolve any §5 artifact's lifecycle: look up its §5 `class`, then read the §8 row for that class** (for the four named `structural-file` state files, read the file-specific row).

| surface / class | growth-pattern | cleanup-trigger | archive-path | retention / retirement | lifecycle SSOT |
|---|---|---|---|---|---|
| **core-tracker** (Daily Status Log, Comms Tracker, Open Meetings, Transcript Register, RAID Log) | `append-event-log` | per-artifact cadence (RAID: closure→ARCHIVE; Comms: weekly cleanup; Daily Status: carry-forward roll) | in-file `ARCHIVE` / `CORE` section (RAID, Comms); register stays an index | **never purged** — closed items move to the in-file ARCHIVE split (RAID Log: closed items archived, never purged) | `OPERATIONS.md §Operational Artifacts` cadence column + [`tracker-schemas.md`](../schemas/tracker-schemas.md); Work-type row in the cross-surface memory contract |
| **structural-state-file → `SESSION_STATE.md`** | `single-record-overwrite` | session-end overwrite (the Session-End Checklist) | n/a — overwrite (prior state not retained in-file) | **bounded — current handoff only**; refreshed in place when stale (the trim rule below), never accreted | `OPERATIONS.md §CORRECTIONS Graduation & Expiry Protocol` neighbor + Work-type row in the cross-surface memory contract; CLAUDE.md §Session Management |
| **structural-state-file → `PORTFOLIO.md`** | `single-record-overwrite` (per-project rows updated in place) | project state change (Cowork writes) | n/a — overwrite | **bounded** — current cross-project health snapshot; Claude Code read-only (operations-bridge) | Work-type row in the cross-surface memory contract (Cowork auto-write); [`operations-bridge.md`](../rules/operations-bridge.md) |
| **structural-state-file → `CORRECTIONS.md`** | `append-correction` (grows per correction; **the unbounded-drift surface this section targets**) | **scheduled graduation/expiry review** (the OPERATIONS.md protocol below) + the existing Pattern-Review event trigger | graduated → corpus (encode-and-evict); expired → Pattern-Review Decision-Briefing record (ARCHIVE-first) then trim | **graduate (Learning→codified) or expire** — no correction sits un-reviewed indefinitely; mature → governance, stale-and-ungeneralized → retire | **`OPERATIONS.md §CORRECTIONS Graduation & Expiry Protocol`** ⟶ [`ADR-029`](../ADRs/ADR-029-memory-corpus-ssot-boundary.md) encode-and-evict; Learning-type row in the cross-surface memory contract |
| **structural-state-file → `SWAP_HANDOFF.md`** | `single-record-overwrite` | auto-overwrite at next swap (harness) | n/a — overwrite (snapshot only on manual preserve) | **bounded** — last-swap only; do not hand-edit (CLAUDE.md §Cross-account handoff) | Work-type row in the cross-surface memory contract (Tier-3 harness auto-write) |
| **generated-artifact** (status reports, briefings, FRDs, comms drafts, …) | `inherit-per-file` | `08-Generated/` **10-business-day** auto-archive sweep if unreviewed | promoted → target 01–08 folder; unreviewed → archived | **per-file lifecycle (A/B/C)** inherited from [`frontmatter-schema.md`](../schemas/frontmatter-schema.md); staging is transient | CLAUDE.md §08-Generated + [`frontmatter-schema.md`](../schemas/frontmatter-schema.md); the cross-surface memory contract notes `08-Generated/` is a transient surface, not memory |
| **typed-plan** (release / implementation / project / test / comms / training plans) | `append-only-history` (baselined; superseded retained) | phase-gate baselining (`operational → baselined-at-phase-gate → superseded`) | Artifact Register superseded rows (append-only) | **superseded rows append-only** (configuration-management history) — never deleted | `OPERATIONS.md §Configuration Management` + [`tracker-schemas.md`](../schemas/tracker-schemas.md); #4 Plan entity |
