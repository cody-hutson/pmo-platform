...
|---|---|---|---|---|---|---|---|
| 1 | **Deliverable-class** | What kind of deliverable a release or project produces — the abstract class that design-aware mechanisms branch on to select the matching best-practice guide. | Open. Named classes: `software`, `governance`, `web`, `data`, `security`, `enterprise-platform`, `hardware`, `process`, `support`, `task-artifact`; a free domain name is permitted where no guide exists yet, and is itself the demand signal for authoring one. **`task-artifact` is gate-load-bearing** — it is the one value in this cell that a gate branches on (Stage-13 close routes to an artifact-acceptance close on it, per `core/schemas/gate-criteria-spec.md` § Close-Class Conditioning Disposition), which is why it must resolve here in one lookup and not merely be a free name. | `release/references/pipeline/stage-04-planning.md` § 5.7. Where a project declares one, `core/schemas/project-schema.md` `deliverable_type` is the authoritative source this field reads. | A `domain:` key nested inside a `domain_practice: { … }` provenance label. | Release plan files under `release/releases/plans/`. | **Concept 6**, which shares the values `governance` / `software` / `process` / `support` / `security` / `data`. Concept 1 classifies the **deliverable**; concept 6 classifies the **document**. |
