...
**Division of labor with the deployed rule.** This section owns the predicate. The **deployed trigger index** — the bounded set of checkpoint signatures, each binding a governing rule, a required ground-truth read, and an emitted token — lives in `core/rules/decision-time-adherence.md`, together with the index-eligibility gate that bounds its growth. This section defines the predicate; that rule deploys it and resolves it to the specific checkpoints — which signature fires, what read discharges it, and what token is emitted.

**The predicate is deliberately carried on both surfaces, and that duplication is registered.** This section and that rule's `§ 1 — What fires a checkpoint` state the same two-limb trigger, the same three intermediate-signal shapes, and the same observable-not-introspected constraint, in near-identical words. It is the two files' only shared region — outside it they share nothing longer than a sentence fragment. The copy is required rather than tolerated: the platform's mirror-pair registry, the surface that makes a corpus file reach a running agent, covers the rules directory and contains no specs-directory entry, so a deployed rule that cited this section for its own trigger would point a running agent at a file absent from its machine. Consolidating is what would break that self-sufficiency; byte-identity registration does not fit, because the two statements are deliberately not word-identical; and the complementary-pair registry admits only same-basename pairs whose exclusively-owned sections are *absent* from the peer, which a shared section is not. The pair is therefore registered as a per-domain prose exemption under the register-or-remove rule in `core/standards/duplicate-source-discipline.md` § 1 condition 3, and this paragraph is that record.

**Co-evolution obligation.** An edit to the predicate — either limb, the three intermediate-signal shapes, or the observable-not-introspected constraint — must land on both surfaces in the same change. Nothing detects a one-sided edit mechanically: the mirror check compares that rule to its deployed copy, never this section to that one. If the mirror-pair registry ever gains a specs-directory entry, the necessity stated above lapses and this exemption is re-decided rather than carried forward.
