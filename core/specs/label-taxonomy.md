...
|---|---|---|---|---|
| `initiative:project-data-architecture` | `0052CC` (blue) | Project Data Architecture initiative — entity model, storage, automation, interface, governance (5-layer framework) |  | [`<OPERATOR_INSTANCE_ROADMAPS_PATH>/project-data-architecture.md`](<OPERATOR_INSTANCE_ROADMAPS_PATH>/project-data-architecture.md) (operator-local) |
| `initiative:template-architecture` | `0052CC` (blue) | Template Architecture initiative — 5-layer framework: taxonomy, catalog, storage, lifecycle, governance |  | [`<OPERATOR_INSTANCE_ROADMAPS_PATH>/template-architecture.md`](<OPERATOR_INSTANCE_ROADMAPS_PATH>/template-architecture.md) (operator-local) |
| `initiative:knowledge-architecture` | `0E8A16` (green) | Knowledge Architecture & Best-Practice Discipline initiative — follows the `initiative:skills-mini-project` pattern |  | — |

**Usage pattern:** Apply to umbrella ticket, all child tickets (new), and any existing tickets that are absorbed under the initiative scope. Query `gh issue list --label "initiative:project-data-architecture"` returns the complete landscape of work tied to the initiative. Roadmap doc §3 Now/Next/Later sequences all labeled issues into the architected path-to-done.

> **Sibling label not yet registered here:** `initiative:skills-mini-project` (no registration ticket filed). It exists on GitHub and follows the same usage pattern; a future PR should register it in this section.
