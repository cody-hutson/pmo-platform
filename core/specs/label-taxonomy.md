...
**Detection signal — a three-way conjunction, because the family shape alone is symmetric.** The mis-tiering fingerprint is *not* "an epic sharing a `project:` label with other epics that are not its native children". That shape was measured against the live tracker and matches **39 of 42 open epics**, because it is **symmetric**: a leaf epic inside a family shares its label with exactly the same non-children its container does, so every leaf matches it by construction and no threshold separates the two. The signal is the conjunction of all three of the following, which no leaf epic satisfies together:
