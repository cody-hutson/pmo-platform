...
7. **Cross-domain bridge writes into `pmo-platform/`** — Cowork (a `projects/` session) never writes to `pmo-platform/`.
   ...
   - *Enforcement:* `block-autonomy-ceiling.sh` `BLOCK-AUTONOMY-002`, `always_block`, evaluated above the master-activation gate.
...
7a. **Cross-domain bridge writes into `projects/`** — Claude Code (a `pmo-platform/` session) writing into the operations workspace. **Tier 2, not Tier 0.**
   ...
   - *Enforcement:* `block-autonomy-ceiling.sh` `BLOCK-AUTONOMY-004`, `apply_block`, mode-gated and evaluated below the master-activation and workspace-scope gates. Under the shipped master-OFF default it is inert.
