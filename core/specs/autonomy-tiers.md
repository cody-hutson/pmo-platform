...
6. **Governance-file modification without approval** — CLAUDE.md, OPERATIONS.md, RELEASE_PROTOCOL.md, the `.claude/` security surface, etc. require Issue + plan + approval.
   ...
   - *Enforcement:* `block-autonomy-ceiling.sh` `BLOCK-AUTONOMY-001`, `always_block`, evaluated above the master-activation gate. Every member is identified by resolved LOCATION, never by basename alone, so a same-named file elsewhere on disk is not in the set. The enumerated member list is held in the hook's registry entry for that rule and is deliberately not duplicated here.
