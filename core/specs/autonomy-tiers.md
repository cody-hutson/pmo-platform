...
**What that permanence attaches to.** It attaches to a **correctly-scoped item**, not to an item's wording as first drafted. No approval, standing authorization, or `automation_level` setting can lower a listed item out of Tier 0 — that is the permanence, and it is absolute. What remains available is the prior question of whether an item is scoped to the acts that actually warrant it: an item drafted more broadly than its own stated rationale supports is corrected by re-scoping the item, through the governed change path, with the reasoning recorded in an ADR. Item 7 is the worked example — it was written symmetrically over two directions whose risks are not symmetric, and the direction that carries its rationale stays Tier 0 permanently while the direction that never did is re-tiered (see item 7a and `core/ADRs/ADR-149-cross-domain-bridge-writes-are-not-symmetric.md`). The distinction is the whole safeguard: re-scoping is a visible, reviewed, evidence-bearing act; lowering a correctly-scoped item is not available at all.
...
- Action modifies governance state. **This bullet decides no membership except the one exclusion it states at the end.** The deciding authorities are the two § Irreducible Human Tasks item 6 already cites, and this bullet deliberately does not re-cite them: item 6's *Source rule* decides what is **governed**; item 6's *Enforcement* routes to the registry entry that decides what is **mechanically blocked**. Those are two different questions — read each at the one that answers it, and do not read an answer to one as an answer to the other. **Two surfaces are contested and this bullet does not settle them:** the Autonomy Tier of a write to `PORTFOLIO.md` or `SESSION_STATE.md` is answered inconsistently across those two authorities and `core/disciplines/memory-architecture.md` § 2, whose write-authority column binds to this document. That conflict is open and is settled by neither this bullet nor any authority cited here, so read all three named surfaces before classifying a write to either file, and do not read this bullet's silence as a resolution. A skill's `SKILL.md` is deliberately NOT governance state for this indicator — see § Irreducible Human Tasks item 6a.
...
**Observable indicators:**
- Scope is named in a manifest, allowlist, or schema (e.g., `cascade_scope` field, `.claude/script-execution-allowlist.txt`, `08-Generated/` directory). **A named scope is a Tier-2 *declaration*; whether it is also *enforced* is a separate question.** `.claude/script-execution-allowlist.txt` is read by a PreToolUse hook, and such a control is in force only when **all four** of these hold, failing when any one fails: (1) **loading** — the session resolved a settings surface declaring the hook wiring (any session, main or spawned, whose working directory is under the governed workspace root; a session resolving no such surface loads no hooks at all, and one outside the root is excluded by the scope guard); (2) **bypass** — `CLAUDE_HOOK_BYPASS` was not set in the launching environment (layer 1, which exits **both** hook classes, so the security/workflow asymmetry does **not** exist there); (3) **master-activation class** — `security` always enforces, `workflow` is inert while master activation is off; (4) **mode** — most block hooks warn-and-allow in warn mode, a minority are mode-independent. Naming fewer than four overstates the coverage; canonical statement in [`subagent-security-posture.md` § 3.1](../standards/subagent-security-posture.md). **The tier classification does not depend on enforcement** — a declared scope is what makes an action Tier 2 rather than Tier 3, and that holds whether the declaration is machine-enforced or honored by convention. What must not happen is a *risk assessment* that reads the declaration as an interlock without checking those four conditions on the path the action actually runs on.
...
4. **Governance-modification test** — Does the action modify a governance file? **This test decides no membership except the one exclusion it states at the end.** Its one authority is the *governed* class, cited by § Irreducible Human Tasks item 6 under *Source rule*; the registry entry item 6 cites under *Enforcement* answers a different question — what a hook mechanically refuses — and is **not** this test's authority. **This is a widening, not a restatement:** the three-file enumeration this test previously carried inline is removed rather than narrowed, and the *governed* class it now routes to reaches surfaces that enumeration never named — classify from the authority, never from the removed list. This test also inherits the unresolved conflict named in the Tier-0 observable indicator above; read it before answering for either surface named there. If yes → Tier 0 unless the change is part of an approved release plan with explicit governance scope (then Tier 3 within plan; never Tier 2 — governance never auto-cascades per OPERATIONS.md cascade rule). A skill's `SKILL.md` is deliberately outside this test's file set: it is governed by a sanctioned editor session plus its own edit-time gate per § Irreducible Human Tasks item 6a, and answering this test "yes" for a skill returns the wrong tier.
...
6. **Governance-file modification without approval** — requires Issue + plan + approval. **This line names the obligation, not its members.** What is *governed* is decided by the *Source rule* cited below; what is *mechanically blocked* is decided by the registry entry cited under *Enforcement* below. Those are two different questions and this line answers neither. The enumeration this line previously carried was illustrative and open-ended, which let a reader take it for a closed set; no closed set may be derived from this line or from anything else in this document.
   ...
   - *Enforcement:* `block-autonomy-ceiling.sh` `BLOCK-AUTONOMY-001`, `always_block`, evaluated above the master-activation gate. **This bullet states no membership rule — neither the enumeration nor any property of it.** Both the member list and the tests that decide membership live in the hook's registry entry for that rule (`core/rules/bypass-mode-readiness/block-autonomy-ceiling.md`); read membership there, and treat nothing here as licensing an inference about it. In particular, a governance document inside a working tree of this repository is a case that entry decides and this one does not — do not read the absence of a location from this bullet as absence from the set. *Why the citation extends to the rule and not only the list:* an earlier revision of this bullet declined to duplicate the member list and then restated an invariant over it, which a later change to the membership test falsified while the list itself stayed correct. A restated property drifts exactly as a restated list would, with none of the scrutiny a duplicated list attracts.

6a. **Skill-definition edits — a skill's `SKILL.md` and its `references/`** — **not a member of item 6's set.** Governed by a purpose-built edit-time gate plus a sanctioned editing session. **Tier 1, not Tier 0.**
   - *Why NOT irreducible:* Every other member of item 6 has **no sanctioned agent-editing path at all**, which is precisely what makes an unconditional block the right instrument for it. Skills are the exception: they have a purpose-built editing skill (`pmo-skill-editor`) and a purpose-built gate (`block-skill-direct-edit.sh`, `BLOCK-SKILL-EDIT-002`) that distinguishes a live, correctly-targeted, non-stale editor session from a straight edit — migration-marker scoping, session-target matching, a 30-minute sentinel TTL, and an exemption surface. An `always_block` can make none of those discriminations, because it consults no signal at all. Carrying skills in the floor therefore duplicated that gate with a blunter instrument AND made the sanctioned path structurally unreachable: a valid editor session satisfied the skill gate and was still denied by the floor, leaving an agent that had followed every documented instruction with no permitted way to proceed.
   - *This is a scope correction, not a relaxed floor:* a sanctioned session does **not** satisfy a Tier-0 floor, and no session type ever will — that is what irreducible means. Skills left the floor because they were never rightly in it, not because membership became negotiable. The assertion moves from "no agent may edit a skill" to "no agent may edit a skill outside a sanctioned session." The members that remain in item 6 admit no such qualifier, and adding one to them would be a relaxation.
   - *Still governed, at Tier 1:* CLAUDE.md "No ungoverned changes" continues to require an Issue plus an approved implementation plan before a skill is modified. Leaving the irreducible floor moves a skill edit from never-delegable to delegable-after-approval — it does not make one ungoverned, and this item is not authority to edit a skill without that approval.
   - *Correct path:* mint an editor session through the sanctioned minter (`core/deploy/tools/start-skill-editor-session.sh`, whose TTL is pinned to the gate's own sentinel TTL) and edit through `pmo-skill-editor`. A direct edit outside such a session is still refused.
   - *Coverage caveat — the governing gate is weaker on a default instance than the floor it replaced:* `BLOCK-SKILL-EDIT-002` is `workflow`-class and sits behind master activation, which ships OFF, whereas the item-6 floor is always-block LIVE regardless of mode and level. Where the four coverage conditions do not all hold — and on a default instance they do not — the edit-time gate is a **convention, not an interlock**, and the deploy-time checks are the enforcing half of the pair. Any doc citing this gate as a control owes that boundary; the canonical statement is cited below rather than restated here.
   - *Source rule:* `core/standards/canonical-skill-structure.md` § Coverage boundary of the edit-time gate — canonical statement.
   - *Enforcement:* `block-skill-direct-edit.sh` `BLOCK-SKILL-EDIT-002` at edit time (master- and mode-gated) plus `deploy.sh --check` skill-structure checks at deploy time (unconditional). `block-autonomy-ceiling.sh` `BLOCK-AUTONOMY-001` deliberately does NOT cover this surface.
   - *Provenance:* commit `2351582c` (2026-08-14) removed skills from the `BLOCK-AUTONOMY-001` case set and added the sanctioned minter, making the two-gate design reachable for the first time. Footnote only — the rule above stands without it.

7. **Cross-domain bridge writes into `pmo-platform/`** — Cowork (a `projects/` session) never writes to `pmo-platform/`.
   - *Why irreducible:* The two domains are **sibling directories, not nested**, and only this direction can place content where it becomes committable and pushable: `pmo-platform/` is a git repository and is **public**. An operations session writing into it is a disclosure surface, not merely a layer-integrity question. Irreducible in the strict sense — no `automation_level`, no mode, and no master-activation state permits it.
   - *Source rule:* `.claude/rules/operations-bridge.md` § Rules for Claude Code (rules 1, 2, 4).
   - *Enforcement:* `block-autonomy-ceiling.sh` `BLOCK-AUTONOMY-002`, `always_block`, evaluated above the master-activation gate. A target counts as `pmo-platform/` when it sits in any working tree of the platform repository, wherever that tree lives — repository membership through the shared helper `core/hooks/lib/platform-membership.sh`, per `{{ADR:cross-domain-classification-is-repository-membership}}` — so a platform worktree outside the checkout is covered.

7a. **Cross-domain bridge writes into `projects/`** — Claude Code (a `pmo-platform/` session) writing into the operations workspace. **Tier 2, not Tier 0.**
   - *Why NOT irreducible:* `projects/` is not a git repository and is not in the platform repo's path space at all (`git check-ignore` reports it as *outside repository*), so this direction cannot reach the tracked repo and carries **no disclosure component**. What remains is a layer-ownership signal — Cowork owns `projects/` — which is real but is a discipline concern, not an operator-irreducible act. Enforcing it at always-block severity made the control blunter than the risk it models, and the remedy it named (*route the change through the owning agent*) does not operationally exist in a single-operator workspace.
   - *Correct response:* relaunch the work in the operations workspace, where it is in-domain. The bounded-auto surface is the write itself, not the boundary crossing.
