...
- **Mitigation:** before emitting a delta, query the log for the session's existing rows (`query-pipeline-event.sh --release <milestone-slug> --event-subtype recommendation-choice-delta` — `--release`, not `--version`: the join key is the milestone slug per `pipeline-event-log-schema.md` § 2a, and a raw `--version vX.Y` filter returns zero rows for a slug-keyed release, which would read as "the live path emitted none" and produce exactly the duplicate this mitigation exists to prevent). Emit a hindsight delta ONLY when the live path emitted none, and mark it `via:session-retro` so the provenance is explicit. Everything else the retro emits — the no-decision feedback class, session friction — is structurally outside the decision-moment path and never duplicates it.
