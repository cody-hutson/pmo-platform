...
- `evals.json` — test prompts in the preserved eval-harness schema (id, name, prompt, expected_output, files, assertions). **Two runners consume it, and they answer different questions.** `pmo-skill-refiner/scripts/run_eval.py` is a *trigger* harness (`{query, should_trigger}`) — it grades whether a skill fires, never what it produced. `pmo-skill-refiner/scripts/run_scenario_eval.py` is the *output-scoring* runner: it executes the assertion-grading path (`assertions[]` → `grading.json`) for the **deterministic** assertion types, through an optional `assertions[].check` predicate an assertion may carry. **Scope of that execution, stated so it is not over-read:** the deterministic types (`structural` / `resolution` / `read-only` / `non-triviality`) are runner-executed; `judgment` and `acceptance` remain a **grader-honored contract, not runner-executed** — the grader agent honors them in prose. Assertion `type` is still an open enum with no code validator; the runner branches on `check.kind`, not on `type`. An assertion carrying no `check` is **ungraded** and leaves both numerator and denominator, so an existing suite needs no edit to stay valid. Contract: `release/skills/pmo-skill-refiner/references/scenario-eval-contract.md`.
