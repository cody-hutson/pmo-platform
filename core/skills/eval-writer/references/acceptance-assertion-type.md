...
| Rule | Parse behavior |
|---|---|
| **P1 — Source block** | Ingest ONLY the list items under the issue body's `Acceptance Criteria` **or** `Completion condition (verifiable)` heading — at any heading level (the issue forms render H3; the task and spike body shape renders H2), matched from the start of the heading text, so `Acceptance Criteria (testable)` is read and `Cross-Issue Acceptance Criteria` is not. The first matching heading opens the block and the next heading of any level closes it — a second matching heading included, so two lists are never unioned. `release/tools/check-ac-binding.py` reads the same vocabulary. |
| **P2 — Item shape** | A block's criteria are its top-level list items of one shape: its task-list lines (`- [ ] <text>` or `- [x] <text>`) when any are present; otherwise its ordered items (`1. <text>`, a checkbox after the number allowed); otherwise its plain `-` / `*` bullets. In a task-list block a plain bullet is an annotation — a declined or superseded item — not a criterion. A nested item attaches to its parent as detail, never as a separate criterion, and a thematic break (`* * *`) is not an item. `release/tools/check-ac-binding.py` applies the same rule. |
