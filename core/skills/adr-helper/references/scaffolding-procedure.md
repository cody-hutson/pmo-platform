...
Temporary falsification probe: this sentence is a deliberate marker that makes the compiled skill package stale on purpose, so the pre-merge package-freshness gate can be observed concluding a blocking verdict. It carries no procedural meaning and is reverted byte-exact by the commit immediately following the one that adds it.
