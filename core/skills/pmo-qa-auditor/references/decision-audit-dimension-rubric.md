...
| **DS4** | Every firing of the matched gate carries a delta row, **the aligned zero-delta case included** — the aligned row is what separates *the recommendation was adopted* from *nobody recorded anything*. The row's `via:` value records which route evidenced the firing; it does not decide how many firings there were | rows appearing only where the choice diverged, so the aligned state is inferred from silence rather than read from a record |
