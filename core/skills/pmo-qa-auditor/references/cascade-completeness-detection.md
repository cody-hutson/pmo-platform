...
|---|---|---|---|
| G8-02 | Each declared sweep command is reproducible | each `### Cascade-Sweep` "Sweep command(s)" entry (1) **names the engine that executed it** — `grep -nE`, `python3` `re.search`, `git grep -wF`, or another named engine; (2) is **re-runnable by the grader on this platform** with explicit file scope; and (3) **does not rely on a construct the named engine does not implement**. A file-scoped `python3` line-scan predicate is admitted alongside `grep -nE`. Limb (3) is the engine-parity obligation stated as a property of the engine, not as a prohibition on one tool — the permitted-invocation set and the forbidden form are owned by § 5.6's Sweep-command form clause and its rationale by `review-discipline-principles.md` § 8 PV-2e; do not restate either list here | All declared sweeps name their engine and cite a runnable, file-scoped invocation the named engine implements |
| G8-03 | **Re-run completeness — the load-bearing check.** Re-execute each declared sweep against the changed-file set on the release branch; every returned match has a corresponding `(file, OLD-value)` row in the swept-declaration table | for each `(file, OLD-value)` pair, the live match-set ⊆ swept-table row-set, where the re-run uses an invocation from the **same permitted-invocation set G8-02 asserts** and `<regex>` is **derived from the OLD-value string** (see § 2). The grader's own re-run is itself a probe: an invocation relying on a construct its engine does not implement returns a clean-looking zero and would grade a defective sweep as PASS | Zero un-enumerated matches → PASS; any match absent from the table → FAIL, with the exact `file:line` cited |
