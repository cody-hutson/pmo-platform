...
| `_frontmatter.py` | six consumers, all in this directory: `backfill-relationship-edges.py` + `build-doc-index.py` + `check-doc-frontmatter.py` (Check 50) + `check-version-anchors.py` (Check 18b/18d) + `compose-portfolio.py` + `stamp-node-frontmatter.py` (shared library). The last two of those checks load it via `importlib`, not a static `import`, so a static-import grep under-reports the set | library (not a check) | The single shared YAML-frontmatter block reader — the F1 consistency seam so Check 18b and Check 50 parse frontmatter byte-identically |
