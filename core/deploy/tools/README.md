...
| `generate_release_index.py` | `deploy.sh` Check 23 (`--verify`); `release-executor` Mode E (Stage 13 close — `--verify` only) | `--verify` / `--self-test` / `--output-stdout` read-only; **bare = destructive full regenerate** | Cross-assert `RELEASE_INDEX.md` against `RELEASE_LOG.md`. **Stage 13 is append-only** — a bare invocation rewrites every row and restamps the grandfathered `Date` cells the INDEX header declares must not be rewritten (`release-process.md` § D6 / CR-D6) |
