...
**This rule is mechanically enforced.** Arm E of
`release/tools/check-selftest-coverage.py --reconcile` runs the same derivation
on every PR touching `core/deploy/tools/**`, and fails naming each undocumented
tool, orphan row, and duplicate row. Its population is this directory, never the
self-test manifest — that manifest's population is smaller by design, and an arm
that inherited it would report green over a set it never measured. The `comm`
pipeline above remains the equivalent by-hand derivation; the two must agree.
...
| `_frontmatter.py` | six consumers, all in this directory: `backfill-relationship-edges.py` + `build-doc-index.py` + `check-doc-frontmatter.py` (Check 50) + `check-version-anchors.py` (Check 18b/18d) + `compose-portfolio.py` + `stamp-node-frontmatter.py` (shared library). The last two of those checks load it via `importlib`, not a static `import`, so a static-import grep under-reports the set | library (not a check) | The single shared YAML-frontmatter block reader **and** the single shared corpus-traversal predicate (`is_corpus_path`) — the F1 consistency seam for both halves: so Check 18b and Check 50 parse frontmatter byte-identically, and so the three corpus walkers agree on whether a doc is in the corpus at all. A file is in the corpus for all of them or out of it for all of them; dot-leading directory segments and dot-leading filenames are both excluded |
...
| `repair-warn-log-escapes.py` | Operator (one-off recovery); `release-tooling-smoke.yml` `selftest-discovery` job (`--self-test`, reached by discovery — no roster) | `--dry-run` (**default**) / `--apply` / `--verify` / `--self-test` | Recover the `deploy-check-warn-log` family's rows whose `detail` carried a raw control character before `json_escape_detail()` shipped, and which are therefore not valid JSON. **Conserves rows rather than re-joining them** — no record spans a line break, so the row count is UNCHANGED by contract and any change to it is a bug in the tool. Escapes C0 characters only INSIDE a JSON string (TAB/LF/CR between tokens are legal whitespace), proves conservation by reconstructing the source byte-for-byte from the repaired line, preserves rows appended concurrently by copying bytes past the recorded length verbatim, takes a `.pre-repair-<date>` copy, and replaces via `os.replace()` with the source file's mode copied onto the replacement. Never mutates in place |
...
| `check-milestone-epic-membership.py` | `deploy.sh` Check 56 (warn-mode; enforce-flip deferred); `release-tooling-smoke.yml` (`--self-test`); `hub-spoke-bridge.md` Procedure 1 Step 6.5 (`--milestone … --leg M3`) | `--leg {M1,M2,M3,M4,all}` / `--milestone` / `--fixture` / `--self-test`; TSV out | Milestone↔issue-population invariants across legs at **deliberately different severities** (the leg list here is the authority — no count is stated, because a count goes stale the next leg that lands). **M1 membership** — every open non-sub-task child of an epic-declaring milestone must sit under that epic; enforce-capable. **M2 reconciliation** — the description's `### Scope` card list vs live membership; the **ADVISORY** emitter class, routed through `flag_advisory_only` under its **own** check_id `milestone-description-reconciliation`, because a description legitimately lags membership mid-release. Structurally non-gating twice over — the emitter has no mode branch and no failure-counter increment, and the id it names is disjoint from the `milestone-epic-membership` dial M1 graduates through, so a flip of that dial cannot reach M2. Each `named-not-member` ref carries exactly ONE inline bracketed sub-class token — `[elsewhere:ms#N]` · `[no-milestone]` · `[member-excluded:sub-task]` · `[unresolved]` — summarized by `COUNT_M2_NNM` plus one sub-counter per token, which sum to it. **Read the counters with `awk` exact field equality**: a `grep` for `COUNT_M2_NNM` prefix-collides with all four sub-counters. `unresolved` is a positively-emitted unknown, never a default — the ref→milestone overlay is tri-state (`{ref: n}` / `{ref: None}` / key absent), and `M2_REF_RESOLUTION` reports `fetched` vs `degraded` so a dead resolver is distinguishable from a population that genuinely could not be resolved. **M3 scaffold completeness** — advisory-only structurally, routed through `flag_advisory_only`, and never moves the exit code. Its input population comes from an UNCAPPED cursor-paginated issues-connection walk that asserts its own completeness in band against the connection's `totalCount`, and reports that assertion as `M3_SCAN <status> <total> <scanned> <matched>`, which is five-valued (`fetched` · `truncated` · `degraded` · `not-run` · `fixture`); `truncated` means every M3 row and both `COUNT_M3*` values are a LOWER BOUND, `degraded` means the read FAILED so the population is UNMEASURED (never 0) and those rows are ABSENT entirely, on a non-measuring status the numeric fields are `-` rather than `0`, and a consumer MUST branch on it before reading `COUNT_M3` / `COUNT_M3_ADV`. A `degraded` walk routes through `flag_not_evaluated` — the never-escalating emitter — on `deploy.sh`'s path, so an M3 measurement outage can never gate the M1/M2 verdict; the `--leg M3` / `--milestone` path instead exits 3, because there M3 is the whole check and its output feeds a human scaffold gate. **M4 sub-task milestone orphans** — a pipeline sub-task carrying **no milestone at all**, which is invisible to every milestone-scoped query the pipeline runs, including this check. WARN-capable with a real enforce path, on its **own** dial (`milestone-subtask-orphan`, committed default `warn`) so a graduation of the shared cohort cannot flip it by side effect; it is *not* advisory, because M2's and M3's advisory rationale is a legitimate state their predicates cannot distinguish from a defect and M4 has none. Repository-scoped, so it is a distinct leg rather than an extension of M3, whose orphan class is evaluated per-milestone by matching the slug inside the title and is therefore structurally blind to a sub-task naming no slug. Counted with `search/issues` `total_count`, **never** through the stage-title fetcher — the two legs ask different questions of different populations, and M4's `no:milestone` predicate is answered EXACTLY and uncapped in a single `search/issues` call, whereas answering it from the stage-title fetcher's issues-connection walk would mean filtering every issue in the repository in process for a ~300-row answer, at 54 requests instead of one. Emits `COUNT_M4` plus `COUNT_M4_OPEN` + `COUNT_M4_CLOSED`, which sum to it; **read them with `awk` exact field equality**, since a `grep` for `COUNT_M4` prefix-collides with both sub-counters. Gates on the **OPEN** subset only — a closed sub-task's missing milestone is history, not drift. `M4_SCAN <status> <total> <enumerated>` is five-valued (`fetched` · `truncated` · `degraded` · `fixture` · `not-run`); on `degraded` and `not-run` the counters are **omitted rather than zeroed**, and `not-run` is emitted positively on the milestone-scoped path so a consumer never reads "never looked" as "found nothing". Per `review-discipline-principles.md` § 8 PV-7: `degraded` and `not-run` are Register A statuses, the counters are ABSENT rather than zeroed, and a consumer MUST branch on the status before reading any counter. M1/M2 exclude sub-tasks; M3 and M4 count them, because the scaffold is their subject. Exit 0 no M1/M2 findings · 1 findings present · 3 input failure. M3 and M4 never move the exit code |
| `check-work-hierarchy.py` | `deploy.sh` Check 22 (`--emit-kinds` — the Gate-1 Step-0 licensed-kind vocabulary); `deploy.sh` Check 55 (warn-mode; enforce-flip deferred); `deploy.sh` Check 75 (`--validate-packs` — pack-grammar conformance; warn-mode initial, enforce-flip deferred); `deploy.sh` Check 80 (`--validate-pack-content` — pack content-completeness; warn-mode initial, enforce-flip deferred); `release-tooling-smoke.yml` `selftest-discovery` job (`--self-test`, reached by discovery — no roster) | structural (`--skip-backlog` = H1 only) / `--fixture-parent-map` / `--emit-kinds` / `--validate-packs [--pack-root]` / `--validate-pack-content [--pack-root]` / `--resolve <archetype> [--pack-root]` / `--self-test` | Work-hierarchy drift detector, three invariants. **H1 (doc, offline):** no normative doc ASSERTS a banned parent tier (`Initiative` / `Roadmap`, per ADR-049 — neither is a container tier) above a licensed work-item kind, the kind vocabulary being DERIVED from `core/packs/*/pack.toml` at runtime, never hardcoded. **H1 scan scope:** all three platform modules — `core/`, `release/`, `operations/` — plus `core/CLAUDE.md.template`, which the tree walk cannot reach on its own because the walk filters on a `.md` suffix. `release/releases/` is EXCLUDED as the archival + generated release corpus, which legitimately narrates historical hierarchies. A configured scan surface that does not resolve is reported as a `SKIP H1` row, and an empty scan population exits 3 — a configured surface never contributes a silent zero. A chain inside a quoted or code span is CITED, not asserted, and is suppressed; the single-quote delimiter is boundary-anchored so a possessive or contraction does not mask the assertion around it. **H2 (backlog, needs `gh`):** no open `type:epic` issue has a `type:epic` parent; ONE batched paginated GraphQL query, never an N+1 loop; SKIPs offline. **H3 (coextension, advisory):** no open `type:epic` issue is really an initiative container — an issue coextensive with a whole `project:` family rather than with one thrust inside it, which H2 cannot see because such containers carry no epic-parent edge. Asserted as a three-way conjunction — family shape AND title coextension AND in-family fan-out — because the family shape alone is symmetric and matches every leaf epic in every family. Rides H2's single query (fields added to the existing selection set, no second call). **Advisory exit contract:** H3 findings are excluded from the exit-code total and can never fail a run in warn or enforce mode; every finding emits its matched slug tokens and in-family references as its own falsifiable evidence. A node set lacking H3's `labels`/`title`/`body` triple exits 3 rather than reporting a vacuous zero, and a skipped leg emits `SKIP H3` with no `COUNT_H3` row so NOT-EVALUATED is never read as zero (§ 8 PV-7 Register B). **PACK-GRAMMAR MODES (the same corpus, read richer):** `--validate-packs [--pack-root <dir>]` validates a pack root against the work-item type-pack meta-schema, which no executable validated before it — the grammar could say anything and nothing in the tree would notice, so a criterion asserting "a pack conforms" was ungradable by construction. It emits one row per finding **carrying its rule id** (`PACK-P01..P08` header, `PACK-K01..K07` per-kind, `PACK-L01..L02` label facet), so a rejection is attributable to a rule rather than to a bare exit code — the property that lets each self-test arm assert its own id **and the absence of every other**, which is what distinguishes a live rule from a dead one. The reader is **section-scoped**, not line-anchored, because `applies_to` appears in the meta-schema in two unrelated senses (`[meta].applies_to`, a string; `[controls.applies_to]`, an object) and a line scan would read a control's span as a pack header field. The label-group domain is read live from `core/specs/label-taxonomy.md` rather than restated, and is reported as `SKIP PACK-L01` — never as a silent pass — when that grammar is unreadable. `--resolve <archetype> [--pack-root <dir>]` prints the eligible pack set and the kind union, each row tagged by role, making the archetype-vs-kit eligibility join executable: a pack is eligible when its `applies_to` equals the archetype, OR its `applies_to` is `*` **and** its role is `kit`. It resolves eligibility and the union only — which kit a deployment selected is a configuration-axis question upstream of the tool. Both modes are separate argv branches that return before the H1/H2/H3 legs and share no mutable state with them, so a defect in either cannot reach Checks 22/55; both fail loud in **both** directions (a partial read or a pack root resolving to zero packs exits 3, never a clean zero), and both accept a `--pack-root` that is either one pack directory or a directory of pack directories. **PACK-CONTENT MODE (the same corpus, a different question):** `--validate-pack-content [--pack-root <dir>]` asks not whether a declaration is well-formed but whether it was **made**, and whether it says where it came from. It ranges over one enumeration — a kind × surface pair, surfaces being `fields`, `criteria.readiness`, `criteria.done`, `criteria.gate` — and emits eight rule ids in three families: `PACKC-C01` (kind-scoped completeness), `PACKC-P01..P04` (entry-, block- and control-level provenance) and `PACKC-S01..S03` (entry shape, closed domains, `criteria_version`). **The findable condition is silence, not emptiness:** an array present-and-empty carrying a block-level `source` is a *reasoned empty set* — a position was taken, and it is green; an array that is **absent** is a surface on which the kind said nothing, and that is the finding. Rule-id disjointness is total by construction, each of the three site states admitting exactly one family, so a finding count equals a defect count. It is a **sibling** of `--validate-packs` rather than a widening of it, for a measured reason: every declaration site in the grammar mode's own fixture corpus is array-absent — its *conforming* fixture included — so a silence rule sited there would return findings on both of that check's control arms and destroy its discrimination. Two self-test arms run the same bytes through both modes and assert that contrast. The reader extension is a defaulted `capture_content=False` parameter on `parse_pack` / `read_pack_root`, so every pre-existing call site stays a one-argument call and the grammar mode's verdicts are unmoved as a property of the call signature; the content scanner is separate from `_scan_pack_value` (the grammar mode's value reader, which splits a captured array on every comma) and is string-aware and nesting-aware. Where the two readers disagree about where an array ends, the read is reported **PARTIAL (exit 3)** rather than silently under-counted. **Fixture home:** `core/deploy/tests/fixtures/packs/`, one directory per fixture — deliberately OUTSIDE `core/packs/`, because the licensed-kind union walks every directory under `core/packs/` holding a `pack.toml` with no allowlist and no naming filter, so a fixture placed there would join the live gate's vocabulary; a self-test arm asserts that separation held. The content mode's own pair lives one level deeper, at `core/deploy/tests/fixtures/packs/content/cc-{silent,complete}/`, because `--pack-root` accepts a whole parent and enumerates `<root>/<child>/pack.toml`: two more top-level fixtures would be absorbed into a whole-parent grammar run and move its result set, while a subdirectory holding no `pack.toml` of its own is skipped. |
...
| `check-automation-coverage.sh` | `deploy.sh` Check 76 AND `.github/workflows/automation-registry-check.yml` (PR-time leg) — both invoke THIS script, so the predicate cannot drift between them | (bare) assert coverage over the live tree / `--self-test` | The admission gate proper. Where the conformance predicate below judges the rows that are PRESENT, this one judges which rows EXIST AT ALL, so a registry cannot be perfectly conformant and completely empty — the exact failure the registry was created to prevent. Four assertions: **A-01** every automation the corpus declares has a row (the acceptance criterion), **A-02** every row has a declaring automation, **A-03** every row's entrypoint declares that row's own id — the arm that separates this from a presence-only check, since a row can name a real file and still name the wrong one — and **A-04** every scheduled workflow is registered, with no exclusion allowlist by deliberate decision. An automation declares itself at its file's head in one of two forms: markdown frontmatter at file position 0, or a `# automation_id:` comment in a workflow; position is what makes the first precise rather than a substring hunt, so the document that DEFINES the field can never be mistaken for one that DECLARES it. The field is LIST-VALUED — one document may declare more than one routine and one already does — with a bare scalar accepted as a one-element list and a per-element shape guard. The field list and column order are PARSED from the schema at run time through the schema's own machine-parse contract, and the two columns read here are located BY NAME within that order, never by position: a private copy would be a second source of truth that drifts silently, and the self-test proves the absence of one by deleting a field from a copy of the SCHEMA and requiring a hard-fail. Every self-test arm mutates a copy of the LIVE population rather than a fixture, and asserts that population is non-empty before any arm runs. Exit 0 covered · 1 findings · 2 usage · 3 scan-surface error (a surface absent, the schema parsing 0 fields, the registry header disagreeing with the schema, an EMPTY roster, or a malformed declared id) — ALWAYS hard-fail, because a gate reporting "0 findings" over an empty roster would be reporting the very state it exists to detect |
| `check-automation-registry.sh` | `deploy.sh` Check 76 AND `.github/workflows/automation-registry-check.yml` (PR-time leg) — one consolidated check carries this predicate and the coverage predicate above, rather than two editors landing on the same high-contention file | (bare) validate the live registry / `--self-test` | Asserts every routine-spec row PRESENT in `core/automations/registry.md` conforms to `core/schemas/automation-registry-schema.md`: six REQUIRED fields, each within its declared value space, plus the total cadence-by-trigger matrix in which `hybrid` requires a cron because one row registers one routine and what it registers is the automated limb. Thirteen rejection classes, each of which is also a mandatory negative fixture in the self-test, paired with a well-formed control row that must return zero findings — a validator whose malformed arm passes fails its own job, so the self-test runs as a hard step ahead of any live scan regardless of posture. The two enum value spaces are PARSED from their owning surfaces under a cardinality guard rather than hand-copied (duplicate a parse, which fails loud, never a policy, which drifts silent); a restructured owner fails loud instead of degrading the scan to a permissive pass. CONFORMANCE ONLY — coverage is a separate invariant over a different population, and keeping them disjoint is what makes a partially-populated registry a valid state at every intermediate commit. Exit 0 conforming · 1 findings · 2 usage · 3 scan-surface error (registry absent, 0 rows parsed, a header outside the declared 6-column form, or a parse arm returning the wrong cardinality) — ALWAYS hard-fail regardless of posture |
...
| `check-convention.sh` | `deploy.sh` Check 49 (warn-mode initial, so a finding annotates without blocking during the shakedown window) | (bare) scan | The platform-convention linter for the five residual dimensions no other gate owns: lowercase-kebab `[topic]-[type].md` naming across the authored-doc dirs; a git-tracked file whose PATH sits under `projects/` (a Layer-2 boundary violation); a governance file that makes a dated factual assertion yet carries zero evidence-quality labels anywhere; unfilled bracketed-placeholder leakage outside backticks (a backticked placeholder is documentation OF the token and is exempt); and an event-log read site keyed on the release-version form rather than the milestone-slug join key `pipeline-event-log-schema.md` § 2a makes canonical (line-scoped `event-log-key: allow — <reason>` marks a deliberate one, reason mandatory; fixture data rows are counted out of class on the `OK:` denominator line, and an empty scan scope exits 3 rather than reporting clean). Deliberately does NOT re-enforce dead-file-reference, depersonalization, issue-reference, localized-context, or internal-link integrity — each has an owning gate, and duplicate enforcement is governance debt. Emits `FAIL:`/`OK:` lines plus a trailing `SUMMARY: N finding(s)` |
...
| `check-label-parity.py` | `deploy.sh` Check 51 (`--source core/specs/label-taxonomy.md` plus each `core/packs/*/pack.toml`, `--dispositions core/config/allowlists/label-attribute-dispositions.txt`, `--output-format tsv`; warn-mode initial, MISSING-leg enforce-flip deferred) | `--source` (repeatable; `.md` and `.toml` auto-detected by extension) / `--output-format {tsv,text}` / `--emit-fix` (read-only renderer, a separate flag because the check pins `tsv`) / `--dispositions` (per-row attribute-disposition registry; absent file tolerated as empty, present-but-unreadable is exit 3, malformed record is exit 2) / `--repo` (owner/name; derived from git remote origin when omitted) / `--max-labels` (default 1000) / `--fixture-labels` (offline payload injection) / `--self-test`; needs `gh` | Compares the canonical label registry — the UNION of the grammar doc's namespace patterns and the per-pack `[[labels]]` rows, so a relocated-but-live label resolves and does not false-orphan — against the live label set read over **REST** (`gh api "repos/<slug>/labels?per_page=100" --paginate`), at four asymmetric severities. **MISSING** (canonical label absent from GitHub) is ENFORCE-capable, because a gate referencing a non-existent label fails silently. **ORPHAN** (live label unregistered) is WARN only, since some are legitimately operator-local or pending registration; a live label is an orphan only if it matches neither a concrete registered label nor a registered namespace prefix. **EXCLUDED_LIVE** (live label the grammar declares excluded, read from `label-taxonomy.md` § Excluded Labels by a section-anchored parser) is a separate class from ORPHAN because the remedies are OPPOSITE — an orphan may simply need registering, whereas an excluded-but-live row means one of the two surfaces must change. **DIVERGED** (declared AND live, but colour and/or description disagrees with the declaration) is **ADVISORY-ONLY and structurally non-escalating**: the three arms above are all NAME-keyed and every one of them passes on such a row, but the remedy overwrites live label metadata — repository STATE, not git-revertible — and the gate cannot tell a deliberate operator override from drift, so the class does not enter the exit expression and `deploy.sh` emits it through `flag_advisory_only` rather than `flag_warn_or_issue`. That emitter choice is structural rather than a posture preference: `resolve_check_mode` is keyed per check-id, not per arm, so the escalating emitter would arm this arm on any future `label-parity` enforce flip. A row recorded in the `--dispositions` registry as `accept-override` is suppressed (axis-scoped — accepting a colour does not accept a description); a `reconcile-*` row is owned but still reported until the operator-run cascade closes it, so the arm DRAINS rather than standing as a permanent signal stream. **DIVERGED-STALE** reports a registry row naming a label that is no longer divergent or no longer live — the audit affordance a suppression surface owes. A markdown source carrying no `## Excluded Labels` header is exit-3 fail-loud, never a silently-empty excluded set. `--emit-fix` renders the `gh label` commands that would close the gap and runs none of them, in CREATE / RECONCILE / UNRESOLVABLE / DELETE blocks ordered by increasing severity so DELETE — the irreversible one — is never reached by momentum; it shares `diff_declarations` with the DIVERGED arm rather than forking it, so the two cannot drift on what "diverged" means. **Transport is REST, not GraphQL**: `gh label list` rides `POST /graphql`, so the check could only report an indeterminate exit 3 during a GraphQL-quota-exhausted window even though the label data was fully served over REST — and exhaustion clusters at release close-out, exactly when Check 51 runs. Both live-read paths (the gate diff and `--emit-fix`) moved; the slug is derived, never hardcoded. Pagination is `--paginate` at `per_page=100` parsed through a framing-tolerant normalizer, and the former silent 500-row truncation — which reported every unread canonical label as a spurious MISSING — is now the fail-loud `--max-labels` bound. Exit 0 clean · 1 findings (MISSING / ORPHAN / EXCLUDED_LIVE only — divergence never moves the exit code) · 2 usage, including a malformed `--dispositions` record · 3 a source unreadable, the union parsed to zero labels, the repo slug unresolvable, the live set exceeded `--max-labels`, a present `--dispositions` file unreadable, or no markdown source carried the `## Excluded Labels` header |
| `check-mirror-pair-parity.py` | `deploy.sh` Check 77 (`--root . --output-format tsv`, via the top-level `_c77_compute_verdict` body shared verbatim by the lifecycle `--check` surface and the `--check-required-subset` pre-merge runner); `release-tooling-smoke.yml` `selftest-discovery` job (`--self-test`, reached by discovery — no roster) | `--root` / `--output-format tsv` / `--self-test`; TSV out | Asserts that every holder of the SOURCE-side mirror-pair path set declares an IDENTICAL set. Holders SELF-REGISTER in band — each wraps its array literal in a `mirror-pair-set:` BEGIN/END marker declaring that holder's own id, separator word (`colon`/`tab`) and 1-indexed field — and the tool discovers them by corpus scan, so adding holder N+1 costs one marker pair inside the new holder and ZERO change here. **Arity-general by construction:** a pairwise diff would encode arity 2 in its shape, which is the defect one abstraction level up, and a central registry would be a further list of the same kind that can itself desynchronise. Splits on the FIRST separator occurrence, matching both consuming holders' runtime semantics byte-for-byte — a last-occurrence split against a value carrying a second separator compares every pair against a path that does not exist while reporting clean. Only field 1 is comparable across holders: their field-2 values are not in the same form, one carrying a deploy-root prefix the other does not. **Vacuity guard, two limbs.** ARITY: fewer than two discovered holders FAILS — "all holders agree" over one holder is vacuously true and a pass there is indistinguishable from a working check. EMPTINESS: two or more holders that ALL declare zero members is likewise an input failure, never PARITY — the symmetric difference over empty sets is empty, so an unguarded check reads clean over an empty mirror, and no comparison can catch it because it is SYMMETRY that buys the false pass (emptying only ONE holder is correctly DIVERGENT). That limb is reported UNPARSEABLE rather than NOT-EVALUATED so it FAILs on both surfaces — nothing went un-measured, the holders were read and found empty — and it exits 3 to match the zero-member guard `check-rules-budget.py` applies to these SAME holders, because two checks reading one corpus must not disagree about whether a state is an input failure. A marker that announces itself but fails the grammar is reported UNPARSEABLE, never skipped, because a typo'd marker reading as "not a holder" would silently deregister it. An unreadable tracked file makes the scan incomplete, so parity is WITHHELD as NOT-EVALUATED rather than claimed; the wrapper maps that to the never-escalating not-evaluated emitter on the lifecycle surface and to FAIL on the gate surface, where every holder is a committed file. Because discovery is line-based, any file quoting the marker registers itself — this tool's own first live run discovered its docstring and fixtures and correctly reported them unparseable, so its marker text is box-quoted in prose and variable-assembled in fixtures, with a self-test arm asserting it registers zero holders. Exit 0 PARITY · 1 a non-PARITY verdict (finding or withheld) · 3 input failure (zero holders discovered — the marker convention moved — or the discovered holders declare no members between them) |
...
| `check-rules-budget.py` | `deploy.sh` Check 78 (`--root . --output-format tsv`, via the top-level `_c78_compute_verdict` body shared verbatim by the lifecycle `--check` surface and the `--check-required-subset` pre-merge runner); `release-tooling-smoke.yml` `selftest-discovery` job (`--self-test`, reached by discovery — no roster) | `--root` / `--output-format tsv` / `--self-test`; TSV out | Holds the deployed rules set to the admission thresholds published in `core/standards/rules-corpus-admission-standard.md` — the admitted set inside the §3 byte ceiling, every member over the per-file trigger carrying a §2 conditional scoping field (`paths:` **or** `unscoped_rationale:`), and every member carrying the §2 required frontmatter including `type: rule`, which is the admission decision made machine-readable. **The member list is not in this file, by construction.** The subject of this gate is one body of content declared in several places and drifting between them; a hardcoded set here would add a further desyncable list and reproduce that defect one level up. Members are read from the SAME marker-registered holders `check-mirror-pair-parity.py` discovers, by IMPORTING that tool's discovery and parsing functions rather than re-implementing its marker grammar — one grammar, one parser, and adding a rule to a holder is the only edit that changes what this measures. It asserts the SET, not holder agreement: parity across holders is Check 77's invariant, and duplicating it here would let two checks disagree about one fact. It measures the UNION of what the holders declare, deliberately the conservative direction — under a divergence the union is the larger set, so this reports a breach rather than missing one while parity reports the divergence. **Vacuity guard:** a sum over zero members is 0 and 0 is inside every ceiling, so an empty scan would be byte-indistinguishable from a healthy one — zero holders, or holders declaring zero members, is NOT-EVALUATED and exit 3, never WITHIN. An unreadable member withholds the verdict rather than shrinking the total into a false pass; the wrapper maps that to the never-escalating not-evaluated emitter on the lifecycle surface and to FAIL on the gate surface, where every input is a committed file. Every finding of every class is emitted as its own row regardless of which supplies the headline verdict token, so a lower-precedence finding is never masked by a higher-precedence one. Verdicts: `WITHIN` / `OVER-BUDGET` / `UNSCOPED-VIOLATION` / `FRONTMATTER-VIOLATION` / `NOT-EVALUATED`. Exit 0 WITHIN · 1 a non-WITHIN verdict (finding or withheld) · 3 input failure (no holder, or no members declared) |
...
| `check-issue-ref-validity.sh` | the `issue-ref` job in `.github/workflows/repo-integrity.yml`, which is a **required status check on `main`** — the gate's only production caller; plus `release-tooling-smoke.yml` `selftest-discovery` (`--self-test`, reached by manifest discovery). **Not** a `deploy.sh --check` leg, and deliberately so: it needs a base/head SHA pair and network issue resolution, neither of which a local `--check` run has — so this row's `Used by` names a CI gate rather than a Check number | `--base`/`--head` (added-lines delta; CI parity) / `--path` (whole-file; the local form that needs no workflow run) / `--resolver gh\|fixture` + `--fixture-map` / `--self-test` / `--equivalence <pre-sha>` | The Issue-reference validity gate, lifted out of the workflow job that used to carry it inline. Flags an issue reference in changed markdown that does not resolve to a real issue IN THIS REPO (404, redirect, a transfer to another repo, or a pull-request number), the deprecated legacy identifier, and a resolving reference placed outside a designated reference block with no inline provenance marker; honors the whole-file override, the single release-tracking path exemption, and fenced code. That one path exemption is all the enumeration the scope filter carries: the self-documentation files that quote worked examples are exempt because they carry the override marker, discovered on every run, rather than because a hand-maintained list names them — an enumerated exemption keeps a file silently exempt after the marker justifying it is gone, and a discovered one cannot. Two seams, both forced rather than chosen: the input seam is what makes a verdict reachable without a workflow run, and the resolver seam is what makes a fixture suite hermetic, since three of the five verdicts are properties of the live issue graph. `--equivalence` materializes the pre-extraction body straight out of git — never a tracked copy, which could drift from what it claims to represent — and asserts identical REPORT TEXT in BOTH directions, since flagging fewer is a weakened gate and flagging more is a strengthened one; it carries a sensitivity control and a mutation arm, because a differ that cannot report DISAGREE cannot meaningfully report AGREE. That report-text claim is **bounded by declaration**: it covers everything above the `### Categories` heading — findings, `::notice::` lines, the verdict, every part of the output a scan result reaches — and excludes the static advisory block below it, which is literal `echo` carrying no scan result. Diffing that block too pinned the gate's own failure message to a frozen copy of itself, so correcting a wrong sentence in it failed a correctness gate. The exclusion is asserted rather than assumed, in both directions: a narrowing control fails the arm when the cut marker is absent from either side, and an inertness arm fails it if that region's output DIFFERS BETWEEN THE ARM'S TWO FIXED PROBES — two samples of the finding set, not the finding set. That difference is the arm's whole reach, and it is narrower than the exclusion's safety condition: below-cut output that is CONSTANT across finding sets, that varies with the finding set but takes the same value on both probes, or that is keyed to a finding class neither probe carries passes it, so those classes rest on review rather than on an arm. The decision, those measured residuals, and an accounting of what the narrowing does and does not buy are recorded at `normalize_report()` in the tool. Exit 0 no findings · 1 findings · 3 input/config failure |
| `check-operator-toml-schema.sh` | `deploy.sh` Check 70 — the deploy-time companion and the ONLY locus that can see the C70a live-instance leg, since CI has no operator instance to read; and the `operator-toml-schema` job in `.github/workflows/repo-integrity.yml`, which carries the load-bearing C70b/C70c legs (`--self-test` then `--repo-integrity`); plus `release-tooling-smoke.yml` `selftest-discovery` (`--self-test`, reached by manifest discovery) | `--repo-integrity` (C70b + C70c) / `--instance` (C70a) / `--emit-delta` (for `update.sh`) / `--root` / `--config-root` / `--self-test` | Check-70 predicate for the declared `operator.toml` schema (`core/config/operator-toml-schema.json`, per ADR-140) — asserts the declaration is the ONE place the key set is declared and that every downstream surface agrees. **Two populations, two postures:** the live operator instance is mode-dialed (`operator-toml-schema`, committed default `warn`), because an instance behind the repo is not a repo defect and must never red-wall a deploy — each miss names the key and the remedy command — while the tracked-repo legs C70b/C70c ENFORCE. **C70b is the anti-drift falsifier:** it should be tautologically true precisely because the generator DERIVES from the declaration, and it exists to catch a future hand-edit that re-introduces a literal per-key emit and re-creates the shadow SSOT. C70c holds in BOTH directions, so the hand-authored template and the declaration cannot drift apart without one generating the other. **Declared coverage boundary:** key SETS and the absence of a hand-written emit — NOT that a declared `enum` constrains an on-disk value, that a declared `type` matches one, or anything about opt-in keys in a live config. An unresolvable parser is a LOUD non-zero, never a quiet zero. Exit 0 clean · 1 findings — one FAIL line each, raised by the ENFORCING C70b/C70c legs; the mode-dialed C70a emits WARN lines that never set it, and `--emit-delta` reuses 1 to signal a non-empty delta to `update.sh` · 3 input / scan failure |
| `start-skill-editor-session.sh` | `release-tooling-smoke.yml` `selftest-discovery` (`--self-test`, reached by manifest discovery) — its sole automated caller; otherwise operator/agent invocation, allowlisted in `core/config/allowlists/script-execution-allowlist.txt` | `<skill-name>` (mint) / `--end <skill-name>` / `-h`/`--help` / `--self-test` | Mints (or ends) the `pmo-skill-editor` session sentinel `<skill-dir>/.editor-session` that Gate 2 of `core/hooks/block-skill-direct-edit.sh` (`BLOCK-SKILL-EDIT-002`) requires before a direct edit to a migrated skill. The gate's own deny message instructed the reader to start an editing session while **no minter existed**, so the only way through was the global `CLAUDE_HOOK_BYPASS` kill switch across every hook — this tool makes the sanctioned path real so the blunt one stops being the only one. **Not a bypass:** Gate 2 still enforces the skill's migration marker, that the sentinel names THIS skill, a non-future timestamp, and the TTL. The sentinel is runtime state, `.gitignore`d, never committed. Exit 0 ok · 1 skill not resolvable · 2 usage · 3 could not write the sentinel |
...
| `check-issue-body-anchors.sh` | `deploy.sh` Check 72 (`--output-format tsv`; **ADVISORY and never enforce-capable** — there is no `.mode` file and no enforce flip is planned). Offline it SKIPs with a logged reason, mirroring Checks 39/40/51/52/53/55, because a backlog invariant must never read green because it could not run | `--state` / `--issues` / `--milestone` / `--repo` / `--root` / `--resolver` / `--fixture-dir` / `--output-format` / `--census` / `--self-test`; needs `gh` | Check-72 predicate: resolves a section-anchor citation in an OPEN issue body against the ACTUAL headings of the file it names, catching a citation whose section number is right for some OTHER file or whose target has been restructured since the issue was written — one that looks precise and resolves to nothing. The inverse of `check-issue-ref-validity.sh` on BOTH axes: that gate scans changed repo markdown and resolves `#N` against GitHub, this one scans issue bodies and resolves file anchors against the repo; `check-citation-anchors.sh` (Check 66) names this predicate in its own coverage boundary as one it does NOT run. **Advisory permanently, and structurally so:** the corpus labels sections in conventions the predicate cannot enumerate, so a citation naming a real-but-unmodelled label is indistinguishable from one naming nothing — a false positive in a gate costs more than a missed anchor, so this surface SURFACES and never verdicts. Counts in its header are MEASURED at a recorded `BASELINE_PIN` over a live population; re-measure with `--census` before relying on any of them (audit-baseline discipline) |
| `check-mode-declaration-residual.py` | `release-tooling-smoke.yml` `selftest-discovery` (`--self-test`, reached by manifest discovery) — its only automated caller. **Not** a `deploy.sh` check leg: `deploy.sh` names it in prose beside Check 35 as the checked-in instrument for that check's residual class, and invokes it nowhere | (bare) measure the residual / `--self-test`; any other argument is a usage error | Measures Check 35's residual class — skills declaring >=2 modes in a shape Check 35's recognizer cannot see — with an oracle INDEPENDENT of that recognizer. Two arms, neither sharing a shape with it: a natural-language cardinal bound to the word "modes", and a markdown table carrying a `Mode` column with >=2 distinct values. The independence is the whole point: the first attempt defined the residual using the predicate's own body-heading arm, which is EMPTY BY CONSTRUCTION on every possible corpus — it measured a recognizer's under-reach with the recognizer under test, read 0, and the 0 was the defect. **A NON-ZERO residual is the expected, healthy result**, so the exit code grades the ORACLE, not the residual: it exits non-zero only when a control arm is inconsistent (sensitivity 0, specificity fires, or a mutation fixture misbehaves), the one condition under which the number may not be believed. A green run reporting residual 0 with consistent controls is itself the signal that the oracle was re-derived from the recognizer — grade it as a failure. The body-heading pattern is LIFTED from `deploy.sh` at run time rather than restated, so a hand-copied regex cannot drift into the exact defect the instrument exists to catch (CIAC-2). Residual detector, NOT a population detector: it deliberately does not see multi-mode skills that declare in the machine-recognizable forms |
| `resolve-automation-binding.sh` | `.github/workflows/automation-registry-check.yml` (`--self-test`, a HARD step — the workflow shipped that step ahead of this file and it flipped from a loud no-op to a gate the moment the file landed, with no edit to the workflow); `release-tooling-smoke.yml` `selftest-discovery` (reached by manifest discovery) | (bare) resolve every row / `<routine-id>` resolve one / `--self-test` | **The scheduler ADAPTER** — the consumer side of the registry, not a predicate over it. It resolves a routine's registry `id` to the binding that fires it on THIS install, and emits one binding record per routine with the state token as the FIRST FIELD. Resolution is BY REGISTRY `id` and by nothing else: the primary key, the entrypoint's `automation_id` marker and the name the backend registers are one string on three surfaces, so a roster declaration with no row exits non-zero — omitting a row costs the routine its ability to fire, which is the admission gate's forcing function made executable rather than merely asserted. The backend class is DERIVED from `entrypoint` and never stored: a workflow entrypoint is repo-fired (its own host fires it, and the operator's scheduler must NOT also register it or it double-fires) and every other tracked spec is session-fired, which is what lets the adapter support both real backends while the registry names neither. The `[adapters].scheduler` selector is read FRESH at every call — a cached selector fires the backend that is no longer there while the routine looks healthy — and a value outside the parsed value space is a hard error, never a silent fall back to `none`, because indistinguishable-from-absent is the failure this adapter exists to make visible. Under `scheduler = "none"` (the shipped default, and the state of an install that has registered nothing) every session-fired routine degrades to a `MANUAL` record carrying the exact invocation, so a no-scheduler install is READABLE rather than silent — the silence was the defect. The value space is PARSED from the key declaration and the field list from the registry schema, with the two columns located by name in the parsed order; nothing is copied. The **AC-3 no-unused-arm control ships in the tool, not only in its self-test**: the parsed enum and the implemented arms are asserted to be the SAME SET on every run, so an enum value with no arm (a backend the config offers and nothing fires) and an arm with no enum value (dead code no operator can select) are both scan-surface errors. Exit 0 resolved · 1 routine-level failure · 2 usage / ambiguous primary key / out-of-range selector · 3 scan-surface error — ALWAYS hard-fail. Contract: `core/standards/scheduler-adapter-routine-firing.md`, whose `SD-1`..`SD-4` ids the self-test greps for |
| `reconcile-gate-posture.py` | `.github/workflows/gate-posture-reconciliation.yml` (`--self-test` as a HARD step, then the report); `release-tooling-smoke.yml` `selftest-discovery` (`--self-test`, reached by manifest discovery) | `--workflows <dir>` / `--live <file>` / `--self-test`; markdown out | **The Requirement (c) coverage audit, given a runner.** `core/standards/gate-efficacy-standard.md` DERIVES a declaration's posture from what its enforcement surface actually delivers, notes that the declaration does not itself edit branch protection, and assigns the declared-vs-live reconciliation to Requirement (c) — an audit that, before this tool, had no implementation anywhere: every occurrence of the branch-protection API in an executable surface across the tracked tree was a comment describing the operator step. Reports three classes — declared-and-live, declared-but-unregistered, and live-but-undeclared (the more serious direction: enforcement with no in-tree record of posture, invariant, or falsification test). **Report-only, structurally and permanently:** its Verdict-Input Closure over repository paths is EMPTY, so it can never be `required` and must never join `deploy.sh --check-required-subset` (`rs_checks`) — a transient `gh`, auth or rate-limit failure would become merge-blocking, the same disqualifier that made Check 51 (`label-parity`) a ratified advisory. **The live read is three-valued** (`fetched` · `unavailable` · `not-run`) and under either non-measuring status the delta sections are ABSENT rather than empty, per `review-discipline-principles.md` § 8 PV-7 — a reconciliation that printed a clean zero when it could not read branch protection would launder a non-measurement into a clean bill of health, which is the exact defect class it exists to surface. Reading branch protection is admin-scoped, so a 403 from the default `GITHUB_TOKEN` is an EXPECTED outcome, not a defect. **Single parser:** the header grammar is imported from `core/deploy/tests/test_gate_efficacy_declarations.py`, never re-implemented — two parsers over one format is a second thing to keep in agreement. Exit 0 report produced (findings or not, live read succeeded or not) · 2 `--self-test` arm failure · 3 the declared set could not be read at all (no workflows resolved, parser unreachable, or zero declared contexts discovered — a broken probe, never a gate verdict) |
