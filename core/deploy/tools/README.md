...
| `check-mirror-pair-parity.py` | `deploy.sh` Check 77 (`--root . --output-format tsv`, via the top-level `_c77_compute_verdict` body shared verbatim by the lifecycle `--check` surface and the `--check-required-subset` pre-merge runner); `release-tooling-smoke.yml` `selftest-discovery` job (`--self-test`, reached by discovery — no roster) | `--root` / `--output-format tsv` / `--self-test`; TSV out | Asserts that every holder of the SOURCE-side mirror-pair path set declares an IDENTICAL set. Holders SELF-REGISTER in band — each wraps its array literal in a `mirror-pair-set:` BEGIN/END marker declaring that holder's own id, separator word (`colon`/`tab`) and 1-indexed field — and the tool discovers them by corpus scan, so adding holder N+1 costs one marker pair inside the new holder and ZERO change here. **Arity-general by construction:** a pairwise diff would encode arity 2 in its shape, which is the defect one abstraction level up, and a central registry would be a further list of the same kind that can itself desynchronise. Splits on the FIRST separator occurrence, matching both consuming holders' runtime semantics byte-for-byte — a last-occurrence split against a value carrying a second separator compares every pair against a path that does not exist while reporting clean. Only field 1 is comparable across holders: their field-2 values are not in the same form, one carrying a deploy-root prefix the other does not. **Vacuity guard:** fewer than two discovered holders FAILS — "all holders agree" over one holder is vacuously true and a pass there is indistinguishable from a working check. A marker that announces itself but fails the grammar is reported UNPARSEABLE, never skipped, because a typo'd marker reading as "not a holder" would silently deregister it. An unreadable tracked file makes the scan incomplete, so parity is WITHHELD as NOT-EVALUATED rather than claimed; the wrapper maps that to the never-escalating not-evaluated emitter on the lifecycle surface and to FAIL on the gate surface, where every holder is a committed file. Because discovery is line-based, any file quoting the marker registers itself — this tool's own first live run discovered its docstring and fixtures and correctly reported them unparseable, so its marker text is box-quoted in prose and variable-assembled in fixtures, with a self-test arm asserting it registers zero holders. Exit 0 PARITY · 1 a non-PARITY verdict (finding or withheld) · 3 input failure (zero holders discovered — the marker convention moved) |
