...
per-hook drop-in source fragments under `core/rules/bypass-mode-readiness/`:
    ...
    block-<hook>.md   one per BYPASS-MODE security hook (the 7); each carries a
                      field table (Hook / Matcher / Scope / Mode), a Rule Registry
                      sub-table, and any per-hook path-resolution / posture detail
...
Output structure (deterministic):
    ...
    ## The Hooks                                     (AUTO-GENERATED table — one
                                                      row per block-<hook>.md, in
                                                      lexicographic hook order)
    <each block-<hook>.md body, lexicographic order>
...
def discover_hook_sources(source_dir: Path) -> list[Path]:
    """Return the per-hook source files (block-*.md), lexicographically sorted.

    Underscore-prefixed fragments (_header.md, _cross-cutting.md) are excluded —
    they are cross-cutting, not per-hook.
    """
    return sorted(
        p for p in source_dir.glob("block-*.md") if not p.name.startswith("_")
    )
...
def build_hook_table(hook_sources: list[Path]) -> str:
    ...
    rows = [
        ...
        "per core/rules/bypass-mode-readiness/block-*.md source. Do not hand-edit. -->",
        "",
        "| Hook | Scope |",
        "|---|---|",
    ]
...
def assemble_index(source_dir: Path) -> str:
    ...
    if not hook_sources:
        raise FileNotFoundError(
            f"no per-hook sources (block-*.md) found under "
