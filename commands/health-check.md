...
- First token = mode. The seven modes: `full` (default), `timeline`, `attribution`, `comms`, `plan <name>`, `raid`, `sources`.
...
- `plan` requires a plan name (`plan <name>`); invoked with no name, ask "which plan?" — do not default to a plan.
