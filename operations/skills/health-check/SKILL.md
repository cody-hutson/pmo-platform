...
The full contract — the `E1 ∪ E2 ∪ E3` denominator model, the coverage envelope, the ordered first-match-wins routing table, the confidence projection, the migration-telemetry surface and the reserved escalation seam — lives in `references/structure-mode.md`.
...
- **Principal response vs. junior response:** Principal writes `MM-0 72/100 · MM-1 entities 8/9 · MM-2 fields 41/45 · MM-3 relationships 15/17 · 9 of 19 entity types in denominator, 10 excluded (not-expected) · [TIER UNPOPULATED: cross-project-shared]` and the operator can see exactly what was measured. Junior writes `100/100` on a three-entity project, the operator reads it as a clean bill of health, and a go-live decision ships on data that was never measured.
