...
audits: "The schema-conformance axis, for each entity in the derived expected set: (a) entity present at its declared storage_tier home, (b) required fields populated, (c) required relationships resolve. Population is entity RECORDS, never files (the project-entity-model boundary axiom). Reports MM-0 = MM-1 x MM-2 x MM-3 as a single 0-100 number — the metrics are defined in the migration-enforcement protocol and cited here, never redefined — with every factor rendered n/d, a mandatory entity-type coverage line, a list-form unpopulated-tier banner, and UNMEASURED rather than 0% for any factor that could not be measured. Rule authority is cited by rule ID from entity-field-schemas.md; no rule text and no rule count is copied. Every violation names rule ID + entity + field/relationship — a bare count is not a finding. Takes --scope <project> only. See references/structure-mode.md."
