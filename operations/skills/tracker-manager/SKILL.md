...
5. **Schema validation:** Validate all 15 fields against [`raid-log.schema.json`](../../../core/schemas/raid-log.schema.json) (the RAID Log schema-of-record) before writing. `source_ref` is an optional dialect column — its presence is not required, but when present it carries a provenance token.
...
- **Methodology-projected kinds/fields (read at invocation):** When validating or rendering a tracker entry's work-item **kind or fields**, read the project's `delivery_approach` from PROJECT.md **at invocation** and project the methodology-appropriate kinds/fields for that archetype — per the OPERATIONS.md Methodology Awareness Protocol (Rule 1: read `delivery_approach` at invocation; **do not cache across invocations**; Rule 2: consult the methodology archetype matrix row for the archetype's lifecycle/ceremonies/artifacts; Rule 3: for `delivery_approach: Custom`, read the `custom_methodology_definition` block directly — no blind archetype fallback). Do **not** hardcode sprint-centric kinds/fields on a non-`timeboxed` lifecycle (Rule 4 PROC-4). On an absent or unparseable field, render methodology-agnostic with `[ASSUMPTION – CONFIRM] delivery_approach absent` and **never silently assume Scrum** (Rule 4 PROC-3). This cites the shared contract; it introduces no second read-contract.
