...
- **Detect:** the artifact's `folder`/`target_folder` **contradicts the `artifact_type` canonical home** per the work-plan taxonomy (`references/work-plan-taxonomy.md` canonical-target-folder column). The canonical case: an artifact whose `promotion_state` is `promoted` but whose `folder` is still a staging bin — promoted-but-not-moved. This is the schema-declared consistency rule `promotion_state: promoted ⇒ folder ∉ {08-generated, _generated}` (`frontmatter-schema.md` Domain C), read directly off the dedicated location field. The staging-bin set is the **ADR-080 folder union**: the legacy `08-generated` bin AND the `_generated/` staging bin of the closed 5-bin project taxonomy both count as staging, so a promoted artifact left displaced in EITHER bin is caught. Keying the rule on the union rather than on the legacy path alone is what keeps the check live across the taxonomy migration window, during which both bin forms occur in the corpus.
