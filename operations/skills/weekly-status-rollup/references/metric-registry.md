...
This registry produces per-**metric** RAG. The single **PROJECT-level** RAG is composed **UPSTREAM** by the transparent-roll-up rule in [`channel-formats.md`](../../comms-writer/references/channel-formats.md) § RAG Threshold Standards → **Watermelon detection rule** — *"component-level RAGs that roll up transparently to the project-level RAG"*. "The project color is driven by its **worst component**" is this registry's dominance reading of that requirement, not the source's own wording (the source states the transparency requirement; it does not use the word "worst"). That composition is executed in `weekly-status-rollup` Section 1 (Portfolio Health Dashboard health-color logic). This registry does **NOT** define a divergent composition algorithm.
...
The AC-bearing level. Each band cell for a REFERENCED row says "see `<doc> § <heading> → <sub-locator>`" (it does not reprint the sibling's numbers — duplicate-source-discipline; the `<!-- reference-durability: allow-link -->` fence at the top of this file permits the cross-links). The `Provenance` column records each band's source status as one of `referenced:<doc> § <heading> → <sub-locator>` | `audited:<doc> § <heading> → <sub-locator>` | `UNSOURCED-DOMAIN`, making source-of-truth status machine-checkable per row. When the target heading itself begins with `§` (as `km-protocols.md`'s do), that character serves as the separator rather than being doubled — `km-protocols.md §4 Bus-Factor Rules`. Every cited doc is linked once in § References.

**Citations locate by section name, never by line number.** The `§` segment carries the target heading's text verbatim. Where the referent is not itself a heading — a row inside a table, a numbered rule, an interpretation line — the citation names the **nearest enclosing heading** in the `§` segment and identifies the sub-referent by its own label after the `→`; a `§` is never invented for something that is not a real heading. A line number is never used: every line number in a long file still "resolves" after the target is edited, so a citation that has drifted onto the wrong content is indistinguishable from a correct one by any mechanical means — it fails open. A section name that no longer exists returns zero from a grep, which is the whole point of the form.

| Metric | Formula | Data Source | 🟢 GREEN | 🟡 YELLOW | 🔴 RED | Provenance | Decision Rule |
|---|---|---|---|---|---|---|---|
| **Schedule Performance Index (SPI)** | `earned schedule value ÷ planned schedule value` (EVM SPI; "X% behind schedule" ⇒ SPI = 1 − X/100, e.g. 8% behind ⇒ 0.92) | `PROJECT.md` milestone %-complete + plan baseline (EVM tracker) | see `channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` (`≥ 0.95`) | see `channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` (`0.85 ≤ SPI < 0.95`) | see `channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` (`< 0.85`) | `referenced:channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` | `WHEN 0.85 ≤ SPI < 0.95 THEN 🟡 YELLOW → watch milestone; flag at next status. WHEN SPI < 0.85 THEN 🔴 RED → escalate to sponsor WITHIN 2 business days; replan critical path.` |
| **Budget (CPI)** | `earned value ÷ actual cost` (EVM CPI) | EVM tracker / cost actuals | see `channel-formats.md § RAG Threshold Standards → Budget (CPI) row` (`≥ 0.95`) | see `channel-formats.md § RAG Threshold Standards → Budget (CPI) row` (`0.85 ≤ CPI < 0.95`) | see `channel-formats.md § RAG Threshold Standards → Budget (CPI) row` (`< 0.85`) | `referenced:channel-formats.md § RAG Threshold Standards → Budget (CPI) row` | `WHEN 0.85 ≤ CPI < 0.95 THEN 🟡 YELLOW → watch spend. WHEN CPI < 0.85 THEN 🔴 RED → escalate; re-baseline budget.` |
| **Scope** | approved CRs impacting baseline (qualitative bands per source) | `PROJECT.md` + change log | see `channel-formats.md § RAG Threshold Standards → Scope row` | see `channel-formats.md § RAG Threshold Standards → Scope row` | see `channel-formats.md § RAG Threshold Standards → Scope row` | `referenced:channel-formats.md § RAG Threshold Standards → Scope row` | `WHEN major baseline-impacting CR THEN 🔴 RED → gate change-control.` |
| **Risk** | open high / critical risk profile (qualitative bands per source) | RAID Log | see `channel-formats.md § RAG Threshold Standards → Risk row` | see `channel-formats.md § RAG Threshold Standards → Risk row` | see `channel-formats.md § RAG Threshold Standards → Risk row` | `referenced:channel-formats.md § RAG Threshold Standards → Risk row` | `WHEN any critical risk unmitigated OR ≥3 high risks THEN 🔴 RED → escalate.` |
...
| **Overdue RAID Count** | `count of RAID items past due / review date` | RAID Log | `0` | `1 ≤ count < 3` | `count ≥ 3` | `audited:raid-templates.md § 3.2 Action Lifecycle Rules → rule 4 (overdue actions auto-escalate); count-band registry-owned` | `WHEN 1 ≤ count < 3 THEN 🟡 YELLOW → watch; clear at next cadence. WHEN count ≥ 3 THEN 🔴 RED → escalate.` (Per-item AGE escalation timing per `escalation-thresholds.md` — not yet shipped; referenced, not restated. Per-item lifecycle escalation per `raid-templates.md §3.2 item 4`.) |
...
| Metric | Formula | Data Source | 🟢 GREEN | 🟡 YELLOW | 🔴 RED | Provenance | Decision Rule |
|---|---|---|---|---|---|---|---|
| **Aggregate SPI** | roll-up of constituent projects' SPI | program EVM roll-up | see `channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` (`≥ 0.95`) | see `channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` (`0.85 ≤ SPI < 0.95`) | see `channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` (`< 0.85`) | `referenced:channel-formats.md § RAG Threshold Standards → Schedule (SPI) row` | `WHEN 0.85 ≤ aggregate SPI < 0.95 THEN 🟡 YELLOW → watch program schedule. WHEN < 0.85 THEN 🔴 RED → escalate to program sponsor; replan.` |
| **Cross-Project Dependency Health** | `% items with unresolved cross-project dependencies` | program dependency register | see `backlog-health.md § 1.1 Primary Health Metrics → Dependency ratio row` (`< 15%`) | see `backlog-health.md § 1.1 Primary Health Metrics → Dependency ratio row` (`15–30%`) | see `backlog-health.md § 1.1 Primary Health Metrics → Dependency ratio row` (`> 30%`) | `referenced:backlog-health.md § 1.1 Primary Health Metrics → Dependency ratio row` | `WHEN 15% ≤ unresolved-dep ratio ≤ 30% THEN 🟡 YELLOW → watch. WHEN > 30% OR any blocked dep in next sprint THEN 🔴 RED → escalate.` (Cite `backlog-health.md § 1.1 Primary Health Metrics → Dependency ratio row`; do NOT cite `dependency-rules.md` — it uses a 1–5 scale, not %.) |
...
| Metric | Formula | Data Source | 🟢 GREEN | 🟡 YELLOW | 🔴 RED | Provenance | Decision Rule |
|---|---|---|---|---|---|---|---|
| **Portfolio Health Index** | `% active projects at 🟢` | `PORTFOLIO.md` roll-up | see `backlog-health.md § 1.2 Health Dashboard Formula → Interpretation line` (`> 80`) | see `backlog-health.md § 1.2 Health Dashboard Formula → Interpretation line` (`60–80`) | see `backlog-health.md § 1.2 Health Dashboard Formula → Interpretation line` (`< 60`) | `referenced:backlog-health.md § 1.2 Health Dashboard Formula → Interpretation line` | `WHEN 60 ≤ index ≤ 80 THEN 🟡 YELLOW → attention needed; review at-risk projects. WHEN < 60 THEN 🔴 RED → immediate intervention.` |
...
| **Aggregate Budget Health** | portfolio CPI roll-up | portfolio EVM roll-up | see `channel-formats.md § RAG Threshold Standards → Budget (CPI) row` (`≥ 0.95`) | see `channel-formats.md § RAG Threshold Standards → Budget (CPI) row` (`0.85 ≤ CPI < 0.95`) | see `channel-formats.md § RAG Threshold Standards → Budget (CPI) row` (`< 0.85`) | `referenced:channel-formats.md § RAG Threshold Standards → Budget (CPI) row` | `WHEN 0.85 ≤ portfolio CPI < 0.95 THEN 🟡 YELLOW → watch portfolio spend. WHEN < 0.85 THEN 🔴 RED → escalate.` |
...
| Metric | Formula | Data Source | 🟢 GREEN | 🟡 YELLOW | 🔴 RED | Provenance | Decision Rule |
|---|---|---|---|---|---|---|---|
| **Capacity Utilization** | `committed demand ÷ effective supply` (effective-capacity formula in `capacity-model.md §1`) | sprint plan + capacity model | see `capacity-model.md § 9. Demand-Supply Gap RAG Thresholds → 🟢 Green row` (🟢 `≤ 0.85`) | see `capacity-model.md § 9. Demand-Supply Gap RAG Thresholds → 🟡 Amber row` (🟡 `0.85 – 1.00`) | see `capacity-model.md § 9. Demand-Supply Gap RAG Thresholds → 🔴 Red row` (🔴 `> 1.00`) | `referenced:capacity-model.md § 9. Demand-Supply Gap RAG Thresholds → 🟢 Green row` | `WHEN 0.85 < utilization ≤ 1.00 THEN 🟡 YELLOW → protect reserve; resist mid-sprint scope additions. WHEN > 1.00 THEN 🔴 RED → re-scope / de-commit / re-baseline.` (Reproduce the source's inclusivity — `≤ 0.85` is GREEN; do NOT restate as `< 85%`. Reference the effective-capacity formula; do not restate it.) |
| **Sprint Commitment Reliability** | `delivered ÷ committed` | sprint board | see `backlog-health.md § 1.1 Primary Health Metrics → Estimation coverage row` (`> 90%`) | see `backlog-health.md § 1.1 Primary Health Metrics → Estimation coverage row` (`75–90%`) | see `backlog-health.md § 1.1 Primary Health Metrics → Estimation coverage row` (`< 75%`) | `referenced:backlog-health.md § 1.1 Primary Health Metrics → Estimation coverage row` | `WHEN 75% ≤ reliability ≤ 90% THEN 🟡 YELLOW → watch. WHEN < 75% THEN 🔴 RED → investigate estimation / over-commitment.` (**Band-source note.** `backlog-health.md` carries no commitment-reliability row; these bands are borrowed from its **Estimation coverage** row, whose thresholds are identical. The borrowing is named here rather than left implicit behind a locator — re-source or confirm it before treating these bands as owned.) |
...
| **Bus-Factor Risk** | bus-factor-critical artifacts without a K1 externalization path | KM artifact frontmatter + `grep` | per `km-protocols.md §4` | per `km-protocols.md §4` | per `km-protocols.md §4` | `referenced:km-protocols.md §4 Bus-Factor Rules` | `WHEN a bus-factor-critical artifact lacks an externalization path THEN escalate per km-protocols.md §4.` (Single-operator framing: bus-factor = 1 is the operating model, not a defect — do NOT restate the rule; reference `km-protocols.md §4`.) |

**Provenance discipline.** Every band cell-set carries a `Provenance` value. REFERENCED bands say "see `<doc> § <heading> → <sub-locator>`"; they never reprint a sibling's numbers, and they never locate the referent by line number. The only registry-OWNED numeric band is **Overdue RAID Count** (plus the `UNSOURCED-DOMAIN` qualitative / seed bands), which carry inline values. **Never fabricate an `audited:` citation** — if no in-corpus anchor exists, the value is `UNSOURCED-DOMAIN` (per the `capacity-model.md` precedent for above-cap Amber/Red boundaries).
...
Band:    SPI band (referenced, channel-formats.md § RAG Threshold Standards → Schedule (SPI) row): 🟢 ≥0.95 · 🟡 0.85≤SPI<0.95 · 🔴 <0.85.
...
Metric RAG: 🟡 YELLOW, citing SPI band channel-formats.md § RAG Threshold Standards → Schedule (SPI) row.
Project RAG: per the transparent-roll-up rule (channel-formats.md § RAG Threshold Standards → Watermelon detection rule), the project color is
...
**Cited sources (every doc a band cell or `Provenance` value names, linked once).** Follow the link, then locate the referent by the citation's `§ <heading> → <sub-locator>`:

- [`channel-formats.md`](../../comms-writer/references/channel-formats.md) — owned by `comms-writer`; § RAG Threshold Standards carries the SPI / CPI / Risk / Scope bands and the Watermelon detection rule.
- [`backlog-health.md`](../../delivery-engine/references/backlog-health.md) — owned by `delivery-engine`; § 1.1 Primary Health Metrics and § 1.2 Health Dashboard Formula.
- [`capacity-model.md`](../../delivery-engine/references/capacity-model.md) — owned by `delivery-engine`; § 9. Demand-Supply Gap RAG Thresholds.
- [`raid-templates.md`](../../delivery-engine/references/raid-templates.md) — owned by `delivery-engine`; § 3.2 Action Lifecycle Rules.
- [`km-protocols.md`](../../../../core/disciplines/km-protocols.md) — core discipline; §4 Bus-Factor Rules.
- [`failure-mode-standard.md`](../../../../core/standards/failure-mode-standard.md) — core standard; § Watermelon RAG acceptance — INPUT.
