...
**What you do**:
1. Read `references/gate-checklists.md` for the DoR criteria and `references/lifecycle-stages.md` for the stage entry criteria governing this transition (DoR is the entry gate to the Build phase per the universal lifecycle). This DoR check is the specific instance of the universal §5.2 per-transition entry-criteria enforcement applied at the Prepare→Build boundary `T(6→7)` — the LG-4 gate, bound to `T(6→7)` in `references/gate-definitions.md §4.2`; render the DoR verdict in the §4.1 `PASS / CONDITIONAL PASS / FAIL` vocabulary. The DoR criteria are the Agile-framing refinement of the canonical **G1 (Triage Readiness)** criteria in [`gate-criteria-spec.md`](../../../core/schemas/gate-criteria-spec.md) § Gate 1 — a DoR PASS maps to a `G1-*` PASS. **Then resolve the methodology kit for this item's kind and apply `references/gate-definitions.md` §4.3**: the **pack** from `delivery_approach` (the OPERATIONS.md Methodology Awareness Protocol read, **per invocation, never cached**), the **kind** from the item's `type:<kind_id>` label — and where neither resolves, take §4.3's unresolved arm rather than inferring a kind. Take §4.3's three-outcome branch, admit only what its predicate admits, and report the remainder with its disposition. **The key you read is `criteria.readiness`** — that is what §4.3's **read scope** binds to a *readiness* transition, and it is one key of that scope rather than the whole of it; §4.3 owns the binding, and the scope's full key set is stated there and not restated here. **Read the resolved manifest directly — never call the shipped pack reader:** on an inline-table array it returns fragments *and reports a clean parse*, so a mechanical read would evaluate shredded input while appearing to have worked.
...
3. **Enforce estimation discipline (do not merely read the parameters — apply them, and reject/adjust on violation):**
   - **Range validation — reject point estimates, and floor the width at MEASURED precision.** Apply `estimation-standards.md` §5: any single-point estimate (story-point or velocity figure) is REJECTED and returned as a range, no tighter than the item's Cone-of-Uncertainty band (§1) for its current lifecycle phase. State the phase and the band that set the width. If the lifecycle phase is unknown, default to the **widest applicable band** and label the phase `[ASSUMPTION – CONFIRM]` — never silently accept the point. **Then apply §8.6.1 Lever 2 before the width is fixed** (the lever is stated in full in the calibration-advisory bullet below): the declared phase's band is the floor **only while the team's measured precision is at least as tight as its claimed precision**. When the **realized-precision phase** (§8.4 — grant **G5**) is **wider** than the item's declared `Estimate Phase` (grant **G7**), the floor is the **realized** band's `low×–high×` bounds (grant **G6**), and the emitted range widens to them. This is the composition point — a measured-precision floor that binds only at reflection time never reaches the estimate it was measured to correct.
   ...
   - **Estimation-calibration advisory — surface, never apply.** Apply `estimation-standards.md` §8 under its **§8.6.1 consumer contract**, which is **read-only**: consume **only the elements the §8.6 grant table marks for Mode D**, and read that table rather than any restatement of it — a second copy of the grant kept here is a second home that drifts from the first, which is the defect that left Lever 2 requiring an element the contract did not grant. Compose the advisory and **render it — never apply it, and never write any value back.** `estimation-standards.md` §3, `sprint-defaults.md` §1.2, and `capacity-model.md` are **read-only to this path**: a focus-factor move is surfaced as a `[RECOMMENDED]` proposal **inside §3's valid range** requiring a governed edit (issue → plan → PR), **never performed here** — §3's Consumers note makes the focus factor a value two other docs inherit *by role* without restating it, so an automated write there would propagate into both with **no diff trail in either**. Emit `B` with its direction word, its 🟢/🟡/🔴 band cited from **§8.3 by role** (**never restate a boundary value**), `N`, and the §3.1 confidence word; emit `S` on a **separate labelled line** with its realized-precision phase. **At most ONE bias lever per emission** — 🟢 → none; 🟡 → the `[RECOMMENDED]` multiplier proposal, and do **not** change the §3 focus factor at this band; 🔴 → a `[RECOMMENDED]` re-baseline, which **supersedes** the multiplier (a lever applied twice on one signal biases the plan in the opposite direction). **Run the corroboration test BEFORE the `[RECOMMENDED]` line is composed, not after**: never fire a lever on an **F1-only 🔴** without F2 or F3 corroboration (§8.7 V3 + the §8.7 evidence-quality bar). Never apply a bias when the band is 🟢 or the figure is `not computable`; never pool signal families (§8.1 rule 1); never collapse `B` and `S` into one figure (§8.2).
     - **Lever 2 — spread → a range-width floor. This mode implements it, because this is where future estimates are composed.** §8.6.1 assigns Lever 2 to **this consumer**, and it is the substantive feedback path — *"a demonstrable change to a future estimate."* **Fire it here, at step 3's range validation:** when the **realized-precision phase** (**G5**) is **wider** than the item's declared `Estimate Phase` (**G7**) — an ordering read off §8.4's width column (**G6**) — **widen the emitted §5 range to the realized band**, whose bounds are that table's `low×–high×` pair, and **name BOTH phases in the emission** (the phase the estimates claimed, and the phase they realized). Withholding either leaves the reader a widened range with no stated cause. **It is not a second bias lever and MAY fire alongside the bias lever** — it changes the range's **WIDTH, never its centre** — so the "at most ONE bias lever" rule above does not suppress it, and an emission carrying a bias lever **and** Lever 2 is correct, not a double-correction. **`S` MUST NOT be fed through a multiplier**: it is a ratio of extremes with no centre and no direction, so multiplying by it injects a bias into a team that has none (§8.2, Team B). **This mutates nothing** — it makes §1's existing cone rule bind at the team's *measured* precision rather than its *claimed* precision. **The failure this closes:** a measured-precision floor that fires only in Mode E's reflection-time report never binds on the estimate it exists to correct, so the loop's stated value proposition — a demonstrable change to a future estimate — goes unrealized while every document reads as though it were satisfied.
3.5. **Facilitation-technique surfacing (silent-by-default — at most one suggestion per activity).** A sprint-planning input can carry up to **two distinct facilitation-shaped activities** — an *estimation* (sizing) activity and an *iteration/release-planning* (goal-setting + scope-cut) activity — which may co-occur in one session. Each is a separate activity with its own domain file, and each surfaces **at most one** technique under the identical three-condition gate; the two are evaluated independently, so a session that is both sizing and scope-cutting may surface one estimation card AND one planning card (never two from the same activity).
   ...
   For **each** in-scope activity, surface ONE technique suggestion from that activity's domain file **only when all three hold**: (a) the technique's `when_to_use` matches the in-scope activity, AND (b) its `methodology_compatibility` fits the project's `delivery_approach` (read per the OPERATIONS.md Methodology Awareness Protocol), AND (c) its `when_NOT_to_use` is **not** triggered. When surfaced, render it as a `[RECOMMENDED]`-tagged note carrying the **named technique + a one-line why + the participants/time line** — e.g., `[RECOMMENDED] Planning Poker — whole-team relative sizing where the divergence discussion surfaces hidden scope/risk; ~1–2 min/item, whole team + facilitator.` or `[RECOMMENDED] MoSCoW — band over-capacity scope into Must/Should/Could/Won't so the cut is a stated decision; ~20–40 min, team + facilitator + PO.` Surface **at most one** technique per activity; **stay silent** when no entry matches, when the activity is not facilitation-shaped, or when the matched technique is contraindicated (e.g., do not suggest Planning Poker on a 1–2-person team, and do not suggest MoSCoW when the backlog is already force-ranked). **Never block the plan on a suggestion** — it is an additive note, never a gate.
4. Produce a sprint plan:
   ...
   - **Estimation calibration + advisory — the block is MANDATORY; silence is a defect, not a pass**: render **exactly one** calibration block per signal family, and open every one of them with the literal token **`estimation calibration`**, per `estimation-standards.md` **§8.6.1 § Emission requirements** (which carries this requirement, the negative-path rendering, and the token-absent → FAIL check). The block carries, in this order: **coverage** — `N` completed iterations, the pair count **against the iteration's planned item count**, the **capture-exception count for that family with its closed-set reason breakdown** (grant **G13**, read from the tracker's `## Capture Exceptions` — this is that population's **only** reader, and a coverage line without it is a **FAIL**, since a window with three uncaptured closes then reads identically to a window with none), the **unclosed remainder** `planned − (pairs + exceptions)` as a number rather than an absence, and the §3.1 confidence word (coverage renders at **any** `N`; it is the survivorship control per §8.7 V2 and is most useful when the sample is thin); **window** — the `Window Key` read and **whether it is the most recent closed window** (grant **G12**), because a lever fired off a stale window is otherwise indistinguishable from one fired off the current one; **calibration bias** — `B` + its **direction word** + its 🟢/🟡/🔴 band, cited from **§8.3 by role** with **no boundary value restated**; **calibration spread** on a **separate labelled line** — `S` + its realized-precision phase + whether that phase is **wider** than the declared `Estimate Phase`, expressed as a **sentence, never a colour** (§8.4); the **active `[RECOMMENDED]` lever(s)** — **at most one BIAS lever**, plus **Lever 2's range-width floor when it fires** (Lever 2 is not a second bias lever: it moves the range's width and never its centre, so the two may render together) — each rendering **prior → adjusted** plus its **basis** (`B`, the signal family, `N`, the confidence word, and the lever name; Lever 2's basis is the **declared and realized phases, both named**) — an operator-visible estimate that differs from the figure the team produced **without** that basis is a defect; the **throughput** line — the points-per-item trend direction (grant **G15**), rendered **whether or not** the inflation flag fires, because while only the flag renders a non-firing flag cannot be told apart from a cross-check that was never run; the **inflation flag** when `B` trends toward 1.00 **while** points-per-item trends up (`sprint-defaults.md` §3.2 rule 4's already-mandatory cross-check) — rendered **in this same block, never in a separate artifact**, so the correction and its own falsifier travel together; and the **evidence grade** of the window — its **weakest** grade present plus the count of `[ASSUMPTION – CONFIRM]` pairs (grant **G16**) — which is the state §8.7's bar conditions on when it forbids such a pair *alone* driving a 🔴, and is what makes the corroboration test above observable rather than merely asserted. **Label the two figures with their units — `B` is a central multiplier, `S` is a range ratio — so they cannot be read as one composite**; a summed, averaged, differenced, or otherwise collapsed "accuracy" figure is **prohibited** (§8.2, and the §8.3 naming guard). The adjusted figure is a **RANGE, never a point** (§5 binds unconditionally). **When the figures are not computable** — no §8 data at all, or `N` below §8.3's window floor — **the block still renders**, carrying §8.3's negative-path string verbatim (`estimation bias: not computable — N iteration(s) of history (< 3); do not use for forecasting`) plus a quantified `[RECOMMENDED] accrue <§8.3 floor − N> more completed iteration(s) before reading a band`. Never a colour, a greyed or provisional band, a blank cell, or a defaulted GREEN on the negative path. **An emission carrying no calibration block at all is a DEFECT** — its absence is indistinguishable in output from a working loop that had nothing to say, so the **absence of the `estimation calibration` token is the detectable failure signature**, and a check over the emission **fails closed**: token present → the block is graded; token absent → FAIL, never a pass.
...
2. Identify emerging risks: scope creep (new items added mid-sprint), velocity
   ...
   **Estimation-Calibration Report sub-protocol (team × signal family × window).** The retrospective read of estimate accuracy, rendered from `estimation-standards.md` §8 under its **§8.6.2 reporting contract**. **Read-only**: this sub-protocol renders §8's outputs and writes no value into §8 or any doc that inherits from it. Where Mode D's advisory acts at estimate-composition time, this reads at reflection time. Invoked from step 1.
   ...
   - **Render coverage FIRST, always.** `N` completed iterations + the pair count **against the iteration's planned item count** + the **capture-exception count for that family, with its closed-set reason breakdown** (grant **G13**, read from the tracker's `## Capture Exceptions`) + the **unclosed remainder** `planned − (pairs + exceptions)` + the §3.1 confidence word. The three counts **reconcile** against the denominator, and the exception count is that population's **only** reader — a coverage line that omits it is a **FAIL**, because a window with uncaptured closes then reads exactly like a window with none, and each of the four closed-set reasons implies a different remedy (`no-estimate-of-record` → an admission gap in Mode C; `estimate-not-in-window` → a window-boundary effect; `item-descoped-at-close` → genuine attrition; `unit-change-pending-re-anchor` → a §8.7 V5 exclusion). Coverage renders at **any** `N` — it is computable when nothing else is, and it is §8.7 V2's survivorship control, so it is *more* useful when the sample is thin, not less. Never suppress the whole report for want of a band.
   - **Render the window** (grant **G12**): the `Window Key` read, and **whether it is the most recent closed window**. A report off a stale window reads identically to a current one unless the window says which it is, and the trend's ordering is unreadable without it. Renders at **any** `N`, negative path included — a not-computable report still names the window it could not compute.
   ...
   - **Render trend** across consecutive windows as **`improving` / `stable` / `degrading`** — adopted verbatim from `core/schemas/gate-evaluation-spec.md` § Layer 3, **not a new enum** — under **grant G14**, whose domain and strength bound travel with the word and are read from **§8.6.2 § Trend**: the domain is **≥ 2 banded windows** (a comparison needs two operands, and each window must itself be banded, so §8.3's `N ≥ 3` binds transitively), and the trend's strength is bounded by **§8.3's own `9+` / High-confidence row** — below it the trend is an **observation carrying its confidence word**, at or above it a **finding**. **Mint no window count here**: a threshold this mode invents is exactly the class the grant table exists to prevent, and the previous "≥ 2 consecutive windows" form asserted a meaningfulness §8.3 reserves for `9+`. When the trend is **`stable` while the band is 🟡 or 🔴** across the windows compared, do not report `stable` as a neutral word: state that **the signal is being produced and not acted on**, at the strength its confidence row allows and **never suppressed for want of a higher row**. An advisory nobody acts on is observationally identical to a working loop, and this is the only place that difference becomes visible.
   - **Render throughput — the flag's second input, always** (grant **G15**): the **points-per-item trend direction**, rendered **whether or not** the inflation flag fires. While only the flag renders, a non-firing flag is indistinguishable from a cross-check nobody ran, and the rule that mandates the cross-check becomes unfalsifiable — the reader cannot tell a clean cross-check from an absent one.
   ...
   - **Render the evidence grade** (grant **G16**): the **weakest** `Evidence Grade` present in the window's pairs, plus the count of `[ASSUMPTION – CONFIRM]` pairs. This is the state §8.7's evidence bar conditions on — an `[ASSUMPTION – CONFIRM]` pair **may not alone drive a 🔴**, and F1 is capped at `[INFERRED]` — so without it rendered, a 🔴 that violated the bar and one that honoured it look the same on the page.
   ...
   - **Negative-path defaults are explicit, never silent (fails closed).** Below §8.3's window floor, each suppressed element states **its own** binding reason — `estimation bias: not computable — N iteration(s) of history (< 3); do not use for forecasting` · `calibration spread: not computable — requires the same ≥ 3-iteration window` · `trend: not computable — requires ≥ 2 banded windows` — because the three constraints differ and a reader needs to know which one binds to know what to do next. **Each string is the corpus literal, rendered unaltered** — the bias string is §8.3's fenced block **verbatim**, its own leading token serving as the element label, and the spread and trend strings are §8.6.2's (per §8.6.2 § One literal, checked by equality). **Never relabel, abbreviate, or reflow one**: a second near-variant makes "renders it verbatim" unfalsifiable, and Mode E and Mode D render the identical bias literal. Add the quantified `[RECOMMENDED] accrue <§8.3 floor − N> more completed iteration(s) before reading a band`, and carry the confidence word even here (it is the *reason* the band is withheld). **NEVER a colour, NEVER a greyed or provisional band, NEVER a blank cell, and NEVER a fourth colour for "unknown"** — a colour in the colour column reads as a verdict, and below the floor **no verdict exists**.
...
the **Estimation-Calibration Report** (coverage · window + recency · calibration bias + its band ·
calibration spread + its realized-precision phase · trend · throughput direction · inflation flag ·
the window's weakest evidence grade), **or** its `not computable`
...
**What you do**:
1. Read `references/gate-checklists.md` for the checklist templates AND `references/gate-definitions.md` for the lifecycle-gate entry/exit criteria, the §4 transition BLOCK rule, the **§4.1 tri-state verdict semantics**, and the **§4.2 gate→`T(n→n+1)` binding**, AND `references/lifecycle-stages.md` for the stage exit criteria governing the transition (the §3 stage→gate seam + the §5.2 per-transition enforcement). The DoD/release-readiness criteria are the Agile-framing refinement of the canonical **G3 (Release Readiness)** criteria in [`gate-criteria-spec.md`](../../../core/schemas/gate-criteria-spec.md) § Gate 3 — a DoD/release-readiness PASS maps to a `G3-*` PASS. **Then resolve the methodology kit for this item's kind and apply `gate-definitions.md` §4.3**: the **pack** from `delivery_approach` (the OPERATIONS.md Methodology Awareness Protocol read, **per invocation, never cached**), the **kind** from the item's `type:<kind_id>` label — and where neither resolves, take §4.3's unresolved arm rather than inferring a kind. Take §4.3's three-outcome branch, admit only what its predicate admits, and report the remainder with its disposition; a block whose admitted subset is empty still emits the report with its denominator. **The key you read is `criteria.done`** — that is what §4.3's **read scope** binds to a *done* transition, and it is one key of that scope rather than the whole of it; §4.3 owns the binding, and the scope's full key set is stated there and not restated here. On shipped pack content an empty admitted subset is the ordinary outcome at this key, so that report is what a DoD run should expect to see rather than an exception. **Read the resolved manifest directly — never call the shipped pack reader:** on an inline-table array it returns fragments *and reports a clean parse*, so a mechanical read would evaluate shredded input while appearing to have worked. **Apply the §4 transition rule at WHICHEVER lifecycle gate the transition sits at — LG-1…LG-10, not LG-6 alone** (the gate-application loop):
   ...
   - **Where the transition is one the resolved kind declares a per-item gate for, the key §4.3's read scope binds is `criteria.gate`, not `criteria.done`** — a kind's `criteria.gate` block guards the Axis-1 edge that check's own `guards_transition` names, so a gate-class transition reads that block and admits from it on the same predicate. Where the kind declares no such gate, the block is read and its reasoned-empty or unresolved arm is taken exactly as §4.3 states; it is never skipped on the assumption that a gate-class transition has no kit criteria.
   ...
   - **Estimate/actual capture at close (the calibration loop's only input path).** This gate is where an estimate meets its outcome, so it is where the pair is captured — schema: [`../../../core/schemas/tracker-schemas.md`](../../../core/schemas/tracker-schemas.md) **§ Tracker 10: Sprint Tracker**. Emit a `TRACKER_UPDATE` against `[Project]_Sprint_Tracker.md`; Tracker Manager validates and writes the row. **Emit rows, never figures** — the ratio, the calibration bias/spread pair, the band, and the window floor are owned by [`references/estimation-standards.md`](references/estimation-standards.md) § 8 and are read back by **Mode D** (estimate feedback) and **Mode E** (the team calibration report), never computed here.
     - **Item close — this loop's LG-5 verdict.** On an **LG-5 Dev Complete (DoD)** exit **PASS** or **CONDITIONAL PASS** at `T(8→9)`, capture the item's pair: `Actual` and `Actual Date` only (`Estimate` was frozen when Mode C admitted the item at LG-4 and is **not** re-sent, so the F1 re-score stays blind). On **FAIL** or **NO-EVIDENCE**, capture **nothing and record no exception** — the item is still open and this gate will fire again. Capture binds to the **verdict**, never to a "done" claim.
     - **One `MODIFY` per admitted signal family — a close completes up to TWO rows.** The row key is `(Item Ref × Signal Family × Close Ordinal)`, so the close emits a **separate** instruction per family, each `entry_id` resolving the **full triple**: **F1** carries the blind re-score as `Actual` and **no `Elapsed`**; **F2** carries the elapsed business-day figure, where `Actual` **equals** `Elapsed`. **Two instructions from one close is the specification, not a double-write** — they land in two different cells. Emitting only one family's `MODIFY` leaves the other family's row open and drives F1 and F2 onto **disjoint item sets**, which silently destroys the F1↔F2 corroboration §8.7 V2 depends on. An `entry_id` that does not resolve all three key components is **ambiguous — reject and surface it**, never infer the family from the presence of an `Elapsed` field.
     - **Team close — end-of-sprint review.** Write the `## Sprint History` row (including `Window Key` and `Planned Items`) **after** every item capture for that window has landed. Writing it first lets `Completed` and the pair count disagree with no way to tell which is stale.
...
- **Mitigation:** Bind the lever to the figure by **dimension, not by shape**. `B` is a
  **central multiplier** and changes an estimate's **value**; `S` is a **range ratio** and
  ...
  cone-phase equivalence → widen the emitted §5 range to the realized band (Lever 2 — fired at
  **Mode D step 3**, where estimates are composed, and applied again at Mode E step 6 whenever
  that mode emits a range);
...
| `references/gate-definitions.md` | Mode F (gate transition across LG-1…LG-10), Mode G (gate decisions), Mode C (LG-4 DoR binding) | Project-lifecycle gate sequence (LG-0 Idea Screen → LG-10 Closure): per-gate entry/exit/authority/artifacts/escalation + the §4 gate-transition BLOCK rule + tri-state `PASS / CONDITIONAL PASS / FAIL` verdict semantics (§4.1) + gate→`T(n→n+1)` binding with the no-advance/no-skip rule (§4.2) + the resolved-kit criteria admission-and-reporting rule (§4.3 — the three-outcome branch on a block-level `source`, the admission predicate whose default is non-admission, the pack-and-kind resolution, the agent-read prohibition, and the one-block `NOT-EVALUATED` report) |
...
| `references/estimation-standards.md` | Mode D (Sprint Planning), Mode E (Execution Control) | Cone of Uncertainty, planning-horizon rules, the canonical focus-factor table, buffer three-zone model, buffer-consumption RAG banding (§4.1), velocity-as-range enforcement, contingency vs. management reserve, milestone-variance (SPI) RAG (§7), the estimation-calibration loop (§8 — estimation ratio, calibration bias + estimation-bias RAG band, calibration spread as a §1 cone-phase equivalence, the actuals-capture input contract, and the feedback/report consumer contracts) |
...
| `core/standards/facilitation-techniques/README.md` | Mode D (Sprint Planning — estimation + iteration/release-planning technique surfacing, step 3.5), Mode E (retrospective context hook + the deferred stand-up extension point) | The K1 delivery-lifecycle facilitation-techniques corpus — one card per technique (9-field schema) organized by lifecycle domain. Consulted to surface at most one fitting, non-contraindicated `[RECOMMENDED]` technique per in-scope activity; silent-by-default. Estimation (`estimation.md`), Retrospective (`retrospective.md`), and Planning (`planning.md`) domains are seeded; other domains are deferred (see the corpus index domain manifest) |
