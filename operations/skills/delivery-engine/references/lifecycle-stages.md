...
| **`delivery_approach: Hybrid`** (or a 2-element array `[A, B]`) with `dual_framing_enabled: true` | For the literal `Hybrid` value, render the **Hybrid column**; for the **array form** `[A, B]` (project-schema §6.5), resolve the §4 column for **each** constituent and render one section per constituent (union per work-organization-mapping-framework.md §2.5). Either way, honor the dual-framing the SKILL.md Dual-output rule already requires — the iterative/timeboxed track gets stage/flow naming, the phase-gated track gets milestone naming. (Methodology Awareness: Hybrid/array skills MUST ALSO read `dual_framing_enabled`.) |
