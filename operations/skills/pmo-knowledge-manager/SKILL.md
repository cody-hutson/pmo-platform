...
**Process:** (1) run the pre-creation governance check FIRST (see `## Guardrails`) and cite it; if a home exists, route content *into* it rather than minting a parallel asset; (2) apply the durability test (FM-3) — reusable lesson/procedure/decision/reference, not point-in-time state; (3) chain to `artifact-generator` Generate Mode to produce + stage (`lifecycle_state: draft` + `promotion_state: staged` in `08-Generated/`); (4) chain to Mode 3 for the governed home and place it there; (5) render the summary with the asset, its home, and a reversibility tier + confidence.
...
- **Design-time best-practice anchor:** [`core/standards/domain-best-practices/governance.md`](../../../core/standards/domain-best-practices/governance.md) — the authoritative knowledge-codification practice guide (Nonaka SECI · Diátaxis; knowledge capture, structuring, and documentation discipline) this Specialist consults as design-consumption input. Pointer only — no content absorption ([ADR-019](../../../core/ADRs/ADR-019-specialists-compose-not-absorb.md) compose-by-reference); mirrors the Stage-5 design spoke's domain-guide consultation in [`release/references/pipeline/stage-05-solutioning.md`](../../../release/references/pipeline/stage-05-solutioning.md) §5.7.
