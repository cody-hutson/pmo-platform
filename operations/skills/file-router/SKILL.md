...
- **Signature (observable signal):** A file is routed to a destination based on filename
  ...
  to 4-Evidence/Transcripts/ but containing meeting-agenda or email-forwarding content).
...
- **Signature (observable signal):** A FILE ROUTING SUMMARY reports a file in the HIGH
  ...
  85%), or a file auto-routed to an approval-required folder (1-Governance/, 2-Delivery/,
  3-Operations/, 5-Reference/) without the approval-required gate
  surfaced.
...
- **Mitigation:** Emit the exact numeric confidence score in every routing decision; route
  ...
  targets even at HIGH confidence per the table's second condition (1-Governance/ through
  5-Reference/ routes require approval regardless of confidence).
...
- **Signature (observable signal):** The `_inbox/_unsorted/_queue.md` file
...
- **Signature (observable signal):** A transcript is saved to the correct
  4-Evidence/Transcripts/ sub-folder, but no TR-### Transcript Register entry, no structured
...
- **Signature (observable signal):** On a direction 3 (Promotion) action, file-router
  physically moves a staged file from `_generated/` to its target folder and/or stamps
...
### Cross-project auto-write into another project's 4-Evidence/ or _generated/ folder — TRIG

- **Signature (observable signal):** On a direction 4 (Cross-project) route, a file that
  originates outside the winning project is auto-written into that project's 4-Evidence/Transcripts/,
  4-Evidence/Emails/, or _generated/ folder without an approval gate, on the reasoning that those
...
- **Conditional:** do NOT treat a cross-project destination's auto-write folders as
  ...
  approval-gated even into 4-Evidence/ or _generated/, since a wrong cross-project route contaminates another
  project's Transcript Register, email archive, or generated-staging surface and is
  expensive to unwind from the outside.
- **Root cause:** The auto-write folder list (4-Evidence/ + _generated/ free; 1-Governance/2-Delivery/3-Operations/5-Reference approval) is memorized
...
- **Principal response vs. junior response:** Principal recognizes that auto-write is an
  ...
  the approval prompt. Junior sees "_generated" on the auto-write list and writes into the
...
- **Mitigation:** Always run Direction Classification first. Check the file's existing state
  (registered? `promotion_state` stamped? already in a `[Project]/` closed-set bin?) before any
...
- **Principal response vs. junior response:** Principal runs the pre-step, detects the file is
  ...
  back to `_generated/` surfaces the collapse later.
