...
Wrapper Mode writes the same frontmatter block as Generate Mode, extended by one new field-value (`source: external`) and one new field (`source_origin`), so the header round-trips through every existing consumer (Promotion Workflow, Artifact Health scan, auto-archive). `status` is always `PENDING_REVIEW` — **Wrapper Mode is never promoted on ingest**. Full schema and the Domain-C forward-map note are in the [SKILL.md §Wrapper Mode](../SKILL.md).
...
- Sourcing posture (no runtime coupling): [ADR-021](../../../../core/ADRs/ADR-021-skill-sourcing-coupling-posture.md)
