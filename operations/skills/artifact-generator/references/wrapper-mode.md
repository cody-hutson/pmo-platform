...
Wrapper Mode writes the same frontmatter block as Generate Mode, extended by one new field-value (`source: external`) and one new field (`source_origin`), so the header round-trips through every existing consumer (Promotion Workflow, Artifact Health scan, auto-archive). On emit it stamps `lifecycle_state: draft` (the `Artifact-DRAFT` content entry; the former `PENDING_REVIEW` value, re-specced onto the reconciled model) + `promotion_state: staged` — and because `promotion_state` is always `staged` (never `promoted`) on ingest, **Wrapper Mode is never promoted on ingest**. Full schema and the Domain-C forward-map note are in the [SKILL.md §Wrapper Mode](../SKILL.md); the canonical state vocabularies, the application-layer stamping rules, and transitions live in [`lifecycle-states.md`](lifecycle-states.md).
