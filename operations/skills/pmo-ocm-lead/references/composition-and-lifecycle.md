...
Every `pmo-ocm-lead` mode is a **role-level orchestration** over one or more `change-management` modes, invoked through the `core/` registry skill-chain (ADR-007) — never inline-reimplemented. The composed `change-management` modes are owned by `change-management`; the citations below pin each one by **section-name anchor** — a `§` segment carrying the target heading's text verbatim, over a plain link to the target file with no `#fragment`. A line-number anchor is never used: every line number in a long file still "resolves" after the target is edited, so a citation that has drifted onto the wrong content is indistinguishable from a correct one by any mechanical means. A section name that no longer exists returns zero from a grep, so the same edit that would silently break a line anchor makes a name anchor loudly detectable. Where the referent is not itself a heading — a numbered step inside a mode, for instance — the citation names the **nearest enclosing heading** and identifies the sub-referent by its own label in prose.

| pmo-ocm-lead mode | Composes `change-management` mode(s) | Evidence (`change-management/SKILL.md`) |
|---|---|---|
| **Mode 1 — Change Impact** | **Mode A** (Change Impact Assessment); ingests an existing matrix via **Mode E** (Change Matrix Ingestion) when one is supplied | [`change-management` § Mode A — Change Impact Assessment](../../change-management/SKILL.md) · [`change-management` § Mode E — Change Matrix Ingestion](../../change-management/SKILL.md) |
| **Mode 2 — Training & Adoption** | **Mode B** (Training Plan) + **Mode G** (Adoption Tracking — ADKAR barrier/timing, champion ratio, sponsor engagement) | [`change-management` § Mode B — Training Plan](../../change-management/SKILL.md) · [`change-management` § Mode G — Adoption Tracking](../../change-management/SKILL.md) |
| **Mode 3 — Readiness Go/No-Go** | **Mode C** (Readiness Checklist → READY / CONDITIONAL / NOT READY) | [`change-management` § Mode C — Readiness Checklist](../../change-management/SKILL.md) |
| **Mode 4 — Hypercare & Adoption Outcome** | **Mode D** (Hypercare Plan — tiered risk register, SLA-compliance, exit gate) + **Mode G** (adoption-outcome / deployed-vs-adopted) | [`change-management` § Mode D — Hypercare Plan](../../change-management/SKILL.md) · [`change-management` § Mode G — Adoption Tracking](../../change-management/SKILL.md) |
| **Mode 5 — Change Comms Program** | **Mode F** (CM Communications Schedule — the *schedule*; Mode F routes individual drafts to `comms-writer` via its `[COMMS]` handoff) | [`change-management` § Mode F — CM Communications Schedule](../../change-management/SKILL.md) |

Coverage vs. the issue's worked examples: Impact→A, Training→B, Readiness→C, Hypercare→D, Comms→F — all honored, with Mode E folded into impact entry (matrix ingestion) and Mode G folded into training and hypercare (adoption instrumentation). `change-management` Mode D's exit gate ([`change-management` § Mode D — Hypercare Plan](../../change-management/SKILL.md) → its exit-criteria step, where an open SLA breach is a hard, non-waivable exit gate) and the Mode G deployed-vs-adopted failure mode are the cited hardening Mode 4 carries forward.
