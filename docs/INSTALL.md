...
7. Resolves operator-identifying tokens — via interactive prompts by default, or from each token's declared default with no read from stdin under `--non-interactive`; writes the canonical `operator.toml` at `~/.config/pmo-platform/operator.toml` (XDG-spec; mode 0600). Which keys that file carries is declared at `core/config/operator-toml-schema.json` rather than hardcoded in the generator, so a key the platform adds later reaches an **existing** install too: `./update.sh` reconciles your config against the declaration, and `docs/scripts/setup-workspace.sh --reconcile-config` does the same on demand. A backfill takes the declared default; a value you set yourself is preserved, and no key is removed.
...
10. Installs the post-merge deploy git hook into the source repo's hooks directory, so a merge to `main` in the primary checkout redeploys the skills that merge brought in without a manual deploy. Installed when the repo sits inside the workspace root (the layout above); otherwise the install prints the one-line command to enable it.
11. Installs composition-surface seed files (allowlists, exemption lists) from `core/config/allowlists/` to runtime locations (`~/Claude/.claude/` for hook-tier; `~/Claude/pmo-instance/` for instance-tier), wrapped in MANAGED SECTION + OPERATOR ADDITIONS fences per [`composition-surface-spec.md` §2](../core/standards/composition-surface-spec.md). Install-if-missing semantics: operator edits to OPERATOR ADDITIONS sections are preserved on re-run. The same pass installs the **operations context anchor** at `~/Claude/projects/CLAUDE.md` — a pointer-only file naming the platform governance an operations-rooted session loads, so a session started anywhere under `projects/` reaches the operational procedure set without you doing anything. It restates no procedure and is never overwritten once present.
12. Runs a post-install verification gate. On pass, writes `.workspace-setup.state` with `verification_passed: true` and prints the validate-install invocation hint.
...
ls -d "${CLAUDE_WORKSPACE_ROOT:-$HOME/Claude}"/pmo-instance/{inbox,ambient-intake,external-sync}
```
