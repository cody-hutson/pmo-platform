...
----------------------------------------------
  ...
  R3  citation sweep     — branch-scoped (see the R3 scoping rule below), and on
                           an INDEX surface additionally attributed per LINE: a
                           bare number there is very often another record's entry.
  R4  index surfaces     — the HAND-MAINTAINED index (`core/ADRs/README.md`) gets
                           a path-exact rewrite (always) + an attributed bare
                           sweep + a re-sort + an append to the § Renumber log,
                           and NAMES the case where the index carries no entry for
                           the moved record at all. The PROJECTED index
  ...
  R6  zero-dangling verify — re-scan; any surviving in-scope ``ADR-<old>`` is a
                           failure, and the whole staged set is reverted. Plus a
                           SECOND arm asking the complementary question: does every
                           ``ADR-<new>-*.md`` reference this run WROTE resolve? The
                           first arm cannot see a mis-attributed rewrite, which
                           carries the NEW number on a FOREIGN slug and leaves no
                           ``ADR-<old>`` token behind at all.
...
time there always is one, because that is what made this a collision.
  ...
  Exception:     an INDEX surface is governed by ATTRIBUTION rather than by scope
                 membership, in R3 and R4 alike (`_bare_sweep_mask`). A path-exact
                 reference to the moved file (``ADR-<old>-<slug>.md``) is an
                 identity match and is rewritten unconditionally. A BARE number is
                 rewritten only on a line the branch itself authored, because an
                 index's whole job is to enumerate OTHER records by number — so a
                 bare token there is foreign by default, not by accident. On a
                 PROJECTED index the exception is stronger still: the projector
                 derives every row from the file set, so it cannot reach another
                 record's number by construction.
  ...
  Not covered:   the same NON-INDEX file carrying BOTH a branch citation and a
                 mainline citation of ``<old>``. ``--exclude-path`` is whole-file,
                 so that case still needs a hand pass.

                 The line-range scoping this note used to name as the durable fix
                 now EXISTS (`_branch_added_lines`) and is applied to index
                 surfaces, where the collision is not merely possible but
                 structural — an index enumerates other records by number, and the
                 observed failure was a neighbouring record's heading and `File:`
                 link repointed at a path that does not exist, under a green
                 verify and a zero exit. It is deliberately NOT yet applied to
                 ordinary branch files: there the residual is bounded and has an
                 escape hatch, and widening the mask to every file would change
                 the sweep's population everywhere at once. Widening it is a
                 one-line change at the `_bare_sweep_mask` predicate when that
...
def rewrite_citations(text, old, new, preserve_historical=True, only_lines=None):
    """Return (new_text, count, review) — every whole-token ADR-<old> → ADR-<new>.

    ``only_lines`` is the ATTRIBUTION MASK: a set of 1-based line numbers this
    sweep may touch, or ``None`` for every line. A line outside the mask is
    passed through untouched and is NOT added to ``review`` — it was never this
    sweep's to decide. See ``_bare_sweep_mask`` for why an index surface needs
    one and an ordinary branch file does not.
    ...
    for i, (line, verdict, carve) in enumerate(
        ...
        if only_lines is not None and i not in only_lines:
            # NOT OURS. The mask is checked BEFORE the verdict on purpose: a line
            # this branch did not author is outside the sweep's population, so it
            # is neither rewritten nor reported as an ambiguous site the operator
            # must disposition. Its tokens are counted by the caller instead.
            out.append(line)
            continue
        if verdict == RECORD:
...
def _strip_projected_region(lines):
    ...
    LINE-NUMBER PRESERVING: a region line is BLANKED, never dropped, so index *i*
    of the result is still physical line *i* of the file. Two callers depend on
    that. The attribution mask (`_bare_sweep_mask`) is keyed on physical line
    numbers read from a git diff, so a strip that shortened the list would silently
    shift every line past the region and mask the wrong ones. And R6's REVIEW block
    prints `rel:lineno` for a human to open — those numbers were off by the region's
    height for every projected surface before this, pointing the reader at the
    wrong line of the right file.

    Blanking is also the more faithful edit: the region is a block, and dropping it
    would splice the paragraph above onto the paragraph below, which is precisely
    the kind of accidental paragraph join `_is_paragraph_continuation` exists to
    get right.
    """
    out, inside = [], False
    for ln in lines:
...
def _index_surfaces(root):
    """Every ADR index surface — hand-maintained AND projected.

    The set a BARE-NUMBER sweep may not be trusted on. An index enumerates
    OTHER RECORDS BY NUMBER, which makes it the one surface where a foreign
    record's identity is written down as the bare token this tool rewrites.
    """
    return set(_index_files(root)) | set(PROJECTED_INDEXES)


HUNK_HEADER_RE = re.compile(r"^@@ -\d+(?:,\d+)? \+(\d+)(?:,(\d+))? @@")


def _branch_added_lines(ref, root, rel):
    """The 1-based line numbers of ``rel`` THIS BRANCH authored, vs ``ref``.

    Read off ``git diff -U0 <ref>...HEAD`` hunk headers, so the numbers are
    post-image — HEAD's version of the file, which R1's clean-tree refusal makes
    identical to the working tree the sweep is about to edit.

    THREE-DOT IS LOAD-BEARING. It diffs against the merge base, so mainline
    content merged INTO the branch is not in the diff and is correctly not
    attributed to the branch. That is the same asymmetry ``--exclude-path``
    exists for, decided per LINE here instead of per file.
    """
    diff = git("diff", "-U0", f"{ref}...HEAD", "--", rel, root=root, check=False)
    added = set()
...
def _bare_sweep_mask(ref, root, rel, index_surfaces):
    """Which lines of ``rel`` a BARE-NUMBER sweep may rewrite. None = all of them.

    THE ATTRIBUTION RULE, and the reason it is scoped to index surfaces.

    ``ADR-<old>`` is not a unique key at renumber time. A collision IS two records
    holding one number, and a collision is the only state in which this tool runs —
    so the sweep's key is ambiguous exactly when the sweep fires. R3's whole-file
    scoping resolves that ambiguity by PROVENANCE: a mainline-unchanged file citing
    ``<old>`` cites the other record, so it is out of scope. That argument runs one
    way only, and the module docstring already records the residual — a file the
    branch ALSO modified carries mainline prose, mainline citations included.

    On an ordinary branch file that residual is bounded and has an escape hatch
    (``--exclude-path``, whole-file, hand-verified). On an INDEX it is neither:
    an index's whole job is to enumerate other records BY NUMBER, so the foreign
    tokens are guaranteed present rather than incidental, and excluding the file
    wholesale would also stop updating the branch's OWN index rows — trading one
    silent corruption for a different one.

    So on an index surface the sweep is attributed per LINE instead: it may rewrite
    a bare number only where the branch itself authored the line. This is the
    "scope to the branch's own changed LINE RANGES" fix the module docstring names
    as durable, applied to the surface where the residual is not merely possible
    but structural. Path-exact rewriting (``rewrite_path_exact``) is unaffected and
    still runs everywhere: ``ADR-<old>-<slug>`` names one file and no other.
    """
...
def _foreign_token_count(body, old, mask):
    """``ADR-<old>`` tokens on lines the attribution mask excluded."""
    if mask is None:
        return 0
    return sum(len(citation_re(old).findall(ln))
               for i, ln in enumerate(body.split("\n"), start=1)
               if i not in mask)


def _adr_basenames(root):
    """Every ADR filename that exists, across both directories."""
    return {p.name for d in ADR_DIRS for p in (root / d).glob("ADR-*.md")}


ADR_FILE_REF_TEMPLATE = r"ADR-%03d-[A-Za-z0-9._-]*\.md"


def _unresolvable_new_refs(root, rel, new, known):
    """``ADR-<new>-*.md`` filename references in ``rel`` that name no real file.

    THE SECOND DETECTOR, and it asks a DIFFERENT QUESTION than R6's dangling scan.
    That scan asks "did any ``ADR-<old>`` citation survive?" — a question about the
    OLD number. A mis-attributed rewrite carries the NEW number on a FOREIGN slug,
    so it leaves no ``ADR-<old>`` token anywhere and the old-number scan is blind to
    it by construction, not by oversight. Existence is the discriminator that
    separates the two cases: a CORRECT rewrite names the file R2 just created, and
    a mis-attributed one names a file that was never there.

    Keyed on ``ADR-<new>`` rather than on every ADR reference, because ``<new>`` was
    free before this run (R1 refuses otherwise) — so any ``ADR-<new>``-shaped
    filename in a file this run WROTE is this run's own creation. That keeps a
    pre-existing broken link elsewhere in the corpus from reverting a correct move,
    which matters because this arm's verdict is a revert.

    Fenced blocks are skipped: an illustrative filename inside a code fence is not
    a reference the corpus resolves.
    """
    pattern = re.compile(ADR_FILE_REF_TEMPLATE % new)
    out, fenced = [], False
...
def do_renumber(old, new, ref, root, apply_changes, extra_paths, log,
    ...
    # The attribution mask's population, resolved ONCE and shared by all three
    # sweep call sites below (dry-run report, R3, R4). One derivation, so the
    # prediction and the two writers cannot disagree about which lines are ours.
    index_surfaces = _index_surfaces(root)
    ...
    if not apply_changes:
        ...
        hits = exempt_total = foreign_total = 0
        review_all = []
        for rel in in_scope:
            ...
            if not present:
                continue
            # THE SAME MASK THE APPLY PATH USES. Structural parity is this block's
            # whole contract, and a mask applied to R3 but not here would predict
            # rewriting the foreign entries R3 now correctly leaves alone — the
            # dry-run/apply divergence class ACT 14 exists to pin, reintroduced.
            mask = _bare_sweep_mask(ref, root, rel, index_surfaces)
            _, would, review = rewrite_citations(body, old, new, only_lines=mask)
            amb_tokens = sum(len(citation_re(old).findall(ln)) for _, ln in review)
            foreign = _foreign_token_count(body, old, mask)
            exempt = present - would - amb_tokens - foreign
            hits += would
            exempt_total += exempt
            ...
            # A FOURTH column, APPENDED so the first three keep their field
            # positions for existing readers. `not-ours` is reported separately
            # from `exempt (record)` rather than folded into it: the two are
            # different exclusions — one is a line that RECORDS a number on
            # purpose, the other is a line this branch never wrote — and a single
            # column would misname whichever it absorbed. Emitted only where the
            # mask applies, because on an unmasked file the count is not merely
            # zero, it is undefined.
            log(f"    would rewrite {would:>3} × ADR-{old:03d} in {rel}"
                f"  ·  exempt (record) {exempt}"
                f"  ·  REVIEW {len(review)}"
                + (f"  ·  not-ours {foreign}" if mask is not None else ""))
        ...
        log(f"DRY-RUN: {hits} citation(s) would move; {exempt_total} exempt "
            ...
            f"{foreign_total} bare ADR-{old:03d} token(s) on index lines this "
            f"branch did not author (left to the record that owns them).")
    ...
    for rel in in_scope:
        ...
        # THE OBSERVED DEFECT. R3 reaches index surfaces too — they are ordinary
        # members of the branch diff whenever the branch touched them — and on an
        # index a bare `ADR-<old>` is very often ANOTHER record's entry. Attributed
        # per line, the sweep rewrites only what this branch wrote; unattributed it
        # rewrote a neighbouring record's heading and `File:` link to a path that
        # does not exist, and every later step reported clean.
        mask = _bare_sweep_mask(ref, root, rel, index_surfaces)
        updated, n, review = rewrite_citations(body, old, new, only_lines=mask)
        foreign_left += _foreign_token_count(body, old, mask)
    ...
    # UNCONDITIONAL, zero included. This count is the only instrument that
    # distinguishes "the index carried no foreign entry" from "the attribution
    # mask is not running at all", and the second reads exactly like the first
    # if the line is emitted only when it is non-zero.
    log(f"R3 attribution: {foreign_left} bare ADR-{old:03d} token(s) on index "
        f"lines this branch did not author — left to the record that owns them")
    _log_review_block(log, review_all)
    ...
    for rel in _index_files(root):
        ...
        original = body
        present = len(citation_re(old).findall(body))
        # TWO MATCHERS, RUN IN ORDER OF CERTAINTY, and the scope test that used to
        # choose BETWEEN them is gone. It selected the bare-number sweep whenever
        # the branch had also modified the index — which is not a statement about
        # whether the tokens are ours, only about whether we touched the file.
        #
        #   (1) PATH-EXACT is an identity match: `ADR-<old>-<slug>` names this
        #       record's file and no other, so it is safe unconditionally and runs
        #       on every index surface whether or not the branch touched it.
        #   (2) BARE NUMBER is ambiguous by construction at renumber time, so it is
        #       attributed per line. A file the branch never touched yields an empty
        #       mask and the sweep is a no-op — the old `else` branch's behaviour,
        #       now falling out of the rule instead of being a separate case.
        body, _ = rewrite_path_exact(body, old, new, slug)
        mask = _bare_sweep_mask(ref, root, rel, index_surfaces)
        body, _, _ = rewrite_citations(body, old, new, only_lines=mask)
        # THE NOTICE ASKS ABOUT THE INDEX, NOT ABOUT THIS STEP'S EDIT COUNT.
        # Counting R4's own rewrites was the obvious predicate and it is wrong:
        # R3 sweeps index surfaces too, under the same mask, so on a record that
        # DOES have a branch-authored entry R3 has already moved it and R4 finds
        # nothing left to do — reporting "no entry" about an entry that is right
        # there. Asked as a POST-STATE question the answer does not depend on
        # which step performed the write. Evaluated before `append_renumber_log`
        # so the sentence that step appends cannot answer its own question.
        has_entry = (f"ADR-{new:03d}-{slug}" in body
                     or any(citation_re(new).search(ln)
                            for i, ln in enumerate(body.split("\n"), start=1)
                            if mask is None or i in mask))
        if not has_entry:
            # NAMED, NOT SILENT — and deliberately NOT a refusal. `core/ADRs/README.md`
            # states on its face that it is a curated thematic document which has
            # never enumerated the full record set, so a moved record having no entry
            # is the ORDINARY case and refusing on it would block most renumbers.
            # What makes the notice worth emitting is the second line: `<old>` present
            # with none of it ours is exactly the state in which the pre-fix tool
            # rewrote a neighbouring record, so the operator is told the number is
            # spoken for here and by whom it is not.
            log(f"R4 index: NO ENTRY for the moved record in {rel} — no entry "
                f"rewritten there (the § Renumber log append is separate)")
            if present:
                log(f"    ADR-{old:03d} appears {present}× in {rel} and every "
                    f"occurrence belongs to ANOTHER record; left untouched. If "
    ...
    for rel in sorted(PROJECTED_INDEXES):
        ...
        target = root / rel
        if rel in in_scope:
            # Masked like every other index surface. The managed region is
            # regenerated below regardless; this sweep is only for prose OUTSIDE
            # it, where a bare number is as ambiguous as it is anywhere else.
            body, n, _ = rewrite_citations(
                target.read_text(encoding="utf-8"), old, new,
                only_lines=_bare_sweep_mask(ref, root, rel, index_surfaces))
            if n:
                target.write_text(body, encoding="utf-8")
                touched.add(rel)
    ...
    for rel in in_scope:
        ...
        # THE SAME MASK R3 SWEPT UNDER. Without it this step reverts every correct
        # move on an index that carries a foreign entry: R3 deliberately left that
        # entry alone, so its ADR-<old> token survives — and a scan that reads a
        # survivor as a DANGLING citation would call R3's correctness a failure.
        # The rule is one rule: a line this branch did not author is not this
        # branch's citation, whether the question is "rewrite it?" or "did we miss
    ...
    if dangling:
        return revert(f"{len(dangling)} in-scope file(s) still cite ADR-{old:03d}: "
                      + ", ".join(dangling))
    # ---- R6b link-target existence (the OTHER direction) --------------------
    # The scan above asks whether any ADR-<old> citation SURVIVED. It cannot see a
    # mis-attributed rewrite, because that carries ADR-<new> on a foreign slug and
    # leaves no ADR-<old> token behind — the damage and the detector sit in
    # different number spaces. This arm asks the complementary question of every
    # file this run WROTE: does each ADR-<new>-shaped filename it now names
    # actually exist? A correct rewrite names the file R2 created; a mis-attributed
    # one names a file that never existed, which is a dangling link on a surface
    # the Dead-file-reference gate has registered as a required check.
    known = _adr_basenames(root)
    unresolvable = []
    for rel in sorted(touched):
        if not (root / rel).is_file() or not _is_utf8_text(root / rel):
            continue
        unresolvable.extend((rel, i, name)
                            for i, name in _unresolvable_new_refs(root, rel, new, known))
    # UNCONDITIONAL, zero included, for the same reason every other verdict in this
    # step is: a check that speaks only on failure is indistinguishable from a
    # check that never ran, and this one exists precisely because its predecessor
    # reported clean over a corpus it had corrupted.
    log(f"R6 link check: {len(unresolvable)} unresolvable ADR-{new:03d} file "
        f"reference(s) across {len(touched)} written file(s)")
    if unresolvable:
        for rel, lineno, name in unresolvable:
            log(f"    R6 link: {rel}:{lineno}: {name} names no ADR file")
        return revert(
            f"{len(unresolvable)} reference(s) to a nonexistent "
            f"ADR-{new:03d}-*.md were written by this run — a rewrite landed on a "
            f"record that is not this move's subject")
...
def self_test():
    ...
    eq("projected/strip-region", _strip_projected_region(fenced),
       ["prose cites ADR-004 outside the fence", "", "", "",
        "tail prose cites ADR-004 too"])
    # THE POSITIONAL ARM. The equality above already pins the blanking, but it
    # pins it as a literal, and a literal is what somebody edits when a future
    # change shortens the list again. This states the PROPERTY the two callers
    # actually depend on — same length, and the surviving prose still sits at its
    # original index — so it fails for the right reason rather than as a diff.
    eq("projected/strip-region-preserves-line-numbers",
       (len(_strip_projected_region(fenced)) == len(fenced),
        _strip_projected_region(fenced).index("tail prose cites ADR-004 too"),
        fenced.index("tail prose cites ADR-004 too")),
       (True, 4, 4))
