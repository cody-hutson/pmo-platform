...
| Version | **{{RELEASE_VERSION}}** — not pinned at plan time; bump class **minor**, identity **slug-only** per ADR-092. Rule-computed at Stage 12 per `claim-version.sh`, which resolves this token at the atomic claim. Sequenced behind `operational-folder-enforcement-and-migration` per **D-38**, so the next-free value is not determinable at planning time. **`v4.36` has since been claimed by milestone #346** (the concurrent release, merged to `main` while this branch was in flight) — this release therefore resolves to **next-free at its own Stage-12 claim** per **ADR-092**. No version is pinned here, and none is owed before Stage 12. |
