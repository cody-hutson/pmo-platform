...
**Governance blocker on (a), independent of all three:** `gate-efficacy-standard.md` conditions a flip on drain evidence, and no `NOT_FREE` verdict has ever been observed. Flipping would declare an enforcement the surface cannot deliver — which the same standard forbids, and which is that standard's own founding anti-pattern.
...
- [ ] **CIAC-5 — post-CAS coherence (#4194 × #4174).** The post-CAS region holds **one coherent ordered sequence** in which #4194's rebuild obligation and any #4174 hook both execute — neither silently bypasses the other, and neither sits on a path the other short-circuits. *Method:* read the region end-to-end; assert both obligations present, ordered, on the same control-flow path. Reproducible companion: the rebuild grep returns non-zero, against a pre-change control of zero.
