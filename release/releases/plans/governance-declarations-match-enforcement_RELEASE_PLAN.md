...
| #1772 | AC-2 | `grep -A4 'Bridge Files (Layer 3)' core/CLAUDE.md.template` shows the pointer to § Session Management · control arm, same instrument, same target: `grep -c 'Session Management'` → non-zero | The pointer is present and resolves to post-#5518 text, which states the session-surface write rule |
...
### Executor dispatch coverage — which rows `verify-release-plan.sh` can run, and which it cannot

Stated so the executor's exit status is read correctly rather than as a silent failure. The executor's per-issue family dispatches **content assertions** (`grep` / `test -f`); it has **no family for a `git diff` scope assertion**, so three rows below classify as `unclassified` and are dispatched **ERROR**. That verdict is the tool being honest — it declines to grade a row it cannot interpret rather than returning a fabricated PASS — and it is the reason the executor exits non-zero on this plan even when every check it *can* run passes.

| Row | Executor verdict | Who runs it |
|---|---|---|
| #5518 AC-1 … AC-4 | dispatched, **PASS** | executor (`per-issue` family) |
| #1772 AC-1, AC-2 | dispatched | executor (`per-issue` family) |
| #1772 AC-3 | dispatched | executor |
| **#5518 AC-5** · **#1772 AC-4** | **`unclassified` → ERROR** | **Reviewer / Stage 7.** Both are `git diff` scope assertions — the exact class the executor has no handler for. They are load-bearing (AC-5 confines the blast radius; AC-4's `:(top)` prefix is the guard against a cwd-relative false pass) and are run manually with their control arms |
| **#1772 AC-5** | **`unclassified` → ERROR** | **Reviewer.** The Outcome Statement lives on the milestone, not in a file, so there is no path for a content assertion to target. Verify-only — already satisfied at the first Stage-4 gate |

**Do not "fix" these by inserting a classifier keyword into the method text.** The executor's own classifier carries a documented incident where a runnable method that merely *said* `exercise` was routed to a family that returned PASS having executed nothing. A method reworded to match a keyword it does not honestly belong to buys a green row and loses the check. The gap is routed forward as a tool finding instead (§ Findings routed forward F-5).
...
| **F-5** | `release/tools/verify-release-plan.sh` has **no check family for a `git diff` scope assertion**. Scope-confinement ACs — "edits are confined to file X", "path Y is untouched" — are a recurring and load-bearing AC shape (both cards in this release carry one), and every one of them lands in `unclassified` → ERROR. The executor therefore cannot reach a clean exit on any plan whose ACs assert scope, and the honest ERROR is indistinguishable at the exit code from a real failure | Tool-internal gap, unrelated to either card's subject matter; fixing it here would be unrequested scope | Intake, proposal tier. A `scope-diff` family dispatching `git diff --name-only <base>...<head> -- <pathspec>` against an expected-empty / expected-non-empty predicate would close it |
| **F-6** | The shipped version-less exemplar [`declarations-have-a-firing-surface_RELEASE_PLAN.md`](/release/releases/plans/declarations-have-a-firing-surface_RELEASE_PLAN.md) contains the literal release-version placeholder **twice** — in both cases inside a sentence asserting the plan emits none. `_prov_elements_present` scans for that placeholder as a **literal substring**, so it emits `release-version-stamp` for a plan that has no stamp manifest: the plan's own § V-3 claim that the element is "correctly absent" is falsified by its own prose. Squarely this milestone's theme — a declaration that does not match what the surface enforces | A different release's shipped artifact; editing it is outside this release's File Change Matrix | Intake. Either the detector should ignore placeholder mentions inside a negation, or version-less plans should name the token without emitting it (the form this plan adopts) |
