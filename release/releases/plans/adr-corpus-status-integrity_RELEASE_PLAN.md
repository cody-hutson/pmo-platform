...
- **The release-corpus home adapter successor** — **ROUTED to epic #1184 (PORT-Adapters), NOT created.** Two independent bars, either sufficient: a Stage-6 spoke may mutate no issue but its own sub-task, and this milestone is composition-locked at Stage-4 Planning entry (`release-process.md` § A7 — `issues_added` MUST be **0** for every refresh disposition, with no exception path), so the hub cannot add it here either. The child is created on #1184, outside this milestone. The spec it should carry, verified live rather than transcribed:
...
| #5823 | AC-3 | **Corrected, and re-classified NOT SATISFIABLE IN THIS RELEASE — it must not be graded met.** The original row carried the release-notes probe, one criterion off. AC-3 requires an open child of epic #1184 whose title names the release corpus. Two independent bars, either sufficient: a Stage-6 spoke may mutate no issue but its own sub-task, and this milestone is composition-locked at Stage-4 Planning entry (`release-process.md` § A7 — `issues_added` MUST be 0 for every refresh disposition, no exception path), so the hub cannot add it here either. What this card delivers is the ROUTED SPEC. Method, two parts: `grep -n "corpus_home" release/releases/plans/adr-corpus-status-integrity_RELEASE_PLAN.md` reading the `### Exclusions` entry; then the card's own predicate re-run after creation, `gh api repos/{owner}/{repo}/issues/1184/sub_issues` | Routed spec present, naming the selector, its config surface, the hardcoded Check-33 consumer and the still-owed location decision. The sub-issue limb is an **open post-release obligation on the hub/operator, recorded UNMET**. **Control:** the same `gh api` call run at build time returns the current open-child set with no release-corpus title, so a later non-zero is a creation rather than an empty read |
...
| #5823 | AC-6 | **Corrected — the original row grepped the #5823 card body, the wrong artifact entirely; a body merely naming the disposition would pass while the metadata stayed `completed`.** AC-6 grades #1549's close reason. Method: `gh api repos/{owner}/{repo}/issues/1549 --jq .state_reason` — the porcelain `gh issue view` routes through GraphQL and is unavailable at this quota | `not_planned`. **Observed at build time: `completed`, alongside a `status: rejected` label — the mislabel is live.** A Stage-6 spoke may mutate no issue but its own sub-task, so this is **routed to the hub/operator and recorded UNMET**, not graded met. **Control:** the same call against #2427, closed NOT_PLANNED when the migration was withdrawn, returns `not_planned` — proving the field is read and its value discriminates rather than every issue returning one string |
