...
| Edge | Type | Evidence |
|---|---|---|
| **E1** `#6172 → #6167` | compositional, ordering-critical | #6172's parse-only exemption belongs in the flag walk. #6167 widens the verb classifier at `block-destructive.sh:1477` and `:1491` and the operand domain at `:491`. Landing the widening first forces the exemption to be retrofitted across a larger interpreter set instead of inherited from the walk. |
...
**Wave 4 — #6167 (re-scoped).** Verb classifier at `:1477` / `:1491` + `script_operand_implicated()`, plus the new per-rule phase constant. Lands **after** both narrowings (E1, E2) and after the corrected guidance (E4). Last of the -022 cluster by construction.
...
| **#6167** | verb classifier (`:1477`, `:1491`) + `script_operand_implicated()` | 476–501, 1477, 1491 | **adjacent to #6172** |
| **#6229** | `script_qnext()` / `script_qadvance()`, `script_qbail` heredoc branch, segment splitter | 784–889, 1160–1162, ~1269 | none |
| **#6166** | `check_script_target()` message strings | 597, 603, 609 | none |
...
# ADD + EDIT — #5653 (Wave 1 AC1, Wave 5 AC4)
core/ADRs/ADR-NNN-matcher-vs-parser.md                       ADD    #5653 — AC1: records the widened-shared-matcher decision (number allocated at authoring)
core/hooks/tests/block-destructive.test.sh                   EDIT   #5653 — AC4 control arm
core/hooks/tests/block-egress.test.sh                        EDIT   #5653 — AC4 control arm
core/hooks/tests/block-fs-boundary.test.sh                   EDIT   #5653 — AC4 control arm
core/hooks/tests/block-rm-prefer-trash.test.sh               EDIT   #5653 — AC4 control arm
...
# EDIT — #6166 (Wave 3)
core/hooks/block-destructive.sh                              EDIT   #6166 — check_script_target() hint strings at :597, :603, :609 (hint text only; no predicate moves)
...
# EDIT — #6167 (Wave 4)
core/hooks/block-destructive.sh                              EDIT   #6167 — verb classifier at :1477 / :1491 + script_operand_implicated(); new per-rule phase constant
core/hooks/tests/block-destructive.test.sh                   EDIT   #6167 — widened-interpreter arms + phase-constant arm
core/rules/bypass-mode-readiness/block-destructive.md        EDIT   #6167 — declared scope follows the widened classifier
...
# EDIT — #6229 (Wave 2)
core/hooks/block-destructive.sh                              EDIT   #6229 — script_qnext() / script_qadvance(), script_qbail heredoc branch, segment splitter
core/hooks/tests/block-destructive.test.sh                   EDIT   #6229 — heredoc-suppression arms
core/rules/bypass-mode-readiness/block-destructive.md        EDIT   #6229 — heredoc-modelling residual recorded
...
|---|---|---|---|---|
| #6172 | AC-1 | runtime | Run the hook suite from the materialized CI layout and assert the four relative spellings of `core/deploy/tools/check-convention.sh` and `core/deploy/tests/test_check49_mode_identifier_unification.sh` return exit 0. Control arm that must fire: `bash core/deploy/tools/no-such-tool-xyz.sh` returns exit 2 with `BLOCK-DESTRUCTIVE-022` | Four spellings ALLOW; control arm observed BLOCK, so a green AC-1 cannot be a dead allowlist |
...
| #6172 | AC-3 | runtime | Assert `bash -n <non-allowlisted>.sh` returns exit 0. Sensitivity arm that must fire: the same path without `-n` returns exit 2 + `BLOCK-DESTRUCTIVE-022` | Subject ALLOW and sensitivity arm BLOCK, both observed; a green subject without a red arm is a blanket allow |
...
| #6172 | AC-5 | runtime | Executing-form fixture table: `bash <p>`, `bash -x <p>`, `bash -s <p>`, `bash -- <p>`, `bash -c 'echo hi' -n <p>`, `bash -n -c 'echo hi' <p>` each return exit 2 + `BLOCK-DESTRUCTIVE-022`; control arm `bash -n <p>` returns exit 0 in the same run | Six executing forms BLOCK, control ALLOW — the table discriminates rather than blanket-blocking |
| #6172 | AC-6 | runtime | Paired mutation arm: copy the hook to a temp path, revert the exemption, re-run AC-3's subject. Assert PASS on the shipped hook and FAIL on the mutant | Both verdicts recorded; a mutation leaving AC-3 green is inert and fails this criterion |
...
| #5555 | AC-all | runtime | Run `core/hooks/tests/block-fs-boundary.test.sh` from the materialized layout; assert unresolvable/variable-bearing operands across the measured nine-verb set no longer emit BLOCK-FS-BOUNDARY-003, with a must-fire control arm on a genuinely out-of-scope write | Nine-verb set no longer false-positives; control arm observed non-zero refusal |
| #5653 | AC1 | content | file-state check: the ADR exists under `core/ADRs/`, carries the canonical section headers, and records the widened-shared-matcher decision | ADR present and schema-conformant |
| #5653 | AC4 | runtime | Control arm across the four anchor-carrying hook suites, run after #5555 lands (E5): legitimate non-destructive commands are not newly blocked | Four suites green; no newly-blocked legitimate command |
| #6115 | AC-all | runtime | Positive reproduction first (the obvious probe fail-opens from a non-repo cwd), then `core/hooks/tests/block-draft-files.test.sh` from the materialized layout; assert AC3's `PMO_SCOPE_GUARD_ROOT` narrowing is not introduced | Reproduction observed before the fix and absent after; suite green; no env-narrowing introduced |
| #6166 | AC1,AC3 | content | Hint-ordering grep at `block-destructive.sh:597/:603/:609` asserting the allowlisted-relative retry is named first; plus a zero-line allowlist diff for this card's own commit | Relative retry first in all three strings; allowlist diff zero lines for #6166 |
| #6167 | AC-all | runtime | Widened-classifier arms plus CIAC-4's phase-constant assertion; every newly-admitted interpreter resolves through the per-interpreter parse-only table (INT-2) | Widened set adjudicated; phase gate is a per-rule `readonly` constant; no interpreter silently exempted |
| #6229 | AC-all | runtime | Heredoc-suppression arms: a report quoting a script invocation inside a heredoc is no longer refused; control arm asserts a genuine destructive command inside a heredoc still blocks | False positive removed; control arm observed non-zero block |
