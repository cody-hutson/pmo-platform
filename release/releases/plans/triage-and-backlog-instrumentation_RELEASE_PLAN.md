...
**Landed-state correction — the extraction card (#4901).** Its row in the Change Description's *Issues resolved* table still reads `pending`; it has **landed**. The deliverable as shipped is wider than that row states: the gate logic is extracted to an invocable checker with a crossed fixture suite, the four SIGPIPE sites are rewritten during the lift rather than exempted, and the workflow job is reduced to a thin caller. The correction is recorded here rather than in the row itself because the row **cannot be edited**: it leads with a bare issue reference, and the reference-durability hook evaluates the incoming edit fragment with its in-block state machine restarting at fragment line 1, so any fragment touching that row scores `OUTSIDE-BLOCK` even though this header sits above it in the file. The compliant expansion — growing the fragment until it contains this header — would mean rewriting the 260-odd lines in between, which is a destructive rewrite of other cards' content, not a surgical edit. No bypass was set and no scheme was invented. **The same block affects the sibling rows still reading `pending` whose work has also landed; flipping the column is a single-pass job for the Stage-9 gate, where the whole table can be rewritten as one fragment.**
...
**File-surface reconciliation — the tiering card.** Its fourth file is `core/deploy/tools/README.md`, whose matrix row is claimed by the README-coverage card sequenced earlier. That tool **already holds a row**, so under the coverage rule — a new leg on an existing tool is an in-place cell edit, never a new row — this card **edits the existing row's `Purpose` cell in place**: `| core/deploy/tools/README.md | MODIFY — the existing work-hierarchy row's Purpose cell extended in place to name the third invariant, its advisory exit contract and its exit-3-not-zero rule for a field-deficient node set; `Used by` and `Mode(s)` unchanged (the leg adds no flag and no consumer); never a second row |`. **The release path count is unchanged at 17** — that path was already in the machine-readable list. Two earlier characterizations are **superseded for this card only** and are corrected here rather than silently overwritten where they land: the Implementation Sequence row calls its README work "a README append", and the Contention Map classes the four-way README contention as append-pattern. Both are correct for the extraction card, which adds a genuinely new basename; they are **not** correct for this card or for the detection-leg card, whose tools each already hold a row. Verified on the merged surface: 32 rows, 32 distinct basenames, 0 duplicates, exactly one row keyed to the work-hierarchy tool, and zero rows for the standalone tiering check this release decided must not exist. **`deploy.sh` region, post-edit:** the matrix and Contention Map state this card's region as lines 9534–9608, which were the planning-time bounds of the Check 55 block. Every hunk landed at old-side lines 9536–9604 — strictly inside them and strictly above the Check 56 boundary the detection-leg card owns — and the block now ends at 9663, moving Check 56 to 9664 and Check 57 to 9904. The load-bearing constraints hold unchanged: Check 55's block only, no new check number, and no overlap with the detection-leg card's region.

**Verification expectation — the tiering card (supersedes the Verification Plan row's summary form).** The row states the method; these are the observables it must produce, because a "does not flag a genuine leaf epic" phrasing carries an *implied* zero and an implied single invocation. **Live:** the lint reports exactly the **two** known-open containers out of a **42**-issue open-epic denominator with **zero** false positives, each row carrying its own evidence (the matched slug tokens and the in-family references). **Fixture form:** the same two reported, and **exactly zero** rows for each of seven near-miss classes, every one of which differs from a true positive in exactly one conjunct. **Anti-vacuity control:** a node set lacking the leg's field triple **exits 3** and emits **no** count row — a fixture written for the older leg leaves this leg's population structurally empty, so a zero there would pass for the wrong reason. **Skip semantics:** the offline path emits an explicit skip row and **no** count row, so an unevaluated leg is never read as a clean one. **Exemption:** an entry for one container suppresses that one and leaves the other still reported. **Advisory contract:** `deploy.sh --check` exit code is unchanged by these findings in both warn and enforce mode, and the leg makes no write call.
...
- [ ] **CIAC-4 (#4899 × #4926 on `deploy.sh --check`):** both extended check legs are reachable from `deploy.sh --check` and each **emits a finding on a purpose-built fixture** and **emits none on a conformant control** — the conjunction, never one arm. *(Re-worded at the Stage-5 batch-2 gate: both legs are deliberately advisory and never move the exit code, so "fails" was unsatisfiable by a correct implementation. "Emits a finding" is the observable an advisory and an enforcing leg both produce, and matches the milestone's own Success Indicator.)* *Method:* two routes, because no single invocation carries both claims. **Reachability:** read each leg's `deploy.sh --check` region and confirm it invokes that leg's primitive — Check 56 (`check-milestone-epic-membership.py`, leg M4) and Check 55 (`check-work-hierarchy.py`, leg H3). **The two arms:** run each primitive's own `--self-test`, the committed two-armed fixture harness, and assert on each that the must-flag arm is non-zero and the must-not-flag arm is exactly zero. That is the route Stage 7 and Stage 8 actually used to grade both legs, and CI re-runs it in the `Discovered tool self-tests` job — both primitives are listed in `selftest-coverage-manifest.txt`. *(Method re-worded at the Stage-9 gate: `deploy.sh` accepts no fixture path — `--check` takes only an optional `--warn`, and no other flag takes one either — so the prior wording, "run `deploy.sh --check` against both fixtures and both controls", named no executable route. Checks 55 and 56 also do not yet surface a `CTRL:` self-test line through `--check` the way Checks 63, 64, 66, 67 and 68 do, so the arms — which are committed and real — are not observable through the wrapper, and the method names the primitives directly instead. The criterion's substance is unchanged: each leg must still emit a finding on a purpose-built fixture and emit none on a conformant control, the conjunction and never one arm.)* *Graded at Stage 9 QC3.5.*
