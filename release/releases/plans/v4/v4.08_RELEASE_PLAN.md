...
**Version:** `v4.08` *provisional-display* · **Bump-class:** `minor`
...
> **Version binding.** Every version reference in this file is the `v4.08` token, never a literal. The concrete number is unknown until the Stage-12 Phase B3 atomic ref-CAS. Per `release/governance/RELEASE_PROTOCOL.md` § Versioning Phase 1, *the provisional-display version is a label, not a reservation*.
...
2. **The version slot is contested and that is a designed-for outcome, not an incident.** Several in-flight siblings resolve to the same next-free slot. Under ADR-092 a lost race is a `v4.08` re-stamp, not a rename cascade. Do not hedge by pre-claiming.
...
**Bump-class:** `minor`. The concrete number is `v4.08` and binds only at the Stage-12 Phase B3 atomic ref-CAS.
...
**Live-risk note:** with several siblings resolving to the same slot, a rung (1) or (3) fire on this release is a realistic, expected, cheap outcome — a `v4.08` re-stamp, not a rename cascade. Budget for it; do not treat it as an incident.
...
|---|---|---|---|---|
| **R1** | A sibling merges first and takes the slot. | HIGH likelihood / LOW impact | None pre-emptive. ADR-092 makes the loss a `v4.08` re-stamp. Rungs (1)/(3) catch it. Do **not** pre-claim. | CHEAP |
...
- [ ] **CIAC-4 — provisional-vs-bound vocabulary (#4174 × #4199 × #3828).** No surface this release authors describes a **pre-claim** version as `bound`, `claimed`, or `authoritative`; each pre-claim reference reads `provisional` or carries `v4.08`. *Method:* diff-scan for those adjectives; for each hit assert it describes a post-CAS state. Live control: `provisional` on the same diff returns non-zero.
...
**Rollback is NOT the mitigation for a lost version race.** A lost slot is handled by re-stamping `v4.08` per ADR-092, never by reverting the release.
