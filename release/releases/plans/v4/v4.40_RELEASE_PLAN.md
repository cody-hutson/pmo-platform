...
Populated at Stage 13 Close (2026-08-28), after the release merge at `0fbf275957f854a23411644879fc7b20c2dd073e` and the `v4.40` tag.

**How to read this section — and what it deliberately does not contain.** The per-card Stage-6 / Stage-7 / Stage-8 two-arm evidence for this release lives on the per-card sub-tasks, where each stage recorded it at the time it was measured: Stage-8 acceptance verdicts on #6081 (#4732), #6085 (#4719), #6089 (#4923), #6093 (#4978), #6097 (#5523), #6101 (#5816) and #6105 (#5069); the Stage-9 GO on #6106; Stage-12 execution on #6109. It is **not transcribed here.** Re-authoring measured arms at close from secondhand summaries would produce figures that read as measurements while being restatements, which is the defect class this release exists to close. What follows is the evidence Stage 13 itself produced, each item run at close rather than inherited.

**Close-class resolution (Phase A7.5).** `DEPLOYABLE`, resolved via **rung 1** — the release plan's `domain_practice.domain` declares `governance`, a guide-backed deliverable class and not the literal `task-artifact` that routes the artifact-acceptance branch. Not a fall-through: rung 1 resolved a declared value. The Artifact-Acceptance Record is therefore **ABSENT** (explicit no-op) — this release declares no task-artifact deliverable, so no empty block is written and no ceremony is performed.

**Phase A1 — issue closure audit.** The release PR #6120 carried **zero** `closingIssuesReferences`, so no member auto-closed on merge. This is the plan's own design, not an anomaly: § Delivery Strategy states per-issue closure is a Stage-13 action and that no close-family keyword goes in an Engineering commit or the release PR body. Milestone #350 held **17** open issues at close entry — 7 delivery cards, 7 Stage-8 sub-tasks, and 3 release-scoped sub-tasks. The 9 hub-spoke stage sub-tasks were closed first as close-out housekeeping with per-thread stage-completion records; the 7 delivery cards were closed through the close-out's D-1 path with per-issue comment overrides stating the by-design basis rather than the generic auto-close-anomaly default.

**Phase A2 — deferred-item disposition.** No stragglers. The two detached members (#4768, #5284) were dispositioned at Mode O entry — milestone removed, terminal `status: deferred` retained — and #5229 was closed `NOT_PLANNED` as subsumed. All 7 locked members delivered, so the enumeration of bundled-but-not-closed is empty. Recorded rather than omitted: the velocity producer's own stderr notes that its planned-recovery limb returns #5284 to `planned`, which is why the Velocity field reads `planned 28` against the plan's locked raw 24.

**Phase A4 — QC4-05 invariant re-verification.** **N/A on an empty declared population.** This plan declares **zero** `AV-N` invariants (denominator stated: the token `AV-` occurs 0 times in this file). No per-AV verdict is manufactured to fill the beat.

**Phase A4.5 — native-dep drift parity.** No drift findings across the release-scoped issues. Informational at this stage either way.

**Phase A5 — G-CL6 design-artifact refresh gate.** **N/A on an empty declared population.** This plan declares no *Tier-A activated design artifacts* section (denominator stated: the token `Tier-A` occurs 0 times in this file). Warn-mode and specified-not-yet-emitting regardless.

**Phase A8 — close-out completeness (output set).** `assert_output_set` PASS before the chore commit, over a 3-member manifest: `velocity-field` (required) PRESENT · `learnings-block` (required) PRESENT · `close-class-telemetry` (required-if telemetry-cutover-armed) N/A, predicate false. All release-corpus surfaces landed in one chore PR: `RELEASE_LOG` `DEPLOYED → VERIFIED`, `RELEASE_INDEX` row, `RELEASE_DIGEST` entry, `notes/v4.40_RELEASE_NOTES.md` (flat, per the notes standard), `CHANGELOG.md` `## [v4.40]`, and `.version` stamped `v4.39 → v4.40`. `assert_anchor_hygiene` PASS — 182 annotated tags, tag-to-Release sets equal modulo 4 recorded exemptions, INDEX 189 rows == LOG 189 rows.

**Phase A8.1 — §3.2 note-content close gate.** PASS, clean for v4.40, on both the in-script phase and a direct `lint_release_corpus.py --check note-content` run. **Both arms run:** the specificity arm is the clean exit-0 over the whole notes corpus with zero findings naming v4.40; the sensitivity arm injected a §2.4 banned-jargon term into this release's own note and the lint exited **1** naming this version's note file and the matched term, after which the injection was reverted and the clean result reproduced. The pass is therefore measured, not vacuous.

**§2.8 sample-block advisory.** This release **is** schema/format-changing on five independent grounds — a new `decision-superseded` enum value, a new `integrity_cutover` schema key, the closed three-form `domain_practice source:` grammar, the new `#### 11.8.1` machine-parsed registry sub-table, and the widened evidence-shape structure in both issue templates. The note carries a fenced sample block outside the Section 6a bullet span (the § 11.8.1 registry row). `lint_release_corpus.py --check note-content --sample-block-advisory v4.40` emits no advisory finding.

**Phase A8.2 — Check 61 decision-emission (advisory).** Verdict **INCOMPLETE** corpus-wide: 13 missing-class findings across 41 checked releases. **None names v4.40** — the zero is measured, with the 13 findings serving as the sensitivity arm that the check can report. Pre-existing findings for other versions do not block this close (audit-baseline discipline), and the gate is warn-mode: the committed `.github/decision-emission.enforce` token is not `enforce`. The remedy for the 13 is to emit the missing rows per Procedure 4a, never to waive them.

**Phase A8.3 — ADR-092 plan-identity gate.** PASS — filename and nested home agree with the `RELEASE_LOG` row for v4.40.

**Phase A10 — QC4-06 goal attainment. Verdict: ATTAINED.**

The Outcome Statement's AFTER reads *"Pipeline stage specs agree with each other, with the tools that implement them, and with the event vocabularies they declare,"* against a BEFORE naming seven disagreeing surfaces. Post-deploy `main` exhibits that state on all seven clauses, each with a verifiable anchor: Surface-1 emit ownership now states Stage 12 identically across the stage-12 spec, the stage-13 spec, the release-notes standard, the close-out tool and the release process, with the Stage-13 phase re-framed as an idempotent backstop that records a `SURFACE1-STATE` witness token; the Override-Record obligation carries a single normative statement with both arms worked (`stage-08-qa-testing.md` § PARTIAL deferral criteria, plus a stated falsification pair); the release-class trigger-(b) set is a closed 6-member set with the ledger removed; the G1-03 predicate admits a second, Evidence-section-scoped evidence shape; `verify-release-plan.sh` carries an always-on `provenance-survival` family with a mandatory absolute-presence limb; the two QC4 subtypes now resolve a declared payload vocabulary through the `#### 11.8.1` registry, verified live in the shipped schema; and `check-event-record-integrity.sh` ships with `integrity_cutover` bound at the merge instant, which is what makes its verdicts discriminate rather than pass on every input.

**The one clause a Stage-8 residual questioned was re-measured at close rather than inherited.** `stage-08-qa-testing.md`'s claim that every other mention of the obligation *cites* rather than restates was tested by enumeration over 1,279 tracked `.md` / `.yml` files: 59 mentions across 25 files, of which the live non-ledger stage-spec and skill surfaces the claim names — `stage-07-dev-testing.md`, `stage-09-plan-review.md` (2 sites), `release-personas.md`, and `qa-acceptance-report-template.md` — **all cite** `stage-08-qa-testing.md § Step 0` rather than restating the trigger, and `OPERATIONS.md`'s occurrence is the known RAG-recolor homonym, not this obligation. Both arms run: the enumeration returned 59 hits (sensitivity) and a deliberately misspelled control token returned 0 (specificity). The residual's precision concerns stand as known limits and are carried in the release note; they do not falsify the clause.

Verdict **ATTAINED** rather than PARTIALLY-ATTAINED because every AFTER clause has a delivered mechanism verifiable on `main`; the open residuals are precision and coverage defects recorded as known limits and follow-ups, not undelivered outcome.

**Phase A11 — G-CL8 documentation-impact resolution (warn-mode).** Presence limb **7/7 MET** — every delivery card carries a `Documentation Impact` value; absence of a value is the gate-failing state and no card has one. Resolution limb, measured against the 61 files the release merge changed: #4719 1/1 named doc modified · #4732 4/4 · #4923 1/1 · #5523 explicit-none form with rationale · #4978, #5069 and #5816 carry narrative declarations naming no file, so their resolution limb is **present but not mechanically evaluable** and is recorded as such rather than scored as a pass on an unmeasured limb. Zero FAIL. Both arms run — a known-changed path resolves in the diff set, a fabricated path does not.

**Phase A12 — audit-recommendation badge update.** No-op. No operator-local `analysis/<audit>/recommendations.md` is touched by this release's closing issues; the `analysis/` tree is git-ignored operator-instance content and this beat is non-blocking.

**G-CL9 — promised ADR ratification flips.** **6/6 landed.** ADR-143 through ADR-148 each shipped carrying a conditional ratification whose named ratifying review is the review this release closed; each now reads `Accepted` with provenance naming the v4.40 close gate, in both the frontmatter `status:` field and the `## Status` section body. The other 28 `Proposed` records in the ADR corpus are untouched — their ratifying reviews are elsewhere or still in flight, which the criterion states is correct and not a failure. The flips drifted the generated `release/ADRs/README.md` Status column and the ADR-number integrity gate **caught it**, failing with six DRIFT rows; re-projected with the gate's own remedy, after which `--verify` exits 0 with zero DRIFT / MISSING / ORPHAN rows and the ADR-number check reports 150 records contiguous `001..150`.

**Two ledger fields were reconciled at close rather than left carrying stale claims.**

- **Cycle-Time.** The field read `T_GO=absent`. That was true when Stage 12 wrote it and is false now: the `gate-outcome/plan-review-go` row exists at `2026-08-28T17:44:55Z` but was back-emitted **after** the deploy at `2026-08-28T17:11:56Z`, so the anchors are inverted and `compute-cycle-time.sh` returns **-32m** while flagging a pipeline-event-log integrity condition. A negative interval is not a cycle time. The field now records the inversion and no value; **Phase A6 appends nothing** to the cycle-time baseline, per that file's own maintenance rule that `N/A` releases do not shift the rolling-3 window. The Stage-9 gate itself was rendered at `2026-08-28T16:43:35Z` as a `decision` / `d-class` row, which is not a T_GO anchor and is recorded as context, not as the measurement.
- **Close-Class-Telemetry.** The field first emitted `retro-conformance N/A — no retro register found` against a register that **exists** at the path the register producer's own `--print-path` returns. Cause: the close-out's phase 6.8 invokes `compute-close-class-telemetry.sh` without `--retro`, and that tool implements **no** path-by-convention fallback despite its help text documenting one — its retro path is populated only by the flag. Stage-13 Phase A7.2 assigns the spoke that argument; the script path does not carry it, so a script-driven close silently degrades Indicators 1, 2 and 5. Re-run with the register path and the field corrected to `retro-conformance 10/10 (1.00) … rollup-presence present`. Both arms run: the real path resolves 10/10, a nonexistent path reproduces the N/A line byte-for-byte, so the difference is attributable to the register and to nothing else.

**Phase A7 sub-threshold disposition (signal-only).** The pattern-detect scan over a 5-version window found **0** qualifying clusters and **4** near-threshold (span ≥ 2 versions, below the cluster minimum): `gate` and `reproduced` in the surprise field, `consumes` and `gate` in would-change. They gate nothing and file nothing; their terminal disposition belongs in the operator-authored §2.3 / §2.4 rows of the close-out learnings register, which the producer deliberately does not machine-fill.

**Plan-conformance re-run at close, with a baseline control.** `verify-release-plan.sh` on this file returns exit 3 — `4 PASS / 9 FAIL / 5 SKIP / 11 ERROR`. **The identical roll-up reproduces against this plan as merged on `origin/main`**, before the Verification Evidence section was appended, so none of it is attributable to this amendment; the control was run rather than assumed. The 9 `fcm-delivery` FAILs are all `declared-add-not-delivered:release/ADRs/ADR-*.md` — this plan declares its six ADR additions as a **glob** while the delivery check compares **literal** paths, so the six records that did land read as undelivered. The 11 ERRORs are `unclassified-method (no family match)` over the § Verification Plan table's free-text Method cells. Both are pre-existing properties of the plan-vs-checker fit, not undelivered scope: the six ADRs are present on `main` and the ADR-number check reports the corpus contiguous. Recorded here rather than repaired, because widening the FCM glob or rewriting the Method cells at close would edit the scope surface Stage 9 already graded. This sits alongside the § Verification Plan caveat for the checker's `operations/` allowlist gap.

**One family did pass, and it is this release's own.** The `provenance-survival` checks #4978 delivered run green on this very file: `PROV-PRESENCE` PASS (one conformant label), `PROV-GRAMMAR` PASS (`form=X`, `date=2026-08-24`), `PROV-COVERAGE` PASS, and `PROV-DELTA` **SKIP** — explicitly *"does NOT pass by default"*, because no Stage-4 comment surface was supplied to compare against. That SKIP is the designed behaviour ADR-147 records, and it is the reason the absolute-presence limb was made mandatory.
