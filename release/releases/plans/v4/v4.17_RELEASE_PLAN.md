...
**Version identity:** **slug-only** per **ADR-092**. Through the pipeline this file was `102-specialist-role-coverage_RELEASE_PLAN.md` and the branch was `release/102-specialist-role-coverage`; no version stem appeared in the plan filename, the branch name, or this plan's identity prose. Bump class is `minor` — a capability release, not corrective, so the patch floor does not apply. The concrete number bound at the **Stage-12 atomic claim** as **`v4.17`**. The rename into the major-version bucket did **not** fire at that claim — this plan carries zero `{{RELEASE_VERSION}}` tokens, so `--stamp-slug` had nothing to resolve — so it was performed by hand in the Stage-13 close-out chore PR, which is how this file now resolves to `release/releases/plans/v4/v4.17_RELEASE_PLAN.md`. The upstream cause is tracked in #5063.
