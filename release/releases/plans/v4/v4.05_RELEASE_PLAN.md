...
| 7 | **#4196** count-vs-structure lint (`deploy.sh` Check 63) | story | M | prevent | `core/deploy/deploy.sh`, `count-structure-baseline.txt` (new), release-hub skill files |
| 8 | **#4197** readiness-map TH-3 invariant (`deploy.sh` Check 64) | task | S | prevent | `readiness-map-template.md`, `core/deploy/deploy.sh`, `packages/release-hub.skill` |
...
| 7 | **#4196** | `deploy.sh` Check 63 + committed baseline. Highest design risk in the release. |
| 8 | **#4197** | `deploy.sh` Check 64 + TH-3. Adjacent to #4196 so **one** `release-hub` package rebuild serves both (R-3). |
...
CONTENTION (not dependency):
  ...
  #4196 ∥ #4197   → core/deploy/deploy.sh (slots 63 / 64) AND packages/release-hub.skill (BINARY whole-file)
```
...
7. **#4196** — `deploy.sh` **Check 63** (count-vs-structure lint), **enforce-mode narrowly scoped**, with a committed **73-entry baseline** at `core/deploy/allowlists/count-structure-baseline.txt`. Both a `# Check 63` def-block and a `log "Check 63:"` emitter, per the Check-57 roster contract.
8. **#4197** — `readiness-map-template.md` TH-3 invariant + `deploy.sh` **Check 64** + the mandatory `build-skill-packages.sh release-hub` rebuild (serves #4196 and #4197 together).
...
| **`core/deploy/deploy.sh`** | **#4196 × #4197** | append-pattern, distinct slots | **63 → #4196**, **64 → #4197** (rule-computed at Collective Review). Both append at the check-roster tail; neither edits an existing check body. Each needs **both** a `# Check NN` def-block and a `log "Check NN:"` emitter (Check-57 roster contract). |
...
|---|---|---|---|---|
| **R-1** | **`core/deploy/deploy.sh` cross-PR contention.** The highest-traffic file in the repo; sibling releases edit existing check bodies. | Engineering (#4196, #4197) | Allocate slots **63** and **64** (**63** verified free at the Commit-0 re-pin; **64** verified free at the Stage-6 renumber — see Δ-renumber). Append at the roster tail — never edit an existing check body. Rebase before each commit if a sibling merges. | CHEAP / HIGH |
...
- [ ] **CIAC-1 (#4195 × #4196 × #4197 × #4198 — the probe-validity clause):** every check or lint this release authors reports a **denominator** and runs a **control expected to return non-zero**, and **cites** the probe-validity rule in `review-discipline-principles.md` rather than restating it.
  - *Method:* grep the Check-63 and Check-64 blocks in `core/deploy/deploy.sh`, the TH-3 row in `readiness-map-template.md`, and the identity-assertion step in `.github/workflows/reference-durability.yml` — each must match. **Control:** run the same grep against a pre-existing check block and confirm it does **not** match, proving the grep discriminates rather than matching everywhere.
...
- [ ] **CIAC-4 (#4196 × #3938 × #4197 — dogfooding: the release passes its own lint):** the release's own edits introduce **no** new stated-count-vs-structure mismatch, and the Check-63 lint this release ships flags none of the files this release touches.
  - *Method:* run `deploy.sh --check` filtered to Check 63 against the release-branch head, scoped to the File Change Matrix paths; confirm zero `  FAIL:` lines. **Control:** introduce one deliberately wrong count in a scratch copy of a touched file and confirm Check 63 flags it (mutation control; revert).
...
- [ ] **CIAC-5 (#4197 × #4196 on `readiness-map-template.md`):** #4197's token-count edit to the TH-2 row is consistent with what #4196's Check 63 asserts about that same line — the two cards must not disagree about the same file.
  - *Method:* after both land, run Check 63 scoped to `readiness-map-template.md` and confirm it does not flag the TH-2 row #4197 edited. **Control:** revert the #4197 edit in a scratch copy and confirm Check 63's verdict changes, proving the check actually reads that line.
...
core/deploy/deploy.sh                                                    # edit — #4196 Check 63 (def-block + log emitter); #4197 Check 64 (def-block + log emitter). APPEND ONLY, no existing check body edited
core/deploy/allowlists/count-structure-baseline.txt                      # new  — #4196 committed 73-entry accepted-population baseline
...
The File Change Matrix is entirely internal pmo-platform artifacts. 29 of 36 paths are `core/` / `release/` governance, pipeline-spec, and standards documents (`domain: governance`); the secondary class is `software` for the 7 executable paths (`core/deploy/deploy.sh`, 4 × `core/hooks/**`, `.github/workflows/reference-durability.yml`, `docs/scripts/setup-workspace.sh`). Sourcing-exempt per `stage-04-planning.md` § 5.7 — the entire matrix is internal artifacts and no external practice governs criterion-ID citation hygiene — but the domain class is mandatory in every mode and travels unchanged into Solutioning and Engineering.
...
| **Δ-lock** | Stage-4 sized #4198, #4196, #4197 from card bodies | **LOCK AT MEASURED SCOPE.** #4198 = 7 constants / 21 declaration sites / 10 files + a new shared lib + co-deploy across 3 installers. #4196 = lint + a committed 73-entry baseline. #4197 = check + `deploy.sh` slot 64 + the mandatory package rebuild. **No expansion adds a capability** — each is the measured size of already-approved work. | Collective Review scope-lock |
...
| **Δ-defer** | Three items had been deferred at Stage 5 | **All three accepted:** (a) #4195 CD-2(a)/(b) — two `hub-spoke-bridge.md` edits closing the actor-reach gap; (b) #3838 CD-1 — §3 declared in-milestone, its deferral rationale having expired when #3839 canonicalized the resolving rule minutes later; (c) **CIAC-5**, binding #4197's token-count edit to #4196's Check 63 over the same `readiness-map-template.md` row. | Collective Review scope-lock |
| **Δ-slots** | Stage-4 identified slot **62** as free; #4197 and #4198 slots undetermined | **Rule-computed, then re-computed.** Final: `deploy.sh` slot **63 → #4196**, slot **64 → #4197**. The original assignment was 62/63; a concurrent release claimed slot **62** on `main` mid-run (register runner-resolution, #4208), so both shifted up one — see Δ-renumber. **#4198 needs no slot and no `deploy.sh` edit** — its control rides the existing tri-invoked `run-fragile-ref-fixtures.sh` harness. | Collective Review scope-lock; Stage-6 renumber |
| **Δ-baseline** | Stage-4 pinned `c4dde614`; contention map measured there | **Re-pinned at Commit 0 to `93023d8e`** (+25 commits: two siblings merged, `v4.02` claimed by milestone 298). **PROCEED** — 13 of 14 target files verified untouched, the then-assigned slot 62 verified still free. **Every chip re-locates its edit anchors by content, not by line number.** | Collective Review; Commit-0 re-verify |
...
| **Δ-renumber** | Slots **62 / 63** assigned to #4196 / #4197 | **Shifted to 63 / 64.** A concurrent release claimed slot **62** on `main` mid-run (register runner-resolution, #4208). Max Check N on `main` = 62, so **63** and **64** were the next free pair — verified free on `main` and on this branch with sensitivity and specificity controls. Renumbered across every referencing surface in one pass: both `deploy.sh` def-blocks and `log` emitters, the two blocks' cross-references, the block-local shell variable prefixes (`c62_`→`c63_`, `c63_`→`c64_`), the count-structure predicate / baseline / fixture-pair / fixture-runner, the theme-token harness, and this plan. Check-57 roster contract and both checks' measurements re-ran **identical** across the renumber. The version-slot lesson of Δ-version now has a check-slot twin: **a slot is not owned until it merges.** | Stage-6 renumber; concurrent-claim response |
...
| **Δ-S9** | Stage 9 rendered **GO** with four operator-named conditional fixes to land before merge | **All four applied on-branch.** (a) **`methodology-archetype-matrix.md`** — "9 rows (8 archetypes + Custom)" over an 8-row table, and the glossary's "the 9th row", corrected to **7 archetypes + Custom = 8**; the `delivery_approach` enum is 8 values *including* `Custom`. Invisible to Check 63: the preamble is not colon-terminated, so it is neither examined nor baselined. (b) **`review-discipline-principles.md` §8.3 row 9** — `PV-3` did not catch "hard-wrapped corpus defeats a line-anchored grep"; running the §8.2 record verbatim over a live wrapped clause **inside that same file** returns subject 0 against ground truth 1 with every required element established, i.e. **CLEAN on a false zero**. No other clause catches it, so the row is **re-classified** as an uncovered residual (clause `none`), §8.3's verdict re-stated 11→10 of 12, and §8.4 reconciled (`COVERED` 11→10; `truncated-or-empty-extraction` 4→3, which had been silently absorbing shape 9 under a class §8.4 never named) plus a declared **8th class**. Two adjacent §8.4 inaccuracies corrected in the same pass: the ACTOR BOUNDARY now names `hub-spoke-bridge.md` Procedure 7 Step 4 (landed by **this** PR) as the hub's delivery surface, and the wrong-pattern class is declared observed in its false-ALARM direction only. (c) **Check 64 narrowed to colour-scheme contexts** — `@media print` registered as a third theme block and demanded the full token set (reproduced: 8 false findings, exit 1); `@supports` filed root-scope declarations OUT-OF-ROOT (reproduced: 2 false findings, exit 1). Both are ordinary CSS that would redden a required gate on correct content. Two committed must-not-flag fixtures added; they **fail** against the pre-narrowing predicate and pass after, so they guard the behaviour. Live corpus byte-identical at **141** resolutions, both files CLEAN. (d) **This row**, plus the executable-path correction above. | Stage 9 GO conditions (operator) |
| **Δ-residual-2nd-order** | Stage 5 abandoned its `blast-radius.sh --depth=2` run on the cross-chain-index wiring card (>4 min, no output), substituted `--depth=1`, and recorded **Second-order 0** for both of that card's targets under an `[ASSUMPTION – CONFIRM]`. Stages 6, 7 and 8 each restated the gap as *inherited unverified*; Stage 8 recorded that it wanted an owner rather than another deferral. **No row here recorded the residual at all**, so this plan read as carrying none. | **Measured; residual discharged.** The operator rendered disposition **(a) — assign an owner and measure it** on the second-order blast-radius card (2026-08-09); the measurement ran 2026-08-10 against `origin/main` `81e35588`. The recorded `0` was **not a measurement**: `compute_second_order()` returns before scanning whenever `--depth` is below 2, so the depth-1 invocation could not have produced a non-zero — a **broken probe** under the probe-validity discipline *this same release shipped*, not a clean result. The >4-min hang was itself a since-fixed process-spawn defect in the tool; depth-2 now completes in 14 s and ~4 min for the two targets. Re-run at `--depth=2` (1,356 files scanned per run, exit 0, no `--max-expand` truncation): `gate-criteria-spec.md` **145 first-order / 627 second-order**; `cross-chain-architecture-map.md` **13 / 198**; **772** unique second-order paths in union (53 shared). **Effective second-order surface: 0 of 772.** All three vectors by which that card's edit could reach a second-order consumer — the renamed `— [DEFERRED]` heading anchor, a restated stale deferral claim, and the deleted remedy sentence — return **0** corpus-wide, each probe having returned **1** against the pre-change blob at `25f8c3ea^` (mutation-form sensitivity) alongside a discriminating specificity arm. The four stages' *judgment* — that first-order is the correct instrument for an additive change that moves no path and adds no criterion ID — is **confirmed**; only the verification was missing. | Post-release residual disposition (a), measured and posted on the second-order blast-radius card |
...
| #4196 Check 63 + committed baseline | `git revert` — but see the asymmetry note | **MODERATE** |
| #4197 Check 64 + TH-3 + package rebuild | `git revert`, then rebuild the package | **CHEAP** |
...
**Asymmetry note (R-11).** Check 63 ships **enforce**. Reverting it after other work has merged green against it silently re-opens the gap rather than announcing it — the one non-symmetric surface in this release. Its committed 73-entry baseline is the mitigation: the accepted population is auditable in-tree, so a revert is legible rather than silent.
