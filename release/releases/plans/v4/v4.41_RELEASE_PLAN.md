...
| ADR number integrity | integration | `check-adr-numbers.py` — **PASS**, 142 ADRs contiguous, no duplicates, *as read on the branch at this position*. Superseded by two later renumbering rounds; re-derived on `main` at `33886bd7`: **154 ADRs, contiguous `001`–`154`, 0 duplicates, 0 gaps**. |
...
| Doc-link integrity | integration | `deploy.sh --check` Check 14 — **OK, no broken cross-refs in scope.** |
| ADR number integrity | integration | `check-adr-numbers.py` — **PASS**, 144 ADRs contiguous `001..144`, no duplicates, *as read on the branch at this position*, where the record then held its branch-local number and `144` was the corpus size. Both figures are superseded: the record renumbered to **ADR-154** at the Stage-12 merge, and re-derived on `main` at `33886bd7` the corpus is **154 ADRs, contiguous `001`–`154`, 0 duplicates, 0 gaps**, with next-free **155** (`renumber-adr.py --next-free`) — not 154, which is now taken by this release's own record. |
...
| Check-number next-free | integration | **71** *as read on the branch at this position* — 69 registrations / 68 distinct, min 1, max 70, gaps `[15, 24]` (both RETIRED-RESERVED, never refilled). Specificity control `log "Check 999:` → 0. **Superseded at the Stage-12 merge:** the mainline had claimed 71 for BLOCK-DESTRUCTIVE-022 (#5250), so this release's check landed at **72** across 18 tokens in 6 files. Re-derived on `main` at `33886bd7` under the same `log "Check N:"` predicate: **71 registrations / 70 distinct, min 1, max 72**, same two gaps, next-free **73**. Controls re-run: specificity `log "Check 999:` → 0; sensitivity `log "Check 72:` → 1. |
...
| ADR numbering | integration | **ADR-151.** The oracle returned 142 *as read on the branch at this position*, because it reads the mainline; 142/143/144 were claimed by siblings on this branch. `check-adr-numbers.py`: **PASS — 145 ADRs, contiguous 001..145, no duplicates.** `check-adr-durability.py`: 145 scanned, **0 findings attributable to this record**. The corpus figures are superseded by two later renumbering rounds; re-derived on `main` at `33886bd7`: **154 ADRs, contiguous `001`–`154`, 0 duplicates, 0 gaps**, next-free **155**. This record's own number is unchanged at ADR-151. |
