...
| Version | **v4.36** — identity **slug-only** per ADR-092. The concrete number binds at the **Stage-12 atomic claim**, when `claim-version.sh --stamp-slug` resolves this token. **D-Version was DETERMINED `version-less` (condition B) at Stage 4** — see § Open at plan time. Mainline at Commit 0 was `v4.35`. |
| Bump Class | `minor` |
