...
| **A5** G-CL6 design-artifact refresh | **N/A** | The plan declares **0** Tier-A activated design artifacts (`python3 re` scan for `Tier-A` returns 0 on the same reader that returns 1 for `domain_practice`) |
