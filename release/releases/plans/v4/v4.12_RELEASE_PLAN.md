...
**Out-of-repo mutation (not a file path, listed for completeness):** **29** published GitHub Release bodies re-emitted via `gh release edit --notes` under #3699. **IRREVERSIBLE.**

> **Reconciled at Stage 13 close (2026-08-05).** This plan was authored against a believed population of **eleven**. The Stage-4 full-population re-probe put the true figure at **55**, split by operator decision into **29** (this release) and **26** (deferred to #4747). The count is corrected here, in the Rollback Strategy, and in the Reversibility summary. The **eleven** appearing in the BEFORE clause and in the Stage-4 dependency-edge note is left as authored — those are point-in-time records of what was believed when each was written, and both already carry their own correction trail. The "**gap is invariant at exactly 11**" in DEV-3 is a *different* eleven (a corpus-denominator delta) and is untouched.
...
- **IRREVERSIBLE:** #3699's re-emit of **29** published GitHub Release bodies (scope corrected 11 → 29 at the Stage-4 re-probe; see the File Change Matrix note). Mitigated — not undone — by the capture-before-overwrite gate: all **29** prior bodies are captured to `release/releases/_captures/` and merged to `main` before the first `gh release edit`, so the prior state is recoverable from the repo even though the published surface is not revertable by git. The captures span **two** dated folders — `2026-08-04-release-body-precapture` (the original 11) and `2026-08-05-release-body-precapture-partA-ext` (the 18 bucketed notes added when the population was corrected) — both verified clean at `11 + 18 = 29` via `shasum -c`.
...
**MODERATE / Confidence HIGH.** Every file change reverts with `git revert -m 1`. The **29** re-emitted release pages do not revert; their prior bodies are captured into the repo before the first overwrite.
