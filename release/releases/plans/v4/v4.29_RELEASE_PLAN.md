...
The plan file, the branch, and all hub state are keyed on the milestone **slug**, never a version. Through Stages 4–11 this file carried an unresolved version placeholder and **no baked version number anywhere**, per ADR-092: the version binds only at the Stage-12 atomic compare-and-swap, where git's ref CAS is the authority — it bound as **`v4.29`**.
...
4. **The version placeholder in this plan's header — DISCHARGED.** It was left unresolved at this release's own close because the stamping flag was deliberately not passed: ADR-092 was `Proposed` rather than Accepted, the Stage-12 spec never named the flag, and stamping would have renamed this slug-named plan against the release's slug-primary posture. Ownership passed to the in-flight ADR-092 plan-file-identity release, and that release discharged it: the header resolves to **`v4.29`** and this plan now sits at its ADR-092 claim-time home under `plans/v4/`.
