...
**Version identity:** **slug-primary** per **ADR-092**. Through the pipeline this file was `methodology-fields-and-statuses_RELEASE_PLAN.md` and the branch was `release/265-methodology-fields-and-statuses`; no version stem appeared in either. Bump class is `minor` — a capability release, not corrective, so the patch floor does not apply. The concrete number bound at the **Stage-12 atomic claim** as **`v4.16`**. The rename into the major-version bucket did **not** fire at that claim — `--stamp-slug` HALTed because this plan carries zero `{{RELEASE_VERSION}}` tokens (see § Deviation Log and the `RELEASE_LOG` Deployment Log block) — so it was performed by hand in the Stage-13 close-out chore PR, which is how this file now resolves to `release/releases/plans/v4/v4.16_RELEASE_PLAN.md`. The upstream cause is tracked in #5063.
...
- Release plan: this file, top — `release/releases/plans/v4/v4.16_RELEASE_PLAN.md` (authored slug-keyed as `methodology-fields-and-statuses_RELEASE_PLAN.md`; moved here in the Stage-13 close-out)
- Milestone: `methodology-fields-and-statuses` (milestone 265) — see § References for the per-card index
- User-facing release note: authored at Stage 13 Close at `release/releases/notes/v4.16_RELEASE_NOTES.md`, the version having bound at the Stage-12 atomic claim per ADR-092. This section is the operator-facing pre-merge artifact and does not substitute for that note.
