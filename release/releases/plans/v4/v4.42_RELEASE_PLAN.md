...
| 20 | D-Version recorded as **v4.39** at Stage 4 | **Re-verified at Engineering Commit 0 → PROCEED, and deliberately NOT baked into this file.** Three claimed-set arms each with sensitivity + specificity controls return `v4.39` × 0; `claim-version.sh --dry-run` independently recomputes `v4.39`. The plan is slug-primary per ADR-092 and carries `v4.42`, so a concurrent sibling claiming the slot costs a re-version at Stage 12 and **no edit to this file** | Engineering Commit 0, #6146 |
...
- User-facing release note (authored at Stage 13, not by this section): `release/releases/notes/v4.42_RELEASE_NOTES.md`
