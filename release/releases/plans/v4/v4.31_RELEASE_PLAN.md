...
The release ran **slug-identified**. No version literal appeared in this file, in the branch name, or in any commit subject on that branch; the concrete number was recomputed next-free and claimed atomically at the Stage-12 merge tag (defer-to-claim, per ADR-092). The durable declaration here is the **bump-class (`minor`)**, not a number. **This release claimed `v4.31`.**

**Claim-time stamp: did not fire, and was completed retroactively.** At the Stage-12 claim this file was one of **three** token-bearing plans at the `plans/` root, so `_derive_stamp_slug` refused rather than guess, `claim-version.sh` returned zero without stamping, and the tag was bound while the rename and the token resolution were not. The condition is silent by construction — the refusal is a correct decline, and nothing downstream reported that the stamp had been skipped.

The rename to the ADR-092 claim-time home and the resolution of the version placeholder were performed by the in-flight `plan-file-identity-adr-092` release, which was at its Stage-9 gate when this one merged. That release also wired the Stage-12 claim to pass `--stamp-slug` explicitly, so a multi-candidate corpus can no longer silently skip a stamp.
