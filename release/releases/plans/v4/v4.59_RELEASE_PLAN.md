...
> **Milestone:** `release-tools-invocable-gates-enforced` (336) · **Release Class:** `cross-cutting` — held at Stage 5 (Tight engagement density / **Deep** Stage-9 review depth / Stage-5 activation bias ALL / 30-day outcome window) · **Version:** `v4.59` — **unbound through Stages 4–11; binds at the Stage-12 atomic claim (ADR-092)** · **Scope:** 5 cards, 13 raw pts × 1.3 = **17 effective** (band 15–25, PASS) · **Topology:** D-C `SINGLE` · **Concurrency posture:** P0 fully-serial · One release branch, one PR, one merge gate · **Branch:** `release/release-tools-invocable-gates-enforced` (slug-primary, no version stem).
...
| Field | Value |
|-------|-------|
| **Version** | `v4.59` — unbound; the concrete `vX.Y` binds at the Stage-12 atomic claim (ADR-092). This cell is the machine-read **stamp manifest**, not prose. |
...
| 5 | `v4.59` stamp manifest | Stage 4 D-Version | **Token PRESENT; the determination behind it ABSENT — gap G-5** | § Header, `**Version**` cell |
...
**G-5 — `D-Version` was never rendered, so no bump-class intent exists (element 5).** The Stage-4 comment carries zero occurrences of `D-Version`, `Bump Class`, or `v4.59`. Two facts are separable here and are kept separate:
...
| 3b · `release/tools/claim-version.sh --verify-stamp <slug>` | **PERFORMED, post-write and pre-commit.** Real output recorded in the Stage-6 verification evidence for this slice. The plan carries the `v4.59` token, so the manifest half of the re-verify is assertable even though the version half is not. |
...
| **R9** | **The `v4.59` stamp is intact but its bump class is not determined** (G-5), so Stage 12's next-free computation has no floor. | Release identity | CHEAP (a determination costs nothing) | HIGH | Operator renders the bump class at or before Stage 12. Tracked as **AI-003**. Do not synthesize a floor. |
