...
| **DEV-11** | **The Stage-5 `domain_practice` Mode B → Mode A upgrade directive is declined, and the Form-X classification stands.** Stage 5 on the gate-bind sub-task directed a Mode-A label naming the Kanban Method (Anderson 2010) core practice 2 as its `source`, and instructed that the upgrade be recorded in this log; neither limb was executed. Three independent readings found the silence — the gate-bind card's Dev Testing (`F-01`, Minor/Tier 2), a second Dev Testing reviewer reaching it from a different direction, and QA re-verifying at HEAD as a Lane-3 item — tracked as `AI-001`. **The competing reading is substantive and is recorded, not dismissed:** the Method was not merely cited, it *decided* four design values — no `default` on the control, `min = 0` rather than `min = 1`, `WARN-HEALTH` over `BLOCK-TRANSITION`, and `scope = "parent"` over `board` — each settled by reading what the Method does and does not prescribe, which is an external body of practice consumed as an input to the design. | **Operator ruling at the Stage-9 gate (2026-09-11)**, on the two clauses that govern the token. Stage-4 § 5.7 makes *sourcing-exempt* a property of **where the deliverable's files live**, holds that and *what informed the design* to partition the case **with no residue**, and routes an all-internal File Change Matrix to **Form X verbatim**; its Form-A branch opens only where the matrix is *not* entirely internal. Stage-5 § 5.7's upgrade obligation is conditioned on Stage 4 having emitted `Mode B — UNSOURCED-DOMAIN` and is defined as replacing that token — Stage 4 emitted Form X, so the antecedent never held and the carry-forward clause, under which the label travels unchanged, is the one that applies. | **DECLINED — and the reviewers' observation upheld in the home the grammar gives it.** The label stays `N/A — pipeline-internal release`; the classification is unchanged. The four values were decided by the Method, and that provenance is recorded in the control-declaration record's **D-a** and in the `source` strings on the gate entry and the controls declaration — not in the plan's `domain_practice` token, which reports only where the deliverable's files live. Logged rather than silently retained: a directive three reviewers found missing must be **visibly declined**, or a later reader reaching only the Form-X prose finds the winner with no trace of the contest. |
