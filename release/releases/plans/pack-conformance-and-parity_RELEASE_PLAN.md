...
| **DEV-11** | **The Stage-5 `domain_practice` Mode B → Mode A upgrade directive is declined, and the Form-X classification stands.** Stage 5 on the gate-bind sub-task directed `domain_practice: { source: Kanban Method (Anderson 2010) — core practice 2, Limit work in progress; … }` as a Mode B → Mode A upgrade and instructed that the upgrade be recorded in this log. Neither limb was executed, and the silence was found three times independently — raised by the gate-bind card's Dev Testing as `F-01` (Minor, Tier 2), reached from a different direction by a second Dev Testing reviewer, and re-verified at HEAD by QA as a Lane-3 item — tracked across the stage record as `AI-001`. **The competing reading is substantive and is recorded here rather than dismissed:** the Method was not merely cited, it *decided* four design values — no `default` on the control, `min = 0` rather than `min = 1`, `WARN-HEALTH` over `BLOCK-TRANSITION`, and `scope = "parent"` over `board` — each settled by reading what the Method does and does not prescribe, which is an external body of practice consumed as an input to the design. | **Operator ruling at the Stage-9 gate (2026-09-11)**, grounded on the two clauses that govern the token. Stage-4 § 5.7 separates *sourcing-exempt* — expressly "a property of **where the deliverable's files live**" — from *what informed the design*, states that the two "partition the case with no residue", and routes a File Change Matrix consisting entirely of internal artifacts to **Form X verbatim**; its Form-A branch is reached only when the matrix is **not** entirely internal, which is not this release. Stage-5 § 5.7's upgrade obligation is conditioned — *"If Stage 4 emitted `Mode B — UNSOURCED-DOMAIN`"* — and its mechanism is defined as replacing that exact token; Stage 4 emitted Form X, so the antecedent never held and the applicable clause is § 5.7's carry-forward obligation, under which the label travels unchanged. | **DECLINED — and the reviewers' substantive observation is upheld, in the home the spec gives it.** The label stays `N/A — pipeline-internal release` and the classification is unchanged. The four design values genuinely were decided by the Method; that provenance is recorded where the grammar routes it — the control-declaration record's **D-a**, and the `source` strings carried on the gate entry and on the `[[controls]]` declaration themselves — and not in the release plan's `domain_practice` token, which reports where the deliverable's files live and nothing else. Logged rather than silently retained: a directive three reviewers found missing has to be **visibly declined**, and a reader who reaches only the Form-X prose at lines 42–44 would otherwise find the winning argument with no trace that it was contested. |
