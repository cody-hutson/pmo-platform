...
| #6377 | AC-6 | `bash release/tools/blast-radius.sh --self-test` — the instrument fix ships with an arm that can fail. RED→GREEN: the suite must gain at least one assertion exercising a `.py` reference, and that assertion must fail against the unpatched type list | Assertion count strictly greater than the pre-change 36, all passing; the new arm observed failing on the unpatched instrument, so the green is falsifiable |
| #6361 | AC-1 | Run the new validator mode over a conforming kit fixture: `python3 core/deploy/tools/check-work-hierarchy.py --validate-packs --pack-root core/deploy/tests/fixtures/packs/conforming-kit`. Graded at the **kind level**, not the pack level — the predicate asserts that a kind whose projection sets the neutral archetype sentinel is accepted, because a change relaxing only the pack-level field would validate a kit whose every kind is still archetype-welded and the capability would be absent | Exit 0 with the kit's kinds accepted; the mutation arm removing the kind-level neutrality is rejected under its own rule identifier |
...
| #6361 | AC-6 | `python3 core/deploy/tools/check-work-hierarchy.py --self-test` — a validator exists and is runnable against a pack, so AC-1 is mechanically gradable rather than read by eye. Both arms required: accept the three shipped packs, reject the deliberately-nonconforming fixture, and **each mutation arm asserts its own rule identifier and the absence of every other**, so a dead rule fails an arm instead of riding a green suite | Suite green with the arm count strictly greater than pre-change; every rule identifier exercised by at least one arm that fails when its rule is mutated |
...
| #6362 | AC-3 | Null-expecting: `git diff --stat origin/main -- core/governance/OPERATIONS.md` — resolution runs through the existing five-rung cascade and introduces **no parallel resolution path** · control: the same command over `core/config/operator.toml.template` returns a non-zero diffstat on the same instrument | **Empty diffstat** on the resolver surface · control non-zero |
| #6362 | AC-4 | Run the selection arm over the selection fixtures: `python3 core/deploy/tools/check-work-hierarchy.py --resolve <archetype> --kit <pack_id> --pack-root core/deploy/tests/fixtures/packs` — selection is orthogonal to methodology, asserted by resolving the same kit under two different archetypes and the same archetype under two different kits | Four resolutions, each emitting the expected kind set with provenance; the kit axis and the methodology axis each vary independently |
...
| #6381 | AC-1 | Run the traversal arm over the rollup fixture: `python3 core/deploy/tools/check-work-hierarchy.py --validate-packs --pack-root core/deploy/tests/fixtures/packs/rollup-kit`. Graded as restated by the design, not as literally worded: container levels resolve an entity type, never a kind — so a fixture declaring kinds at portfolio, program and project is the **rejection** arm | Exit 0 on the rollup fixture with the full traversal depth reported; the tier-claiming fixture rejected under its own rule identifier and no other |
| #6381 | AC-2 | Null-expecting: `grep -cE '^\s*base\s*=\s*"(?!Work Item)' core/deploy/tests/fixtures/packs/rollup-kit/pack.toml` expressed as a literal scan for any non-`Work Item` base — no new entity node is introduced at any level · control: `grep -c 'Work Item' core/deploy/tests/fixtures/packs/rollup-kit/pack.toml` returns non-zero on the same file with the same instrument | **0** non-`Work Item` bases · control non-zero |
...
| #6381 | AC-4 | `python3 core/deploy/tools/check-work-hierarchy.py --self-test` restricted to the level rules — a nested grouping kind is rejected, and the rejection names its own rule | The nested-grouping fixture rejected under its own rule identifier and no other |
...
| #6381 | AC-6 | Regression arm: `python3 core/deploy/tools/check-work-hierarchy.py --emit-kinds` before and after the change on the same tree — the existing hierarchy finding set and kind-emit output are byte-identical. Null-expecting on the diff, so it carries its arms · control: the same comparison against a seeded extra fixture pack must produce a **non-empty** diff | **Byte-identical** output · control non-empty |
