...
| 26 | 6 | The `since` value on the new `operator-toml-schema.json` key is written as the literal **`4.46`**. | The concrete release number binds only at the Stage-12 atomic claim, but every sibling key in the declaration carries a bare numeric `since` and the field has no `{{RELEASE_VERSION}}` consumer (`claim-version.sh --stamp-file` resolves to a **tag**, `vX.Y`, which is not the shape this field uses). `4.46` is the Commit-0 recomputed next-free the Header records. **Named rather than hidden:** if the atomic claim wins a different number, this one value needs the same edit the plan's own version stamp gets. Its blast radius is one display string in C70a's WARN-mode remedy line. Minor adjustment; no scope change. |
