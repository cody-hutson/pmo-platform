...
> **Address correction, decision unchanged (recorded at Engineering Commit 0).** This decision named check number **75**. That number was free when the decision was rendered and is not free now — a sibling release landed a check at 75 on the mainline, and the highest defined number there is 75, so next-free is 76 as measured at Commit 0. The *decision* — one consolidated deploy + CI check, single-sourced on one predicate — stands unamended. #5861 derives the number at its own first commit; see § Corrections Engineering must carry.
...
- **The deploy-check number moved: derive it at the owning slice's first commit, do not transcribe `75`.** D-DeployMirror placed the consolidated check at number **75**, correctly, on a reading that the file then carried 74 checks and 75 was untaken. A sibling release has since landed **`Check 75: Pack-grammar conformance`** on `origin/main`, and the highest defined check number there is now 75 — so **next-free is 76**, measured at this release's Engineering Commit 0. **The decision is unchanged and is not reopened**: the gate still runs deploy + CI as one consolidated check, single-sourced on one predicate. Only the *address* moved, and the address was never the decision. Treat this exactly as the ADR-number obligation is treated — it is the same failure mode in a second numbering namespace, where a number is allocated at authorship but claimed at merge, so a slice that transcribes a stage-old reading collides. #5861 re-derives the next-free check number at its own first commit and again at Stage 9, rather than reading it from this plan.
