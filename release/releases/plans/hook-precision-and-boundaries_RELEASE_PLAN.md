...
*Operator-facing. Authored at Stage 6 Phase C1 as required before the pull request is marked ready for review. Refreshed at the final Engineering slice against the branch's own change set and commit log — not against a summary of either. All five work-units have landed.*
...
The orchestrator gains one obligation and one standing check: every path it writes into a spoke brief takes a sanctioned form, and it scans the rendered brief before each launch and does not spawn on a hit. The shared detection primitive becomes agent-executable for the first time and is allowlisted accordingly. The path-leak rule leaves the shared mode cohort, which drops from eight readers to seven — an operator who flips the shared dial now reaches seven hooks, not eight, and no longer reaches this one.

**One operator action is required after deploy, and the release is not fully realised without it.** The hook-wiring re-home extends hook loading to repo- and worktree-rooted sessions. Until it runs on a given instance, the path-leak guard is not active there at any mode setting, and the warn log it would populate stays empty in a way that is indistinguishable from clean. Read the install validator's hook-wiring re-home verdict before reading any warn-log result.
...
- [ ] **CIAC-2 (mode-coupling × posture surface, on the path-leak hook and the bypass-mode readiness rule).** The documented mode posture equals the implemented mode posture. The shared-mode cohort list in the readiness rule's warn-versus-enforce section **matches the set of hooks actually reading the shared mode file** — documented as four at Stage 4, corrected to eight by the mode-coupling work-unit, and **seven** after the posture-surface work-unit removes the path-leak hook from the cohort — and the path-leak hook's post-change dependency guard and mode posture are described as implemented, at `warn`, on its own mode file.
...
core/hooks/.gh-path-leak-mode.template
...
**Seven further paths added at the final Engineering slice**, all required rather than discretionary: the public-vs-operator-instance taxonomy (the second cite-only pointer the boundary clause owes), the detection primitive (it gains the whole-file arm the pre-spawn guard invokes), the script-execution allowlist (see below), the new mode template, the workspace-setup install call, the CI-layout sandbox seed, and `.gitignore` (the dedicated-mode-file glob).

**The script-execution allowlist obligation activated, and the original N/A was incomplete rather than wrong.** No new executable script is added — but the path convention makes an **existing source-only script agent-executed**, which triggers the same obligation. Without the four invocation forms the pre-spawn guard is blocked agent-side on every launch and the feature never runs. Four forms plus a suffix glob ship in this pull request. The rule to carry forward: the obligation fires when a script becomes agent-executed, not only when one is added.
...
|---|---|---|---|
| s1 | Boundary clause present at both homes | `grep -c '^## 6. Egress boundary' core/standards/analysis-workspace-standard.md` — expect 1; and `grep -c 'analysis-workspace-standard.md.*6' core/standards/depersonalization-spec.md core/standards/public-repo-vs-operator-instance-taxonomy.md` — expect a non-zero pointer in each | The rule body is present at the corpus's owning standard and each of the two sibling standards carries a cite-only pointer rather than a restatement. Sensitivity control: the same grep against the pre-change file returns 0 |
...
| s2 | Path convention present | `grep -c '^## Spoke-brief path scan' release/skills/release-hub/references/spoke-launch.md` — expect 1; `grep -c 'sanctioned' release/skills/release-hub/references/spoke-launch.md` — expect non-zero inside § Output path | The convention is present as a delta on the output-path section (no new heading of its own), the pre-spawn scan takes the one new heading, and the ordinal sentence is preserved with the new section self-declaring as fourth |
| s2 | Scan arm exists and fails loud | `bash core/deploy/tools/path-leak-patterns.sh --self-test` — expect `SELF-TEST PASS (18 cases)` | The whole-file arm returns three distinct exit values, and the unreadable case returns 2 rather than 0. The three scan-file cases are the sensitivity, subject and fail-loud arms of one probe |
...
| s3 | Posture surface shipped at `warn` | `cat core/hooks/.gh-path-leak-mode.template` — expect `warn`; and the hook's `get_mode()` default-when-absent is `warn` | The per-rule mode template ships and its content is `warn`, matching the script default, which is the operative posture |
| s3 | Cohort documented equals cohort derived | `grep -lF 'MODE_FILE="${HOOK_DIR}/.mode"' core/hooks/*.sh` — expect the same 7 names the readiness rule enumerates | Set equality, both directions. Sensitivity: the full `MODE_FILE=` census returns 11 declarations, so the pattern discriminates rather than matching everything. Specificity: a never-used mode path returns 0 |
| s3 | Exclusivity is proved by arms that cannot agree | `core/hooks/tests/block-gh-path-leak.test.sh` — the three exclusivity cases | With the dedicated file absent, a shared file at `enforce` must not promote (caught by exit code) and a shared file at `off` must not silence (caught by warn-log growth plus stderr). The third case, dedicated file present at `enforce`, is the sensitivity control without which both arms would pass on a hook that reads no mode file at all |
| s3 | The mode-file name in the operator-facing notice is pinned | `core/hooks/tests/block-gh-path-leak.test.sh` warn case — pattern runs past the comma to `\.gh-path-leak-mode=warn` | Under the shipped posture the warn branch is the only one that executes, so its mode-file name is the sole operator-facing instruction for changing the posture and must be covered by a gate |
...
**E-14 · Decision-record renumbered ADR-129 → ADR-130 at Stage 8.** E-5's allocation was correct when it was made and was overtaken. A sibling release merged first and took `release/ADRs/ADR-129-*` while this branch carried `core/ADRs/ADR-129-*`; because the directories and filenames differ, `git merge-tree` exited 0 with **no conflict** and the merged tree would have carried two ADR-129s and no ADR-130 — silently, with the integrity gate then failing every subsequent PR. The numbering space spans two directories and git's conflict detection is path-based, so it cannot see a semantic collision across them. Renumbered with `release/tools/renumber-adr.py`, which rewrote nine citations across two files (three of them in ADR-078, which a bare rename would have orphaned), updated the index, wrote the § Renumber-log entry, appended the provenance note to the record's Status section, and verified zero dangling citations.

**E-15 · The renumber sweep corrupted three historical sentences in this log, and they were restored.** `renumber-adr.py`'s R3 citation sweep is a textual `ADR-129 → ADR-130` rewrite over the branch diff. It cannot distinguish a **live citation** — a reference that should follow the record — from a **historical statement about the number itself**, which must not move. It rewrote the Commit-0 measurement (`next-free is therefore ADR-129`), A-10's re-derivation, and E-5's allocation, leaving E-5 reading *"allocated at ADR-130 … against a later spoke-level instruction to reserve ADR-130"* immediately followed by *"so 129 is the true next-free slot"* — an audit trail contradicting itself. The three sentences were restored verbatim from the pre-renumber revision and this entry appended instead, so the log records what was decided when, and separately what changed it. Surfaced by Stage 8 QA as Q-01. **The general form is worth carrying past this release:** an automated citation sweep over prose is safe for references and unsafe for history, and a decision log is mostly history.

**E-16 · Release Outcome Statement limb 2 is not delivered, and was mis-stated at bundling.** The milestone's Outcome Statement promises *"the security-reminder matcher stops false-positiving on prose and comments."* No change in this release touches that matcher. #3805 shipped the **hook-response discipline** — how a spoke responds when a control fires — which is a different deliverable from making the control precise. The card's own title has always read "Codify spoke hook-response discipline"; the Outcome Statement, written at bundling on 2026-07-28, described the slot as "security-reminder matcher precision". The statement mis-described the card before Engineering began, so this is a bundling-time defect rather than a build shortfall. Recorded here rather than by editing the Outcome Statement, because the operator declined amending the milestone description at the Stage 9 gate. **Limbs 1 and 3 are delivered and measured:** the LIB-MISSING guard degrades rather than over-blocks (all nine cohort hooks resolve `warn` on a shipped-default install, over-block clears rather than relocating), and the corpus-to-public-surface boundary is stated at `core/standards/analysis-workspace-standard.md` § 6/§ 6.1 — which then caught a live leak in a QA reviewer's own draft.

**E-17 · Four Stage-9 conditional edits, landed before the GO took effect.** The operator's Stage 9 GO was rendered conditional on these: (a) the `block-gh-path-leak.sh` comment contradiction, where line 70 asserted an exclusive read while later comments still named the shared `.mode` in the resolved value and the precedence chain — a maintainer reading only those blocks could have reintroduced the fallback D-1 removed; (b) the `test-runner.sh` RESIDUAL pass-through, since the runner discarded suite output on every summary-producing run, making a disclosure whose purpose is to qualify a green run unreachable on every green run; (c) honest-scope notes on **both** unenforced "MUST carry verbatim" universals in `spoke-launch.md` — the release found the second one, which predates this release, and softening only the first would have left an identical false universal four sections above it; (d) this entry and E-16.

**E-6 · The path convention landed as a delta inside the launch-policy file's output-path section; only the pre-spawn scan took a new heading.** A-9(d) directs the convention to be authored as a delta on the upstream release's run-directory section rather than as an independent guard, and two adversarial reviewers reached that conclusion separately. The pre-spawn brief scan is a different thing — a genuinely new standing guard, in a file whose convention is one heading per guard, and the destination A-8 routed the deferred mechanical check to. So the section count still rises by exactly two from the merge-base, satisfying the cross-issue criterion's first limb: the hook-response guard is one and the brief scan is the other. Measured: merge-base **9**, branch **11**. The ordinal sentence was preserved and the new section self-declares as fourth, satisfying the second limb by verification rather than by increment. Minor / CHEAP / HIGH.

**E-7 · The script-execution-allowlist companion obligation activated after all, and the plan's N/A was incomplete rather than wrong.** The matrix reasons "no new executable script is added, so the obligation is N/A" — true as written. What it misses is that the path convention makes an **existing source-only script agent-executed**: the pre-spawn guard instructs the hub to run `path-leak-patterns.sh --scan-file`, and that script is in no allowlist. Without the four invocation forms the guard is `BLOCK-DESTRUCTIVE-022`-blocked agent-side and the feature is dead on arrival — the failure the allowlist file's own comments describe twice. Found empirically, by the control firing on this spoke's own invocation of it. Four forms plus a suffix glob added to the composition surface. The generalizable rule: the obligation triggers on a script becoming *agent-executed*, not only on a script being *added*. Major / CHEAP / HIGH.

**E-8 · The whole-file scan is a library-level function, not logic confined inside the direct-invocation guard.** The design placed it inside the `BASH_SOURCE` guard so the sourcing surface would stay byte-identical for both consumers — a property the adversarial review verified and endorsed. Deviated deliberately: logic reachable only through the CLI cannot be exercised by the primitive's own self-test, and shipping an untested arm in the release specifically about untested arms is the wrong trade. The sourcing surface therefore grows one additive function whose name exists nowhere else in the tree, and the self-test now covers all three of its exit values. Minor / CHEAP / HIGH.

**E-9 · The scan prints every hit rather than stopping at the first.** The design specified first-hit-then-exit. A hub repairing a rendered brief would otherwise re-scan once per leak; the observed instance was 19 occurrences. Exit semantics are unchanged. Cosmetic / CHEAP / HIGH.

**E-10 · The version-skew mitigation is two-part, and the option the review listed first does not work alone.** Changing the unknown-flag arm to exit 2 protects every copy at or after this version — and cannot repair a copy that predates the arm, which is the entire installed base on day one, which is precisely the case the finding is about. Verified against the **unfixed** script before asserting the defect: `--scan-file` against a genuinely dirty file returned **exit 0**, indistinguishable from clean. Both halves therefore ship — the durable arm fix, and a caller-side capability assertion stated in the guard as the part that covers the un-refreshed base. Major / CHEAP / HIGH.

**E-11 · Local invocation of the hook test suite was blocked mid-run by `BLOCK-DESTRUCTIVE-022`; the pre-merge checks are the authoritative runner for it.** A command shape this session had already run successfully several times began returning the script-execution block partway through the run; the deployed guard and its allowlist were both last written a week earlier, so the change is not attributable from inside the session and is reported as an observation rather than a diagnosis. It was surfaced, not routed around: the static hardening check ran locally and passed, and the behavioral suites are covered by the pre-merge run on the pushed head, which is the same harness. Two adjacent gaps surfaced and are **not** absorbed: the CI-layout builder and the source-tree hook test files carry no allowlist entry in any form, so the hook suite has no sanctioned agent-side invocation path at all. Minor here / Major for agent-side testability · CHEAP / MEDIUM.

**E-12 · No architecture decision record authored for this card.** The mode-coupling record already on this branch states the resolution order and precedence this card composes against, and records the dedicated-mode-file decision in its own terms. A second record answering "how a mode surface is scoped and defaulted" would restate that one's Decision section from the other side. The posture rationale that would have been its Context is stated where a reader meets it — in the hook's own header and in the readiness rule's cohort section. Minor / CHEAP / MEDIUM.

**E-13 · Two cross-issue criteria are ungradeable as written under the `warn` decision; restated here so a conforming build is not marked NOT-MET.** This is an accuracy correction, not a scope reduction, and follows the same move the sibling card made for its own unachievable criterion. (a) The integration criterion asserting the hook is "on its own file **at enforce**" is false in the authorized build — regrade against **the shipped posture recorded in the mode template and the script default**, which is `warn`. (b) The criterion grading "the **enforce block message** names a reword path" has no subject, because the enforce branch never executes in the authorized build — regrade against the **warn notice**, which is now the branch that prints, and which this card gave both the sanctioned path forms and all three escapes. (c) The card's third measurable outcome — that the check is *in force* on this surface — is knowingly **deferred**, not met: what ships is the mechanism. Its reopening trigger is the install validator's hook-wiring re-home verdict, after which a warn-log reading becomes interpretable for the first time. Major / CHEAP / HIGH.
