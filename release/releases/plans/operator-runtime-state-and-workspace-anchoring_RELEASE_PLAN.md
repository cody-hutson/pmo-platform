...
Rollback for the whole phase is a revert of the merge commit -- **for the repository**. That is not the whole story for the command-position fix: the hook bundle is a deployed, non-git surface, and reverting the merge does not restore the deployed hooks directory. The installer takes pre-write backups, but they live in its session temp directory and are destroyed at exit, so they cover in-run failure only, not a rollback decided after a successful refresh. See the Rollback Strategy and Operational Deployment Manifest sections above for the deployed-bundle limb and the Stage 11 snapshot that backs it.
