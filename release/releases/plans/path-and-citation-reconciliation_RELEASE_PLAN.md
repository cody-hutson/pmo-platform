...
> **Milestone:** `path-and-citation-reconciliation` (331) · **Release Class:** `cross-cutting` (Tight engagement / **Deep** Stage-9 review / Stage-5 ALL / 30-day outcome window) · **Version:** `v4.25` — **provisional; binds at the Stage-12 atomic claim (ADR-092)** · **Scope:** 4 issues, indexed below · **Topology:** D-C `SINGLE` · **Concurrency posture:** P0 fully-serial · One release branch, one PR, one merge gate · **Branch:** `release/path-and-citation-reconciliation` (slug-primary, no version stem).
...
The plan file, the branch, and all hub state are keyed on the milestone **slug**, never the version. `v4.25` is **provisional** and binds only at the Stage-12 atomic compare-and-swap, where git's ref CAS is the authority.
