...
7. **#4196** — `deploy.sh` **Check 63** (count-vs-structure lint), **enforce-mode narrowly scoped**, with a committed **51-entry baseline** at `core/deploy/allowlists/count-structure-baseline.txt`. Both a `# Check 63` def-block and a `log "Check 63:"` emitter, per the Check-57 roster contract.
