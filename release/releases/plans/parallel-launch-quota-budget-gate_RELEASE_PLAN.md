...
- **M3 Pattern Cache Scan:** Applicable confirmed patterns cited — the verify-before-recommend discipline / M1 Audit-Snapshot sub-mechanism (drove R-7/R-8 reconciliation: verify AC premises against canonical source before building); the intake-pre-rendered-artifacts inverse check (confirmed these issues are NOT already-shipped: grep shows no quota-gate content in current `hub-spoke-bridge.md`/`stage-04-planning.md`, so this is genuine net-new work, not verify-and-close); the reconcile-don't-annotate discipline (the stale ACs get reconciled to current state at Stage 5, not annotated-and-deferred).
...
- **M3 Pattern Cache Scan:** the issue-creation duplicate-discipline (don't collapse distinct tracked scope) and the issues-not-chat-prompts multi-phase rule (multi-phase work = issue-per-phase) both favor keep-separate. No confirmed pattern argues for merge.
...
| **(c)** | **Stage-6 AC reconciliations for #23 (applied at Engineering, DEV-10).** Four stale ACs in the #23 body are reconciled via `gh issue edit 23 --body` at Engineering Commit 0: (1) "extend `spoke-completion` event with optional `tokens_used` field" → "add a new `spoke-launch` event_type (subtype `quota-reservation`) per `pipeline-event-log-schema.md` § 3 + the `append-pipeline-event.sh` static mirror + ADR-026" (`spoke-completion` does not exist AND the `test-run` payload-key route is unwritable); (2) "`release-process.md § Procedure 2 § Parallelism Rules` table notes…" → re-pointed to `hub-spoke-bridge.md` Procedure 2 Step 5 (the table's only home; `release-process.md` has 0 hits); (3) "Mirror-pair sync (Check 9) enforced" → marked **N/A** (none of #23's target files is in Check 9's MIRROR_PAIRS set — 7 `core/rules/*` + `release/rules/release-process.md` + the OPERATIONS.md dual-write pair); (4) the `v12.13.1`-literal cutover token + the "A9" phase reference + the WARN→STAGGER routing + the batch-size `[CALIBRATE-AFTER-3]` target → reconciled to the no-version-token introducing-release/SHA cutover, the A6 (A2-sourced) phase, the PROCEED/SERIALIZE/DEFER/REDUCE-scope verdict hierarchy (STAGGER secondary), and the cumulative-draw-budget calibration target. | #23 rev-2 Stage 5 spec (sub-task #784) R-2/R-3/R-6/R-7; the live corpus (the `test-run` writer contract; the Check 9 MIRROR_PAIRS set; the A1-A5 live phase set; the `release-process.md` zero-hit grep). | Building to the as-filed #23 ACs would extend a non-existent event, author a note into a non-existent table, block Stage 8 on an impossible Check-9 criterion, and elevate a non-load-bearing mitigation (stagger) for the cumulative usage-window root cause. Reconcile-don't-annotate: correct the ACs to current state in the issue body + record here. |
