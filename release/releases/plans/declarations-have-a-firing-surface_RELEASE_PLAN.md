...
| #5825 | AC-2 | Run the Check-62 predicate over the edited standard: `deploy.sh --check` Check 62, or its extracted `_rr_compute_verdict` semantics | `CLEAN` with a pointer count of **6** — four pre-existing, plus Instance 1's runner pointer, plus Instance 3's, which slice 2 adds when it ships the detector (Δ9 → Δ12). Instance 2 remains a named gap and correctly carries none: it has no resolvable runner at all |
...
| #5825 | AC-4 | Read the standard's non-coverage clause and confirm it enumerates what the mechanism cannot detect | Five numbered non-coverage items present — the runtime-skip limit, the auto-memory-store limit, the payload-indistinguishability limit, the R1-granularity (anchor-presence, not predicate-completeness) limit, and the advisory deploy-time-only limit |
...
| #5826 | AC-1 | Seed a synthetic memory declaring a live-read obligation with an enumerated kind list and holding those kinds as values; run the Check-36 Class-6 arm-1 predicate over it. Control arm, same instrument, same target: the reconciled form of the same note | The seeded surface is flagged, naming the declared kind and its line; the reconciled form is evaluated and reads clean. The opposite verdict on two near-identical notes is the discrimination claim |
| #5826 | AC-2 | Read the class table in `core/disciplines/knowledge-architecture.md` § drift classes and the Check 36 header enumeration in `core/deploy/deploy.sh` | Both read six, the `external-target-referent-stored` row is present in the table, and the `{#drift-classes}` anchor is unchanged |
| #5826 | AC-3 | Run the Check-36 Class-6 arm-2 predicate over a synthetic `operator.toml` carrying `[trackers.<id>]` with only `id`, `platform`, `identifier`, `scope`. Control arm, same instrument, same target: the same subtable plus one key outside that set | The sanctioned subtable is silent — the `identifier` address does not flag, because it is on the whitelist rather than exempted. The out-of-schema key is flagged, naming the key and its line |
| #5826 | AC-4 | Run the Class-6 arm-1 predicate over an operator-side practice statement that names an external target and carries no live-read declaration | Silent — the file is not in the population at all, which is a distinct outcome from evaluated-and-clean and is reported as such |
| #5826 | AC-5 | Read the non-coverage enumeration in the Check 36 header block and in `knowledge-architecture.md` § 7.1 | Both state that the auto-memory store IS reached at deploy-check time on an operator machine and is structurally unreachable in CI, and both enumerate what is not reached: write time, an undeclared cache, operator config outside `[trackers.*]`, and the codified corpus |
| #5826 | AC-6 | Read the Class-6 fixture set in `core/deploy/tests/test_check36_drift_classes.sh` | The observed violation is present as the flagging fixture in its pre-reconciliation form — declaring the obligation and holding the same kinds as values — alongside its reconciled twin as a clean fixture |
...
|---|----------------------|-----------|-----------------|
| **V-1** | Check 62 resolves | The `_rr_compute_verdict` computation over the edited register | `CLEAN`, pointer count **6** — four pre-existing, plus Instance 1's (slice 1), plus Instance 3's (slice 2, the CIAC-2 affirmative branch). The count moved 5 → 6 at Δ12; a run still reporting 5 means Instance 3's pointer did not land |
