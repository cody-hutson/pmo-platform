...
| #5825 | AC-4 | Read the standard's non-coverage clause and confirm it enumerates what the mechanism cannot detect: `grep -c 'does NOT detect' core/standards/gate-efficacy-standard.md` · control arm, same instrument, same target: `grep -c 'Runner resolution'` → non-zero | Five numbered non-coverage items present — the runtime-skip limit, the auto-memory-store limit, the payload-indistinguishability limit, the R1-granularity (anchor-presence, not predicate-completeness) limit, and the advisory deploy-time-only limit |
