...
|-------|----|--------------------|-----------------|
| #5825 | AC-1 | `python3 -c "import re;t=open('core/standards/gate-efficacy-standard.md').read();print(len(re.findall(r'class 3-O', t)))"` then read the three appended register rows and confirm each of the three Evidence instances has one | 3 register rows present, one per Evidence instance; each states its admission, its runner-or-named-gap, and its disposition |
| #5825 | AC-2 | Run the Check-62 predicate over the edited standard: `deploy.sh --check` Check 62, or its extracted `_rr_compute_verdict` semantics | `CLEAN` with a pointer count of **5** (4 pre-existing + 1 new — Instance 1's runner pointer. Instances 2 and 3 are named gaps and correctly carry none: Instance 2 has no resolvable runner at all, and Instance 3's runner does not carry the predicate until #5826 lands, per Δ9) |
| #5825 | AC-3 | Read the shipped admission text and confirm the mechanism is a two-limb test plus a register row, not an exhortation: `grep -c 'L1′' core/standards/gate-efficacy-standard.md` · control: `grep -c 'L1 — verdict limb' <same file>` → non-zero, so a zero on the subject is a real absence rather than an unreadable file | `L1′` present; the register row is data and Check 62 is a computation over it; no new prose instruction to the agent is added anywhere in the diff |
| #5825 | AC-4 | Read the standard's non-coverage clause and confirm it enumerates what the mechanism cannot detect | ≥4 numbered non-coverage items present, including the runtime-skip limit, the auto-memory-store limit, the payload-indistinguishability limit, and the R1-granularity (anchor-presence, not predicate-completeness) limit |
| #5825 | AC-5 | Read the `external-target-referent-stored` register row and CIAC-3's disposition | **Explicitly excluded from #5825 and registered by it**; delivered by #5826 in the same PR |
...
| **V-3** | Plan conformance | `bash release/tools/verify-release-plan.sh release/releases/plans/declarations-have-a-firing-surface_RELEASE_PLAN.md` | All families PASS or a named SKIP. The `release-version-stamp` element is **correctly absent** — it fires only when `{{RELEASE_VERSION}}` is present, and a version-less plan emits no token |
...
| **V-6** | Register-row anchor liveness | For each new `runner-def: <path>::<anchor>`, assert the anchor is **absent** from the runner-definition file at `origin/main` and **present** after the edit | Absent-before / present-after for both new anchors — the pair is what proves the pointer asserts something the release actually added |
