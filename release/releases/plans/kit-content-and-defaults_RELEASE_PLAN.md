...
| **CIAC-1** — every authored entry carries a `source` | **PASS, over its full two-file population** | Structural extraction of every inline table inside `checks = [` / `kind_specific = [` across **both** manifests: **29** entries, **0** source-less. **The pre-existing source-less `story_points` FieldDecl is inside this population and no card authored it** — it is migrated here, so CIAC-1 turns green rather than staying red for a reason unrelated to this card. Four present-and-empty sets, **0** without a block-level `source`; **33** `source` keys total (29 entry + 4 block) |
| **CIAC-1 — sensitivity arm** | **fires on the pre-change tree** | The identical probe over `origin/main`'s manifest → **1** entry, **1** source-less (`story_points`), **11** present-and-empty sets with **0** block `source`. The probe discriminates; the clean is measured, not a dead reader |
| **CIAC-2** — no `source` value names a repo-internal artifact | **PASS on both arms** | Verbatim method: **0** hits · sensitivity `Scrum Guide\|INVEST\|Kanban` **23** · specificity (fabricated token) **0**. Stricter **prose** arm (*any* repo-relative path — `core/ release/ operations/ docs/ packages/` or a `.md/.toml/.yml/.py` extension) over all **33** values: **0**. **Third arm, because a control proves the reader is live and not that the population can exhibit the behaviour:** the same STRICT pattern finds **5** repo-path-shaped tokens elsewhere in the two manifests, so the zero is a property of the `source` **values**, not of a pattern that cannot fire here. Run under `python3`, never the local `grep`, which is `ugrep` |
...
| **CIAC-2** — no `source` value names a repo-internal artifact | **PASS on all three arms** | § Cross-Issue Acceptance Criteria's **verbatim** pattern over both content manifests → **0** hits. **Control arm:** `Scrum Guide\|INVEST\|Kanban` → kanban **15** lines, scrum **24**. **Third arm, because a control proves the reader is live and not that the population can exhibit the behaviour:** the same pattern matches a seeded positive line, and a stricter prose arm (any `core/ release/ operations/ docs/ packages/` prefix, a `.md`/`.toml`/`.py`/`.sh`/`.yml` extension, or `#` + two-or-more digits) over all **40** values returns **0** while returning **21** on the non-`source` lines of the same two files. Every load-bearing arm ran under `python3`; the local `grep` is `ugrep` and was used only to reproduce the plan's own verbatim invocation |
...
| **CIAC-2** | **PASS on all three arms** | The plan's **verbatim** pattern over both manifests → **0** hits (`grep` exit 1, the desired outcome). **Control:** `Scrum Guide\|INVEST\|Kanban` → **30** lines. **Third arm, because a control proves the reader is live and not that the population can exhibit the behaviour:** the subject pattern evaluated against a line built from this population's own `source =` shape carrying a repo-internal value **matches**, while the control pattern does not. Both engines agree at 30 — a cross-instrument reconciliation, not one reader trusted twice |
...
**Byte-identity, measured per manifest rather than in aggregate** (§ Deviation Log **DEV-21**): the base pack's content hash is identical at the merge base and at the branch tip; the two methodology packs differ, **+77 / −25** combined. **The specificity arm is the manifest that did not change**, so a reader reporting "changed" for all three is excluded.
