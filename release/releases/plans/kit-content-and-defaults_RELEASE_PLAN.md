...
**The per-file gate-coverage question was asked before any tool ran, because a prescribed set is still a fixed set.** For each file in the write set: which gate actually reads this file type? `core/packs/kanban/pack.toml` → **Check 75** reads it (`--validate-packs` over the live pack root) and **Check 63 does not** (its population is markdown). `release/releases/plans/…_RELEASE_PLAN.md` → **`lint_release_corpus.py --check plan-identity`**, **`verify-release-plan.sh`** and **`claim-version.sh --verify-stamp`** read it; **Check 63 does not** (frozen-artifact prefix); **Check 14** evaluates it for link integrity. Both answers are measured below rather than assumed, and the one `none` is stated as a `none`.
