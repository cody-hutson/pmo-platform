...
**Write set: one file, `core/packs/kanban/pack.toml`.** Diff **+14 / −6**, 62 → 72 lines, 6,607 characters. **Two commits, the Blocker string first and alone** — it is one line, and it is the thing that must not ship wrong. The plan-file rows in § Deviation Log and this block are the A2 *plan update* special sub-task, as each prior wave carried its own.
