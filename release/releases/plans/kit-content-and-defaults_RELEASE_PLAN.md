...
| `deploy.sh --check` **Check 63** (count-structure) | **`FAIL` at 3 paths — every one inherited, none this card's; and the pass on the `.toml` is VACUOUS, stated as such** | The three named paths are `operator-instance-home-and-isolation-key.md:128,:182` and `release-notes-standard.md:527` — the same pre-existing baseline wave 2 returned the check to. Proven inherited rather than inferred: `git diff origin/main..HEAD` over both files is **empty**, so they are byte-identical to `main` and the verdict on them cannot have been introduced by this branch. **The honest limit:** Check 63's population is `git ls-files "*.md"` (its own DENOM reads `files=904`), and `core/packs/scrum/pack.toml` is **not in it** — the check never examined this card's deliverable, so a clean verdict on it would be a guard passing on empty input. Control arms confirm the scope rather than assume it: the release plan `.md` **is** in the population (1) and `git ls-files "*.toml"` **does** see the pack (1), so the exclusion is a property of the glob, not of a dead reader. **The plan-file rows this card writes ARE inside Check 63's population**, and the check was re-run over them |
...
**One prescribed tool could not grade this card, and saying so is the point.** Check 63 is the tool that caught wave 2's defect, and it reads `*.md` only. A card whose deliverable is a `.toml` gets **no** count-structure coverage on its deliverable — the diff-vs-design reconciliation (**+58 / −19**, 128 → 167) and the structural extraction that reconciles 21 checks + 8 FieldDecls + 4 empty sets = 33 `source` keys are what stand in its place, and they are stated as substitutes rather than as the check itself.
