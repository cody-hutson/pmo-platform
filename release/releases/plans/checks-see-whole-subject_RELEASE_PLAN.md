...
|---|---|---|---|
| **#4720** | 2 | **`10638`–`10687`** as landed; **now `10779`–`10828`** after position 5 (see the shift note below) | Inside the Check-56 M3 leg, entirely AFTER the `C56-EMIT-END` sentinel — recorded at `:10615`, **now `:10756`**. Anchored on the `CAVEAT:` comment text and on the existing `awk -F'\t' '$1=="…"'` reads, never on a line number. |
| **#4734** | 5 | Six regions, all disjoint from the above: **`2183`–`2344`** (the two new top-level helpers) · **`4601`–`4611`** and **`4645`–`4663`** (Check 5(d), per D-13) · **`9020`–`9058`** (Check 35 in `cmd_check`) · **`9126`** (one line) · **`14124`–`14154`** (the `c35r` mirror). File grows 14,218 → 14,359 lines. | Every site resolved by inline marker: the `# ─── Skill package content-freshness (Check 7 …` neighbour for the helper insertion, `# --- modes: set-equality`, `# Check 35 — Mode-invocation drift`, `log "Check 35:`, and `# --- Mode-invocation drift (Check 35) ---`. No line number was used to locate an edit. |
