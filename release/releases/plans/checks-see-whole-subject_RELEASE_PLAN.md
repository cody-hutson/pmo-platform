...
| Doc-link integrity | integration | `deploy.sh --check` Check 14 — **OK, no broken cross-refs in scope.** |
| ADR number integrity | integration | `check-adr-numbers.py` — **PASS**, 144 ADRs contiguous `001..144`, no duplicates. ADR-154 is next-free across both ADR directories; no gap, no duplicate. |
| ADR durability lint | integration | `check-adr-durability.py` — **ADR-154 clean.** One R2-COUNT finding on it was **self-caught and fixed** (a live corpus count in durable prose, re-anchored historically); the 2 remaining findings are pre-existing on another release's record, so the lint is not returning an empty result. |
