...
|---|---|---|---|
| #6298 | AC-1 | Mutate `_g3_14_compute_verdict`'s floor comparison `>=` → `>`, run `bash core/deploy/tests/test_check73_bundle_metrics_discrimination.sh`; restore and re-run | Seeded run exits **non-zero**; control run exits **0** with four arms executed and one opposite-verdict pair per gate |
| #6298 | AC-2 | Open the PR; read the checks list | Contexts **"Pre-merge load-bearing check subset"** and the `install-tests` shell-tests job both present and reporting (both workflows filter-free, so neither can be absent) |
| #6298 | AC-3 | Re-run the workflow-denominator probe | Target **≥ 1** (was 0); sensitivity `deploy.sh` > 0; specificity `g3-99` = 0 — all three reported. Strengthened arm asserts R1 ∧ R2 reachability with an inverting control |
| #6298 | AC-4 | Read the two new coverage-register rows | `posture` reads **`required (warn-mode-initial)`**, not `advisory`; each names its CI job |
| #6298 | AC-5 | Inspect the diff for `--check` invocations added to any workflow | **0** — only the existing `--check-required-subset` and the fixture suite are invoked; no full `--check` sweep enters CI |
| #4214 | AC-1 | Read each gate's declaration; assert the declared sink is in the written-sink set | Every declaring gate writes, or carries a recorded deadline-armed deferral |
| #4214 | AC-2 | Locate the written-sink convention clause in `gate-efficacy-standard.md` | Convention clause present |
| #4214 | AC-3 | **Re-rendered at Stage 5 (D-9)** — the original form was unsatisfiable | Deadline-arm reader resolves against #6298's Check 73 entry point |
| #4214 | AC-4 | Re-run the register evaluation | Every row resolves to flipped, or to a deferral whose blocker is no longer "signal not derivable" |
| #4214 | AC-5 | Read the register post-change | One row per cohort member |
| #5588 | AC-1 | Detect unbounded direct appends to the warn log in `core/deploy/deploy.sh` | Zero unbounded direct appends · **control:** `pmo_instance_path` → 19 refs (non-zero) |
| #5588 | AC-2 | Drive appends past the ceiling in a sandbox | Size / line count stops growing |
| #5588 | AC-3 | Assert drain reads survive a rotation boundary | No records the drain needs are lost (CIAC-1) |
| #5588 | AC-4 | Read the append path | Silence removed, or documented accepted risk |
| #5588 | AC-5 | Run `./deploy.sh --check` | Green |
| #4751 | AC-1 | Read the register | Checks 66 / 67 each carry a decision + finding count + measured population |
| #4751 | AC-2 | Enumerate the sibling ids against register rows | All covered — **re-scoped to 8 undisposed + 3 cross-referenced** (Stage-4 R6) |
| #4751 | AC-3 | Fixture-fail any gate flipped to enforce | Real failure demonstrated — **N/A if no gate flips**, recorded as such rather than passed |
| #4751 | AC-4 | Read each held row | Names the ending condition **and** the sink (CIAC-3) |
| #1686 | AC-1 | Read the shared warn log; count by row shape | **Already MET at plan time** — 45 days; gating 148,324 / recommend 31,426 / advisory 302 |
| #1686 | AC-2 | Classify milestone-scoped gating rows | Zero unresolved false positives; recommend-tier counted separately and excluded |
| #1686 | AC-3 | Run the shipped Check-22 region against the deploying milestone | Post-filter finding count 0 · **control:** seed a defect → count > 0 |
| #1686 | AC-4 | Re-run the evaluation region over #1686 itself | **Already MET at plan time** — 0 findings (control 190/190 rows matched the issue-# shape) |
| #1686 | AC-5 | Drive all four resolver verdicts including a deliberately-invalid case | Invalid case BLOCKS |
| #1686 | AC-6 | Read the disclosure line | Names the intended release + milestone-set denominator |
| #1686 | AC-7 | Seed a structural G1 defect, run `--check` at `enforce`, revert | Blocks on seeded; passes clean |
