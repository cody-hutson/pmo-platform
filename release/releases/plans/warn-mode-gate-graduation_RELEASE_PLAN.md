...
| #4214 | AC-4 | `grep -c "SKILL_SUITE_GATE_ESCALATE_DAYS" core/standards/gate-efficacy-standard.md` — the G3-14 / G3-15 register row names the committed constants a reviewer can read, so its ending condition no longer rests on a git-ignored drain. Paired arm: `grep -c "^readonly SKILL_SUITE_GATE_" core/deploy/deploy.sh` — the constants are declared in the tracked script, not in operator config. Specificity arm: `grep -c "ZZZ_FABRICATED_TOKEN_CONTROL" core/deploy/deploy.sh` returns 0 | register row **1** · tracked constants **4** · specificity **0**. The row names its blocker and its exit criterion reads committed state, so it is no longer "signal not derivable" |
