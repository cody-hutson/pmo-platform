...
| #5592 | AC-4 | `bash core/hooks/tests/block-egress.test.sh` — arm `AC-E007-D5a` (structural census of `core/config/allowlists/egress-allowlist.txt`, token-resolved), arm `AC-E007-D5b` (behavioural agreement), and the header-clause arms `AC-E007-D11*`, `AC-E007-D12*` and `AC-E007-D13*` (discard, trim, path-site guard — DEV-31) | D5a: 15 of 15 managed rows declared, 0 undeclared, 0 violations · control: the same census over the pre-change shape (directives stripped) returns 15 undeclared, and over a planted mis-declared fixture returns at least one violation · D5b as AC-1 · D11–D13: a directive discarded by a blank, a comment or a second directive leaves its row as the header says, a trimmed value still declares its row, and a glob first segment is never a gh api path candidate, each with its control · the header states the two-domain grammar the matcher applies |
