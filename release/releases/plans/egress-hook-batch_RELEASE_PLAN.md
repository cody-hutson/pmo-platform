...
| **DEV-23** | **The pre-fix scan did not apply its own terminator set, and the replacement does.** V6b's first run against the linear scan disagreed on six seeded commands, each with `(` or `>` directly after `<<`. A discriminating run — seven edge cases, prediction written first — refuted a process-substitution explanation (`<<end`, `<<a/'b'`, `<<6'x'` and `<<3'x'` agree) and confirmed the narrower one: as bash 3.2 ran the one-bracket set, a delimiter word did not end at `>`, `(` or `)`. The shell's own tokenizer ends it there, so each such command is a syntax error to the shell and now reads `none` where it read `unquoted`. The reference model states the set one escaped terminator at a time | Implements the design's stated scan, which the first build did not; Stage-6 engineering judgement, measured | **APPLIED — minor** at `150265eb`. Scope: only the `heredoc` member of commands carrying `<<` directly followed by `>`, `(` or `)`; such commands are shell syntax errors, so their `shell_parse` reads `error` either way and no classification by `shell_parse` moves |
