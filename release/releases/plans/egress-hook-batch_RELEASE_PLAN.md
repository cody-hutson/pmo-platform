...
**CIAC-1's record-contract arm is authored with #6201's slices (2a/2b).** Its `unparseable` limb asserts the FS-6201 fields, which do not exist before those slices. #5592's limb — a `not-allowlisted` record still carries the denied path after the scope change — is pinned from slice 1a by arm D1's evidence assertion (`AC-E007-D1e`).
...
*Authored at Stage 6 Phase C1 per RELEASE_PROTOCOL § Change Description Protocol, by #5592's Engineering spoke; #6201's Engineering spoke refreshes it when its slices land. Operator-facing.*
...
| #6201 | every BLOCK-EGRESS-007 `unparseable` refusal record carries non-identifying structure, in both writers | PENDING — the next Engineering spoke |
...
**CHEAP — HIGH confidence.** `git revert -m 1` of the merge, then republish the hook bundle and let the update path regenerate the allowlist. A directive is a comment line to the prior matcher, and that is measured, not argued: the `origin/main` hook grants exactly the same verdicts over the new-format file as over the old one (0 of 30 differ). The claimed version tag is retained and recorded, never deleted.
...
- Milestone: `egress-hook-batch`.
- Decision record: {{ADR:egress-allowlist-rows-declare-their-match-domain}}.
