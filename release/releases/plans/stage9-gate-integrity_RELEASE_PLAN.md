...
**Row-level census under the shipping extractor:** 2,042 declaration rows across the 117 — 347 unconditional ADD obligations, 5 conditional ADDs, 60 excluded (READ / non-scope / rename), 951 uninterpreted (bare, no marker), and 84 marked-but-pathless rows. Path forms: **27 glob-bearing tokens across 15 plans**, which is what makes the glob arm mandatory rather than optional.
...
**Cutover posture, stated explicitly.** Forward-only and **run-target-triggered** — the check fires only when the executor is pointed at a specific plan and never sweeps the corpus, so the 71 plans that would report uninterpreted rows are judged only when someone deliberately points the tool at them. Under CDF-2 row-granularity accounting, 46 of 117 are coverage-PASS-eligible today, 49 carry a mix of marked and bare rows, and 20 are entirely bare; all 69 emit a **named** `fcm-rows-uninterpreted:<n>` SKIP rather than a silent zero. **No historical plan is red-lined and no plan is re-authored by this release.**
