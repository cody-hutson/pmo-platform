...
| `core/deploy/deploy.sh` | EDIT | the deployment-emitter card (flag, emitter, observer, self-test group DS), the close-ordering gate card (Procedure-7a fixture) |
...
**Ten unconditional ADDs, every one a concrete resolvable path.** This is deliberate and it is the release dogfooding its own gate. The matrix-delivery gate card ships the check that compares a plan's declared ADDs against the merged diff; a matrix that declared its ADD set only in prose, or only as a glob, would make that reflexive run **vacuous** — a glob path form yields a guaranteed FAIL against the shipped path taxonomy, and a marker-less matrix yields an intent-undeclared verdict, which is a SKIP rather than a PASS. Naming the nine fixtures individually plus this plan file creates a real obligation the new gate can judge. **The nine fixture filenames are the matrix-delivery gate card's to deliver**; a rename is an `amend`-class Deviation Log entry, never a silent substitution.
...
```
release/ADRs/ADR-NNN-<slug>.md                        ADD   — CONDITIONAL on D-ADR-Disposition = author-retroactively
core/config/allowlists/script-execution-allowlist.txt EDIT  — CONDITIONAL on any new tracked *.sh shipping
```

- **The ADR number is deliberately unallocated.** Allocating before the disposition is rendered would write a path into the machine-readable set that can never match — precisely the false-positive class the matrix-delivery gate exists to prevent. Number at Engineering time via `release/tools/renumber-adr.py --next-free`, which is global-monotonic across **both** `core/ADRs/` and `release/ADRs/`. Per ADR-115 an unmerged branch claim is **advisory**, and next-free is `anchor(origin/main) + 1`, never `max(claimed_set) + 1`: a gap blocks the repository, while a duplicate inconveniences one branch and is resolved by tooling at Stage 12. The in-flight sibling adds no ADR file, so no unmerged sibling claim is at risk.
- **New-executable companion obligation: does NOT fire.** No tracked executable shell script is an ADD in this matrix. The nine fixtures are `.md` and `.tsv`; the test script already exists. Per the Stage-4 authoring contract the obligation attaches to an `add` row for a tracked executable, so no allowlist row is owed. Should any card diverge and author a new `*.sh`, the allowlist row plus all four invocation forms is **mandatory in this release**, not a follow-up.
...
- **The matrix-delivery gate card.** A plan declaring an ADD that the merged diff does not contain fails; a conformant plan passes; a CONDITIONAL row that did not fire is not a violation; a marker-less matrix reports an intent-undeclared verdict rather than a zero-obligation clean. The corpus survey behind the path-form taxonomy must be re-run with the **shipped fence-blind extractor**, not a fence-aware substitute, and the population denominator must be **pinned with a stated method** before any percentage is relied on — four independent measurements returned 142 / 127 / 126 / 116. A **glob arm** joins the path-form taxonomy; 18 glob tokens across 8 plans make it an established authored form.
