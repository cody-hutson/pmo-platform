...
| **DEV-8** | **The AC-12 arms are SOURCE-ORDER assertions over `update.sh`, not a live update run.** Driving `./update.sh` end to end needs a full sandboxed instance and exits in preflight against anything less, so an arm built that way would report on preflight rather than on Phase 5c | Engineering judgement, with the bound written into the suite source | **APPLIED — minor, and not a weaker proxy for the property under test.** The defect is an ORDERING defect: where the deployed-flag read sits relative to the `rc` branch IS the bug and IS the fix. The arms were observed RED by restoring the pre-fix file beneath them. **Live end-to-end exercise of Phase 5c is a PERMANENT ENVIRONMENTAL RESIDUAL, not deferred stage scope** — amended after Stage 8 under D7, replacing the earlier "Stage-7/8 scope" framing that Stage 7 and Stage 8 each correctly declined to discharge. The reason is the environment, not a missing permission: a direct `./update.sh` run mutates the operator's live workspace, which no verification stage may do, and the ground does not change with the stage that reads it. The source-order arms (`17a`–`17d`, with `15f` tying the caller's constant to the status the delegate actually returned) remain the release's evidence for AC-12 |
...
| **DEV-12** | **One post-Stage-9 [ADJUST] carrying three items, delivered as a fifth Engineering iteration after Stage 8 had rendered ACCEPT at `0ab722ae`.** (1) **R-1** — the suite's terminal expression consulted `FAIL` alone, so a run with skipped arms exited 0 and the required check read GREEN with AC-1 / AC-2 / AC-4 / AC-9 unmeasured (observed at `63817750`: `72 passed, 0 failed, 5 skipped`, job SUCCESS). The suite now exits non-zero on `SKIP > 0` unless `REFRESH_HOOKS_ALLOW_SKIP=1` permits it on purpose; the banner keeps naming the unmeasured criteria and gains one line saying the run is failing because of them and naming the permit. (2) **DEV-8** amended from "Stage-7/8 scope" to a permanent environmental residual with its actual reason. (3) The Verification Plan's prose said the `bash` rows read as a SKIP or ERROR "naming the verb"; the executor emits `unclassified-method (no family match)`, which names nothing — one sentence corrected | Operator decision **D7** at the Stage-9 gate: GO **conditional on** hardening R-1 before merge (carried as `AI-001`), with Stage 7 and Stage 8 re-verifying at the hardened head because Stage 8's ACCEPT is bound to `0ab722ae` and does not transfer. Items 2 and 3 are Stage-8 findings the gate consumed — first routed to the Stage-13 chore PR to avoid moving the head, then folded into this iteration once D7 moved the head anyway. A **Tier-1 [ADJUST]** under the approved plan, same class as DEV-10 | **APPLIED.** R-1 was proven RED-first on this tree, every total observed and none derived: with the environment probe forced to its not-a-checkout branch (`GIT_DIR` pointed at a path that does not exist — no tool substituted, no tree touched), the unchanged head read `72 / 0 / 5` **exit 0**; after the change the same forcing read `72 / 0 / 5` **exit 1** without the permit and `72 / 0 / 5` **exit 0** with it; the ordinary full-history run reads `77 / 0 / 0` exit 0, so the rule does not fire where CI runs it. The single CI invocation site (`install-tests.yml` `shell-tests`, `fetch-depth: 0`) was re-verified: no other workflow, glob runner or regression-member list invokes the suite. § Verification Evidence carries the row. The `fetch-depth: 0` line is now guarded by consequence rather than by comment: removing it turns the required check RED instead of silently green |
...
| #5251 | `core/standards/subagent-security-posture.md` — § 3.1, the canonical home of the coverage-boundary statement the registry entry restates | **UPDATED** | `f0a2146d` | The currency caveat, in the standard's own register, with the same substance as the registry entry — **measured, not asserted**: 15 of 15 load-bearing phrases present in both files, with a control phrase unique to the standard's own § 3.1 at 0 / 1. Pulled in at the Stage-9 gate (D8, DEV-11); the FOUR cardinality is untouched here too, and the paragraph says so |
...
| **Suite exit rule — R-1, RED-first** (iteration 5, D7's condition) | Skipping runs manufactured on this full-history tree by forcing the environment probe to its not-a-checkout branch — `GIT_DIR` pointed at a path that does not exist, so `rev-parse --git-dir` fails; no tool substituted, no tree touched. **Unchanged head `0ab722ae`:** `72 passed, 0 failed, 5 skipped`, **exit 0** — the defect, reproduced (same five arms and counts as CI at `63817750`). **After `23339701`, same forcing, no permit:** `72 / 0 / 5`, **exit 1**, banner line `FAILING this run because of it … REFRESH_HOOKS_ALLOW_SKIP=1`. **Same forcing, `REFRESH_HOOKS_ALLOW_SKIP=1`:** `72 / 0 / 5`, **exit 0**, banner line `the skip is PERMITTED for this run`. **Ordinary full-history run:** `77 passed, 0 failed, 0 skipped`, exit 0, no banner — the rule does not fire where CI runs it. `FAIL` is 0 in every skipping run, so the non-zero is attributable to `SKIP` alone. Invocation-site re-verification: `test_refresh_hooks.sh` appears in exactly one workflow step (`install-tests.yml` `shell-tests`, `fetch-depth: 0`); the regression runner's member list excludes it; no glob runner over `core/deploy/tests/` exists |
...
**An unmeasured arm fails the suite — the same rule the publisher now obeys, applied to its own evidence.** The regression suite's five history arms skip, loudly, where the checkout carries no history to plant from. Skipping was honest; exiting 0 while skipping was not, and it was observed rather than hypothesised: with the CI job's `fetch-depth: 0` absent, the required check read GREEN with four acceptance criteria unmeasured. The suite now exits non-zero on any skipped arm unless a runner sets `REFRESH_HOOKS_ALLOW_SKIP=1` on purpose — so the one YAML line that keeps those arms alive is guarded by consequence, not by a comment. Added after the Stage-9 gate as the operator's condition on GO (D7).
...
The regression suite gains one exit condition a caller might notice: a run that skipped any arm now exits non-zero even with zero failures. The CI job that runs it checks out full history, so nothing changes there; a runner that deliberately checks out shallow — or a tree that is not a checkout — must set `REFRESH_HOOKS_ALLOW_SKIP=1` to keep the prior exit-on-`FAIL`-alone behaviour, and the skip banner names that variable when it fires.
