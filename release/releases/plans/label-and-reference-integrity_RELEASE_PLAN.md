...
| #5058 | AC-1 | Trace the outbound request from `core/deploy/tools/check-label-parity.py`; assert the endpoint is the REST labels path | No GraphQL call on the label-fetch path | **[AMENDED — Stage 7 DT F-05] The GH_DEBUG trace method is UNEXECUTABLE as specified** — `capture_output=True` swallows the trace and yields a 0-byte capture, so the probe cannot answer the question. Grade instead on the source: assert the live invocation is the REST labels path and that no `gh label list` call survives on an execution path, each with a firing control.
...
| #4223 | AC-4 | `grep -c 'type:feature' release/tools/compute-release-velocity.sh` | **0** — the label does not exist in the live set, so an arm naming it can never fire | **[GRADING NOTE — Stage 7 DT F-07] `grep -c` prints `0` and EXITS 1 on zero matches**, and the plan executor maps exit 1 to FAIL — so this row fails precisely because it produced the zero it requires. Grade on the printed COUNT, not the exit code; or invoke as `grep -c … || true`. A FAIL verdict on this row from the executor alone is an instrument artifact, not a defect.
