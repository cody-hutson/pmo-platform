...
# Added at Stage 8 by operator ruling. Both restate G1-03's superseded
# bracket-only predicate and neither line carries the `G1-03` token, so a
# token-keyed sweep cannot see them; they were reported at Stage 7 rather than
# fixed because neither path was declared. See § Deviation Log.
core/schemas/stage-io-contracts.md                                          edit
release/references/pipeline/stage-01-intake.md                              edit
...
**Counts.** **32 distinct `edit` paths** (30 source + 2 package artifacts) · **3 `add` path groups** (one new tool, two fixture directories — `event-record/` 11 files, `prov-*` ~8 files) · **6 ADR `add` rows** · **3 READ-only** · **6 explicit non-scope**. One `.skill` package rebuild (**1 of 55**).
...
| `core/schemas/stage-io-contracts.md:50`, `release/references/pipeline/stage-01-intake.md:90` | **FCM ADDED at Stage 8 by operator ruling (+2 `edit` rows, under #4923) — both fixed** | Surfaced at Stage 7 while grading CIAC-2 by enumeration rather than match count. Both restated G1-03's superseded bracket-only predicate (*"≥1 evidence-labeled claim"*), and both are invisible to a `G1-03`-keyed sweep because neither line carries the token — which is why Stage 7 did not see them. Stage 7 reported rather than fixed them because neither path was declared; the Stage-8 ruling is to make reality match the documented design rather than narrow the design, so both are now FCM rows and both now cite § Gate 1 G1-03 instead of restating it. `stage-01-intake.md:90` both **cited and restated**, the case #6106 caveat 3 names — its citation is retained and only the restatement is replaced. `ADR-144:35` carries the same string as an **archival blockquote** in its Context section and remains correctly untouched — editing it would falsify the decision's own history (caveat 1); it is classified here, never silently skipped. |
...
| § File Change Matrix **Counts** line | **CORRECTED — wrong at authoring, then drifted; 4 off before this amendment** | The line read *26 distinct `edit` paths (24 source + 2 package artifacts)*. Re-derived by parsing the fenced matrix at **every** revision of this file rather than by trusting the prose or by assuming the drift began with the appends: the matrix held **28** `edit` rows (26 source + 2 package) the moment the line was authored at `b2baa7bb`, so the figure was **wrong on arrival by 2** — the author appears to have taken the *source* count as the total and then subtracted the two package artifacts a second time. It then drifted a further 2 as two operator-approved rows were appended without re-derivation: `core/deploy/tools/check-issue-ref-validity.sh` at Stage 7 (`df170ad2`, 28→29) and `core/deploy/tests/test_g1_form_family.sh` at Stage 6 (`226372e3`, 29→30). True state immediately before this amendment: **30 (28 source + 2 package)**; after the +2 above: **32 (30 source + 2 package)**, all distinct. The other counts re-derive unchanged (3 non-ADR `add` groups + 6 ADR `add` rows · 3 READ · 6 non-scope). Recorded rather than silently normalised: a self-count inside the matrix that Stage 9 grades scope against is a governance surface, and *when* it broke is the part a reviewer cannot recover from the corrected value alone. |
| Stage-8 rework (acceptance-review findings) | **CONSOLIDATED REWORK PASS** | One batched pass closing the Stage-8 acceptance findings across four surfaces — operator-ruled, on the principle *make reality match the documented design, do not narrow the documentation*. **#4923 declared-scope gap:** the `G1-03 evidence label` row in `gate-criteria-spec.md`'s G1 Enforcement-Layer Split is named in #4923's own Affected Files and had not moved; its Criterion cell takes the same treatment `0c5fd95d` applied at `:328`. Its Layer-A cell is deliberately untouched — ADR-144 records Layer-A validation as unchanged by this card. **#4923 restatements:** the two paths reported-not-fixed at Stage 7 are now FCM rows and both cite § Gate 1 G1-03; `ADR-144:35` stays untouched as an archival blockquote. **ADR-144 claim integrity:** `:103`'s flip count re-derived to **8** (was 9 — the nine-body out-of-section cohort and the flip set are different sets; shape (b) rescues one of the nine) and date-anchored; `:95`'s *zero restatements survive* re-derived FALSE for six of the seven surfaces it names, and corrected to name the six deliberate author-facing reproductions rather than strip guidance #4923's own Documentation Impact requires. **ADR-143 + milestone #350:** trigger (b)'s member count re-derived to **3** at Commit 0, at the Stage-7 head and post-amendment — it FIRES under both the old seven-member and new six-member sets, because `RELEASE_LOG.md` is not a matrix row at all. **The Release Class is unchanged**: trigger (a) fires independently at 6 stage files (5 before this pass added `stage-01-intake.md`). **ADR-145 charset:** the registry parser widened `[a-z0-9_-]` → `[A-Za-z0-9_-]` so the recorded superset contract holds by construction rather than by the coincidence that every declared token is lowercase; proven non-regressive with a byte-identical 5-key parse under both charsets plus a firing sensitivity arm. No card scope was implemented beyond these findings. |
