...
| 32 | § ADR Allocation records five reserved slots as sufficient — *"six proposals resolving to five distinct decisions after dedup"* — and the ADR-159 row states the table is **deliberately NOT extended** | **No dedup materialized; the table is extended to ADR-160.** All six ADR-warranted proposals resolved to six distinct decisions. #5253's — that when one engine hosts two invariants, each population belongs to its *rule* rather than to the engine — dedups into none of the five landed records and **extends** ADR-119 rather than superseding it (ADR-119 stays `Accepted`). Extension executed per the rule the ADR-159 row points at: oracle re-run against `origin/main` **and** both branch ADR dirs scanned. Sibling branch `origin/release/pipeline-spec-self-consistency` also claims 148; that claim is **unmerged and therefore advisory** on D-CR2's own ratified basis, and stepping to 149 would open the numbering **gap** D-CR2 names as the blocking failure while a duplicate is tooled | Engineering slice 9, #6147 |
...
| 43 | The ADR number space is stable enough for a per-collision recovery to settle | **Contended beyond what per-collision recovery can absorb — a standing finding, not a one-off.** `origin/main` moved **twice inside this single Stage-7 pass** (`b528f47d` → `0ae7a583`), and across the release the same six records have now been renumbered by **three** separate sibling merges: `143–148 → 149–154` (recovery 1), `149→155` + `150→156` (recovery 2, this pass), and a third that moved `151–154 → 157–160` (executed at Stage 12, per the deferral this row recommends). Each recovery is individually correct and each was invalidated by the next. The mechanism is behaving exactly as ADR-115 says it must — a number is allocated at authorship and claimed at merge — so this is **contention, not drift**, and the remedy is not a better renumber tool. It is a sequencing decision: serialise the concurrent releases, or accept that a long-lived release branch re-numbers once per sibling merge and defer the citation sweep to a single pass at Stage 12. Recorded so the cost is visible to whoever makes that call. **The deferral was taken:** Stage 12 integrated `origin/main` once and swept citations once, and the mechanical sweep's own cost surfaced there — it rewrote an `ADR-151` token inside this log's *historical* prose, producing the incoherent range `ADR-157–154` in row 42. The tool exempts the § Renumber log and the `## Status` provenance notes from its sweep, but a Deviation Log row is neither, so narrative that RECORDS an old number is swept as though it CITED one. Caught by a structural probe for ranges whose low end exceeds their high end, and repaired in the same commit | Stage 7 DT pass 2 · executed Stage 12, #6149 |
...
- **Two records generalize past this release.** ADR-159 establishes that an invariant spanning several implementations must be executable rather than annotated; ADR-160 establishes that when one engine hosts more than one invariant, each population belongs to its rule rather than to the engine. Both are cited rather than restated by the surfaces they govern.
...
- Architecture records added by this release: ADR-157, ADR-158, ADR-159, ADR-160, ADR-155, ADR-156 — see § ADR Allocation.
