...
| **Status** | Executing — Stage 6 Engineering: Engineering Commit 0 (this file) landed; slices 1–7 pending |
| **Branch** | `release/telemetry-is-computable` |
| **PR** | Draft, opened at Engineering Commit 0 (SINGLE topology); its number is recorded in the Commit-0 output on #7738 and written into this cell by the next slice; it transitions to ready-for-review at the Stage-9 gate |
| **Milestone** | `telemetry-is-computable` |
...
| **DEV-12** | **Phase A6.5 reviews of #5245's, #5467's and #6252's designs** — review #7874, comment 5831451771 (Minor: PR-1, FM-1, CD-1); review #7885, comment 5831764850 (Majors PR-1 and FM-1 by decision 7; Minor: PR-2, CD-1); review #7886, comment 5834019858 (Majors by decisions 2, 3, 9 and 12; Minor: PR-3, FM-3, FM-4) | Scope-lock decisions 2, 3, 7, 8, 9, 12 and 13 | **ADOPTED:** #7874 FM-1 and CD-1 (slice 4); #7885 PR-2 (slice 6); #7886 PR-3 (AC-3's three rates), FM-3 and FM-4 (slice 7). **RELAYED:** #7874 PR-1's reworded R3 relay to ms#395 and ms#334 (its Lane-1 restatement is carried in R8). **ROUTED → intake at Stage 13:** #7885 CD-1, the per-subtype home-stage table (also Stage 9 disclosure 1); #7886's R-a (the `hub-session-continuity.md` precedence note and `:172`, outside the write set) and R-b (keeping release-level markers off the anchor subtypes; Stage 9 disclosure 3) |
...
| Issue | Declared docs (the body's Documentation Impact field) | Status at Commit 0 | Lands in |
|---|---|---|---|
| #5586 | `release/references/standards/close-class-telemetry.md` — the value domain gains the caller-omission value | pending | slice 1 |
| #7432 | the tool's usage text, if it describes C4's join; no change to the close gate's documented predicate | pending | slice 2 (the tool header) |
