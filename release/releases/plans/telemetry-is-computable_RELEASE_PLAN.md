...
| **Slice 2 (#7432) — commits** | `75ce7445` (the seven ADD fixtures) · `e2d72f93` (the tool and the four prose-only fixture edits) · the slice's final commit (this plan). Change units: sub-issues #7921–#7927 under #7432, each marked closed with its landing SHA (DEV-26) |
| **Slice 2 — #7432 AC-1** | **RED** with the new fixtures and arms on the unchanged engine: `bash release/tools/check-event-record-integrity.sh --self-test` → 55/83. The limb-(b) arm for `fixture-c4-rel-a` AI-001 fails at PAST, MID and FUTURE, and so does the AC-1 control's "the other release's finding remains": a bare-id join lets `fixture-c4-rel-b`'s terminal event satisfy A's stale row. **GREEN**: 83/83. A's stale AI-001 is reported under its own release's key, and with A's own terminal event present (the control log against the same tree) nothing is reported for A in either bucket, while B's finding remains |
