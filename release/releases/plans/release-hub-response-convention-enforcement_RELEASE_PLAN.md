...
- **Stage-6 re-check (2026-07-27, at `36c80813`) — point-in-time, since superseded: the population is NO LONGER empty.** Three open PRs existed: **#4133** (`release/v3.97-build-philosophy-corpus`, ready-for-review), **#4167** (`release/v3.97-decision-audit-and-learning`, draft), **#4166** (dependabot, non-release).
- **File contention with this release: ZERO.** Neither #4133 nor #4167 touches `release/skills/release-hub/**` or `packages/release-hub.skill*`. Verified by enumerating each PR's changed-file set. **Still ZERO at the Stage-7 re-check below.**
- **Version-slot contention at the Stage-6 reading: REAL** — both #4133 and #4167 displayed `v3.97`. **Superseded: `v3.97` is now claimed** and this release's next-free has moved to `v3.98`. See the Stage-7 re-check below, **R12**, and § Version Determination.
- **Stage-7 re-check (2026-07-27, at `origin/main` = `c10de940`): the `v3.97` contention resolved by a CLAIM, not by withdrawal.** **#4133 is MERGED** (merge-commit `c290126c`, 2026-07-27T13:07:24Z) and **claimed `v3.97`** — signed tag `505cdbbd`, RELEASE_LOG row `| v3.97 | build-philosophy-corpus | … | VERIFIED | 2026-07-27 |`, user-facing notes at `release/releases/notes/v3.97_RELEASE_NOTES.md`. **Its Stage-12/13 close-out completed correctly** — tag, DEPLOYED row, VERIFIED transition, and notes are all present. **#4167 remains an OPEN draft still carrying `v3.97`** in its branch name and plan-file path, so it now collides with a *claimed* number rather than a provisional one; it independently adds a duplicate **`ADR-098`** (`ADR-098-decision-audit-host-qa-auditor-mode-j.md`) against the `ADR-098-portability-seventh-first-class-value.md` that landed on `main` via #4133 — next-free is **ADR-099**. Both are #4167's to resolve and neither touches this release's file set.
- **Re-check obligation stands:** Stage 9 Phase A6.5 (PRIMARY, HALT-eligible) and Stage 12 Phase A.5 (post-GO ultima ratio) must both re-run this audit. Every reading recorded here — Stage 4, Stage 6, and Stage 7 alike — is a point-in-time reading.
...
Left uncorrected this would have produced a mis-named Commit-0 artifact and a probable Stage-12 rename collision — a live risk, since at the Stage-6 reading two concurrent releases carried `v3.97`-stemmed plan files, one of which (#4133) has since merged and claimed the number (see R12).

### Version Determination

**Bump-class: `minor`.** Protocol-text modification within an existing structure plus reference-document updates — both map to `minor` in the Bump-Class Selection Guide. Under the two-phase allocation rule this declares the **floor**, and binds no concrete number — **with one named exception, the `SKILL.md` frontmatter literal recorded at D-VersionBinding below.**

**Commit-0 version re-verify (2026-07-27, at `36c80813`) — result: PROCEED.** Point-in-time; **superseded by the Stage-6/7 recomputation below**, which is the operative next-free reading.
...
Neither HALT condition fires, so Commit 0 proceeded. **But the display slot was contended at that reading** — two concurrent in-flight releases (#4133, #4167) displayed `v3.97` in their branch names, PR titles, and plan-file paths, and `claimed_set()` by construction reads published Releases ∪ origin tags ∪ RELEASE_LOG DEPLOYED rows, **not** in-flight branches or open PRs. This release's slug-primary identity plus its `{{RELEASE_VERSION}}` placeholders are exactly the ADR-092 protection for this case: whichever release wins the Stage-12 compare-and-swap takes the number, and the losers recompute upward.

**Stage-6/7 recomputation (2026-07-27, at `origin/main` = `c10de940`) — next-free is now `v3.98`.** This supersedes the Commit-0 row above; `v3.97` is no longer free.

| Probe | Result |
|---|---|
| Highest origin tag | **`v3.97`** — annotated tag object `505cdbbd` |
| Highest RELEASE_LOG version key | **`v3.97`** — row `\| v3.97 \| build-philosophy-corpus \| #2601, #2379 \| #4133 \| … \| VERIFIED \| 2026-07-27 \|` |
| `v3.97` claim state | **CLAIMED** by #4133 (MERGED at `c290126c`, 2026-07-27T13:07:24Z), with user-facing notes at `release/releases/notes/v3.97_RELEASE_NOTES.md`. Its Stage-12/13 close-out completed correctly |
| `claim-version.sh --dry-run --bump minor` | **`v3.98`** — the value written into the `SKILL.md` frontmatter at implementation step 6 |
| `v3.98` claim state | **FREE** — no `v3.98` tag, no `v3.98` RELEASE_LOG row |
| `v3.98` display contention | **REAL and unresolved.** `release/v3.98-release-hub-mode-r-depth` exists on the remote and that release is at **Stage 8 — ahead of this one.** `v3.98` may itself be lost at this release's Stage-12 claim |
| `v3.97` display contention (residual) | **#4167** is an OPEN draft still carrying `v3.97` in its branch name and plan-file path, now colliding with a *claimed* number; it also adds a duplicate `ADR-098` (next-free is **ADR-099**). #4167's to resolve; zero file contention with this release |

**D-VersionBinding (operator-rendered at the Stage-6 exit gate) — keep the `v3.98` literal in the `SKILL.md` frontmatter.** This is the **single deliberate exception** to the otherwise-unbound slug-primary posture, and it is **forced by tooling rather than chosen**: the frontmatter `version:` field sits **inside the package-freshness hash**, and the Stage-12 stamp mechanism (`claim-version.sh --stamp-file`) resolves file content but has **no package-rebuild step** — so a `{{RELEASE_VERSION}}` placeholder would leave Check 7 red on `main` after **every** claim. Both options fail identically if Stage 12 forgets the rebuild; they differ in **failure mode**, and a forgotten rebuild with the literal ships a *stale-but-valid* version where the placeholder would ship a literal `{{RELEASE_VERSION}}` string — a broken artifact. Reversibility: **MODERATE · confidence: MEDIUM**.

> **Hard Stage-12 obligation (AI-013).** If the atomic claim lands on any tag other than `v3.98` — which R12 rates more likely than not, and which the `release/v3.98-release-hub-mode-r-depth` branch above makes likelier still — Stage 12 MUST re-version `release/skills/release-hub/SKILL.md` **AND rebuild `packages/release-hub.skill` + `.sha256` in the same commit**, then verify with a **direct** `core/deploy/deploy.sh --check-package-freshness` run. **Never accept CI green as the evidence** — the CI mirror ships the `warn` token and will pass a stale package.

**Everything else in this release stays unbound, and that remains correct per ADR-092.** The slug-primary plan filename, the slug-primary branch, and every `{{RELEASE_VERSION}}` placeholder in this file bind nothing until the Stage-12 compare-and-swap resolves them. **Do not hand-write a literal version into any of them** — the `SKILL.md` frontmatter is the one named exception. See implementation step 6 and R12.
...
- **Shadow-SSOT split (three surfaces, three jobs, zero overlap):** `reversibility-protocol.md` owns the **vocabulary** (tier definitions, confidence enum, the pmo-qa-auditor G4 test) and is **untouched**; `## Reversibility Discipline` owns the **per-artifact tier assignment** and receives only its artifact-inventory sentence reconciled at `:146` (the two tier bullets are deliberately unchanged — requirement 5 becomes their first real consumer rather than duplicating them); `## Output Contract` requirement 5 owns the **emission-time obligation**, stated once, **and states the label format at its point of use** — a deliberate restatement of a **closed** enum under the same **D-3** precedent this design applies at `decision-briefing.md:9` (values at point of use, semantics cited), so it creates no shadow SSOT; the file-hop to look the format up was itself part of the adherence tax this release exists to remove, and requirement 5's pointer is correspondingly softened to the vocabulary alone. The Guardrail bullet at `:163` **cites** requirement 5 rather than restating scope, preserving exactly one scope-defining clause for CIAC-1.
...
| **R12** | Version contention (**new at Stage 6; refreshed at Stage 7**) | **The contended slot has moved up one number and is still contended.** At the Stage-6 reading #4133 and #4167 both displayed `v3.97`. **#4133 has since MERGED and CLAIMED `v3.97`** (signed tag `505cdbbd`, RELEASE_LOG row `VERIFIED`, notes published), so this release's next-free recomputed to **`v3.98`** — and `v3.98` is itself displayed by `release/v3.98-release-hub-mode-r-depth`, which sits at **Stage 8, ahead of this release**. **#4167** remains an OPEN draft still displaying the now-claimed `v3.97`. `claimed_set()` reads published Releases ∪ origin tags ∪ RELEASE_LOG DEPLOYED rows and does **not** see in-flight branches, so next-free legitimately returns `v3.98` for both this release and the Mode-R-depth release | **High** / Moderate | Stage 9 / Stage 12 | This release is slug-primary with `{{RELEASE_VERSION}}` placeholders (ADR-092), so it binds nothing until the Stage-12 compare-and-swap arbitrates; the losers recompute upward and the winner keeps the number. **Consequences to carry:** (i) the step-6 frontmatter literal is more likely than not to need a Stage-12 re-version — **now likelier still, since the release ahead of this one displays the same `v3.98`** — so budget for it rather than treating it as an exception, and pair every re-version with a package rebuild verified by a **direct** `--check-package-freshness` run per **D-VersionBinding / AI-013**; (ii) Stage 12 must run the pre-merge freeness gate against fresh host state, never against this plan's recorded probe; (iii) merge order, not authoring order, decides the number | CHEAP pre-merge · MODERATE at Stage 12 · HIGH |
...
- [ ] File Integrity — both source files parse; frontmatter valid; `deploy.sh --check` Check 6 (canonical structure — required-field **presence**, not format) passes
...
- **Version-slot contention:** the contended slot has moved up one number — `v3.97` was claimed by #4133, so this release's next-free recomputed to **`v3.98`**, which the Stage-8 release `release/v3.98-release-hub-mode-r-depth` also displays. Everything in this release stays unbound until the Stage-12 compare-and-swap **except** the `SKILL.md` frontmatter literal (D-VersionBinding), so Stage 12 should expect a re-version of that field — **paired with a package rebuild verified by a direct `--check-package-freshness` run**. See R12 and § Version Determination.
