...
- [ ] **CIAC-4 (#5232 × #5282 on `hub-spoke-bridge.md`):** the merged file carries exactly one gate-eligibility statement and exactly one close-time gate description — the two cards edit the same file in adjacent regions and must not each leave a copy. *Shared surface:* `release/references/how-to/hub-spoke-bridge.md`. *Method:* count occurrences of the eligibility definitional block and of the close-time gate-description block in the merged file; expect 1 and 1. **Control arm:** the same counter against the Stage-4 baseline pin (§ Baseline pin) returns 1 and 1, establishing the pre-state, so a post-merge 2 is attributable to this release. *Graded before the merge: the verification-execution executor grades this criterion at Stage 6/7 against the final release-branch head and emits the verdict; Stage 9 reads that verdict (Phase A3.6) and does not re-run it.*

**Executability, recorded at Stage-6 Iteration D.** The executor runs one command and one threshold per criterion, from a closed read-only verb set (`grep`, `test`, `ls`, `head`, `wc`, `cat`). None of the four methods above is written in that set, so each grades `SKIP`. None could be rewritten into it faithfully. CIAC-1 needs a census of member names across line, table and list shapes. CIAC-2 needs set equality between two files. CIAC-3 needs this release's added lines, and no verb in the set can read a diff. CIAC-4 is a conjunction of two exact counts. The methods are left as authored until the operator decides how they are graded. The Deviation Log entry for Stage-6 Iteration D records the evidence for each.
