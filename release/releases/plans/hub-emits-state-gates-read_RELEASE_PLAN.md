...
| `core/standards/{hub-session-continuity,hub-action-tracking,failure-mode-standard}.md` · `core/specs/reversibility-protocol.md` · `core/ADRs/ADR-019` · `core/ADRs/ADR-033` · `release/governance/release-process.md` · `release/references/standards/{triage-design-rereview,bundle-composition-doctrine}.md` | PROVENANCE | backticked repo-relative path | Rationale and authoring standards the hub cites but does not execute. The engine matches only `[text](target)` and `[label]: target`, so a backtick is genuinely not a navigational affordance — and it is the corpus's own established convention for citing a non-deployed file |
