...
| **#4318 — live probe, GitHub remote (OBL-3; OBL-4's control)** | `bash core/deploy/deploy.sh --check-close-completeness` in this worktree, whose remote is the GitHub repository, with `origin/main` at the branch base `35dbf418` → exit **2**, handshake `close-completeness-exit: 2`, `INCOMPLETE — 15 90`: 13 §5.1 drift and 2 Velocity-field findings. **0 version-less CHANGELOG findings** (the baseline: 7); the DENOM line names the 7 version-less rows with limb (d) declared EXCLUDED. Network leg `76 present / 0 absent / 0 NOT-EVALUATED of 76`, the repository resolved by gh; 0 absent-Release findings, no aggregate NOT-EVALUATED line, no DEGRADED rider. GraphQL spend 2 points; 2 min 23 s. **PASS** — the forecast (INCOMPLETE 15 of 90, exit 2, handshake 2, 0 version-less CHANGELOG findings) held |
| **#4318 — live probe, local-path remote (OBL-4)** | A clone of the release branch at `cef8f82a` whose `origin` is a local filesystem path (its `deploy.sh` blob equal to HEAD's), the same invocation from the clone's root → exit **2**, handshake `close-completeness-exit: 2`, `INCOMPLETE — 2 90`. The instrument reports "the repository did not resolve (gh api rc=1): … none of the git remotes configured for this repository point to a known GitHub host". Network leg `0 present / 0 absent / 76 NOT-EVALUATED of 76`, with **exactly one** aggregate line ("NOT-EVALUATED — the network leg … was not measured for 76 of 76 network-scoped versioned row(s): repo-unresolvable (76 row(s)) … this is not a clean result"), 0 per-row withheld lines, and the DEGRADED rider beside the 2 Velocity findings, which still report. **0 `no published GitHub Release` findings**, where the pin reported 75 for the same clone shape with the 13 real drift findings masked: those 13 rows are now withheld as NOT-EVALUATED, neither reported absent nor passed as clean. GraphQL spend 0 points. **PASS** — the forecast held |
| **#4318 — probe record for the two live runs' zeros** | *Probe:* a line classifier over each run's stderr, one pattern per finding class, each pattern the emitter's own format — (d) `missing CHANGELOG.md ## [<key>] section`, (h) `no published GitHub Release (Surface 1 absent on main)`, (i) the §5.1 drift line, (j) the Velocity lines — plus the aggregate NOT-EVALUATED prefix, the DEGRADED prefix and a per-row withheld form. *Denominator:* every stderr line (21 on the GitHub-remote run, 11 on the local-path run). *Control — sensitivity:* a planted line in each emitter's exact format → 1 for all seven patterns. *Control — specificity:* each pattern's near-miss → 0 for all seven; for (d) and (h) the near-miss is present in the subject itself (the DENOM line naming limb (d); the network-leg line reading `0 absent`). *Results:* GitHub remote (d) 0 · (h) 0 · (i) 13 · (j) 2 · aggregate 0 · rider 0 · per-row 0; local-path remote (d) 0 · (h) 0 · (i) 0 · (j) 2 · aggregate 1 · rider 1 · per-row 0. *Verdict:* CLEAN |
| **#4318 — the draft PR's CI** (read 2026-09-26 at about 02:45Z) | **The self-test job** — `decision-emission.yml`'s gate-integrity job, whose `Run deploy.sh --self-test` step runs every hermetic group on `ubuntu-latest` — is **RED at both RED-arm commits and GREEN at every fix commit**, read from the job logs: `1eb7a2ee` failure, `self-test: FAIL (38 failure(s))`, with the same per-arm distribution as the local RED ((8)–(10) 1, the mapping function absent · (12) 3 · (13) 15 · (14a) 2 · (14k) 2 · (14m) 3 · (15) 3 · the other (14) arms 1 each, none for (14e)); `dc81c7e1` success, `self-test: PASS`; `e6fb603b` failure, `self-test: FAIL (1 failure(s))`, the RC-5b line; `eeaa22be` and `cef8f82a` success. **The consumer defect, observed in CI before the fix:** at Commit 0 (`907ab739`, the pre-fix probe) the Close-completeness job concluded *success* — the probe printed `INCOMPLETE — 22 90`, the gate step received `RC: 0`, and its binary `if [ "$RC" -eq 0 ]` printed "every in-scope VERIFIED release has its full Stage-13 output-set". **Not yet readable:** every `macos-latest` job after Commit 0 — Close-completeness, Install tests, Deploy-check required subset, Skill package content-freshness, Release-corpus completeness, Version freeness — was still queued for a hosted macOS runner (repository-wide at the read: 55 runs queued across four release branches, 1 in progress). The post-fix consumer's CI run is among them; Stage 7 reads it |
...
|---|---|---|
| #4318 | the close-completeness probe exits 0 only on CLEAN; INCOMPLETE is advisory 2, a withheld verdict is 3, and a failed GitHub lookup is NOT-EVALUATED, never "absent" | LANDED (`1eb7a2ee`, `dc81c7e1`, `e6fb603b`, `eeaa22be`); verification resumed in attempt 2 after a GraphQL quota stop (DEV-41, DEV-42) — the AC-6 mutation arm, both live probes and the self-test CI reads PASS; the post-fix `deploy.sh --check` and the plan-driven executor pending |
