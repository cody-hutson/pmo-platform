...
| **Status** | Executing — Stage 6 Engineering: order 1 of 7 (#4318) complete; order 2 (#5287) next |
...
| **DEV-42** | **#4318's verification resumed in a second Stage-6 session** (attempt 2, 2026-09-26 UTC) from the banked head `cef8f82a`, with its scope narrowed to the six un-run rows; no commit was redone. One implementation form differs from the Verification Plan's wording: the AC-6 mutation ran on the worktree's own `core/deploy/deploy.sh`, mutated in place, invoked by its repository-relative path and restored byte-identical (blob checked against HEAD), rather than on a copy under the scratch directory. The script-execution allowlist matches a script's path as written and admits the repository path, not a scratch-directory copy; the mutant was never staged. Beyond the six rows, attempt 2 also completed Phase C1.5, the release PR body's #4318 rows, which the same quota stop had left undone; the sub-task binds the whole Stage-6 checklist by reference | The hub's resume note on the Stage-6 brief; the sub-task's bind-by-reference to the Stage-6 checklist; the script-execution allowlist | **RECORDED** — § Verification Evidence |
...
| **#4318 — the quota stop, and the resumption** | Attempt 1: at 02:15Z `gh issue comment` was refused ("GraphQL: API rate limit already exceeded"; `gh api rate_limit` read graphql 0 of 5000, reset 02:20:17Z; core 4697 of 5000), and per the brief's quota rule the card stopped with six rows un-run (DEV-41): the AC-6 mutation arm, both live probes, the post-fix `deploy.sh --check`, the post-fix executor and the draft PR's CI reads. Attempt 2, a second Stage-6 session on 2026-09-26 (UTC), resumed from the banked head `cef8f82a` and ran all six; the rows below are its results (DEV-42). Each network run is bracketed by `gh api rate_limit`, an uncharged read. The GraphQL pool is shared with other sessions, so a counter delta is an upper bound on a run's draw, not an exact figure; the REST `core` counter did not move across runs that made REST calls, so it is not used as evidence |
| **#4318 — AC-6 mutation arm (OBL-2)** (attempt 2, at `cef8f82a`) | **Predictions, stated before the run:** the predicate `_cc_st_pv7_violations` has two callers, arm (10) against the real mapping and arm (11) against the collapsed map, so replacing its body with a constant zero keeps (10) green and fails (11) alone — exactly 1 FAIL line, `self-test: FAIL (1 failure(s))`, exit 1. **Control** (unmutated, same session): `bash core/deploy/deploy.sh --self-test` → `self-test: PASS`, exit 0, 0 FAIL lines. **Mutant** (the body replaced with `printf '0\n'`): exit 1, `self-test: FAIL (1 failure(s))`, and the one FAIL line is arm (11)'s ("the PV-7 predicate returned 0 violations against a mapping that DELIBERATELY collapses INCOMPLETE/warn onto 0"); every assertion group still started. **Restore:** the pristine bytes copied back; `git diff --quiet -- core/deploy/deploy.sh` exit 0, and the working-tree blob equals HEAD's (`c08b5633`). *Probe record:* `grep -c '^FAIL'` over each run's output; denominator — the eleven group-start lines, present in both runs; sensitivity — the mutant's count, 1; specificity — the control's output carries arm (11)'s own label in its PASS roster and counts 0; verdict CLEAN. **PASS** — the population arms pass, and the killed predicate fails arm (11) alone |
| **#4318 — live probe, GitHub remote (OBL-3; OBL-4's control)** | `bash core/deploy/deploy.sh --check-close-completeness` in this worktree, whose remote is the GitHub repository, with `origin/main` at the branch base `35dbf418` → exit **2**, handshake `close-completeness-exit: 2`, `INCOMPLETE — 15 90`: 13 §5.1 drift and 2 Velocity-field findings. **0 version-less CHANGELOG findings** (the baseline: 7); the DENOM line names the 7 version-less rows with limb (d) declared EXCLUDED. Network leg `76 present / 0 absent / 0 NOT-EVALUATED of 76`, the repository resolved by gh; 0 absent-Release findings, no aggregate NOT-EVALUATED line, no DEGRADED rider. The GraphQL `used` counter moved by 2 across the run (an upper bound on its draw); 2 min 23 s. **PASS** — the forecast (INCOMPLETE 15 of 90, exit 2, handshake 2, 0 version-less CHANGELOG findings) held |
| **#4318 — live probe, local-path remote (OBL-4)** | A clone of the release branch at `cef8f82a` whose `origin` is a local filesystem path (its `deploy.sh` blob equal to HEAD's), the same invocation from the clone's root → exit **2**, handshake `close-completeness-exit: 2`, `INCOMPLETE — 2 90`. The instrument reports "the repository did not resolve (gh api rc=1): … none of the git remotes configured for this repository point to a known GitHub host". Network leg `0 present / 0 absent / 76 NOT-EVALUATED of 76`, with **exactly one** aggregate line ("NOT-EVALUATED — the network leg … was not measured for 76 of 76 network-scoped versioned row(s): repo-unresolvable (76 row(s)) … this is not a clean result"), 0 per-row withheld lines, and the DEGRADED rider beside the 2 Velocity findings, which still report. **0 `no published GitHub Release` findings**, where the pin reported 75 for the same clone shape with the 13 real drift findings masked: those 13 rows are now withheld as NOT-EVALUATED, neither reported absent nor passed as clean. The GraphQL `used` counter did not move; 27 s. **PASS** — the forecast held |
| **#4318 — probe record for the two live runs' zeros** | *Probe:* a line classifier over each run's stderr, one pattern per finding class. Each pattern is the emitter's own format under either row label, `vX.Y:` or `<slug> (version-less):`: (d) `missing CHANGELOG.md ## [<key>] section`, (h) `no published GitHub Release (Surface 1 absent on main)`, (i) the §5.1 drift line, (j) the Velocity lines. Added to those: the aggregate NOT-EVALUATED prefix, the DEGRADED prefix and a per-row withheld form. *Denominator:* every stderr line (21 on the GitHub-remote run, 11 on the local-path run). *Control — sensitivity:* planted lines in each emitter's exact format, in both label forms, all match. For (d) there is also a real-emitter arm: the pre-fix probe's own output in CI at Commit 0 carries 7 version-less CHANGELOG lines, and the pattern counts 7. *Control — specificity:* each pattern's near-miss → 0. For (d) and (h) the near-miss is in the subject itself: the DENOM line naming limb (d), and the network-leg line reading `0 absent`. *Results:* GitHub remote (d) 0 · (h) 0 · (i) 13 · (j) 2 · aggregate 0 · rider 0 · per-row 0; local-path remote (d) 0 · (h) 0 · (i) 0 · (j) 2 · aggregate 1 · rider 1 · per-row 0. *Verdict:* CLEAN. *Repair:* the record committed at `1ddc5730` matched only the `vX.Y:` label, so its planted arm never showed the probe could see a version-less line — the very class of the (d) zero. The label was widened and the probe re-run, and every result is unchanged |
| **#4318 — the draft PR's CI** (read 2026-09-26 from 02:45Z to 03:16Z) | **The self-test job** — `decision-emission.yml`'s gate-integrity job, whose `Run deploy.sh --self-test` step runs every hermetic group on `ubuntu-latest` — is **RED at both RED-arm commits and GREEN at every fix commit**, read from the job logs: `1eb7a2ee` failure, `self-test: FAIL (38 failure(s))`, with the same per-arm distribution as the local RED ((8)–(10) 1, the mapping function absent · (12) 3 · (13) 15 · (14a) 2 · (14k) 2 · (14m) 3 · (15) 3 · the other (14) arms 1 each, none for (14e)); `dc81c7e1` success, `self-test: PASS`; `e6fb603b` failure, `self-test: FAIL (1 failure(s))`, the RC-5b line; `eeaa22be` and `cef8f82a` success. **The consumer defect, observed in CI before the fix:** at Commit 0 (`907ab739`, the pre-fix probe) the Close-completeness job concluded *success* — the probe printed `INCOMPLETE — 22 90`, the gate step received `RC: 0`, and its binary `if [ "$RC" -eq 0 ]` printed "every in-scope VERIFIED release has its full Stage-13 output-set". **The consumer, commit by commit** — every Close-completeness run on the branch completed, and each log was read. `907ab739` and `1eb7a2ee`, the pre-fix probe: `RC: 0`, no handshake, and the completeness claim over `INCOMPLETE — 22 90`. `dc81c7e1` and `e6fb603b`: `RC: 2` and `HANDSHAKE: 2`, rendering the advisory-finding branch ("**This is not a pass.**" and a warning that exit 2 is never clean), still over 22 findings with the 7 version-less CHANGELOG lines. `eeaa22be`, `cef8f82a` and `1ddc5730`: the same branch over `INCOMPLETE — 15 90`, with 0 version-less CHANGELOG lines. The consumer flips at the contract commit, and the drain lands at limb (d)'s commit. **Other gates at the fix commits:** Deploy-check required subset (`--check-required-subset`) is green at `e6fb603b`, `eeaa22be` and `cef8f82a` — all 4 load-bearing checks pass in its `cef8f82a` log — and so are Release-corpus completeness, Version freeness and Release tooling smoke. **Still queued at the last read (03:16Z):** Install tests at the five post-fix commits (the row-2 runtime suite, green at `907ab739` and `1eb7a2ee`), Skill package content-freshness at three, and three gates at `1ddc5730`. The queue was 55 runs repository-wide at 02:43Z; no workflow declares a top-level `concurrency:` block, so superseded runs are never cancelled |
| **#4318 — post-fix `deploy.sh --check`** (attempt 2, 13 min 38 s) | `bash core/deploy/deploy.sh --check` → exit 1, "2 issue(s) found": **the baseline's two pre-existing issues, Check 47 and Check 63, and no new one.** **Check 48** (lifecycle): WARN, 15 findings across 90 VERIFIED rows, 13 §5.1 drift and 2 Velocity-field (the baseline: 22, of which 7 version-less CHANGELOG); network leg `76 present / 0 absent / 0 NOT-EVALUATED of 76`. **Check 14** OK, no broken cross-references in scope. **Check 62** OK, all 34 register pointers resolve. **Check 69** OK, 0 unsanctioned spellings; its control, the sanctioned-occurrence count, rose from the baseline's 299 to 397, and the six matrix files' own count rose from 97 to 195 (+98), so the whole delta is this card's added text. **Checks 7, 9 and 13** OK: 55 skill packages content-fresh, the mirror pairs in sync, 60 template-injection entries matching. **Check 63** FAIL at three sites in two files outside this matrix, pre-existing. **Check 47** FAIL with 14 findings where the baseline had 13, although neither its code nor the drift engine changed on this branch: 0 of the 41 `deploy.sh` hunks since the branch base fall in Check 47's block (a planted hunk inside it is flagged), and the drift engine's diff is empty. A re-read about ten minutes later ran the drift engine once per row over all 71 versioned rows from v4.0 onward. It returned exactly the 13 known drift rows and 58 matches, and v4.49, which read N/A in the same Check 47 pass, compared cleanly. `[INFERRED]` a transient read failure surfaced as drift through the drift engine's own body read — the residual routed to #4714 (#7878 F-1). A published body edited between the two reads is not excluded. The shared GraphQL counter moved by at most 10 across the run and the re-read |
| **#4318 — post-fix plan-driven executor** (attempt 2, at `1ddc5730`) | `bash release/tools/verify-release-plan.sh --stage4-comment <the three Stage-4 parts, concatenated> <this plan>` → **11 PASS / 14 FAIL / 8 SKIP / 38 ERROR** over 54 per-issue rows (exit 3), in 13 min 10 s; every non-PASS row is classified. Against Commit 0 (12 / 13 / 8 / 38), three rows flipped. **FCM-5** went FAIL → PASS: this plan is delivered. **#4318 OBL-1 and CIAC-1** went PASS → FAIL because the fix landed: each asserts that `grep -c 'if \[ "\$RC" -eq 0 \]' .github/workflows/close-completeness.yml` reads 0. It now does — rc 1, which grep returns for a zero count; at the pin it reads 1, rc 0 — and the executor grades the count on the command's exit status (#7899). The observed value equals the Expected cell. **FAIL 14:** those 2; FCM-1..FCM-4, declared ADDs that orders 3 and 6 deliver; #5287 AC-2 and AC-4 and #6871 OBL-4, pre-fix zero counts that orders 2 and 7 raise, graded on exit status (#7899); CIAC-5, #6871's convention paragraph (order 7); and the four sync rows (#4318 OBL-3, #5287 AC-5 and OBL-2, #6237 OBL-1), graded on `deploy.sh --check`'s exit 1 alone, whose two pre-existing issues the row above names. **SKIP 8** and **ERROR 38** are unchanged from Commit 0, row for row: 36 `unclassified-method` rows (self-test arms and named reads outside the executor's closed read-only verb set) and 2 `count-unreadable` rows (#4318 AC-3, #6865 AC-2). **PASS 11:** FCM-COVERAGE (`declared=57 interpreted=57 obligations=5 excluded=11 conditional=1 uninterpreted=0 pathless=0`), FCM-5, FCM-6 and the four provenance rows are real verdicts. #6237 AC-1, #6871 AC-3, CIAC-3 and CIAC-4 are exit-status passes over pre-fix counts, with no evidence until their cards land. PROV-DELTA's PASS is again vacuous on this host (three `awk: newline in string` lines on stderr, #7898). Re-computed independently — the executor's five element predicates under `/usr/bin/grep`, the set difference outside `awk` — 5 of 5 elements survive, and an in-memory copy with its `CIAC-` tokens altered loses exactly `ciac` |
| **#4318 — the remaining C4 answers** | **ADR index:** N/A — #4318 adds no record under `release/ADRs/`. **Skill-package freshness:** Check 7, 55 of 55 fresh; no #4318 path is a rostered skill source. **Runtime suite:** `deploy.sh` maps to row 2 of the runtime-suite selection map, the deploy suite, which is `install-tests.yml`'s `run:` steps. The map names CI as that suite's authoritative locus. It passed there at `907ab739` and `1eb7a2ee`, and its post-fix runs were queued for a macOS runner at hand-off, so no Stage-6 `test-run` event carries a result. Stage 7 Phase A8 reads the completed run, or runs the map's local fallback |
...
|---|---|---|
| #4318 | the close-completeness probe exits 0 only on CLEAN; INCOMPLETE is advisory 2, a withheld verdict is 3, and a failed GitHub lookup is NOT-EVALUATED, never "absent" | LANDED (`1eb7a2ee`, `dc81c7e1`, `e6fb603b`, `eeaa22be`) and verified — every verification row the design names ran, the last six in a second session after a GraphQL quota stop (DEV-41, DEV-42) |
