...
| `core/config/allowlists/script-execution-allowlist.txt` | #4561, #3821 | **edit** ×2 | Companion allowlist rows for each added executable, in all four resolved invocation forms, so the test is runnable agent-side on arrival |
| `.github/workflows/install-tests.yml` | #4561, #3821 | **edit** ×2 | Wire each new regression test **and** the pre-existing title-floor test, which had never been CI-wired |
...
**Path count:** 20 machine-readable rows (19 repo paths + this plan). Three are new files; one is a conditional add. *(Extended at Engineering Commit 2 — #3821 added `core/deploy/tests/test_g1_form_family.sh` and `core/deploy/tools/check-work-hierarchy.py`; see the Deviation Log.)*
