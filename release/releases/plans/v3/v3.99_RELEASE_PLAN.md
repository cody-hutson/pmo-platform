...
> **Milestone:** `release-hub-response-convention-enforcement` (#294) · **Parent epic:** #4019 (agent-response governance) · **Release Class:** `novel` · **Version:** `v3.99` (pre-claim — binds atomically at the Stage-12 claim per ADR-092) · **Scope:** 2 issues (#4020 `size:S`, #4021 `size:M`) · One release branch, one PR, one merge gate.
...
| **D-Version** | Provisional **`v3.99`**; bump-class **minor**. Binds atomically at the Stage-12 claim, never at plan time (ADR-092). | Recorded determination | CHEAP pre-Engineering; MODERATE after Commit 0 · HIGH |
...
- In-file version references: `v3.99`, resolved at the Stage-12 claim by `claim-version.sh --stamp-slug`.
...
**D-VersionBinding (operator-rendered at the Stage-6 exit gate) — keep the `v3.98` literal in the `SKILL.md` frontmatter.** This is the **single deliberate exception** to the otherwise-unbound slug-primary posture, and it is **forced by tooling rather than chosen**: the frontmatter `version:` field sits **inside the package-freshness hash**, and the Stage-12 stamp mechanism (`claim-version.sh --stamp-file`) resolves file content but has **no package-rebuild step** — so a `v3.99` placeholder would leave Check 7 red on `main` after **every** claim. Both options fail identically if Stage 12 forgets the rebuild; they differ in **failure mode**, and a forgotten rebuild with the literal ships a *stale-but-valid* version where the placeholder would ship a literal `v3.99` string — a broken artifact. Reversibility: **MODERATE · confidence: MEDIUM**.
...
**Everything else in this release stays unbound, and that remains correct per ADR-092.** The slug-primary plan filename, the slug-primary branch, and every `v3.99` placeholder in this file bind nothing until the Stage-12 compare-and-swap resolves them. **Do not hand-write a literal version into any of them** — the `SKILL.md` frontmatter is the one named exception. See implementation step 6 and R12.
...
- User-facing release notes: `release/releases/notes/v3.99_RELEASE_NOTES.md` (authored at Stage 13 Close per the release-notes standard)
