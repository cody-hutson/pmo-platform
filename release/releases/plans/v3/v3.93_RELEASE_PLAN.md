...
plan_type: release
status: CLOSED
release: versioned — provisional v3.93 (bump-class minor; slug-primary pre-claim, version binds atomically at the Stage-12 claim)
...
**Version identity:** **versioned** — bump-class `minor`, provisional **v3.93** (recorded determination; binds atomically at the Stage-12 tag per the two-phase allocation rule). This plan is authored **slug-primary / pre-claim** under the **Hybrid dogfood** decision (D-Dogfood **(C)**): the branch, this plan filename, and in-file version references carry the slug / `v3.93` placeholder now; the concrete number binds at Stage 12 via the **current** `claim-version.sh` engine (this introducing release does **not** self-exercise the unmerged claim-time substitution pass it ships — reflexive-pipeline-loop discipline).
...
release/tools/claim-version.sh                        | #2548-core | edit — ADD pre-CAS pre-flight + post-CAS v3.93 stamping + slug→plans/v<MAJOR>/vX.Y rename on the CAS-win path (AC3); CAS arithmetic/retry UNTOUCHED
...
release/skills/release-planner/references/release-plan-template.md | #3993 | edit — Version→v3.93, Branch→release/<slug>, Milestone→<slug>, header slug form (AC4/AC6)
...
| **R7** | Version-slot contention (live) | Heavy concurrent-release churn; a sibling may claim the next provisional slot before this release merges. | D-Version is a recorded determination (bump-class `minor`, provisional v3.93); re-verify at Engineering Commit 0 + the Stage-12 atomic claim — the standing mechanism, no new work. | CHEAP·HIGH |
...
- **D-Version.** Recorded determination: bump-class `minor`, provisional v3.93; re-verify at Engineering Commit 0 + the Stage-12 atomic claim.
