...
|-------|----|---|-------------------|----------------|
| #5863 | AC-1 | behavioral | `python3 -m scripts.run_scenario_eval --suite evals/scenario-runner/evals.json --out <run>/grading.json` from `release/skills/pmo-skill-refiner/` | Exit 0, and the named report file exists carrying a `summary.pass_rate` of `1.0` over the committed baseline fixture. The scenario is scored end-to-end from committed inputs with no live model call, so the run is reproducible rather than sampled. |
| #5863 | AC-2 | file-content | `git diff --name-only <merge-base>..HEAD` must not list `release/skills/pmo-skill-refiner/scripts/run_eval.py`, and both runner files must exist at head | Null expectation: **zero** occurrences of the trigger runner's path in the delivered diff · control: the same instrument over the same diff must list `release/skills/pmo-skill-refiner/scripts/run_scenario_eval.py` → non-zero, proving the diff resolves and the extraction reaches the directory. Both files present at head. |
| #5863 | AC-3 | behavioral | Run the same suite twice with `--fixture fixtures/regressed.yaml` and with the committed baseline fixture; compare the two emitted pass rates | The regressed run's pass rate is **strictly lower** than the baseline run's, and the baseline run is at `1.0` · control arm, required: the unregressed fixture scores at baseline, proving the score discriminates rather than always passing. A second, declarative control ships inside the suite — every resolution predicate is re-run against a committed empty fixture and **all** must fail; a suite whose predicates resolve against an empty fixture is a no-op and the run reports it as such. |
| #5863 | AC-4 | contract-documentation | Reviewer read of `release/skills/pmo-skill-refiner/references/scenario-eval-contract.md` without opening the runner source | The document carries the scenario schema, the closed predicate vocabulary with each member's shape, the suite-level control block, the command-line signature, and the closed exit-code set. Grading fields are **cited** to the framework's schema document rather than restated. The acid test is the second suite: the gate card authors its corpus from this document alone. |
| #5864 | AC-1 | behavioral | Run the corpus at `core/disciplines/evals/behavioral-regression/` through the runner delivered by #5863 | The corpus contains more than one scenario and scores end-to-end on demand, through the same documented interface a third corpus would use — no corpus-specific code path in the runner and no runner-specific logic in the corpus. |
| #5864 | AC-2 | file-content | Inspect the workflow trigger block of `.github/workflows/behavioral-regression.yml` | The trigger block is legible in one read: a filter-free pull-request rung that exercises the gate every release, and a tag rung bound to the major-release boundary. Both the binding boundary and the advisory rung are visible without reading a script. |
| #5864 | AC-3 | behavioral | Dispatch the gate against the regressed fixture and against the baseline fixture | The gate **FAILS** on the regressed input and **PASSES** on the baseline · control: it passes when above, proving it can fail. The tag-pattern filter is additionally asserted by a committed fixture through an entry point invocable locally and invoked by the job — a filter verified only by reading its glob is unverified. |
| #5864 | AC-4 | file-content | Read the declared field row in `core/schemas/platform-config-schema.md` and the catalogue entry in `docs/platform-config-reference.md` | The floor and the boundary it binds to are recorded as data in exactly one numeric home, and no other surface restates the magnitude. An unresolvable floor fails closed — empty means unresolved, never zero, because a zero default satisfies every comparison and greens the gate permanently. |
