...
**The `D-N` identifiers are one namespace shared with the release PR body's Deviation Log, and the gap between D-1 and D-11 is deliberate.** Both logs opened at the same D-1. The PR body's then grew to D-10 during Engineering while this table did not, so D-2 through D-10 are recorded there and are not restated here — restating them would create a second copy that drifts, and their third column is Severity where this one carries Stage. The reconciliation rows below therefore continue the shared sequence at D-11 rather than reusing numbers that already mean something else. One of them, D-11, does restate a deviation the PR body already carries as its D-5, because that reconciliation belongs on the durable surface and a pull-request body is not part of the repository corpus; the two are the same event, deliberately recorded twice.
