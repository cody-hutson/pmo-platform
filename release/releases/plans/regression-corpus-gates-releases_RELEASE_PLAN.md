...
**The `D-N` identifiers are one namespace, and it is now whole on this surface.** Both this log and the release pull request's opened at the same D-1, but neither surface could see the other's numbers: the PR body's grew to D-10 during Engineering while this table did not, and the reconciliation rows authored later took D-11 onward rather than reusing numbers that already meant something else. That left D-2 through D-10 recorded **only** in a pull-request body — a surface that is editable in place, leaves no trace in git history, and is not the artifact the close-out stage asserts. The nine rows have therefore been ported here, keeping their identifiers, their description, rationale and disposition text verbatim, with the third column mapped from the PR body's `Severity` to this table's `Stage`. The stage for each was derived from what the row itself describes and cross-checked against the commit it cites; all nine fall in Engineering. The pull-request body is deliberately left unedited as the historical record of what was written where, so the two surfaces now overlap rather than diverge. D-11 likewise restates a deviation the PR body carries as its D-5: the two are the same event, deliberately recorded twice, because that reconciliation belongs on the durable surface.
...
| D-2 | The decision record owed by the gate card was authored by the **runner** spoke at Commit 0 rather than deferred to the gate card's spoke. | 6 | The brief authorized both records at Commit 0 and directed that the gate card's be authored if its content was settled by its own Solutioning output and dependent on no implementation. Both conditions hold. Authoring both together also lets them open contiguously at the allocator's next-free rather than leaving a number to be claimed later against a moving anchor. | Recorded. The gate card's spoke consumed that record and allocated **no** second number. |
| D-3 | The plan's per-issue verification rows were re-authored after Commit 0, from prose into executable two-arm probes. | 6 | The plan-verification executor graded all eight prose rows as unclassifiable errors, so the Survival-Set element whose named consumer is that executor was producing no verdicts. The executor classifies on method-string keyword only and holds a closed read-only verb set; both properties were read from its source rather than inferred. | Repaired in `bf81988`; the runner card's four rows now grade PASS. |
| D-4 | Three CI link gates went red on the runner card's first push and were repaired in place. | 6 | Both defects were that spoke's. Three cross-module paths in the new contract document were one directory level too shallow, and four net-new markdown links landed on added lines in two files that carried no link-exemption marker. **A probe-validity note belongs with this:** the link checker run locally reports zero broken links over **zero files scanned**, for every invocation including a deliberately-broken control file. That is a broken probe, not a clean result. No local run of it was treated as evidence by either spoke; CI is the authoritative arm. | Repaired in `2137cd1`; all three gates green. |
| D-5 | Both decision records were renumbered mid-run — 177 → 180 and 178 → 181. | 6 | A sibling release merged to the mainline while the runner card's Engineering run was in flight and claimed 177, 178 **and** 179. A number is allocated at authorship and **claimed at merge**, so first-to-merge keeps it and the other claimant moves. **This is the allocation rule behaving correctly under contention, not drift.** The governed tool made both moves, so the citation sweep, the renumber log, the index projection and each record's own provenance note moved together. | Repaired in `1c18e61` and `11a001f1`. |
| D-6 | The gate card's decomposition uses the **PR-body checklist** container although the A2 threshold predicate selects the **sub-issue** container. | 6 | The predicate is unambiguous and selects sub-issues: 6 logical units is above the threshold of 5, the work is cross-file, and it tunes a threshold value — three independent triggers. It is recorded rather than silently taken. The container was not switched because the predicate's own stated purpose is that it be *"evaluable from the change matrix alone before any container is created, so the choice is reproducible"*; creating six sub-issues after the work has landed records nothing that the six commits and this checklist do not already record, and it would split one milestone's decomposition across two container types on a single PR. **Routed to the hub, not decided here:** if the container choice is meant to bind retroactively, the sub-issues are cheap to create. | Recorded, not repaired. Hub to route. |
| D-7 | The scenario-eval contract shipped with **two** optional fields where the decision record for it states one. | 6 | Operator-authorized mid-release, to discharge the open integration criterion the corpus needed: a corpus gated at a 1.00 floor otherwise has only an allowlist or a loosened threshold with which to carry a known-open defect, and both hide the exception inside a number. **The decision record is left exactly as written** — an accepted record is a dated decision, superseded rather than edited — and the delta is recorded in the plan's Change Description instead. Its stated bound was backward compatibility rather than arithmetic, and that bound holds: the new field defaults to the identity, and the runner card's own two reports are **byte-identical** across the amendment once the run timestamp is normalised. | Recorded; verified non-regressive. |
| D-8 | The Stage 5 spec called for a gate-coverage register row **and** a flip-decision status row; only the register row was added. | 6 | Measured, not assumed: across the 21 table lines of that subsection, the three sibling warn-mode CI gates have **zero** rows (column 1 and anywhere) while `version-freeness` (a `deploy.sh` check) and `BLOCK-DESTRUCTIVE-022` (flagged in-table as an exception) each have one; a control arm on a fabricated gate name returns zero. The subsection's own heading scopes it to warn-mode `deploy.sh --check` and stage-gate criteria, and this gate is neither. The graduation clause lives in the sentinel, which is where the standard says whoever performs the flip reads it. | Recorded, not repaired. |
| D-9 | The PR title was broadened to name the whole milestone, keeping its single `(#5863)` reference intact for the title-grammar gate. | 6 | The title named one card of a two-card milestone. The body's Implementation table and Issue References block carry both. | Recorded. |
| D-10 | The release-tooling smoke gate went red on the plan file's frontmatter `status: EXECUTING`, which is outside the declared enum `ACTIVE` / `CLOSED` / `ABANDONED`. Repaired here as a one-token change, although the line was authored at Commit 0 by the runner card and no gate-card commit touches any frontmatter line. | 6 | **Not cosmetic.** `release-corpus-schema.md` § Plan-status lifecycle states the Stage-13 close-out phase *"fails loudly on a value outside the enum rather than overwriting it"* — so left alone, this release's own close-out would refuse to transition its own plan. Measured rather than assumed before changing anything: across 193 plan files, `CLOSED` 39, `ACTIVE` 6, no status field 147, and `EXECUTING` **exactly once** — this file. The schema's own instruction is to set `ACTIVE` at authorship. It surfaced only now because the job was among the queued macOS gates that had not settled during the runner card's run. | Repaired in `246b504`; the linter exits 0 with zero non-advisory findings. |
| D-11 | **Branch reconciliation at Engineering — mainline sync, and the first ADR-number collision.** Merge `5deae055` absorbed **20** mainline commits. A sibling release merged during this release's Engineering run and claimed 177, 178 and 179, so both of this release's records moved: **ADR-177 → ADR-180** (`1c18e612`) and **ADR-178 → ADR-181** (`11a001f1`). | 6 | A number is allocated at authorship and **claimed at merge**, so first-to-merge keeps it and the other claimant moves. This is the allocation rule behaving correctly under contention, not drift. The allocator's own detector reported both records `DUPLICATE` against the mainline and named the next free slots above the new anchor of 179. Both moves were made by the governed tool rather than by hand, so the citation sweep, the renumber log, the index projection and each record's own provenance note moved together. | Recorded, not escalated — Tier 1 |
| D-12 | **Branch reconciliation before the gate — a README text conflict masking a real collision.** Merge `de9721b0` absorbed **66** mainline commits and resolved one conflict, in `core/ADRs/README.md`. The text resolution alone did not settle it: **both sides claimed ADR-180**, mainline had merged first and therefore bound, so this branch's runner record moved **ADR-180 → ADR-182** (`c09bedf3`). | 8 | Reconciled at the Stage 7→8 boundary. Performed with the governed renumber tool under `--renumber 180 182 --apply`, never by hand; the authoritative detect before the move read anchor 180 at the mainline, next-free 181, this branch's ADR-180 `DUPLICATE` with next 182 and its ADR-181 `BINDS`, and both records read `BINDS` after it. **Three files were excluded from the citation sweep** — `core/schemas/platform-config-schema.md`, `docs/platform-config-reference.md` and `core/ADRs/README.md` — because their ADR-180 tokens cite the *other* record holding that number, not this one. Each was classified by reading its surrounding context rather than by pattern match: an unscoped sweep would have rewritten 14 citations, **4 of them the mainline record's**, silently repointing another record's own references. | Recorded, not escalated — Tier 1 |
| D-13 | **Branch reconciliation at the Stage-9 remediation — the same masking shape a second time.** Merge `c5072ff1` absorbed **47** mainline commits and resolved conflicts in **both** `core/ADRs/README.md` and `release/ADRs/README.md`; mainline had claimed **ADR-181**, so this branch's floor record moved **ADR-181 → ADR-183** (`87365295`). Mainline then moved a further **8** commits mid-run, absorbed by merge `a12fe432`. | 9 | Both conflicts were resolved as a **union, not a side-selection**: the `core/ADRs/README.md` conflict sits inside the renumber log, which is chronological rather than current-state, so taking either side would have deleted the other record's hop from the permanent record. `release/ADRs/README.md` is the projected index and was regenerated by its own projector as part of the renumber. **No path exclusions were needed this time, and that was established rather than assumed** — all 67 `ADR-181` tokens were classified by reading their context, over a denominator of 1908 tracked text files, before the sweep ran, then re-verified from the tree afterwards rather than from the tool's own count. Assuming this incident repeated D-3's shape would have been the error; the difference was measured. | Recorded, not escalated — Tier 1 |
| D-14 | **The F-4 identifier-hygiene rename landed after the Stage-9 GO, under explicit post-gate authorization.** The scoring step's id in `.github/workflows/behavioral-regression.yml` moves `corpus` to `score`, and its single consumer follows. Two lines, one file. | 9 | Recorded rather than absorbed, because the state that ships is not byte-identical to the state the gate reviewed. **Framing, stated because it has been overclaimed once already: this ships as identifier hygiene — what D4's enumeration actually names — and NOT as closing the grep residue.** A search for `corpus` across the workflows directory already returns roughly 40% of that directory, and this file legitimately keeps most of its own occurrences as sanctioned prose, so no rename here or anywhere could close that residue. Stage 7 established the rename safe on its own terms: the step id is referenced exactly once, in the same file, with no job-level `outputs:` and no `needs:`, and the branch-protection context is the job `name:`, which is unchanged. | Recorded, not escalated — Tier 1 |
| D-15 | **The `D-N` deviation namespace was reconciled onto the durable surface, and a stale version-determination citation was corrected.** The namespace was shared between two logs that never saw each other's numbers, so this table jumped D-1 → D-11 while D-2 … D-10 existed only in the release pull request's body. The nine rows are ported here with their identifiers and their description, rationale and disposition text preserved verbatim, their third column mapped from `Severity` to `Stage`. Separately, the Change Description cited `D-Version-R2`, superseded when the Commit-0 value was claimed by a sibling release mid-Stage-7; the citation now reads `D-Version-R3`. | 9 | A pull-request body is editable in place, leaves no trace in git history, and is not the artifact the close-out stage asserts, so nine deviations recorded only there were one edit away from disappearing without trace. Porting rather than renumbering preserves each row's identity, so a reader following a `D-N` reference from either surface lands on the same deviation. The PR body is deliberately **not** edited: it stays the historical record of what was written where, and the goal is a complete durable surface rather than an erased perishable one. The version fix is a citation repair, not a re-rendering — no concrete version number is stamped here, because the determination is `anchor + bump-class floor` and binds only at the Stage-12 atomic claim. | Recorded, not escalated — Tier 1 |
...
- **D-Version-R3:** bump class `minor`. The slot this release provisionally held has been claimed by a concurrent release more than once; the rule never moved, only the anchor. `R2` was the recomputation at Commit 0 against both authoritative surfaces; `R3` superseded it when that Commit-0 value was itself claimed by a sibling release mid-Stage-7. No concrete number is stamped here — the determination is `anchor + bump-class floor`, never `max(claimed) + 1`, and it binds only at the Stage-12 atomic claim.
...
- **A malformed fixture is a runner error, and the runner now says so with exit 2 (Stage-9 remediation, F-6).** `load_structured()` parsed with no guard on either branch, so a malformed fixture raised through `main()` and Python exited **1** — the single code the falsification arm reads as a successful falsification. The gate built to prove that it *can* fail would have announced exactly that on a run which graded nothing, which is the anti-pattern this release exists to eliminate, reachable inside the release's own mechanism. Both branches now raise the runner's named suite error, which maps to **exit 2** — the code the contract's exit-2 row already assigned this family, and the code the gate decision's already-correct fail-closed path treats as *could not measure*. The same guard covers a fixture whose bytes are not UTF-8, because a read failure leaks the identical exit 1 through the identical route as a parse failure. Verified independently by the hub against a live control: malformed YAML exits 2, malformed JSON exits 2, and the pristine fixture run in place exits 0 — so each subject reading is a measurement rather than an artifact.
- **The recorded floor now governs across its declared domain (Stage-9 remediation, F-7).** `if failed: return EXIT_FAIL` preceded the floor comparison, so within the documented `[0.0, 1.0]` domain only `1.00` was distinguishable — *no assertion failed* and *the pass rate is 1.0* were the same statement — and the "or lower the floor" remedy named in this release's own floor-and-boundary decision record silently no-opped. A recorded decision that cannot take effect is worse than an unrecorded one, because it reads as an available lever to whoever inherits it. When `--fail-under` is supplied the pass rate is now compared against it, and that comparison alone decides between 0 and 1; when it is absent, any failing gradable assertion still exits 1, and that default is now stated as a decision rather than left implicit, because defaulting to a tolerance would install a standing regression budget by omission. Hub-verified on a `0.5455` run: floor `0.50` exits 0, floor `0.60` exits 1, and floor `1.00` is **unchanged** — so the shipped configuration, which runs at `1.00`, behaves exactly as it did before. Only sub-`1.00` floors changed meaning, which is precisely the domain the decision record named and could not previously reach. **The fix creates one hazard it is worth naming here:** before it, a floor read as `0.0` could not actually green the gate forever, because a failing assertion exited 1 regardless — making the floor govern makes that documented warning load-bearing for the first time. The fail-closed resolver was therefore probed rather than trusted: a missing key, a non-numeric value and a value outside the domain each exit 2 with a named error, and the workflow resolves the floor under `set -euo pipefail`, so an unresolvable floor aborts the job before any value reaches the runner.
- **The falsification arm's predicate was deliberately NOT touched, and that was established by diff rather than by assertion.** The arm's `-eq 1` strictness is what caught this gate's own first-run defect; loosening it to a `-ne 0` form would have swallowed both that defect and F-6. The binding constraint on the remediation was therefore that the fix land in the **runner** and never in the **arm** — tightening the arm could not have helped, because the exposure was the arm's *input*. The predicate has come through both the remediation and the later F-4 identifier rename byte-identical, confirmed by an empty diff against the pre-remediation tip and re-run independently by the hub rather than taken on the implementer's word. A future reader should read the untouched arm as a constraint that was honoured, not as an oversight.
