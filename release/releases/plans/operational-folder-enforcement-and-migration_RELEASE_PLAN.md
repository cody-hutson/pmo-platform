...
**Correction to the record — the FK invariant is `95 → 1`, not `8 → 0`.** Stage 7 measured the FK population as `disposition == 'ok'` (47 → 8 unstamped at baseline; 38 → 0 at HEAD). **That is not the population the writer touches.** In `plan_edges`, step 6 appends the `BELONGS_TO` edge **before** step 8 assigns `no-provenance-anchor`, and `run()` emits on `if do_emit and plan.edges:` — **disposition is not a gate.** Over the emitted population: **282 → 95 unstamped at baseline; 181 → 1 at HEAD.** 143 of the 181 edge-carrying files at HEAD sit at `no-provenance-anchor` and would still be written. **The card under-claimed its own best result by roughly 12×**, and no acceptance criterion credits it — not one card AC mentions edges.

**The surviving violation is NOT inert — recorded as an AI-010 Stage-12 item against CIAC-1's accepted-residual disposition.** The one remaining FK violation *is* the declared accepted-residual orphan: a depth-1 corpus-root `.md` (opaque key `6ebdb617a810`). On the **node** axis it is an orphan — never stamped, no `project` field — and CIAC-1's disposition is sound as a node-axis statement. On the **edge** axis it is active: `_project_of()` returns `rel_path.parts[0]`, which for a depth-1 corpus-root file **is the filename**, so the planner emits a `BELONGS_TO` whose target is **its own basename stem**. At Stage 12 that would write an edge naming a project that does not exist, onto a file with no node record. **This falsifies a claim the commit makes about itself**: `c7de12fa`'s message and the `NONRECORD_PROJECT_SEGMENTS` comment both state *"the edge population stays a SUBSET of the node population."* Directional test at HEAD: `edges ⊆ stamped` → **False, by exactly one file.** The mirroring covers the **exclusion** tiers; it does not cover the **orphan** class. **Severity MODERATE, not Blocker** — one file, one edge, `--emit` is gated (verified exit 3 without the token) and the D-11 snapshot covers the reversal. **Stage-12 obligation:** confirm the emit skips it, or add a depth guard to `plan_edges` so a depth-1 corpus-root file plans no `BELONGS_TO`.
