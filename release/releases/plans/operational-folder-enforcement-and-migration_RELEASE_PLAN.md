...
| Version | **{{RELEASE_VERSION}}** — bump class **minor**, identity **slug-only** per ADR-092. The concrete number binds at the **Stage-12 atomic claim**, when `claim-version.sh --stamp-slug` resolves this token and renames the file into `plans/v<MAJOR>/`. Mainline at authoring was `v4.33`. **D-38** sequences this milestone AHEAD of `migration-protocol-and-skill-integration`, so this release claims first. |
