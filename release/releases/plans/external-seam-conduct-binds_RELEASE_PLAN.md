...
The set was therefore re-derived from the shipped change rather than from any restatement of it — the discipline the Collective Review's own root-cause finding demands. **#6904's declared write set is 18 authored paths**, the 16 the artifact of record supported plus two the design and both adversarial reviews missed and a corpus-wide cascade sweep found:
...
**Release totals, reconciled against the actual branch diff:** 18 (#6904 authored) + 4 (#6905) − 1 (`_cross-cutting.md`, edited by both and counted once) = **21 authored paths**, plus **12 derived package artifacts** (six skills × `.skill` + `.sha256`, tool-produced) and the plan file = **34 files**, which is exactly what `git diff --name-only` reports for the release branch against its baseline. Stating the reconciliation this way is deliberate: a path count that cannot be checked against the diff is the same class of unverifiable number the original 17 turned out to be.
...
| **D-6-7** | D-6-6's open gap is **CLOSED**, and closed by derivation: #6904's declared set is **18 authored paths**, not 16 and not 17. The stated 17 was an adds-plus-edits tally that reconciles to no enumerated list — there was no seventeenth path to find | Engineering, logged here — the hub explicitly declined to assert a number and directed the pass to derive it from its own File Change Matrix | The set was re-derived from the shipped change and reconciled against `git diff --name-only` (34 files = 21 authored + 12 derived package artifacts + the plan). A path count that cannot be checked against the diff is the same unverifiable construct the original 17 turned out to be |
