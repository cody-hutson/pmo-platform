...
**Why `core/ADRs/ADR-NNN-project-health-home.md` is CONDITIONAL.** #5839's Stage 5 has not rendered its decision between a Project field and a separate fact record, and the record's slug follows from that choice. The row is promoted into the unconditional set, carrying its concrete slug, in the commit that resolves it.
...
|---|---|---|---|---|
| #5837 | AC-1 | file-content | `grep -c "System of record" core/ADRs/ADR-162-system-of-record-per-mirrored-element.md` ≥ 3 | The decision record's table assigns one system of record for at least three named elements. |
| #5837 | AC-2 | file-content | `grep -c "wins" core/ADRs/ADR-162-system-of-record-per-mirrored-element.md` ≥ 3 | The reconciliation rule is directional in every branch and names a winner; "flag both without a direction" is explicitly excluded. |
| #5837 | AC-3 | file-content | `grep -c "ADR-051" core/ADRs/ADR-162-system-of-record-per-mirrored-element.md` ≥ 3 | The record names ADR-051 explicitly and states its disposition as superseded in part. |
| #5839 | AC-1 | file-content | `[DEFERRED — Beat 1 sibling record; slug binds at #5839's Stage 5 decision]` | Deferred until #5839's record slug is bound. |
| #5839 | AC-2 | file-content | `[DEFERRED — Beat 1 sibling record; slug binds at #5839's Stage 5 decision]` | Deferred until #5839's record slug is bound. |
| #5839 | AC-3 | file-content | `[DEFERRED — Beat 1 sibling record; slug binds at #5839's Stage 5 decision]` | Deferred until #5839's record slug is bound. |
...
- **#6278** — the Stage-6 Engineering sub-task under which this commit landed.
