...
| **close-completeness** (`deploy.sh --check-close-completeness`) | **INCOMPLETE (advisory, warn-mode, non-blocking)** | 18 findings across 59 checked rows. The only one naming this release is `missing CHANGELOG.md ## [one-system-of-record-per-element] section` — which **contradicts the projector**, that refuses to emit a version-less CHANGELOG key. The gate fires identically on all **3** post-cutover version-less rows, so it is a standing mechanism conflict, not a defect in this close. Not silenced by hand; routed as a recommendation |
| **plan-status lifecycle** | **REPAIRED IN THIS PR** | flipping the ledger row to `VERIFIED` made the plan's `status: ACTIVE` incoherent, and the plan-identity lint caught it (`PLAN-STATUS-NOT-TERMINAL`). Transitioned to `CLOSED` per the release-corpus-schema plan-status lifecycle — the document's own working life, which is why the terminal value is `CLOSED` and not `VERIFIED`. On the script path this is `phase_transition_plan_status`; that script never dispatches for a version-less release, so the hand-assembled Phase-B close owes the same outcome |
