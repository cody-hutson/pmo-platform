...
| `release/ADRs/ADR-191-…-fifth-form-is-a-widening.md` | add | #5227 | Records the form-set decision: four canonical; the fifth `*/…` form subsumes forms 1–3, does not cover form 4, and admits an arbitrary path prefix. |
...
| #4216 | AC-3 | `bash core/deploy/deploy.sh --check-decision-emission` with the event-log path pointed at a nonexistent path → assert exit **3** · **control arm, same instrument, same target:** the same invocation against a seeded complete fixture exits **0**, so the 3 is a real classification and not a constant | The live arm reports NOT-EVALUATED, never a pass; the job summary carries an explicit "not a pass" statement; the job itself exits 0 |
