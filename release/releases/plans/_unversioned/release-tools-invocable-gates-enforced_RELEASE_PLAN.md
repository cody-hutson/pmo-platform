...
add   release/ADRs/ADR-192-allowlist-form-set-four-canonical-fifth-form-is-a-widening.md
...
| `core/deploy/deploy.sh` | edit | #4216, #4217 | #4216: the `--check-decision-emission` exit contract becomes a **five-code space** — `0` CLEAN · `1` fail-or-enforce-degraded · `2` INCOMPLETE (non-enforce) · `3` SKIP (**sentinel-agnostic**, mirroring `--check-package-freshness`) · `4` NOSET (non-enforce) — so a degraded verdict can no longer collapse onto CLEAN. CI branches on exit codes and re-encodes no predicate (**D-8**). Delivered with **two added workflow branches** and **three added self-test arms**, including `DE-10c`, which asserts PV-7 as a *population* property (exactly two of ten verdict pairs produce 0, both CLEAN). #4217: −3 dead `c31_globs` roots, +3 line-scoped markers. **Neither slice adds `decision-emission` to `rs_checks`** — that roster maps `SKIP` into its pass arm. |
...
| **R5** | **Card line-number citations are stale.** Cited positions had drifted from live ones; the "four canonical forms" claim disagreed with a file carrying five. | Currency | CHEAP | HIGH | **RESOLVED at #5227 D-1** — four canonical, fifth and sixth are per-tool exceptions with written reasons, recorded in ADR-192 and in the file's own convention block. #5283's count criterion resolves to four, so a correct implementation grades MET. |
...
| #5033 | AC-1 | Read the `ADR-number integrity gate` declaration in `repo-integrity.yml`, then a live `gh api …/branches/main/protection` read-back recorded at Stage 12 | **Post-D-13 (Reading A):** the declaration states what the enforcement surface *actually delivers*. The branch declares `posture=advisory  enforcement=ci-enforce:"ADR-number integrity gate"` plus `invariant:` and `falsification:`, **and** the live read-back confirms the context is correspondingly **absent** from `required_status_checks.contexts`. Registering it is **AI-002**, a repo-settings action no pull request can perform; until it runs, `advisory` is the true declaration and `required` would be the false one |
