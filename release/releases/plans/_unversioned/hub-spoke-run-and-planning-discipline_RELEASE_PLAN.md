...
| A4 QC4-05 invariant re-verification | **N/A — no operand** | The plan declares no `AV-N` invariants (`AV-` occurs 0 times; the § Verification Plan carries per-AC probes, not declared release invariants). N/A by absence of the declared set, not by skipping. |
...
| A5 G-CL6 design-artifact refresh | **N/A by construction** | The plan declares no *Tier-A activated design artifacts* section (`Tier-A` occurs 0 times in the plan). With an empty declared set the per-artifact gate has nothing to grade. Warn-mode regardless. |
