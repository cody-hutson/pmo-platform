...
| #6596 | AC1 | Read the disposition of all three `v[0-9]` sites across `core/skills/finops-usage-extractor/scripts/rollup-attribution.sh` and `estimate-usage.sh` | **2 updated, 1 preserved, 1 sibling arm added** — the disposition set correction 6 records. The preserved site is a *branch* predicate that correctly parses 25 live `chore/vX.Y-*` refs plus one legacy `release/v3.97-*`; a uniform "fix" would break working behaviour to satisfy a sweep |
...
| #6597 | AC3 | Probe `core/specs/autonomy-tiers.md` for `etc.` and **classify every survivor** rather than reading the bare count | The governed-set `etc.` is **removed**: item 6 now *"names the obligation, not its members"* and states the prior enumeration was illustrative and open-ended, with **no closed set derivable** from it or anything else in the document. Probe: `etc.` **3 → 2**, and both survivors are **out of scope** — the file-router folder list (line 80) and the Automation Tier schema-doc list (line 127), neither the governed set. **AC3 passes on its subject; a reviewer grading the count alone will mis-read it as a partial fix** |
