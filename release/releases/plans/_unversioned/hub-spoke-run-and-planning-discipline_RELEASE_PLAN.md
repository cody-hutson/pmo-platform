...
**Precision, measured at the Stage-6 gated write: the path is in scope but the hook does not currently fire on it.** `block-skill-direct-edit.sh` exits 0 at an earlier precondition than the sentinel check — it gates only a `SKILL.md` carrying `skill_discipline_migrated_v10_2: true`, and `core/skills/finops-usage-extractor/SKILL.md` is one of **5 of 56** in-scope `SKILL.md` files without that marker (sensitivity: 51 carry it; specificity arm on a bogus `v99_9` marker: 0). The routing verdict is unchanged — the path sits inside the gate's declared scope, and per the editor skill's own coverage-boundary note the *discipline* is the control on a default instance — but "gated" here means **routed through the sanctioned session**, not "an interlock stopped anything". Recording the first without the second would overstate the control.
