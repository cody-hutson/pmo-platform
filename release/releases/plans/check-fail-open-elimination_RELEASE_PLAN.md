...
Persisted at Stage 13 Close per the Stage-13 spec's Phase B2; this section is the home gate criterion G-CL4 reads, and the per-phase verdicts below are written into it. Post-deploy anchor: `origin/main` at the release merge `19e1def0102be7d67d1917b45947f30493220f0e`, re-measured after the Stage-13 corpus chore PR landed at `7b8a8679e651d285792f39730de84beb7d727fe8`.

**Probe-validity note.** Every zero below is reported with the control arm that establishes it as a measured zero rather than an empty extraction, per the probe-validity discipline in `core/disciplines/review-discipline-principles.md` section 8. A zero whose control also returned zero is recorded as an unusable probe, never as a verified absence.

### Per-phase verdicts

| Phase | Criterion | Verdict | Evidence |
|---|---|---|---|
| A1 | Issue closure | **PASS** | All six delivery cards closed at Stage 13. The release PR was authored parser-clean (its closing-issue reference count is zero, verified), so no auto-close fired and Stage 13 owned the close. Milestone counter re-read after closing: `open_issues` 2, `closed_issues` 37 — the two open rows are the hub-owned Stage-12 and Stage-13 sub-tasks |
| A2 | Deferred-item disposition | **N/A** | No bundled-but-not-closed card. All six members of the frozen scope shipped; none was deferred, split or dropped |
| A3 | Verification-evidence compilation | **PASS** | This section |
| A4 | QC4-05 invariant re-verification | **N/A** | The plan declares no `AV-N` assertions. Recorded as a genuine absence — a whole-file probe for the `AV-` token returns zero occurrences |
| A5 | G-CL6 design-artifact refresh | **N/A** | The plan declares no "Tier-A activated design artifacts" section, so the gate has no declared population to grade |
| A6 | Cycle-time baseline | **N/A (no-op)** | `compute-cycle-time.sh` returns N/A — the release emits no `deployment-status` deploy event (content-only, the case the standard sanctions). Per the baseline's own maintenance rule, an N/A release appends nothing and does not shift the rolling-3 window |
| A7 | Release-learnings synthesis | **PASS** | A `release-synthesis/learnings-triple` row was emitted keyed on the milestone slug (the release join key); the rendered `Release Learnings v4.29` H4 block landed in `RELEASE_LOG.md` via the chore PR |
| A7.1 | Recommendation-choice delta roll-up | **NOT-RECORDED — surfaced** | `query-pipeline-event.sh` filtered to this release returns **0 rows**. Control arm: the same query unfiltered returns **17 rows** (all keyed to another milestone), so the probe demonstrably returns rows and the zero is real. This release emitted **no** delta rows at all — neither `diverged` nor `aligned` — despite rendering several D-class decisions. Recorded as a missing emit, **not** as a zero-delta release, because those are different facts |
| A7.2 | Close-out learnings register | **PASS** | Produced from the tracked template to the operator-instance register path, seeded from the A7 triple. Producer seeds, operator authors — no lessons or actions row is machine-written |
| A7.5 | Close-class resolution | **DEPLOYABLE** | Resolved via **rung 1** (`domain_practice.domain`); declared value `software`. The typed branch fires only on the literal `task-artifact`, so this routes to the default deployable path. Reported rather than taken silently, per the A7.5.2 fall-through rule |
| A8 | Close-out completeness | **PASS** | Mandated automated path (`automated-closeout.sh --apply`), not hand-assembly. Full output set landed on main and was verified there — see the Step-4 table in the Stage-13 close-out record |
| A8.1 | Note-content close gate | **PASS** | `lint_release_corpus.py --check note-content` exits 1 with **0 findings naming v4.29**; the control arm reports **12** findings for other (legacy v3.x) versions, so the extraction was non-empty and this version's silence is a validated pass. A legacy finding for another version does not block this close, per audit-baseline discipline |
| 2.8 | Sample-block advisory | **PASS** | The release is declared schema/format-changing (it freezes a two-register token vocabulary), so the advisory was run explicitly and emits no ADVISORY line for this version. Control arm: the same flag against the preceding release emits `NOTE-SAMPLE-BLOCK-MISSING`, proving the check discriminates rather than silently passing |
| A8.2 | Check 61 decision-emission | **INCOMPLETE (advisory, non-blocking)** | Verdict `INCOMPLETE — 12 / 30` (missing-class count over checked-release count), warn-mode. **This release is not named in the finding**, because the gate iterates `VERIFIED` log rows and this row was still `DEPLOYED` at run time. Given A7.1's zero-emission result, it is expected to register missing classes on the next run now that the row is `VERIFIED`. The remedy is to emit the missing rows, never to waive the finding |
| A10 | QC4-06 goal attainment | **ATTAINED** | Narrative below |
| A11 | G-CL8 documentation impact | **PASS (6 of 6)** | Every declared non-None doc was modified inside the release commit range; the control arm — a path not in the release — returns 0, so the probe discriminates. Detail below |
| A12 | Audit-recommendation badges | **N/A (no-op)** | A recursive search of the operator-local analysis tree returns **0** `recommendations.md` files, so no badge is owed. Recorded as a measured no-op rather than an unchecked skip |
| B-OPS1 | Operational deployment manifest | **N/A** | The plan declares no operational deployment manifest — empty manifest, skip to close-out on the existing empty-set path |
| B-OPS5 | Absorption reconciliation | **N/A** | No codification issue with memory ties in this release's membership; the stranded set is empty |

### A10 — Goal-attainment narrative (QC4-06): **ATTAINED**

The milestone's AFTER state is *"a broken measurement is distinguishable from a clean result: every check reports degraded/unknown rather than silently passing."* Post-deploy `main` exhibits it, and the most direct evidence is a check that now fails where it previously passed: `deploy.sh --check` reports exactly **one** genuine `FAIL:` line — Check 63, naming four files whose stated count does not reconcile with the adjacent structure. None of those four is in this release's 28-file changed set, and all four are byte-identical to pre-merge `main`: a diff against the merge's first parent returns empty for all four, while the same diff returns +283/−39 for `core/deploy/deploy.sh`, a file the release did change — so the probe demonstrably detects change. That is **pre-existing drift surfaced by a newly-fail-closed check**, which is the release's thesis working as designed rather than a regression it introduced.

The Success Indicator — *"the gate or check each one names demonstrates a real failure on a fixture before it is trusted"* — is satisfied: the vocabulary gate was demonstrated **failing** on the real unfixed corpus first, naming all five sites, and only then passing after the fix, with the sanctioned-occurrence control rising by exactly the five converted sites. Merging took the mainline population of the unsanctioned spelling from 12 to 0; an independent post-merge census reads **0** unsanctioned standalone renderings against a control of **87** sanctioned occurrences across 15 tracked files.

### A11 — Documentation-impact detail (G-CL8)

Per-card verdicts, keyed by the card index in the Issue References block at the head of this file.

| Card (row order) | Declared | Verdict |
|---|---|---|
| Umbrella / degraded-state contract | `core/deploy/tools/README.md`, `core/disciplines/review-discipline-principles.md` | **PASS** — both modified in range |
| Check 56 M2 advisory class | `core/deploy/tools/README.md` | **PASS** — modified in range |
| Indicator 6 denominator integrity | `core/deploy/tools/README.md` | **PASS** — modified in range |
| Aggregation-trace obligation | `release/references/pipeline/stage-05-solutioning.md` | **PASS** — modified in range |
| Count-vs-structure path scoping | `None — no documentation impact (rationale: …)` | **PASS** — explicit-none form with separator and rationale present; a bare `None` would fail |
| Version-freeness injection | `core/standards/gate-efficacy-standard.md` | **PASS (format note)** — the file was modified in range, but the declaration is authored as prose rather than the `path \| action \| rationale` row form the other five use. Content-conformant, shape-divergent; recorded rather than silently normalized |

### Artifact-Acceptance Record (AAR)

**ABSENT — explicit no-op.** The release declares no task-artifact deliverable, so no AAR block is written and no ceremony is performed. Gate criterion G-CL3 takes the unchanged deployable path, satisfied by the `RELEASE_LOG` `DEPLOYED` to `VERIFIED` transition.

### Residuals carried out of this release

1. **The published GitHub Release surface for this version does not exist.** A live query returns `release not found`, and the close-out's anchor-hygiene phase independently reports a missing-Release divergence (annotated tag present, no published Release). The Stage-12 emit did not land it. Per the Stage-13 Phase B5.6 rule this is non-blocking and routes to operator decision — publish standalone, or accept as residual and backfill. Surfaced rather than self-resolved: publishing is a public-content action reserved to the operator.
2. **The whole-corpus vocabulary gate runs on proposed changes only.** Its workflow declares no push trigger, so the gate structurally cannot evaluate a merge commit — confirmed by the merge commit's 26 check-runs, which contain no vocabulary-gate job. Whole-corpus scanning is the correct fix for merge-borne violations, but `main` itself is never directly scanned post-merge, so a violation arriving through a merge is caught on the next proposed change. Bounded exposure rather than open-ended, and worth an intake ticket.
3. **`core/deploy/tools/check-pv7-vocabulary.sh` is tracked mode `100644`,** where sibling tools in the same directory are `100755`. Both consumers invoke it via `bash <path>`, so this is cosmetic today rather than functional — recorded so it is neither mistaken for a live defect nor left to surprise the first caller who invokes it directly.
4. **The `{{RELEASE_VERSION}}` placeholder remains unresolved in this plan's header.** The stamping flag was deliberately not passed: ADR-092 is `Proposed` rather than Accepted, the Stage-12 spec never names the flag, and stamping would have renamed this slug-named plan against the release's slug-primary posture. Owned by the in-flight ADR-092 release.
5. **The deployed script-execution allowlist mirror lags the tracked source.** The tracked allowlist carries all four companion invocation forms for the new gate script; the deployed mirror the hook reads does not, so local invocation of that script is blocked. Owed to a deploy run, and to be verified **by hash rather than by exit status**. No bypass variable was set and no deployed allowlist was edited from this session.
