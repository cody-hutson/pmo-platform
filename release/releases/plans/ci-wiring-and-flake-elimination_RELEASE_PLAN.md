...
> **Milestone:** `ci-wiring-and-flake-elimination` · **Release Class:** `novel` (Standard / Deep / ALL / 30-day) · **Version:** `v4.17` — **provisional; binds at the Stage-12 atomic claim (ADR-092)** · **Scope:** 3 issues (#4188, #3832, #3936) · **Topology:** D-C `SINGLE` · **Concurrency posture:** P0 fully-serial · One release branch, one PR, one merge gate · **Branch:** `release/ci-wiring-and-flake-elimination` (slug-primary, no version stem).
...
The plan file, the branch, and all hub state are keyed on the milestone **slug**, never the version. `v4.17` is **provisional** and binds only at the Stage-12 atomic compare-and-swap, where git's ref CAS is the authority.
...
| 3 | Release identity | Release Class *proposed* `novel`; D-Version recorded `v4.16` | Release Class **is `novel`** (rendered at the Stage-4 gate) · D-Version **is `v4.17`** | Stage-4 operator gate; Commit-0 re-verify above |
...
**Release Class: `novel`** — rendered at the Stage-4 operator gate (the Stage-3 `routine` declaration is superseded). **D-Version: `v4.17`** — the handed-down `v4.16` was REFUTED at Stage 4 (that tag is taken by milestone `methodology-fields-and-statuses`) and `v4.17` is next-free. Re-verified free at Engineering Commit 0 (see § Release Identity).
...
| **R-6** | **Version-slot contention, three ways.** `v4.16` was claimed by #4980 at `876f8632` **during this planning run**. #4979's plan still carries `v4.16` as its provisional; #4924 is `minor` slug-only. All three compete for `v4.17` | Stage 12 | **MEDIUM** | CHEAP / HIGH | Tier-S serialization edge — record in the Parallelization Map. Bind at the Stage-12 atomic claim only; **read the tag surface, never the ledger** (see R-7) |
...
**Recorded determination — NOT a gate.** Next-free `minor` = **`v4.17`**.
...
**Identity:** slug-primary per ADR-092 — branch `release/ci-wiring-and-flake-elimination`, plan file `ci-wiring-and-flake-elimination_RELEASE_PLAN.md`, no version stem. `v4.17` is **provisional**; it binds at the Stage-12 atomic compare-and-swap and re-verifies against the tag surface there. Bump class `minor` (capability/remediation, not corrective against a deployed release).
