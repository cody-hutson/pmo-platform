...
- **CI arm:** ran, and discharged an open item — the SKIP ledger emits **8 of 16** of the self-test's own checks in CI, against **7** locally, and the 8th entry is one the local empty-bare-origin harness could not have produced. AC-4 limb (a) was upgraded from code-reading to measured on that evidence. **Re-read at the delivered head `aa590212`** rather than carried forward: job `97663355553` `Discovered tool self-tests (Ubuntu)`, step group `release/tools/cleanup-orphan-state.sh --self-test`, runtime lines `self-test: SKIP ledger — 8 of 16 checks SKIPPED` and `OK: release/tools/cleanup-orphan-state.sh`.
