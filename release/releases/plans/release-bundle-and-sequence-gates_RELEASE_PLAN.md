...
| **#3827** | `release/skills/release-planner/references/dependency-analysis.md` | **edit** | § Step 4 Topological Sort — cite the selector, do not restate (ADR-019). **Shares this file with #3817** |
...
| *release* | `packages/release-planner.skill` · `packages/release-hub.skill` | **rebuild** | Both source skills are edited (#3822/#3827, #2719) → `.skill` packages drift. `automated-closeout.sh` Phase 9.95 `rebuild_skill_packages` handles this at close-out; listed so Check 7 does not surface it as a surprise |
...
| **#3827** | file-path+state (5) | `grep` § 2.4 for `topology-first` **and** `value/WIP-first`; for both failure-mode guards; confirm the rule-14 citation with **no second formula**; `grep` `dependency-analysis.md` for the citation | All five present/absent as specified. **Baseline: 0 hits repo-wide today**, so any hit is attributable to this release |
...
| **DEV-7** | **R-2 delimiter repair not taken.** | Not in scope at Stage 4. | Stage-5 flagged the § Gate G-BR criteria table's delimiter row as malformed (5 cells under a 6-column header; systematic — 6 such tables in the file, 1 well-formed). The spec marked the repair **conditional on operator approval (R-2)**, which has not been recorded. Slice 1 therefore made **3** edits to `gate-criteria-spec.md`, not 4, and appended `G-BR5` to the table as-is. | **Deferred — operator judgment.** Surfaced, not silently applied or silently dropped. |
...
- **Six gate-criteria tables carry a 5-cell delimiter row under a 6-column header** (Stage-5 OSD-2 / R-2). Systematic and pre-existing; only 1 well-formed 6-cell delimiter exists in the file. See DEV-7 for the one in this release's path.
