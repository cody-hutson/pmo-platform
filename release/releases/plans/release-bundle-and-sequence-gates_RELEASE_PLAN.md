...
> **Milestone:** `release-bundle-and-sequence-gates` (#299) · **Release Class:** `cross-cutting` (capacity weight 1.3) · **Version:** `{{RELEASE_VERSION}}` *(unresolved by design — bump-class `minor`; the token binds to the won tag at the Stage-12 atomic claim, when `claim-version.sh --stamp-slug` resolves it and renames this file to the version-keyed form in the same stamp commit. The provisional determination re-verified at Engineering Commit 0 is recorded in § Commit-0 Version Re-Verify. This plan file and its branch stay **slug-primary** until the claim, per ADR-092; the token is load-bearing rather than decorative, because the stamp pre-flight HALTs the claim on a plan that carries none.)* · **Scope:** 6 issues · 26 raw pts / 34 effective · One release branch, one PR, one merge gate (D-C **SINGLE**, D-Concurrency **P0 fully-serial**).
...
> **Superseded — read § Measured at writing below before relying on any number in this section.** The Commit-0 reading was accurate when taken and was overtaken within four minutes by a sibling's Stage-12 claim. It is retained as the historical record of the first detection rung, not as a live determination.

**Method.** `git fetch --tags origin && git fetch origin main`, then each arm of the claimed set evaluated independently against `origin/main`. Ledger input read via `git show origin/main:<path>`, never the worktree copy. **The table below is the reading taken at Commit 0** — a point-in-time record, not a present-tense claim.

| Claimed-set arm | Probe | Result **as read at Commit 0** (superseded) |
|---|---|---|
...
**Verdict at Commit 0: PROCEED.** `anchor()` = `v4.05`; floor (minor) = `v4.06`; `claimed_set()` had no member at or above the floor. No HALT condition was present **at that instant**. **This verdict is superseded — see below.**

### Measured at writing

**`v4.06` is claimed.** Milestone #290 `governance-hardening` took it. All four mainline arms agree, re-measured on 2026-08-04 (Tuesday) against `origin/main`:

| Claimed-set arm | Probe | Result |
|---|---|---|
| Origin tags | `git ls-remote --tags origin` | `v4.06` present (annotated tag) |
| Published GitHub Releases | `gh release list` | `v4.06` published 2026-08-03T17:04:08Z, marked Latest |
| Mainline version file | `git show origin/main:.version` | `v4.06` |
| Mainline release ledger | `git show origin/main:release/releases/RELEASE_LOG.md` | `v4.06` row present — merge `39284a2d`, PR #4564, status VERIFIED, 2026-08-03 |

**The Commit-0 reading was not wrong; it was overtaken.** Every arm flipped *after* Commit 0, the earliest by under four minutes. This is the two-phase binding discipline behaving exactly as specified — the detection rung is advisory precisely because the claimed set can move between detection and claim:

| Time (local) | Event |
|---|---|
| 11:20:31 | `39284a2d` — #290 merges to `main`. **This is Commit 0's parent**, so DEV-1's "cut from post-merge `main`" holds. |
| **11:30:35** | **Commit 0 (`02a0b152`)** — re-verify runs. `origin/main:.version` genuinely read `v4.05`; no `v4.06` tag, Release, or ledger row existed yet. |
| 11:34:24 | `v4.06` annotated tag created — #290's Stage-12 CAS. **+3m49s.** |
| 11:59:16 | `a59bdb95` — `.version` → `v4.06`, ledger row written. **+28m41s.** |
| 12:04:08 | GitHub Release `v4.06` published. **+33m33s.** |

**Next-free, derived at writing: `v4.07`.** Rule-computed via `claim-version.sh --sha <origin/main> --bump minor --dry-run` → `v4.07`; corroborated by the ledger (`v4.07` occurs zero times on `origin/main`) and by a scan of every remote branch's `release/releases/plans/` tree, which holds no version-keyed plan at or above `v4.06` beyond the merged `v4.06` plan itself.

**`v4.07` is derived-at-writing, not reserved — and it is contended.** Milestone #305 `release-notes-and-learnings` re-determined its own D-Version `v4.06 → v4.07` on the same event, and its plan registers this release as risk **R5** (*"the concurrent milestone #299 claims `v4.07` first"*). The contention is mutual, recorded on both sides, and **not** resolvable by either plan: whichever release reaches Stage 12 first wins the number by compare-and-swap, and the loser re-versions upward. Both releases are slug-primary and bind no version until then (ADR-092).

**Nothing here is a version this plan hardcodes.** The `**Version:**` field keeps its unresolved release-version placeholder (the single literal token, in the header block above); `claim-version.sh --stamp-slug` resolves it to the **won** tag at the Stage-12 claim and renames this file in the same stamp commit. `v4.07` is a *reading*, not a reservation — do not substitute it into that placeholder, and do not treat this section as authorizing a number.
...
core/disciplines/execution-framework.md
docs/FIRST_STEPS.md
release/ADRs/
core/ADRs/
...
| **#2719** | `release/references/how-to/hub-spoke-bridge.md` | **edit** | **Procedure 0 Step 5** — the step is split into three ordered clauses; clause **(c)** has the hub relay the Mode R briefing as the sub-task's first comment, with the fenced briefing block and its fingerprint operands. **This is the shipped substitute for the reversed Mode-R-writes design** (DEV-12) and the reason #2719 needs no new hub-state surface. **Added to the matrix at batch 5** — the edit shipped in slice 5 unattributed (DEV-19). **Shares this file with #3581** (whose Step-5 marker emission becomes clause (b)) **and #3819** (Procedures 1 + 7) |
| **#2719** | `release/skills/release-hub/SKILL.md` | **edit** | Name the re-review artifact Mode R owes Phase A0: Mode R's `**Output:**` clause (b) and Output-Contract requirement 3 move off the two-of-six-columns `C1/C2/C3 + PT-1..4` naming onto the `triage-design-rereview` § 1 artifact (8 header fields + six-column table); the double-running failure mode's `Root cause` (two conditions → three) and `Mitigation` (names the hub relay, not a future step). **The Mode R state-mutation guardrail is NOT narrowed** — the reversal at A6.5 left it untouched, and it is byte-identical to `main` (DEV-12) |
...
| **#2719** | *(none — no new file, no new surface, no ADR)* | — | The A6.5 reversal (DEV-12) deleted the Mode-R-writes design that required them. **Not created, deliberately:** `release/releases/hub-state/readiness-briefing.md.template` (Surface D), `release/releases/hub-state/README.md` + `core/standards/hub-session-continuity.md` (Surface D registration), and #2719's ADR — the threshold argument rested on a guardrail narrowing that the reversal removed (DEV-13). `release/references/standards/triage-design-rereview.md` was **optional per Stage-5 R4 and not taken** — its original AC3 target (`§ Integration`) does not exist, and no defensive edit is authored to satisfy a stale assertion (DEV-4). **Net for #2719: 4 MODIFY, 0 NEW, 0 contract changes, 0 ADR** |
...
| **#3819** | `release/skills/release-hub/SKILL.md` · `release/skills/release-hub/references/orchestration-playbook.md` | **edit** | Cascade (operator-approved R-6). One region each, token-anchored on `one sub-task per stage per issue` — **disjoint from #2719's `cache-read` anchor** in the same SKILL.md. **Shares `SKILL.md` with #2719**, slices 5 and 6 |
| **#3819** | `core/disciplines/execution-framework.md` · `docs/FIRST_STEPS.md` | **edit** | Cascade, one line each. Both described the scaffold as per-issue-per-stage; `execution-framework.md` additionally enumerated Stages 9/12/13 as per-issue. **Added at Commit 6** (DEV-16) |
...
**Next-free at Commit 0 = `ADR-110`.** A number is **allocated at authorship and bound at merge, and only the mainline binds** — next-free is `anchor(origin/main) + 1`, never `max(claimed_set) + 1`. At Commit 0 `origin/main` maxed at **ADR-109** (`git ls-tree -r --name-only origin/main -- core/ADRs release/ADRs`), so this release takes **110** and **111**. Scanning sibling branches is still worth doing — it predicts a merge-time renumber and lets the operator sequence the merge — but it changes the *report*, never the *number*. `check-adr-numbers.py` globs the working tree only, so it is not the authority for the **anchor**; it remains the authority for **contiguity**, and it passes here (`111 ADRs, contiguous 001..111, no duplicates`).

**Merge-order contingency — the slots are contended, and pre-reserving is the wrong response.** Re-derived at Stage 7/8 remediation across all remote branches: the `adr-corpus-conformance` release holds unmerged claims on **110, 111, 112**; `agent-edit-discipline-codification` holds **112, 113**; `103-skill-suite-conformance-and-usability-ac` holds **114**. Stepping this release to 113/114 would (a) be the rejected `max(claimed_set) + 1` reading, (b) collide with two *further* branches rather than avoiding a collision, and (c) if this release merged first, land a **gap at 110–112** on `main`, where the contiguity gate then fails **every** subsequent pull request — a duplicate inconveniences one branch, a gap blocks the repository. The contention is therefore **recorded, not mitigated**: whichever release merges second renumbers via the governed renumber path. Because an unmerged record has no mainline citations, that move is confined to its own branch diff. **Verified at this commit: 518 relative ADR links across the branch, 0 dangling, with a firing sensitivity arm.**
...
| **#3817** | ADR-105 | **COLLIDED** — `release/ADRs/ADR-105-release-corpus-normalization.md` merged with #290 | Re-derive at its Commit — next free after 110. **Outcome: discharged by deriving — landed `ADR-111`** (DEV-13) |
| **#2719** | ADR-105 | **COLLIDED** — same | Re-derive at its Commit — next free after 110. **Outcome: no ADR authored** — discharged by *elimination*, not derivation: the reversal deleted the guardrail-narrowing clause the threshold argument rested on, so no number was ever claimed (DEV-13, DEV-19) |
| **#3827** | none | — | none |
| **#3822** | none | — | none |
...
**Discharged — this instruction is closed, not outstanding.** Slice 3 derived `ADR-111`; slice 5 authored no ADR at all. **R-12 is closed for both slices.** The release's final ADR set is `ADR-110` + `ADR-111`, and no further re-derivation is owed.
...
| `release/references/how-to/hub-spoke-bridge.md` | **#3581, #2719, #3819** | line-range-overlap | **NEW at Stage 5** (DEV-3); **#2719 added to the row at batch 5** (DEV-19). #3581 takes **Procedure 0 Step 5 only**; #3819 takes **Procedure 1** (six regions) + **Procedure 7**. Disjoint procedures, disjoint anchor tokens. Slices 1 and 6 — maximally separated in the serial order. #3581's edit is append-only within Step 5 and reads none of #3819's state, so **no sequencing preference is asserted**. **#2719 (slice 5) also lands in Procedure 0 Step 5** — the only genuine *same-region* pair in this release: it restructures the step into three ordered clauses, absorbing #3581's marker emission as clause (b) and adding the briefing relay as clause (c). Serial execution made this a sequential rewrite, not a conflict; the recorded ordering is #3581 → #2719, and a revert of either must respect it (see § Rollback Strategy). |
...
| **R-10** | Two `.skill` packages (`release-planner`, `release-hub`) drift when their source files change. | Build hygiene | LOW | Package artifacts are built, not authored; drift surfaces as `deploy.sh --check` Check 7 / G6-06. | `automated-closeout.sh` Phase 9.95 `rebuild_skill_packages` handles this at close-out; both are in the File Change Matrix so the rebuild is expected rather than a surprise FAIL. **G6-06 is PR-time**, so the rebuild should land in this PR, not wait for close-out. **CLOSED at slice 6:** `release-planner` rebuilt at slice 4, `release-hub` at slice 6, each by its last package-touching slice; each rebuild cleared exactly its own two `--check` FAIL lines and added none. | Stage 6 (final slice) / Stage 13 | CHEAP / HIGH |
| **R-11** | Milestone #305 is at Stage 4 concurrently and shares `automated-closeout.sh` (#4451). | Contention | LOW-MED | Two Stage-4 planning runs in the same window with no cross-plan reconciliation step in Procedure 0. | Hub-level: reconcile the two Stage-4 File Change Matrices before either enters Engineering. Whichever enters Engineering second records the other's baseline. **Also the live version-claim contender:** both releases now read `v4.07` as next-free and contend that slot — see § Commit-0 Version Re-Verify § Measured at writing. | Hub (Procedure 0) | CHEAP / HIGH |
| **R-12** | **NEW at Commit 0.** Three slices' Stage-5 ADR allocations collided with numbers landed by the #290 merge. | Governance / build | **CLOSED** | ADR next-free was derived at Stage 5 against a then-current mainline; an in-flight sibling merged four ADRs before Engineering started. `check-adr-numbers.py` globs the working tree only and cannot see in-flight branches. | **DISCHARGED for all three slices.** #3581 took `ADR-110` at Commit 0; slice 3 (#3817) re-derived and landed `ADR-111`; slice 5 (#2719) authored no ADR, discharged by elimination (DEV-13). Final set `ADR-110` + `ADR-111`; nothing outstanding. | — | — |
...
| #2719 | Phase A0 cache-read + currency guard + the Procedure 0 Step 5 briefing relay + skill-doc reconciliations | **No template and no ADR to revert** — the A6.5 reversal never created them (DEV-12, DEV-13, DEV-19). Reverting the cache-read **must** also revert the "shipped" reconciliations and the Step-5 relay, or the docs claim a behavior that no longer exists. Because #2719 restructured the same Step 5 that #3581 wrote, revert #2719's clause split **before** #3581's marker emission, never after. |
...
|---|---|---|---|
| **D-Version** | What version does this release claim? | **Recorded determination, not a gate.** Bump-class `minor`. Commit-0 reading (anchor `v4.05`, next-free `v4.06`) is **superseded**: #290 claimed `v4.06` four minutes after Commit 0. Re-derived at writing — anchor `v4.06`, next-free **`v4.07`**, *contended by #305*. Slug-primary naming and the unresolved release-version placeholder stand until the Stage-12 atomic claim, which is the only resolving authority. | CHEAP pre-Engineering / MODERATE after Commit 0 / HIGH |
...
| **DEV-12** | **#2719's design was reversed at A6.5; the shipped shape is hub-writes.** | Stage-5 Revision 0 made **Mode R** the writer: it narrowed Mode R's read-only guardrail to carve out a self-persistence exception, added a 4th hub-state surface (`readiness-briefing.md.template` + `hub-state/README.md` rows + `hub-session-continuity.md` § 3.4), and drafted an ADR. 4 MODIFY + 2 NEW + 1 ADR; impact **Structural**. | The operator **reversed** on the A6.5 reviewer's counter-design: the **hub** relays the briefing into the Stage-4 sub-task comment thread at `hub-spoke-bridge.md` Procedure 0 Step 5 — a surface the hub already creates and already posts to. **Mode R's read-only guardrail is untouched**, so no contract changes. Net **4 MODIFY, 0 NEW, 0 ADR, 0 new surfaces**; impact drops to **Behavioral**. The trust boundary the bridge doc already mandates supplies a provenance control the operator-local file could not have. | **Applied in slice 5.** Guardrail non-modification verified by restricting the diff to the `Mode R state mutation` bullet and the `Recommend-only boundary` paragraph — **empty**. |
| **DEV-13** | **R-12's ADR re-derivation is discharged for slice 5 by elimination, not by derivation.** | DEV-2 issued a standing instruction that **slices 3 and 5** re-derive their ADR number at their own commits, after the #290 merge collided all three Stage-4 allocations. | Slice 3 discharged it by deriving (**ADR-111**). Slice 5 authors **no ADR at all**: Revision 1's threshold argument rested on *"a narrowing of a shipped skill's hard-rejection guardrail"* as its load-bearing clause, and the reversal deleted that narrowing. What remains is an application of existing conventions (the bridge doc's stage-I/O channel) and existing ADRs (ADR-019 compose-not-absorb) — below the bar. **No number claimed, so no number to re-derive.** | **Discharged in slice 5 by elimination.** R-12 closed for this slice. |
| **DEV-14** | **#2719's AC3-semantic probe over-matches on live state.** | The Stage-5 verification plan expects `grep -rn -E "not yet (shipped\|wired)\|until it ships\|when the cache-read wiring ships" release/skills/release-hub/` → **0 hits**, as the arm that catches a caveat reworded rather than removed. | On the landed tree it returns **1**: `milestone-readiness-checklist.md` L23, *"until it ships, the hub chains `intake-desk` + `delivery-engine` directly."* That is a **pre-existing, unrelated, and true** caveat about the unshipped **triage-analysis capability** (groups 1 + 5) — not a reworded cache-read caveat. Verified pre-existing: at branch tip `12f8775e` the token appeared **twice** (L23 and L101); this slice's diff removes L101, the cache-read caveat, and leaves L23 byte-untouched. A cache-read-scoped narrowing of the same probe returns **0**. | **Logged, not corrected.** Editing a true statement about a different unshipped capability to satisfy an over-broad probe would be a defensive edit. **Stage 8 grades the intent** (no reworded *cache-read* caveat survives), not the literal arm. |
| **DEV-15** | **A third `C1/C2/C3 + PT` naming site in `release-hub/SKILL.md` is deliberately not edited.** | Blocker 1 named **two** SKILL.md spots carrying the two-of-six-columns misnaming — Output-Contract requirement 3 and Mode R's `**Output:**` clause (b) — plus the failure mode's `Root cause` + `Mitigation`. Four anchors. | The § Composition table's groups-3-and-4 row also reads *"emits the C1/C2/C3 + PT schema"* — a coarser third naming of the same artifact. It matches **no** acceptance probe (AC1 is scoped to the pipeline spec; AC3 and AC3-semantic do not hit it), and editing it would expand the **four-anchor declared scope** that #3819's contention analysis for this shared file relies on. | **Routed to the hub, not actioned.** Cross-scope findings are routed, never acted on directly (Procedure 3 spoke discipline). Intake candidate for a follow-up sweep of coarse schema namings. |
| **DEV-16** | **#3819's four cascade files were in the Stage-5 file list but not in the Stage-4 File Change Matrix.** | The matrix's machine-readable path list carried #3819's three primary files (`hub-spoke-bridge.md`, `automated-closeout.sh`, `check-milestone-epic-membership.py` + `deploy.sh`) and both `release-hub` package artifacts. | Stage-5 Revision 1 § "Revised file-level spec" rows 5–8 name four cascade files — `core/disciplines/execution-framework.md`, `docs/FIRST_STEPS.md`, `release/skills/release-hub/SKILL.md`, `release/skills/release-hub/references/orchestration-playbook.md`. Only `SKILL.md` was already in the list (claimed by #2719). The other three are **added at Commit 6** with per-file intent rows. `execution-framework.md`'s line was not merely imprecise — it enumerated Stages 9/12/13 as *per-issue*, the exact conflation this card removes. | **Applied in slice 6.** Path list + two matrix rows added. No new contention: the three added files have no other claimant in this release. |
| **DEV-17** | **The close-out self-test legs land as 4i/4j, not the design's 4e/4f.** | Stage-5 Revision 1's file-level spec says `automated-closeout.sh` gains *"`--self-test` Tests 4e/4f"*. | `Test 4e` (await-merge budget/escape, #1705) and `Test 4f` (RELEASE_LOG VERIFIED re-derivation guard, #1681) **already exist** in the host file; the design's numbering was allocated without reading the live test roster. Landed as **Test 4i** (the three-rung ladder, carrying T-13) and **Test 4j** (T-14's double-call idempotence). Content is exactly as specified — only the labels moved, to labels that were free. | **Applied in slice 6** as a B3 minor adjustment. No test was renamed or displaced. |
| **DEV-18** | **Two T-11 live-leg expectations drifted with the population between design and implementation.** | Stage-5 Revision 1's T-11 table states four live legs with expected values *in advance*: `#299 → COUNT_M3 0 / COUNT_M3_ADV 0`; `#264 → COUNT_M3 > 0, ≥21 ORPHAN-STAGE-TITLE`; `#305 → COUNT_M3 0, M3-POLICY MISSING 10 + 11, SCAFFOLD_MARKER 0/24`; `#290 → COUNT_M3 0, M3-POLICY ×5, SCAFFOLD_MARKER 0/74`. | Three legs matched **exactly**, including both arms that were designed to be able to fail. The fourth, #305, returned `SCAFFOLD_MARKER 0/**25**` — the milestone gained one stage sub-task between the 2026-08-03 census and Commit 6; every asserted class (`COUNT_M3 0`, exactly `MISSING 10` + `MISSING 11`) held. Separately, the **default** (all-open-milestones) path now fires on **six** milestones, not the census's three: #290 has since merged and closed, and #246/#273/#286/#306 have each acquired a single stage sub-task. `COUNT_M3` is **0** across the whole live open population. | **Logged, not corrected.** A denominator that tracks live membership is the leg working, not drifting. Stage 8 should grade the asserted classes, not the census-era counts. |
| **DEV-19** | **#2719's File Change Matrix rows still described the pre-reversal Revision-0 design.** | The matrix carried #2719 as 3 MODIFY + 2 NEW + 1 ADR, one row instructing the slice to *"**narrow** the Mode R state-mutation guardrail"*, and no `hub-spoke-bridge.md` attribution — the Revision-0 shape DEV-12 records as reversed. | Five corrections, all to the record only: **(1)** the guardrail row is the **exact inverse** of the shipped invariant — the reversal left the guardrail untouched, and it is **byte-identical to `main`** (the two-region hash matches while the whole-file hash differs, so the probe fires); the row now describes the schema-naming and failure-mode corrections that did ship. **(2)** Three rows named artifacts deliberately **not** created (Surface D template, its two registration surfaces, and the ADR discharged by elimination at DEV-13); a fourth, `triage-design-rereview.md`, was optional-and-not-taken per DEV-4. All four are folded into one explicit *not-created* row and dropped from the machine-readable path list, where they were pinning baselines on files this release never touches. **(3)** `hub-spoke-bridge.md` Procedure 0 Step 5 shipped in slice 5 **unattributed** — it is the surface the reversal moved the briefing onto, so its absence made the reversal unreadable from the matrix alone. Now attributed, with the #3581 / #3819 co-tenancy named. **(4)** Net restated as **4 MODIFY, 0 NEW, 0 contract changes, 0 ADR**, matching DEV-12. **(5)** The same phantom-ADR claim survived in three further places, each of which read as an *outstanding obligation* to a Stage-9 reader: § ADR Allocation's #2719 row (*"re-derive at its Commit"*), its standing instruction to slices 3 and 5, and risk **R-12** (*"two remain unresolved"*, severity MED). All three now carry the discharge — #3817 by deriving `ADR-111`, #2719 by elimination — and R-12 is **CLOSED**. Final ADR set for the release: `ADR-110` + `ADR-111`. **No number is renumbered; only the record of what was already done.** | **Applied at batch 5 (records only).** DEV-12's *"Stage-5 outcome wins"* precedence already let a careful reader resolve this, but **DEV-16 set the precedent of updating the matrix** for exactly this class. No behavior change; no file outside the plan is touched. |
| **DEV-20** | **QA-F-01 — `fetch_stage_titled` reads against the GitHub search 1000-cap with no truncation guard.** | Stage-8 QA finding, disclosed here rather than fixed. | `fetch_stage_titled` (`core/deploy/tools/check-milestone-epic-membership.py`) issues one `gh issue list --search "Stage in:title" --state all --limit 1000` and never checks whether the result hit the cap — in a file whose own docstring states *"a truncated membership set must never read green."* Population measured **2990** stage-titled issues (issue-scoped, all states) at batch 5, against a control arm returning **0**, so the probe discriminates; QA measured ~2988. Roughly two-thirds of the population is silently dropped. **Bounded:** the M3 leg routes through `flag_advisory_only`, which has no `ISSUES` increment and no mode case, so it **cannot move a `--check` exit code**; and `automated-closeout.sh` does not reference the script at all, so **Stage 13 is untouched**. | **Filed as #4720** (`bug`, OPEN). **Not fixed here** — a code change is out of scope for a records batch. Stage 9 sees a disclosed, tracked, advisory-only finding, not a silent one. |
