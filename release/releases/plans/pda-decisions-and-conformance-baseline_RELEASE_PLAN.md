...
core/ADRs/ADR-178-instance-conformance-validator-surface.md  ADD
...
| **#5841 conditional rows resolved** (Stage 6 execution note) | Minor | Both `CONDITIONAL:` matrix rows resolved in the ADR-authoring commit per the same-commit rule: the validator-surface row **PROMOTED** (the recommendation implies an architectural decision — ADR-178 authored, number allocated at the commit instant via `--detect` above the union of the mainline anchor and this branch's own ADR-177 claim); the `core/ADRs/README.md` row **DROPPED** (the README declares itself a curated thematic document, NOT an index — "do not file 'missing records' as a completeness defect" — so no registration exists to perform; zero README edit made). The R7 `NOT DELIVERED` branch was not taken. |
| **#5841 Wave-2 finalization collapsed into the Wave-1 slice** (Stage 6 execution note) | Minor | The § Implementation Sequence placed the cadence finalization at Wave 2 behind a finish-to-finish edge on #5840's baseline. The baseline landed complete (zero UNMEASURED cells) before this slice began, so the edge was already discharged and the P-CADENCE band was selected inside the authoring slice — band **B2** from the measured volumes, recorded in ADR-178 Decision §4. No edge violated; the plan's Wave-2 beat for #5841 is discharged early, leaving #5836 as the sole Wave-2 item. |
| **#5836 consuming-edit set enlarged by one surface** (Stage 6 design delta) | Minor | Discharging entry condition 6 has a consequence the Stage-5 blast radius did not carry: anchoring the namespace root on `project_id` requires the seed-idempotency clause that re-scopes the folder-slug derivation in `core/schemas/project-schema.md` from source-of-truth to seed-time fallback, and the ninth binding's enforcement row must live in that same persistence dialect — the only surface where both `id` and `project_id` are visible (Project's entity field table carries no `project_id` field, verified at source, so this binding crosses a layer boundary the other eight do not). That file was previously listed as a consumer owing no edit; it owes two. **No consuming edit lands in this release** — the delta is recorded in ADR-175 § Consequences and belongs to the delivery child on milestone #358. |
| **#5836 README registration dropped** (Stage 6 execution note) | Minor | The Stage-5 handoff directed Commit 0 to register the new record in `core/ADRs/README.md`. Re-verified at source rather than inherited from the sibling slice's identical finding: that README self-declares at line 9 as *"a curated thematic document, NOT an index"* and instructs that missing records are not to be filed as a completeness defect, with the always-complete list being the file set itself. No registration exists to perform; zero README edit made. Same disposition the #5841 slice reached independently. |
| **#5836 person-FK census corrected in-flight** (Stage 6 execution note) | Minor | The Stage-5 design stated 13 inbound person-FK X-rules; an independent row census at the authoring baseline returned **12**, matching the schema's own §3.10 anchor note verbatim. The corrected figure is what ADR-175 carries. Recorded because a wrong count inside an immutable governance record propagates — the design's own thesis is census precision. |
...
(Authored per `release/governance/RELEASE_PROTOCOL.md` § Change Description Protocol; refreshed as slices land on this branch — as of Engineering Commit 0 plus the #5840, #5838, #5841, and #5836 slices, **all four issues are engineered**: the baseline measurement, the lifecycle audit-trail carrier ADR (ADR-177), the validator-surface ADR (ADR-178), and the cross-boundary key-form ADR (ADR-175). No slice remains.)
...
| #5841 | Validator-surface recommendation (conditional matrix row promoted) | `artifact-accepted` (`core/ADRs/ADR-178-instance-conformance-validator-surface.md` on this branch) |
| #5836 | Cross-boundary key-form ADR | `artifact-accepted` (`core/ADRs/ADR-175-cross-boundary-key-form-for-entity-and-tracker-identifiers.md` on this branch) |
| #5838 | Lifecycle audit-trail carrier ADR | `artifact-accepted` (`core/ADRs/ADR-177-lifecycle-audit-trail-carrier-or-retirement.md` on this branch) |
...
#358 / #360 / #361 consume the three ADRs and the baseline; they remain blocked until this release lands, and their sizing reads the baseline's per-class counts. Two sizing deltas surfaced during Engineering and are recorded in § Deviation Log for the consuming children rather than acted on here: the key-form decision's consuming-edit set gains one surface (`core/schemas/project-schema.md`, which owes the seed-idempotency re-scope and the namespace-root enforcement row), and the baseline found the instance's largest reference class carried in name form rather than id form with most values unresolvable — a conformance-remediation workload the key form does not cure.
...
| Deliverable | Declared canonical path | Acceptance evidence |
|---|---|---|
| Lifecycle audit-trail carrier decision record (#5838, task-class) | `core/ADRs/ADR-177-lifecycle-audit-trail-carrier-or-retirement.md` (this branch) | The record itself (durability lint COUNT 0 with fired control arm; AC method probes above; status `Proposed` pending Stage 13 Phase A13 ratification — the record's own `status:` field is the authority) + #6663 Stage-6 comment |

### #5841 slice (Engineering spoke #6655, 2026-09-02)

| Check | Evidence | Verdict |
|---|---|---|
| AC-1 (one surface named; ≥2 rejected alternatives with trade-offs) | ADR-178 Decision §1 names the deploy-check family member (check id `instance-conformance`, engine `core/deploy/tools/check-instance-conformance.py`, warn-mode initial); § Alternatives Considered carries **eight** rejections with grounds (doctor mode · structure-mode-as-home · scheduled-task-alone · hook · direct-invocation cadence · per-gate sink · flip-criterion-as-drafted · restatement-without-instrument). Method probes at the branch tip: `grep -cE '^## Decision'` → 1; rejection bullets in § Alternatives → 8; fabricated-token control → 0 | PASS |
| AC-2 (warn-to-enforce path + flip evidence named against a run-grain-capable instrument) | ADR-178 Decision §3: **unconditional per-run summary row** appended to the shared sink from both invocation paths (row shape stated; extends the sink's realized non-violation row classes `advisory:true` / `evaluated:false`); flip criterion restated to **K=3 consecutive run-summary rows with `warns: 0`, counted over run-summary rows — never row-absence** (zero rows in window = INSTRUMENTATION-SUSPECT). **Emitter named and run-outcome universe closed at Stage 9 (F3 / F4):** the row's emitter is `flag_run_summary`, a mode-independent sink appender with no mode `case` and no `ISSUES` increment — explicitly **not** `flag_warn_or_issue`, which writes the sink only on its `warn` leg (verified at source: its `enforce` leg logs a FAIL and increments `ISSUES`, touching the sink not at all), so the "warn mode and enforce mode alike" property had no named emitter that could deliver it and a reader could infer the row rode the one emitter that cannot. The shape is the realized one — `flag_advisory_only` and `flag_not_evaluated` carry no mode `case` either. The outcome universe now enumerates **three** — completed / skipped / usage-error — all three emitting the row (the usage-error row carries an `error` field; the sink lives in the platform state container, so it stays writable when `--root` does not resolve), and the criterion excludes rows carrying `skipped` or `error`. That closes the leak in "consecutive": an unenumerated silent outcome would not break the streak, so clean / usage-error / clean / usage-error / clean would have satisfied K=3 with 40% of its runs measuring nothing; W1 disclosure carried ("specified, not yet emitting"); W2 register row authored with the emitter at the delivery child's release, blocker named per the blocker-not-schedule rule (remediation children merged + burn-down re-enters B1); flip form = committed default `resolve_check_mode "instance-conformance" "enforce"` via PR, never the mode file | PASS |
| AC-3 (#158 named and dispositioned) | ADR-178 Decision §6: **rejected as the recurring home on five limbs** (per-project grammar; LLM-executed vs deterministic fixture ACs; scheduled output writes inside the project tree; advisory-only runner admission with no sink → no flip path; per-project token cost) and **adopted in two capacities** (shared-class contract baseline — derived denominator, coverage envelope, UNMEASURED-never-zero; per-project judgment surface), with the partition rule stated once. § Provenance names #158 explicitly. Method probe: `grep -c "structure" core/ADRs/ADR-178-instance-conformance-validator-surface.md` → 9 (non-zero); engine attribution matches #5840's report (C1–C4 → structure engine; C5–C7 → spike predicates; validator succeeds the spikes) — CIAC-3 co-verification pre-staged | PASS |
| Entry condition 8 (flip-evidence repair — binding, scope-lock) | Discharged by **both permitted routes composed**: a run-grain record is specified (route a — Decision §3 row spec) AND the criterion is restated against that now-readable instrument (route b — K counts run-summary rows). Route b alone was unavailable and the ADR records why: no governed surface records run-grain state (stdout unpersisted; deploy logs ungoverned; a per-gate sink against the W1 shared-sink convention). The `flag_not_evaluated` `evaluated:false` row class is the cited precedent that run-state rows are sink-conventional | DISCHARGED |
| Entry condition 9 (scheduled-path emitter bifurcation) | Discharged by **adopting adversarial counter-design CD-1**: the scheduled entry is a new single-check dispatcher arm `--check-instance-conformance` wrapping the same block (engine + `resolve_check_mode` + **both** emitters, `flag_warn_or_issue` for per-finding rows and `flag_run_summary` for the §3 run-grain row + shared sink + rotation). **Exemplar set corrected at Stage 9 (F5):** the row previously named *"five live arms verified at source: version-freeness, close-completeness, decision-emission, required-subset, package-freshness"* — the count 5 was right and two members were wrong. Re-derived at source, `deploy.sh` carries **seven** `--check-*)` case arms; `--check-lifecycle` is a listing mode, not a check probe, and `--check-required-subset` is a multi-check roster runner (its help text: "CI subset runner — enumerated load-bearing checks"), which makes it the single arm in the family lacking the one-arm-to-one-check-identity property this discharge rests on. The five arms that do realize the composed form are version-freeness (Check 41), close-completeness (Check 48), decision-emission (Check 61), **release-corpus (Check 32 — a genuine single-check probe the original list omitted)** and package-freshness (Check 7); register precedent = numbered check + single-check probe under one identity. Direct engine invocation is declared a diagnostic view (stdout-only, sink-blind, mode-blind) and excluded from the cadence recipe; a committed enforce flip binds both cadences through the one precedence model | DISCHARGED |
| Adversarial dispositions (FM-3 / FM-4 / CD-1 / CD-2 / PR-2 / PR-3) | **FM-3 ADOPTED** (ADR §5): engine `--root` REQUIRED (no default, no env read — corrects the design's resolver mis-statement; `pmo_operations_path_for` carries no env/toml tier, takes workspace root as caller argument); SKIP names the resolved root; `resolver-miss-suspect` WARN when workspace root exists but subject dir absent; open P2 resolver defect named in Provenance. **FM-4 ADOPTED** (ADR §4): instrument swap declared; first deterministic run = re-baseline with per-class reconciliation + named band re-selection trigger. **Robustness leg WITHDRAWN at Stage 8 (AI-004):** the disposition previously read *"band robust regardless (deterministic-subset volume alone clears the threshold)"* — the deterministic subset clears at **itemized grain only** (217, margin 17, 96.3% one rule class), and the same subset at record grain derives to ≤76 = B1, so that cell shares the baseline's grain convention but not its instrument and settles nothing about the swap. The band now rests on the pinned V unit plus the declared-provisional first-run re-baseline, and a first-run drop into B1 is recorded as an expected outcome that triggers re-selection. **CD-1 ADOPTED** (= condition 9). **CD-2 ADOPTED** (ADR §9): fixture self-test CI wiring MANDATORY (install-tests shell-tests job, no paths filter, + deploy self-test group) — the scored CI differentiator is delivered, not optional. **PR-2 CORRECTED** (ADR §4): no governed ritual invokes the full `--check` battery (close-out binds an output set via the Step-4 table + CI single-check lanes; completeness checks evaluate whenever `--check` runs, they never cause it) — the honest floor is operator habit + the load-bearing scheduled dispatcher run. **PR-3 MOOT-BY-REDERIVATION**: `--detect` re-run live at the commit instant, stale Stage-5 namespace figures never consumed | RECORDED |
| ADR numbering (CIAC-1 input) | `renumber-adr.py --detect` at the commit instant: `ANCHOR 172 origin/main / NEXT-FREE 173 / CLAIMED-SET-BRANCH-ONLY 173,174,175,176 (never binds) / CLAIM ADR-177 … BINDS BRANCH-CLAIM` → union of mainline anchor + this tree's own claim = allocate **ADR-178** (never `max(claimed)+1`; sibling branches' 174–176 claims are detection-only). Post-write re-run: `CLAIM ADR-177 … BINDS` + `CLAIM ADR-178 … BINDS`. Numbering-provenance block recorded in the record's Status section | PASS |
| Durability lint (governed check, with control arm) | `check-adr-durability.py --files core/ADRs/ADR-178-… --diff-base origin/main` → `COUNT 0`, exit 0; control arm — `## Reversibility` heading temporarily broken → `R5-NEW` fired (`COUNT 1`, exit 1), restored → `COUNT 0`, exit 0 — instrument verified live | PASS |
| INT-1 (cadence cites #5840's measured volume; band selected) | ADR-178 Decision §4 cites the baseline's per-class counts (full set preserved in the record's `source_observations`) and the derived sums — 873 itemized / 412 (dominant field class collapsed to record grain) / 217 deterministic-instrument subset at itemized grain, derivation stated — all three **measured** cells above the 200 threshold → **band B2 selected**. **Invariance claim CORRECTED at Stage 8 (AI-004):** §4 previously read *"invariant to grain choice and to the instrument split"*; each figure varies one axis and the joint deterministic-at-record-grain cell was never measured (derives to ≤76 = B1), so the record now claims stability across the **measured cells only** and states the unmeasured cell, its B1 derivation, the 17-margin and the 96.3% single-class concentration explicitly. **Root cause fixed:** the band key's `V` carried no unit, so the 200 threshold had no reproducible basis; V is now pinned to *itemized findings counted by one deterministic-validator run* (the run-summary row's `warns`), and the selection is declared provisional against that unit until the validator's first run re-baselines it. **The pin names its reading** — findings **counted**, never WARN rows **emitted**: at B2 the reporting grain is throttled to one row per violating class per project, so as of the same baseline the run counts 217 findings while emitting 9 rows, and those readings fall in different bands. Reading V off emitted rows would make the band a function of its own throttle (a B2 selection re-deriving as B1 with nothing changed); the reporting grain is an output of the band, never an input. This ambiguity was the residual half of the unpinned unit — Stage 7's F2 table listed the 9-row cell — and an unqualified pin would not have been reproducible. K=3 bound for the flip at B1 re-entry; threshold 200 stays [RECOMMENDED] operator-ratifiable | PASS (post-repair) |
| INT-2 (engine attribution identical across #5840 report and #5841 record) | ADR-178 Decision §6 attributes the baseline's entity/field/value/reference classes to the structure engine and RAID/tracker/taxonomy to the spike predicates — identical to the report's engine-attribution; no class attributed to different engines; the validator's takeover of the deterministic classes is declared as succession with the §4 instrument-swap threat, not silent re-attribution | PRE-STAGED (Stage-8 graded) |

**Artifact-Acceptance Record (deliverable_state: `artifact-accepted`):**

| Deliverable | Declared canonical path | Acceptance evidence |
|---|---|---|
| Validator-surface decision record (#5841, task-class) | `core/ADRs/ADR-178-instance-conformance-validator-surface.md` (this branch) | The record itself (durability lint COUNT 0 with fired control arm; AC method probes above; status `Proposed` pending Stage 13 Phase A13 ratification — the record's own `status:` field is the authority) + #6655 Stage-6 comment |

### #5836 slice (Engineering spoke #6659, 2026-09-02) — Wave 2, final build

| Check | Evidence | Verdict |
|---|---|---|
| AC-1 (one key form decided; ≥2 rejected alternatives with trade-offs) | ADR-175 `## Decision` D1 decides the qualified composite `<project_id>/<id>` under one canonical grammar; `## Alternatives Considered` carries **five** rejections with grounds (URI scheme · global-id promotion for all entities · type-tag-only · folder-derived-root anchoring · dropping the redundant `<entity>_id` fields). Method probes at the branch tip: required §3 section set present **and in order** (7 of 7; specificity arm — a fabricated heading — 0); `## Alternatives Considered` rejection items → 5 | PASS |
| AC-2 (unconditional `id` ↔ `<entity>_id` binding rule) | ADR-175 D3 states the eight-entity byte-identity rule; D2 clause 2 states the **ninth** binding on the namespace root per scope-lock condition 6. Method probe: `This rule is unconditional` → **2** occurrences (one per rule); hedge-word scan inside the rule sentences (`should|unless|typically|generally|may`) → **0** | PASS |
| AC-3 (surviving FK-target convention named; affected X-rule ids enumerated) | ADR-175 D5 states the surviving convention (`<Entity>.<entity>_id` for the eight dual-key entities, `<Entity>.id` for the eleven single-key entities, co-referential under D3) and enumerates the delta by rule id — **X-05 / X-28 / X-29** change target form, **X-24** explicitly unchanged (dissolved by D2), V-CORE-07's `relationships[].target` gains a value-grammar note. Method probe: all four ids present; specificity arm (`X-99`) absent | PASS |
| Entry condition 6 (root re-anchoring — binding, scope-lock) | Discharged, and the finding was **re-verified at source on `origin/main` before adopting it** rather than relayed: the persisted key set derives `Project.id` from *"the project folder slug, normalized per artifact-naming-standard"*, while the project-identity resolution rule governs `project_id`, *"the kebab-case slug carried in `PROJECT.md` frontmatter"*, names **that** field the sole join key, forbids ever deriving a `project_id` from a basename, and states that the two *"are **not required to be equal**, and equality is never assumed"* — stronger than the finding as filed. ADR-175 D2 anchors `PROJECT` on `project_id`, adds the ninth identity binding (`Project.id` MUST be byte-identical, anchor = `project_id`), and adds the seed-idempotency clause re-scoping the folder-slug derivation to a seed-time fallback | DISCHARGED |
| Entry condition 7 (R-REF-1 / R-REF-3 reconciliation; X-05 `.id` binding vs the repair guarantee) | Both discharged unconditionally. **Rule pair:** ADR-175 D4 R-REF-1 extends the bare-reference resolution chain to the two singleton global tiers and Project, so a bare person reference is in-chain and R-REF-1 never demands the qualification R-REF-3 forbids; R-REF-3 is restated as R-REF-2's forbidden limb (a corollary, not a competing rule) with a worked bare-person-reference example. **Guarantee:** D3 replaces *"no inbound edge ever breaks"* — false as stated — with the scoped form (natural-key edges preserved; alias-bound edges are not) plus the mandatory second repair step re-pointing alias-bound inbound references. Verified at source: X-05 declares `→ <any entity>.id` | DISCHARGED |
| Minor adversarial corrections (4, each dispositioned) | **Person-FK count CORRECTED** 13 → **12**: independent row census of the §4 table at the authoring baseline (37 rows; sensitivity arm V-CORE rows 7; specificity arm 0) returned 12, matching the schema's own §3.10 anchor note. **Denominator NOT CARRIED**: the tracked-markdown population is a moving figure (three different values were observed at three pins across this release) — it is confined to `source_observations` (schema-exempt point-in-time grounding evidence) and never stated as a live body claim. **Citation REPLACED**: the "view of `id`" note attributed to a tracker-schema line does not exist there (that line is the RAID_ID row); R-REF-5 is re-grounded on the `source_ref` / `source_inputs[]` provenance token domain, verified at source, and the false identity premise is withdrawn in the rule text. **P-invariance overclaim CORRECTED**: the global-promotion elimination now names which legs carry it (write-time unenforceability, frozen-surface semantics change, consumer blast) and explicitly **withdraws** the volume-dependent re-key leg. **Withdrawal ground CORRECTED at Stage 7:** the leg is withdrawn as *unevidenced*, not as measured-away — `project_id` is absent from all 3 live PROJECT.md records, so an anchor-key collision count has denominator 0 (the earlier "measured zero-collision result drains of force" phrasing rested a volume leg on a count read against the rejected key) | RECORDED |
| Grammar-prose defect (adversarial PR-5) | **CORRECTED in the record**: `LOCAL` is stated as *a single reference token drawn from the T1 charset*, not *one T1 segment* — the naming standard makes `_` the primary segment separator, so the predecessor prose was false of its own regex. The widening is now declared with its reason (tracker row-id dialects `TR-004` / `R-PPM-001` are legitimate targets a kebab-only production would reject) | RECORDED |
| Counter-design CD-2 (canonical serialization) | **ADOPTED** into D4 R-REF-2/R-REF-4: qualification REQUIRED iff crossing and FORBIDDEN otherwise; TAG REQUIRED iff polymorphic and FORBIDDEN otherwise — exactly one admissible spelling per (referencing-scope, declared-target) pair. This closes the one-referent-two-spellings failure mode during the migration window D6 opens, and strictly narrows the admissible set (every currently-canonical value stays valid). Adopted as part of the entry-condition-7 reconciliation, not as a re-decision of the key form — the qualified composite itself was attacked and held, and is not reopened | RECORDED |
| ADR numbering (CIAC-1 input) | `renumber-adr.py --detect` run from the worktree root at the commit instant: `ANCHOR 172 origin/main / NEXT-FREE 173 / CLAIMED-SET-BRANCH-ONLY 173,174,175,176 (detection only — never binds)` with `CLAIM ADR-177 … BINDS` and `CLAIM ADR-178 … BINDS` → union of the mainline anchor and this tree's own two claims = allocate **ADR-175** (never `max(claimed)+1`; the sibling branch-only claims on 175–176 are detection-only, and a cross-branch collision is governed by merge-time renumber, never dodged by skipping ahead). Post-write re-run returns three `BINDS` rows, no DUPLICATE and no WOULD-GAP. Numbering-provenance block recorded in the record's Status section | PASS |
| Durability lint (governed check, with control arm) | `check-adr-durability.py --files core/ADRs/ADR-175-… --diff-base origin/main` → `SCANNED 1 / COUNT 0`, exit 0 (R5 + R6 armed by the diff base). Control arm — `## Reversibility` heading temporarily broken → `R5-NEW` fired (`COUNT 1`), restored → `COUNT 0`, exit 0. Second control arm on R2-COUNT — a seeded live-count line fired 2 rows on this file, removed → 0, corpus-wide specificity arm 0 — so the record's zero is a measured zero, not an unarmed instrument. Count-bearing prose carries the checker's own closed historical-framing anchors, which is the sanctioned form for a dated measurement in an immutable record | PASS |
| Doc-link integrity (Phase C4) | The record contains **zero** markdown links by construction (ADR references are made in ADR-number and prose form per the schema's value conventions), so Check 14 has no link to resolve in this file. Stated as a trivially-satisfied check rather than reported as a pass earned | N/A — no links |
| PII / public-surface audit (with control arm) | Subject arms over the record: personal-email 0, operator-path 0, scratch-path 0, operator-handle 0. Sensitivity control (`project`, case-insensitive) → 103 non-zero; specificity control → 0. Close-family-verb-adjacent-to-`#N` scan → 0, with the issue-reference sensitivity arm non-zero (3 refs present, all inside the designated References block) | PASS |
| INT-1 (ADR cites #5840's measured datum; placeholder does not survive) | ADR-175 D6 discharges the migration-volume parameter **MEASURED**, not `UNMEASURED`: 7 bare-slug cross-boundary values of 93 stored cross-entity reference values, and 0 pairwise Project-id collisions across 3 live ids — every figure taken from the baseline's own class table, with the full measured set preserved in the record's `source_observations`. Method probe: `P_REKEY_DATUM` → **absent**; the keyword scan returns 28 hits carrying counts. Two properties are recorded rather than assumed. **The first was stated FALSELY at Stage 6 and was repaired at Stage 7:** the collision count was NOT read on the anchor key — it was read on frontmatter `id` (the kebab-fold of the folder basename, the surrogate Alternative 4 rejects), because `project_id` is absent from **all 3** live PROJECT.md records, so an anchor-key collision count has **denominator 0** and is structurally unmeasurable. D2 does not rest on that zero; it rests on governance grounds. The record now states this, and the absence is reframed as an empty anchor population to *seed* under D2.3 rather than a divergent population to scan. The second property stands as written: the baseline's person-reference finding is carried into `## Consequences` as work this decision does not cure | REPAIRED AT STAGE 7 (Stage-8 graded) |

**Artifact-Acceptance Record (deliverable_state: `artifact-accepted`):**

| Deliverable | Declared canonical path | Acceptance evidence |
|---|---|---|
| Cross-boundary key-form decision record (#5836, task-class) | `core/ADRs/ADR-175-cross-boundary-key-form-for-entity-and-tracker-identifiers.md` (this branch) | The record itself (scoped durability lint COUNT 0 with two fired control arms; AC-1/AC-2/AC-3 method probes above; entry conditions 6 and 7 discharged; status `Proposed` pending Stage 13 Phase A13 ratification — the record's own `status:` field is the authority) + #6659 Stage-6 comment |
