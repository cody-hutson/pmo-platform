...
(Authored per `release/governance/RELEASE_PROTOCOL.md` § Change Description Protocol; refreshed as slices land on this branch — as of Engineering Commit 0 plus the #5840, #5838, #5841, and #5836 slices, **all four issues are engineered**: the baseline measurement, the lifecycle audit-trail carrier ADR (ADR-177), the validator-surface ADR (ADR-178), and the cross-boundary key-form ADR (ADR-179). No slice remains.)
...
| #5836 | Cross-boundary key-form ADR | `artifact-accepted` (`core/ADRs/ADR-179-cross-boundary-key-form-for-entity-and-tracker-identifiers.md` on this branch) |
...
| INT-1 (ADR cites #5840's measured datum; placeholder does not survive) | ADR-179 D6 discharges the migration-volume parameter **MEASURED**, not `UNMEASURED`: 7 bare-slug cross-boundary values of 93 stored cross-entity reference values, and 0 pairwise Project-id collisions across 3 live ids — every figure taken from the baseline's own class table, with the full measured set preserved in the record's `source_observations`. Method probe: `P_REKEY_DATUM` → **absent**; the keyword scan returns 28 hits carrying counts. Two properties are recorded rather than assumed. **The first was stated FALSELY at Stage 6 and was repaired at Stage 7:** the collision count was NOT read on the anchor key — it was read on frontmatter `id` (the kebab-fold of the folder basename, the surrogate Alternative 4 rejects), because `project_id` is absent from **all 3** live PROJECT.md records, so an anchor-key collision count has **denominator 0** and is structurally unmeasurable. D2 does not rest on that zero; it rests on governance grounds. The record now states this, and the absence is reframed as an empty anchor population to *seed* under D2.3 rather than a divergent population to scan. The second property stands as written: the baseline's person-reference finding is carried into `## Consequences` as work this decision does not cure | REPAIRED AT STAGE 7 (Stage-8 graded) |
