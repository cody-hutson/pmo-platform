...
| Entry condition 9 (scheduled-path emitter bifurcation) | Discharged by **adopting adversarial counter-design CD-1**: the scheduled entry is a new single-check dispatcher arm `--check-instance-conformance` wrapping the same block (engine + `resolve_check_mode` + `flag_warn_or_issue` + shared sink + rotation) — the family's realized pattern (five live arms verified at source: version-freeness, close-completeness, decision-emission, required-subset, package-freshness; register precedent = numbered check + single-check probe under one identity). Direct engine invocation is declared a diagnostic view (stdout-only, sink-blind, mode-blind) and excluded from the cadence recipe; a committed enforce flip binds both cadences through the one precedence model | DISCHARGED |
