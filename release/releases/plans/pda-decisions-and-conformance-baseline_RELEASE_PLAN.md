...
| Adversarial dispositions (FM-3 / FM-4 / CD-1 / CD-2 / PR-2 / PR-3) | **FM-3 ADOPTED** (ADR §5): engine `--root` REQUIRED (no default, no env read — corrects the design's resolver mis-statement; `pmo_operations_path_for` carries no env/toml tier, takes workspace root as caller argument); SKIP names the resolved root; `resolver-miss-suspect` WARN when workspace root exists but subject dir absent; open P2 resolver defect named in Provenance. **FM-4 ADOPTED** (ADR §4): instrument swap declared; first deterministic run = re-baseline with per-class reconciliation + named band re-selection trigger; band robust regardless (deterministic-subset volume alone clears the threshold). **CD-1 ADOPTED** (= condition 9). **CD-2 ADOPTED** (ADR §9): fixture self-test CI wiring MANDATORY (install-tests shell-tests job, no paths filter, + deploy self-test group) — the scored CI differentiator is delivered, not optional. **PR-2 CORRECTED** (ADR §4): no governed ritual invokes the full `--check` battery (close-out binds an output set via the Step-4 table + CI single-check lanes; completeness checks evaluate whenever `--check` runs, they never cause it) — the honest floor is operator habit + the load-bearing scheduled dispatcher run. **PR-3 MOOT-BY-REDERIVATION**: `--detect` re-run live at the commit instant, stale Stage-5 namespace figures never consumed | RECORDED |
...
| INT-1 (cadence cites #5840's measured volume; band selected) | ADR-174 Decision §4 cites the baseline's per-class counts (full set preserved in the record's `source_observations`) and the derived sums — 873 itemized grain / 412 record grain / 217 deterministic-instrument subset, derivation stated — every grain above the 200 threshold → **band B2 selected, invariant to grain choice**; K=3 bound for the flip at B1 re-entry; threshold 200 stays [RECOMMENDED] operator-ratifiable | PRE-STAGED (Stage-8 graded) |
