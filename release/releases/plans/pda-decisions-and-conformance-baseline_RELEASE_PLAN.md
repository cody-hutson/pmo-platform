...
| INT-1 (ADR cites #5840's measured datum; placeholder does not survive) | ADR-175 D6 discharges the migration-volume parameter **MEASURED**, not `UNMEASURED`: 7 bare-slug cross-boundary values of 93 stored cross-entity reference values, and 0 pairwise Project-id collisions across 3 live ids — every figure taken from the baseline's own class table, with the full measured set preserved in the record's `source_observations`. Method probe: `P_REKEY_DATUM` → **absent**; the keyword scan returns 28 hits carrying counts. Two properties are recorded rather than assumed: the collision count was read **on the frontmatter basis**, which is the key D2 anchors the namespace root on, so the zero measures the anchor actually chosen; and the baseline's person-reference finding is carried into `## Consequences` as work this decision does not cure | PRE-STAGED (Stage-8 graded) |
