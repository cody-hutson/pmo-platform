...
(Authored per `release/governance/RELEASE_PROTOCOL.md` § Change Description Protocol; refreshed as slices land on this branch — as of Engineering Commit 0 plus the #5840, #5838, and #5841 slices, the baseline measurement, the lifecycle audit-trail carrier ADR (ADR-173), and the validator-surface ADR (ADR-174) are the engineered portion; the cross-boundary key-form ADR lands in the remaining slice.)
