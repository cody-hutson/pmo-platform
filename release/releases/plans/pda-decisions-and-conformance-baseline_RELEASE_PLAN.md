...
(Authored per `release/governance/RELEASE_PROTOCOL.md` § Change Description Protocol; refreshed as slices land on this branch — as of Engineering Commit 0 plus the #5840, #5838, #5841, and #5836 slices, **all four issues are engineered**: the baseline measurement, the lifecycle audit-trail carrier ADR (ADR-177), the validator-surface ADR (ADR-174), and the cross-boundary key-form ADR (ADR-175). No slice remains.)
...
| #5841 | Validator-surface recommendation (conditional matrix row promoted) | `artifact-accepted` (`core/ADRs/ADR-174-instance-conformance-validator-surface.md` on this branch) |
