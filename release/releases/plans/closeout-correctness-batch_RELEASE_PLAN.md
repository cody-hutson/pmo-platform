...
release/ADRs/ADR-195-action-item-status-classified-by-membership.md       add
...
**None — and this is enumerated rather than assumed.** The matrix carries **two** `add` rows (this plan file and #6841's `ADR-195`, both markdown documents) and **zero** `add` rows for a tracked executable. No `*.sh` is created by any member, so no `core/config/allowlists/script-execution-allowlist.txt` companion row and no CI-wiring statement is owed — **that conclusion is unchanged**, because it turns on the *executable* count, which is still zero. **What does NOT survive is the trigger claim this paragraph used to append:** it read *"this also preserves `routine` trigger (c) — zero new files outside the plan — intact,"* and ADR-195 is precisely a new file outside the plan, so trigger (c) no longer fires. See § Release Class declaration, where the cell is corrected and the class verdict's independent basis is recorded.
...
| RV-6 | ADR index freshness | **This release DOES add a record under `release/ADRs/`** — #6841 ships `release/ADRs/ADR-195-action-item-status-classified-by-membership.md`. Assert the ADR is present, its `status:` is one of the four-token Nygard enum, and `release/ADRs/README.md` carries its index row | **Not a no-op — a real check with a real subject.** The Stage-4 expectation (*"adds no record"*, graded as the honest no-op) is **falsified by the delivered diff**, which carries exactly **two** `add` paths: this plan file and ADR-195. Expected: ADR-195 present, `status: Accepted`, indexed in `release/ADRs/README.md` with its release column reading `closeout-correctness-batch`. **This mattered beyond bookkeeping:** Stage 7's deprecated-path scan is *conditional on new-file deliverables*, so a plan asserting zero new files invites a scan that never runs |
...
| routine (c) zero new files added | **no — CORRECTED at Stage 9** | The Stage-4 reading (*"exactly one `add` row — this plan file. No member adds a file"*) is **falsified by the delivered diff.** Measured `git diff --diff-filter=A` over the release range: **two** added paths — this plan file **and** `release/ADRs/ADR-195-action-item-status-classified-by-membership.md`, authored by #6841. So a member does add a file and this trigger **does not fire** |
| routine (d) zero new D-class decisions | **holds — on its own terms; its ADR clause is CORRECTED** | The D-class limb is unchanged and still holds: the plan's D-decisions are *recurring* entries present on every plan, and Stage 5 raised no new release-specific D beyond the wave-1 dispositions. **The clause appended to it is false, however:** this cell asserted that *no ADR is authored by any member that reached Stage 5 with a design, so novel trigger (c) does not fire.* #6841 authors **ADR-195** (`status: Accepted`, release `closeout-correctness-batch`), so **novel (c) does fire** and that clause is withdrawn |
