...
`#6411` changes **which worktrees classify REMOVE** — a lock-held worktree becomes `SKIP — worktree locked …`, its own classification and **not** the pre-existing `SKIP — live session (pid …)`. The two are separate protective classes that the tool counts separately, and collapsing the locked finding into the live one is the inference the operator's **D-Locked-Label** decision forbade: lock-held is a *removability* finding, liveness is a *process* finding, and `core/rules/git-workflow.md` § Sweep-deletion safety clause (c) prohibits reading the one off the other (an absent lock file is not evidence a worktree is free, so a lock is not evidence a session is live). `#6207`'s fix must project *branches freed by the same run's worktree removals*, so its projection consumes the classifier's output set. Landing `#6207` first would write its projection and its new fixture against a classifier `#6411` then changes. The edge is **build-blocking, not ship-gating** — both ship in one PR.
...
release/ADRs/ADR-197-action-item-status-classified-by-membership.md       add
...
**16 declared path×card write obligations over 9 distinct paths** (8 unconditional + the CONDITIONAL `stage-12-execute.md`). `release/tools/automated-closeout.sh` is written by six of the seven members plus the D-Sibling-Fold's sibling; § Contention Map carries the ordering. **The matrix carries TWO `add` rows, not one** — this plan file and #6841's `ADR-197`; the ADR row was added at the Stage-9 gating correction, having been delivered by #6841 without being declared here. The always-on `fcm-delivery` check compares declared `add` rows against the merged diff, so an undeclared delivered add is a divergence the check is built to catch rather than a bookkeeping nicety.
...
**Two files LEFT this block at the Stage-9 gating correction.** `release/tools/synthesize-release-learnings.sh` and `release/tools/audit-epic-rollup-close.sh` were declared `NOT EDITED` above; the operator has since **taken their five abort-class fixes into this release** under a separate work item, so they are **in scope** and a diff touching them is no longer a scope violation. The rows are removed rather than annotated, because a `NOT EDITED` row that no longer holds is exactly the falsifiable boundary this block exists to provide — leaving it in place with a caveat beside it would make the boundary unreadable in both directions. `issues_added` **remains 0**: this is an amendment folded into an approved scope on the same authority and the same precedent as the **D-Sibling-Fold** (§ File Change Matrix), not a new member entering the composition. **`[ASSUMPTION – CONFIRM]` the work item's identifier** — the operator's routing names the decision but this correction could not resolve a number for it from the tracker, and no number is invented here; the Stage-9 gate is the place to bind it.

**The non-scope block is explicit and load-bearing, not hygiene.** The files listed above, and the two that have now left the block, carry the *same defect class* #5649 repairs.

**The abort class is 5 sites, not the 3 this section first named — and the undercount was in one file.** Named by the **key each site reads** rather than by line number, because line pins in this plan rotted across the release and the key name is what survives a re-read:

| File | Sites | The key each read | Shape |
|---|---|---|---|
| `release/tools/audit-epic-rollup-close.sh` | **3** — this section previously named **1** | `claude_workspace_root` · `operator_github` · `pmo_platform_repo_name` | `$(grep … \| head -1 \| awk …)`, no `\|\| true`, top-level under `set -euo pipefail` |
| `release/tools/synthesize-release-learnings.sh` | **2** — correctly counted | `operator_github` · `pmo_platform_repo_name` | `$(grep -m1 … \| awk …)`, no `\|\| true`, top-level under `set -euo pipefail` |

**Why the undercount happened, recorded because it is instructive:** the two missed sites read different keys but sit in the *same* idiom as the one that was named, so a reader who had found one site had no signal that two more followed it. The count is the reference here — naming one site of three understates the work by the same factor whether or not the paths are right.

**`extract-usage.sh` stays out of scope and its characterisation is unchanged:** the masked-but-not-removed `head`-pipe shape at its `operator_instance_finops_store_path` and `claude_workspace_root` reads is **benign for abort** — that file carries `set -uo pipefail` with no `-e`, and its reads already carry `\|\| true` inside the substitution — but it is idiom-bearing, and it ships inside a distributed `.skill` package. Declaring a file out of scope is what makes the boundary falsifiable — a diff touching a file still in the block is a scope violation, not a judgement call. `selftest-coverage-manifest.txt` is named because it is the one file a reader would expect to need an edit and does not: the roster is keyed on tools that dispatch `--self-test`, it is **discovered rather than enumerated**, it already carries `automated-closeout.sh` at row 18, and it is regenerated by its own emitter and never hand-edited. Adding arms to an existing tool's suite requires no manifest edit.

### New-executable companion obligations

**None — and this is enumerated rather than assumed.** The matrix carries **two** `add` rows (this plan file and #6841's `ADR-197`, both markdown documents) and **zero** `add` rows for a tracked executable. No `*.sh` is created by any member, so no `core/config/allowlists/script-execution-allowlist.txt` companion row and no CI-wiring statement is owed — **that conclusion is unchanged**, because it turns on the *executable* count, which is still zero. **What does NOT survive is the trigger claim this paragraph used to append:** it read *"this also preserves `routine` trigger (c) — zero new files outside the plan — intact,"* and ADR-197 is precisely a new file outside the plan, so trigger (c) no longer fires. See § Release Class declaration, where the cell is corrected and the class verdict's independent basis is recorded.
...
| **R-10** | **RETIRED — the risk as framed rested on a criterion the operator re-bound before Engineering began.** The original framing was that #6841's AC bullet 4 *"cannot be discharged by this PR"* because re-reading the live drifted ledger rows is an action against **operator-instance** ledgers (Layer 2, outside the repository), and so had to be routed as an operator-executed Stage-13 action item. **That is the negation of what the operator decided.** The bullet's original subject population — live uppercase-`OPEN` rows — had already been normalised away, so the bullet was *unfalsifiable* rather than merely unmet: measured across the operator-instance ledger corpus, **0** of **1541** `AI-NNN` rows carried uppercase `OPEN`, against a sensitivity arm of **409** carrying lowercase `open`, so the zero is a live-reader zero and not a dead probe. | MEDIUM | CHEAP | **SUPERSEDED by operator decision `D-AC4-Rebind`, rendered at the Stage-4 plan gate (2026-09-07, actor `operator`, verdict "Re-bind to a seeded fixture", reversibility CHEAP · HIGH).** AC bullet 4 now reads: *a **seeded** unclassifiable row — one carrying a status the enum does not admit — is caught by the corrected predicate at every site, with a well-formed all-terminal ledger as the paired negative control.* That is an **in-repository** fixture assertion, so **this PR does discharge it** and it is a Stage-7 arm rather than a Stage-13 action item. The defect itself was never in doubt — it remains real at all three predicate sites; only the evidence the bullet named had gone stale. **A later hub correction on the same card narrows the measurement without disturbing the re-bind:** that 1541-row probe keyed the status column by field index and required more than 11 pipe-delimited fields, so malformed-width rows were structurally invisible to it, and a second live mechanism (row width, not vocabulary) exists inside the part it could not see. |
...
**All seven members' rows are POPULATED — the five that were scaffolded `[OWED]` were filled at the Stage-9 gating correction from the Stage-7 Dev Testing and Stage-8 Acceptance Review reports posted on each member's own sub-tasks.** This table is a Commit-0 Survival Set element that Stage-6 C4 self-verification and Stage-7 re-execution both consume; a member reaching Stage 7 with no row leaves both stages grading against an empty artifact — and five members reached **Stage 9** with the scaffold text still in place, which is the defect this correction repairs. **Every figure below is transcribed from what those reports measured; none was re-derived here and no suite was re-run to produce it** — this was a documentation correction, so the provenance of each figure is the sub-task that observed it. A criterion this release deliberately will not verify still carries its row with the method cell reading `[DEFERRED — <reason>]`; none of the seven now does.

**`ac_baseline` above is a Commit-0 reading at a named SHA and is deliberately NOT restamped.** It records #6207 at 5 criteria; Stage-8 graded that member **6/6** because `INT-1` is an integration criterion carried in addition to the issue's own AC set, not a drift in the issue's criterion count.
...
| #5762 | AC-1 — the exit-codes block describes what exit 2 actually covers | **Content, plus a mechanical arm that binds it.** The block names the two entry-gate phases by FUNCTION identifier (`phase_preflight`, `phase_detect_open_issues`) and states the pre-phase guard class non-enumeratively; it asserts no gate on the version tag, and its tag sentence is mode-conditional because `phase_publish_github_release` returns early under both `--no-merge` and `--dry-run`, ABOVE its own tag preflight. Mechanically bound by **Test 7 limb C**, which EXTRACTS the exit-2 dispatch set from the shipped text and asserts each name present in the RENDERED exit-2 entry. | Render measured, not source: `--help` emits **221** rows (was 210) and terminates on the `3 = ` entry, so the block is complete. `--self-test` **EXIT=0, 0 FAIL**. **Sensitivity (mutation M1):** delete `phase_detect_open_issues` from the rendered text → limb C FAILs naming that phase, EXIT=1. **Anti-vacuity (M3):** break the exit-2 extractor → the anti-vacuity floor FAILs alone. **Extractor control (M4):** break the exit-3 arm → the floor-30 control FAILs alone. M3 and M4 are a specificity pair — neither floor masks the other |
| #5762 | AC-2 — no enumeration in that file asserts a gate the code does not implement | **Bounded per the scope-lock to the two enumerations the defect spans** — the Exit-codes block and the phase-2 roster row — not the 43-row roster. The bound is not a convenience: a gate-vocabulary sweep over the header window returns 25 hits and MISSES the defect line itself, because "tag missing" uses no gate vocabulary, so the lexical instrument cannot discharge the universal form. Graded by reading both enumerations against the code. | Roster row 2 already reads "tag RECORDED (not gated)" and is untouched. The Exit-codes block now asserts the tag is NOT gated at entry, and names where it IS gated with the mode qualification the code carries. `tag missing` **pre 1 → post 0**; sensitivity arm `\btag\b` (re.I) observed **165**, so the zero is a measurement. The universal reading over all 43 roster rows is recorded **out of scope**, matching how this release already treated #5649's AC-3 |
| #5762 | AC-3 — control arm: the exit-2 conditions the code DOES implement remain documented | **Differential over the executable exit-2 census, not a citation.** Enumerate every executable, non-fixture `exit 2` statement and assert each class is reachable from the exit-codes line. | **4 of 4 documented.** Census on the branch: `:383` instance-path resolver guard and `:872` workspace-boundary guard (both pre-phase, both named by the guard clause), plus the two guarded dispatches `phase_preflight` and `phase_detect_open_issues` (both named by identifier and bound by limb C). Denominator 14,431 lines; sensitivity `\bexit 3\b` observed **55**, specificity `\bexit 27\b` observed **0**. Heredoc-fixture stubs correctly excluded. The old line named ONE class and asserted a fifth that does not exist; the replacement names four and asserts none that do not |
| #6204 | AC-1..AC-4 | **DELIVERED AND GRADED** — Stage-7 Dev Testing (#7227) and Stage-8 Acceptance Review (#7228). Method per criterion, as executed: AC-1 read the ownership statement in the owning stage's own spec and the non-owning stage's disclaimer; AC-2 graded as a disjunction with **both** limbs read (`--title` passed on the edit path, **and** the title composed from the note H1 so the two cannot diverge by construction); AC-3 read the pre-mutation comparison of posted title against note-derived title; AC-4 graded the sensitivity/specificity arm pair by **in-place mutation**, not by a green run. Scope-lock corrections honoured: the `phase_publish_github_release` outcome token is unchanged and the withhold is recorded in the phase detail. | **AC-1 MET · AC-2 MET · AC-3 PARTIAL · AC-4 MET.** Acceptance **ACCEPT**, disposition `accept` on all four. AC-3's `PARTIAL` is the truthful verdict for substance-delivered-under-an-approved-narrowing: the comparison **is** an executed pre-mutation check, and the remainder (no close-time hard fail; a converged state reports PASS) is **non-AC-blocking** on the PARTIAL Test, so **no Operator Override Record is owed** — recorded as a measurement rather than a silence. Regression surface intact: `automated-closeout.sh --self-test` exit 0, **0** `FAIL:` lines, 45 output lines, 36 group-echo `validated` lines. AC-3's residual carries a gate-efficacy register row per the scope-lock. |
| #6841 | AC-1..AC-5 | **DELIVERED AND GRADED** — Stage-7 Dev Testing (#7231) and Stage-8 Acceptance Review (#7232). AC-1 read the membership ladder `EMPTY-LEDGER → UNRESOLVED → UNCLASSIFIABLE → RESOLVED` at the predicate site, confirming the unclassifiable residue reaches a blocking state rather than falling into an implicit terminal `else`; AC-2 read the **single** normalisation point in each of the three predicate copies; AC-3 ran both arms of the non-vacuity control (detection arm on a seeded unclassifiable row, negative arm on a well-formed all-`done` ledger); **AC-4 graded in its re-bound form per `D-AC4-Rebind`** — seeded rows measured at every site with a well-formed all-terminal ledger as the paired negative control; AC-5 read the verdict-neutrality claim and its scoping in the same sentence. Both scope-lock corrections honoured: `BAD` does not outrank `UNRES`, and the live-witness mechanism is an **empty field-11 arising from a short row** — arity, not vocabulary — so the fixture reproduces a condition that actually occurs. | **AC 5/5 MET**, acceptance score **1.00**, 0 NOT MET / 0 PARTIAL / 0 drift; Step-0 not fired, **0 override records required**; **ACCEPT**. AC-4 is discharged **by this PR**, not operator-executed — see R-10, which is retired. The live defect reproduced: **5 of 69** operator-instance ledgers flip `RESOLVED → UNCLASSIFIABLE` under the corrected predicate. Suites: `automated-closeout.sh --self-test` EXIT 0 / 0 `FAIL:`; `test_action_item_gate_predicate.sh` **32 pass / 0 fail**; both green in CI's macOS job (`ARM A PASSED: 3 of 3`) and re-run locally. ADR gates on the new `ADR-197`: numbering **195 contiguous**, durability COUNT **0** (own self-test 138/138, so the zero is a measurement), index `--verify` COUNT **0**. |
| #6255 | AC-1..AC-4 | **DELIVERED AND GRADED OVER TWO DEV-TESTING PASSES** — Stage-7 (#7235, pass 1 `dt-return` then pass 2 PASS) and Stage-8 Acceptance Review (#7236). AC-1 read the `MERGED/*)` arm as the **first** `case` arm (first-match-wins), marking PASS, and proved the arm executes by **mutation** (`MERGED/*` → a mutant token reddens it); AC-2 graded both limbs — still-polled via the `>= 2 pr view calls` assertion, and the timeout limb via arm (j), mutation-proven to redden the exact `if false` mutation that exposed the pass-1 gap; AC-3 read `CLOSED/*)` as its own arm evaluated after `MERGED/*)`, so merged and closed-unmerged cannot collapse; AC-4 established the elapsed bound structurally — the `MERGED/*)` arm is evaluated on the first loop iteration **before** the sleep, so elapsed is 0 at return. Scope-lock correction honoured: `MERGE_TIMEOUT=2` on arm (g) only, and the **≥2 assertion is not weakened** — it is the only pin on design decision D-3. | **AC 4/4 MET**, acceptance score **1.00** all-drift-out, 0 NOT MET / 0 PARTIAL; Step-0 no-fire, no override record owed; **ACCEPT**. **AC-2 transitioned PARTIAL → MET between passes, and both halves of that transition were established** rather than inherited: that arm (j) now grades the timeout limb, and that nothing graded it before. Both pass-1 blockers CLOSED by mutation (F-01 the extraction-line claim inverted under mutation; F-02 the ungraded AC-2 timeout limb). Suite: `automated-closeout.sh --self-test` exit 0, **0** `FAIL:` lines, 45 lines, 36 `validated`; sibling `cleanup-orphan-state.sh --self-test` exit 0, **23 PASS**, SKIP ledger **0 of 20**. |
| #6411 | AC-1..AC-4 | **DELIVERED AND GRADED** — Stage-7 Dev Testing (#7248) and Stage-8 Acceptance Review (#7249). AC-1 read the locked-detection clause, the two emitted labels, the cwd-independence of the classification and the apply-side guard, with group **LK** as the arm set (LK-1 locked tree SKIPs with no cwd holder · LK-2 an unlocked clean tree in the **same run** still REMOVEs · LK-3 `--apply --force` does not override the class and the tree survives · LK-4 the control tree **is** removed in that run, so LK-3 is not vacuous); AC-2 graded the **global-null** fail-closed limb via group **GN** (GN-1 an `lsof` that runs and returns an empty map fails closed through the self-canary · GN-2 the real resolver restores, so GN-1 is not an oracle that never worked); AC-3 graded the split failure lines via group **FL** (two distinct lines, each absent when its own surface has no failure, classifier total over its closed set); AC-4 scoped per the GRADING NOTES to the single target `core/rules/git-workflow.md` — clause (c) deliberately **not** edited. Both scope-lock corrections honoured: change-spec item 4 DROPPED (the live worktree read is deliberate; routing it through the snapshot opens a TOCTOU, and item 4's rationale is impossible because the recheck loop is demote-only), and the **stale COUNT moved with the change** — `Two` → `Three` protective classifications, a count being a reference. | **AC 4/4 MET**, acceptance score **1.000** all-drift-out, 0 NOT MET / 0 PARTIAL; Step-0 no-fire, **0 override records owed**; **ACCEPT**. **LK, GN and FL were each observed FAILING under a deliberate mutation before being accepted** — an arm never seen to fail is not yet evidence. Suite locally: **EXIT 0, 20 checks, SKIP ledger 0 of 20**. AC-4 measured on the two revisions: exactly **one** changed line, link target set **identical** (2 links, both leading-`/` workspace-rooted — the only correct mirror-pair form), issue-ref delta **0**, raw-URL delta **0**, against a link regex finding **13** links in the file, so those zeros are measurements; `check-doc-links.py --require-targets` header row only, exit 0. **PR-time coverage is partial and the gap is named:** `locked-worktree check` is one of the nine checks that self-skip on a merge-ref checkout, so **AC-1's arm set has no PR-time CI coverage** (#7304), while GN and FL are **not** in the skipped nine — both PASS lines read in the job log. The deployed-mirror obligation for the rules edit is recorded on #7250. |
| #6207 | AC-1..AC-5 (+ INT-1) | **DELIVERED AND GRADED** — Stage-7 Dev Testing (#7240, this release's DT pilot) and Stage-8 Acceptance Review (#7241). AC-1 read `projected_freed_branches` emitting a distinct `WILL-DRAIN — freed by same-run worktree removal` action token, so the distinctness from removable-now is **structural** rather than presentational; AC-2 verified both halves — the freed branch reports as will-be-drained, and the predicate returns the drained verdict only when the recorded action set warrants it; AC-3 read the single Totals line serving both modes with mode-aware vocabulary and confirmed the arithmetic identity holds, which is what makes the line relayable as an approval scope; AC-4 ran the fixed-point fixture asserting dry-run projection equals apply outcome where a worktree removal frees a branch, **with the sensitivity arm observed firing** when the projection is removed; AC-5 graded on the **directness clause** per the card's GRADING NOTE rather than on a flag string. Scope-lock correction honoured: **E5 re-pinned at Stage 6** because #6411 builds first and its fourth Totals guard lands at the head of this card's originally-pinned range — no arithmetic collision, the counters being disjoint, but the pins were stale by construction. All fixture runs executed inside a **throwaway local clone**, never against the live workspace. | **AC 6/6 MET** (five issue criteria + **INT-1 MET** as the scope-lock anticipated), acceptance score **1.00**, **0 NOT MET / 0 PARTIAL**; Step-0 no-fire, no override record; **ACCEPT**. Group-PS arms proven to EXECUTE by mutation with a firing sensitivity arm: dropping the plan entry from the chore-PR `files=()` array — the #6258 defect itself — produces `FAIL: PS-1`, two `FAIL: PS-2` lines and `self-test: FAIL (3 failures)`. Post-failure state read: **no orphan state stranded**. **This pilot established the release-wide skip finding** that CIAC-3 and RV-1 now carry: the same suite reports **SKIP ledger 9 of 20 at the PR head against 3 of 16 on `main`**, cause `no origin/main ref` from the `refs/remotes/pull/<n>/merge` checkout — including the `fixed-point check` carrying this card's whole AC-4 arm set. Routed **Tier 2, defer** as **#7304**; not repaired by this release. |

### Release-Level Verification

| # | Check | Method | Expected |
|---|---|---|---|
| RV-1 | Both close-out tools' self-tests pass | `bash release/tools/automated-closeout.sh --self-test` and `bash release/tools/cleanup-orphan-state.sh --self-test`, run by each Engineering spoke after its own commit and re-run at Stage 7 | `EXIT=0`, zero `FAIL:` lines, with the **observed counts recorded**. Baseline for the sibling: **`SELFTEST_CHECK_COUNT` = 20 checks** (16 → 17 by #5649's key-read-tolerance group, 17 → 20 by #6411's three new groups — both inside this release, so 16 was never the count this release ships). **SKIPPED is recorded as observed with its checkout named, not compared to a target:** the count is a property of the checkout — **0 of 20** where an `origin/main` ref exists, **9 of 20** on a PR merge-ref checkout (`refs/remotes/pull/<n>/merge` creates none, so nine checks self-skip with `no origin/main ref`) — deferred as #7304 and not repaired here, so a flat `0 SKIPPED` expectation would fail a correct CI run |
...
| RV-5 | Skill-package freshness | `printf '%s\n' <changed-path> \| bash core/deploy/tools/build-skill-packages.sh --skills-for-paths` (STDIN, **not** argv — an argv invocation returns empty for every input, which reads as "nothing to rebuild" while having measured nothing) | expected **NON-empty: `release-executor` and `release-hub`.** The Stage-4 expectation of *empty — no changed path lies under a rostered skill tree* is **falsified by the delivered diff**: #6204 edited `release/skills/release-executor/SKILL.md` and #6841 edited `release/skills/release-hub/references/orchestration-playbook.md`, so two rostered skill trees changed and both `.skill` packages plus their `.sha256` siblings are rebuilt in this release. **The argv-vs-STDIN caution stands and matters more here, not less** — an argv invocation returns empty for every input, so against a release that genuinely owes two rebuilds it would have read "nothing to rebuild" while measuring nothing, and the falsified `expected empty` would have masked it. Report the resolved skill names with the STDIN form named |
| RV-6 | ADR index freshness | **This release DOES add a record under `release/ADRs/`** — #6841 ships `release/ADRs/ADR-197-action-item-status-classified-by-membership.md`. Assert the ADR is present, its `status:` is one of the four-token Nygard enum, and `release/ADRs/README.md` carries its index row | **Not a no-op — a real check with a real subject.** The Stage-4 expectation (*"adds no record"*, graded as the honest no-op) is **falsified by the delivered diff**, which carries exactly **two** `add` paths: this plan file and ADR-197. Expected: ADR-197 present, `status: Accepted`, indexed in `release/ADRs/README.md` with its release column reading `closeout-correctness-batch`. **This mattered beyond bookkeeping:** Stage 7's deprecated-path scan is *conditional on new-file deliverables*, so a plan asserting zero new files invites a scan that never runs |
...
- [ ] **CIAC-3 (#6411 × #6207 — the `cleanup-orphan-state.sh` dry-run report):** the dry-run report agrees with what apply does **in both directions in a single run** — a lock-held worktree reads `SKIP — worktree locked …` in dry-run *and* apply, **and** a branch freed by that same run's worktree removals is reported as a predicted consequence rather than as skipped. Release-scoped because each issue fixes one direction of the same divergence and **neither alone makes the report relayable as an approval scope**. *Shared surface:* the dry-run classification path in `classify_worktree`, and `resolve_freed_branches` with its projection sibling `projected_freed_branches`. **Named by symbol, not by line:** these functions moved during the release (the file grew 3291 → 4093 lines), so a line pin authored at Stage 4 grades against the wrong region by Stage 9 — the defect this criterion itself carried until the Stage-9 gating correction. *Method:* `bash release/tools/cleanup-orphan-state.sh --self-test` with both members' new fixtures present in one invocation; assert the projected branch set equals the applied branch set on the fixture where a worktree removal frees a branch, and that the locked-worktree fixture classifies SKIP in both modes.

  **Baseline for comparison: `SELFTEST_CHECK_COUNT` = 20 checks, `EXIT=0`, zero `FAIL:` lines.** The count is 20, not the 16 this row carried at Stage 4 — raised 16 → 17 by #5649's key-read-tolerance group and 17 → 20 by #6411's three new groups, both inside this release.

  **The SKIPPED count is deliberately NOT asserted as a figure, because the correct figure is a property of the checkout, not of the code.** Where the suite runs against a **PR merge-ref checkout** — `refs/remotes/pull/<n>/merge`, which creates no `refs/remotes/origin/main` — **9 of the 20 checks self-skip with `no origin/main ref`**, and the nine include the `fixed-point check` carrying #6207's whole AC-4 arm set. Measured both ways by Stage-7 Dev Testing: **9 of 20 at the PR head against 3 of 16 on `main`**, same suite, the difference being the checkout. Run where an `origin/main` ref exists, the same suite reports **0 of 20 SKIPPED** and `self-test: PASS`. The Stage-4 expectation of `0 SKIPPED` therefore **holds in a local checkout and cannot hold in the PR-time CI job**, so grading a correct implementation literally against it marks it NOT MET. That skip gap is **deferred as #7304 — the `no origin/main ref` self-skip on a merge-ref checkout — and is not repaired by this release** (routed Tier 2, defer, by the Stage-7 pilot). So the graded expectation is `EXIT=0` with zero `FAIL:` lines and every non-skipped check passing; a SKIPPED count is **recorded as observed, with its checkout named, and never compared to a target**.

  **Which of this criterion's two limbs the PR-time run can actually grade — measured, not assumed.** Both skipped-nine memberships were read from the job log at head: the **`locked-worktree check` (group LK) IS among the nine**, so **the locked-worktree limb of this CIAC has no PR-time CI coverage at all**, and so is the **`fixed-point check`** carrying the freed-branch limb. Both limbs of CIAC-3 therefore sit inside the skipped set, and grading this criterion from the PR-time CI log alone would read *absence of a FAIL* as *presence of a PASS*. **Grade it instead on a checkout carrying an `origin/main` ref** — where the same suite reports 20 of 20 run, 0 SKIPPED, `EXIT=0` — and on the LK and fixed-point arms' mutation evidence, each of which was observed *failing* under a deliberate mutation before being accepted. *Graded at Stage 9 QC3.5 on the merged PR.*
...
| routine (c) zero new files added | **no — CORRECTED at Stage 9** | The Stage-4 reading (*"exactly one `add` row — this plan file. No member adds a file"*) is **falsified by the delivered diff.** Measured `git diff --diff-filter=A` over the release range: **two** added paths — this plan file **and** `release/ADRs/ADR-197-action-item-status-classified-by-membership.md`, authored by #6841. So a member does add a file and this trigger **does not fire** |
| routine (d) zero new D-class decisions | **holds — on its own terms; its ADR clause is CORRECTED** | The D-class limb is unchanged and still holds: the plan's D-decisions are *recurring* entries present on every plan, and Stage 5 raised no new release-specific D beyond the wave-1 dispositions. **The clause appended to it is false, however:** this cell asserted that *no ADR is authored by any member that reached Stage 5 with a design, so novel trigger (c) does not fire.* #6841 authors **ADR-197** (`status: Accepted`, release `closeout-correctness-batch`), so **novel (c) does fire** and that clause is withdrawn |
...
**The two corrected cells do NOT overturn the class, and the reason is on the record rather than inferred here.** `routine` was rendered as operator decision **`D-Class-Release`** at the Stage-4 gate, and that decision was justified **on trigger (b) alone** — (b) is independently verified above and is untouched by ADR-197. Triggers (c) and (d) were explicitly *not* addressed when the decision was put; (d)'s conflict with `novel` (b) was resolved at that time on the reading that a **recurring** D is not a *new* one, with the alternative's consequence pre-computed at 22 effective points before the operator ruled. So the corrections above restore two cells to what the delivered diff measures **without** re-opening the verdict those cells did not carry. **Re-classifying the release is an operator decision, not a plan-file edit** — this correction does not make one, and flags the input rather than acting on it: a reader who considers `novel` (c) firing material to the class should route it to the operator at the Stage-9 gate.
...
| **DEV-9** | **#5762's regression pin ships with two limbs, not three.** The Stage-5 design specced a third, lexical limb asserting the rendered exit-2 text does not name the version tag. The repaired text **does** name the tag — it must, in order to say the tag is not an exit-2 condition and to say where it actually gates — so a lexical "tag absent" needle cannot distinguish the disclaimer from the phantom it replaced. | Phase A6.5 adversarial review CD-1, which reached the same conclusion independently; Engineering B3 minor-adjustment authority | **DROPPED, with the assertion relocated to a structural arm rather than deleted.** Limb C binds the exit-2 phase set by EXTRACTION from the shipped dispatch, so the property limb 3 approximated lexically is now asserted structurally and in the inclusive direction as well. This is the fifth failure on this card of a vocabulary-bound instrument over this surface; the pin does not add a sixth. |
| **DEV-10** | **#5762 touches a fourth region the Stage-5 footprint did not name.** The design fixed the footprint at three regions (exit-codes block, Test 7, terminal echo). The widened cascade sweep the scope-lock prescribed — `preflight[ -]failure`, not `preflight failure` — finds a second hit at `:367` (`:378` post-change): the justification comment for the instance-path resolver guard, which this change reclassifies OUT of preflight. | Phase A6.5 adversarial review PR-4 site 1 (Major), carried into the scope-lock as binding correction 4 | **FIXED IN THE SAME COMMIT, and named rather than absorbed.** Left alone it would have read "Exit 2 is this file's documented preflight-failure code" against an exit-codes block that says exit 2 is expressly not confined to preflight — the defect under repair, re-created by the repair. This is cascade completeness on the change being made, not a scope addition: the path is already this card's declared write surface and `issues_added` stays 0. |
| **DEV-12** | **Six Stage-9 gating corrections to this plan file, plus four in-class cascade edits.** The operator rendered GO at Stage 9 **conditional on these landing first**, because `CIAC-3` is graded at Stage 9 QC3.5 and, as written, marked correctly-delivered work NOT MET on three independent grounds. The six: **(1)** `CIAC-3` — the forbidden `SKIP — live session` label for the lock-held case, the stale `16 checks` count, the `0 SKIPPED` expectation that cannot hold on a merge-ref checkout, and four rotted line pins replaced by symbol anchors; **(2)** the second bare `16 checks` figure at `RV-1`; **(3)** `R-10` and #6841's Per-Issue row, which asserted the **negation** of operator decision `D-AC4-Rebind`; **(4)** `RV-6`, `RV-5` and two Release-Class trigger cells, all falsified by `ADR-197`; **(5)** the out-of-scope note's 1-vs-3 undercount of the abort class in `audit-epic-rollup-close.sh`, and those five fixes now being **in** scope; **(6)** five `[OWED]` Per-Issue rows and an empty § Verification Evidence, for work graded twice over. | Operator, at the Stage-9 gate — GO conditional on these corrections; the four correction sets are recorded on sub-task #7251, each verified against the plan before routing. `D-AC4-Rebind` is the authority for (3); the delivered diff is the authority for (4). | **APPLIED.** Every site was located **by content**, never by the line pins the correction sets carried — several of these defects exist *because* line pins rotted, so re-pinning would have reproduced the defect class while repairing its instances. **One site was deliberately NOT changed:** the #5649 sibling-limb row states its baseline as *"16 checks / 0 SKIPPED **preserved or exceeded**"*, and 20 exceeds 16, so that clause is satisfied as written — the three `16 checks` sites were classified by **reading each clause for a floor-vs-figure distinction**, which is why this correction could not be a find-and-replace. **Four in-class cascade edits** accompany (4) and (5), because a claim is a reference and a count is a reference: the `ADR-197` `add` row added to the File Change Matrix with its row/path counts moved from 15/8 to 16/9; the frontmatter `reversibility:` clause "no member adds a file outside `release/releases/plans/`" withdrawn; the § New-executable companion obligations restatement of `routine` trigger (c) corrected while its executable-count conclusion stands unchanged; and the two now-in-scope files removed from the `NOT EDITED` fence rather than annotated inside it. **`issues_added` remains 0** and no issue was created or closed by this correction. |
| **DEV-11** | **PR-4 site 2 is routed, NOT fixed.** `release/tools/compute-release-velocity.sh:155` reads "Guarded and NON-FATAL, unlike automated-closeout.sh's exit-2 preflight", contrasting against the very guard this change reclassifies. It is a genuine cross-file duplicate and is inside `duplicate-source-discipline.md`'s two-or-more-files predicate — the one place that standard actually reaches on this card. | Scope-lock IN FORCE; the file is not on #5762's declared write surface | **TIER 2 [SCOPE CHANGE] SURFACED TO THE HUB, not executed.** Adding a second file to a scope-locked member is an addition, and the review's own guidance was that either fixing it or routing it is defensible while silence is not. Surfaced with its measurement so the hub can rule; the one-word fix is `entry-gate` for `preflight`. |

---

## Verification Evidence

**Populated at the Stage-9 gating correction, transcribed from the Stage-7 Dev Testing and Stage-8 Acceptance Review reports posted on each member's own sub-tasks, plus the read-only validator runs named in the Provenance column.** Every figure is an **observed** result, and each row says **where it was observed** — a figure whose provenance is a sub-task report was not re-derived here, and **no suite was re-run in order to write this section**. Where a check cannot be observed until merge or until the PR is non-draft, that is stated rather than filled.

### Per-member acceptance — Stage 8

| Member | Criteria graded | Verdict | Score | Provenance |
|---|---|---|---|---|
| #5649 | 4 | **AC 4/4 MET** · ACCEPT | 1.000 | Stage-8 QA #7220 |
| #5762 | 3 | **AC 3/3 MET** · ACCEPT | — | Stage-8 QA #7224 |
| #6204 | 4 | **AC-1 MET · AC-2 MET · AC-3 PARTIAL · AC-4 MET** · ACCEPT, disposition `accept` on all four | — | Stage-8 QA #7228 |
| #6841 | 5 | **AC 5/5 MET** · ACCEPT | 1.00 | Stage-8 QA #7232 |
| #6255 | 4 | **AC 4/4 MET** · ACCEPT (AC-2 PARTIAL → MET across two DT passes) | 1.00 | Stage-8 QA #7236 |
| #6411 | 4 | **AC 4/4 MET** · ACCEPT | 1.000 | Stage-8 QA #7249 |
| #6207 | 5 + INT-1 | **AC 6/6 MET** · ACCEPT | 1.00 | Stage-8 QA #7241 |

**Release total: 30 criteria graded — 29 MET, 1 PARTIAL, 0 NOT MET.** The single `PARTIAL` is #6204's AC-3, whose remainder is **non-AC-blocking** on the PARTIAL Test, so **no Operator Override Record is owed anywhere in this release** — recorded as a measurement, with what would have fired it: a `NOT MET` dispositioned anything but fix-now, or a `PARTIAL` whose remainder was AC-blocking.

### Release-Level Verification — observed

| # | Observed result | Provenance |
|---|---|---|
| **RV-1** | `cleanup-orphan-state.sh --self-test` → **EXIT 0**, `self-test: PASS`, **20 checks, SKIP ledger 0 of 20**, 23 `PASS` lines. `automated-closeout.sh --self-test` → **exit 0**, **0** `FAIL:` lines, 45 output lines, 36 group-echo `validated` lines. `test_action_item_gate_predicate.sh` → **32 pass / 0 fail**. **The PR-time figure differs and is recorded, not smoothed:** in the CI job whose checkout is a PR merge ref, the same `cleanup-orphan-state.sh` suite reports **SKIP ledger 9 of 20** (`no origin/main ref`) against **3 of 16** on `main` — deferred as #7304 | Local run this pass, detached at the branch head; per-card figures from DT #7227 / #7231 / #7235 / #7240 / #7248; the 9-of-20 vs 3-of-16 pair measured by the DT pilot #7240 |
| **RV-2** | `check-doc-links.py` over the Check-14 scan scope with `--require-targets` → **exit 0, header row only, 0 findings**. Tool `--self-test` → **exit 0, 11 fixtures passed**, so the zero is a measurement and not an empty walk. `check-release-links.py` → **exit 0, 0 broken links**; tool `--self-test` → **exit 0**, its fail-closed fixture reporting the absent note as designed | Local runs this pass |
| **RV-3** | `check-release-links.py --plan-depth-lint` over this plan → **exit 0, 0 broken links, 0 depth-sensitive plan links**. **A caution on this method as the plan states it:** the checker's summary counts *files carrying findings*, so its "across 0 files" line reads identically whether the corpus is clean or was never walked — read it together with the `--self-test` result, which is what arms it | Local runs this pass |
| **RV-4** | **NOT OBSERVABLE PRE-MERGE — stated rather than filled.** The mirror is deploy-produced; `./deploy.sh --deploy` followed by `--check` Check 9 runs after merge. The obligation is recorded on #7250, and #6411's own AC-4 verification confirmed the repo-source edit is link-clean in the mirror-pair form: 2 links, both leading-`/` workspace-rooted, against 13 links found in the file | DT #7248 / QA #7249 for the source-side read |
| **RV-5** | **NON-empty, as the corrected expectation now states.** The STDIN form resolved **`release-executor`** and **`release-hub`** — two rostered skill trees changed this release (#6204's `SKILL.md` edit, #6841's playbook edit), and both `.skill` packages plus their `.sha256` siblings are rebuilt in the delivered diff | Local run this pass, STDIN form, over the release's changed-path set |
| **RV-6** | **ADR-197 present and indexed.** `release/ADRs/ADR-197-action-item-status-classified-by-membership.md`, `status: Accepted`, release `closeout-correctness-batch`; index row present in `release/ADRs/README.md`. ADR gates: numbering **195 contiguous**, durability COUNT **0** against its own self-test at **138/138**, index `--verify` COUNT **0** | ADR/index read this pass; gate figures from DT #7231 |
| **RV-7** | **NOT OBSERVABLE FROM THIS FILE.** The parser-clean check runs against the PR body draft at PR-ready time, not against the plan | — |

### Cross-Issue Acceptance Criteria — status at this correction

**All four cross-issue criteria are graded at Stage 9 QC3.5 on the merged PR and are NOT marked MET here.** This section records what the members' own verification established beneath them; the CIAC verdicts themselves are Stage 9's, and the plan verifier is their sole runner. Two things Stage 9 needs in hand:

> **Authoring hazard, recorded because this correction tripped it.** The CIAC parser is **line-based**: it treats *any* line matching `**CIAC-<digits>` as a criterion declaration, and where such a line carries no `*Method:*` marker it falls back to **the first backticked span on the line** as the method. Prose that opens a bold span with a CIAC identifier therefore registers a phantom criterion — inflating the indexed CIAC count above the four this section declares, with a method that is whatever happened to be backticked first. Refer to the criteria in prose without leading a bold span with the identifier.

1. **Do not grade CIAC-3 from the PR-time CI log alone.** Both of its limbs — the `locked-worktree check` (group LK) and the `fixed-point check` — are inside the nine checks that self-skip on a merge-ref checkout, so that log shows neither a PASS nor a FAIL for them. Grade on a checkout carrying an `origin/main` ref (20 of 20 run, 0 SKIPPED, EXIT 0) plus the LK and fixed-point arms' mutation evidence. #6411's GN and FL groups are **not** in the skipped nine and were read PASS in the job log.
2. **CI state as measured at the PR head:** all **17** required branch-protection contexts `bucket=pass`, count stable across two consecutive polls; full roster **54** rows, 53 pass, **1** fail — `Discovered tool self-tests (Ubuntu)`, which is **not** a required context and is **pre-existing and baseline-identical**, established by arm-for-arm diff against `main` rather than asserted. The PR being `draft` at Stage 7 is the specified state at that stage. **R-13 still governs the reading of all of this:** neither tool this release is about is exercised by any required context, so green required CI is not evidence for this release — the per-member self-test and mutation records above are.

### What this section deliberately does not claim

**No figure here was produced by re-running a suite to populate the plan.** The correction that filled this section was a documentation correction; re-executing the release's verification in order to write its record would substitute this pass's observations for the graded stages' own, and Stage 7 and Stage 8 are where those verdicts were earned. The local validator runs in the Provenance column are read-only checks over the corrected file itself — RV-2, RV-3, RV-5 and the RV-6 reads — not re-executions of any member's acceptance arm.
