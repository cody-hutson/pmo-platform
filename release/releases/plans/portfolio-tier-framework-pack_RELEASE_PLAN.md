...
| #2577 | AC-7 | duplicate-source-discipline: `grep -rcE 'global default [→] portfolio [→] program [→] project [→] individual' core/ release/` — the resolution-precedence chain must appear at its existing single home and **not** be restated beside the new selector. **The pattern brackets each arrow so that the pattern string does not match itself.** An earlier form of this row spelled the chain verbatim, which made the plan file a third occurrence of the string the criterion counts — an instrument inside its own population, which would have flipped a correctly-authored release to NOT MET on the plan's own text. Bracketing is what makes the plan file measurable by its own criterion: the regex requires a bare arrow between the words, and this row has a bracketed one. Reconciled at S4 | **2** — the pre-release value measured at the `539c4440` baseline over 1078 `.md` files under `core/` + `release/` (`core/schemas/platform-config-schema.md`, `release/references/how-to/hub-spoke-bridge.md`). The count must still read **2** after every S1–S4 edit; the selector cites the precedence, never re-defines it. *Control arms:* sensitivity — `most-specific` fires at 13 files; specificity — a fabricated chain returns 0 |
