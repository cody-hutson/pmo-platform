...
| **ERROR (4)** | #3616 AC-3, AC-4 · #2577 AC-4, AC-6 | Three are `unclassified-method (no family match)` — genuine documented-decision methods with no local runnable command (two network reads of live GitHub state, one worked-example resolution). **#2577 AC-4 is a different failure and is named separately:** `count-unreadable:non-integer-count-field`. Its method is a real, runnable `ls \| grep -cE` over the named set, but the pipe is markdown-escaped for table rendering and the count extractor cannot read the result. **The criterion is MET and was verified directly at Stage 6:** the probe returns **7**, and its control arm against the unselected `zzz-not-selected/` directory returns **0**. **Not padded with keywords to make the ERROR disappear** — that would buy a verdict rather than earn one. |
