...
3. **The new verification verb is not wired into any recovery path, deliberately.** On a correctly recovered repository the plan has already been renamed, so the pre-claim plan resolver fails before the token test is ever reached — the verb would report a healthy repository as broken. Its scope is stated narrowly at both its procedure home and its usage text: pre-claim only, first Engineering spoke only.
