...
| **PR** | Opened in **draft** by the first Stage-6 spoke once #6199's GREEN commit lands; transitions to ready-for-review at the Stage-9 gate. The number is not known at Commit 0 and is carried by the PR itself. |
| **Milestone** | `autonomy-ceiling-domains-resolve-canonically` |
...
`${AUDIT_DATE_UTC}` = **`2026-09-24`**, resolved at Engineering Commit 0 via `date -u +%Y-%m-%d`. The release carries one load-bearing dated identifier: the `date:` field of #6200's new ADR (`core/ADRs/ADR-NNN-cross-domain-classification-is-repository-membership.md`). #6200's Engineering slice writes this resolved value there; no other dated identifier is created.
...
| #6200 | GENERATES | `core/ADRs/ADR-NNN-cross-domain-classification-is-repository-membership.md` | #6200 → file | File Change Matrix (add; supersedes ADR-149 in part) |
...
| #6200 | `core/ADRs/ADR-NNN-cross-domain-classification-is-repository-membership.md` | no | no (conjunct 1) | unconstrained | unconstrained | ordinary Engineering spoke |
...
| DEV-18 | Both suites' mutation arms | The CIAC-1 pair aliases a subdirectory (X-17/X-18); D-9's must-diverge set re-derived to D-1, D-2, D-3, D-4, D-4b, D-5, D-5b, D-5f; the D-1b/D-5c/D-5d differential rows removed; D-5f added; the D-9 arm name made count-free; X-19 added | #6200 design F2 (corrected); #7585 PR-1; #7584 PR-3 / CD-2 | #6199's half lands in the #6199 RED commit; #6200's half with #6200's slice |
...
| DEV-20 | IP-1 contract | #6199's obligation 4 struck; the replacement mapping (an undeterminable working-directory membership answer is never `unresolvable`); X-16e added | CR item 2; #7584 F-1; #7585 FM-1c | APPLIED in § Integration Points; X-16e lands with #6200's slice |
| DEV-21 | The shared region | #6200 design Changes 2c/2e/2f superseded by the composed end state plus the slice-B deltas; `CWD_RESOLVED` → `ABS_CWD` | CR item 1; #7585 FM-1a/b, CD-1; #7584 F-2 | Governs #6200's GREEN |
| DEV-22 | `-002` reasons; X-5/X-16c; fragment paragraph 9d | The FM-4 reason split (`LIB-MISSING:` / `UNDETERMINABLE:`) and the consequence sentence | #7585 FM-4 | Lands with #6200's slice |
| DEV-23 | #6200 RED commit | Per-arm vector recorded; Change 4b moved to GREEN; the AC-8 arm strengthened; the AC-8 twin added | #7585 FM-3 | Lands with #6200's slice |
| DEV-24 | `block-draft-files.test.sh` `test_case` | `CLAUDE_WORKSPACE_ROOT` pinned to an anchor-free directory (extends DEV-13) | #7585 FM-2 | Lands with #6200's slice |
| DEV-25 | `block-draft-files.test.sh` comments | Retired-predicate prose re-scoped; arm names unchanged | #7585 FM-5 | Lands with #6200's slice |
| DEV-26 | Hook comment 2e / fragment row 9c / ADR Decision 2 / `autonomy-tiers.md` 7a; ADR residuals | "cannot reach" scoped to working trees of this checkout's repository; the separate-clone residual added | #7585 PR-2 | Lands with #6200's slice |
| DEV-27 | #6200's ADR | The ADR draft is filed as an `adr` issue (#7633) with no milestone, so the composition lock stays intact; its number is claimed at Engineering by #6200's slice against the mainline anchor, and its file carries the seam spec's edits E-1..E-5 | CR-Q4 | The issue is filed; the file lands in #6200's docs commit |
...
- **D-Anchor C3, REPLACE, tri-state helper, M4 documentation home:** see § Decision Record.

### Reversibility

**MODERATE — HIGH confidence.** `git revert -m 1` of the single merge restores `main`; runtime recovery is a hook-tier refresh with a hash re-verify (plus trashing the orphan helper after #6200). The version tag is retained and recorded rather than deleted if the release is withdrawn.
...
The composed seam spec for the shared hook region is #7634; the Stage-5 designs are #7563 and #7565; the user-facing release note is authored at Stage 13 under `release/releases/notes/`.
