...
| #6368 | AC-4 | *A fixture demonstrates two different kits producing materially different elicitation for the same request.* **READER-GRADED BY DECLARATION — no runnable command is present in this row, and the reason is a recorded deviation rather than an omission.** The declared fixture file is **not delivered** (Deviation D-9): the card's design satisfies this criterion from shipped pack content instead, because the two manifests already produce materially different elicitation for one request — the reasoned-empty arm on the Kanban card kind included — and a fixture would add a second divergeable copy of values the packs already declare. **Expected executor verdict: a named SKIP reading no-executable-command-in-method** | Graded by resolving one request against both shipped manifests and comparing the field asks and depth guidance that result. **Fails** when the two resolutions produce the same elicitation. **If Stage 8 reads this criterion as requiring a fixture *file*, that is a hub decision and the row is cheap to add** — Deviation D-9 says the same, and the decision belongs to Stage 9 rather than to this row |
...
| #6369 | AC-1 | *Readiness and done gates resolve the kit and evaluate its declared criteria.* Runnable limb: `grep -c -e 'criteria.readiness' -e 'criteria.done' operations/skills/delivery-engine/references/gate-definitions.md` — expect exactly 4, both facets named on the gate side | Both mode steps are wired to the resolved kit rather than to a static checklist. **Fails** when either facet is dropped. **Carried into Stage 8 as a known partial:** Dev Testing graded this PARTIAL because no fixture case exercises the done side; the rule states it and the demonstration is absent |
...
| #6369 | AC-3 | *A gate over a kind with no criteria reports that explicitly rather than passing silently.* Runnable limb: `grep -c -F 'NOT-EVALUATED' operations/skills/delivery-engine/references/gate-definitions.md` — expect exactly 10 | The rule states the explicit report with its three required fields and its denominator. **Carried into Stage 8 as a known NOT-VERIFIED:** Dev Testing found no shipped kind with an empty `criteria.readiness` or `criteria.done`, so the rule is stated and undemonstrated on shipped content — a fixture question routed to Stage 9, not repaired here |
...
| #6369 | AC-5 | *Existing gate behaviour for an unresolved methodology is preserved.* Runnable limb: `grep -c -F 'NO-EVIDENCE' operations/skills/delivery-engine/references/gate-definitions.md` — expect exactly 14, the pre-existing unresolved-path vocabulary this card does not touch. **Second limb is READER-GRADED:** whether § 4's five steps and § 4.1's closed three-value verdict set survive byte-for-byte is a diff the executor cannot run | The unresolved path keeps its verdict and says so once; § 4's five steps survive intact and the verdict set stays closed at three values; 0 changed `[LG-N-EX-k]` criterion lines against a sensitivity arm of 71 changed content lines in the same file, so the zero is attributable. **Fails** when a fourth verdict value appears or a step is renumbered |
...
| **RES-3** | **`{{ADR:…}}` deferred-binding tokens are live by design** and are not resolved by this reconciliation | The tokens bind at the Stage-12 stamp, **0** of them in link position — a token in a link target parses as a path and reports a broken cross-reference before the stamp runs | **Leave unresolved.** Stamping is `renumber-adr.py --stamp` at Stage-12 Phase A.5.7e, after the number binds |
