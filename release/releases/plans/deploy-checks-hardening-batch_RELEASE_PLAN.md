...
| #6419 | AC-4 | `grep -c json_escape_detail core/deploy/deploy.sh` — expect at least 10, being one definition plus nine call sites. The full criterion is graded by the **name- and keyword-agnostic** three-part instrument recorded verbatim in the release-scoped cross-issue criteria below, which keys on neither the `local` keyword nor any variable name and additionally asserts **0** surviving inline escape pairs. **Control arm:** `grep -c zzq_not_a_helper core/deploy/deploy.sh` must return 0 on the same non-empty file | `0 1 9` from the three-part instrument · **null-arm control:** the same instrument on the same file at baseline `18e3e787` returns `9 0 0`, so it demonstrably counts these constructs and a post-fix reading is a measurement |
