...
core/deploy/tests/test_check56_m2_advisory.sh                              edit

# ── #6742 · flag_advisory_only contract + call sites (position 2) ──
core/deploy/deploy.sh                                                      edit
core/deploy/tests/test_advisory_emitter_contract.sh                        add
core/ADRs/ADR-191-advisory-emit-asserts-only-its-own-scope.md              add
...
core/deploy/deploy.sh                                                      edit
core/deploy/tools/check-operator-toml-schema.sh                            edit
.github/workflows/repo-integrity.yml                                       edit
core/deploy/tests/test_lib_instance_path.sh                                edit
core/standards/planning-solutioning-handoff.md                             edit
release/references/standards/release-notes-standard.md                     edit
release/tools/check-release-links.py                                       edit
...
**41 declared path×card write obligations over 34 distinct paths** (reconciled to delivered state at Stage-7 DT remediation — DEV-13). `core/deploy/deploy.sh` appears once per writing card and is written by all eight; § Contention Map carries the ordering.

Two facts a reader needs alongside that count, because neither is inferable from the block above. **Seven declared paths did not land**: `core/governance/OPERATIONS.md`, `release/governance/RELEASE_PROTOCOL.md`, `release/skills/release-executor/SKILL.md`, `core/skills/pmo-qa-auditor/SKILL.md`, `operations/skills/health-check/SKILL.md`, `release/skills/release-planner/SKILL.md` and `core/hooks/block-destructive.sh`. The first three are the Tier-0 boundary that held for the whole release and were prepared as operator handoffs instead; the remaining four are open write obligations for Stage 8 to adjudicate, not silent drops. **The rows are deliberately retained rather than deleted** — a declared-and-unwritten row is a falsifiable obligation, and removing it would convert a visible gap into an invisible one.
...
| `core/deploy/tools/repair-warn-log-escapes.py` | **Not required** — enumerated rather than assumed: `core/deploy/tools/` carries 53 allowlist rows today, and the tool is an operator-run one-off over out-of-tree data, not a platform-invoked script. Confirmed empirically at Engineering: `python3 core/deploy/tools/repair-warn-log-escapes.py --self-test` runs unblocked | **CI-executed by DISCOVERY, not by a named step** — a `core/deploy/tools/*.py` file that carries a `--self-test` dispatch is picked up by `release/tools/check-selftest-coverage.py --run`, which the `selftest-discovery` job in `release-tooling-smoke.yml` gates. That discovery carries **two companion obligations of its own**, both of which turned the reconciler red until they landed: a row in `core/deploy/allowlists/selftest-coverage-manifest.txt` (Arm B manifest floor) and a row in `core/deploy/tools/README.md` (Arm E doc coverage). Both are declared above |
...
Only #6419's rows were populated at Commit 0; every other card's rows were to be populated by that card's Engineering spoke from its own Stage-5 design and its scope-locked criteria. **None were** — seven of the eight members reached Stage 7 with no row at all, while this table is a Commit-0 Survival Set element that Stage-6 C4 self-verification and Stage-7 re-execution both consume. All seven are populated below at the Stage-7 DT remediation (DEV-14), from each card's scope-locked criteria and the DT evidence on its now-closed Stage-7 sub-task. A criterion this release deliberately will not verify still carries its row with the method cell reading `[DEFERRED — <reason>]`.

**Two AC-count corrections, measured rather than carried forward.** The `ac_baseline` above was read at `18e3e787`, before the Collective Review scope-lock added criteria: #6742 carries **7** criteria at the tip (not 5) and #6165 carries **6** (not 3). The baseline line is left as the historical read it is; the rows below are the current population.

**Verdicts below are the Stage-7 DT reading, not a Stage-8 verdict.** A `PARTIAL` is reproduced as DT recorded it, with the specific residual named — several are "the instrument is sound but has never executed", which the allowlist gaps in this same remediation address on the tracked side while the deployed allowlist still lags.
...
| #6742 | AC-1 | `grep -c 'flag_advisory_only.*G-[A-Z]' core/deploy/deploy.sh` — expect **1**. **Null arm:** the same pattern over the extracted emitter body returns **0**, so the single hit is caller-owned, not shared-code-owned | **MET.** The one hit is `G-CL9` on Check 58's own call site, where the authority is true. DT extended past the declared coverage boundary: 0 gate identifiers in the 17-line / 683-byte body, sensitivity arm 8 hits over the rest of the file, specificity arm 0 |
| #6742 | AC-2 | `grep -c 'NEVER ENFORCE-CAPABLE' core/deploy/deploy.sh` — expect **2**, and both must be comment lines rather than emitted strings | **MET.** Both survivors are caller-owned comments and both are true — `adr-flip-verify` and `issue-body-anchor-drift` each measure 0 escalating emits. DT's own matcher returned 0 residual on the emitted-line population against a sensitivity arm of 40 |
| #6742 | AC-3 | `grep -c 'local _posture=' core/deploy/deploy.sh` — expect **1**, the single declaration site, whose value is the `nounset`-safe optional-third-argument form the Stage-5 spec mandated. *The pattern deliberately stops at the `=`: a probe carrying the parameter expansion itself is mangled by the invoking shell and reports a false zero* | **MET.** Canonical read of the `_scope` branch: it initialises to `""`, has two literal comparisons and one interpolating `log`, and no third branch and no default — so a token-less caller renders the class-true clause and asserts **no scope** rather than a wrong one |
| #6742 | AC-4 | [DEFERRED — the criterion grades the **execution** of control arms AE-D / AE-E / AE-F inside `core/deploy/tests/test_advisory_emitter_contract.sh`, which `BLOCK-DESTRUCTIVE-022` refuses agent-side until the deployed allowlist carries the four rows already present in the tracked one. Not deferred by choice; no permitted invocation form exists] | **PARTIAL at DT; the denominator half is now closed.** DT read the arms as correctly constructed but **never executed**, and found the stated denominator drifted: true population **17 sites / 10 families** where the gate saw **16 / 9**. The gate now sees 17 / 10 (finding A). **Residual: the arms have still never run.** Grade at Stage 8 after `./update.sh --surfaces-only` |
| #6742 | AC-5 | `test -f core/deploy/tests/test_advisory_emitter_contract.sh` — the loudness must live in a file **outside** the emitter, and this asserts that file exists rather than that the emitter grew an escalation path | **MET.** The body carries no `case`, no `ISSUES` in any form, no `exit` and no `return` >=1. Behaviourally, Arms B/C/D/E of `test_check56_m2_advisory.sh` drive the real emitter against a real counter: an M2 finding at `mode=enforce` yields `ISSUES=0`, an M1 finding at the same mode yields `ISSUES=1` — so the counter is real and can move |
| #6742 | AC-6 | `grep -c 'AC-3 standing regression' core/deploy/tests/test_check56_m2_advisory.sh` — expect **1**, asserting Arm F's block survives verbatim, since its extraction anchors are what the arm reads | **MET.** Live run: `F 0 of 196 flag_warn_or_issue call sites claim unconditional warn-only (192 carry a governing comment); sensitivity arm returns 1 hit(s)`. Suite **8 passed / 0 failed** after this remediation added Arm G (was 7/0 at DT) |
| #6742 | AC-7 | `test -f core/ADRs/ADR-191-advisory-emit-asserts-only-its-own-scope.md` and `grep -c 'PV-7c' core/disciplines/review-discipline-principles.md` — expect the file present and **1** rule reference | **MET.** ADR-191 sits beside ADR-134 (the degraded-state emit contract); the one-sentence rule is inside the existing `PV-7c`; the emitter header states arity, the closed set, the omission behaviour and why the loudness is external |
| #6222 | AC-1 | `grep -c 'OPERATOR_INSTANCE_' core/config/operator.toml.template` — expect **20** lines carrying a registered token (17 distinct tokens) | **MET.** 9 template rows + 9 schema keys landed; per-token corrections applied, with markers at the sites that are illustrative by design |
| #6222 | AC-2 | `grep -c 'OPERATOR_INSTANCE' core/standards/depersonalization-spec.md` — expect a non-zero registry population, asserting the triage dispositions reached the spec rather than staying in the ticket | **MET.** 19 dispositions recorded across the three classes (`register` / `correct-to-existing` / `exclude-as-illustrative`); the registry deltas match the triage table |
| #6222 | AC-3 | [DEFERRED — the criterion grades an observed **verdict** from `core/deploy/tools/check-operator-toml-schema.sh --repo-integrity`, which `BLOCK-DESTRUCTIVE-022` refuses agent-side. The `operator-toml-schema` job in `repo-integrity.yml` runs the same C70b/C70c/C70d legs pre-merge, so Stage 8 can read the verdict from CI] | **PARTIAL.** The predicate is verified by canonical read — bidirectional loops, and self-test arm 6 asserts it fires on a registry-to-registry disagreement naming the token and both surfaces. **Verdict never observed** |
| #6222 | AC-4 | [DEFERRED — the diff-scoped C70e leg needs a base commit to decide what is newly authored, so it runs only at the PR surface; the same execution block applies agent-side] | **PARTIAL.** C70e and self-test arms 8 / 9a / 9b / 9c verified by reading. The workflow checks out with `fetch-depth: 0` precisely so this leg can run, so Stage 8 reads it from CI rather than agent-side |
| #6222 | AC-5 | `grep -c 'C70d' .github/workflows/repo-integrity.yml` — expect **2**, and the `operator-toml-schema` job must carry **no** `paths:` filter, so the scan is whole-population rather than diff-scoped | **MET.** Confirmed as recorded: the job has zero `paths:` keys, so no path filter narrows it |
| #6222 | AC-6 | [DEFERRED — the criterion grades the execution of `core/deploy/tests/test_lib_instance_path.sh`, which had **zero** allowlist rows and **zero** workflow occurrences and so was reachable by no route at all. Finding G of this remediation supplies both routes; the deployed allowlist still lags] | **PARTIAL at DT; both routes now exist.** The test is well constructed (6c sensitivity, 6d non-vacuity) but **had never executed on any surface**. It is now enrolled in `REGRESSION_MEMBERS` and carries the four tracked allowlist forms. **Residual: still unexecuted** |
| #6214 | AC-1 | `python3 -c "import re; L=open('core/deploy/deploy.sh').read().split(chr(10)); h=[i for i,l in enumerate(L) if re.search(r'#\\s*Check 72',l)][0]; print(sum(len(re.findall(r'c71_[A-Za-z0-9_]+',l)) for l in L[h:]))"` — anchored at the Check-72 header, expect **0**. The pattern carries **no** leading `\b`, which is load-bearing because `\b` cannot fire before `c71_` when the preceding character is `_` | **MET. 0** occurrences. **Sensitivity arm: 25** at the pre-change revision over the Check-72 region, so the zero is a measurement rather than an unresolvable-pattern zero |
| #6214 | AC-2 | `grep -c 'c71_' core/deploy/deploy.sh` — expect **65** lines, carrying 152 occurrences across 18 distinct names, all of them Check 71's own | **MET.** The Check-71 region is **byte-identical** pre vs post, and **0** `c72_` identifiers leaked into it |
| #6214 | AC-3 | `grep -c 'c72_' core/deploy/deploy.sh` — expect **21** lines, carrying 25 occurrences across 6 distinct stems | **MET.** `c72_exit`(4) `c72_hit`(4) `c72_out`(6) `c72_row`(4) `c72_script`(4) `c72_seen`(3); file-wide `c71_` moved 177 -> 152, the exact complement |
| #6214 | AC-4 | `grep -c 'issue-body-anchor-drift' core/deploy/deploy.sh` — expect a non-zero count, asserting Check 72's advisory emit and its check_id survive the rename rather than being silently dropped | **MET.** The check still emits: header line + 10 DENOM rows + 5 ADVISORY findings on a live run — not silent, not absent. *Method deliberately static: grading this by a full `deploy.sh --check` makes the row's verdict depend on unrelated pre-existing instance drift.* |
| #6165 | AC-1 | `grep -cE '^_c16_[a-z_]+\(\) \{' core/deploy/deploy.sh` — expect **3**: `_c16_population`, `_c16_exempt_pair` and `_c16_violators`, all at column 0 so both surfaces call one body | **MET.** Shared definition shipped with a parity assertion retained as its efficacy test; the report block reads 0 fetches, 0 filters, 4 shared |
| #6165 | AC-2 | `grep -c '_c16_violators I[1-4] "$c14r_json"' core/deploy/deploy.sh` — expect **4**, asserting the report path calls the shared engine for every invariant instead of carrying its own copy | **MET.** Divergence reproduced on the reconstructed pre-fix source at the live population in **both** directions, with a comparator shown able to report AGREE — so the demonstration is not a one-way artifact |
| #6165 | AC-3 | `grep -c 'SHARED BODY' core/deploy/deploy.sh` — expect **2**, the parity assertion's own markers, which is what makes the arm able to go red rather than merely present | **MET.** Arm D red against the mutant; it diverges on I2 only (I2 0 vs 1) with I4 still 2, so the red is the known drift rather than a broken harness |
| #6165 | AC-4 | `grep -c '_c16_guard I' core/deploy/deploy.sh` — expect **4**, one per invariant, asserting all three divergences plus the guard land together rather than partially | **MET.** All three divergences land in `024c07af`, the only commit touching `deploy.sh` in that slice, each named by its own row |
| #6165 | AC-5 | `grep -c 'scope' core/deploy/tools/tests/test-status-label-invariant.sh` — expect a non-zero count, asserting the gh stub is scope-aware in the file the parity arms rest on | **MET.** `cac70c32` touches only the test file and lands **before** `ee581f04`, which adds Arms C and D. Arm 0 pins 7 unscoped / 4 scoped |
| #6165 | AC-6 | `grep -c '_c16_exempt_pair' core/deploy/deploy.sh` — expect **6**: one top-level definition plus the references that make the predicate reachable from both surfaces. *A probe spelling the here-string form itself is NOT usable here — its `$(` and `<<<` are rejected by the matcher, which reports unreadable rather than zero; the live-consumer count is recorded in the next column instead* | **MET, and strengthened.** All three `_c16_*` are top-level and defined before both `cmd_check` and `cmd_report`. The engine's unknown-id guard is now enforced on **both** consumers — the check path discarded its status through a here-string until finding H. Measured with a fixed-string matcher over the whole file: **0 live here-string consumers remain** and **4** guarded capture sites replace them (the one textual occurrence left is the comment that documents the defect). Verified separately: an assignment preserves a status of 2 where the here-string form yields 0 |
| #5651 | AC-1 | `grep -c 'co-deployed-lib' core/deploy/deploy.sh` — expect **10**, the co-deployed rows of the 20-row publish map the parity arm expands into concrete pairs | **MET.** 31 mapped pairs with 0 unclassified on either side at DT; the drifted-copy run reported `MATCH 30 / DRIFT 1` with a `DIVERGENT` classification. Re-measured after this remediation: **32 mapped pairs, MATCH 30 / DRIFT 1 / NOT-DEPLOYED 1** |
| #5651 | AC-2 | `grep -c "return 'DIVERGENT'" core/deploy/deploy.sh` — expect **1**, asserting the STALE-versus-DIVERGENT classifier that distinguishes a refreshable platform copy from an accepted operator edit is present | **MET.** The diverged hook is reconciled: sha256 identical (185,245 B), and the deployed mtime postdates the source commit |
| #5651 | AC-3 | `grep -c 'hook_parity_remedy' core/deploy/deploy.sh` — expect **4**, the single declared constant every finding cites, which is what makes D-5651-1 carrier-agnostic | **PARTIAL, and correctly so.** The decision is **recorded**, not **decided**, and the criterion asks for both. Which carrier regenerates the S4 hook tier is an open operator decision. **Routed to Stage 9** — not a DT gap |
| #5651 | AC-4 | `grep -c 'def compare_pair' core/deploy/deploy.sh` — expect **1**, and `grep -c 'compare_pair('` — expect **5**, asserting the control arm and the row loop share one comparison rather than the arm asserting a property of `hashlib` | **MET, and materially strengthened.** DT discharged this with the drifted-copy probe — **not** with the in-check control arm Stage 6 cited, which certified nothing: mutating the verdict to compare every file against itself left it printing `CTRL: PASS` over byte-identical output. That mutation now yields **`CTRL: FAIL`**. Finding C adds the denominator floor: an emptied or degraded listing collapses the population 32 -> 8 and now reports `ARMA FAILPROBE` |
| #5564 | AC-1 | `grep -c 'skill_content_drift' core/deploy/deploy.sh` — expect **4**, the ground-truth predicate that distinguishes "nothing to deploy" from "detection could not establish what to deploy" | **MET.** The scan is hoisted above the E-02 exit, and the E-02 line carries `Verified current: N of N roster skills match source` rather than a bare success |
| #5564 | AC-2 | `grep -c 'Deployed: ' core/deploy/deploy.sh` — expect a non-zero count, asserting the footprint-derived summary counter survives beside the new selection path | **PARTIAL.** Selection verified. The counter is verified by **canonical read** of its increment site rather than by execution — the residual DT recorded, carried forward unchanged |
| #5564 | AC-3 | `grep -c 'injected_ref_basenames' core/deploy/deploy.sh` — expect **6**, asserting Check 1's `TEMPLATE_SYNC_MAP` exclusion is applied by the diff-based predicate | **MET.** Re-measured on **both** live targets: 12 -> 2 with the exclusion, against a null-exclusion arm of 12 and `only-in` = 0 |
| #5564 | AC-4 | [DEFERRED — the second clause grades a stale-baseline deploy failing visibly through `core/deploy/tests/test_deploy_detection_honesty.sh`, which `BLOCK-DESTRUCTIVE-022` refuses agent-side. The first clause DID execute: `test_refresh_surfaces.sh` 22/0/0, Arm 2b asserting exit 0 and that the E-02 line does not match] | **PARTIAL.** The suite's expected shape was stated as 14 arms in the Stage-6 handoff against a measured **17** — corrected by finding F, which moves the expectation into the suite as a self-asserting `EXPECTED_ARMS` floor (`grep -c 'EXPECTED_ARMS=17'` returns 1). **Residual: the suite has still never run agent-side** |
| #6213 | AC-1 | `grep -c 'ADR-144' core/deploy/deploy.sh` — expect **1**, the corrected citation at the G1-03 evidence-shape site | **MET.** `59b1b552` renames the citation to the decision record that actually carries the evidence-shape rule — a one-line comment change |
| #6213 | AC-2 | [DEFERRED — graded at Stage 8. Stage 7 was a declared **SKIP** for this card per the Stage-4 applicability matrix (one comment line, no functional impact to dev-test), with the corpus sweep explicitly routed to Stage 8] | **Declared deferral, not an unverified criterion.** The sweep confirms no other citation in the corpus names the superseded record |
...
| **DEV-12** | **#6419's matrix gains two further rows the Stage-5 design did not carry** — `core/deploy/allowlists/selftest-coverage-manifest.txt` and `core/deploy/tools/README.md`. | The `selftest-discovery` gate's own reconciler, run at Engineering | **DECLARED, and the discovery is the point.** A `core/deploy/tools/*.py` file carrying a `--self-test` dispatch is picked up by discovery rather than by a roster, and that pickup carries two obligations the Stage-5 matrix could not have anticipated without running the reconciler: an Arm-B manifest-floor row and an Arm-E doc-coverage row. Both were **red** before they landed (`UNLISTED` and `UNDOCUMENTED` respectively, plus a derived denominator-identity error), and both now read `ARM B PASSED — manifest floor reconciles (76 paths)` and `ARM E PASSED — 44 of 44 tools documented`. Shipping the tool without them would have turned CI red on a gate this card does not otherwise touch. |
| **DEV-13** | **The File Change Matrix is reconciled to delivered state: 16 paths landed that it never declared, across five cards.** Stated obligations move **21 → 41** and distinct paths **17 → 34**. The stated figure was stale in a third way as well: before this reconciliation the block already carried **25 rows over 18 distinct paths**, so the prose under it had drifted from the block above it independently of any missing row. | Measured at the Stage-7 DT remediation over the 24 release commits (`18e3e787..c622946d`, no merge commits, so every path is this release's own work): `git diff --name-status` cross-referenced against the FCM block, with per-path attribution to the commit that introduced it | **DECLARED, following DEV-7's precedent, and larger than the DT finding it came from.** The consolidated DT brief named three undeclared paths (a work-hierarchy tool, a status-label test, a refresh-surfaces test); re-measuring for placement found **16**, by two independent matching methods (full path and basename). All sixteen are added to their own card's block with the verb git reports — `#6419` +1, `#6742` +3, `#6222` +6, `#6165` +2, `#5564` +4 — and the population now reconciles exactly: **0 landed paths remain undeclared.** The seven declared-but-unlanded rows are retained, not deleted, and named under the matrix so a reader sees both directions of the gap. |
| **DEV-14** | **The Per-Issue Verification table carried rows for ONE member of eight.** It held **4** rows (AC-1..AC-4), all #6419; #6742, #6222, #6214, #6165, #5651, #5564 and #6213 had none — through Stage 6 and all seven Stage-7 spokes. The table is a Commit-0 Survival Set element that Stage-6 C4 self-verification and Stage-7 re-execution both consume, so both stages ran against an artifact that was empty for 7 of 8 cards. | Measured at the Stage-7 DT remediation; rows reconstructed from each card's scope-locked criteria at the tip and the AC-verdict evidence on its now-closed Stage-7 sub-task (#7143, #7147, #7151, #7155, #7159, #7163, #7167) | **POPULATED — 33 rows added, 37 total** (#6742 7 · #6222 6 · #6165 6 · #6214 4 · #5651 4 · #5564 4 · #6213 2, beside #6419's existing 4). Each row carries the criterion, the verification method, and the DT-observed result, reproducing DT's own `PARTIAL` verdicts with their residual named rather than rounding them to PASS. Two AC-count corrections were measured in passing: `ac_baseline` was read at `18e3e787` and predates the Collective Review scope-lock, so #6742 carries 7 criteria (not 5) and #6165 carries 6 (not 3); the baseline line is left as the historical read it is. #6213's AC-2 carries an explicit `[DEFERRED — graded at Stage 8]`, matching the Stage-4 applicability matrix that made its Stage 7 a SKIP. |
