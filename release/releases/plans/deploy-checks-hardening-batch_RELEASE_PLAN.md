...
| #6742 | AC-1 | No advisory line emits an authority pointer belonging to a different check. Gate-identifier pattern over the extracted emitter body, with a sensitivity arm over the rest of the file and a specificity arm over near-miss-carrying input | **MET.** 0 gate identifiers in the 17-line / 683-byte body; sensitivity arm 8 hits (`G-CL5`/`G-CL8`/`G-CL9`); specificity arm 0. Extended past the declared boundary: across all advisory call-site text exactly **1** hit, `G-CL9` at L14559 — Check 58's own, where it is true |
| #6742 | AC-2 | No advisory line asserts an enforce-capability posture contradicting the emitting check's behaviour. Both halves of the retired sentence matched over the whole file | **MET.** 0 residual against a sensitivity arm of 40 and a superset-matcher specificity arm also returning 0. The two surviving `NEVER ENFORCE-CAPABLE` banners are caller-owned comments and both are true — `adr-flip-verify` and `issue-body-anchor-drift` each measure 0 escalating emits |
| #6742 | AC-3 | A caller supplying neither input fails loudly or emits no such clause — never a silent wrong default. Direct canonical read of the `_scope` branch | **MET.** `_scope` initialises to `""`, two literal comparisons, one interpolating `log`; no third branch and no default, so a token-less caller asserts **no scope**. `${3:-}` is the Stage-5-mandated form so `nounset` cannot abort a run over one bad caller |
| #6742 | AC-4 | A control confirms the assertion is non-vacuous, over a stated denominator. `bash core/deploy/tests/test_advisory_emitter_contract.sh` — arms AE-D (sensitivity), AE-E (sensitivity), AE-F (specificity) | **PARTIAL at DT, denominator half now closed.** DT read the controls as correctly constructed but **never executed**, and found the stated denominator drifted: true population **17 sites / 10 families** where the gate saw **16 / 9**. The gate now sees 17 / 10 (finding A of this remediation) and the harness's four allowlist rows are tracked. **Residual: the harness has still never run** — the deployed allowlist lags the tracked one, so `BLOCK-DESTRUCTIVE-022` refuses it agent-side until a deploy. Grade at Stage 8 after the deployed allowlist carries the rows |
| #6742 | AC-5 | The loudness is a static assertion **outside** the emitter body, never an in-body escalation path. Static read of the body plus behavioural arms B/C/D/E of `test_check56_m2_advisory.sh` | **MET.** Body carries no `case`, no `ISSUES` in any form, no `exit`, no `return` ≥1, and the assertion lives in a separate file. Behaviourally: Arm B drives an M2 finding through the real emitter at `mode=enforce` → `ISSUES=0` with one `ADVISORY:` line; Arm D drives an M1 finding through the same region at the same mode → `ISSUES=1`, so the counter is real and can move; Arm C shows warn/enforce byte-identical; Arm E shows the mode file is read |
| #6742 | AC-6 | Arm F of `test_check56_m2_advisory.sh` still passes — its extraction anchors and its single-hit sensitivity arm both survive | **MET.** Live run: `F 0 of 196 flag_warn_or_issue call sites claim unconditional warn-only (192 carry a governing comment); sensitivity arm returns 1 hit(s)`. Suite **8 passed / 0 failed** after this remediation added Arm G (was 7/0 at DT) |
| #6742 | AC-7 | The chosen shape is recorded where the other emitter contracts are documented | **MET.** `ADR-191` sits beside `ADR-134` in `core/ADRs/`; the rule is inside the existing `PV-7c` in `review-discipline-principles.md`; the emitter header states arity, the closed set, the omission behaviour and why the loudness is external |
| #6222 | AC-1 | Every referenced `<OPERATOR_INSTANCE_*>` token resolves or is corrected | **MET.** 9 template rows + 9 schema keys; per-token corrections applied, with markers at the illustrative sites |
| #6222 | AC-2 | Per-token triage recorded individually, one of `register` / `correct-to-existing` / `exclude-as-illustrative` | **MET.** 19 dispositions recorded; registry deltas match the triage table |
| #6222 | AC-3 | The check fails on registry-to-registry disagreement, naming the token and both surfaces. `bash core/deploy/tools/check-operator-toml-schema.sh --repo-integrity` (C70b/C70c/C70d) | **PARTIAL.** Predicate verified by canonical read — bidirectional loops, and self-test arm 6 asserts it. **Verdict never observed:** execution refused by `BLOCK-DESTRUCTIVE-022`. Grade at Stage 8, or from the `operator-toml-schema` job in `repo-integrity.yml`, which runs the same legs pre-merge |
| #6222 | AC-4 | The diff-scoped same-change arm fires on a newly authored token with no §4 row. `--diff-base <ref>` (C70e) | **PARTIAL.** C70e and self-test arms 8 / 9a / 9b / 9c verified by reading; same execution block. The PR surface runs it with `fetch-depth: 0`, so Stage 8 can read the verdict from CI rather than agent-side |
| #6222 | AC-5 | The scan covers the whole tracked population with no path filter (verify-only) | **MET.** Confirmed as recorded |
| #6222 | AC-6 | Needle-bearing resolvers assert out-of-tree. `bash core/deploy/tests/test_lib_instance_path.sh` | **PARTIAL at DT, route now provided.** The test exists and is well constructed (6c sensitivity, 6d non-vacuity) but **had never executed on any surface** — it was reachable by no route at all: zero allowlist rows and zero workflow occurrences. Finding G of this remediation enrols it in `REGRESSION_MEMBERS` and gives it the four tracked allowlist forms. **Residual: still unexecuted** pending a deploy of the allowlist |
| #6214 | AC-1 | Zero `c71_`-prefixed identifiers in Check 72's block. `c71_[A-Za-z0-9_]+` — **no** leading `\b` — over the Check-72 region | **MET. 0** occurrences / 0 stems. **Sensitivity arm: 25** on the pre-change revision, so the zero is a measurement. The widened pattern is load-bearing: `\b` cannot fire before `c71_` when the preceding character is `_` |
| #6214 | AC-2 | Check 71's block is untouched. Same pattern over the Check-71 region | **MET. 152** occurrences / 18 names, region **byte-identical** pre vs post, and **0** `c72_` leaked in |
| #6214 | AC-3 | The rename is complete, not partial. `c72_[A-Za-z0-9_]+` over the subject | **MET. 25** occurrences / **6** stems — `c72_exit`(4) `c72_hit`(4) `c72_out`(6) `c72_row`(4) `c72_script`(4) `c72_seen`(3); file-wide `c71_` 177 → 152 |
| #6214 | AC-4 | Check 72 still emits its advisory verdict. `deploy.sh --check`, read the Check 72 line | **MET.** Header line + 10 DENOM rows + 5 ADVISORY findings — not silent, not absent |
| #6165 | AC-1 | One population across both the check and report paths | **MET.** Shared definition shipped (`_c16_population` / `_c16_exempt_pair` / `_c16_violators`) with a parity assertion retained as its efficacy test; report block reads 0 fetches · 0 filters · 4 shared |
| #6165 | AC-2 | Divergence demonstrated before the fix, with an agreement control | **MET.** Reproduced on the reconstructed pre-fix source at the live population in both directions, with a comparator shown able to report AGREE |
| #6165 | AC-3 | The parity assertion is shown able to fail | **MET.** Arm D red against the mutant; diverges on I2 only (I2 0 vs 1) with I4 still 2, so the red is the known drift rather than a broken harness |
| #6165 | AC-4 | All three divergences closed in one change, each named | **MET.** All three land in `024c07af`, the only commit touching `deploy.sh` in that slice, each named by its own row |
| #6165 | AC-5 | The gh stub is scope-aware **before** any parity assertion rests on it | **MET.** `cac70c32` touches only the test file and lands before `ee581f04`, which adds Arms C and D. Arm 0 pins 7 unscoped / 4 scoped |
| #6165 | AC-6 | The exemption predicate is reachable from the report path | **MET.** All three `_c16_*` are column-0 top-level, defined before both `cmd_check` and `cmd_report`, with `main "$@"` last. **Post-remediation note:** the engine's unknown-id guard is now enforced on **both** consumers — the check path discarded its status through a here-string until finding H of this remediation (`0` of 4 sites still consume it that way) |
| #5651 | AC-1 | A parity check compares each source hook against its deployed counterpart and reports divergence. Check 79 Arm B | **MET.** 31 mapped pairs, 0 unclassified on either side; the drifted-copy run reports `MATCH 30 / DRIFT 1` with a `DIVERGENT` classification. Re-measured after this remediation: **32 mapped pairs, MATCH 30 / DRIFT 1 / NOT-DEPLOYED 1** |
| #5651 | AC-2 | The currently-diverged hook is reconciled, or its divergence explained and accepted | **MET.** sha256 identical (185,245 B); deployed mtime postdates the source commit |
| #5651 | AC-3 | Whether hooks should be deploy-published is decided separately and recorded | **PARTIAL.** Recorded as **D-5651-1**, carrier-agnostic via `hook_parity_remedy()`. Recorded, not decided — the criterion asks for both. **Operator decision, routed to Stage 9** |
| #5651 | AC-4 | Control arm: an intentionally-modified deployed hook is reported as divergent | **MET, and materially strengthened.** DT discharged it with the drifted-copy probe — **not** with the in-check control arm Stage 6 cited, which certified nothing: it hashed two in-memory literals, so mutating the verdict to compare every file against itself left it printing `CTRL: PASS` over byte-identical output. Finding B of this remediation routes the arm through the same `compare_pair()` the row loop uses; that mutation now yields `CTRL: FAIL`. Finding C adds the denominator floor — an emptied or degraded source listing collapses the population 32 → 8 and now reports `ARMA FAILPROBE` instead of PASS |
| #5564 | AC-1 | `--deploy` distinguishes "nothing to deploy" from "detection could not establish what to deploy" | **MET.** Scan hoisted above the E-02 exit; the E-02 line carries `Verified current: N of N roster skills match source` |
| #5564 | AC-2 | The second case resolves by ground-truth selection, non-zero is reserved for residual, and `skills_changed` stays footprint-derived | **PARTIAL.** Selection verified. Counter verified by **canonical read** of the increment site rather than by execution — the residual DT recorded |
| #5564 | AC-3 | Any diff-based deploy predicate applies Check 1's `TEMPLATE_SYNC_MAP` exclusion | **MET.** `injected_ref_basenames` applied; re-measured on both live targets, 12 → 2 with the exclusion against a null-exclusion arm of 12 and `only-in` = 0 |
| #5564 | AC-4 | Control arm: a genuine no-op still succeeds; a stale-baseline deploy fails visibly. `bash core/deploy/tests/test_refresh_surfaces.sh` and `bash core/deploy/tests/test_deploy_detection_honesty.sh` | **PARTIAL.** First clause **executed**: `test_refresh_surfaces.sh` 22/0/0, Arm 2b asserting exit 0 and that the E-02 line does not match. Second clause rests on `test_deploy_detection_honesty.sh`, whose expected shape the Stage-6 handoff stated as 14 arms against a measured **17** — corrected by finding F of this remediation, which moves the expectation into the suite as a self-asserting `EXPECTED_ARMS` floor. **Residual: the suite has still never run agent-side** (deployed allowlist lags) |
| #6213 | AC-1 | The citation names the correct decision record | **MET.** `59b1b552` renames the citation to ADR-144 at the G1-03 evidence-shape site — a one-line comment change |
| #6213 | AC-2 | A corpus sweep confirms no other citation names the wrong record | **[DEFERRED — graded at Stage 8.]** Stage 7 was **SKIP** for this card per the Stage-4 applicability matrix (one comment line, no functional impact to dev-test), with the sweep explicitly routed to Stage 8. This is a declared deferral, not an unverified criterion |
