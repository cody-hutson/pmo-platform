...
core/ADRs/ADR-193-advisory-emit-asserts-only-its-own-scope.md              add
...
| #6742 | AC-7 | `test -f core/ADRs/ADR-193-advisory-emit-asserts-only-its-own-scope.md` and `grep -c 'PV-7c' core/disciplines/review-discipline-principles.md` — expect the file present and **1** rule reference | **MET.** ADR-193 sits beside ADR-134 (the degraded-state emit contract); the one-sentence rule is inside the existing `PV-7c`; the emitter header states arity, the closed set, the omission behaviour and why the loudness is external |
