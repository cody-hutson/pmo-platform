...
|-------|----|-------------------|----------------|
| #4995 | AC-1 | `grep -c 'ADR-170' release/ADRs/ADR-170-adr-citations-bind-at-the-claim-not-at-authorship.md` resolves, and the record's `## Status` names the amendment of ADR-115 while its `## Decision` states the citation-time rule: `grep -c 'Amends ADR-115' release/ADRs/ADR-170-*.md`. Specificity arm, same instrument same target: `grep -c 'ZZQQ-NOT-A-STATUS' release/ADRs/ADR-170-*.md` | Record exists; `## Status` names the amendment ≥1; `## Decision` states when a number enters branch-authored prose and what resolves it · specificity **0** |
| #4995 | AC-2 | Simulated concurrent allocation in the shell suite: on a fixture branch author a record at next-free, land a colliding number on a simulated mainline, run the reconciliation, and assert the changed-file set is the ADR file plus the index surfaces | Changed-file set = the ADR file + index surfaces only. **Sensitivity arm (the pre-change behaviour):** the same fixture under the baseline tool yields a strictly larger changed-file set — measured, not asserted |
| #4995 | AC-3 | Branch-scoping preserved: assert every mainline-unchanged file is byte-identical after the reconciliation, and that a mainline file citing the old number for a *different* record is untouched **without** needing a manual `--exclude-path`. Method: `git diff --name-only origin/main...HEAD` before/after the fixture move | Zero mainline-unchanged files modified · control: a file deliberately inside the branch diff **is** modified (non-zero), so the zero is a measurement rather than an unresolvable scope |
| #4995 | AC-4 | The exemption is widened at the level of its **population**: `python3 release/tools/renumber-adr.py --self-test` carries one arm per facet — `exempt/wrapped-continuation-is-record` (hard-wrapped continuation), `exempt/devlog-row-is-ambiguous` (Deviation Log row), and the dry-run parity arms (#5080's facet c) — all consuming one registry. Structural limb: `python3 -c "import re; s=open('release/tools/renumber-adr.py').read(); print('CLASSIFIER_DEFS', len(re.findall(r'^def classify_lines', s, re.M)))"` | `--self-test` exits 0 with all three facet arms green · `CLASSIFIER_DEFS 1` · **negative control per facet (mandatory):** the pre-fix tool sweeps or mis-predicts each fixture — a green arm whose control is also green is a broken probe |
...
| #4995 | AC-6 | `grep -n 'A.5.7' release/references/pipeline/stage-12-execute.md` resolves and the surrounding text matches ADR-170's decision — specifically that the stamp is the **last** step of the claim (`detect → reconcile → stamp → verify-zero-tokens`) and that every named review site is dispositioned. Specificity arm: `grep -c 'A.5.99' release/references/pipeline/stage-12-execute.md` | A.5.7 resolves; the ordering and the review-disposition obligation are both stated · specificity **0** |
| #4995 | AC-7 | `grep -n 'G-EX9' core/schemas/gate-criteria-spec.md` resolves and its criterion text is consistent with the amended mechanism — the ALL-conjunction gains the `--stamp --check` limb and the step-set is restated as seven. Line-disjointness limb (R2 / AI-006): assert the Method column's provenance predicate is **byte-unchanged**, since #4762 owns it. Specificity arm: `grep -c 'G-EX99' core/schemas/gate-criteria-spec.md` | G-EX9 resolves; stamp limb present; step-set reads seven; Method-column provenance predicate byte-unchanged · specificity **0** |
...
| #6244 | AC-1 | A single-number historical citation inside a release-plan Deviation Log row is **not** rewritten. Method: fixture ACT 4 row-literal assertion — `grep -cF '\| DEV-41 \| prior hop \| ADR-004 was renumbered after the sibling merge \|' "$PLAN"` = 1, plus `grep -c 'DEV-41.*ADR-006' "$PLAN"` = 0 | Row byte-unchanged (1) and did not acquire the new number (0) · **precondition arms first:** the fixture plan is demonstrably in sweep scope, and the move exits 0 — both fail *silently green* if unasserted |
| #6244 | AC-2 | A genuine live citation in the same file class **is** still rewritten — the discrimination arm. Method: assert the swept form outside the Deviation Log section (§ Implementation Sequence) **and** after the section closes (§ Notes) | Both swept, expected **non-zero**. The after-the-section site is the only one that distinguishes "the section opened" from "the section closed at the next same-or-higher heading" — without it the region rule could be implemented as open-and-never-close and every other arm would still pass |
| #6244 | AC-3 | The incoherent-range case does not reproduce. Method: seed a range straddling swept and unswept tokens; assert coherence after, via a `python3` scan counting ranges whose low end exceeds their high end (the local `grep` is `ugrep`, and a rejected back-reference pattern yields a plausible zero) | Straddling range byte-unchanged (1) · incoherent ranges **0** · sensitivity: the general property arm would still fire on a *different* incoherent range, so the zero is not string-specific. **Residual recorded, not claimed as covered:** a range straddling swept and unswept tokens *outside* any record region is still split incoherently — the design fixes the Deviation-Log instance, not the range class |
| #6244 | AC-4 | Where the sweep cannot classify a site, it names the site rather than rewriting it. Method: assert the `R3 REVIEW:` block names the fixture plan and names the un-swept live row inside the section | Ambiguous site named and unmodified · the un-swept live row named too (after ratified premise 1 that line is its **only** detector) · **zero-site control:** the `R3 REVIEW:` block is emitted on a run with no ambiguous sites at all — a step that is silent when it finds nothing is indistinguishable from a step that did not run |
| #6244 | AC-5 | A regression arm on a record with ≥2 prior hops, since the surface only populates after a second renumber. Method: hop 2 of ACT 4 — a row recording hop 1 is authored by hand between the hops, then the second renumber runs | Second hop exits 0; the hop-1 row survives **byte-identical**; its current number was not silently advanced. **This is a *run*, not a string:** the row's `ADR-<current>` token is the live hazard, and hop 2 is the only moment it exists — a naive sweep rewrites it and the row becomes syntactically valid, internally consistent, and false |
...
| #4929 | AC-2 | Check 54's I3 predicate can fire. Method: negative control — a fixture declaring `Maintains-entity: <E>` for an entity whose § 6 Maintainer is a different skill; assert `check-ownership-collision.py` exits **1** and names the collision | Post-adoption tool exits **1** and names the collision · **control, same instrument same fixture:** the pre-adoption tool exits **0** on the identical fixture. A predicate that cannot be made to fire is not a check |
