...
| **R7** | **`main` had read red for 6 days on `selftest-discovery` when this row was filed** — 17 consecutive failures at that point | MED | — | **HISTORICAL — the condition is closed, and closing it is what the release does.** At filing, Stage 7 / 8 spokes had to be briefed that this red was baseline rather than their regression, and the discriminator was specific: the *expected* red named only `.py` operands under `Arm F MANDATE REACHABILITY` and printed `ARM F POSTURE:`, while any Arm B / C / D / E / runner-partition line, or a non-`.py` operand under Arm F, was a genuine regression. **On this branch the job concludes `success`** — a residual-only run exits `4` and the gate-decision step maps that to `exit 0` — so no spoke is owed the briefing, and the hand-discrimination this row asked a reader to perform is precisely what the release now performs. The `main` streak stood at **18** at its last measurement (`DEV-18`); the **17** above was accurate when filed and is left as filed |
...
| #7440 | AC-1 — a residual-only run's job conclusion differs from the conclusion reported when an arm that must hold has regressed | Drive **one** tree through both the baseline `343fbd20` tool and the branch tool, three ways — residual only · residual **+** a genuine Arm B(i) regression · residual **+** a genuine Arm E regression — with the residual present in all three, and compare exit codes | Baseline `1 / 1 / 1` — indistinguishable, the defect exactly as filed. Branch `4 / 1 / 1` — separable, **with the residual still present in all three trees**. Exit `4` maps to job **success**, exit `1` to **failure**. One measurement carries AC-1, AC-2 and AC-3 together `[S6 #7448 · S7 #7451 → PASS]` |
| #7440 | AC-2 — a deliberately introduced Arm B regression changes the job's conclusion, via a probe that fails against current behaviour and passes after the fix | Strip the `--self-test` dispatch from a victim tool while a keeper tool preserves the directive's yield, manifest path on disk; run `--reconcile` | Exit `1`, `Arm B(i) MANIFEST FLOOR` fires, residual still named. The baseline tool reads `1` **both** ways, which is what makes the probe fail before and pass after rather than pass vacuously `[S6 #7448 · S7 #7451 → PASS]` |
| #7440 | AC-3 — the same holds for a deliberately introduced Arm E regression | Place an undocumented tool under `core/deploy/tools/`; run `--reconcile` | Exit `1`, Arm E fires, residual unchanged. Baseline `1` both ways `[S6 #7448 · S7 #7451 → PASS]` |
| #7440 | AC-4 — a reader tells an expected residual from a genuine regression **at the check surface**, without opening the run log | Read the `Discovered tool self-tests (Ubuntu)` job's **annotation list** on this branch — not the log body, and not a fixture | A **success** conclusion carrying `##[warning]selftest-discovery: EXPECTED RESIDUAL only — no arm that must hold has regressed`, so the annotation alone decides it and no log read is required `[S6 #7448 · S7 #7451 → PASS]` |
| #7440 | AC-5 — the tool's `--self-test` covers the new distinction, including a control arm that must fire | T-62 carries the collapsed-table control arm; run the `M-COLLAPSE` mutant — `EXPECTED-RESIDUAL` collapsed onto member `0` — against the **unmodified** suite | T-62 fires; suite **59/62**. **A control arm that cannot be made to fire has asserted nothing**, so the mutant is what makes T-62's green readable `[S6 #7448 · S7 #7451 → PASS]` |
| #7440 | AC-6 — the precision probes Arm F's step ordering protects still run, and still fail on a genuine regression | Read the branch CI log for the producer step and the two probe steps | `Control OK — the unmutated copy passes` and `Anti-narrowing guard OK (exit 1)` both present. **Strengthened rather than merely preserved:** the producer step runs under `set +e` and always exits 0, so it cannot skip a later step, and the only failing step is last `[S6 #7448 · S7 #7451 → PASS]` |
| #7201 | AC-1 — the annotation carries the posture marker, visible in a run's annotation list | Run `--reconcile` on the real tree and read the run's **annotation list** — the surface a CI reader actually sees, not a fixture | `::warning::EXPECTED-RESIDUAL (advisory — …)`, rendered by the host as `##[warning]EXPECTED-RESIDUAL (advisory …)` with the marker in **first position**, and **zero** Arm F `::error::` emissions in the run. **Probe-validity note carried for the next reader:** searching a downloaded log for the raw `::warning::` form returns a **false** zero, because the log renders the directive as `##[warning]` — a sibling-needle control arm is what distinguishes that artifact from an absent annotation `[S6 #7449 · S7 #7452 → PASS]` |
| #7201 | AC-2 — the runtime log note's **intent** is preserved: the note undisturbed, **except where a sibling card in this release invalidated its premise** | Grade against the **amended** criterion on #7201, not the superseded literal it replaced; read the diff scope, then clause-check the corrected sentence against the partitioned behaviour. **The retired wording is deliberately not quoted in this cell, and restoring it would break RV-10:** `verify-release-plan.sh` keyword-classifies the Method column, and the retired literal's own vocabulary routes this row to an executor whose command is a whole-workspace probe with no bearing on this criterion — a FAIL that grades the workspace rather than the card. The retired wording is quoted in `DEV-8` and `DEV-13`, which the classifier does not read | Exactly one sentence changed, plus the two clauses of the comment block immediately above it that the edit itself falsified, declared as a scope note rather than folded in silently; `git show --stat f80c00e1` = **1 file, 32 insertions, 5 deletions**, nothing else in the log touched. Every surviving clause true after the partition, and the falsified *"stay red"* claim absent from live output. **Grading against the superseded literal would produce a false finding here** `[S6 #7449 c4 — the DEV-13 Tier 1 [ADJUST] record · S7 #7452 → PASS]` |

**Both `[OWED]` scaffolds are now filled, and the rule they carried is why the gap mattered.** A member reaching Stage 7 with an empty row leaves both Stage 6 self-verification and Stage 7 re-execution grading against an empty artifact. #7193's rows were authored at Engineering Commit 0; #7440's and #7201's were owed to their own Engineering spokes, were not filled there, and are transcribed here at the **Tier 1 [ADJUST] plan reconciliation (D-PlanReconcile, recorded on sub-task #7447)** — the gap was raised at Stage 7 as finding **F-02** on #7452, Minor and not blocking. **No figure in these rows is re-derived and no suite was re-run to write them:** every value is transcribed from the cited sub-task's own record, per this section's standing rule. The **AI-001** disposition stands unchanged — #7201 resolved to **branch (b)**, so it is *implemented* and graded on its own criteria rather than recorded satisfied-by-design.
...
| RV-7 | `check-selftest-coverage.py`'s own suite stays green | `python3 release/tools/check-selftest-coverage.py --self-test` after #7440 and after #7201 | `EXIT=0`. **Note R7 (historical): `main`'s `selftest-discovery` job had read red for 6 days on the Arm F residual when this row was written — that red was baseline, not a regression introduced here. On this branch the job concludes `success`, which is this release working rather than a condition still to be worked around** |
...
| **DEV-3** | `.github/release-corpus-completeness.enforce` is likewise absent from the card; its mode legend states the collapse and becomes false on this commit. | Stage-5 substrate-drift finding | Added to the File Change Matrix at Stage 5, same class of addition. Comment lines only — the token line is untouched, re-verified after the edit. |
| **DEV-4** | **The falsification note is a three-site cascade, and a fourth file states the collapse too.** The card's Documentation Impact says *"the gate's falsification note"* — singular | Stage-5 Phase 0.5 substrate-drift finding, class C2 | **DESIGN COVERS ALL FOUR.** All four sites rewritten to one text, adapted only to each site's comment prefix, so no site describes any collapse as intended. The fourth file's own File-Change-Matrix addition is recorded separately at DEV-3. |
...
| **DEV-6** | **A defect this PR introduced and then closed.** The new exit-3 arm rendered the PV-7a Register B terminal token space-separated in its summary heading. Check 69 / the `pv7-vocabulary` CI job freeze that token at one sanctioned spelling — the hyphenated form — and are **enforcing, whole-corpus, with no mode gate**, so this would have turned the PR red. | Caught by `deploy.sh --check` at HEAD (`FAIL: pv7-vocabulary — …:156`), not by CI | **Reconciled in `28b0bfdb`.** The primitive now exits 0 with **0** unsanctioned occurrences over **2042** tracked files, and its own control arm moved **228 → 229** sanctioned occurrences — which is what confirms the extractor read this line rather than skipping it. Every other rendering in the release already used the hyphenated form. |
| **DEV-7** | **#7440's Stage-5 fixture audit under-counted by six.** The design stated, fixture by fixture and explicitly "not assumed", that **exactly two** existing `--self-test` fixtures would change expected exit code (T-44 and T-56). The measured count on running the suite is **eight**: T-16, T-19, T-25, T-41, T-44, T-46, T-48 and T-56 | Measurement at Engineering — the suite was executed, not re-read | **ALL EIGHT UPDATED, AND STRENGTHENED RATHER THAN RENUMBERED.** Each asserted `rc == 0` on a fixture tree where Arm E or Arm F could not measure, so each was the suite recording the *unmeasured-shares-the-clean-member* defect as expected behaviour — the design's diagnosis was right and only its denominator was wrong. Each now asserts `3` **plus** the arm marker its control depends on, which is strictly stronger than the bare `== 0` it replaces. Recorded because the design claimed to have checked each fixture individually |
| **DEV-8** | The design specified the runtime `ARM F POSTURE:` note as **byte-unchanged**. Its text was byte-unchanged at this commit; its **guard** is not — it moved from `if unreachable:` to the residual group. | Engineering judgement against the release's own Outcome Statement | **Narrowed deliberately.** With the classes separated, a BLOCKING-only run would have printed "EXPECTED to stay red … this is the arm reporting, not a broken gate" over a genuine regression — the masking defect this release removes, re-introduced in the log channel one commit after removing it from the exit channel. No fixture asserted the line (swept: 1 occurrence corpus-wide, in the tool itself), and #7201's stated criterion at the time was that the runtime note is unchanged, which the text then satisfied. **Superseded in part by DEV-13:** that criterion was later amended and the note's text corrected. The narrowed guard this row records is untouched by that correction and still stands. |
| **DEV-9** | **A pre-work item bound to this sub-task as "CD-3" was already landed.** The brief described the **SKIP-clause full-checkout dependency** comment; that content shipped in `bae6ea6f` at `deploy.sh` `_c32_verdict_exit_code` | Verified against the branch before acting: **1** occurrence of `sparse-checkout` in `deploy.sh`, introduced by that commit; the consuming workflow takes a bare `actions/checkout@v7` with no `sparse-checkout:` and no `filter:` | **NOT DUPLICATED** — a second copy of that reasoning is precisely the drift the authoring-home discipline exists to prevent. The A6.5 counter-design finding **actually numbered CD-3** (one derivation sentence for how a consumer disposes of `NOT-EVALUATED`) was *not* landed and **is** what this release's new `3)` arm makes load-bearing, so that is what was implemented instead |
| **DEV-10** | **Implementing CD-3 at the convention's declared authoring home adds `core/deploy/deploy.sh` to this card's change matrix**, which the Stage-5 File Change Matrix did not list for #7440 | Flagged rather than silently taken | **ACCEPTED, COMMENT-ONLY.** Two paragraphs, zero behaviour change; `bash -n` clean, and no `deploy.sh` check executes the amended tool (swept: **7** references to it under `core/deploy/`, of which **2** are these new comment lines and **none** is an invocation) |
| **DEV-11** | **The Stage-5 specimen rendered the posture marker space-separated** (`EXPECTED RESIDUAL`), which would have been a second spelling of a token this same file freezes as `EXPECTED-RESIDUAL` in `_V_EXPECTED_RESIDUAL`, in the `RECONCILE VERDICT:` line, and in the docstring's exit table. | Engineering judgement against this release's own subject matter | **Shipped hyphenated.** A divergent rendering of a token whose whole purpose is that two channels agree would be this release authoring its own defect one register over — and the corpus already carries an enforcing gate (Check 69 / `pv7-vocabulary`) against exactly that failure for the sibling Register B token. The leading token of the annotation is now byte-identical to the verdict name printed at the end of the same run, which is what integration criterion INT-1 asks for. |
| **DEV-12** | **The Stage-5 specimen also reworded the message BODY**, replacing "so the agent executing that stage cannot run what the spec tells it to run" with "and their operand suffix domain is EMPTY". | Engineering judgement; the specimen predates #7440's A6.5 fix | **Prefix added, body byte-unchanged.** The specimen was written before Finding 1's existence limb landed, and #7440's own Stage-6 record cites the shipped sentence — "Each operand below **exists on disk** and its suffix domain is EMPTY" — as the fail-open fix visible in plain sight. Taking the specimen's body literally would have deleted that evidence. The card's acceptance criterion is that the annotation *carries the posture marker*, which the prefix satisfies; its estimate is "approximately one line", which a body rewrite exceeds. Flagged rather than silently reworded in either direction. |
| **DEV-13** | **The runtime `ARM F POSTURE:` note asserted that the `selftest-discovery` job "is EXPECTED to stay red until the `.py` population above drains" — a claim #7440 falsified inside this same release.** With the exit space partitioned, a residual-only run exits `4`, the gate-decision step maps `4` to `exit 0`, and the job concludes **success** while Arm F still reports and still names all five `.py` operands. | **Tier 1 [ADJUST], operator-rendered 2026-09-12**, amending #7201's AC2: the note's *intent* is preserved — undisturbed **except where a sibling card in this release invalidated its premise**. This row previously recorded the finding as observed-and-held; that hold was correct, an AC whose premise a sibling card invalidated being an operator disposition rather than an engineering judgement | **Executed — one sentence.** Left standing, the note instructed a reader to expect a red that no longer occurs: this release's own defect surviving in its own output, and a future genuine red waved through by anyone trusting it. The `ARM F POSTURE` marker is unchanged and still leads the line, as does every clause that remains true — the job is ADVISORY and is NOT a required context, the population is unmatchable BY CONSTRUCTION and not remediable today, and this is the arm reporting rather than a broken gate. What replaced the false clause is the discriminating fact: the job's conclusion now SEPARATES that residual from a genuine regression, so a red on this job is never this population. **T-60C strengthened, not renumbered** — it asserted `"ARM F POSTURE" in out`, the note's PRESENCE and never its CLAIM, which is how a sentence the partition had already falsified went on shipping under a green control; the claim is now pinned beside the marker, its positive limb load-bearing and its negative limb readable off that positive limb on the same buffer. Mutation-verified: reverting the sentence alone takes the suite to **61/62 with T-60C the single failure**. T-50 asserts nothing about this text and is untouched. |
| **DEV-14** | **#7193's severity framing was aspirational.** *"A false clean here can admit a merge"* is not true today | Stage-4 Tier 1 [ADJUST] | **RECONCILED to latent-rather-than-live**, with the body left as historical record. A public issue edit does not scrub its history, and the design is built against current state |
| **DEV-15** | **#7201's Affected Files named the wrong file entirely** | Stage-4 Tier 1 [ADJUST] | **RE-POINTED BEFORE ENGINEERING** to `release/tools/check-selftest-coverage.py`, with Documentation Impact and Proposed Change reconciled to match |
| **DEV-16** | **#7193's SKIP is sentinel-AWARE — a deliberate departure from the nearest sibling precedent** (Check 61's SKIP is sentinel-agnostic) | Stage-5 design decision, grounded on a measured premise | **RATIFIED.** Check 61's agnosticism rests on its not-evaluated state being *structurally unreachable to fix in CI*. That premise is **false for Check 32**: its verdict input is a **tracked** file, present and parseable on every CI checkout, so a CI `SKIP` means the PR deleted or broke the ledger — which the PR author can fix in the PR. Importing the sibling's remedy would ship a gate that goes quiet the instant its ledger is deleted. **Bound worth recording:** the argument's strength is a property of *this workflow's checkout configuration* (full `actions/checkout`, no sparse-checkout, no filter), not merely of the file being tracked |
| **DEV-17** | **The plan's Deviation Log and the PR body's had desynchronized, and the plan's Per-Issue Verification carried two `[OWED]` rows into Stage 8.** Measured on the branch at `f80c00e1`: plan **11** rows, PR body **13**, with four identifiers agreeing (`DEV-1`, `DEV-2`, `DEV-4`, `DEV-5`) and **seven** denoting a *different* deviation on each surface. #7440's spoke mirrored its deviations into the plan at `7a9a8245`; #7201's spoke recorded its three in the PR body only. Separately, the plan's `DEV-9` still asserted that the `ARM F POSTURE:` note's text was byte-unchanged and that #7201's second criterion was satisfied by that byte-identity — two clauses DEV-13 had falsified | **Tier 1 [ADJUST] — D-PlanReconcile**, operator-rendered 2026-09-12, recorded on sub-task #7447. Raised at Stage 7 as findings **F-01** (desynchronized log) and **F-02** (the `[OWED]` rows) on #7452, both Minor and neither blocking. Run *before* Stage 8 deliberately: Stage 8 grades per-criterion against the Per-Issue Verification section, so an empty row would have degraded its own grading | **RE-SYNCED ON ONE NUMBERING, AND THE PR BODY'S IS CANONICAL.** `DEV-1`–`DEV-13` keep the identifiers the PR body already carried; the three rows the plan held alone are appended as `DEV-14`–`DEV-16`, so **no existing PR-body identifier changes meaning**. The two `[OWED]` rows are filled by transcription from each member's Stage-6 record and Stage-7 report, re-deriving no figure and re-running no suite. `DEV-8` is reconciled to the PR body's already-corrected text. **Plan-file only** — no code, workflow, test or issue body was touched, and PR #7462 stays draft |
| **DEV-18** | **Two ship-defects in the release's own narrative surfaces, caught at Stage-8 QA after acceptance had already graded 13/13.** (a) The Change Description's `#7201` row asserted *"the runtime posture note is byte-unchanged"* in the plan while the PR body's twin row asserted the note was *corrected* — the two surfaces stating opposite things about the same fact, with the plan's clause the one DEV-13 had falsified. The earlier D-PlanReconcile sweep (DEV-17) re-synced the Deviation Log 17/17 and both `[OWED]` rows but never reached the Change Description, so the surfaces re-diverged on the very claim that sweep was run to repair. (b) Both surfaces' Outcome paragraph said the release-tooling job had reported `failure` on **"six consecutive `main` runs"** — a hub-authored conflation of the six-*day* figure with a run count, then "confirmed" by a `gh run list --limit 6` query whose cap was read as a measurement | **Tier 1 [ADJUST] — D-ShipDefects**, operator-rendered 2026-09-13, recorded on sub-task #7447. Raised as Stage-8 QA findings **QA8-1** and **QA8-2**, neither blocking, both on narrative surfaces rather than on delivered behaviour | **BOTH CORRECTED; THE PR BODY IS CANONICAL FOR (a).** The plan's `#7201` row now carries this body's clause verbatim, and all three Change Description rows are byte-identical across the two surfaces — `#7193` and `#7440` already were, and serve as the comparator's control arm. For (b) the streak was **re-measured, not transcribed**: over a 60-run `main` window of `release-tooling-smoke.yml` (control arm **39 `success` / 21 `failure`**, so the probe discriminates), the newest **18** runs are consecutive `failure`, the first `success` sits at index 18 (`5b17d615`, 2026-09-06T17:31:54Z), and in every sampled streak run the `selftest-discovery` job (`Discovered tool self-tests (Ubuntu)`) is the **sole** failing job — so the run-level streak is a faithful proxy for the job-level one. **"six" → "18" on both surfaces.** Three neighbouring figures are deliberately **untouched** because they are correct: the **6 days** in `R7` and `RV-7` (2026-09-06 → 2026-09-12 is exactly six days) and the **17 consecutive failures** in `R7`, which was accurate at issue-filing time. Harmonising those to 18 would have replaced one error with three. **Narrative surfaces only** — no code, workflow, test, issue body or milestone description was touched, and PR #7462 stays draft |
| **DEV-19** | **The Arm F pass line counted SUPPRESSED operands among the ADMITTED — A6.5 finding FM-3 limb B, ruled a CIAC-1 violation at the Stage-9 gate.** `mode_reconcile`'s Arm F sends every operand down exactly one of three paths — matched by an allowlist row, suppressed by a written exclusion, or unreachable — but its pass line read *"all {len(invocations)} spec-prescribed invocation(s) are admitted by the allowlist"*, which is the **total**, not the admitted count. On any run that suppressed anything the sentence was false, and on an all-suppressed tree it asserted that all N were admitted when **none** were. A suppressed finding is a **degraded verdict, not a resolved one**, and the arm's own pass line collapsed the two — this release's defect class reproduced inside the release. Three narrative defects rode alongside it: the plan's Deviation Log had re-desynchronized at **17** rows against the PR body's **18**; `CIAC-2`'s *Shared surface* still cited pre-partition sites (`~L1035` / `~L1065`, which now land inside **Arm E**, a different arm entirely); and `R7` / `RV-7` framed `main`'s red as ongoing when the job concludes `success` on this branch | **Tier 1 [ADJUST] — D-FM-3 + D-GateFix**, operator-rendered 2026-09-13 as the four conditions on the Stage-9 GO, recorded on sub-task #7456 | **MESSAGE CORRECTED, EXIT CODE UNTOUCHED, AND THE FIX CARRIES AN ARM THAT DISCRIMINATES.** The pass line now states both counts unconditionally — `{admitted} of {total} admitted by a matching allowlist row, {n} suppressed by a written exclusion (dispositioned, not admitted)` — with `admitted` **counted in the loop rather than derived by subtraction**, so a future fourth disposition cannot silently restore the over-report. **Limb A (the exit mapping) is explicitly out of scope**, operator-routed to a next-release card: whether a suppressed operand should also move the *verdict* is a separate question from whether the sentence describing it is true, and only the sentence is settled here. **T-57 added** on T-52's all-suppressed tree, with a strengthened **T-55** as its control — the pair differs by exactly one fact, whether the operand was admitted or suppressed, and T-55 now asserts the clean side's `1 of 1` / `0 suppressed` so the counters are pinned as unconditional. Mutation-proven in isolation on scratch copies, never on a worktree file: the unmutated copy runs **63/63** in the same location (so a red is attributable to the mutation, not the tree); a **wholesale** revert of the sentence takes the suite to **61/63** with T-57 *and* T-55 failing; and a **partial** revert that keeps the honest counters and re-appends the false total takes it to **62/63** with **T-57 the single failure** — which is what proves the arm's negative limb load-bearing rather than decorative. Suite **62/62 → 63/63**; `check-pv7-vocabulary.sh` clean at exit 0 (control arm **246** sanctioned occurrences over **2042** tracked files). The three narrative defects are corrected on both surfaces, and `CIAC-2`'s sites were **re-derived after** the code edit that moved them rather than transcribed — `warn()` at ~L1252 and `return _V_EXIT[verdict]` at ~L1326. **Three neighbouring figures are deliberately untouched** because each is correct at its own instant: the **6 days** and **17 consecutive failures** in `R7`, and the **18** measured at `DEV-18` |

**One numbering, and it is the PR body's.** `DEV-1`–`DEV-13` keep the identifiers PR #7462's body already carries, and this file is brought to them; the three rows the plan held alone are appended as `DEV-14`–`DEV-16`, so **no existing PR-body identifier changes meaning**. The PR body is canonical because its identifiers are the ones cited by artifacts this reconciliation may not rewrite — #7201's **amended second acceptance criterion names `DEV-13` in the issue body itself**, as do the reopen note and the Tier 1 [ADJUST] record on #7449, the Stage-7 Dev Testing report on #7452, and `DEV-8`'s own *"superseded in part by DEV-13"* clause. Renumbering `DEV-13` would dangle every one of them.

**What cited the retired plan-side identifiers — stated, not left to be discovered.** Seven plan rows moved, and the Stage-7 report on #7452 cites three of them by their old numbers: its finding **F-01** carries a *Plan ID* column naming `DEV-9`, `DEV-10` and `DEV-11`, and its *Noted, not raised* list names *"Release plan `DEV-9` (L427)"*. That report is this crosswalk's own source and it recommended the renumber; the mapping is restated here so a reader arriving with the old numbers lands in the right row.

| Subject | Retired plan ID | Canonical ID |
|---|---|---|
| #7440's Stage-5 fixture audit under-counted by six | `DEV-8` | **`DEV-7`** |
| the `ARM F POSTURE:` note's guard narrowed | `DEV-9` | **`DEV-8`** |
| the "CD-3" pre-work item was already landed | `DEV-10` | **`DEV-9`** |
| CD-3's authoring home adds `core/deploy/deploy.sh` | `DEV-11` | **`DEV-10`** |
| #7193's severity framing was aspirational | `DEV-3` | **`DEV-14`** |
| #7201's Affected Files named the wrong file | `DEV-6` | **`DEV-15`** |
| #7193's SKIP is sentinel-AWARE | `DEV-7` | **`DEV-16`** |

**Nothing inside this plan cited a `DEV-N` outside the table itself** — measured before the change: **11** occurrences, every one of them a row label — so the renumber's in-file blast radius is the table, and the citations to reconcile are the external ones named above.

**The two surfaces agree on identifier, subject, authority and disposition; they are not byte-identical, by design.** Where this file already carried a row whose clauses are all true, its own fuller prose is kept rather than overwritten with the PR body's shorter rendering; where a clause was false (`DEV-8`) or scoped to a row that now exists separately (`DEV-4`), the PR body's already-reconciled text is taken; where this file carried no row at all (`DEV-3`, `DEV-6`, `DEV-11`, `DEV-12`, `DEV-13`), the PR body's cells are transcribed. Reconciling the wording further would delete content on one surface or the other, which is not what the desynchronization cost.
