...
| **DEV-8** | **#7440's Stage-5 fixture audit under-counted by six.** The design stated, fixture by fixture and explicitly "not assumed", that **exactly two** existing `--self-test` fixtures would change expected exit code (T-44 and T-56). The measured count on running the suite is **eight**: T-16, T-19, T-25, T-41, T-44, T-46, T-48 and T-56 | Measurement at Engineering — the suite was executed, not re-read | **ALL EIGHT UPDATED, AND STRENGTHENED RATHER THAN RENUMBERED.** Each asserted `rc == 0` on a fixture tree where Arm E or Arm F could not measure, so each was the suite recording the *unmeasured-shares-the-clean-member* defect as expected behaviour — the design's diagnosis was right and only its denominator was wrong. Each now asserts `3` **plus** the arm marker its control depends on, which is strictly stronger than the bare `== 0` it replaces. Recorded because the design claimed to have checked each fixture individually |
| **DEV-9** | **The runtime `ARM F POSTURE:` note's GUARD is narrowed, though its TEXT is byte-unchanged as specified.** It moves from `if unreachable:` to the residual group | Engineering judgement against the release's own Outcome Statement | **NARROWED DELIBERATELY.** With the classes separated, a BLOCKING-only run would print *"EXPECTED to stay red … this is the arm reporting, not a broken gate"* over a genuine regression — the masking defect this release removes from the exit channel, re-introduced one commit later in the log channel. Swept before changing: **1** occurrence corpus-wide (the tool itself); **no** fixture asserts the line. #7201's second criterion is that the runtime note is unchanged, which the byte-identical text satisfies |
| **DEV-10** | **A pre-work item bound to this sub-task as "CD-3" was already landed.** The brief described the **SKIP-clause full-checkout dependency** comment; that content shipped in `bae6ea6f` at `deploy.sh` `_c32_verdict_exit_code` | Verified against the branch before acting: **1** occurrence of `sparse-checkout` in `deploy.sh`, introduced by that commit; the consuming workflow takes a bare `actions/checkout@v7` with no `sparse-checkout:` and no `filter:` | **NOT DUPLICATED** — a second copy of that reasoning is precisely the drift the authoring-home discipline exists to prevent. The A6.5 counter-design finding **actually numbered CD-3** (one derivation sentence for how a consumer disposes of `NOT-EVALUATED`) was *not* landed and **is** what this release's new `3)` arm makes load-bearing, so that is what was implemented instead |
| **DEV-11** | **Implementing CD-3 at the convention's declared authoring home adds `core/deploy/deploy.sh` to this card's change matrix**, which the Stage-5 File Change Matrix did not list for #7440 | Flagged rather than silently taken | **ACCEPTED, COMMENT-ONLY.** Two paragraphs, zero behaviour change; `bash -n` clean, and no `deploy.sh` check executes the amended tool (swept: **7** references to it under `core/deploy/`, of which **2** are these new comment lines and **none** is an invocation) |
