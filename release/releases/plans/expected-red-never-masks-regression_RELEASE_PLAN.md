...
| #7201 | AC-2 — the runtime log note's **intent** is preserved: the note undisturbed, **except where a sibling card in this release invalidated its premise** | Grade against the **amended** criterion on #7201, not the superseded *"the runtime log note is unchanged"* literal; read the diff scope, then clause-check the corrected sentence against the partitioned behaviour | Exactly one sentence changed, plus the two clauses of the comment block immediately above it that the edit itself falsified, declared as a scope note rather than folded in silently; `git show --stat f80c00e1` = **1 file, 32 insertions, 5 deletions**, nothing else in the log touched. Every surviving clause true after the partition, and the falsified *"stay red"* claim absent from live output. **Grading against the superseded literal would produce a false finding here** `[S6 #7449 c4 — the DEV-13 Tier 1 [ADJUST] record · S7 #7452 → PASS]` |
