...
release/skills/pmo-release-manager/SKILL.md                              edit
release/skills/pmo-release-manager/references/composition-contract-release-manager.md  edit
packages/release-executor.skill                                          edit
packages/release-executor.skill.sha256                                   edit
packages/pmo-release-manager.skill                                       edit
packages/pmo-release-manager.skill.sha256                                edit
...
| `release/references/standards/decision-outcome-tracking.md` | `FIX-OUTCOMEKEY` | New permitted-key-forms subsection. The frontmatter `version:` bump and Version History row this row originally also called for were **deliberately not made** — see § Deviation Log **DV-18**; the bump is sequenced into the Stage-13 chore PR, once the tag is claimed. |
...
| `core/deploy/deploy.sh` | `BLD-OUTPUTSET` | **Moved from READ to edit at Stage 7** to match the as-built (DV-22). Comments only, zero behaviour change: the `cc_telemetry_cutoff` assignment is promoted to a documented cross-tool READ contract, because the close-out resolves that value out of this file rather than keeping a second copy. |
| `release/skills/pmo-release-manager/SKILL.md` + its `references/composition-contract-release-manager.md` | `FIX-PREFLIGHT` | **Added at Stage 7** to match the as-built (DV-21). The downstream half of the struck pre-flight-tag claim, reconciled across five sites. **Fires the package-rebuild obligation** for this skill too. |

**Package-rebuild obligation (fires; do not skip).** `FIX-PREFLIGHT` edits a rostered skill's `SKILL.md` **and** a file under its `references/`, so the `.skill` package and its `.sha256` content baseline must be rebuilt via the package builder and committed **in this PR**. The `skill-package-freshness` CI gate enforces this pre-merge; a stale package cannot merge green. That obligation is the only reason the `packages/` rows appear in the matrix. It fires **twice**: the same rule reaches `pmo-release-manager` once the downstream tag-claim reconcile (DV-21) edits that skill, so its package and `.sha256` are rebuilt and declared on the same footing. All four rows are **`edit`**, not `add`: every package and `.sha256` baseline already exists on `main` for these rostered skills, so a rebuild REPLACES tracked content rather than introducing a new path.
...
| **DV-13** | **`core/deploy/deploy.sh` is added to the file change matrix** under the close-out-completeness card (comments only, zero behaviour change). | The operator-rendered P-1 wire-through. The `required-if` predicate's only source of truth is that file's `cc_telemetry_cutoff` default, which the evaluating process could not see — the token occurs **0** times in `automated-closeout.sh` against **28** in `deploy.sh`. Without the seam the predicate reads false, grades the row `N/A`, and `N/A` is a **satisfied** state, so a required output is recorded satisfied because its membership test could not run. The assignment is promoted to a documented cross-tool read contract; the close-out READS it rather than keeping a second copy, so arming the cutover stays a one-value change in one place. |
| **DV-14** | **`release/skills/release-executor/SKILL.md` and its `.skill` package are added to the matrix** under the same card — one additive bullet in Mode D Step 3. | The Stage-5 Blast Radius marked Mode D **Action** ("Mode D's dry-run review surface should render the output-set verdict") while the File Change Matrix omitted it. Resolved toward the Blast Radius rather than away from it: Mode D **is** the operator's dry-run approval gate, and that gate's visibility is the entire warrant for shipping the assert as a live dry-run phase. Downgrading the cell instead would have paid for a net-new phase and then withheld its benefit. |
| **DV-15** | **The per-member report line's delimiter is changed from `\|` to ` / ` within a row and ` · ` between rows.** | The Stage-5 design specified a pipe-delimited line for a phase whose only reporting channel is the phase record — where `\|` is simultaneously `get_phase`'s field separator and the markdown report table's column separator. The file documents that constraint twice in its own comments. The design also asserted the phase "renders additively with no renderer change"; both statements could not hold, and changing the delimiter is the one that costs nothing. |
| **DV-16** | **The Phase-9.56 manifest is bounded to the PRE-COMMIT-ASSERTABLE subset** of the Step-4 output set, stated explicitly in the function header. | The design said "each manifest member" without bounding the set. Most Step-4 rows name post-merge, on-main facts — the annotated tag, the closed milestone, the published Release — that do not exist at 9.56 and cannot be asserted there without asserting a falsehood. The bounded set is exactly the three members lane 1 historically did not carry, which is why they are the three that went missing silently. |
| **DV-17** | **Verification rows 7-9 are RENDERED from a Phase-9.56 record global, not recomputed** in `phase_run_verification`; and the `**Not-produced:**` marker's insert anchor is **declared** as the line immediately after `**Result:**`. | Two points the design left unspecified. The render follows the shipped row-6 discipline for the same stated reason — that phase runs after the milestone close, so a value re-derived there is a verdict the close was never gated on. The anchor is declared because the block's field order is asserted by exact string equality in the self-test, so an undeclared insert point would shift fields under those literals; `**Result:**` is also the line the surface resolver keys on, so the anchor exists whenever the target resolved at all. |
| **DV-18** | **`FIX-OUTCOMEKEY`'s frontmatter `version:` bump and Version History row on `decision-outcome-tracking.md` were deliberately NOT made.** The § File Change Matrix note that called for them is corrected to say so; the bump is sequenced into the Stage-13 chore PR. | A deliberate non-action, not an omission. The plan binds its version as `{{RELEASE_VERSION}}` and the concrete number binds only at the Stage-12 atomic claim (ADR-092), so at authoring time **no correct value existed**. The token itself is not a substitute: `claim-version.sh`'s cross-grammar pre-flight refuses a `{{RELEASE_VERSION}}` token on a `version:` line, and the refusal is not scoped to patch-form tags — it fires on a minor release too. Stamping an unclaimed tag would have written a value that names a release with no validated baseline. |
| **DV-19** | **`FIX-PREFLIGHT`'s pre-flight condition set is corrected, not merely relocated.** One member — the annotated tag — is **struck** as a phantom that was never a pre-flight condition; two conditions that previously had no class are named (a close-out invoked from the primary checkout, and a milestone slug matching other than exactly one ledger row). | Sub-check (e) records `STATE_TAG_EXISTS` and has no `else`, no `mark_phase … FAIL` and no non-zero return; the real gate is a **remote** `ls-remote --tags` check thirty dispatch positions later at the publish phase. Carrying the tag forward with a halt-class attached would have upgraded a false member into an instruction to hand-assemble a close the script would have run cleanly. Routing the enumeration through the script's `--help` roster was considered and **declined**, which left the prose as the only statement of the condition set — so it had to be correct standing alone, and the phantom member plus the two unclassified conditions are corrected here rather than deferred. |
| **DV-20** | **The `release-executor` frontmatter `version:` was bumped to this release's tag during `FIX-PREFLIGHT`, then REVERTED to `v4.10`** in a follow-up commit on this branch. No skill takes a version bump in this release. | Same category error as DV-18, caught after the fact rather than before. `core/standards/version-field-semantics.md` § Definition defines that field as the **editor-managed skill version, not a release tag**, and `claim-version.sh` encodes the same distinction as an executable refusal. The Mode D edits **are** material under § Bump Rules — the materiality judgement is not being re-litigated; what was wrong was the VALUE, not the decision to move it. Restored to the value its five peer skills read today. Whether the suite takes a bump once the tag ships is a release-level call left to the hub. |
| **DV-21** | **`pmo-release-manager`'s `SKILL.md`, its `references/composition-contract-release-manager.md`, and its rebuilt `.skill` + `.sha256` are added to the file change matrix** under `FIX-PREFLIGHT` — four surfaces the matrix did not declare and the branch had already delivered. Added at Stage 7. | The downstream half of DV-19. `FIX-PREFLIGHT` struck the pre-flight-tag claim from the corpus and recorded that two further sites still carried it as **out of scope rather than edited silently**; a follow-up commit on this branch then edited them. The reconcile covered **five** sites, not the two the card named — correcting only the two would have left the mode table, the FM-3 Conditional and the FM-3 Mitigation asserting the opposite three paragraphs away, and a self-contradicting skill file is worse than a uniformly stale one. Editing a rostered skill fires the package-rebuild obligation, which is why the two `packages/` rows come with it. |
| **DV-22** | **`core/deploy/deploy.sh` moves from READ to `edit` in the file change matrix.** Reconciles DV-13, which already asserted the file "is added to the file change matrix" while the matrix still classified it read-only and excluded from the delivery obligation set. Corrected at Stage 7. | DV-13's assertion was right about the intent and the matrix was never updated to match, so the plan declared a delivered file as an untouched input. The edit is comments only, zero behaviour change: the `cc_telemetry_cutoff` assignment is promoted to a documented cross-tool READ contract that the close-out resolves out of this file rather than copying. A file the release commits is a delivery obligation regardless of how small the change is — declaring it READ under-declared the delivered set. |
