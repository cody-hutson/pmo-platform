...
| #6355 | `core/ADRs/ADR-198-frontmatter-laxity-is-constrained-at-the-consumer.md` | add | The D-12 decision record, authored by Engineering at C2. **This is the release's first `add` row**, which falsifies the Stage-4 line *"No `add` rows and no `delete` rows in this release"* — recorded as **DEV-6** below rather than papered over. The new-executable companion obligation still does **not** fire: the obligation is `*.sh`-scoped and this is a markdown record. **Module corrected to `core/` at Stage 7 — see DEV-16**; this row was declared against `release/ADRs/` at Commit 0 and the record was filed, correctly, under `core/ADRs/`. |
...
| #6174 | AC-4 | `bash core/deploy/tests/test_g1_form_family.sh` at the release merge-base and at the head; PASS rows compared with digits normalised | **0 failed, and all 32 intake-time rows retained and passing.** The total is not the criterion: it is 32 plus the rows this card adds to arm L, so it moves as arm L grows (38 at Stage 8, 41 after remediation 6). Reinterpreted reading, per DEV-18 | **regression floor, verified by a retention probe:** the merge-base's PASS rows must all appear at the head (0 retired). Sensitivity arm: renaming one intake-time row in a scratch copy reports 1 retired while the suite itself still prints 0 failed, which is why the count alone cannot carry this criterion |
...
| #5707 | AC-2 | demonstrate the false red before, absent after | before (variable unset): red on conforming content — exit 1 and a false *does not resolve* finding; after, variable unset: no content verdict — exit 3, a refusal naming `GITHUB_REPOSITORY`, zero findings (the refusal AC-1 sanctions, not a green); after, variable set: green, exit 0 | the AC *is* the discrimination requirement |
...
| #6242 | AC-4 | re-scan `release/tools/tests/**` for the pipeline-into-early-reader shape | count re-derived, not inherited | **re-derived at `a3083858`: 1 line in 1 of 19 shell sources** (99 tracked files), this card's own line; **0 after the fix `a07e5006`**. The issue's inherited *1 of 76* does not hold at its own filing commit `1efa872a`: 79 tracked files there, 18 of them shell sources, and the class was **7 lines in 2 files**, this card's line plus six piped `if printf … \| grep -q…` predicates in `test_spoke_run_directory.sh`, which a sibling here-string fix removed on 2026-09-02. So the numerator did not hold at 1; it fell from 2 files to 1 before this card changed anything. Re-measured per DEV-19. Control arm fired at 6 of 65 over `core/deploy/tests/**` — full probe record in § Evidence |
...
| **DEV-16** | **The D-12 decision record is filed under `core/ADRs/`, not the `release/ADRs/` path the Stage-5 additions matrix declared.** The row above declared `release/ADRs/ADR-<frontmatter-laxity-is-constrained-at-the-consumer>.md`; the delivered file is `core/ADRs/ADR-198-frontmatter-laxity-is-constrained-at-the-consumer.md`. **The relocation is correct and the plan was the thing out of date** — the record governs a core deploy tool with six core consumers, carries no release-pipeline mechanics, and validates against a grammar that also lives in core. | `release/ADRs/README.md` § Scope, in terms: *"Decisions that apply platform-wide (cross-module governance, cardinality models, PMBOK alignment, module boundary definitions, tooling design) live in `core/ADRs/`"*. The ADR number sequence is global across both directories, so the module choice does not move the number. | **RECONCILED AT STAGE 7, not at Commit 0 where it should have been.** PR #7404 carried the deviation as D-9 and the Commit-0 output comment asserted every deviation was transcribed into this log; this one was not, so the plan — the surface Stages 8, 9, 12 and 13 all read — declared an `add` the release did not deliver, and `verify-release-plan.sh` reported `FCM-1 | fcm-delivery | … | declared-add-not-delivered` FAIL. The matrix row is corrected to the delivered path and this row records the delta. **The ADR itself is not moved.** |
| **DEV-17** | **#5707's AC-2 verification row expected an outcome the ratified refusal contract cannot produce.** As transcribed from Stage 4, the row's Expected Result read *"before: red on conforming content; after: green"*. With `GITHUB_REPOSITORY` unset the shipped gate refuses by design — exit 3, the variable named, zero findings — and the AC-1 row directly above permits exactly that refusal, so the two rows disagreed. Per the Stage 8 review, Stages 6 and 7 both graded AC-2 against the refusal without naming the departure. The row now states the outcome the contract produces: the false red before; with the variable unset after, a refusal rather than a content verdict; with it set, green. | Stage 8 acceptance review of #5707, finding N-3 (Minor; Stage-7 escape E-2), recorded on sub-task #7378; reconciled in Stage 6 remediation 5 | **Reconciled in this plan.** No acceptance criterion moves: AC-2's own text is unchanged, and it was already graded on this reading — the refusal is the loud, machine-distinguishable outcome the issue's Expected Behavior asks for. Only this plan's restatement of the expected result was wrong, and it is the surface Stages 9, 12 and 13 read. |
| **DEV-18** | **#6174's AC-4 is graded on a reinterpreted reading, and its verification row now states that reading.** The criterion reads *"remains 32 passed / 0 failed"*. 32 was the suite's size at intake, and this card's own AC-3 arms were added to the same suite, so the total moved: 38 at Stage 8, 41 after remediation 6. The row's Expected Result still read `32 passed, 0 failed`, and its last column is the **discrimination method**, not a disposition, so no stage had recorded the re-mapping. Two upstream records are corrected here. Stage 6 said the criteria are jointly satisfiable only under the floor reading; they are not, since the new arms could have lived in a separate file, which would also have left the literal 32 standing. Remediation 4 called the last column the disposition; it is the discrimination method. The floor reading is right, but it was chosen, not forced. | Stage 8 acceptance review of #6174, AC-4 graded REINTERPRET-WITH-RATIONALE with a Tier 1 [ADJUST] to its wording, recorded on sub-task #7384; applied in Stage 6 remediation 6 | **Reconciled in this plan. The criterion is not edited.** The row now states the reading Stage 8 graded, *0 failed with all 32 intake-time rows retained and passing*, and the retention probe that verifies it. At `73557171` the merge-base (`130f3a92`) and current `main` each give 32 passed / 0 failed with the same row set, and the head gives 41 passed / 0 failed: 0 retired, all 32 intake-time rows present byte-identical, 9 added, all in arm L. The probe's sensitivity arm is also the reason the total cannot serve as the criterion. Renaming one intake-time row leaves the suite at 41 passed / 0 failed, and the probe reports 1 retired. |
| **DEV-19** | **#6242's AC-4 verification row carried an inherited figure that is false at its own commit.** The row read *"issue measured 1 of 76 at its filing commit. Denominator grew 76→99; numerator held at 1."* Re-measured at the filing commit `1efa872a`, `release/tools/tests/` holds **79** tracked files (61 under `fixtures/`, 18 top-level), **18** of them shell sources, and no natural count there gives 76: recursive 79, top-level entries 19, top-level files 18, `.sh` plus `.py` 21, `test_*` 15. The class there was **7 lines in 2 files**: this card's line 66, and six `if printf … \| grep -q…` predicates in `test_spoke_run_directory.sh` (lines 95, 100, 108, 113, 136 and 167). A sibling fix converted those six to here-strings on 2026-09-02, in `37d3201d`: six such lines at its parent, none after. So the numerator did not hold at 1. It was 2 files at filing, 1 at the plan baseline `a3083858` (99 tracked, 19 shell sources) before this card changed anything, and 0 after the fix `a07e5006`. The row's *"1 of 99"* also mixed denominators: 99 counts every tracked file, while the shape this card fixes runs only in the 19 shell sources. | Stage 8 acceptance review of #6242, finding E-1 (Minor; Stage-7 escape), recorded on sub-task #7385; re-measured independently and reconciled in Stage 6 remediation 6 | **Reconciled in this plan; no verdict moves.** AC-4 asks for the *current* scope claim to be re-derived, and it was, correctly: 1 at the baseline and 0 after the fix. Only the inherited history was wrong. **What these figures are bounded to:** shell sources under `release/tools/tests/`, identified by a `.sh` or `.bash` extension or a shell shebang, with backslash continuations joined. A line counts when it pipes into `grep` with `-q`, `--quiet`, `--silent` or `-m`, or into `head`, outside a comment and outside `$( )` or backticks, and the pipeline's status is the decision: the line opens with `if`, `elif`, `while`, `until`, `!`, `&&` or `\|\|`, or the pipeline is followed by `&&` or `\|\|`. A leading-keyword classifier and a status-consumption classifier agree at the filing commit, the baseline, the fix and the current head. Pipes into `sed … q`, `awk … exit` or `read` add no lines at any of those four. **Residual, not sampled:** a reader named through a variable or in quotes, and a `while read … break` loop. |
| **DEV-20** | **The Reversibility section's self-announcing claim is false for #5556, and it was stated release-wide.** The transcribed line read *"The revert is self-announcing: the new self-test cases fail on reverted code, so a partial revert turns `selftest-discovery` red rather than silently restoring the defect."* #5556 added **no** new self-test case, and `selftest-discovery` invokes each discovered tool by **absolute path** — the one addressing form in which this card's defect never showed. Re-measured independently here: the pre-#5556 blob from `origin/main` (`e3006812…`) run through `check-selftest-coverage.py --run --only` gives `self-test: PASS`, `OK:`, `ARM A PASSED: 1 of 1`, exit 0. `core/deploy/tools/README.md` names that runner this tool's sole automated caller, and a grep of `.github/workflows/` for the tool returns nothing. So for this card the green runner was consistent with the defect being back. | Stage 8 acceptance review of #5556, finding S8-02 (Minor; Stage-7 escape, origin S4/S6), recorded on sub-task #7386; re-measured and reconciled in Stage 6 remediation 7 | **Reconciled in this plan; the claim is replaced, not narrowed to a footnote.** The section now states what does and does not announce a revert, for #5556 only. Remediation 7 also changed the fact underneath it: self-test arm C1 re-invokes this file by a bare relative path under an exported `CDPATH`, an environment it builds itself, so the property is now reachable from the absolute invocation CI uses. Measured after the change: the pre-remediation one-liner, the same line with `CDPATH=''` dropped, and the original `$0` self-invocations restored at their original sites each turn the runner **red**; silencing the `cd` with `>/dev/null` instead of disabling CDPATH also turns it red, on the decoy conjunct. **Two residuals, both named rather than closed:** a revert that removes arm C1 as well restores the defect silently, and recombining the two assignments while keeping the guard stays green, because C1 tests CDPATH-safety and not `set -e` coverage. Nothing was measured for the other five Slice-B cards, and the corrected line says so. |
...
**CHEAP / Confidence HIGH** for Slice B (#5707 · #6251 · #6111 · #6174 · #6242 · #5556) — six textually independent cards, zero shared files, single-commit revert applies cleanly. **MODERATE / Confidence HIGH** for Slice A (#6355 · #5817 · #5832) — three cards entangled across `_frontmatter.py`, `compose-portfolio.py` and `stamp-node-frontmatter.py`; granularity is the whole merge, and the plan states that rather than implying finer granularity than it has. No schema change, no data migration, no host-side state, no published artifact mutated. **What announces a revert is per-card, and only #5556's was measured** (DEV-20). `selftest-discovery` invokes every discovered tool by ABSOLUTE path, which is the one form in which #5556's defect is invisible: the pre-#5556 blob (`e3006812…`) run through that runner gives `self-test: PASS`, `ARM A PASSED: 1 of 1`, exit 0, so a restored defect did **not** turn the runner red. Remediation 7 closed that for the resolution itself — self-test arm C1 builds the hostile addressing environment inside the tool, so the runner now goes red on a revert of the resolution lines alone (measured on three: the pre-remediation one-liner, the same line with the `CDPATH=''` guard dropped, and the original `$0` self-invocations restored at their original sites). **A revert that also removes arm C1 restores the defect silently** — nothing outside this file tests it. This line asserts nothing about the other five Slice-B cards: their detection is stated per card in § Verification Evidence, and no CI-reachability measurement of the kind above was run for them.
