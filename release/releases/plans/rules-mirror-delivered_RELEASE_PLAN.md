...
| #4738 | AC-3 | `grep -q 'absent from holder' core/deploy/deploy.sh` | The divergence message names both the path and the holder it is missing from. The message is rendered by the check wrapper, which is where the union-vs-holder difference is turned into prose; the primitive emits the machine-readable `MISSING` rows the wrapper renders |
...
| #4739 | AC-3 | `grep -cF '](../' core/rules/git-workflow.md` | Zero relative cross-tree links remain in this retained corpus member · control: the same fixed-string instrument on `core/rules/doc-link-maintenance.md` before the rewrite returns non-zero (baseline at the pin: 7 here, 2 there), so a zero is a real removal rather than an unread file. Fixed-string form deliberately — a regex form carrying backslash escapes does not survive transport through a markdown table cell |
...
| #4739 | AC-5 | `python3 core/deploy/tools/check-doc-links.py --target-paths 'core/rules/*.md' --output-format tsv \| grep -cv 'source_file'` | Zero — no finding rows beyond the TSV header, i.e. no unresolved internal cross-reference across the rule corpus · control: the same pipeline with `grep -c 'source_file'` returns 1, so the zero is a read that happened rather than an empty stream. A target glob is required; without one the primitive refuses rather than scanning nothing. Check 14 invokes this same primitive inside the lifecycle battery — same engine, no network |
