...
source_observations:
  - "The threshold read one lower than it was written. Every *deployable*-class release writes a mandatory `RELEASE_LOG` row at Stage 12 and transitions it at Stage 13, so membership handed every honest File Change Matrix one free touch and the `≥3` threshold behaved as `≥2` in practice."
...
**Cutover, and the reflexive loop.** The new reading applies to releases classified at Stage 3 strictly after this note's introducing-release merge SHA recorded in the release log; pre-cutover classifications are grandfathered, and **the introducing release itself is exempt**. This is not ceremony: the release carrying this change was classified under the old reading, will write its own mandatory ledger row at Stage 12, and a literal post-close re-count under the old seven-member set would then read 3 against the 2 its rationale records. The class would not change — another trigger already fires — so the defect would be one of **rationale integrity** rather than misclassification. Under the new reading the re-count reads 2 and the recorded rationale stays true. A release cannot fire its own new classification rules on its own planning, and the exemption clause is what makes that explicit rather than accidental.
