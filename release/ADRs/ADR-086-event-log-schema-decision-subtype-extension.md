...
**The event-log schema is extended in place, additively, on both axes; § 3 remains the single source of truth and the tool's static list remains a strict mirror of it.**
...
2. **The tool's static fallback mirror is updated in the same commit**, in the same relative position, preserving the documented § 3 ↔ fallback lockstep invariant. The mirror carries no independent authority — where the two could disagree, § 3 wins, because the runtime parse takes precedence whenever the schema file is readable.
...
- The § 3 ↔ fallback lockstep is a standing two-site obligation. Any future subtype or event-type addition must touch both sites in one commit; the fallback is a mirror, never a second source of truth.
