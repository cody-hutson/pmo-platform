...
- **[ADR-146](ADR-146-dry-run-predicts-apply-asserts-mode-branch-placement.md)** — dry-run predicts, apply asserts. A sibling in this release on the same tool, and the same underlying posture in a different register: a convention that lives only as comments gets rediscovered one instance at a time. Its paired-arm discipline is the reason this decision's fixture bindings each carry a vacuity floor — a check that passes on the defective input is worse than no check.
