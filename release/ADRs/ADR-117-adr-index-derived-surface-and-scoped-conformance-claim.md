...
**Unchanged, deliberately.** The corpus is not fully conformant to the section set after this release and this record does not claim otherwise. Sections whose backfill would be authoring rather than recording remain out of scope with their blocking authority named, and one frozen record remains permanently non-conformant. The residual in Decision (5) is the honest statement of what that leaves open. **That residual now has a tracked owner rather than a remembered one** — the section-backfill card recorded in § References below, filed at this release's Stage-9 gate and carrying the per-section sizing re-derived through the durability lint's own section-set predicate. It was sized before and named in prose here, but no open card held it; a residual that is disclosed and un-owned is indistinguishable, a release later, from one that was quietly dropped.
...
**The declaration obligation shipped with no conformant instance anywhere, its own exemplar included.** Alongside the ratchet, this release obliges a marker declared under the narrow criterion to carry a trailing rationale **on the same line, inside the comment**, naming which limb applies — a bare marker being, in the standard's own words, a silenced warning rather than a declaration. Measured as of this release's Stage-9 gate, with a sensitivity arm on a synthetic declared marker, a specificity arm on a bare one, and the record denominator asserted in the same command that produced the figure: of the **60** marker occurrences the ADR corpus carried across **58** files, **0** carried an inside-the-comment rationale; repo-wide the count was **0 of 185**. The specimen the authoring guide cites as the form *already practiced in the deploy test suite* carries its rationale **outside** the comment close, so it satisfies "trailing rationale on the same line" and not "inside the comment" — **the exemplar does not pass the rule it exemplifies** — and no grandfather clause exists in the guide, in the core module's ADR README, or in the repository-integrity workflow. This is stated rather than repaired, and the reason is the same one that keeps the section backfill out of scope above: retrofitting the existing markers is the identical corpus sweep as retiring them, and widening the obligation or grandfathering the population would be a change to the standard rather than hygiene against it. Both belong to the sweep and should be graded on the sweep. Recording it here matters because a standard whose entire live population is non-conformant on the day it ships reads, to anyone measuring later, exactly like a standard that regressed.
...
- #4758 — the canonical-section backfill card for the records this release's sweep deliberately did not author into; it owns the residual named in Consequences and carries the per-section sizing.
