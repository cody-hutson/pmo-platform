...
source_observations:
  ...
  - "CORRECTED at Stage 7 — the case analysis was not exhaustive. It read: an exhausted pool presents as a SUCCESSFUL read returning zero remaining, never as a probe failure. Exhaustion has TWO successful-read presentations: (a) a depleted window reporting near-zero remaining, which the axis handles correctly by rendering DEFER; and (b) an UNSTARTED window reporting a full pool with used = 0, on which the axis renders PROCEED — into the exact harm it exists to prevent. A Stage-7 reviewer independently observed graphql 5000/5000 reported while GraphQL calls were being rejected. What survives unchanged is the narrower claim the fail-open decision actually rests on: a probe FAILURE evidences an instrument or authentication fault and carries no exhaustion signal. What is added is that a probe SUCCESS can also carry no signal."
...
Adding the axis is straightforward. What is not straightforward is the property it drags in behind it. **The host-API axis has an instrument and the usage-window axis has none.** The rate-limit endpoint returns each pool's remaining and limit.

> **Stage-7 evidence correction.** This section originally added that the endpoint is *itself unmetered, so the probe does not draw against the pools it reports* — recorded as verified rather than assumed. **It was not verified.** The supporting observation (a `used` counter that stayed at zero across repeated reads) is confounded by a presentation discovered at Stage 7: some reads return an **unstarted window**, which reports `used = 0` regardless of whether the probe draws. That claim is therefore **UNVERIFIED** and is not used as evidence anywhere below. The same re-measurement showed the instrument is **non-deterministic** — 12 reads inside 3.6 seconds gave 6 distinct `reset` epochs per pool and two different `remaining` figures, against a local control that returned exactly one distinct value. Having an instrument is still more than the usage-window axis has, and is still why this axis exists; it is **not** enough to grade its figures `[SOURCE]`. See `source_observations` 2 and 3.
...
2. **Exhaustion on the new axis never presents as a probe failure.** A depleted pool returns a *successful* read rather than an error. A probe that *fails* is therefore reporting an instrument or authentication fault, and carries no capacity signal whatsoever. **Corrected at Stage 7:** the converse does not hold — a probe that *succeeds* does not thereby carry a capacity signal either, because exhaustion has a second successful-read presentation (an **unstarted window**: a full pool with `used = 0`) on which the axis renders PROCEED into the harm it exists to prevent. The fail-open decision below rests only on the first claim, which stands; the second is a residual named rather than solved.
...
- **Per-axis evidence grades.** A host-API figure is a reading and a usage-window figure is a projection; the two kinds are never conflated, never borrowed across axes, and **the two axes are never averaged.** A number blending them is neither, and destroys both — which is why the axes combine by disjunction rather than arithmetic. **Corrected at Stage 7 — the grades themselves:** this bullet originally licensed labelling a host-API figure `[SOURCE]`. That grant is **withdrawn**. A non-deterministic instrument does not yield `[SOURCE]`, so **both** axes now render `[ASSUMPTION – CONFIRM]` — for different reasons (no instrument vs. an unreliable one). The disjunction rule is unaffected: it never depended on the grades differing.
...
- **A probe failure fails OPEN, on evidentiary grounds and not convenience.** Exhaustion cannot present as a probe failure, so a probe failure carries no exhaustion signal. Failing closed there would deadlock the pipeline on a fault that says nothing about capacity. The basis is recorded as unstated, the axis renders PROCEED, and the reason is **rendered visibly** rather than swallowed. **Stage-7 addendum — the residual is wider than this decision assumed.** A *successful* read of an unstarted window also carries no exhaustion signal, and unlike a failed probe it does not announce itself: the axis renders PROCEED on a green-looking figure. The decision stands (failing closed still deadlocks on a signal-free fault), but the blind-launch residual it accepts now covers both branches, and is discharged by the write-early discipline on both.
...
| **Estimate pool draw** from a count of host calls | Rejected — **basis corrected at Stage 7** | Original basis (*measured: repeated identical reads consumed nothing*) rested on the confounded zero-`used` observation and is **withdrawn**. The rejection stands on independent ground: pool draw is set by what the *work* requests, and the pools are drained concurrently by sessions the hub cannot observe, so a hub-side call count is not the denominator either way. |
...
**One call spends one axis to read the other.** Whether the probe draws against the pools it measures is **unverified** (see the Context correction), but it is certainly a tool call, so it certainly draws against the usage window. That cost is **cross-axis** and is stated in the protocol rather than hidden. It is bounded by reading once per routing turn and reusing the result for every launch issued in that turn — a bound that holds on either answer to the unverified question, which is why the correction does not disturb this consequence.
