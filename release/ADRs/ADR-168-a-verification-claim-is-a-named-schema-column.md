...
**A pre-existing collision on this branch was recorded and then resolved, not silently stepped over.** At this ADR's authoring time `--detect` reported `core/ADRs/ADR-165-bounded-by-relocation-not-by-discard.md` as `DUPLICATE MAINLINE`, because mainline already carried `ADR-165-post-merge-skill-deploy-is-a-local-git-hook.md`. That record belongs to another card, so it was surfaced from here rather than adjudicated here — and the collision was then resolved deliberately on this branch by renumbering **the branch's** record to **ADR-169**. Mainline's `ADR-165-post-merge-skill-deploy-is-a-local-git-hook.md` keeps `165` and is untouched. No collision remains: mainline carries no `ADR-169`.
