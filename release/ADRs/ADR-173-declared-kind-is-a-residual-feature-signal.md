...
- **ADR-170** — the sibling record establishing that an unrecognized verdict class is a finding rather than an absence. The same instinct governs the degrade path here: an unavailable kind tier is announced, never silently absorbed into a quieter bucket.
