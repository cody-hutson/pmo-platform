...
| Could not read | ERROR | The executor could not read or evaluate the row | Yes | `count-unreadable:…`, `stdin-reader:<verb>`, `no-operand:<verb>`, `no-comparator:<verb>`, `method-cell-empty` |
...
- #6236 — the card whose Decision lines join this record from its own slice (D47).
