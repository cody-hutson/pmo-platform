...
**Pre-reserving above a sibling's claim is not the remedy, and the asymmetry is the whole argument.** The contiguity gate fails a *gap* exactly as readily as a duplicate, and it fails it worse: a branch that steps over an unmerged claim and then merges first lands a hole on `main`, where the gate then fails **every** subsequent pull request until someone fills it. A duplicate inconveniences one branch; a gap blocks the repository. Allocating at the mainline next-free slot is safe under **every** merge order; allocating above it is safe under exactly one.

**Merge-order contingency (live at authoring).** The `adr-corpus-conformance` release holds unmerged claims on **110, 111 and 112**; two further release branches hold **112 and 113**, and **114**. This record and its sibling therefore sit in genuinely contended slots. That contention is recorded, not mitigated: **whichever release merges second renumbers**, via the governed renumber path (rename, branch-scoped citation sweep, index-surface and renumber-log update, a `## Status` provenance note, and a zero-dangling verify). The move is cheap by construction — an unmerged record has no mainline citations, so its inbound references are confined to its own branch diff.
