...
1. **Input collection:**
   ...
   - Confirm the Phase-A7 `release-synthesis/learnings-triple` row for this release exists BEFORE invoking the script: `release/tools/query-pipeline-event.sh --release <milestone-slug> --event-type release-synthesis --event-subtype learnings-triple`. The release join key is the milestone SLUG, not the version — a raw `--version v<X.Y>` filter returns zero rows with exit 0 for any slug-keyed release and would report a false absence. If the row is missing, capture it with one `release/tools/append-pipeline-event.sh` invocation before proceeding. The script asserts the same condition at Phase 2 pre-flight and hard-exits (exit 2) at `--apply` when it is absent, so capturing it here costs one command while capturing it later costs a re-run. This is a missing **input**, not a not-yet-ready state: the `stage-13-close.md` Phase A8 hand-assembly fallback does NOT apply to it, because hand-assembling the close would produce by hand exactly the unevidenced learnings block phase 6.7 exists to refuse.
...
2. **Pre-flight (delegates to script `preflight()`):**
   ...
   - Script verifies: gh auth, clean working tree, worktree cwd (not primary per `git-workflow.md` § Primary Checkout Discipline), Stage 12 chore PR landed (RELEASE_LOG row exists with DEPLOYED state per the chore-PR protocol), the milestone slug resolving to exactly one RELEASE_LOG row, no unfilled scaffold token in this version's release note, and the Phase-A7 `release-synthesis/learnings-triple` row captured
   - The script also RECORDS whether the annotated version tag exists, and that is a report field rather than a gate: a tag-absent state at dry-run time is normal, because the tag is pushed at Stage 12 Phase B3, which has not run when the operator dry-runs the close. The tag blocks much later, at the GitHub-Release publish phase, on a different exit code. Do not read a tag-absent pre-flight row as a halt reason.
...
3. **Dry-run review:**
   - Present the dry-run report to the operator covering: the issue-closure audit verdict from Step 2.5 (clean, or N open with the enumerated milestone-issue list), the Phase-2 pre-flight verdict INCLUDING any `WARN` row — a missing Phase-A7 `release-synthesis/learnings-triple` row marks pre-flight `WARN` under `--dry-run` rather than aborting the preview, and the same condition FAILs the close at `--apply`, so a `WARN` here is a blocking condition the operator must clear before approving apply, not an advisory to scroll past — planned diffs (RELEASE_LOG DEPLOYED → VERIFIED transition, RELEASE_INDEX append, RELEASE_DIGEST append, RELEASE_NOTES scaffold per `release-notes-standard.md` Part 1 Template, Phase 9.5 CHANGELOG.md prepend per Layer-1 dual-write Surface 2, Phase 15.5 `gh release create | edit` Surface 1 emit per Layer-1 dual-write — see `automated-closeout.sh` Phase 9.5 + Phase 15.5 for the canonical pattern), planned chore PR title/body/metadata (with parser-clean discipline check per the N=2 confirmed parser-clean pattern), planned Milestone close, planned manual-close list (if Phase 4 auto-close anomaly detection finds open release issues), the carry-forward / deferred-item disposition (the Deferred items summary the script writes into the chore PR body — none on a clean close, otherwise the enumerated deferred list), planned verification commands per `hub-spoke-bridge.md` Procedure 7 Step 4, planned orphan-cleanup invocation.
   - **Surface the Phase-9.56 output-set completeness verdict.** The dry-run report carries a per-member line for the close-out output-set manifest — each member's id, its `Req` flag (`required` / `required-if <predicate>` / `optional`), and whether it WOULD be present in the committed tree. Present it: this is the operator's only pre-approval sight of whether the close will produce its complete output set, and it is the reason the assert exists as a live dry-run phase rather than a post-commit check. Read the verdict the same way as the pre-flight row above — a `WARN` here is a **blocking condition to clear before approving apply**, not an advisory: the phase is non-blocking under `--dry-run` (nothing is committed, so no record can be corrupted) and the same condition FAILs the close at `--apply`. Two states warrant naming explicitly to the operator: a member reported `WOULD-BE-ABSENT` (the close will abort before the chore commit unless the output is produced first), and a member reported `INDETERMINATE` — its membership test could not run, which blocks by design and is repaired at the seam, never by treating the member as not-owed. A `**Not-produced:**` marker shown beside a member is evidence of why it is absent and never an exemption from it; do not read a marker as clearance to approve.
...
5. **Apply:**
   ...
   - On Phase-N FAIL: script halts with structured error; operator decides Retry (re-invoke the script — re-running after a halt is its supported recovery, but the script does not assert that every phase is idempotent; the re-run behaviour its `--self-test` pins is listed in its header, which `--help` prints) / Escalate (Tier 2 [SCOPE CHANGE] per § Inter-Stage Feedback Protocol) / Rollback (operator-only; `git revert <chore-PR-merge-SHA>` reverses the chore PR; `gh api repos/.../milestones/<N> -X PATCH -F state=open` re-opens Milestone)
