...
| 7 | `## Implementation Sequence` | Mode B Step 5 | `### Commit Plan`, `### Stage Applicability Matrix` | Always |
...
| 10 | `## Operator Decisions (D-Gate Block)` | Stage 4 D-Gate | `### D-N: <decision title>` per D-decision with required subsections (Gate input / Pre-decided / Gate decision / Blocks / Upstream compatibility / Reversibility-Confidence / Spoke recommendation) | Always (one H3 per D-decision; `Upstream compatibility` subsection is structurally required) |
| 11 | `## Recommendations` | Mode B Step 8 | (none — numbered list) | Always; each entry must be Stage 6+ chip-prompt-input-shaped (actionable, not "consider X") |
| 12 | `## Verification Evidence` | Stage 13 Close | (none — placeholder until Stage 13) | Always (initially placeholder) |

**Header field rule (row 1) — the Version cell is a machine-read stamp manifest.** The Header's `**Version**` value is the literal `{{RELEASE_VERSION}}` token. Mode B never substitutes a version, a provisional-display label, or determination prose into that cell — the bump-class determination goes in `**Bump Class**`. The token is the stamp manifest `claim-version.sh` resolves at the Stage-12 claim; overwriting it disables the plan's auto-rename and blinds the identity-conformance check. Mode B Step 7 writes the plan; step 3b of the Commit-0 procedure asserts the token mechanically.
...
**D-Concurrency Posture D-row (a recurring entry in the row-10 `## Operator Decisions (D-Gate Block)`).** Mode B emits a `### D-Concurrency Posture` H3 in the D-Gate block declaring the per-release Stage-6 Engineering parallelism posture (per [`parallelism-posture-taxonomy.md`](../../references/standards/parallelism-posture-taxonomy.md)), carrying the same structurally-required subsections every D-row carries. Default **P0 fully-serial** when undeclared. Scaffold:
...
### Version token overwritten by the bump-class determination — OUT

- **Signature (observable signal):** the authored Header carries prose where the
  `{{RELEASE_VERSION}}` token belongs — a bump class, a provisional-display label, or a
  determination narrative in the `**Version**` cell — or the plan omits `## Header`
  entirely. A `grep -cF` for the braced token over the written plan returns 0.
- **Conditional:** do NOT write a determination, a provisional label, or a version
  literal into the `**Version**` cell when authoring a pre-claim plan, because that cell
  is the stamp manifest and its loss is silent at every downstream gate — the Stage-12
  claim still exits 0, the plan is never renamed into `plans/v<MAJOR>/`, and the
  identity-conformance check reports a pre-claim plan as already claimed.
- **Root cause:** the determination had no home of its own, and it is the human-legible
  one, so it took the only version-shaped cell available.
- **Mitigation:** put the determination in `**Bump Class**`; leave `**Version**` as the
  bare token; `claim-version.sh --verify-stamp <slug>` at Commit 0 is the mechanical
  backstop that turns a silent loss into a visible finding before the plan is committed.
- **Principal response vs. junior response:** Principal treats a machine-read cell as an
  interface and records the determination beside it. Junior improves the cell's
  readability and disables the mechanism — and because every downstream gate stays green,
  nothing announces the breakage until the release is claimed under the wrong identity.
