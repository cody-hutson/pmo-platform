...
**Sourcing note.** Data-architecture practice **is** now sourced in this corpus: [`core/standards/domain-best-practices/data.md`](../../../core/standards/domain-best-practices/data.md) ships the data-domain design-consumption guide, and the framework catalog registers the data-domain frameworks it cites. This Specialist's method has **not** yet been re-authored against that guide — that re-authoring is its own follow-on, and it is exactly the three steps this note previously named as unbacked: Mode 1's idempotency and replay-safety step; Mode 2's assertion set, which the guide's named data-quality dimension taxonomy now makes checkable rather than resting on this author's enumeration; and Mode 3's dimensional-modelling step, which the guide's grain-first modeling concept now sources. Until that follow-on lands, state the limitation in any output where the depth of an external framework would change the answer.
