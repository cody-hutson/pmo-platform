...
**Domain and altitude do not appear in this table, and that is the point.** Enterprise altitude and the data domain are *parameters* on Modes 1 and 2 — the same composition edges, the same modes invoked, a different scope. Only a distinct method earns a row here, which is why security has one and the other two dimensions do not ([ADR-127](../../../../core/ADRs/ADR-127-domain-is-a-parameter-of-the-architect-role.md)).
