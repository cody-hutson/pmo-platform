...
|---|---|---|---|
| **Mode 1 — Go/No-Go Evidence (Stage 9)** | [`release-planner` § Mode A — Backlog Analysis](../release-planner/SKILL.md) / [`release-planner` § Mode B — Release Planning](../release-planner/SKILL.md) — read-only dep-graph, plus the Critical Path ([`release-planner` § Mode B — Release Planning](../release-planner/SKILL.md) → its **Step 5c — Critical-path emit (CPM longest-chain)**), and the Risk Register / Cross-Milestone Validation ([`release-planner` § Mode B Output Format](../release-planner/SKILL.md) → the emitted plan's `## Risk Register` and `## Cross-Milestone Dependency Validation` sections) evidence (`release-planner` is read-only by contract — [`release-planner` § Operating Principles](../release-planner/SKILL.md)) **+** [`release-executor` § Mode B — Verify Release](../release-executor/SKILL.md) read-only post-state checks | **Assemble the go/no-go evidence package and render the GO / NO-GO recommendation** — bind plan evidence + verification readiness into a Decision Briefing for the Stage-9 operator gate. Decides *what the evidence says*; does not self-authorize the deploy | **Tier 1 — Recommend.** The briefing mutates no state — the recommendation is read-only; operator GO is the ratification. **CHEAP** |
| **Mode 2 — Deploy Execution (Stage 12)** | [`release-executor` § Mode A — Execute Release](../release-executor/SKILL.md) for the apply, chained with [`release-executor` § Mode F — Publish Release (Layer-1 Dual-Write Surface 1)](../release-executor/SKILL.md) for the Surface-1 emit | **Sequence and authorize the tail's deploy step** — confirm the Stage-9 GO + approved plan w/ Dry-Run Record are present, hand `release-executor` the plan, consume its Quality-Gate Ladder (T1→T2→T3) + execution result, then drive the publish. Does NOT re-run the ladder; does NOT author the plan | **Tier 3 — Autonomous, *bounded by the Stage-9/12 operator gate*** (`release-executor` Mode A self-halts without an approved plan + Dry-Run Record / Stage-9 GO — this Specialist inherits that gate). **MODERATE-to-EXPENSIVE** (see `## Reversibility Discipline`) |
| **Mode 3 — Close-out (Stage 13)** | [`release-executor` § Mode D — Close Release](../release-executor/SKILL.md) — wraps `automated-closeout.sh`; the issue-closure audit is that mode's **Step 2.5** — optionally [`release-executor` § Mode E — Author Release Note](../release-executor/SKILL.md) on the prose-fill branch | **Drive the release tail to its terminal close** — confirm the Stage-12 chore PR landed (DEPLOYED row) + tag exists, surface the Mode D issue-closure audit verdict so the milestone is never closed over open issues, and own the carry-forward disposition decision. Does NOT re-implement the close-out script | **Tier 3 — Autonomous, bounded by Mode D's operator Apply gate.** Overall close-out: **MODERATE / HIGH** (chore PR `git revert`-able; Milestone re-openable) — escalates to **IRREVERSIBLE** once downstream releases consume the VERIFIED row as their baseline |

Every mechanism claim in each mode cites the composed `release-executor` / `release-planner` mode by **section-name anchor** (above) — a `§` segment carrying the target heading's text verbatim, over a plain link to the target file with no `#fragment`. A line-number anchor is never used: every line number in a long file still "resolves" after the target is edited, so a citation that has drifted onto the wrong content is indistinguishable from a correct one by any mechanical means — it fails open. A section name that no longer exists returns zero from a grep, so the same edit that would silently break a line anchor makes a name anchor loudly detectable. Where the referent is not itself a heading — a numbered step, or a section of an artifact the composed skill *emits* — the citation names the **nearest enclosing heading** in the `§` segment and identifies the sub-referent by its own verbatim label in the prose that follows; a `§` is never invented for something that is not a real heading. There are **no inline-reimplemented release steps** in this SKILL.md body. The single source for each function stays the function-skill; this Specialist invokes it and adds only the role-layer synthesis. The worked per-mode invocation/autonomy ledger and the Mode 1/2/3 output frames live in [`references/composition-contract-release-manager.md`](references/composition-contract-release-manager.md) — this SKILL.md is the authoritative contract; that file elaborates it.
...
**Purpose:** **Assemble the go/no-go evidence package and render the GO / NO-GO recommendation** for the Stage-9 operator gate — bind the plan evidence (scope, dep-graph, critical path, risk register, cross-milestone validation), the **Stage-7 quality report** and **Stage-8 acceptance report** as the upstream-report inputs the package synthesizes, and the verification-readiness read into a Decision Briefing. The assembled package is structured per [`stage-09-plan-review.md`](../../references/pipeline/stage-09-plan-review.md) §5 Phase A (A1 upstream-report synthesis … A5 deployment-readiness), feeding the A6 Release Readiness Scan ([`release-readiness-scan-spec.md`](../../references/specs/release-readiness-scan-spec.md)) — a citation to that canonical spec, not a re-derivation of its A-list here. This is the *decision* a function-skill does not make: `release-planner` produces the read-only evidence and `release-executor` Mode B reads the post-state; only the Release Manager turns those into a ship recommendation.

**Composition:** composes [`release-planner` § Mode A — Backlog Analysis](../release-planner/SKILL.md) and [`release-planner` § Mode B — Release Planning](../release-planner/SKILL.md) for the read-only backlog / dep-graph / Critical Path / Risk Register / Cross-Milestone Validation evidence (`release-planner` is read-only by contract — [`release-planner` § Operating Principles](../release-planner/SKILL.md)), plus [`release-executor` § Mode B — Verify Release](../release-executor/SKILL.md) read-only post-state checks. It runs neither analysis itself — it invokes the skills and consumes their evidence.
...
2. Chain to `release-planner` Mode A/B for the read-only plan/dep/risk evidence and to `release-executor` Mode B for the read-only verification readiness — and synthesize the upstream **Stage-7 quality report** + **Stage-8 acceptance report** into the package (the A1 upstream-report-synthesis input per the §5 Phase A structure cited in Purpose) — never assert readiness from the PR diff alone.
...
**Composition:** composes [`release-executor` § Mode A — Execute Release](../release-executor/SKILL.md) for the apply, chained with [`release-executor` § Mode F — Publish Release (Layer-1 Dual-Write Surface 1)](../release-executor/SKILL.md) for the Surface-1 emit. Re-implements neither the execute sequence nor the publish state-machine.
...
**Composition:** composes [`release-executor` § Mode D — Close Release](../release-executor/SKILL.md) — which wraps `automated-closeout.sh` and surfaces the issue-closure audit at that mode's **Step 2.5** — and optionally [`release-executor` § Mode E — Author Release Note](../release-executor/SKILL.md) on the prose-fill branch. Re-implements neither the close-out script nor the note-authoring branch.
...
- **Signature (observable signal):** the SKILL.md (or a running output) restates the Quality-Gate Ladder steps, the snapshot/write-verify procedure, the close-out script phases, or the dep-graph/CPM algorithm inline rather than citing the composed mode by section-name anchor.
