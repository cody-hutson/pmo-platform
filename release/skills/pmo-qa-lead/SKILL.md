...
**Mode 2 / Mode 4 compose the auditor's live acceptance + dev-testing modes (load-bearing — D-QAL-1).** The auditor's *acceptance review* mode and *dev testing* mode are **live**, and Mode 2 / Mode 4 compose them directly, by name. The `## Modes` section in [`pmo-qa-auditor`](../../../core/skills/pmo-qa-auditor/SKILL.md) is the sole authority on which modes the auditor ships — this skill cites an *individual* auditor mode by name (with its letter as a convenience) but deliberately restates the **set** neither as a literal nor as a letter range, because a frozen mode-set claim goes stale a release after it is written and then misinforms every caller reading it to decide what it can compose against. The **canonical spec-surfaces those auditor modes implement** remain the authority on the machinery Mode 2 / Mode 4 consume — the **Stage-8 §B acceptance machinery** at [`release/references/pipeline/stage-08-qa-testing.md`](../../references/pipeline/stage-08-qa-testing.md) and the **Stage-7 §5 dev-testing ladder** at [`release/references/pipeline/stage-07-dev-testing.md`](../../references/pipeline/stage-07-dev-testing.md). The QA Lead does **not** re-implement either auditor mode — building them inside this Specialist would violate the RECONCILE boundary in `## Boundary` (b). Full detail: [`references/composition-contract-qa-lead.md`](references/composition-contract-qa-lead.md) §2.
...
**Composition:** composes [`pmo-qa-auditor` § Mode H — Acceptance Review](../../../core/skills/pmo-qa-auditor/SKILL.md) — the **live** Stage-8 acceptance lens, cited by section name against the auditor's own `## Modes` section (the sole authority on its shipped set, per `## Composition`) — to ingest the GitHub Issue AC and render per-criterion `MET / NOT MET / PARTIAL / N-A / REINTERPRET / FLAG-UPSTREAM` verdicts. It consumes the Stage-8 §B machinery that mode implements (verdict enum, 3-lane routing, Operator Override Record); it does not reinvent it.
...
**Composition:** composes [`pmo-qa-auditor` § Mode G — Dev Testing](../../../core/skills/pmo-qa-auditor/SKILL.md) — the **live** Stage-7 spec-conformance lens, cited by section name against the auditor's own `## Modes` section (the sole authority on its shipped set, per `## Composition`) — to ingest the PR + release plan, run the structural → contract → content → integration eval ladder, and post a PR-comment quality report. It consumes the Stage-7 severity vocabulary + the DT↔Engineering loop; it does not re-implement the ladder.
...
- **Composes (invokes, never absorbs):** `pmo-qa-auditor` (its platform-health, single-output, cross-skill-coherence, acceptance-review, and dev-testing modes — all live; named per the auditor's own `## Modes` section, which is the authority on its shipped set), `build-reviewer` (findings register + 6-deliverable output; Complexity Assessment dimension).
...
- **Mitigation:** When a QA-Lead mode needs acceptance / dev-test / detector / RACI output, it INVOKES the auditor mode named in `## Composition` — resolved by section name against the auditor's own `## Modes` section, never against a mode-set restated here — and consumes the result; it never authors that capability in its own SKILL.md. The `## Composition` table is the allowlist of what is invoked; anything not in it is not built here.
- **Principal response vs. junior response:** Principal cites "composed `pmo-qa-auditor` § Mode H — Acceptance Review, consuming the Stage-8 §B machinery it implements" and adds only the sign-off synthesis. Junior writes a full acceptance-verdict engine into `pmo-qa-lead`, duplicating the auditor's acceptance mode, and the cross-skill harness flags the absorption at DT.
