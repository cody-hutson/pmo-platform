...
| **A4 — feasibility quick-check** | Lightweight feasibility read against current file state per §5; plus the advisory architecture evaluative-lens pass when the proposal introduces/reshapes a component (advisory, non-gate); plus the **A4.7 scope-altitude determination (SA-G1 — advisory, non-gate)** per `stage-02-triage.md` §5 A4.7 — resolve **A** and **S** by its R0/A1/A2 + S1–S4 rules, then evaluate Limb A ∨ Limb B against guards N1–N7 (cite §5 for the predicate; do not restate it). | Feasibility flags, including the `SA-G1` value — one of `CONTAINED` / `FLAG(<limb>,<shape>)` / `n/a(N#)` / `UNKNOWN(<reason>)` / `HELD(untrusted-body)`. **`UNKNOWN` is never reported as `CONTAINED`.** |
