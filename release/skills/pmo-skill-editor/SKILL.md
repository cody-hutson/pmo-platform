...
**Coverage boundary — and why an absent block is not a signal.** Gate 2 governs Write/Edit from a main session and from a spawned subagent alike, but only while **all four** coverage conditions hold: the session loaded the wiring ∧ no pre-launch `CLAUDE_HOOK_BYPASS` ∧ the master-activation class admits the hook ∧ the mode admits blocking (canonical four-condition statement: `subagent-security-posture.md` § 3.1; hook-specific expansion: `canonical-skill-structure.md` § Coverage boundary of the edit-time gate). This hook is **`workflow`-class and master activation ships OFF**, and the master gate runs *before* the `.mode` read — so on a default instance the hook exits 0 silently, writing to neither log, and warn-mode and enforce-mode are indistinguishable. **Consequence for this skill: observing no block and no log row during an edit session is NOT evidence that the edit was sanctioned, and NOT evidence about hook loading.** It is the expected result of a class gate you did not check. The discipline this skill carries is the control on a default instance; the hook is the interlock only where all four conditions hold.
