...
5. Verify `detection_patterns` do not collide in a way that creates ambiguous first-match ordering (if collision exists, either order packs in the registry such that the more-specific pack appears first, or tighten the patterns).
