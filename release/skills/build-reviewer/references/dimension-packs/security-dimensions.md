...
**The review-time neighbour — `pmo-technical-analyst` Mode C (Architecture / Infrastructure Review).** The boundary above is against the *design-time* owner. There is a second boundary, and it is the closer call precisely because both sides review rather than decide: Mode C carries **security posture** — authentication, authorization, data protection — among the standard architecture concerns it assesses, so the vocabulary genuinely overlaps. The cut is the **object under review, not the moment**. Mode C reviews an **artifact** — an FDD, an integration spec, an architecture or infrastructure document — and asks whether the design it describes is sound. This pack reviews a **surface** — the controls, hooks, CI security jobs, dependency manifests, and scanner config that actually exist in the tree — and asks whether each one does its job on the branch where it cannot. A request that hands you a *document* routes to Mode C; a request that hands you a *tree* routes here. When a review legitimately needs both, run them as two passes with two verdicts rather than blending them: this pack never grades an artifact's design, and Mode C never grades an enforcer's fail-branch. Both routes are recorded on the platform's consultation map, so a reader entering from the routing surface rather than from this pack reaches the same cut.
