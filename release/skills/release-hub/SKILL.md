...
**Depth bound (C1).** ADR-019 cascade rule C1 (depth ≤ 2): `operator → release-hub → [composed skill]` is depth 2, the maximum. The hub must **not** chain a composed skill onward into a third skill.
...
- A request centered on **driving a milestone through the pipeline** ("orchestrate the release for [milestone]", "run the release hub on [milestone]", "drive [milestone] through the pipeline") → **Mode O — Orchestrate Release** *(follow-on — see status)*.
...
### Mode O — Orchestrate Release  *(follow-on — NOT YET BUILT)*

**Status:** deferred to a follow-on story. `engineering-orchestrator` (Stage 5–6 orchestration) is a build dependency.
**Purpose (planned):** drive the milestone through Procedure 0 → 7 ([`hub-spoke-bridge.md`](../../references/how-to/hub-spoke-bridge.md)), composing `release-planner` (St 4), `engineering-orchestrator` (St 5–6), the stage spokes (St 7–8), and `pmo-release-manager` (tail St 9 → 13); stop at the human gates (Procedure 5 / 7); resume via Procedure 0b. Encodes the **N=3 spoke cap** and the **worktree-cwd guard** (see FM-5).
**Today:** when invoked, state that Mode O is not yet implemented and route to the manual hub (`hub-spoke-bridge.md`) or to `pmo-release-manager` for the tail.
...
- **Composes (invokes, never absorbs):** the triage-analysis capability / `intake-desk` + `delivery-engine` (triage + dup); `triage-design-rereview` (staleness + architecture); `release-planner` (dependencies); `bundle-composition-doctrine` (coherence). **(Mode O, when built):** `engineering-orchestrator`, the stage spokes, `pmo-release-manager`.
...
- **Documented dependencies NOT yet built:** the triage-analysis capability (Mode R's eventual delegate; interim the hub chains `intake-desk` + `delivery-engine` directly); `engineering-orchestrator` (Mode O only).
- Composition edges are skill→skill (invocation), never role→role (absorption); depth ≤ 2 (C1).
...
- **Mode O (when built):** up to Stage 9, **MODERATE** (sub-tasks/branches; git-revertable); Stage 12 deploy, **EXPENSIVE → IRREVERSIBLE** — but those are owned by the composed `pmo-release-manager` behind the operator gate; the hub **never self-authorizes** them.
...
### Spawning stage spokes without the cwd / quota guards (Mode O) — HAND

- **Signature (observable signal):** Mode O launches engineering spokes without a `pwd` / `rev-parse` worktree-cwd guard, or more than N=3 concurrent.
- **Conditional:** do NOT spawn a git-writing spoke without the worktree-cwd guard or above the N=3 cap, because spawned spokes have committed from the primary checkout instead of their isolated worktree (observed), and >3 concurrent exhausts the Cowork launch quota (ADR-026).
- **Root cause:** the guards live in operator memory, not in the prompt; under fan-out pressure the launch step omits them.
- **Mitigation:** encode the cwd guard (`pwd` + `rev-parse`, `cd` into the `agent-` path) and the N=3 cap directly in Mode O's spoke-launch step; never rely on the spoke to self-correct.
- **Principal response vs. junior response:** Principal bakes the guard into the launch template and caps concurrency at 3. Junior assumes each spoke lands in the right worktree and launches them all at once.
