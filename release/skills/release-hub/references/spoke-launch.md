...
2. **Quota-budget gate (Procedure 2 Step 5.5 / Checkpoint B)** — fires before **EVERY `Agent`-tool launch, wave or singleton, at every stage**. It is a standing pre-launch step (each launch faces a different remaining envelope — mid-release drift, window-boundary crosses), not a one-time Stage-4 estimate; a serial stage bounds the concurrent-batch surface, never the envelope. **What it weighs — two independent axes, either sufficient to block:** the *remaining* per-account 5-hour **usage-window** envelope (`tokens_per_spoke × N` against what is left) and the **host-API quota** (the host's separate `core`/REST and `graphql` pools). **Verdict depth follows launch shape:** a wave (N ≥ 2) renders ∈ {**PROCEED** / **SERIALIZE** / **DEFER** / **REDUCE-scope**}; a singleton (N = 1) renders the reduced ∈ {**PROCEED** / **DEFER**} — `SERIALIZE` is meaningless at N=1 and `REDUCE-scope` stays a pre-verdict mitigation, not a singleton verdict. **Wave width is a second output, not a verdict:** alongside the verdict the gate returns **`W_max`** — the cap on spokes in flight at once — keyed on the same remaining-envelope basis, with `⌈N/W_max⌉` re-gated sub-waves when `W_max < N` and `W_max n/a` at N=1. Render it on the same line. Cumulative draw does not change with width; **interruption cost does** — a wave that dies mid-flight loses every spoke in it, so width is the only lever on that loss. Cite the table; do not restate it. **The host-API axis** renders ∈ {**PROCEED** / **DEFER**} on a 20 %-remaining floor and combines **DEFER-dominant**: a host-API DEFER makes Checkpoint B render DEFER, otherwise the usage-window verdict passes through unchanged — **no new verdict token is minted**. **Cost — stated per axis, because the two differ:** the usage-window axis is zero tool calls (a hub-side reasoning step over state the hub already holds, not an instrument, so it cannot draw against the envelope it protects); the host-API axis is one `gh api rate_limit` read per *routing turn*, reused for every launch issued in that turn. Whether that read is *also* free against the pools it measures is **unverified** and is no longer claimed; its certain cost is **cross-axis** — the one call spends the *usage-window* axis to read the *host-API* axis. Even bounding the self-cost at one whole request per routing turn, per-launch firing stays affordable and no launch shape is too small to gate. **Render the verdict every time, including PROCEED, naming BOTH bases** — the gate emits no event, so the rendered line is the only way a skipped gate is distinguishable from a cleared one, and with two axes a verdict naming no basis cannot say which axis produced it; for the host-API axis the line carries observed `remaining/limit` per modelled pool. **`UNSTATED` handling:** with no stated envelope, render `UNSTATED` + the conservative default and never a synthesized percentage; when the host-API probe *fails*, render basis `UNSTATED` and **fail open** to PROCEED with the reason rendered — a failed read evidences an instrument or auth fault and carries no exhaustion signal at all. **A *successful* read can also carry no signal:** exhaustion has **three** successful-read presentations. The *unstarted-window* one (a full pool, `used = 0`) renders PROCEED wrongly and is not detectable from a single sample; the *secondary-limit* one (**every** resource full while real calls on **both** surfaces are refused) renders PROCEED wrongly and is not detectable from that endpoint **at all**, because the limit in force is absent from its response by construction — so no re-read reaches it, and the pool table's extension seam does not either. Treat a host-API PROCEED as weak evidence of headroom, never confirmation of it (§ 4.3b). **Fail-open has a carve-out:** a *real call refused on quota grounds* is not a probe failure — it is the axis's strongest positive evidence, renders basis **`CONTRADICTED`** and verdict **DEFER**, and does not inherit fail-open; a real call failing for a **non-quota** reason still does. The classifier is the refusal reason, never the fact of failure (§ 4.3b, and `review-discipline-principles.md` § 8.1 PV-7/PV-7b for the obligation this satisfies). **DEFER's recovery time is primary-only:** a secondary limit publishes no `reset`, so the § 8 action item carries backoff guidance rather than a timestamp. A non-PROCEED verdict surfaces to the operator; it does NOT reclassify any stage's Autonomy Tier. (Spec: `quota-budget-protocol.md` §§ 4.2, 4.3b, 4.3c.)

"Parallel-safe" is a *coordination* property (no shared write surface) — it is orthogonal to the usage-window envelope, against which concurrent spokes still draw cumulatively. It is not a licence for width either: `W_max` comes from Checkpoint B, never from the stage's parallelism class. (Spec: `hub-spoke-bridge.md` § Per-Account Usage Window Constraint + `quota-budget-protocol.md`.)
...
Auto-launch is bound to the stage's Autonomy Tier: spawn autonomously for Tier-2/3 stages; **preserve the operator gate at Tier-0 — the Procedure 5 gates (the Hub Gate Register rows whose `Proc` is 5 — `orchestration-playbook.md`) are NEVER auto-launched.** (Stage-to-Autonomy-Tier table: `decision-briefing.md` § Gate 0, Input 1.)
...
Every spoke prompt Mode O renders MUST carry the write-early clause verbatim. A spoke produces output early and banks it as it goes, ordering its work so an interruption costs the least-valuable part; the form is channel-differentiated — the **commit** channel (Stages 6 / 12 / 13) pushes each coherent slice rather than saving one terminal push, while the **comment** channel (Stages 5 / 7 / 8) has one atomic write by design and so banks by **ordering, not incrementality**: load-bearing analysis before elaborative reads, composed output accrued into `$SPOKE_OUT` as evidence lands. Canonical clause + its honest bounded-loss boundary: `hub-spoke-bridge.md` § Write-Early Discipline (Spoke Template). Cite it; do not restate it here.

**Honest scope — this MUST is a discipline, not an interlock.** Nothing asserts it per launch, and measurement says it is routinely unmet: across the renders observed in the release that added this note, carriage of a required block ranged from fully intact to entirely absent, and the same untouched clause reached one spoke complete and another at roughly three-quarters. Fidelity varies per launch, so no finite set of observations can discharge a claim of this shape — only a per-launch presence assertion could, and none exists yet. The MUST is retained deliberately rather than softened to match practice: its value is that a deviation is a citable violation instead of a judgment call, and that value survives the absence of enforcement. What does not survive is reading a green pipeline as evidence the clause arrived. It is not evidence; nothing checks.

**A second boundary this block owns and its two siblings do not.** Even when the clause *is* carried and followed, what it buys a comment-channel spoke is bounded to a **resumable session**: it does not bank work across a re-spawn, because a fresh spawn resolves a fresh run directory and § Output path forbids reading another run's artifacts. So a spoke killed and re-spawned starts from nothing regardless of how early it wrote. State that when rendering the block; a reader who takes "bank as you go" to mean re-spawn-durable has over-read it.
