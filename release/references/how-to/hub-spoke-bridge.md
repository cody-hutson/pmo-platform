...
- **Single-branch topology** (D-C SINGLE — default): The plan file is committed on the release branch as **Engineering Commit 0** (the first commit on the release branch produced by the first per-issue Stage 6 Engineering spoke). Until that first Engineering commit lands, the Stage 4 sub-task comment is the working reference.
  ...
  **Commit-0 version re-verify (FIRST Engineering spoke only, spanning the plan-file write and its commit as Engineering Commit 0).** The version selected at D-Version (recommendation time, Stage 4) may have been claimed by a concurrent release in the window between selection and now — the originating incident surfaced exactly this way (a concurrent release shipped the recommended version; the collision was caught only when the Engineering spoke fetched fresh authoritative refs before the plan-file write). Before authoring `release/releases/plans/<slug>_RELEASE_PLAN.md`, the first Engineering spoke re-runs the authoritative-version-selection check — steps 1–3 below, all of which run pre-write. Step 3b then runs **after** the plan-file write and **before** its commit, so the numbered list as a whole spans the beat rather than sitting entirely ahead of it:
    ...
    3. If the planned version is NOT in the claimed set AND equals the recomputed next-free → **PROCEED**: write the plan file. Else → **HALT**: do NOT overwrite a shipped version's plan file. Post a Tier 2 `[SCOPE CHANGE]` comment on the parent sub-task per `release-process.md` § Inter-Stage Feedback Protocol, naming the planned version, the recomputed next-free, and the colliding claim — the tag, plus the ledger row where one has already landed; the operator re-renders D-Version against current state. **Which arm binds, and which only corroborates.** `claimed_set()` unions several claim-recording surfaces and they do not settle at the same instant, so this step declares a precedence rather than leaving a reader to consult whichever arm is nearest. A version becomes claimed the moment its release **merges**, because merging is when the atomic claim pushes the signed version tag: the **tag arm binds**, and step 1's tag fetch is what makes it authoritative rather than local. Every other surface records that same claim **later** — the host's published-release record follows the tag, and the ledger row follows both, landing on a separate Stage-12 chore PR that merges after the release PR it describes. Those surfaces **corroborate and never authorize.** The two verdicts are therefore asymmetric: **HALT** may be raised by *any* arm showing the version claimed, but **PROCEED** rests on the tag arm alone reading it free. **A missing ledger row is not evidence of freeness.** In the interval between a sibling's merge and its ledger chore PR merging, the ledger legitimately carries no row for a version that is already taken; a re-verify that reads that silence as a free slot returns a false PROCEED, and the collision this rung exists to catch earliest and cheapest instead falls through to the Stage-12 pre-merge freeness check or the atomic version claim, where the operator re-renders D-Version at the merge boundary rather than at Commit 0.
     ...
     **What the transcription must carry.** The plan file is not a free re-rendering of the Stage-4 comment: the **Commit-0 Survival Set** enumerated at [`stage-04-planning.md`](../pipeline/stage-04-planning.md) § 6 names the nine elements determined at Stage 4 that the plan file must carry, each with the downstream consumer that reads it from the plan. A dropped element is a spec violation against that enumeration. Rows 1–5 of that table are asserted mechanically at Stage-6 C4 self-verification by [`release/tools/verify-release-plan.sh`](../../tools/verify-release-plan.sh); rows 6–9 are reviewer-read. This step **cites** that enumeration and deliberately states no second one.
...
- **Option-A topology** (D-C OPTION-A — per-issue branches + per-issue PRs per Procedure 6 early-merge precedent): The plan file is committed via a dedicated **Stage 4 release-plan chore PR** authored by the hub from the approved Stage 4 sub-task comment. Mechanics:
  1. Hub copies the Stage 4 sub-task comment content into a new file at `release/releases/plans/<slug>_RELEASE_PLAN.md`, carrying the Commit-0 Survival Set per [`stage-04-planning.md`](../pipeline/stage-04-planning.md) § 6.
...
2. For each issue that is OPEN in the milestone **at scaffold time**, use the release plan's stage applicability matrix to determine which stages apply:
   ...
   - **Late adds.** The set read here is a point-in-time snapshot. When an issue is added to the milestone after this step has run, the hub scaffolds that issue's per-issue sub-tasks at add time — or, if the release has passed the issue's stages, closes them immediately with a Skip Closure naming the late add. A milestone whose membership grew after scaffolding without this step is why per-issue completeness is advisory rather than gating. A late-added issue also has **no row** in the release plan's `### Agent-Editability Read` section, which was authored over the membership as it stood at Stage 4 — a different gap from a plan that has no such section at all. Before scaffolding a late add's Stage 6 sub-task, the hub re-runs the plan's Phase A3.5 procedure (`pipeline/stage-04-planning.md` § 5.9 — read it there; it is not restated here) over that issue's write set, **appends the resulting row to the plan's section first, and only then stamps the sub-task from that row**. The ordering is the invariant: the plan's section stays the single record, and the stamp is always a copy of it, never its origin. Re-invoking the one specified procedure is not a second derivation site; authoring a different procedure here, or stamping a value that never entered the plan, would be.
...
6.5. **Scaffold-completeness verification.** Before presenting to the operator, run:
   ...
   **Read the rows, never the count alone.** `COUNT_M3 0` is emitted by two opposite states and cannot separate them: a scaffolded milestone emits `M3_DENOM` + `SCAFFOLD_MARKER` and then `COUNT_M3 0`; a milestone carrying no scaffold at all emits `SKIP_MS <ms> not-yet-scaffolded` and then the same `COUNT_M3 0`. The row set is the discriminator. **A `SKIP_MS … not-yet-scaffolded` row blocks the Step 7 presentation exactly as a load-bearing finding does** — this step runs immediately after the sub-task set was created, so the row reports the scaffold as entirely absent, which is a stronger signal than any single finding rather than a weaker one. The absence of `M3_DENOM` / `SCAFFOLD_MARKER` is never presented as a clean summary; the step reports the state it actually received. **This blocking rule is scoped to the single-milestone (`--milestone`) invocation.** On Check 56's whole-population path the identical row is routine and never gates — most open milestones have not yet started their release — so the same evidence carries opposite weight by invocation context, and a fix that made the row load-bearing everywhere would flag the majority of the open backlog.
...
**Resolving `{EDITABILITY_CLASS}` / `{EXECUTION_PATH}`.** Both are **copied verbatim** from the release plan's `### Agent-Editability Read` section for this card — the hub transports the classification, it never recomputes it. The derivation has exactly one home, the Stage-4 Planning spec's Phase A3.5; a second derivation site would be a second source of truth for what the floor covers. When the plan carries no such section (a pre-cutover plan), stamp `unresolved — plan predates Phase A3.5` rather than deriving one here. Two further states resolve to a stamp rather than to silence. **The plan carries the section but no row for this card** — a late add: resolve it per Procedure 1's **Late adds.** rule before stamping, and stamp `unresolved — no plan row` only when that resolution has not yet been made. **The card's write set changes after the stamp is written** — the stamp is a point-in-time snapshot on the same terms as `{PARENT_STATUS_LABEL}` above and does not resync itself; Procedure 4's **Editability currency** evaluation dimension names the moment it is refreshed. The stale-`unconstrained` case is the one worth naming explicitly: it reads as a clean result and instructs an Engineering spoke to attempt a write a live control will refuse. The two `unresolved` reason strings are **not interchangeable** — one is resolvable at the hub and one is not — and the Engineering-Brief Editability Pre-flight branches on them differently.
...
1. **Step-by-step item** (numbered step within the chip's "Step-by-step (in order)" block): *"After Phase B5 chore PR merge verified, execute Phase B5.5 Surface 1 emit per [`pipeline/stage-12-execute.md § Phase B5.5`](../pipeline/stage-12-execute.md): verify tag exists on remote (`git ls-remote --tags origin v<X.Y>`); compute the §5.1 frontmatter-stripped body ONCE, using the shared transform — `. release/tools/lib/frontmatter-strip.sh; BODY=\"$(strip_frontmatter release/releases/notes/v<X.Y>_RELEASE_NOTES.md)\"` — and HALT if `$BODY` is empty (§5.1 S4: the strip is fail-closed, and publishing an empty body blanks a live Release page irreversibly); run view-then-create-or-edit state machine — `gh release view v<X.Y> --repo {REPO}`; if release exists AND body matches `$BODY` → PASS; if release exists AND body differs → `gh release edit v<X.Y> --notes \"$BODY\"`; if release does NOT exist → `gh release create v<X.Y> --repo {REPO} --title 'v<X.Y> — <H1-headline>' --notes \"$BODY\" --target \"$MERGE_SHA\" --latest=\"$LATEST\"`. Verify final state via `gh release view v<X.Y>` returns 0."*
   ...
   **The emit MUST resolve `--latest` explicitly, never omit it.** Without the flag `gh` sends `make_latest=legacy` — *"automatic based on date and version"* — so publishing a **lower** version after a higher one can move the repo's **Latest** badge backwards. That badge is the anchor read by `core/hooks/notify-version-skew.sh` **and** by `deploy.sh` Check 39, so a regressed badge silently re-bases every workspace's version-skew comparison. Resolve it before the state branch **from the version-grammar SSOT, never by re-rendering the comparison**: `. release/tools/version-grammar.sh ""; ANCHOR="$(gh api repos/{REPO}/releases/latest --jq .tag_name 2>/dev/null | tr -d '[:space:]')"; LATEST=false; [ "$(version_badge_latest "$ANCHOR" v<X.Y>)" = ADVANCE ] && LATEST=true` — every non-`ADVANCE` verdict withholds the badge, because a badge we decline to take is recoverable with one `gh release edit --latest` while a badge taken wrongly regresses every consumer silently. Record the resolved value and the anchor in the chip's deliverables.
2. **Output deliverables block** (entry in the chip's required-deliverables list): *"Surface 1 emit: GitHub Release v<X.Y> exists per `gh release view v<X.Y> --repo {REPO}` exit 0; final state machine state (CREATED / EDITED / NO-OP) recorded; merge SHA used for `--target` recorded; resolved `--latest` value and the `releases/latest` anchor it was computed against recorded."*
...
5. **Evaluate spoke recommendations** against release context:
   ...
   - **Editability currency:** does the decision change any card's write set — a File Change Matrix expansion, a newly declared affected file, an accepted scope change? If so, that card's editability class may have moved. Re-run the plan's Phase A3.5 procedure (`pipeline/stage-04-planning.md` § 5.9) over the changed write set, amend that card's row in the plan's `### Agent-Editability Read` section **first**, then refresh the `**Editability class:**` stamp on that card's open Stage 6 sub-task from the amended row — the plan's section stays the single record and the stamp stays a copy of it. **Record the refresh in the same Decision-Briefing turn as the decision that triggered it**, so the obligation's discharge is observable in the release record rather than assumed. Re-invoking the one specified procedure is not a second derivation site. A class that moves toward `sanctioned-session-required` or `tier-0-floored` and is not refreshed sends an Engineering spoke at a write a live control will refuse — and a stale `unconstrained` stamp reads as a clean result while doing it.
...
**Mandatory close completeness (outcome-bound):** A release MUST close with its complete enumerated output set produced and verified on main — the canonical Stage 13 output set defined once by the Step 4 Verification table below. The binding is on the **output set**, not on which mechanism produced it. The **mandated mechanism** — the default and expected path — is the automated close-out (release-executor Mode D Close + Mode E Author Note + Mode F Publish, wrapping `automated-closeout.sh`) per `pipeline/stage-13-close.md § Phase A8`. **Hand-assembling the corpus row-by-row is prohibited**: it silently drops outputs (the canonical incident landed only the RELEASE_LOG row of the corpus surfaces — RELEASE_LOG + INDEX + DIGEST + NOTES — leaving the INDEX row, the DIGEST entry, and the RELEASE_NOTES file unwritten until manual review). **Fallback:** when the automated close-out's preflight cannot pass for a **not-yet-ready** condition — `gh auth` unavailable, tree not clean, the DEPLOYED RELEASE_LOG row not yet landed — the operator MAY produce the output set via the documented Phase B chore-PR mechanism per `pipeline/stage-13-close.md § Phase B`; the close is satisfied iff `deploy.sh --check` Check 32 and the Step 4 completion-verification table both pass. Preflight's other blocking conditions do **not** license the fallback. A **missing input** — an unfilled scaffold token in this version's release note, or the Phase-A7 `release-synthesis/learnings-triple` row not captured — is supplied and the close re-run. A **defect** — the close-out invoked from the primary checkout, a milestone slug matching other than exactly one RELEASE_LOG row, or a scaffold-token set the lint tool cannot read — is repaired and the close re-run. Hand-assembly on either of those classes produces by hand the very artifact the gate refuses. Two further reading notes: the version tag is *recorded* at preflight rather than gated there, and exit 2 is not unique to preflight (the open-issue detection phase also exits 2), so read the condition class from `pipeline/stage-13-close.md § Phase A8` rather than inferring it from the exit code. Binding one mechanism would strand a close whenever that mechanism's preflight legitimately blocks; binding the outcome and naming the fallback keeps the close always satisfiable.
...
4. **Completion verification:** Before the hub declares "release complete" / "🎉 RELEASE COMPLETE", the hub MUST independently verify each Stage 13 spec'd output exists on main. This applies the governance-theater discipline — "claim only what is observable on disk or in GitHub state" — to release-close declarations. Empirical motivation: a Stage 13 Close declared "🎉 RELEASE COMPLETE" before verifying the user-facing release note existed on main; the retroactive PR authored the missing artifact. Hub completion-verification closes the loop that the Stage 13 chip pattern (Procedure 3 §Stage 13 Chip Pattern — Release-Notes Authoring Discipline) opens — chip mandates spoke authors the artifact; Hub verifies it landed.
   ...
   **Audit-class detection — run BEFORE the 9 universal verification commands:** Determines whether the audit-class synthesis verification rows (rows 6-7 below) fire as PASS/FAIL or resolve to N/A. The detection result MUST be reported on its own line BEFORE the verification table is rendered (e.g., *"Audit-class detection: 1 new `<OPERATOR_INSTANCE_ANALYSIS_PATH>/<name>-YYYY-MM-DD/SUMMARY.md` file in this release → release IS audit-class. Audit-synthesis rows below run live."* OR *"Audit-class detection: 0 new `<OPERATOR_INSTANCE_ANALYSIS_PATH>/<name>/SUMMARY.md` files in this release → release is NOT audit-class. Audit-synthesis rows below resolve to N/A."*). This makes the N/A pathway load-bearing rather than silent.
   ...
   # 8. .version stamped >= v<X.Y> (release-cut-owned source-of-truth, per pipeline/stage-13-close.md Phase B5.7)
   #    MONOTONICITY, not equality: on an out-of-order close a HIGHER .version is the
   #    CORRECT state, and the equality form reported MISSING for a file that was right.
   #    The verdict comes from the version-grammar SSOT — never a shell string compare,
   #    which ranks v4.9 above v4.10 and reads v2.6 / v2.06 as different versions.
   #    N/A for a version-less release (Phase B5.7 SKIPs with PASS).
   #    UNLIKE items 6 and 6b, this item EMITS ON SUCCESS as well as on failure: the
   #    verdict is three-valued, so silence could not distinguish PASS from UNVERIFIED.
   #    That emit-convention change is deliberate and scoped to this item.
   bash -c '
     . "$(git rev-parse --show-toplevel)/release/tools/version-grammar.sh" ""
     cur="$(git show origin/main:.version 2>/dev/null | head -1 | tr -d "[:space:]")"
     printf "%s — .version on main is %s (target v<X.Y>)\n" \
       "$(version_stamp_state "$cur" "v<X.Y>")" "${cur:-<empty>}"'
   ```
   ...
   **This table is the canonical Stage 13 output set; close-mandate prose references it by name and never re-enumerates it.** Prose elsewhere that names the close outputs cites "the canonical Stage 13 output set (this table)" rather than re-listing the rows; a hand-written enumeration drifts from the table and miscounts it.

   **The `Req` column carries MEMBERSHIP; the `Result` column carries OUTCOME.** The two are different questions and were previously answered by one cell: `N/A` meant both *"this output does not apply to this release"* and *"this output is not owed"*, so a row that was never owed and a row whose test could not run were indistinguishable. The column names what each row already did — no existing row changes behaviour.

   | `Req` value | Meaning | Disposition when the output is absent |
   |---|---|---|
   | `required` | Owed by every close | **BLOCKS** the close |
   | `required-if <predicate>` | Owed when the named predicate holds. The predicate is declared, evaluated, and **reported before the row is graded** | Predicate true: **BLOCKS**. Predicate false: resolves `N/A`, a **satisfied** state |
   | `optional` | Never owed | Recorded (see the `**Not-produced:**` marker below); **never blocks** |

   **A `required-if` predicate that cannot be EVALUATED is a third state, and it BLOCKS.** It is neither true nor false: it is `INDETERMINATE`, reported by name. Grading an unevaluable predicate as false would resolve the row `N/A` — a satisfied state — and an owed output would then be recorded satisfied *because its membership test could not run*, which is the same silent-absence defect this table exists to close, re-created one level down.

   **Absence is recorded, never merely dropped.** When a close-out output is not produced, the producing phase writes a `**Not-produced:** <output-id> — <reason>; recorded at <UTC> by automated-closeout.sh phase <phase-name>` line into the `#### Deployment Log <V>` block, anchored immediately after `**Result:**`. The marker is **evidence, never an exemption**: it does not satisfy the pre-commit assert for a `required` member and does not suppress the post-stamp `deploy.sh` Check 48 sub-checks — both key on the member, and a marker is not the member. What it buys is durability (the absence becomes bytes on `main` rather than a row in an ephemeral run report) and legibility for `optional` members, whose absence would otherwise be indistinguishable from a silent drop.

   **Two lanes assert this set, at different moments and with different powers.** *Lane 1 (close-time, blocking)* is `automated-closeout.sh` phase 9.56, which asserts the pre-commit-assertable members against the working tree **before** the chore commit — so the `DEPLOYED → VERIFIED` stamp cannot reach `main` ahead of its own outputs. *Lane 2 (post-stamp, detective)* is `deploy.sh` Check 48, which reads `main` and selects rows already reading `VERIFIED`; it is a regression catch, and by construction it cannot fire before the stamp. Lane 1 asserting a narrower set than lane 2 is precisely how outputs went missing without a signal.

   ```markdown
   ### Verification
   | Output | Req | Verification | Result |
   |---|---|---|---|
   | User-facing release note | required | `git show origin/main:.../v<X.Y>_RELEASE_NOTES.md` (presence, cmd #1) **+ §3.2 note-content conformance (cmd #1b: `lint_release_corpus.py --check note-content`, version-scoped — a finding for v<X.Y> BLOCKS)** | PASS/FAIL |
   | Release plan identity + placement (ADR-092) | required-if versioned release | **cmd #1c: `lint_release_corpus.py --check plan-identity`, scoped to v<X.Y> by expected-plan-path OR version-keyed needle — a blocking finding for v<X.Y> BLOCKS; advisories never block**; N/A for a version-less release | PASS/FAIL/N/A |
   | Version tag | required | `gh api .../git/refs/tags/v<X.Y>` | PASS/FAIL |
   | Milestone closed | required | `gh api .../milestones/<N>` | PASS/FAIL |
   | RELEASE_LOG entry | required | cmd #4, BOTH limbs on `git show origin/main:release/releases/RELEASE_LOG.md`: the table row (`grep -qE "^\| v<X.Y> "`) AND the visible-H4 block (`grep -qE "^#### Deployment Log v<X.Y>"`). **Not `git log --grep`** — cmd #4 rejects that instrument by name, because a commit subject proves a commit was written, not that the row and block exist on main, and a squash or retitled merge evades it | PASS/FAIL |
   | Sub-issues closed | required | `gh issue list --state open` | PASS/FAIL |
   | GitHub Release (Surface 1, Layer-1 dual-write) | required | `gh release view v<X.Y> --repo {REPO}` exit 0 | PASS/FAIL |
   | Surface 1 body matches in-repo note (§5.1 enforced transform) | required-if gh reachable | `./release/tools/check-release-body-drift.sh v<X.Y>` exit 0 (published body == frontmatter-stripped note); detective-only, shares logic with deploy.sh Check 47; N/A if gh offline | PASS/FAIL/N/A |
   | CHANGELOG.md entry (Surface 2, Layer-1 dual-write) | required-if CHANGELOG.md present | `git show origin/main:CHANGELOG.md \| grep -qE "^## \[?v<X.Y>\]?[[:space:]]"` exit 0; N/A if CHANGELOG.md absent (pre-CHANGELOG state) | PASS/FAIL/N/A |
   | .version stamped (release-cut-owned) | required-if versioned release | `. release/tools/version-grammar.sh ""` then `version_stamp_state "$(git show origin/main:.version \| head -1 \| tr -d '[:space:]')" v<X.Y>` — the SSOT returns the verdict; do not re-render the comparison here. PASS when `.version` is **equal or higher** (a higher value is the correct out-of-order-close state, not a miss); a non-canonical `.version` is **UNVERIFIED**, never PASS. N/A for version-less release (Phase B5.7 SKIP) | PASS/MISSING/UNVERIFIED/N/A |
   | Posted Release TITLE versioned (Surface 1, posted-surface) | required-if versioned release | `gh release view v<X.Y> --repo {REPO} --json name --jq '.name' \| grep -qE '^v[0-9]+\.[0-9]+([a-z]\|-[0-9a-z][-0-9a-z]*)? — .'` exit 0 — the posted title reads `vX.Y — <headline>`, not the bare H1 headline; N/A for a version-less release | PASS/FAIL/N/A |
   | Posted Release BODY link-resolvable (Surface 1, posted-surface) | required | `gh release view v<X.Y> --repo {REPO} --json body --jq '.body' \| grep -nE '\]\((\.\.?/\|release/\|core/\|docs/)'` returns NO match — the published body carries no repo-relative link (a repo-relative link 404s on the Release page) | PASS/FAIL |
   | Audit-class synthesis (audit-class only) | required-if audit-class release | `gh issue list --milestone <target-milestone> --search "in:body \"<audit-folder-name>\""` returns N where N = recommendations count per Stage 4 D-decision granularity rule | PASS/FAIL/N/A |
   | Audit retro Milestone tags (audit-class only) | required-if audit-class release | For each filed Issue: `gh issue view <N> --json milestone --jq '.milestone.title'` returns non-null = `<target-milestone>` | PASS/FAIL/N/A |
   | `**Velocity:**` field on the Deployment Log block | required | Lane 1: `automated-closeout.sh` phase 9.56 block-scoped presence on the resolved surface, pre-commit. Lane 2: `deploy.sh` Check 48 sub-check (j), post-stamp | PASS/FAIL |
   | `#### Release Learnings <V>` block | required | Lane 1: phase 9.56 placement probe — the H4 immediately following the Deployment Log block in the hot ledger, pre-commit. Lane 2: Check 48 sub-check (k), post-stamp | PASS/FAIL |
   | `**Close-Class-Telemetry:**` field on the Deployment Log block | required-if telemetry-cutover armed | Lane 1: phase 9.56 block-scoped presence, pre-commit; the cutover value is READ from `deploy.sh`'s own committed default, so arming it is a one-value change in one place. Lane 2: Check 48 sub-check (l), post-stamp | PASS/FAIL/N/A |
   ```
   ...
   **Sequencing note:** The Milestone-closed check (#3) is the only verification that depends on Step 5's PATCH having fired (it asserts `state == closed`). Hub runs verification commands #1, #2, #4, #5 first (which gate the Step 5 hub PATCH), proceeds to Step 5 (PATCH), then re-runs check #3 to confirm `state=closed`. Final "🎉 RELEASE COMPLETE" declaration awaits all 9 universal checks PASSing AND — on an audit-class release only — the two audit-class rows (`Audit-class synthesis`, `Audit retro Milestone tags`) PASSing. **The count was reconciled, not incremented:** these two prose statements read `5` while the script's render already emitted `6` (the Procedure-7a action-item row landed without the prose following it), and the three output-set rows add the rest. All four statements of this count — both here and both in `automated-closeout.sh` — now agree on one number. The audit-class pair is cited by ROW NAME rather than by ordinal, because an ordinal into a table that grows is a number that goes stale silently; they are `required-if audit-class release` members, not part of the universal set.
   ...
   **Merge commit SHA:** <captured at Stage 12 via gh pr view <PR> --json mergeCommit --jq '.mergeCommit.oid'>
   ...
   | .version stamped (release-cut-owned) | \`. release/tools/version-grammar.sh ""; version_stamp_state "$(git show origin/main:.version | head -1 | tr -d '[:space:]')" v<X.Y>\` | PASS / MISSING / UNVERIFIED / N/A (version-less release) |
