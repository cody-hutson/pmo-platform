...
4. **Completion verification:** Before the hub declares "release complete" / "🎉 RELEASE COMPLETE", the hub MUST independently verify each Stage 13 spec'd output exists on main. This applies the governance-theater discipline — "claim only what is observable on disk or in GitHub state" — to release-close declarations. Empirical motivation: a Stage 13 Close declared "🎉 RELEASE COMPLETE" before verifying the user-facing release note existed on main; the retroactive PR authored the missing artifact. Hub completion-verification closes the loop that the Stage 13 chip pattern (Procedure 3 §Stage 13 Chip Pattern — Release-Notes Authoring Discipline) opens — chip mandates spoke authors the artifact; Hub verifies it landed.
   ...
   **Two lanes assert this set, at different moments and with different powers.** *Lane 1 (close-time, blocking)* is `automated-closeout.sh` phase 9.56, which asserts the pre-commit-assertable members against the working tree **before** the chore commit — so the `DEPLOYED → VERIFIED` stamp cannot reach `main` ahead of its own outputs. *Lane 2 (post-stamp, detective)* is `deploy.sh` Check 48, which reads `main` and selects rows already reading `VERIFIED`; it is a regression catch, and by construction it cannot fire before the stamp. Lane 1 asserting a narrower set than lane 2 is precisely how outputs went missing without a signal.
   ...
   | .version stamped (release-cut-owned) | required-if versioned release | `git show origin/main:.version \| grep -qx v<X.Y>` exit 0; N/A for version-less release (Phase B5.7 SKIP) | PASS/FAIL/N/A |
