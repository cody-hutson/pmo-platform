...
**capability-class = C1 ∧ C2 ∧ C3.** The conjunction is the point, and the motivating failure is why: that release had C1 (four skills instructed to read the roster) and C3 (a name resolves to a person record) but no C2 — nothing seeded the roster on install, so the actor never fired. A predicate that merely asked "does this ship a capability?" would have passed it.
...
The list is closed — extending it is a governed change, not an authoring choice. Four rules keep it from reading as a general escape hatch:
