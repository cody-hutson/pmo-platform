...
|---|---|---|---|
| **DECLARE** | The design artifact **already exists** as prose / tables / a fenced block in the § 2-cited parent, but is **undeclared** — invisible to the § 12 harness | Add the § 9 declaration (marker or frontmatter); author nothing | the consolidated TRIM-TO-DECLARE sweep (~27 artifacts across 5 flow types) |
| **BUILD** | No artifact exists for the flow type — the § 2 current-state column is genuinely empty | Author the artifact, declare it, and place it per the audience arm | the `human-process` FULL-BUILD (§ 2 cites zero; harness confirms zero) |

The MERGE-6→2 that produced exactly these two tasks was the operator's **D-FissionScope** decision (2026-07-24), taken on the spike's gap table — a query result, not an estimate. This how-to is what both tasks cite for their shared completion condition.
...
For a pure DECLARE task (e.g. the declare-sweep), every candidate is disposition **(b)**: the completion condition reduces to "every enumerated existing artifact carries a § 9 declaration verified by the § 12.2 harness (per-flow-type count `> 0`), and each declared region PASSES its § 12.1 rule." A candidate that turns out **not** to render its type's tool at all is not silently marked — it is either (i) re-classified and dropped from the type, or (ii) escalated to a BUILD candidate, or (iii) recorded as a known rule-gap when the miss is the *rule's* — the candidate is recorded as a rule-gap + a mechanism fast-follow, **not** forced to pass (see the declare-sweep's completion condition). The disposition class stands; its former standing example does not. The decision-tree `triage-design-rereview.md` table-rendered-logic edge **was** that example, and it is now **closed**: § 12.1 rule 7 gained a table limb, and that region is a declared, conforming `decision-tree` artifact — see the § 12.3 worked verdict naming it. Disposition (iii) is what a rule-gap looks like *while it is open*; this instance is what closing one looks like.

### 2.2 BUILD-task specialization

For a BUILD task (e.g. the human-process build), the candidate list is authored, not marked. Each artifact is **authored → declared → placed**, then verified exactly as (c) above. Placement follows the audience arm (§ 4). "Every existing process has an artifact" becomes testable because the built artifact is declared and the harness now returns it.
...
Backfill artifacts follow § 3 storage + § 4 naming, under the location clause **as corrected pre-spike** on the six build children. Reference — do not re-derive — this corrected arm:
...
**FAIL that is recorded, not forced — and then closed. The retired rule-edge.** `triage-design-rereview.md` renders genuine decision logic as classification tables. Under the pre-amendment § 12.1 **rule 7** (decision-tree) it could not conform, so per the declare-sweep's completion condition it was **recorded as a known § 12 rule-gap + a mechanism fast-follow logged** — not marked-to-pass, not authored-around inside that milestone. The fast-follow has since shipped: rule 7 is table-aware, and that region is a declared, conforming `decision-tree` artifact (§ 12.3 names it as the worked verdict). What generalizes is the discipline, not the instance — a completion condition that cannot be met by the current mechanism is surfaced as a rule-gap, never faked green, and it closes when the *mechanism* changes, never when the *candidate* is forced.
...
- It does **not** author or move any artifact itself — the DECLARE sweep and the human-process BUILD do that work; this file is the completion contract they close against.
...
The issues this how-to is scaffolding for, each paired with a summary noun phrase so the reference survives a renumber (durability-ladder rung 5, confined to this block per `core/standards/reference-durability-standard.md`):

- **#3798** — the DECLARE sweep (the consolidated Wave-2 TRIM-TO-DECLARE task); primary consumer of this completion-condition.
- **#3441** — the human-process FULL-BUILD (the other surviving Wave-2 task); the second consumer.
- **#3614** — the design-artifact identification spike (Deliverable D2); origin of this corrected completion-condition.
- **#3725** — the shipped § 9 declaration + § 12 detection mechanism this how-to verifies against.
- **#3786** — the fission-scope decision thread carrying the spike's gap table (the D-FissionScope MERGE-6→2 record).
- **#3437**–**#3442** — the original six flow-type build children whose completion conditions this how-to generalizes.
- **#1173** — the parent epic (knowledge-corpus umbrella); **#398** — the fission origin.
