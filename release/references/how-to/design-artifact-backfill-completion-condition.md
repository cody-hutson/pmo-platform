...
**FAIL that is recorded, not forced — the known rule-edge.** `triage-design-rereview.md` renders genuine decision logic as classification tables; § 12.1 **rule 7** (decision-tree) is not table-aware and does not conform on it. Per the declare-sweep's completion condition this is **recorded as a known § 12 rule-gap + a mechanism fast-follow logged**, not marked-to-pass and not authored-around inside this milestone. A completion condition that cannot be met by the current mechanism is surfaced as a rule-gap, never faked green.
