...
| `core/ADRs/README.md` | **NEITHER — curated thematic document** | its own curation | Deliberately not an index and deliberately not projected. Registered here so the negative is explicit and the question is not re-opened. Its § Cross-cutting pattern index falls **inside** this same classification and is likewise hand-authored: cluster membership derives from neither an ADR's filename nor its frontmatter (the field set carries no cluster key), and the clusters span both modules while the ADR projector's population is `release/ADRs/` alone — so a projection would render partial clusters, which is worse than none. The section carries an explicit non-completeness claim in place of the drift check a derived surface would get. |
