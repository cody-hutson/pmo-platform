...
| Host-API reading basis | `MEASURED` when the read succeeded; **`UNSTATED`** when it did not; **`CONTRADICTED`** when the read succeeded *and* a real call against a modelled pool refused it on quota grounds. The first two reuse § 6.1's existing basis tokens and the third joins them; all three are *basis* tokens, not verdict tokens, so **the verdict enum is unchanged**. **`MEASURED` records that a read returned — not that the figure is trustworthy.** The two are independent here, and the evidence label (below) is what carries the grade. `CONTRADICTED` is the only one of the three that carries a positive exhaustion signal, which is why § 4.3b exempts it from the fail-open rule |
...
**The seam does not reach presentation (c), and that boundary is stated so the seam is not mistaken for a general answer.** Under a secondary limit **every** resource the endpoint returns reads full simultaneously — all of them, search pools included — so the condition is invisible in every row at once. Adding rows cannot detect a limit that is absent from the response by construction; only an out-of-band instrument can.
...
**Why only two values, and why nothing is minted.** The axis renders from § 4.3a's existing `{PROCEED, DEFER}` pair — **no new VERDICT token is minted**. (§ 4.1 does add a third *basis* token, `CONTRADICTED`; basis and verdict are separate vocabularies and the verdict enum is untouched.) `SERIALIZE` and `REDUCE-scope` are structurally meaningless here for the identical reason § 4.4 gives for STAGGER: they reduce quantities this pool does not meter. Serializing N spokes consumes the same total points; compacting a prompt consumes none fewer. **Narrowing a wave is therefore not a host-API mitigation either** — a depleted pool blocks every spoke equally regardless of how the batch is arranged, because the pool meters requests the work requires, not their concurrency. Route a host-API block to **waiting for recovery**, never to a count-shaped or width-shaped remedy — under (a) that wait is anchored to a published `reset` epoch, under (c) it is a backoff with no published anchor (below).

**DEFER is actionable on this axis in the PRIMARY case — and only there.** When a modelled pool reports genuine depletion, the pools return a **fixed `reset` epoch**, so the hub names an exact recovery time instead of projecting one. Route the § 8 hub-action-tracking entry with that timestamp.

**Under presentation (c) there is no reset epoch to route.** A secondary limit publishes no reset time anywhere in the endpoint's response, so DEFER degrades from *resume at HH:MM* to *retry with backoff*, and the § 8 action-tracking entry carries no timestamp. State that rather than emitting a `reset` read from a pool whose depletion is not the binding constraint — a timestamp copied from an unrelated primary pool is a fabricated recovery time, which is worse than none.
...
**Exhaustion therefore has THREE successful-read presentations, not one.** The earlier text named only the first:
...
| **(c) Secondary limit** | a successful read with **every** resource full and `used = 0`, while real calls on **both** surfaces return `403` / `RATE_LIMIT` for the same user | renders `PROCEED` — **wrong, and unreachable from this endpoint by construction**; the limit in force is not represented in the response at all |

**State the consequence of (b) plainly: in that state the gate renders PROCEED into the exact harm § 4.3b was written to stop.** A spoke launches, spends its full usage-window draw, completes the work, and then cannot post its Issue comment — the deliverable destroyed after it was paid for. The gate does not merely fail to help there; it actively supplies a green reading that a reader will trust. **The same consequence follows from (c), by a different route and with a worse failure profile** — see below. Neither (b) nor (c) is detectable **from this endpoint**: a single read cannot separate a genuinely fresh window from an unstarted-window artifact, and no read at all can surface a limit the response omits. **Treat a host-API `PROCEED` as weak evidence of headroom, never as confirmation of it.**

**(b) and (c) differ in whether a discriminator EXISTS, and that difference decides the remedy.** For (b) the protocol takes no second sample, so the presentation is undischarged pending a mechanism. For (c) the discriminator is known and cheap — **one real call against a modelled pool settles it in a single request**, because the limit that is invisible to the meter is by definition visible to the traffic it refuses. What remains open for (c) is not *whether* it can be detected but *where the probe fires and what it replaces*, which is a mechanism question this prose correction does not close.

**(c) is self-reinforcing, and that is why it is a gate defect rather than a host quirk.** The condition is induced by sustained high-concurrency `gh` traffic — the spoke fan-out this gate exists to govern — and has been observed persisting beyond ten minutes. A wide wave therefore manufactures the blindness that makes the gate render `PROCEED` for the *next* wave. The metered quantity is **requests per interval**, which is neither the usage-window envelope § 4.3 reads nor the interruption exposure § 4.3c's `W_max` bounds; **no axis meters it today**, and saying so is the point — the gap is named rather than implied.
...
**CARVE-OUT — a quota refusal observed on a real call is NOT a probe failure and does NOT inherit fail-open.** The fail-open rule rests on one premise: a fault says nothing about capacity. A real call refused **on quota grounds** inverts that premise exactly — it is a positive, first-hand observation that the resource is unavailable, which is the strongest evidence this axis can hold. Reading such a refusal as "the probe failed" and failing open would spend the window on the strength of the one signal that proves it should not be spent. So the branches separate by **what failed, not by whether something failed**:

| What happened | Basis | Axis verdict |
|---|---|---|
| the meter read itself failed (instrument / auth / network) | `UNSTATED` | `PROCEED` — fail open, unchanged |
| meter read succeeded; no contradicting observation | `MEASURED` | `PROCEED` |
| **a real call was refused on quota grounds** | **`CONTRADICTED`** | **`DEFER` — fail-open does NOT apply** |
| a real call failed for a **non-quota** reason (auth, network, 404) | `UNSTATED` | `PROCEED` — this *is* a probe failure, so fail-open holds |

The last row is the one an implementer gets wrong: not every failed call is an exhaustion signal, and the classifier is the **refusal reason**, never the fact of failure. Together these four give the axis a **distinct emitted state for every reachable state of its predicate**, which is the obligation `review-discipline-principles.md` § 8.1 **PV-7** places on any check, and which this axis did not previously meet — its `PROCEED` was shared by the healthy and the exhausted state. **PV-7b** is the consuming half: branch on the basis before reading `remaining`, because a counter read without its status consumes "nothing was measurable" as "nothing was drawn."

**`CONTRADICTED` does not coin a token PV-7a forbids, and the distinction is worth stating because a reader will reasonably ask.** PV-7a governs the closed *measurement-state* vocabulary — whether a check measured, and how completely. This axis's basis tokens answer a different question: **how availability was established**. On the PV-7a axis every row above is `fetched` or `degraded` and nothing new is needed; `CONTRADICTED` records that a measurement succeeded and returned an adverse finding, which is a *result*, not a degradation. The two vocabularies compose rather than compete, and the axis's own `{MEASURED, UNSTATED}` pair — which predates this correction — already sat in the same relation.
...
**Presentation (c) is a residual of a different shape, and the distinction is worth keeping.** (b) waits on a mechanism nobody has specified. (c) waits only on wiring a discriminator that is already known to work — so its residual is **scoped to the probe's placement and cost**, not to its feasibility. The verdict and basis vocabulary it will render are settled above and need no further design.
...
**What would restore a stronger grade — and one previously-named path is now known to be UNREACHABLE, not merely absent.** A repeated-sampling discipline reconciling disagreeing reads was recorded here as a precondition for grading this axis above `[ASSUMPTION – CONFIRM]`. **It cannot serve as one.** Sampling addresses disagreement *between reads*; it cannot surface a limit the endpoint omits from **every** read, which is presentation (c) in § 4.3b. No number of samples of a response that does not carry the quantity will ever contain it. The correction matters because the unreachable path was the cheaper of the two on offer, and leaving it stated invites an implementer to spend effort where no amount of it can succeed.

What remains open: a host-side guarantee of read consistency (which addresses non-determinism but still not blindness), or an **out-of-band instrument** — a real call whose refusal is observed directly. The second is the only one that reaches all three presentations, and it is the one § 4.3b's `CONTRADICTED` basis is defined for. Until an instrument of that shape is wired, the grade above binds.
