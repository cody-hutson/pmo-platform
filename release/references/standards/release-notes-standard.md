...
The mechanism matters, because getting it wrong reintroduces the blind spot. A version-less note carries **no version at all**, so the forward-only content floor simply does not apply to it: "no version" is not "older than the floor". The lint therefore admits it through an explicit **version-less branch**, keyed on the sentinel the version parser returns for an unversioned name — **not** by lowering the floor. Lowering the floor would additionally admit any *malformed* filename as though it were deliberately version-less, collapsing a distinction the lint needs to keep.
