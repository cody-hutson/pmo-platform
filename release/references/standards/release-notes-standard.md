...
> **Footnote on `<OPERATOR_INSTANCE_LOG_PATH>`:** This path resolves to `<OPERATOR_INSTANCE_RELEASE_LOG_PATH>` until the extraction lands. Post-extraction, it resolves to the operator-instance path declared in that release (typically `~/Claude/pmo-instance/RELEASE_LOG.md`). The path-shift is a one-time event captured in the release plan deviation log; pre-extraction readers MUST use the in-repo path, post-extraction readers MUST use the operator-instance path.
