...
- Path, version-less release: `release/releases/notes/_unversioned/<slug>_RELEASE_NOTES.md` — a release that ships with Tag `(none)` has no version stem to name, so its note is written to the `_unversioned/` bucket
- One file per release, no patches
- Filename version matches the Milestone version exactly (e.g., `v1.03_RELEASE_NOTES.md`)
- Notes are flat in `releases/notes/`. The one permitted subfolder is `_unversioned/`, which holds the notes of version-less releases; it is written by `release/tools/automated-closeout.sh` `notes_rel_path()` and mirrors `plans/_unversioned/` and `releases/registers/_unversioned/`. **No other subfolder is permitted** — in particular, notes are not sharded by major version (that scheme governs `releases/plans/` only, per ADR-092).
- **PLAN-filename note:** the corpus linter (`core/deploy/tools/lint_release_corpus.py` `CANONICAL_FILENAME_RE`) admits the three-component patch form `vX.Y.Z` for a release **PLAN** filename (e.g., `v3.65.1_RELEASE_PLAN.md` for a hotfix that carries its own plan). This concerns the **plan** filename only — it does **not** alter the "one note file per release, no patches" **notes-content** policy above: a hotfix emits no separate `_RELEASE_NOTES.md` file.
...
<!-- agent: SECTION 6b — Layer B (Operator and engineering detail).
  ...
  - Schema/format-changing release (§2.8): carry the fenced sample block here — one
    sentence naming what it shows, then the fence. When the shape genuinely cannot be
    illustrated, carry the §2.8 `sample-block: n/a` HTML-comment marker instead.
...
| Sample block for a new or changed schema / enum / format / taxonomy (§2.8) | 6b (or its own section after 6a when the shape is user-authored) |
...
The list grows over time. Additions require operator approval; deletions require explicit justification (the term has become unambiguous to all Layer A readers). Every row above is either mapped to at least one of the patterns §3.2 check 10 enforces, or is a deliberate exclusion — and a deliberate exclusion is **both** registered in the lint with its reason **and** marked here, in the row's own second cell, which begins with the literal `NOT ENFORCED` followed by the reason it is not enforced. Registration alone is not enough, because a registry entry is visible only to someone reading the lint's source, which is the disagreement this table's reader cannot see and the reason the pairing is graded at all. To find out whether any row is excluded right now, read the table above — an excluded row says so in its own second cell, which is the whole point of marking it there.

The lint's `--self-test` fails when a row is neither mapped nor excluded; when a registered exclusion is not marked here; when a row marked here is not registered; when a marked row states no reason after the marker; and when a pattern does not fire on the specimen the lint holds for it. Those five are what its arms assert about this table, and two things they do **not** assert are worth stating plainly. It does not compare the reason written here against the reason written in the lint — both are required to be non-empty, neither is required to say the same thing as the other. And it never tests a row's own published words: each specimen is written in the lint, separately from this table, so a pattern and a specimen written from the same mistyped term both pass while the row as published here is not enforced. When adding a row, confirm by hand that its pattern matches the row's own words.
...
### 2.8 Sample block for schema/format changes

**A release is *schema/format-changing* when it adds or changes a shape contract someone must produce or parse** — a schema, an enum, a taxonomy, a file format, a frontmatter field set, or a template's required structure. Renaming a file, editing prose, or adding a gate does not make a release schema/format-changing; adding a field, adding a value to an enum, or introducing a new machine-read file shape does.

**A schema/format-changing release MUST include a fenced sample block in its release note** — one short, realistic instance of the new or changed shape, so a reader sees what it looks like instead of inferring it from prose.

**Placement.** The sample block belongs in **Section 6b (Operator and engineering detail)**, immediately after the theme paragraph that introduces the shape. When the shape is one a Layer-A reader authors or reads directly (a user-editable file's frontmatter, a user-visible enum), the block MAY instead sit in its own short section placed after Section 6a and before Section 6b. It MUST NOT appear inside a Section 6a bullet — a fence at column zero terminates the bullet that the "Why it matters" check scans (§3.2 check 11).

**Shape.** One sentence naming what the block shows, then the fence. Keep the block under about a dozen lines, and keep markdown links out of it (§3.2 check 13 scans the whole published body, fenced content included).

**Worked example** — one entry of a newly-added taxonomy, carried in Section 6b:

```yaml
parallelism_posture:
  id:      string                                  # required, kebab-case
  scope:   enum{serial, file-partitioned, parallel}
  applies: [stage-06]                              # required, at least one stage
```

**When there is nothing to show.** A schema/format-changing release whose shape genuinely cannot be illustrated (an internal contract with no reader-facing instance) carries the marker `<!-- sample-block: n/a — <one-line reason> -->` in the note body instead. Per the fabricate-or-omit rule (§2.5 Rule 1), an honest marker beats a decorative block. The marker follows the same escape-hatch convention as the `<!-- impact:foundational -->` marker in §2.2.

**Why this is not auto-detected.** Whether a release is schema/format-changing is **declared, not detected**. Detection from the note itself was designed, measured against the whole notes corpus, and rejected: every candidate signal either missed the release that motivated this rule or fired on roughly half the corpus, because a note that omits the sample block also omits the vocabulary that would betray the omission. The signal and the defect are negatively correlated by construction, so no regex on the note can be both sensitive and precise. The authoritative declaration is the release plan's File Change Matrix, which the closer already reads to author the note. This is a closed question, not a deferred one.
...
The §3.2 lint runs on **any Stage-13 close that authors a note, regardless of authoring path** — bound to the close *event*, not to a single skill. It runs via `lint_release_corpus.py --check note-content` on all three close paths: [release-executor Mode E](../../skills/release-executor/SKILL.md) (a pre-presentation self-lint, advisory), the [`automated-closeout.sh`](../../tools/automated-closeout.sh) close phase `phase_lint_release_notes` (the authoritative in-script close gate, inserted between note-scaffold and the chore commit), and the pure-hub-direct / Phase-B chore-PR close (verified at [`hub-spoke-bridge.md`](../how-to/hub-spoke-bridge.md) Procedure 7 Step 4 cmd #1b, which runs no script). On every path, **a finding for the release version blocks Milestone close** — the block fires before the milestone-close action (in-script on the script path, at the Step 4 gate on the hand-assembled path). A pre-existing finding for a *different* version does not block this close (audit-baseline discipline). The same structural checks 9-13 are ALSO run post-hoc, corpus-wide, by [deploy.sh Check 20](../../../core/deploy/deploy.sh) as a CI regression catch (warn-mode-initial); check 14 is advisory and Stage-13-invoked only — it is not part of the corpus-wide run. Note-presence drift across released versions is enforced by [deploy.sh Check 26](../../../core/deploy/deploy.sh) (per Check 26 AC#3). The Must-Have Checklist below is the content reference for all paths (also expressed as the [release-notes eval rubric](../../../core/skills/eval-writer/references/release-notes-eval-rubric.md)).
...
10. **Banned-jargon scan.** Section 6a content (between the section header and the next `##` header) contains no entries from the §2.4 banned-jargon list. Case-insensitive match. **The enforced set is not restated here.** §2.4 is the published list; the lint holds the enforcing patterns — some as a literal substring, some as a bounded regex, because a parameterized row such as `schema vX.Y → vX.Z` has no literal form. The lint's own `--self-test` asserts that the two agree row for row: a §2.4 row with no enforcing pattern, or an enforcing pattern with no §2.4 row, fails the self-test, and a row deliberately left unenforced is registered in the lint with the reason it is not **and** marked as unenforced in the §2.4 row itself — doing only one of the two fails the self-test, in either direction. A restated count here would be a third holder of the same value with nothing asserting the three agree — which is how the previous count came to be wrong. Failure → lint error citing the matched term and the §2.4 plain-language replacement. **Finding cardinality:** one finding per matching deny-list pattern, not one per span — each row carries its own §2.4 replacement and the author needs every one that applies, so a span violating two rows yields two findings; and one finding per pattern per note, not one per occurrence.
...
14. **Schema/format sample block (advisory).** For a release **declared** schema/format-changing (§2.8), the note body carries at least one fenced block positioned outside the Section 6a bullet span, or carries the `<!-- sample-block: n/a — <reason> -->` marker. **Advisory: this check never contributes to the lint's blocking exit status and never blocks Milestone close.** It is **not** auto-detected and therefore **not** run corpus-wide by Check 20 — it runs only when the Stage-13 closer passes the declaration explicitly, sourced from the release plan's File Change Matrix: `lint_release_corpus.py --check note-content --sample-block-advisory v<version>`. The rationale for the declared trigger, and the measurement that rejected auto-detection, are in §2.8.
...
**Version-less notes are IN SCOPE for checks 9-12.** A release note whose filename is keyed by a slug rather than by a version — the form the close-out tooling itself writes for a version-less release — is a canonical release note and **is** linted by checks 9-12. It is not exempt, and a "0 findings" result for such a note means *validated*, not *not-scanned*.

The mechanism matters, because getting it wrong reintroduces the blind spot. A version-less note carries **no version at all**, so the forward-only content floor simply does not apply to it: "no version" is not "older than the floor". The lint therefore admits it through an explicit **version-less branch**, keyed on the sentinel the version parser returns for an unversioned name — **not** by lowering the floor. Lowering the floor to that sentinel would additionally admit every note whose filename *does* parse to a real version sorting below the cutover: a `v0.5`-keyed note parses to `(0, 5, …)` and is correctly out of scope today, and a lowered floor would pull it in. That is the distinction the branch keeps, and it is a distinction about **version ordering**, not about filename well-formedness. A malformed name and a deliberately slug-keyed name both parse to the *same* sentinel and are therefore both **in scope** under either arm — the branch neither makes nor needs a malformed-vs-version-less call. Filename well-formedness is not this branch's concern — and for a **note** it is currently no check's concern: the canonical-filename rule has a single call site and it walks the **plans** directory only. A malformed note filename is linted for *content* like any other in-scope note, while its *shape* goes unchecked. That gap is pre-existing and is tracked separately; it is not what the version-less branch exists to address.

Discovery must match the same population. The note-discovery rule keys on the `_RELEASE_NOTES.md` suffix — the corpus's own type discriminator, and the same token the canonical-filename rule anchors on — so filename authority and discovery agree by construction. A discovery rule keyed on a leading version character silently excludes every slug-keyed note while the filename rule still calls those files canonical, which is a lint that declares a file in-scope and is then structurally unable to read it. The discovery rule is stated **layout-independently**: it names no bucket directory and no major-version directory, so it holds regardless of how the notes tree is arranged.

Version-less notes remain **exempt from check 13** (whole-body link purity), and the exemption is deliberate rather than incidental. They are historical artifacts of the same class the check's forward-only floor exists to grandfather — their Section 6b links are the legacy repo-relative template form the floor was introduced to stop *going forward* without retroactively failing the notes that already carry it. This is the same accepted-residual rationale the check's named exempt set encodes for the older lineage whose version tuples sort above the floor but which shipped before it.

Check 14 needs no exempt set: it is flag-gated, so it can only evaluate a release whose closer declared it — there is no historical population to grandfather. It takes effect from its introducing release onward; that release is itself exempt as a gate, though it may satisfy the mandate voluntarily as live-fire evidence.

### 3.3 Eval rubric

The note is "verifiably helpful" per the research grounding. A binary pass/fail eval rubric owned by the [eval-writer skill](../../../core/skills/eval-writer/SKILL.md) scores notes against the Must-Have Checklist; the rubric file lives at [release-notes-eval-rubric.md](../../../core/skills/eval-writer/references/release-notes-eval-rubric.md). The rubric covers all 14 lint checks (existing checks 1-8 + checks 9-12 added at the cutover release + check 13 whole-body link purity + check 14 schema/format sample block). Check 14 is advisory — the rubric scores it and reports a FAIL, but it does not contribute to the lint's blocking exit status. [release-executor Mode E](../../skills/release-executor/SKILL.md) invokes the rubric pre-presentation per Mode E Step 4. Calibration corpus is the set of post-cutover notes under `release/releases/notes/` (per the `PRE_CUTOVER_EXEMPT_VERSIONS` set in `lint_release_corpus.py`).
...
|---|---|---|---|---|---|---|---|
| 1 | **GitHub Releases** | `https://github.com/<owner>/<repo>/releases/tag/v<X.Y>` (the published Release object; emit invocation per §5.5/§5.6 — never `--notes-file`) | Anyone with repo read access (public after Phase 4 public-flip) | **Canonical public surface** — discoverable via the Releases tab; queryable via `gh release list` | Public post-flip (private pre-flip) | Full (mirrors Sections 1–8 of `vX.Y_RELEASE_NOTES.md`) | GitHub-flavored Markdown; release-tag metadata + tag-target SHA |
...
**Frontmatter handling (Surface 1).** Surface 1 publishes the note's **body** (Sections 1–8), not the file verbatim — the leading YAML frontmatter is stripped so the machine metadata does not render on the Releases UI. The committed `vX.Y_RELEASE_NOTES.md` is the source of record; the Release page is the rendered copy people read. The emit therefore uses `--notes "$BODY"`, not `--notes-file <path>`.

**The transform has one implementation, and these are its semantics.** `strip_frontmatter()` lives in `release/tools/lib/frontmatter-strip.sh` and is sourced by every shell tool that derives a published body; two Python mirrors (`release/tools/preflight-release-body-reemit.py`, `core/deploy/tools/lint_release_corpus.py`) model it for prediction and linting. All three are bound to one committed conformance fixture at `core/deploy/tools/fixtures/frontmatter-strip/`, so a divergent reimplementation fails a test rather than publishing quietly. The frozen semantics:

| ID | Rule |
|---|---|
| **S1** | Any lead-in before the opening `---` fence is dropped. (A note carrying a lint directive above its frontmatter would otherwise publish its raw YAML — three shipped notes did.) |
...
| **S4** | Fewer than two `^---$` lines anywhere yields an **empty** body — fail-CLOSED. Every publishing caller MUST refuse an empty result rather than forward it: `gh release edit --notes ""` blanks a published Release body, and GitHub keeps no body history for a Release, so no revert recovers it. |
| **S5** | The fence match is exact `^---$`. A fence carrying trailing whitespace does not close the block. |

Do **not** re-implement this transform inline in a new caller, and in particular do not reach for the `sed`-address form this library replaced: that form yields an empty body on GNU userlands while returning the body on BSD, which is how a Stage-13 drift gate came to report "no drift" because it was comparing nothing against nothing.

**Enforced deterministic emit (Surface 1 is never hand-assembled).** The published Release body MUST be the deterministic frontmatter-strip transform of the in-repo source-of-record note (`strip_frontmatter(vX.Y_RELEASE_NOTES.md)`) — never an ad-hoc draft string passed to `gh release create`. This is not a convention but an enforced invariant on a single mechanism: every emit path derives the body from the note by the same transform. The script path (`automated-closeout.sh` `phase_publish_github_release`) and the Stage-12 spoke emit both apply the shared `strip_frontmatter()` transform to the note; neither composes a body by hand, and neither carries its own copy of the transform. A standing drift gate verifies the invariant holds after the fact — `published Release body == strip_frontmatter(in-repo note)` — implemented once in `release/tools/check-release-body-drift.sh` (the single source of the equality logic) and surfaced by `deploy.sh` Check 47 (an **enforcing** standing audit gate — a divergence FAILS the check and counts as an issue, it is not logged and waved through; gh-guarded — offline ⇒ N/A) and by `automated-closeout.sh`'s post-emit verification phase, where a genuine divergence **blocks the close**: the canonical note is corrected first, then every surface re-emits from it per §5.6. The drift gate is **detective-only**: it flags a divergence, it never re-emits (auto-remediation would raise an autonomy-tier decision). Read *detective-only* as a statement about **remediation, not severity** — the gate detects and reports rather than repairing, and it does so in a failing, close-blocking posture; the two properties are independent, and collapsing them would misread the gate's autonomy tier. Both surfaces read the same cutoff variable (`RELEASE_BODY_DRIFT_CHECK_CUTOFF`), so the standing gate and the close gate can never disagree about the same release: a release outside the cutoff's range is reported without failing, and an operator can dial the standing gate down locally (or disable it via the cutoff's documented opt-out sentinel) without editing this standard. Why an enforced transform rather than a prompt-level mandate: a chip-prompt instruction to "emit from the note" does not *guarantee* the emit — the canonical incident (the v2.26 close) shipped a hand-assembled body that diverged from the in-repo note and stayed stale through a note-correction PR, undetected until the operator caught it. Binding the body to the transform on the mechanism, plus a standing detective gate, closes that gap; the prompt mandate alone did not.

> **Footnote on `<OPERATOR_INSTANCE_RELEASE_LOG_PATH>`:** This path resolves **in-repo today**. ADR-032 Decision 1 assigns this surface to the operator-instance home and still binds, but that ADR deferred its corpus migration at authoring and ADR-141 records that none is scheduled — so readers MUST use the in-repo path. Should a corpus migration ever execute, the path-shift is a one-time event captured in that release's plan deviation log, after which readers use the operator-instance path (typically `~/Claude/pmo-instance/RELEASE_LOG.md`).
...
| `<OPERATOR_INSTANCE_RELEASE_LOG_PATH>` (3) | Append row + visible-H4 Deployment Log block per [`stage-12-execute.md § Phase B5`](../pipeline/stage-12-execute.md) emit format and Stage 13 chore PR `DEPLOYED → VERIFIED` transition. **Already-codified mechanism;** dual-write does not modify it. |
...
| **`<OPERATOR_INSTANCE_RELEASE_LOG_PATH>`** | Full audit — ~50-100 lines per release (existing convention) | Tabular row + visible-H4 Deployment Log block (existing format) | Cross-references the canonical note path; does not duplicate body content |
...
|---|---|---|---|
| **State 0 — no release for tag** | `gh release view v<X.Y>` returns "release not found" (exit code 1) | `gh release create v<X.Y> --notes "$BODY" --title "vX.Y — <headline>" --target <merge-sha> --latest="$LATEST"` where `BODY="$(strip_frontmatter <canonical-note-path>)"` — the frontmatter-stripped note body per the §5.1 enforced-transform invariant, NOT `--notes-file <canonical-note-path>` (which would publish the YAML frontmatter as raw text). **Refuse to publish an empty `$BODY`** (S4). On success → State 2; on transient failure (network / 5xx) → retry once → on second failure → HALT and escalate Tier 2 [SCOPE CHANGE] per [`release-process.md § Inter-Stage Feedback Protocol`](../../governance/release-process.md) | release present at desired content; auditable via `gh release view` |
| **State 1 — release exists; content may differ** | `gh release view v<X.Y> --json body` returns body content (any value) | Compare returned body against canonical note body (excluding frontmatter). If MATCH → State 2 PASS no-op. If DIFFER → `gh release edit v<X.Y> --notes "$BODY"` where `BODY="$(strip_frontmatter <canonical-note-path>)"` — the same frontmatter-stripped body per the §5.1 invariant, NOT `--notes-file <canonical-note-path>` (idempotent) → State 2. **Refuse to publish an empty `$BODY`** (S4): this is the one irreversible step in the sequence — an empty edit blanks a live Release body with no history to restore from | release present at desired content |
...
**`--latest` MUST be resolved explicitly on create, never omitted — and this clause is the single canonical home of that rule.** The other construction sites (`automated-closeout.sh` `phase_publish_github_release`, the Stage-12 chip pattern in `hub-spoke-bridge.md`, `stage-12-execute.md § Phase B5.5`, and `release-executor` Mode F) **cite this clause rather than restating the policy**. Without the flag, `gh` applies `make_latest=legacy` — *"automatic based on date and version"* — a vendor heuristic the platform neither pins nor tests, so publishing a **lower** version after a higher one can move the repo's **Latest** badge backwards. That badge is not cosmetic: it is the anchor read by `core/hooks/notify-version-skew.sh` **and** by `deploy.sh` Check 39, so a regressed badge silently re-bases every workspace's version-skew comparison.

**The policy is fail-closed, and its executable form is single-sourced.** Resolve the flag before the state branch from the version-grammar SSOT, never by re-rendering the comparison:
...
`version_badge_latest` returns `ADVANCE` only when the anchor is **not higher** than the target; **every** other verdict (`WITHHOLD_HIGHER`, `WITHHOLD_UNORDERABLE`, `WITHHOLD_NO_ANCHOR`) withholds the badge. Both operands are gated inside the helper, so a non-canonical value can never reach the comparison and be read as "not higher". Withholding is the correct default because the two errors are not symmetric: a badge we decline to take is recoverable with one `gh release edit --latest`, while a badge taken wrongly regresses every consumer silently — same recovery cost, opposite detectability. Record the resolved value and the anchor it was computed against alongside the emit.

**Do NOT add `--latest` to the State-1 `gh release edit`.** `create` documents a default and therefore needs an explicit value; `edit` documents none, so omitting the flag leaves the badge untouched. A flag there converts a safe idempotent refresh into a badge mutation on every re-emit.
...
**Three-surface re-emit rule (the canonical contract).** A post-VERIFIED note correction re-emits exactly the surfaces that derive their prose body from the note, in source order:

1. **In-repo note (the source)** — corrected first; every other surface derives from it.
2. **Surface 1 (GitHub Release body)** — re-emitted **always** (it is the full mirror of the note body).
3. **Surface 2 (CHANGELOG.md)** — re-emitted **conditionally**, only when the correction touches the user-visible Section 6a bullets.

**Surface 3 (RELEASE_LOG.md) is NOT re-emitted by design** — it recorded the deployment *event* (timestamp, SHA, tag, deployed files), not the prose body, so a prose correction leaves it untouched. (Note: the issue framing's "all three surfaces" resolves to {note + Surface 1 + Surface 2}, NOT the standard's Surface 1/2/3 trio — RELEASE_LOG is deliberately excluded. The two "threes" are different sets; this rule names the correct one so the collision cannot recur.) The standing drift gate (`deploy.sh` Check 47 / `release/tools/check-release-body-drift.sh`) is the **verifier** that the Surface 1 re-emit actually landed — after the correction PR merges, the gate confirms `published Release body == strip_frontmatter(corrected note)`.
...
2. **Re-emit Surface 1 (GitHub Releases).** After PR merge, re-emit the Release body as the §5.1 deterministic transform of the corrected note — source `release/tools/lib/frontmatter-strip.sh`, then `gh release edit v<X.Y> --notes "$(strip_frontmatter release/releases/notes/vX.Y_RELEASE_NOTES.md)"` against the existing release object, refusing an empty result per S4. In practice, prefer `release/tools/reemit-release-bodies.sh`, which already carries that guard. Use `--notes "$BODY"` (the frontmatter-stripped body), NOT `--notes-file <path>` — `--notes-file` would publish the YAML frontmatter as raw text, violating the §5.1 enforced-transform invariant. This invocation is idempotent (see §5.5 State 1 → State 2 transition); no view-then-edit dance required because the release definitely exists. (`release-executor` Mode F exposes the same transform for a standalone re-emit.)
...
5. **Verify the re-emit landed.** Run `release/tools/check-release-body-drift.sh v<X.Y>` (or `deploy.sh --check` Check 47) — it asserts the published Release body now equals the frontmatter-stripped corrected note (exit 0 = clean; exit 1 = the Surface 1 re-emit was missed or stale). The check is detective-only; it confirms the re-emit, it does not perform it.
...
- [ ] Schema/format-changing release (§2.8) carries a fenced sample block outside Section 6a — or the `<!-- sample-block: n/a — <reason> -->` marker
...
- [release/references/pipeline/stage-13-close.md](../pipeline/stage-13-close.md) — adds note-output to the Stage 13 checklist; Part 5 Surfaces 2+3 emit anchor (pipeline-shard codification owned by the sibling ticket); Phase A8.1 invokes the §2.8 sample-block advisory at close
