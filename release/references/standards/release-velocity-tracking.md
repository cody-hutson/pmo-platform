...
|---|---|---|---|
| **planned pts** | Sum over the milestone membership of `size:*` → points at the Stage 3 Bundle commit (membership when the milestone was bundled) | integer `<P> pts` | "Planned" = the bundle scope committed at Stage 3. Uses the EXISTING point scale (§ 3.1), not a new one. |
| **delivered pts** | Sum over the issues actually closed-in-release of `size:*` → points | integer `<D> pts` | "Delivered" = membership that actually shipped (closed at Stage 13). Deferred/dropped issues fall into planned-not-delivered. |
...
- `2` — malformed source (a `size:` label outside the closed `XS..XL` set, or a milestone that does not resolve — source-integrity violation; escalate)
...
| FM4 | **OUT** | Committing the velocity field direct-to-main, or at Stage 12 | When adding the `**Velocity:**` field, do NOT commit the visible-H4 edit directly to main, and do NOT emit it at Stage 12 — it lands via the Stage 13 chore PR | Direct-to-main is prohibited regardless of edit size (the git-workflow "What NOT To Do" rule does not exempt metadata edits); and the **delivered** + **allocation** halves are not authoritative until Stage 13 marks the membership closed, so a Stage-12 emit would record a premature (often zero-delivered) value | § 3.3 + § 11 land the field at the Stage 13 chore PR (same commit as the `DEPLOYED → VERIFIED` transition and the outcome field); the Stage-12 emit template stays cycle-time-only with a forward-note | Principal: appends the field in the Stage 13 chore PR after the membership is closed. Junior: emits velocity at Stage 12 with delivered=0 (membership not yet closed) → records a misleading 0.00 ratio that the recalibration then reads as a systematic under-run |
