...
|---|---|---|---|
| **planned pts** | Sum of `size:*` → points over the **delivered** set PLUS every member dispositioned out of the bundle: those still on the milestone carrying a terminal not-delivered `status:` label, AND those `stage-13-close.md` Phase A2 removed from the milestone on the way out, recovered from their `demilestoned` timeline events | integer `<P> pts` | "Planned" = the bundle scope committed at Stage 3. Uses the EXISTING point scale (§ 3.1), not a new one. **The Phase-A2 recovery is not optional bookkeeping.** Phase A2 applies `status: deferred` *and then removes the milestone*, so a `gh issue list --milestone` read cannot see the deferred member at all — planned would silently shrink to equal delivered and the ratio would read `1.00` on every governed close, feeding a constant into the § 6 recalibration. **Known bounds — the recovery is best-effort in BOTH directions.** The join is keyed on the milestone **title** (the `demilestoned` event payload carries no stable milestone id) and is **temporally unbounded** (any demilestone naming that title counts, whenever it happened). It therefore **under-reports** when a member demilestoned at Phase A2 is later re-bundled elsewhere — it loses its terminal label, leaves the scanned population, and is not recovered — and when the event's title no longer resolves to a live milestone, in which case the fixed-string join can never fire and a zero-recovery outcome is indistinguishable from "nothing to recover". It **over-reports** when a member was milestoned mid-release for provenance and demilestoned long *after* the release closed: nothing distinguishes that from a disposition at this release's Phase A2, so its points are returned to a bundle it was never committed to. `planned` is therefore **not a guaranteed bound in either direction**; closing both gaps needs the Stage-3 membership snapshot the platform does not yet take. **Reachability:** measured at a release's own Stage 13 the demilestone and the close are the same step, so the divergence is not reachable in practice — it bites on *recomputation of a historical row*, where the elapsed window admits later demilestones. |
...
The allocation actuals require mapping each delivered issue's labels onto the three work-classes the 60/20/20 mix names. This map is the single genuinely-new classification this standard introduces; it is grounded in the category-label and cluster-label sets defined at the platform label taxonomy (`core/specs/label-taxonomy.md`).

| Signal tier | Resolves from | Work-class | 60/20/20 role |
|---|---|---|---|
| **T1** declared category / cluster | `enhancement` | **feature** | the ~60% feature third |
| **T1** declared category / cluster | `protocol`, `cluster: process-protocol`, `routing-rules`, `tracker-schema` | **protocol-slack** | the ~20% protocol/process third |
| **T1** declared category / cluster | `bug`, `structure`, `skill-update`, `documentation`, `cluster: architecture` | **debt** | the ~20% debt third |
| **T2** declared work-item kind | `type:<kind_id>` for any kind the **selected** pack set declares — corpus packs *and* the deployment's operator-local (K4) packs | **feature** | the ~60% feature third |
| **T3** stated default | anything else | **debt** | the ~20% debt third |

**Every T1 token above is a declared `[[labels]]` row in the selected pack set — that is a checkable property, not a convention.** Three tokens were retired from this map at the v4.x re-derivation because they were *phantom*: named by the map, present in no live label set and in no pack row, so their arms read as coverage while covering nothing. They were `type:feature`, `feature`, and `cluster: tech-debt`; the live `type:*` set has no `feature` member, and the live cluster set has no `tech-debt` member. Retiring `cluster: tech-debt` does not change any issue's class — a `cluster: tech-debt`-labelled issue still resolves **debt**, now at T3 rather than through a dead arm. The producing tool asserts every surviving T1 token against the corpus pack set in its self-test, so the class cannot silently return; § 13 **FM6** names the failure mode.

**T2 is derived at run time, never enumerated here.** The declared-kind set is resolved by `check-label-parity.py --list-declared-kinds` over the selected pack set — one resolver, shared with the label-parity gate, so the two instruments cannot disagree about what a declared kind is. A kind a deployment adds to its own pack therefore resolves with **no edit to this standard and no edit to the tool**; a `type:*`-shaped label that no selected pack declares resolves at T3, so the derivation is not over-broad. When the resolver is unavailable the tool announces a degraded measurement on stderr and falls back to T1 + T3 — it never silently drops the tier, because a dropped kind tier *under*-reports the feature bucket and reads as a healthier mix than the truth.

**Resolution precedence (a multi-labelled issue resolves to exactly one class): T1 → T2 → T3.** Within T1 the order is unchanged: an explicit **feature** signal wins; then a **protocol-slack** signal; then a **debt** signal. **Default = debt** — an un-feature, un-protocol delivered issue is treated as debt-paydown, never silently dropped, so the three buckets always partition the delivered points (they sum to delivered exactly). This default-to-debt rule is conservative (it never inflates the feature third) and keeps the allocation reproducible rather than ad-hoc.

**Why the kind tier sits BELOW the category tier and not inside it.** A `type:<kind_id>` whose name is co-extensive with a live category row is *the same assertion at two altitudes*, not two competing signals — `core/specs/label-taxonomy.md` § Work-Item-Kind Labels states the binding, and Rule 1 carries it as a structural exception. So the category altitude must resolve first, or the projection contradicts its own parent. Folded into the T1 feature arm instead, a kind signal would invert every bug on any deployment that declares a `bug` kind: `bug` + `type:bug` would resolve **feature**, because the feature signal wins precedence. Precedence alone does this work — no runtime read of the category facet is needed, because wherever a kind name collides with a category row that row is present on the issue and resolves at T1. Recorded as **ADR-173**.

**Why `improvement` is not itself a work-class signal.** `improvement` is the most common label on the backlog and it is deliberately absent from every tier above. It is **template provenance, not a content class**: `.github/ISSUE_TEMPLATE/improvement.yml` carries a *required* Category dropdown offering seven options — Skill Update, Protocol, Structure, Documentation, Enhancement, Tracker Schema, Routing Rules — and `Improvement` is **not among them**; the template applies no category label at submission, and Triage applies the chosen category at CER Resolve (`core/specs/label-taxonomy.md` Rule 1). An `improvement`-carrying issue therefore already declares its content class through a *different* label, and treating the provenance marker as a class would double-count. What the v3.100 reading actually exposed was the absent T2 tier, not an absent `improvement` arm: an `improvement`-labelled **story** now registers as feature allocation through `type:story`, and `improvement` alone still correctly resolves nothing.
...
**Form factor:** a thin wrapper mirroring the `compute-cycle-time.sh` form factor and exit-code contract. It is stdlib-only (PATH pinned to system tools per the bypass-mode-readiness portability tier), takes `<version> --milestone <N>`, and emits the field value sourcing (a) `size:*`-derived points from the milestone membership via `gh` issue labels, (b) files-changed via `git diff --shortstat <merge-base>..<merge-SHA>`, (c) the declared Release Class from the milestone description, (d) the allocation split from `type:`/`cluster:` labels via the § 4 map. It ships a `--self-test` (point scale, round-half-up, work-class precedence, allocation-partitions-delivered invariant; the delivery predicate — both directions, spaced-**and-cased** label match fidelity, exclusion-constant shape, close-state independence carrying the pre-fix close-state rule as a firing sensitivity arm, non-degeneracy, and the allocation partition across an exclusion; and the **Phase-A2 candidate-selection pass**, run offline on synthetic JSON with a firing control per filter and a vacuity control on its completion sentinel) and a `--json` detail mode. The self-test does **not** cover the recovery's `demilestoned` title join — that needs the events API.
...
| FM6 | **PROC** | Mapping a work-class signal to a token that is not a declared pack row | When adding or retaining a token in the § 4 T1 tier, do NOT keep an arm naming a label that does not exist in the selected pack set — confirm the token is a declared `[[labels]]` row first, and retire it if it is not | The map is hand-maintained while the label set is pack-declared, so the two drift independently. A retired or never-existing label leaves a **dead arm that reads as coverage**: the map looks like it handles the case, the arm can never fire, and the issues it was supposed to catch fall silently to the default. The failure has no signal of its own — a dead arm produces no error, only a quieter bucket — which is why three phantom tokens survived here across the full pre-`v4.x` ledger | § 4 states the declared-row property as checkable rather than conventional; the producing tool's self-test asserts **every** surviving T1 token against the corpus pack set, with a firing control on a known-retired token, so a phantom arm is a pre-merge failure rather than a silent drift. T2 sidesteps the class entirely by deriving from the pack source at run time instead of enumerating | Principal: removes the dead arm and adds the assertion that keeps it dead, then checks whether the *value* changed (a phantom whose issues still reach the same class via the default is a cleanup; one whose issues change class is a measurement correction that must be stated). Junior: reads the dead arm as evidence the label exists, adds the missing label beside it, and grows the enumeration — converting a documentation defect into a live taxonomy defect |
