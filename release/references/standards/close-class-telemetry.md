...
| 6 `phase_completion_evidence_preservation_rate` | **BUILD (read-model)** | Reuse-first: the hub-spoke sub-task `gh` state IS the per-phase evidence trail — no net-new store. CLOSED stage sub-tasks carrying a trusted-authored (output / skip-closure) comment ÷ the **measurable** stage sub-tasks scaffolded for the release (terminal stages 12/13 excluded — capture happens *during* them; § 4 Indicator 6) is a real `gh`-queryable preservation rate; the G-CL4 close-gate boolean is retained as a sub-signal. See § 4 Indicator 6. |
...
| **evidence-preservation** (+ retained **evidence-close-gate**) | 6 | reuse-first read-model over the hub-spoke sub-task `gh` state — CLOSED stage sub-tasks carrying a trusted-authored (output / skip-closure) comment ÷ the **measurable** stage sub-tasks scaffolded for the release (terminal stages 12/13 excluded — § 4 Indicator 6); the single Stage-13 G-CL4 close-gate boolean is retained as a sub-signal | `<P>/<S> (<ratio>)` for evidence-preservation; `pass` / `fail` / `N/A` for evidence-close-gate | Rate is primary (a real `gh`-mechanical denominator — no net-new store); the retained boolean loses no signal. N/A when the release scaffolded zero stage sub-tasks, or when every scaffolded sub-task is terminal-stage. |
...
Worked example (a release whose retro fully conformed — all 10 canonical-form markers present — with 8/10 lessons rows populated, 2/3 carry-forwards since closed, rollup present, 12 of 13 **measurable** stage sub-tasks carrying preserved evidence — the two terminal Stage-12/13 sub-tasks held out of the denominator per § 4 Indicator 6 — and the close-gate passed):
...
**Definition:** of the release-execution phases the release scaffolded **whose evidence is terminal at the § 3.2 capture moment**, what fraction preserved their completion evidence. **Disposition:** **BUILD as a bounded read-model — NO net-new store.**
...
**Source:** `gh issue list --milestone <N> --label "sub-task" --state all` (the same `gh`-state read class as Indicator 3, NOT the event log — § 8). **Denominator S** = the **measurable** stage sub-tasks scaffolded for the release — the scaffolded set **less the terminal stages 12 (Execute) and 13 (Close)**, per the capture-moment scope rule below. **Numerator P** = those measurable sub-tasks that are **CLOSED and carry ≥1 trusted-authored comment** (the hub's output or skip-closure comment; "trusted-authored" = comment `authorAssociation` ∈ {OWNER, MEMBER, COLLABORATOR, CONTRIBUTOR}, guarding a public drive-by comment on a closed sub-task from counting; presence fallback when the payload carries no author data). Comment-presence guards a closed-but-empty sub-task from counting as evidence. **Rate** = P ÷ S (round-half-up, § 3.1). The field emits `evidence-preservation <P>/<S> (<ratio>)`; the `--json` detail mode additionally discloses `terminal_excluded` so a reader can reconcile S against the raw scaffolded count without re-deriving the rule.

**Terminal-stage exclusion — why S is not the raw scaffolded count (load-bearing).** § 3.2 mandates capture **at the Stage-13 Phase B chore PR** — that is, *during* stage 13, with stages 12 and 13 not yet at terminal state. Counting them in S makes the published number a function of **when it was read** rather than of how well evidence was preserved: a lower reading would mean "the close is still running", not "a phase lost its evidence", and a consumer cannot tell those apart from the value alone. That is a measurement-timing artifact wearing a close-quality name — the precise failure FM1 exists to prevent, arriving through the numerator's population rather than through a synthesized denominator (recorded as **FM5**). The exclusion is defined on the *stage*, not on the observed state, so it is deterministic and does not move with when the tool runs. **Detection** is mechanical: the sub-task title's `Stage <N>` prefix. **Fail-safe:** a sub-task whose title does not parse to a stage number is **retained** in S — the rule may only ever hold out a sub-task it positively identifies as terminal, never shrink the denominator on an unrecognized title.

**Retained sub-signal (non-destructive upgrade):** the **single Stage-13 close-gate boolean** — G-CL4 ("verification evidence persisted") per [`stage-13-close.md`](../pipeline/stage-13-close.md) § 7 — is retained as `evidence-close-gate pass|fail|N/A`. The rate is the primary reading (a real mechanical denominator); the boolean loses no signal. The terminal stages are not unmeasured, only un-*rated*: stage-13 evidence is exactly what G-CL4 asserts, and stage-12 evidence is the merge/tag record the RELEASE_LOG row itself carries.

**N/A:** when the release scaffolded **zero stage sub-tasks** (pre-hub-spoke / grandfathered releases) → `evidence-preservation N/A — no stage sub-tasks scaffolded` (a clean absence, NOT a 0/0 rate); and when every scaffolded sub-task is terminal-stage, so S collapses to zero → `evidence-preservation N/A — no non-terminal stage sub-tasks scaffolded` (also a clean absence, never `0.00`). The retained boolean is independently `N/A` when the G-CL4 gate did not run / is unreadable.

**Anti-overfit denominator scope (§ 1.1 / FM1 honesty).** "Total phases" resolves mechanically to the **measurable scaffolded stage sub-task set** — the release-execution phases at which phase-completion evidence is actually produced *and is terminal by the capture moment* (the hub-spoke stages: e.g. Solutioning / Engineering / DevTest / QA / Plan-Review). Per-issue pre-release intake stages and the compressed review stages are outside the release-scoped sub-task surface **by construction**, and the terminal Execute / Close stages are outside the *measurable* subset **by capture timing**; the read-model measures the real, `gh`-mechanical denominator that exists rather than a manufactured fixed count. This finds a real denominator via reuse and then bounds it to what the capture moment can actually observe — it does not reverse the § 1.1 discipline of never synthesizing one. The still-unbuilt full-13-phase reading stays documented-deferred (it would require the net-new store § 1.1 declines to build).
...
| 6 evidence-preservation (+ retained evidence-close-gate) | the release scaffolded zero stage sub-tasks → `evidence-preservation N/A — no stage sub-tasks scaffolded` (pre-hub-spoke / grandfathered, a clean absence NOT a 0/0 rate); every scaffolded sub-task is terminal-stage, so the measurable denominator is zero → `evidence-preservation N/A — no non-terminal stage sub-tasks scaffolded` (§ 4 Indicator 6, also a clean absence); the retained G-CL4 boolean is independently `N/A` when the gate did not run / is unreadable |
...
| FM5 | **PROC** | A rate whose population is still in flight at its own capture moment | When an indicator's population includes the very phases that are still executing when § 3.2 says to capture it, do NOT publish the raw ratio — bound the population to what is terminal at the capture moment, or the number measures reading-time instead of quality | A rate is only as real as its *population*, not merely its denominator. Indicator 6 is captured at the Stage-13 chore PR, so the Stage-12/13 sub-tasks are mid-flight by construction; counting them made the reading move with stage-closure timing while the underlying preservation never changed (three readings of one live milestone: 0.46 → 0.61 → 0.75, with P equal to the closed count every time — 100% of the variance was timing, 0% was preservation). A consumer cannot separate "the close is still running" from "a phase lost its evidence" | This standard § 4 Indicator 6 defines S as the **measurable** scaffolded set (terminal stages 12/13 held out, detected on the sub-task title's `Stage <N>` prefix, retain-on-unparseable); the producing tool implements it and discloses `terminal_excluded` in `--json`; the terminal stages keep their own evidence signal via the retained G-CL4 boolean and the RELEASE_LOG merge/tag record | Principal: asks "what is this population's state at the instant § 3.2 mandates the read?" before shipping the rate, and bounds it to the terminal subset — the exclusion keyed to the *stage*, not to the observed state, so it cannot drift with run time. Junior: ships the raw ratio because the arithmetic is correct, and the calibration corpus establishes its baseline on a scaffolding-depth-and-timing index labelled as close quality |
...
| v1.02 | 2026-07-22 | **Indicator 6 denominator scoped to the capture moment.** S is now the **measurable** scaffolded stage sub-task set — the terminal stages 12 (Execute) and 13 (Close) are held out, because § 3.2 mandates capture *during* stage 13, so those two phases are mid-flight by construction and counting them made the published rate a function of reading-time rather than of evidence preservation (three readings of one live milestone moved 0.46 → 0.61 → 0.75 with the numerator equal to the closed count every time). The exclusion is keyed to the *stage* (detected on the sub-task title's `Stage <N>` prefix, retain-on-unparseable), not to the observed state, so it is deterministic; `--json` discloses `terminal_excluded`; a population that collapses to zero emits `N/A — no non-terminal stage sub-tasks scaffolded`, never `0.00`. The terminal stages keep their own evidence signal via the retained G-CL4 boolean and the RELEASE_LOG merge/tag record. New **FM5** codifies the general rule (bound a rate's population to what is terminal at its own capture moment). Landed **before the field's first production emit**, so no published value needs reconciling and the calibration corpus starts clean |
