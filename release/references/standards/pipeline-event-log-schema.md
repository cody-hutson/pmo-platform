...
| `spoke-launch` | Per-spoke startup-token reservation telemetry declared for the quota-budget gate ([`quota-budget-protocol.md`](quota-budget-protocol.md) Checkpoint B); `tokens_used:` rides the payload. **Declared, no producer** — the gate itself renders its verdict in hub output and emits nothing, at every launch shape, so the firing scope below describes what this event *would* cover if wired, not what runs today. Scope if wired: every `Agent`-tool spoke launch, wave or singleton, at every stage — matching Checkpoint B's own scope rather than the parallel-wave subset it was originally declared against | `quota-reservation` |
...
**`decision-superseded` payload convention.** A `decision-superseded` row reuses the standard 10 columns (no new fields). `payload` carries the § 11.8.1 vocabulary and **nothing else**: `superseded:` (the RETIRED decision's ID) / `by:` (the SUPERSEDING decision's ID) / `reason:` (a one-line kebab rationale). `outcome` is `superseded`; `actor` is `hub` or `operator`; `subject` is the superseded decision's own subject. **Emitting this row is the ONLY sanctioned way to retire a decision.**

**No `ms:#N;` prefix on this subtype — and that is enforced, not stylistic.** The `ms:` opener belongs to the FREE-FORM `decision` subtypes (`delegation`, and the playbook's § 4a.2 default), which declare no vocabulary and are therefore unvalidated. This subtype **does** declare one, so `validate_payload_labels` rejects any label outside it: a row written with an `ms:` prefix does not emit at all. The milestone is not lost by the omission — it is already carried by the `version` column (the milestone slug) and by `subject`. A closed vocabulary and an "every row opens with `ms:`" habit cannot both hold; the vocabulary is the one with teeth.

**Each ID is written VERBATIM as the superseded row's own payload wrote it.** The platform carries two decision-ID conventions in live data — sequential (`D-7`) and issue-keyed (`D-4767-Scope`) — and this row is a **join, not a rename**. Canonicalizing here would break the join against the very rows it points at, and this subtype has no mandate to migrate an ID namespace.

**Label ORDER is load-bearing and is fixed by the rows already in the log.** `superseded:` names the row being retired and `by:` names the row retiring it. Reversing them would make every pre-existing supersession row non-conformant against its own newly-declared vocabulary, and — worse — would silently invert the meaning of every row a reader joins on.

**The superseded row is NEVER mutated.** § 4.1 forbids the edit outright, and Check 19's row-count parity control **structurally cannot see** an in-place cell rewrite, so a mutation is both prohibited and undetectable by the control that would otherwise catch it. Supersession is therefore an *append*, exactly like every other event: the retired decision keeps its original row and its original `outcome`, and this new row is what makes the retirement findable. Examples (≤ 300 chars, escaped-pipe per § 4.3a):

```markdown
| 2026-08-24T13:00:00Z | pipeline-spec-self-consistency | 5 | decision | decision-superseded | hub | sub-task:#N | CHEAP | superseded | superseded:D-7; by:D-37; reason:band-expansion-retired |
| 2026-08-24T13:00:01Z | pipeline-spec-self-consistency | 9 | decision | decision-superseded | operator | issue:#N | CHEAP | superseded | superseded:D-4767-Scope; by:D-4801-Scope; reason:scope-recut-at-collective-review |
```
...
**Bind the instant at Stage 12, not at authoring.** The boundary is the merge instant, which does not exist while the change is still being built; writing a guess would date the boundary to a moment nothing happened at. The executor is [`stage-12-execute.md`](../pipeline/stage-12-execute.md) **Phase B3.2**, which derives the instant from the merge commit and then verifies the binding with `check-event-record-integrity.sh --assert-bound` — a deferral naming no executor is a sentence, not a handoff, and while the sentinel stands this validator exits 0 on every input and § 7's Close-time BLOCK clause cannot fire.

**Why a boundary at all — this is not leniency.** § 4.1 forbids editing and § 7 makes this log Vital, so a violation committed before the validator existed is **permanently unrepairable**: it cannot be fixed, only reported forever. A validator with no boundary is therefore *born failing*, and a check that fails on its first run and every run after is muted within a week — which costs more than it ever caught. § 2a renders exactly this decision, on exactly this file, for exactly this reason: it dates its own enforcement boundary and instructs readers to date from it. **`LEGACY` means "does not gate", never "not reported"** — pre-cutover findings are printed with their denominator like every other finding.
...
`--window N` defaults to trailing N **distinct versions** (not rows); `--window-by-row` switches to N rows for explicit row-count semantics. Version-grained is the default because the window is applied to rows the query has ALREADY filtered to the selected source, and one release can contribute many of them — a `session-retro` read takes every session's row, a `release-synthesis` read every contributing event's triple. A row-count window can therefore be filled entirely by a single prolific release, while § 11.5's auto-promotion predicate requires a cluster to span ≥ 2 distinct version tags and the `cross_release_pattern_emergence_rate` denominator counts the events in that window. Windowing by version is what guarantees the window spans N releases regardless of per-release row volume.
...
**This table is the single authority for the recognized-label set, and for the scope each set applies to.** `append-pipeline-event.sh` PARSES **two** columns from this table: the "Input filter" column, which declares the `(event_type, event_subtype)` scope the set governs, and the "Labels recognized in payload" column, which declares the set itself. It resolves a row's recognized set by `(event_type, event_subtype)` first, falls back to the event-type set when a row's filter names no subtype, and rejects any other segment-opening label at emit time; `synthesize-release-learnings.sh` reads to the same set. Restating the set in either tool instead of reading it here is what produced the drift where the schema listed five labels and the tool enforced six — `append-pipeline-event.sh` now asserts its static fallback against this table in `--self-test`, so a one-sided edit on the write path fails rather than ships. `synthesize-release-learnings.sh` does NOT: its label set is a static `SOURCE_CONTRACT` literal that is never read from, nor checked against, this table, so a read-side drift passes `--self-test` silently. Holding that literal in step with this table is a manual obligation, recorded as a PARTIAL acceptance condition in the v4.01 release.

**Why an unrecognized label is a defect, not a curiosity.** The reader terminates each payload field at the next recognized label. An unrecognized label is therefore swallowed into the **preceding** field's content and tokenized as if it were signal — one stray label after `theme:` turned a single real cluster into three. Because the log is append-only, such a row can never be deleted, only redacted. Enforcement is two-sided but bounded to the `;`-separated segment grammar both tools implement: the write-side gate splits the payload on `;` and rejects a stray label that opens a segment, and the read side terminates a field at a recognized label or at any `;`-opened label token and discards the content that follows. A stray label joined by any other separator opens no segment, so the write gate never sees it and the read side never terminates on it — it is swallowed into the preceding field verbatim and tokenized (a `theme:` value renders as `over-building mood:excited`). Over the 94 printable-ASCII separators a payload can carry, `;` is the only one the write gate rejects and the only one on which the read side returns a correct cluster count; of the remainder, 30 inflate one real cluster into three and 63 destroy the genuine `theme` token outright. Closing that residue is recorded as a PARTIAL acceptance condition in the v4.01 release. **Scope:** validation applies exactly as far as a row's declared **Input filter** reaches. A filter naming both an event type and a subtype governs that pair alone; a filter naming only an event type governs all of that type's subtypes. Anything outside a declared filter — an event type absent from this table, **and equally a subtype of a listed type that the row's filter does not name** — carries open free-form payload keys and is deliberately NOT label-validated. Do NOT add rows to **this** table to declare a label set for such a subtype: column 1 is the `--source` enum that `synthesize-release-learnings.sh` validates its `--source` argument against, and a third value there would be rejected by the read tool. Declare it in **§ 11.8.1** instead — a registry whose column 1 is a key rather than a `--source` value, parsed by the same col-2 filter rule and read by no `--source` consumer.
...
#### 11.8.1 Subtype payload-vocabulary registry

Column 1 here is a **registry key, NOT a `--source` value.** `synthesize-release-learnings.sh`
does not read this sub-table; its `--source` enum remains exactly the two values declared in
the § 11.8 table above. This sub-table exists because a subtype can owe a payload vocabulary
without being a pattern-detect grain — declaring it as a `--source` row would add a third
`--source` value the read tool rejects.

`append-pipeline-event.sh` parses this sub-table with the same rule it applies to the table
above: the key is the col-2 **Input filter**, the set is col-3. The parse is bounded by the
§ 11.8 heading, so this sub-table MUST stay under a `####` (or deeper) heading inside § 11.8.

| Registry key | Input filter | Labels recognized in payload | Field(s) clustered | Zero-state excluded from clustering |
|---|---|---|---|---|
| `qc4-05-result` | `--event-type release-synthesis --event-subtype qc4-05-result` | `invariant` / `verdict` / `files` | none — not a `--source` grain | n/a |
| `qc4-06-result` | `--event-type release-synthesis --event-subtype qc4-06-result` | `verdict` / `outcome_excerpt` / `evidence_anchor` | none — not a `--source` grain | n/a |
| `decision-superseded` | `--event-type decision --event-subtype decision-superseded` | `superseded` / `by` / `reason` | none — not a `--source` grain | n/a |

**Why every row here clusters nothing, and why that is enforced rather than chosen.** `synthesize-release-learnings.sh` filters `--source release-synthesis` to `--event-subtype learnings-triple`, so a QC4 row is excluded at the query layer before tokenization; `decision-superseded` is not a `release-synthesis` subtype at all, so no `--source` value reaches it. The `Field(s) clustered` cell records that enforcement; it does not create it. Making any of these participate in cross-release clustering would require a **third `--source` value**, which the paragraph above prohibits and the read tool rejects.

**Registering an exact `(event_type, event_subtype)` key binds THAT subtype only.** `labels_for` resolves the exact pair first and falls back to the type-level rung only when no exact row exists. `decision-superseded` is registered as an exact pair and `decision` declares no type-level set, so every *other* `decision` subtype still resolves to the empty set and stays **free-form** — registering one subtype's vocabulary does not silently impose it on its 21 siblings. That non-regression is asserted directly in `--self-test`, not left to inference.

**Charset contract.** The label tokens declared here are parsed with the charset `[A-Za-z0-9_-]`, which is **equal to** the write-side extractor's `[A-Za-z0-9_-]` and therefore satisfies the standing rule that the registry charset be a superset of the extractor's. It had shipped as `[a-z0-9_-]` — a strict subset — which held only by the coincidence that every declared token is lowercase today. Keeping the registry charset a superset of both the extractor's charset and every declared token is load-bearing: a narrower charset does not fail, it silently DROPS the label, and the conforming payload is then rejected as unrecognized. Widening was verified non-regressive by running the shipped parser over this schema under both charsets: 5 registry keys, byte-identical output.
