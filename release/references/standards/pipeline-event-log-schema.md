...
**Why both rows cluster nothing, and why that is enforced rather than chosen.** `synthesize-release-learnings.sh` filters `--source release-synthesis` to `--event-subtype learnings-triple`, so a QC4 row is excluded at the query layer before tokenization. The `Field(s) clustered` cell records that enforcement; it does not create it. Making a QC4 verdict participate in cross-release clustering would require a third `--source` value, which the paragraph above prohibits and the read tool rejects.
