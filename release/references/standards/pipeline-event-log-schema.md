...
**Charset contract.** The label tokens declared here are parsed with the charset `[a-z0-9_-]`, which is a subset of the write-side extractor's `[A-Za-z0-9_-]`. Keeping the registry charset a superset of every declared token is load-bearing: a narrower charset does not fail, it silently DROPS the label, and the conforming payload is then rejected as unrecognized.
