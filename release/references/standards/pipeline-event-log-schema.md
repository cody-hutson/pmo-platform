...
**This table is the single authority for the recognized-label set.** `append-pipeline-event.sh` PARSES the "Labels recognized in payload" column from this table and rejects any other label at emit time; `synthesize-release-learnings.sh` reads to the same set. Restating the set in either tool instead of reading it here is what produced the drift where the schema listed five labels and the tool enforced six — both tools now assert their static fallback against this table bidirectionally in their self-tests, so a one-sided edit fails rather than ships.

**Why an unrecognized label is a defect, not a curiosity.** The reader terminates each payload field at the next recognized label. An unrecognized label is therefore swallowed into the **preceding** field's content and tokenized as if it were signal — one stray label after `theme:` turned a single real cluster into three. Because the log is append-only, such a row can never be deleted, only redacted. Enforcement is two-sided by necessity: the write-side gate stops new bad rows, and the read side discards unrecognized-label content so already-written rows cannot keep leaking. **Scope:** only the two `--source` grains in this table declare a label set and are validated. Event types absent from this table carry open free-form payload keys and are deliberately NOT label-validated.
