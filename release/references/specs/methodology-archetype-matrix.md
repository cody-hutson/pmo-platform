...
| **Hybrid** | timeboxed + phased (dual) | union of constituent methodologies | union of constituent methodologies + SPM Bridge dual-framing | iteration cadence on one track + phase cadence on the other | delivery-engine, weekly-status-rollup, ppm-agent | enterprise transformation, SPM co-managed, dual-track programs | Native-framing status output in TWO lifecycles simultaneously |
...
| `Hybrid` | Produce dual-framed output (one section per track); use union of track-specific primitives; check `spm_comanaged` to activate SPM Bridge formatting | track-1 + track-2 primitives side-by-side; SPM Bridge dual-framing |
