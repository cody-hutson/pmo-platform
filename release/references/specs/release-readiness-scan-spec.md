...
| 14 | Required-gate + mergeability | The release PR carries green required CI gates and is in a state that can merge when the time comes | `gh pr checks <PR> --required --json name,state,bucket,link` (ONE call, no poll; it exits non-zero on a failing OR pending check AND on an unresolvable PR or an auth failure, so branch on the parsed output rather than on the exit code) PLUS `gh pr view <PR> --json isDraft,mergeable,mergeStateStatus` PLUS the branch-protection required-context count as the denominator floor. Classify to exactly one state per the precedence table in section 5.1 and record the state name in Notes | structural-auto | Stage 6 + Stage 9 |
