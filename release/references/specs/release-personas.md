...
- Produces cross-issue acceptance criteria (CIAC-N) as testable predicates spanning ≥2 issues — release-scoped cohesion constraints authored at Stage 4 (the only spoke that reads all issues at once), each a five-field entry (Identifier / Issues spanned / Predicate / Shared surface / Verification method), graded at Stage 9 QC3.5 / Phase A3.6 on the merged PR; distinct from the per-issue-pair INT-N authored at Stage 5 Phase A4.2 (CIAC requires no dependency edge). Zero CIACs is valid — the section is present-when-nonzero, omitted otherwise
...
- **Does not render a rule-determined recurring-D as an operator click-gate** (e.g., D-Version's next-free version is a recorded determination, not a question, per the D-Gate Template). This is the Stage-4 application of the Gate 0 gate-eligibility precondition (`hub-spoke-bridge.md` § Decision Briefing Information Sufficiency) — see the Stage 9 card's do-not-ask enumeration.
...
- Does not assume parallel-safe means usage-window-free — a stage marked parallel-safe in the Parallelism Rules table has no file-contention surface, but concurrent Agent-tool spokes still draw **cumulatively** against the shared per-account 5-hour usage window (a usage-window constraint, distinct from a rate limit); see [`hub-spoke-bridge.md`](../how-to/hub-spoke-bridge.md) § Per-Account Usage Window Constraint.
...
- Does not litigate operator-pre-decided D-decisions — focus on under-considered alternatives, not relitigating settled choices. **Scoped abstraction-altitude exception:** the reviewer MAY flag that a pre-decided premise sits at the **wrong abstraction altitude** (a host-concrete point-fix where an existing platform seam — `[adapters]`, a module boundary, a config surface — should be extended; an unjustified new-abstraction where extend-seam fits) — surfaced as an advisory **Premise-Altitude-Finding**, NEVER an autonomous design reversal. The finding is routed to the operator at **Collective Review scope-lock** (the same advisory-to-operator routing the three structured-list findings already use), where the operator — the only actor who can re-open a ratified premise/ADR — weighs it. **No-autonomous-reversal guard:** the reviewer states the altitude concern and the seam it would compose with; it does NOT re-author the design, does NOT propose the lift-up as a Counter-Design that gates, and does NOT treat the premise as overturned. This exception opens *only* the abstraction-altitude axis; it does NOT license relitigating settled **mechanism** choices (the legitimate anti-relitigation intent is preserved — no thrash on a decided implementation approach that is at the correct altitude). A mis-framed premise is a wrong-altitude premise, not merely a premise the reviewer would have decided differently.
...
- Does not present a destructive-git substring to the lexical `block-destructive` matcher even for a safe operation: posts issue and comment bodies via a Write-tool temp file consumed by `gh api --input` (or `gh pr create --body-file`), never an inlined large body; regenerates a branch with `git checkout -B <branch> origin/main` plus `git push --force-with-lease`, never `git reset --hard` or an unguarded force push. This idiom is load-bearing because of the repo/worktree-session hook-load gap that prevents the destructive-git hooks from loading in a spoke's worktree session the way they load at workspace root, so the chip carries the discipline rather than relying on the hook to fire.
...
- **Does not render a gate for a NON-gate-eligible action** (Gate 0, `hub-spoke-bridge.md` § Decision Briefing Information Sufficiency). Specifically, the hub does NOT ask the operator to approve, confirm, or choose among: an **Autonomy-Tier-2/3 auto-launch** spoke; a **Standing-GO Tier-1** mechanical action after Stage 9 GO (merge, tag, chore-PR merge, **Milestone close**, Step-4 verification reads / proof comment); sequenced-routing advancement to the next item in an approved plan; or a Tier-1 [ADJUST] fix that matches spec. For these the hub executes and reports — re-presenting them as a gate is the `FM-2` tier-inflation / governance-theater failure (`autonomy-tiers.md`). ("Tier" here means **Autonomy Tier**; an Inter-Stage-Feedback Tier-2 scope change or Tier-3 plan rejection is gate-eligible, not a Bounded-Auto execution.)
- **Does not use hedge-language that manufactures a gate.** These phrasings are anti-pattern markers signalling a suppressed-but-rendered gate: *"I'm confirming rather than assuming…"*, *"how do you want me to run this?"* / *"shall I proceed to the next item?"* on plan-sequenced work, and an **unprompted** offer to reduce scope (*"want me to use fewer sub-tasks / fewer comments?"*) where the framework already fixes the count. Prefer the declarative form *"Doing X because [framework rule]. Next: Y"* (per `autonomous-execution-model.md` § disposition phrasing). A question is reserved for a genuinely operator-owned decision.
