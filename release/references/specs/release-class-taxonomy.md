...
| `cross-cutting` | Modifies ≥3 pipeline stages OR ≥3 governance surfaces; expanded coordination scope. | (a) File Change Matrix declares a change to ≥3 `pipeline/stage-*.md` files; (b) File Change Matrix declares a change to ≥3 of the 6 rule-defining governance surfaces {CLAUDE.md, OPERATIONS.md, RELEASE_PROTOCOL.md, hub-spoke-bridge.md, gate-criteria-spec.md, release-process.md} — see § Trigger (b) set membership; (c) ≥3 in-bundle compositional edges per Stage 4 A2 DAG. |
| `hotfix` | Narrow corrective scope addressing a P1/P2 defect found in a deployed release; in `versioned` mode a sub-release patch (vX.Y.Z trailing dot), in `version-less` mode a corrective release carrying no version string and identified by its capability slug; CHEAP reversibility. | (a) source defect is a P1/P2 Issue raised against a deployed release; (b) bundle contains ≤3 issues; (c) bundle scope is corrective ("fix:" commit posture dominates); (d) **`versioned` mode only** — version-string carries trailing patch number; in `version-less` mode (d) is inapplicable and the class is established by (a)-(c) alone. |

**Release-identity mode and the `hotfix` class.** The `hotfix` definition cell and its trigger (d) are the only places this taxonomy assumes a release carries a version string. Both are conditioned on the **release-identity mode** — the closed enum `{versioned, version-less}` declared at Bundle (the Release-Identity Mode declaration in `release/references/pipeline/stage-03-bundle.md`; asserted at Gate 3 by criterion G3-19 in `core/schemas/gate-criteria-spec.md` § Gate 3). A `version-less` release can still be a genuine `hotfix`: it is narrow, corrective, and raised against a deployed release, but it carries **no version string at all**, so trigger (d) is **inapplicable rather than failed** and the class is established by triggers (a)-(c). `version-less` is an *identity-axis* value, not a malformed or empty version string — do **not** synthesize a placeholder patch number, and do not treat the absent version as a classification failure. The other three classes make no version-string assumption and are unaffected by the mode. **Cutover discipline:** applies to releases classified at Stage 3 strictly AFTER this note's introducing-release merge SHA recorded in the release log; pre-cutover classifications grandfathered; the introducing release itself exempt (reflexive-pipeline-loop discipline).

**Trigger (b) set membership — rule-defining surfaces only.** The named set contains the six documents that **define how the platform behaves**. `RELEASE_LOG.md` is deliberately **not** a member: it is a per-release **ledger** that records outcomes, and its own rules live elsewhere — the row lifecycle and emit format in `release/references/pipeline/stage-12-execute.md` § Phase B5 and `release/references/pipeline/stage-13-close.md` § Phase B (both counted by trigger **(a)**), the Deployment-Log field anatomy in `release/governance/release-process.md` § Phase B5, and the gate assertions at `G-EX4` / `G-EX5` / `G-CL3` in `core/schemas/gate-criteria-spec.md` (both counted by trigger **(b)**). A release that genuinely changes the ledger's format or convention therefore still counts — on the surface where that convention is actually specified. **Why membership was the wrong shape rather than a missing exclusion clause:** every *deployable*-class release writes a mandatory `RELEASE_LOG` row at Stage 12 and transitions it at Stage 13, so membership handed every honest File Change Matrix one free touch and the `≥3` threshold read as `≥2` in practice. An exclusion clause scoped to those mandatory writes does not close the gap — reference repairs, historical row corrections, and archive relocations are all `RELEASE_LOG.md` edits that are equally uninformative about release shape, and each would need its own carve-out. Removing the member closes the question once. It also keeps the arithmetic independent of the **close class**: a `task-artifact`-class release writes no release row at all (per `G-CL3` § Close-Class Conditioning Disposition), and a Stage-3 predicate must not have a count that depends on a Stage-13 branch.

**Worked example.** A release whose File Change Matrix edits `gate-criteria-spec.md` and `release-process.md`, plus the mandatory Stage-12 `RELEASE_LOG` row, counts **2**, not 3 — trigger (b) does **not** fire. This is the `v4.19` case: it was declared `novel`, and a Stage-5 literal re-count of the seven-member set read 3 and disagreed. Under this rule there is one reading. Had the same release *also* edited `hub-spoke-bridge.md`, it would count **3** and trigger (b) would fire on the three rule-defining surfaces alone.

**Cutover discipline:** applies to releases classified at Stage 3 strictly AFTER this note's introducing-release merge SHA recorded in the release log; pre-cutover classifications grandfathered; the introducing release itself exempt (reflexive-pipeline-loop discipline).
...
- **cross-cutting**: classifying a single-stage-spec-edit release as `cross-cutting` because the edit happens to be in a `pipeline/stage-*.md` file. The trigger is ≥3 stage files, not 1. Related: counting the mandatory Stage-12/13 `RELEASE_LOG` row toward trigger (b). The ledger is not a member of the set — see § Trigger (b) set membership.
...
3. **Size-bound recompute — the class is a multiplier on the scope bound.** Before the milestone description is updated, recompute `effective_pts` under the NEW class: `round_half_up( Σ(member.pts) × class_weight )`, reading `class_weight` from `[bundling].release_class_capacity_weights`, the band from `[bundling].release_size_target_pts`, and the rounding mode by reference from `bundle-composition-doctrine.md § 3 Step 5 — Risk-Weighting`. Those are the same three sources G3-15 reads, so the recomputed figure and the gate's own bound cannot disagree. Surface to the operator: raw `Σ(member.pts)`, the resolved `class_weight` and the class that drove it, `effective_pts`, and the band upper bound — together with the disposition those resolve to. A figure above the band routes to G3-15's own disposition menu (split by sub-capability / tight-merge per `bundle-composition-doctrine.md § 4` / reframe-and-keep-with-rationale) and is never a dead-end fail; where the operator keeps the milestone with rationale, the one-line `Override: <rationale>` lands in the milestone-description deviation log exactly as it would at Bundle. **This step runs before step 4, not after** — so the operator renders the class decision knowing its size consequence rather than discovering it at a later stage.
4. Hub updates the milestone description "## Release Class" H2 section via `gh api repos/{REPO}/milestones/<N> --field description=...`.
5. Hub posts a structured comment on the current stage sub-task: `[RECLASSIFY <old> → <new>] <rationale>` with link to the new milestone description, carrying the step-3 recompute on the line below it in the form `effective_pts: raw <Σ(member.pts)> × <class_weight> = <effective_pts> — <disposition> vs band <low>-<high>`. That line is this protocol's **declared observable**: the compliant path emits it and the bypassing path does not, so a re-classification that skipped the recompute is visible in the record rather than only in its consequences.
6. Downstream stages consume the updated class via the same milestone-description read mechanism as Stage 3 Phase B3.

### Reversibility-confidence pairing per reversibility-protocol.md

- **Cheaper-to-stricter** (e.g., `routine` → `novel` or `novel` → `cross-cutting`): CHEAP reversibility — the new class adds ceremony and the per-action authorization signatures remain intact. **One downstream artifact IS invalidated, and step 3 above is where it is recomputed rather than carried forward: the size-bound disposition.** `class_weight` is a multiplier on the quantity the size-check measures, so a cheaper-to-stricter change raises `effective_pts` on unchanged membership and can cross the band ceiling — a `routine` → `cross-cutting` move on a raw 20-pt bundle resolves to effective 26 against a 25-pt ceiling. The re-classification is still CHEAP; it is **not** scope-neutral. HIGH confidence — the new class can revert to the original at the next gate if the original was correct.
- **Stricter-to-cheaper** (e.g., `cross-cutting` → `novel` or `novel` → `routine`): MODERATE reversibility — the new class drops ceremony; downstream stages that already absorbed the stricter posture (e.g., a Deep Stage 9 review already conducted) cannot be un-done. Requires explicit risk acceptance per reversibility-protocol.md § 3 process-weight scaling. Step 3's recompute still runs — a lower weight lowers `effective_pts` and can only move a milestone further inside the band, never out of it, but the recorded disposition must match the class actually in force.
