...
Consumes blast-radius CLI output per [`release/references/protocols/blast-radius-protocol.md`](../protocols/blast-radius-protocol.md). Schema v1 fields cited: `stats.first_order_count`, `stats.scan_scope`, `stats.scan_scope_status`, `first_order[].path`, `first_order[].reference_count`, `first_order[].matches`.
...
- [ ] **1.2** `first_order_count` value reviewed **together with `stats.scan_scope`**, which names the population the count covers. A tier computed from an `all-files` count is **provisional** — that population includes untracked and git-ignored files a tracked-scope run would exclude, so the same change can classify differently across roots; the reviewer records the scope alongside the tier rather than citing a bare number. When `stats.scan_scope_status` is not `fetched`, the tracked population was never established or is measurably short, and the tier is provisional on that basis too. Then: impact-classification tier applied per [`blast-radius-protocol.md`](../protocols/blast-radius-protocol.md) §"Impact classification rules" (cosmetic ≤1 / behavioral 2–5 / structural ≥6 OR any reference from a critical file). *Non-markdown-method accommodation:* the § 5 tiers are domain-agnostic — when a non-default method was used, this check is satisfied by the opt-out record's `Impact classification` line computed from the domain-method's first-order count (the same Cosmetic/Behavioral/Structural tiers applied to the import-graph / component-tree / solution-component fan-out).
...
- [ ] **1.7** *(Conditional — fires only when the spoke output asserts a zero, empty, clean, absent, or "N of M" verification result in a load-bearing position. A `first_order_count` of 0 is such a claim, and so is any "no findings" / "no contention" / "N of M swept" statement elsewhere in the output.)* Each such claim carries a probe record per [`review-discipline-principles.md`](../../../core/disciplines/review-discipline-principles.md) § 1 Rule 15 + § 8 Probe Validity: the invocation, the denominator searched, the sensitivity arm with its observed non-zero result, the specificity arm with its observed zero wherever § 8 triggers that arm, and evidence the extraction was non-empty and untruncated for the subject **and for each control arm**. This check consumes — does not redefine — the `PV-0`..`PV-6` elements, the arm-selection rule, and the verdict rule. **A zero whose sensitivity arm also returned zero is a BROKEN PROBE, not a PASS:** the reviewer records the probe unusable and does not accept the underlying claim, because a probe that cannot detect says nothing about the subject. A specificity arm returning zero is that arm PASSING only when its own input is shown non-empty and shown to carry the near-miss; otherwise it is VACUOUS. Where a required element cannot be established the verdict is INDETERMINATE naming the element — never a PASS — and its first disposition is repair-the-probe-and-re-run. When the output asserts no zero/absent/N-of-M claim, check 1.7 is OMITTED entirely (not marked "N/A"), the same omission discipline check 1.6 follows.

**Pass criterion:** Checks 1.1–1.3 all PASS — satisfied either by the `blast-radius.sh` schema-v1 fields (the DEFAULT doc/governance/pipeline-internal path) OR, when the A3.1 selector recorded a non-markdown method, by the § 12 opt-out record's `Method selected` / `applicability` / `Fan-out performed` / `Impact classification` lines (the non-markdown-method accommodation per check). A populated opt-out record IS a PASS; a non-default method does not fail Section 1 for the absence of schema-v1 fields. Checks 1.4–1.5 conditionally PASS (only-if-applicable, explicitly marked N/A with rationale when not triggered). Check 1.6 conditionally PASS — when a durable-corpus file moves or is renamed, the six-form table (F1–F6) must be present and grounded; when no file moves, check 1.6 is OMITTED. Check 1.7 conditionally PASS — when the output asserts any zero / empty / clean / absent / N-of-M verification result, every such claim carries a complete probe record with both control arms disposed (the specificity arm either observed at zero over a non-empty near-miss input, or explicitly recorded NOT TRIGGERED with the § 8 condition that fails); a BROKEN PROBE, an OVER-MATCHING PROBE, or an unrepaired INDETERMINATE is a FAIL, not a caveated PASS. When the output asserts no such claim, check 1.7 is OMITTED.
...
- [ ] **3.1** **Read `stats.second_order_status` FIRST, before `second_order_count`.** On `fetched` or `truncated` the counter is present and 3.1 proceeds as below. On **`not-run`** (the run used `--depth < 2`, or `--mode=structural`) or **`degraded`** (a scan-listed file could not be read) the counter is **ABSENT from the emit** and **3.1 is NOT satisfiable**: an unmeasured second order is not a zero-blast-radius finding, and marking 3.1 PASS on one is what lets an ALL-conjunction gate return PASS over an input nothing examined. Route instead to a re-run at `--depth >= 2` (or, for `degraded`, to fixing the unreadable input), **or** record an explicit rationale in spoke evidence naming the status and why the unmeasured second order is acceptable for this change. Then: `second_order_count` value reviewed; chains of `depth > 2` traced explicitly, each documented in spoke evidence as `target → via → second_order_path`.
...
**Pass criterion:** All 4 PASS. Where `stats.second_order_status` is `not-run` or `degraded`, 3.1 PASSes only on the recorded-rationale limb — never on the counter, which is absent — and 3.2 / 3.4, which iterate `second_order[]`, are marked **UNEVALUABLE** rather than passing vacuously over an empty array.
**Fail action:** Block Engineering; return to Phase A § A3 for deeper traversal, a re-run at `--depth >= 2`, or mirror-handling refinement.
...
- [ ] **4.7 Seam-composition / abstraction-altitude** — Assess whether the design solves at the correct abstraction altitude relative to the platform's existing seams, BEFORE it is built. This is a BLOCKING check: a design that does not discharge the forcing function below FAILS Section 4 (omission is a fail, not a behavioral marker). The forcing function (stated inline + complete):
      1. **Identify the nearest existing platform seam.** Search the platform's extension surfaces for a seam this capability could compose with: the `operator.toml [adapters]` selectors (`repo_host` / `ticketing` / `kb` / `ai_tool` / `scheduler` — per [`repo-host-adapter-versioning.md`](../../../core/standards/repo-host-adapter-versioning.md) and the `operator.toml [adapters]` table), a module boundary, or a config surface. Name the nearest seam found (or state that the search found none).
      2. **State why this is a point solution vs. an extension.** Classify the chosen design's altitude — **point-fix** (solves this instance directly; no reusable seam) / **extend-seam** (extends the named existing seam) / **new-abstraction** (introduces a new seam because none fits) per [`design-exploration.md`](../standards/design-exploration.md) §2 — and justify why the chosen altitude is correct: if **point-fix** or **new-abstraction** was chosen over **extend-seam**, state the specific reason the nearest existing seam was NOT extended.
      3. **"No seam" PASSes only with the search cited.** A design may legitimately conclude no existing seam fits (a genuine point-fix or a warranted new-abstraction) — but ONLY when step 1's seam-search is cited (the surfaces searched + why each does not fit). A bare "no seam exists" / "point solution" assertion *without* the cited search does NOT pass — it is the unexamined-altitude signal this gate exists to catch.
      **Host-binding cross-reference:** a design that hardcodes a host tool (`gh` / `git` / a host API) as *the* canonical mechanism where an adapter seam exists is a **host-binding leak** per [`knowledge-architecture.md`](../../../core/disciplines/knowledge-architecture.md) §4 — cite that class by name when the gate finds one. Routing: a §4.7 finding (point-fix-or-new-abstraction-over-seam without sufficient justification, or an uncited "no seam") is surfaced to the operator at **Collective Review scope-lock** as an advisory altitude finding — the same decision home an adversarial Premise-Altitude-Finding routes to ([`release-personas.md`](../specs/release-personas.md) Stage 5 Phase A6.5) — never an autonomous design reversal.
      **Cutover (introducing-release-exempt):** Check 4.7 applies to releases entering Stage 5 design review strictly AFTER this criterion's introducing-release merge SHA recorded in the release log. **The introducing release itself is exempt** — the criterion shipping in a release does not fire on its own Stage 5 (it would assess its own not-yet-shipped rule, a reflexive-pipeline loop); the introducing release's own Stage 5 runs under the rules in force before this check shipped.
- [ ] **4.8 Structural-premise review** — When the design perpetuates or extends an existing structure (the T3 §3.2 obligation was recorded per [`planning-solutioning-handoff.md`](../../../core/standards/planning-solutioning-handoff.md#structural-premise-review-obligation)), record the `reviewed → {retained|changed} because {evidence}` determination AND the 3-axis best-practice/scalability/maintainability assertion. **BLOCKING** (mirrors 4.7): omission when the obligation was recorded is a FAIL, not a behavioral marker. The determination is informed by the ticket-vs-live-architecture reconciliation (the pre-build reconciliation step). This is the checklist materialization of [`stage-05-solutioning.md`](../pipeline/stage-05-solutioning.md) §7.2 SR-G1..SR-G4. When no structure is perpetuated, mark 4.8 explicitly N/A.
      **Cutover (introducing-release-exempt):** Check 4.8 applies to releases entering Stage 5 design review strictly AFTER this criterion's introducing-release merge SHA recorded in the release log. **The introducing release itself is exempt** (reflexive-pipeline loop).

- [ ] **4.9 Gate-efficacy runner resolution** — If this design ADDS or MODIFIES a **prose-declared normative predicate** (a passage stating a verdict — FAIL / MUST / blocks — where no code implements it), it MUST name a resolvable runner per [`core/standards/gate-efficacy-standard.md`](../../../core/standards/gate-efficacy-standard.md) Requirement (b) § "Runner resolution — class 3", and carry its row in that file's gate-coverage register. **A prose-declared normative predicate is either verdict-shaped or obligation-shaped, and the check fires on both.** Three limbs: **L1** the passage states a verdict; **L1′** the passage declares a **procedure with a stated trigger** — a named condition (*"when X"*, *"before Y"*, *"whenever …"*) plus an act the agent MUST perform — and states no verdict for the negation; **L2** no code implements it. `L1 ∧ L2` is **class 3-V** and `L1′ ∧ L2` is **class 3-O**; both owe a named runner. Where a passage carries both a trigger and a verdict, L1 governs and L1′ adds nothing. `L1 ∧ ¬L2` is class 2 and the implementing check carries the declaration. **The admission bar for L1′ is the stated trigger and nothing looser** — a rule with no named condition (*"be thorough"*, *"prefer durable structures"*) is NOT admissible, because there is no observable moment at which a runner could be said to have fired. When no runner can be named, take the disposition the standard defines **for that form**: class 3-V → **wire it** (implement as a `deploy.sh --check` gate, CI job, or `--self-test` case) or **downgrade it** (restate as non-normative description, dropping the verdict language); class 3-O → **wire it**, or **register it as a named gap** — keep the obligation, record the row with an empty enforcing-gate cell, and declare the observable that would close it. **Do not downgrade a 3-O passage:** an unrunnable verdict is lying and deleting it is a correction, but an unobservable obligation still correctly states what the agent must do, so deleting it is a regression. A named runner must be **resolvable** in the standard's `R1 ∧ R2` sense — it **carries** the predicate (R1) *and* something **reaches** it (R2); failing either is the same defect as naming no runner at all. **Split of labour, stated because the reviewer half has already failed once:** R1 is now COMPUTED — the register row carries a `runner-def: <path>::<anchor>` pointer and `deploy.sh --check` Check 62 re-resolves it on every run, so do not hand-verify it and do not treat this checkbox as the thing that catches it. What the reviewer owes here is the half no computation can supply: that the named runner is the RIGHT runner for this predicate (a runner whose domain the predicate actually falls in, not merely one whose check set can be made to contain a matching string), and that **R2** holds — something genuinely invokes it. Confirm the row exists and carries a `runner-def:` pointer; **a row that names a runner but carries no pointer is a FAIL**, because it leaves R1 unasserted. **The one row shape that correctly carries no pointer is a class-3-O named gap** — it names no runner, so there is nothing to resolve, and a pointer there would assert a resolution that does not exist. For a named-gap row the reviewer owes a different confirmation: that the **declared observable** is present and is genuinely emitted by the compliant path and not by the bypassing one.

**Pass criterion:** Checks 4.1, 4.2, 4.4, 4.5 PASS; check 4.9 PASSES or is N/A (no prose-declared normative predicate of **either** admission form — verdict-shaped `L1 ∧ L2` or obligation-shaped `L1′ ∧ L2` — added or modified). A 3-O design PASSES on either disposition: a named runner with a resolvable `runner-def:` pointer, **or** a named-gap row carrying its declared observable; check 4.3 conditionally PASS (explicitly marked N/A when no ADR drafted); check 4.6 PASS when a domain guide applies (no fired contraindication, conformance met — including the governance/pipeline-internal case, which runs the full walk against the governance guide rather than short-circuiting) OR explicitly marked `DOMAIN-PRACTICE-NOT-ASSESSED` / NOT-APPLICABLE with the domain named — the not-assessed mark is an honest signal and does NOT block Engineering, but is surfaced forward to Stage 7 Phase C for the conformance assessment; check 4.7 PASS — the seam-composition forcing function is discharged: the nearest existing seam is named (or a cited seam-search concludes none fits), the chosen altitude (point-fix / extend-seam / new-abstraction) is stated, and any point-fix-or-new-abstraction chosen over extend-seam carries its justification. An uncited "no seam" / bare "point solution" assertion is NOT a PASS; check 4.8 PASS — structural-premise determination recorded with evidence + 3-axis assertion, OR explicitly N/A (no perpetuated structure).
**Fail action:** Return to Phase A § A2 (architecture alignment) or § A5 (ADR drafting) for refinement. A fired §3 contraindication (4.6b) or a conformance shortfall (4.6c) returns to Phase A § A2 for the contraindicated practice or shortfall to be re-designed. A §4.7 seam-composition failure — the forcing function not discharged (no seam named and no cited seam-search; OR an altitude chosen over extend-seam without justification; OR an uncited "no seam" assertion) — returns to Phase A § A4 for the design to either lift to the extend-seam altitude or record the cited seam-search that justifies the chosen altitude. The §4.7 finding is also surfaced to the operator at Collective Review scope-lock as an advisory altitude finding. A §4.8 structural-premise-review failure returns to Phase A § A4 for the determination to be recorded (or the structure re-designed). For a 4.9 failure, take the disposition `gate-efficacy-standard.md` Requirement (b) defines **for the failing passage's admission form** before re-submitting — class 3-V: **wire it** or **downgrade it**; class 3-O: **wire it** or **register it as a named gap with a declared observable**. Downgrading a 3-O passage is not an available disposition.
...
- [`core/standards/repo-host-adapter-versioning.md`](../../../core/standards/repo-host-adapter-versioning.md) — the `[adapters]` seam exemplar referenced from Section 4 check 4.7.
- [`release/references/standards/design-exploration.md`](../standards/design-exploration.md) — the altitude-band vocabulary (point-fix / extend-seam / new-abstraction) referenced from Section 4 check 4.7.
- [`core/disciplines/knowledge-architecture.md`](../../../core/disciplines/knowledge-architecture.md) — §4 host-binding leakage class referenced from Section 4 check 4.7.
- [`core/disciplines/review-discipline-principles.md`](../../../core/disciplines/review-discipline-principles.md) — § 1 Rule 15 + § 8 Probe Validity (`PV-0`..`PV-6`, the arm-selection rule, the verdict rule) referenced from Section 1 check 1.7.
