...
**Trigger:** Stage 5 Phase A § A3 runs `release/tools/blast-radius.sh` against each issue's target file(s). Output is the input to this section.

**Reviewer checks:**

- [ ] **1.1** CLI invocation captured in spoke output Evidence section (cite exact command, `cli_version`, `schema_version`).
- [ ] **1.2** `first_order_count` value reviewed; impact-classification tier applied per [`blast-radius-protocol.md`](../protocols/blast-radius-protocol.md) §"Impact classification rules" (cosmetic ≤1 / behavioral 2–5 / structural ≥6 OR any reference from a critical file).
...
**Pass criterion:** Checks 4.1, 4.2, 4.4, 4.5 PASS; check 4.3 conditionally PASS (explicitly marked N/A when no ADR drafted).
**Fail action:** Return to Phase A § A2 (architecture alignment) or § A5 (ADR drafting) for refinement.
