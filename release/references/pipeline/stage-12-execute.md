...
**Cutover discipline (A.5.7):** Applies to releases entering Stage 12 strictly AFTER this sub-step's introducing-release merge; the introducing release itself is exempt (reflexive-pipeline-loop discipline — it cannot fire its own new gate). **The tool is not gated by this cutover — a tool is not a gate.** The introducing release uses `renumber-adr.py` at Engineering Commit 0 to derive its numbers and, if a sibling merges first, pre-merge to reconcile them. The same cutover governs A.5.7c-i and A.5.7e, which arrive with ADR-173: a release whose prose was authored before that record carries literal numbers by design, so `--stamp` finds nothing to resolve and `--stamp --check` passes trivially.
