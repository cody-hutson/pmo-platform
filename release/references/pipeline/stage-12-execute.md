...
A.5.7a **Detect.** Run `python3 release/tools/renumber-adr.py --detect` (read-only; exits 0 always). It reports `anchor(origin/main)`, the branch-only claimed set (**detection, never binding**), and a per-claim verdict. A.5.7b **BINDS** → record `adr-number-freeness = FREE` plus the bound numbers in the Stage 12 sub-task output; proceed to Phase A.6. A.5.7c **DUPLICATE or WOULD-GAP** → run `python3 release/tools/renumber-adr.py --renumber <old> <new> --apply`, commit the result on the branch **before** the Phase B1 merge, and record the move plus its six-step verification in the sub-task output. This is a **Tier-1 mechanical remediation, not a HALT** — the tool is deterministic, its R1 refusal path is the safety property, and its R6 verify reverts the whole staged set rather than leaving a half-swept tree. Escalate Tier 2 only if the tool exits non-zero.
...
**Cutover discipline (A.5.7):** Applies to releases entering Stage 12 strictly AFTER this sub-step's introducing-release merge; the introducing release itself is exempt (reflexive-pipeline-loop discipline — it cannot fire its own new gate). **The tool is not gated by this cutover — a tool is not a gate.** The introducing release uses `renumber-adr.py` at Engineering Commit 0 to derive its numbers and, if a sibling merges first, pre-merge to reconcile them.
