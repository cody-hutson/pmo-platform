...
**Enforcement primitive:** `deploy.sh --check` Check 22 evaluates structural-auto criteria G1-01 / G1-03 / G1-06 / G1-09 (per [`gate-criteria-spec.md` § Gate 1](../../../core/schemas/gate-criteria-spec.md#gate-1-triage-readiness)) across all open `status: bundled` issues. Findings emit via the standard `flag_warn_or_issue` helper to `core/hooks/deploy-check-warn-log.jsonl`. Judgment criteria (G1-02 / G1-04 / G1-05 / G1-08) remain operator-discipline — recommend-tier per gate-criteria-spec.md, not suitable for structural-auto enforcement.

**When-not-applicable (per operator framing 2026-05-11 — *"right tool, right time"*):**
- Issues that have not yet entered bundling (`status: proposed` / `status: approved`) are out of scope; G1 enforcement at intake-time is deferred if the Layer (b) shakedown indicates need.
- Check 22 applies to bundled issues going forward.
- Judgment-class criteria are *not* enforced; surfacing them as warnings would over-gate routine intake and contradict the *"right tool, right time"* framing.

**Cutover discipline:** Applies to all releases going forward.

**Shakedown:** Warn-mode initial per [`bypass-mode-readiness.md`](../../../core/rules/bypass-mode-readiness.md) §Shakedown precedent (matches Checks 8/9/10/14/15/18/19/20/21). Flip-to-enforce after ≥3-day warn-log review with zero false positives.
