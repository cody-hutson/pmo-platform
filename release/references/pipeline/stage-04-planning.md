...
**Phase A0 — Triage→Design Re-Review per [triage-design-rereview.md](../standards/triage-design-rereview.md):** spoke produces re-review artifact (header + per-requirement table) at sub-task output head, before A1 begins. The **C1/C2 proceed-classification is confidence-gated** per [triage-design-rereview.md § 3.0](../standards/triage-design-rereview.md) (mechanism: [reversibility-protocol.md](../../../core/specs/reversibility-protocol.md) § From Label to Gate) — a low-confidence proceed at MODERATE/EXPENSIVE reversibility runs the bounded pause-to-learn loop (the § 3.1 freshness-check is the external-signal injection) before C1/C2 commits; C3 classifications trigger Tier 0 — Premise Rejection per [release-process.md](../../governance/release-process.md) Inter-Stage Feedback Protocol. Effort tier per § 7 (trivial / standard / complex). Always-fires for releases subject to cutover.
