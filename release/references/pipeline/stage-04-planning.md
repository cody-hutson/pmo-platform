...
**The deploy check is reached by declaration.** A row graded by the source-to-deployed check names that check, in backticks, as the row's only command — `bash core/deploy/deploy.sh --check`, `core/deploy/deploy.sh --check`, or the root shim `./deploy.sh --check` — with `--check` as its only argument. The plan verifier then grades the row by the check's exit status. Two things are read ahead of that declaration: an allowlisted-verb probe anywhere in the cell, which grades the row by that probe, and an integration keyword (integration, cross-issue, CIAC), which routes the row to the cross-issue reading. A prose word (unchanged, byte-diff, byte-equivalent), a `--check-<mode>` invocation, another file named `deploy.sh`, a pipeline led by the check, or the check mentioned beside another command routes no row there: that row is graded by the command it does name, or reads as a method with no command. A declared row that names a second command reads UNRUNNABLE when the check passes, because the second command did not run. The check's exit status covers the whole run, and a warn-mode check never fails it, so a claim about one Check's findings needs that Check's own probe, or the declared-deferred form.
