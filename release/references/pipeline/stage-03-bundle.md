...
| G3-05 (presence) | AC section present and carrying ≥1 criterion | structural → auto | Read `ac_present` / `ac_count` from the `bundle-issues-parser.py` parse that Phase A1 already runs over the candidate set — no additional invocation and no second heading grammar. **Block** on `ac_present: false` (no Acceptance Criteria section) or `ac_count: 0` (section present, no criteria). The predicate matches at any heading depth H2–H4, case- and suffix-tolerantly, and counts checkbox, plain-bullet and ordered criteria alike, so a body predating the current issue template is not exempt for want of a template field to find absent. Presence only — shape is G1-05a, measurability is the row below. This evaluation is forward-only by construction: it reads bundling candidates, so no already-bundled card is ever in its population and no cutover clause is needed. The deploy-time backstop (`deploy.sh --check` Check 22, G1-05a presence arm) catches a body edited *after* bundling, which this rung structurally cannot see. |
| G3-05 (measurability) | AC are measurable | judgment → recommend | Agent assesses whether AC can be verified at Stage 7/8. Flag untestable AC. |
...
## 8. Automation Level
Overall Tier 2. Today: the `release-planner` skill's Backlog-analysis mode (Mode A) reads GitHub Issues directly via `gh issue list --json`, parsing the `improvement.yml` body sections (`### Affected Files`, `### Dependencies`, `### Acceptance Criteria`) through `release/tools/bundle-issues-parser.py`; the operator decides Milestone scope at Phase B (Tier 3). The AC-presence rung of G3-05 rides that same parse — it reads the `ac_present` / `ac_count` fields the parser already emits per issue, so the mechanical assertion costs no additional invocation.
