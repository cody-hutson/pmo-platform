...
| Artifact Outputs | Source code, unit tests, build artifacts | Committed files on release branch, PR (created in draft, transitioned to ready at Stage 9), deployed skill copies (if applicable), sub-task completion evidence |
...
**Commit-group traceability (documented simplification — advisory, not enforced).** The sub-task `Commit group` field (Required per [`sub-task-methodology.md § 5`](../standards/sub-task-methodology.md)) declares the commit or commit sequence a sub-task closes on. Under the single-agent-on-a-branch execution model, the plan-to-actual mapping is **advisory documentation, not an automated gate**: it is not mechanically checked against the actual closing commits. This is an intentional simplification — B1 already requires commit messages to reference issue numbers and the sub-task closure comment already names the landing SHA (per [`sub-task-methodology.md § 7`](../standards/sub-task-methodology.md) close-immediately discipline), so the `parent → change-spec → sub-task → commit(SHA)` lineage is reconstructable from existing artifacts without a dedicated plan-vs-actual verifier. **Monitored residual (why this reconstruction can hold, and the one signal that says it did not):** the reconstruction assumes the § 7 close-immediately discipline held — each sub-task was closed the moment its commit landed, with a closure comment naming the SHA. The signal that the assumption broke is a **divergence between the parent's sub-issue rollup and the actual commit history** (the § 7 rollup-monitoring tracking-defect condition): a rollup that does not reconcile to the landed commits means the SHA-to-sub-task mapping was batched or dropped, and the lineage is no longer reconstructable from the closure comments alone — fix it at Stage 6 per § 7, do not defer. Should a future multi-writer concurrency posture (P2/P3 per [`parallelism-posture-taxonomy.md`](../standards/parallelism-posture-taxonomy.md)) make the mapping load-bearing, a `git log`-vs-metadata audit is an additive upgrade that composes into the shared verification executor rather than a standalone check.
...
C2 Create PR with full metadata (title, body, milestone, labels, assignee, reviewer, project). The release PR is created in **draft** state (`gh pr create --draft`) so it is visibly work-in-progress through Engineering + Dev Testing (Stage 7) + QA (Stage 8); it is transitioned to ready-for-review at the Stage 9 gate (per [`stage-09-plan-review.md`](stage-09-plan-review.md) § Phase A8 draft→ready transition) so the operator reviews the PR diff in its review-ready form. This makes C1's "committed BEFORE the PR is marked ready-for-review" a defined lifecycle step rather than an aspirational one. **Single-operator no-op:** in single-operator steady state (OPERATOR resolves to `@me`, `--reviewer` dropped per § PR metadata) the draft is transitioned to ready by its author with no external reviewer — the draft→ready step still runs, but no second reviewer is required for it to complete; the facet is dormant, not broken. Body: implementation summary, per-issue status, documentation impact resolution, deviation log, verification evidence, plan link.
...
Calibration: sub-task accuracy, deviation count by severity, verification coverage, escape rate, cycle time, DORA lead-time-for-changes (the first commit on the release branch is the T_COMMIT numerator anchor per [`dora-telemetry.md`](../standards/dora-telemetry.md) § 2 — a read-model over EXISTING commit-time + deploy events; this stage emits NO new metric event), self-review effectiveness.
Named-gate criteria: **G6 Engineering Completeness** (G6-01..G6-05) per [`core/schemas/gate-criteria-spec.md § Gate 6`](../../../core/schemas/gate-criteria-spec.md). Structural checks — PR exists, commits link source issues, parser-clean PR body, verification-evidence present, Change Description section present; warn-mode.
...
**DORA lead-time anchor (read-model note — emits NO new event).** The first commit on the release branch (Phase B1) is the **T_COMMIT** numerator anchor for the DORA `lead_time_for_changes` metric (commit→deploy window) per [`dora-telemetry.md`](../standards/dora-telemetry.md) § 2.1. That metric is a window read-model computed on demand by [`compute-dora-metrics.sh`](../../tools/compute-dora-metrics.sh) over EXISTING events — the committer-date (`%cI`) of this stage's first commit and the Stage 12 `deployment-status/deploy-*` events. **No new emit-table row is added here** — the existing commit is the anchor; the read-model reads it, this stage writes no DORA event. This is the SIBLING window to the deployment-cycle-time GO→deploy window (which anchors on the Stage 9 GO event, not this commit).
