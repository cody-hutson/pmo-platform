...
3. **D-B citation rewrite (archived release only)** — in `release/releases/notes/` + `release/releases/plans/`, rewrite the release's inline `[...](../RELEASE_LOG.md#<anchor>)` → `[...](../logs/<keyslug>.md)` and frontmatter `links.log_anchor: "#<anchor>"` → `links.log_archive: "logs/<keyslug>.md"` per [`release-corpus-schema.md § Links shape`](../standards/release-corpus-schema.md). Active releases keep `links.log_anchor`. Release identity is the file's own frontmatter `version:` field (authoritative over filename). **Bulk/boundary case:** when a single sweep archives more than one release (an initial fold, or a multi-row reduction), the rewrite applies to EVERY newly-archived release — enumerate the set whose Deployment-Log block left the active head *this* sweep, never a hardcoded version range. (The v1.18 miss at the v2.03 fold, fixed in `8cf06f0`, was a hardcoded `v1.01–v1.17` acceptance-criteria enumeration that omitted the newest-archived row.) `deploy.sh` Check 36 (step 4) is the full-corpus backstop that catches any release left on the active-anchor form.
4. **Verify before commit** — `deploy.sh --check` Check 32 (f) active rows ≤ 11 / (g) every archived version row backed by `logs/<key>.md` / (h) 4 frontmatter keys present, Check 36 (citation-rewrite completeness — every archived release's note/plan carries the `log_archive` form, never the active-anchor `log_anchor:` / inline `../RELEASE_LOG.md#` form), and Check 23 (`generate_release_index.py --verify`) exit 0 (the active∪archive union backs every INDEX row). Byte-identity of the moved block is verifiable via `diff` against `git show <pre-sweep-SHA>:release/releases/RELEASE_LOG.md`. The canonical worked exemplar is **v1.08**.
