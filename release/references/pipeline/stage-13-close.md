...
- The `**Close-Class-Telemetry:**` H4 field appended in the Phase B chore PR ([`compute-close-class-telemetry.sh`](../../tools/compute-close-class-telemetry.sh), per [`close-class-telemetry.md`](../standards/close-class-telemetry.md)) sources from **mechanical surfaces** — the operator-instance retro/lessons registers, `gh` `status: deferred` state (carry-forward closure per [`deferred-item-tracking.md`](../standards/deferred-item-tracking.md)), the `**Outcome:**` field + Phase A7.1 rollup (Indicator 5 presence), the hub-spoke sub-task `gh` state (Indicator 6 phase-evidence-preservation read-model — CLOSED stage sub-tasks with a trusted-authored comment ÷ scaffolded), and the retained G-CL4 close-gate boolean — NOT the event log. Its Indicator 4 (cross-release pattern-emergence rate) is a POINTER to [`synthesize-release-learnings.sh --mode pattern-detect --emit rate`](../../tools/synthesize-release-learnings.sh), never recomputed in the field.
