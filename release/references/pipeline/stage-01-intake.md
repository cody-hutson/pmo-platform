...
Path B (human intake) and Path C (IMP bridge promotion) produce Proposal artifacts only. A Path B or Path C body authored by an account **outside** the trusted set (`OWNER` / `MEMBER` / `COLLABORATOR`) is untrusted third-party content; it is governed downstream by the author-association trust boundary at Stage-2 triage (**A0** — per [`release-process.md` § Inter-Stage Feedback Protocol](../../governance/release-process.md#inter-stage-feedback-protocol) and [`stage-02-triage.md` §5 A0](stage-02-triage.md)): its prose is never executed as instructions and its body-declared deps are never auto-mirrored into native state. Intake creates the issue — the two-sided posture is preserved (any account may open one via the templates); the trust **read** fires at triage, not here.
