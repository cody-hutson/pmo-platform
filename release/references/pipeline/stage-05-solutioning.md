...
**Phase A (Agent):** A1 design scope assessment, A2 architecture alignment check, A3 blast radius analysis via [`release/tools/blast-radius.sh`](../../tools/blast-radius.sh) per [`release/references/protocols/blast-radius-protocol.md`](../protocols/blast-radius-protocol.md) (transitive dependency mapping; manual `grep` deprecated for Stage 5), A4 design specification drafting (exact structure/naming/layout + Mode 3 implementability + Mode 4 debt flags + **A4.1 cascade-completeness sweep per § 5.6**), A5 ADR drafting as GitHub Issues with `adr` label.
...
**Reference: failure-mode-standard.md entry.** The `### Cascade-omission at count update — PROC` entry in [`core/specs/failure-mode-standard.md`](../../../core/specs/failure-mode-standard.md) § Reorg / structure-change examples codifies the anti-pattern this section prevents, with a prior release's DT findings as worked example (originating evidence: 2 Tier 1 [ADJUST] findings from one 20→19 cascade where adjacent occurrences were missed in already-touched files).
