...
**Phase A5 — the ADR-citation beat: a number the release has not won does not enter prose.** Phase A5 is where an ADR is first specified, and it is therefore the first moment a release is tempted to write a concrete ADR number into an artifact. **Do not.** Per [`ADR-181`](../../ADRs/ADR-181-adr-citations-bind-at-the-claim-not-at-authorship.md), an ADR number is allocated at authorship but binds only at merge, and it enters branch-authored prose only at the Stage-12 claim. The design names its record's **slug** and cites it everywhere as `{{ADR:<slug>}}` — in the design output, in the release plan, in spec amendments, in test comments. The literal number belongs to the record's own filename, title and frontmatter (which Engineering stamps at Commit 0 against the mainline anchor) and nowhere else; `renumber-adr.py --stamp` resolves every token from the on-disk filename as the last step of Stage-12 Phase A.5.7.

The reason is a measured cost, not a style preference. The number is provisional until merge, and the price of losing it is a function of how long the branch has been writing it down: a collision caught at Engineering Commit 0 costs a sweep of one file, and the same collision caught after Engineering has cost 98 occurrences across 14 files on a real release. Tokenizing the citations makes that price a rename plus an index update, which is exactly what tokenizing the version already does for the sibling identifier (ADR-092). **The token is prose-only** — a token inside a link target is parsed as a path and reported as a broken cross-reference by continuous integration before the stamp ever runs — so a *link* to the record uses its real path. *Cutover: applies to releases entering Stage 5 going forward; a design authored earlier carries literal citations by design and is historical record.*
...
**Phase A4.3 — Aggregation-rule trace (conditional).** A criterion, dimension, check or indicator decides nothing by itself: an **aggregation rule** — a scoring average, a routing table, an ANY/ALL verdict rule, a count threshold, or the exit-status handler of a consuming process — reduces it together with its siblings into the single outcome someone acts on. A design that specifies the member and never reads the rule can be internally consistent and still wrong about what the member does. When the design **changes the membership or the verdict-relevant properties of an aggregated evaluand set**, or **asserts of any evaluand that it is advisory / non-blocking / cannot gate**, the spoke MUST emit an `### Aggregation-Trace` block per **§ 5.8** — naming each consuming rule by `file:line` with its text quoted, and stating the effect **as the aggregate's behaviour** rather than as the member's own type field. Where the design claims a non-blocking property, that claim is discharged against the aggregation rule; the criterion's type field is not evidence about the aggregate. Neither trigger holding ⇒ no block is authored, and omission is the correct non-ceremony signal per `core/disciplines/decision-discipline.md` G2. Trigger table, block schema and load-bearing test: **§ 5.8**. Forcing function: § 7.2 **SR-G7**.
...
2. Cite those authoritative sources in the Stage 5 design spec, then mirror **one** of them into the `domain_practice` label: replace the label's `source: UNSOURCED-DOMAIN` value with a single **Form A** value, satisfy the companion obligation Form A carries, drop the Mode-B-only field it replaces, and update the `date` field. **What Form A admits is deliberately not restated here.** The closed `source:` value grammar is owned by the Stage 4 Planning spec's Domain-Best-Practice Sourcing-or-Flag Step — the same step this section already inherits its label schema from — and is the decision recorded in [ADR-147](../../ADRs/ADR-147-domain-practice-source-grammar-routes-it-does-not-extend.md). Read the form and its companion obligation there. A second copy in this file would be a second source of truth for a grammar that has already been re-scoped once.

   **Write a citation, not a description of the citations.** The `source:` value is *matched* against that closed grammar by the `_prov_source_form` resolver in [`verify-release-plan.sh`](../../tools/verify-release-plan.sh), which returns a named form or `NONE` and does not interpret the value as prose. A summary phrase — *"the cited source list"*, *"the sources above"*, or any similar description of the citations rather than one of them — therefore resolves to `NONE`, and the label reads non-conformant even when the design spec's citations are perfectly sound. Two consequences follow when a design cites more than one source. The label carries **one** Form A value and the design spec carries the rest: the label is a machine-read pointer to the provenance, not an inventory of it. And a comma-separated list is worse than non-conformant — the label body is comma-delimited, so the field extractor truncates the value at the first comma and the remaining citations are dropped **silently**, leaving a label that resolves to a named form while carrying less than its author wrote.
...
**Cascade visibility — why a cascade aimed at this label can miss this section.** The clause above answers whether § 5.7's own updates *trigger* a cascade. This one answers the inverse question, and it is the one that went wrong: whether a cascade *aimed at* the provenance label can find § 5.7 at all. It could not. When the closed three-form `source:` grammar was ratified, the cascade carrying it swept on a **count** axis — the phrase *"one of three legitimate forms"*, a three-to-four cardinality change — and reached the sibling surfaces that state the rule that way. § 5.7 states the rule in **instruction** shape (*replace the value with X*) and names no count, so a count-shaped sweep was structurally blind to it, and the upgrade-mechanism step above went on instructing a value the grammar does not admit for as long as nobody executed it. **The gap is one of shape, not of scope:** a sweep whose search axis is an enumeration's cardinality will not reach a surface that only ever tells an author what to write, however squarely that surface is in scope. So when the `source:` grammar or its form set changes again, sweep the **instruction** axis alongside the count axis — and start with the step above, which is this file's instance of it.
...
## 5.8 Aggregation-Rule Trace (Phase A4.3)

*Cutover: applies to releases entering Stage 5 strictly AFTER this protocol's introducing-release merge SHA recorded in `<OPERATOR_INSTANCE_RELEASE_LOG_PATH>`. **The introducing release itself is exempt** — it cannot fire its own new gate. All releases that entered Stage 5 prior to the introducing release are also exempt.*

A property asserted where a criterion is *defined* is not a property of the verdict. The verdict is computed by an **aggregation rule** — a scoring average, a routing table, an ANY/ALL verdict rule, a count threshold, or the exit-status handler of a consuming process — and a design that never reads that rule can be internally consistent and still wrong about what it just built. This section makes the rule part of the design.

**Trigger conditions (the rule fires when EITHER holds):**

| # | Trigger | The design… |
|---|---|---|
| **AT-1** | **Membership change** | introduces a new criterion, dimension, check, gate, indicator, or scan row into a set whose members are reduced by a rule to one outcome — or removes a member, or changes a member's verdict-relevant **type, status, weight, or exit code** |
| **AT-2** | **Non-blocking claim** | asserts of any **evaluand**, new or existing, that it is advisory / non-blocking / informational / report-only / "never gates" — however the claim is carried: a type field, an emitter class, an exit code, a status token, or prose |

**Trigger does NOT fire when (omission is the correct non-ceremony signal per `core/disciplines/decision-discipline.md` G2):**

- The design edits only the **content** of an existing member — its wording, its method, its examples — leaving type, status, weight and exit code unchanged, and makes no non-blocking claim.
- The evaluand **stands alone**: it has no sibling set, and no rule reduces its result together with anything else.
- The design **introduces no evaluand at all** — the ordinary case, and the large majority of Stage 5 outputs.

**What AT-2 is about, and what it is not.** AT-2 fires on a claim about an **evaluand** — a thing whose result some rule reduces together with others'. It does NOT fire on the word *advisory* used of a **document's binding status**: a Solutioning pre-read is declared "ADVISORY, not scope" because it is non-binding relative to the issue body (per `release/references/standards/solutioning-output-template.md` § 3.5), and nothing aggregates a pre-read into a verdict. A gate criterion declared advisory, or a check declared report-only, IS an evaluand claim and does fire. Read the trigger on the object of the claim, never on the adjective.

**Required `### Aggregation-Trace` block (one row per (evaluand × consuming rule) pair):**

```markdown
### Aggregation-Trace (per stage-05-solutioning.md § 5.8)

| Evaluand | Consuming aggregation rule (file:line) | Rule verbatim | Effect on the aggregate verdict |
|---|---|---|---|
| `<criterion / dimension / check / indicator id>` | `<path>:<line>` | `<quoted rule text>` | `<what the aggregate does now that this member exists or changed>` |

**Non-blocking claims asserted (AT-2):** <claim, quoted from this design> → **holds / does NOT hold** at `<path>:<line>`, because `<the rule's own words>`.
**Trace date:** `<YYYY-MM-DD>` at commit `<short SHA>`
```

**Authoring rules:**

1. **Locate the rule; do not name it from memory.** Each rule is cited by `file:line` with its text quoted. A rule that cannot be located is reported as located-nowhere, and the trace states that the aggregation behaviour is **UNVERIFIED** — never that it is fine. An unlocatable rule is a finding, not a pass.
2. **State the effect as the aggregate's behaviour.** "Type is judgment, automation is recommend, therefore advisory" is the assertion this section exists to disallow — it is a restatement of the member's own type field. The required form names what the rule now decides: *"the rule is `ANY-FAIL → NO-GO`, this dimension can return FAIL, therefore the aggregate can now return NO-GO where it previously could not."*
3. **Say when a member cannot reach the passing state.** A member that is structurally incapable of the rule's clean value makes the aggregate structurally incapable of its clean verdict. That consequence is stated, not left to be discovered.
4. **A process boundary does not end the trace.** Where an evaluand's state crosses processes, the **consuming** process's exit-status handler IS an aggregation rule and is traced as one. A non-zero exit chosen as an advisory signal is a non-blocking claim under AT-2, and it holds only where the consumer tolerates a non-zero exit.
5. **N/A contributes, and say what it contributes.** Where the trace shows the member is inert under some condition — an ALL-conjunction to which an N/A member contributes `true`, a denominator a structural criterion does not enter — that inertness is part of the effect statement, because it is what bounds the change to its trigger.

**Load-bearing test (the trace is incomplete if ANY hold):**

- The `### Aggregation-Trace` block is omitted while AT-1 or AT-2 holds.
- A consuming rule is named without a `file:line` citation, or without its text quoted.
- An effect is stated as the member's type, status or automation level rather than as the aggregate's behaviour.
- A non-blocking claim appears in the design with no corresponding **holds / does NOT hold** line against a located rule.
- A rule the spoke could not locate is recorded as absent-therefore-fine rather than as **UNVERIFIED**.

**Scope (narrow — by design):**

- **Evaluand-scope:** ONLY the evaluands this design introduces or whose verdict-relevant properties it changes, plus any evaluand the design makes a non-blocking claim about. NOT every criterion in the consuming set.
- **Rule-scope:** ONLY the rules that actually consume those evaluands. Tracing one hop is the obligation; a full transitive audit of every downstream consumer is not.

**Boundary — design-time here, emit-time elsewhere.** What a check owes its reader at **runtime** — the vocabulary of the state it emits, absence-versus-zero on a non-measuring status, and the structurally non-escalating emitter shape — is owned by `core/disciplines/review-discipline-principles.md` Section 8 and is not restated here. This section owns the **design-time** obligation only: locate the rule that will consume the evaluand, and state what that rule then decides. The two meet at exactly one point — a design asserting the non-escalation guarantee **discharges** that assertion here, against the consuming rule, rather than against the emitter's own type field.

**Composition with § 5.6 and R1.** § 5.6 sweeps OLD-VALUE occurrences after a state change; R1 Evidence-Grounding surveys the current state of a convention before canonicalizing it; § 5.8 traces an evaluand forward to the rule that decides. Three orthogonal disciplines on three different axes — a spec may carry all three blocks, and none replaces another. The `### Aggregation-Trace` block rides `### Detail` as a sibling of `#### Blast Radius`, the same delivery seam `### Cascade-Sweep` uses.

**Forcing function:** § 7.2 **SR-G7** confirms the block is present, its citations resolve, and its effect statements are aggregate-shaped.
...
| SR-G7 | **Aggregation-rule trace recorded** (SEPARATE trigger — the Phase A4.3 / § 5.8 predicate, NOT the SR-G1..SR-G4 structural-premise obligation). When the design changes the membership or verdict-relevant properties of an aggregated evaluand set, or asserts that an evaluand is advisory / non-blocking / cannot gate, the `### Aggregation-Trace` block per § 5.8 is present, cites each consuming rule by `file:line` with its text quoted, and states the effect **as the aggregate's behaviour** rather than as the member's own type. A non-blocking claim carried with no `holds / does NOT hold` line against a located rule is not design-complete; a rule the spoke could not locate is recorded UNVERIFIED, never absent-therefore-fine. No evaluand change and no non-blocking claim ⇒ trivially N/A (correct non-ceremony signal). | Inspect the Stage 5 output: when a § 5.8 trigger holds, confirm the block is present, each row carries a resolvable `file:line` + quoted rule text + an aggregate-behaviour effect statement, and every non-blocking claim carries its holds / does-NOT-hold line. Re-read one cited rule at the given line and confirm the quotation. Deterministic (block-presence + citation-resolvability check). | **Gate-blocking** — a design that asserts non-blocking posture with no traced rule returns to Solutioning; **introducing-release-exempt** |
...
**SR-G7 posture (gate-blocking, introducing-release-exempt).** SR-G7 blocks rather than warns, and the asymmetry with SR-G5 is deliberate rather than inherited. SR-G5 ships in shadow because its source is a `grep -F` sweep over **foreign** files that deliberately over-includes, so a raw hit is not yet a finding and a hard block would stop a legitimate merge on a reconcilable non-consumer. SR-G7 reads the **artifact under review**, and its AT-2 limb fires on a claim the design itself made — there is nothing to infer and nothing to over-match, so the shadow rationale does not transfer. The failure costs are asymmetric in the same direction: an SR-G5 false positive costs a disposition line, whereas an untraced non-blocking claim ships a criterion whose real authority nobody has read — a defect that is invisible until the aggregate returns a verdict the designer did not intend. The introducing release is exempt (it cannot fire its own new gate) and produces the trace voluntarily as the first worked example.

**Intra-family (SR-G7 sits outside the extend-vs-build fork).** SR-G1..SR-G4 and SR-G6 together express one extend-vs-build determination across two branches. **SR-G7 is orthogonal to that fork** — it does not ask what you built or whether you should have extended something instead; it asks, of whatever the design produced, **what the rule that consumes it now decides**. It stays within this engineering/architecture axis — an aggregation rule is a structure, and tracing a member to it is a structural-consequence review — so the 5-axis disambiguation above gains no 6th axis. It is also distinct from the probe-validity obligations in `core/disciplines/review-discipline-principles.md` Section 8, which govern what a check **emits at runtime**; SR-G7 governs what a **design establishes before the check exists**. See § 5.8's boundary paragraph for the seam.
