...
**Deprecated-path scan for new-file deliverables (Source: deprecated-path scan spec):** For each new file in the PR scope, execute a deterministic grep scan against the release's `Files Removed` list. The list is sourced from the release plan's explicit `Files Removed` section when present; otherwise derive via `git diff main..release/<milestone> --diff-filter=D --name-only --no-renames`. For each match, classify as Tier 1 (Engineering re-author via `fix(dt):` commit per [DT↔Engineering Iteration Loop Protocol](#dtengineering-iteration-loop-protocol)) by default. **Exception:** matches where the PR body explicitly documents the reference as "intentional documentary reference to deprecated path with paired new-path fallback" are downgraded to Note severity (no routing). Tier escalation to Tier 2 (Scope Change) when re-author requires design revision; Tier 3 (Plan Rejection) when the removed path was load-bearing on the new file's premise.

**Change classes the scan covers.** The derived list must name every path the release *retires*, and exactly two change classes retire one: a **deletion** (the file is gone) and a **relocation** (the file moved or was renamed, so the old path is gone). `--no-renames` is what makes the second class visible, and it is not optional. Git's rename detection is on by default and reclassifies a move as `R`, which `--diff-filter=D` does not select — so without the flag a release that relocated files derives an empty list and the scan reports clean without examining anything. The coverage that remains is also *heuristic* rather than deterministic: a move whose content changed enough to fall below git's rename-similarity threshold decomposes to `D`+`A` and **is** selected, so the unflagged selector catches some relocations and not others depending on how heavily the file was edited in transit. With `--no-renames` every move decomposes to `D`+`A` unconditionally and both classes are selected deterministically. The remaining classes — modification, addition, copy, and type-change — retire no path and are correctly outside the population. Do **not** substitute `--diff-filter=DR`: under `--name-only` a rename prints its **destination** path, so that variant scans new files for references to paths the release has just created.

**Denominator disclosure and the empty-population verdict.** The scan reports the size of the population it scanned, not merely what it found, and an empty population is never reported as a pass. Every verdict carries `denominator: <N>`. Four verdicts, evaluated in order:

| # | Condition | Verdict | Severity and handling |
|---|---|---|---|
| 1 | `git diff main..release/<milestone> --name-only` returns **zero** lines | `SCAN-UNUSABLE — empty range` | **Blocker.** A release PR always changes something, so an empty range means the invocation is wrong — a bad base ref, a wrong branch name, or an unfetched remote. Re-resolve the range and re-run; do not report a result from this run. |
| 2 | Range non-empty, derived list **empty**, and `git diff main..release/<milestone> --name-status --find-renames` returns **≥1 `R` row** | `SCAN-BROKEN — instrument disagreement` | **Blocker.** Rename-detected rows exist while the retired-path population is empty, which cannot occur once `--no-renames` is applied. The selector has regressed; repair the selector rather than filing findings from this run. |
| 3 | Range non-empty, derived list **empty**, zero `R` rows | `NOT-APPLICABLE — no paths retired (denominator 0)` | Note. The release retired nothing, so there was nothing to scan. Record it in these words; it is **not** `PASS` and must not be reported as one. |
| 4 | Derived list **non-empty** | `PASS` (zero matches) or `FINDINGS` (N matches) | Classify and route per the paragraph above. |

Row 2 is a standing regression tripwire on this check's own selector: it costs one command, it can never fire while the selector carries `--no-renames`, and it is the assertion that would have caught the blindness this rule exists to close. It does **not** subsume the selector. A *partially* blind selector still derives a non-empty list and passes row 2 while missing relocations, so the selector guards partial blindness and row 2 guards total blindness; both are required and neither is redundant.
...
| Suite selected but runner errors (infra) — including a sandbox-recipe resolution failure, observable as **zero units executed** | A8-INFRA | Warning | logged; operator / CI investigates (not an Engineering code fix) | `suite-fail` with `reason:runner-error` |
...
**P1 — Mergeability.** Read `gh pr view <PR> --json isDraft,mergeable,mergeStateStatus`. If `mergeable` is `CONFLICTING` OR `mergeStateStatus` is `DIRTY`, that is a **Blocker** and it is terminal for this block — do not go on to classify checks. A conflicting pull request has no computable merge ref, so the host dispatches none of the gates triggered on `pull_request`; the rollup collapses to whatever `push`-triggered checks remain, every one of them passes, and a reading that inspects only conclusions reports the release green. Classifying that as an unreadable measurement would name the symptom and hide the cause, which is why the conflict is read first and reported on its own terms. Name the remediation with the finding: resolve the conflict on the release branch, then re-run Phase A. This predicate mirrors the pre-merge conflict check at [`stage-12-execute.md`](stage-12-execute.md) Phase A.6.3 — same fields, same two values, same meaning — rather than authoring a second conflict predicate.

**P2 — Denominator floor and count stability.** Read the denominator first: the branch-protection required-context count for the pull request's **own base branch**, `gh api repos/{REPO}/branches/<baseRefName>/protection --jq '.required_status_checks.contexts | length'`. Resolve `<baseRefName>` from the pull request itself (`gh pr view <PR> --json baseRefName`) and never hardcode the default branch, or a stacked pull request is measured against a denominator that is not its own. Then read every required row with `gh pr checks <PR> --required --json name,state,bucket,link`; in this `--json` form the command exits **0** even when a required row is failing or pending, so branch on the parsed rows and never on the exit code. **The exit code carries three conditions, not two.** Besides an unresolvable PR and an authentication failure, `gh` exits **1** with non-JSON stdout — `no required checks reported on the '<branch>' branch` — when the required roster is **empty**, which is precisely the zero-row collapse this predicate exists to detect. Observed on this release's own pull request while its head was conflicted; the positive control on a healthy pull request returns exit 0 with the full JSON roster. A non-zero exit must therefore be classified by **reading stdout**, never assumed to be a failed read. The safety invariant holds either way — an unparseable read and an empty roster both enter at § 5.1 state 2, same severity — so the cost is **attribution, not safety**: without this clause the release is told *the read was unreadable* when the truth is *the population collapsed*.

**The count must be settled before it is compared, or the comparison manufactures its own alarm.** The required roster is *dispatch-dependent* — it is still being created for some seconds after a push, and a read taken inside that window is honest but incomplete. Required check-run creation on this release's own head was measured spanning **11 seconds**; an earlier release saw four honest reads of one pull request return **45, 46, 46 and 48**, one of them watching the roster grow 45 → 46 mid-read. Three rules follow, and each exists because its absence has already produced a wrong reading:

1. **Poll on `status`, never on conclusions.** Settled means *no check reports a status other than `COMPLETED`*. An incomplete check returns an **empty** conclusion, so a predicate that waits on `PENDING` / `IN_PROGRESS` / `QUEUED` matches nothing and exits immediately on a roster still in flight — that exact error has already produced a **false collapsed-denominator alarm** in production.
2. **Require the count stable across consecutive polls** before treating it as settled. A count still rising is **unsettled**, which is a different finding from a shortfall and must never be reported as one.
3. **Compare against the same head.** A settled count is comparable only to the required-context count read for that same head; a roster legitimately grows when a change wires a new CI suite, so a number carried from an earlier head is not a baseline.

Once settled, an observed required-row count **lower than the required-context count** is a **Blocker** and is terminal for this block — do not go on to classify. A zero-row read is a shortfall against the required-context count rather than a vacuously satisfied universal.

**P3 — § 5.1 state classification, over a population now known complete.** Classify the result to exactly one state per the six-state precedence table in [`release-readiness-scan-spec.md`](../specs/release-readiness-scan-spec.md) § 5.1, applying that section's settle allowance before its state 3 and its `isDraft`-only draft predicate. Stage 7 authors no new state name and no new precedence order.

**Why the floor is a predicate and not a state — the inversion this ordering repairs.** § 5.1 evaluates state 1 `checks-failing` **first**, and that state is **existential**: a single required row with a `bucket` of `fail` matches it. Stage 7 grades state 1 a **Note** and state 2 `checks-unreadable` a **Blocker**, which inverts § 5.1's own severity ordering. Route the count test through state 2 and the two facts combine into the failure this block exists to prevent: a *collapsed* population that still contains one red row matches state 1 before state 2 is ever reached, grades **Note**, and leaves `PASS` reachable — exactly the outcome the rationale below says must not be reachable. Reordering § 5.1 is not Stage 7's to do, so the floor runs as its own predicate ahead of classification instead. § 5.1's precedence is preserved verbatim, and it is only ever applied to a population already known to be complete — which is the condition that makes "state 1 → Note" sound in the first place.

| State | Stage 7 severity | Why this severity |
|---|---|---|
| **P1** conflict (`CONFLICTING` / `DIRTY`) | **Blocker** | The gates did not run at all, and the remediation is specific and fixable in scope |
| **P2** settled observed row count below the required-context count | **Blocker** | A collapsed population and a green one are indistinguishable to a reading that inspects only conclusions. Evaluated here, ahead of § 5.1, because the existential state 1 would otherwise match first and grade it a Note |
| **P2** count not yet stable across consecutive polls | Warning | Unsettled is not collapsed — reporting a still-dispatching roster as a shortfall is the false alarm this predicate exists to avoid |
| 1 `checks-failing` | Note | Branch protection independently blocks a red required check at the merge instant. Sound **only** because P2 has already established the population is complete |
| 2 `checks-unreadable` — a read that did not complete (command error, API error, rate limit, empty or unparseable output) | **Blocker** | An unreadable measurement is not a passing one |
| 3 `checks-unresolved` — every required row observed, at least one still not `pass` after the settle allowance | Warning | The population is intact; only the verdict is not yet final |
| 4 `draft-blocked` | clean | Draft is the specified state of the release pull request at this stage |
| 5 `clean` | clean | — |
| 6 `merge-blocked-other` | Warning, recording the observed merge-state value verbatim | An unmodelled host state is an honest partial, never a pass |

A Blocker raised here is an input to the Phase D verdict rule below (`FAIL (any blocker)`) and routes through the existing tiers in § Finding Classification with no new routing invented: a conflict on the release branch is a Blocker fixable within scope and goes to Engineering as Tier 1, while a renamed required context or a disabled workflow requires a scope decision and goes to the operator as Tier 2. A `Warning` is deliberately not an input to that verdict rule, which reads blockers and dimension thresholds only — and that asymmetry is precisely why P1, P2's shortfall arm and state 2 carry Blocker severity rather than Warning: at Warning severity a collapsed check population would leave `PASS` reachable, which is the failure this read exists to prevent. A `Note` is weaker still, which is why the shortfall test cannot be routed through a state that grades Note.

**Authoritative gate: branch protection — for state 1, and for state 1 only.** A `bucket` of `fail` on a required row routes as a Note and nothing stronger. Enforcement of a red required check belongs to branch protection on the default branch, which blocks the merge independently, so a second blocking path there would create a second place for one finding to stop a release with no added protection. That rationale is sound for a check that reported and unsound for one that did not, so it does not extend to P1, to P2, or to state 2 — and because state 1 is evaluated existentially and first, it holds only once P2 has confirmed the population is complete. Branch protection acts at the merge instant and can only block a check that produced a verdict; it cannot see a required population that never reported, and it cannot stop the pipeline from spending a Stage 7 wave and a Stage 8 wave rendering judgements against a pull request whose gates never ran. The protection those two outcomes add is therefore real and specific rather than duplicative, which is why they carry Blocker severity. Two scope notes for readers: the gates scan only lines ADDED since the PR base, so pre-existing references in untouched regions are correctly not flagged; and the release tracking surface is exempt by design, because issue and pull-request references are native provenance there.

**Negative control — the false green, in fixture form.** Putting a live pull request into conflict to observe the failure is a host mutation this specification does not perform, so the control ships as a fixture: the same two observations read by the old method and by the new one. Let `N` be the branch-protection required-context count that P2 reads; `N` was **9** when this control was last observed, on 2026-08-27.

| Observation | Old method — conclusions only, over whatever rows came back | New method — P1, then P2, then P3 (§ 5.1) |
|---|---|---|
| Healthy: `mergeable` is `MERGEABLE`, all `N` required rows returned, one of them `fail` | `checks-failing` — correct | `checks-failing` — **unchanged**; neither specificity arm fires |
| Conflicting: `mergeable` is `CONFLICTING`, **0 of `N`** required rows returned | *"all checks settled, none non-passing"* — **the false green** | **P1 Blocker** on the conflict; and had P1 not fired, **P2 Blocker** on 0 < `N`. The sensitivity arm fires on both paths |
| **Collapsed *and* red:** `mergeable` is `MERGEABLE`, only **3 of `N`** required rows returned, one of them `fail` | `checks-failing` → Note → `PASS` still reachable — **the second false green, and the one the predicate order used to produce** | **P2 Blocker** on 3 < `N`, before § 5.1 is consulted at all. This row is the discrimination the reordering buys: routed through state 2 it graded Note, because existential state 1 matched first |
| **Still dispatching:** `mergeable` is `MERGEABLE`, **7 of `N`** rows returned and the count rose 7 → 8 between consecutive polls | `checks-failing` or a **false collapsed-denominator alarm**, depending on the instant sampled | **Warning — unsettled**, not a shortfall. P2 re-polls on `status` until stable; only then is the count compared |

The conflicting row is grounded rather than hypothetical: every context in the required set is dispatched on `pull_request`, so a conflicting pull request returns none of them and the observed count is zero, not merely low. The two rows together are the discrimination this block owes — the old method separates the two observations not at all, and the new method separates them at the first predicate.
