...
**P2 — Denominator floor and count stability.** Read the denominator first: the branch-protection required-context count for the pull request's **own base branch**, `gh api repos/{REPO}/branches/<baseRefName>/protection --jq '.required_status_checks.contexts | length'`. Resolve `<baseRefName>` from the pull request itself (`gh pr view <PR> --json baseRefName`) and never hardcode the default branch, or a stacked pull request is measured against a denominator that is not its own. Then read every required row with `gh pr checks <PR> --required --json name,state,bucket,link`; in this `--json` form the command exits **0** even when a required row is failing or pending — the exit code signals only an unresolvable PR or an authentication failure — so branch on the parsed rows and never on the exit code.
