...
**P2 — Denominator-floored check classification.** Read the denominator first: the branch-protection required-context count for the pull request's **own base branch**, `gh api repos/{REPO}/branches/<baseRefName>/protection --jq '.required_status_checks.contexts | length'`. Resolve `<baseRefName>` from the pull request itself (`gh pr view <PR> --json baseRefName`) and never hardcode the default branch, or a stacked pull request is measured against a denominator that is not its own. Then read every required row with `gh pr checks <PR> --required --json name,state,bucket,link`; in this `--json` form the command exits **0** even when a required row is failing or pending — the exit code signals only an unresolvable PR or an authentication failure — so branch on the parsed rows and never on the exit code. Classify the result to exactly one state per the six-state precedence table in [`release-readiness-scan-spec.md`](../specs/release-readiness-scan-spec.md) § 5.1, applying that section's settle allowance before its state 3 and its `isDraft`-only draft predicate. Stage 7 authors no new state name and no new precedence order: an observed required-row count lower than the denominator is that section's state 2 `checks-unreadable`, and a zero-row read is a shortfall against the required-context count rather than a vacuously satisfied universal.
...
| 1 `checks-failing` | Note | Branch protection independently blocks a red required check at the merge instant |
| 2 `checks-unreadable` — observed rows fewer than the required-context count, or a read that did not complete | **Blocker** | A collapsed population and a green one are indistinguishable to a reading that inspects only conclusions |
...
| Observation | Old method — conclusions only, over whatever rows came back | New method — P1, then P2, then § 5.1 |
|---|---|---|
| Healthy: `mergeable` is `MERGEABLE`, all `N` required rows returned, one of them `fail` | `checks-failing` — correct | `checks-failing` — **unchanged**; the specificity arm does not fire |
| Conflicting: `mergeable` is `CONFLICTING`, **0 of `N`** required rows returned | *"all checks settled, none non-passing"* — **the false green** | **P1 Blocker** on the conflict; and had P1 not fired, state 2 `checks-unreadable` on 0 < `N`. The sensitivity arm fires on both paths |
